...
static char *fkey_bindings[NFKEYS] = { NULL, NULL, NULL, NULL, NULL, NULL,
    NULL, NULL, NULL, NULL, NULL, NULL
};

/* table of pseudo-function keys, to allow ^Q to double as Fkey1, etc */
#define    CTRL_OFFSET        0100    /* ctrl keys are 'X' - 0100 */
static char pfkeys[NFKEYS] = { 'Q' - CTRL_OFFSET, 'W' - CTRL_OFFSET, 'E' - CTRL_OFFSET, 'R' - CTRL_OFFSET,
...
static void print_usage_item(char *op, char *lop, char *help, int col_op, int col_lop, int col_help, int last_col);
static void print_help();
static void parse_cmdline(int argc, char **argv);
static void catcher(int signal);
static FILE *open_script(const char *filename);
static void do_bell();
static bool get_best_speed(const char *script_filename, const char *excersise_label, double *adjusted_cpm);
static void put_best_speed(const char *script_filename, const char *excersise_label, double adjusted_cpm);
...
void banner(const char *text)
{
    ...
    brand_length = utf8len(PACKAGE) + utf8len(VERSION) + 3, brand_position = COLS - brand_length, text_position = ((COLS - brand_length) > text_length) ? (COLS - brand_length - text_length) / 2 : 0;
...
static void display_speed(int total_chars, double elapsed_time, int errcount)
{
    ...
    if (cl_personal_best)
    {
        had_best_speed = get_best_speed(global_script_filename, __last_label, &best_cpm);
        new_best_speed = (!had_best_speed || adjusted_cpm > best_cpm) && !is_error_too_high(total_chars, errcount);
        if (new_best_speed)
            put_best_speed(global_script_filename, __last_label, adjusted_cpm);
    }

    /* adjust display position/height */
    line -= (had_best_speed ? 1 : 0) + (new_best_speed ? 1 : 0);
    ...
    move(line++, COLS - utf8len(message) - 1);
    ADDSTR_REV(message);
    sprintf(message, SPEED_PCT_ERROR, (double) 100.0 * (double) errcount / (double) total_chars);
    move(line++, COLS - utf8len(message) - 1);
    ADDSTR_REV(message);
    if (had_best_speed)
    {
...
static void do_drill(FILE * script, char *line)
{
    ...
    while (1)
    {
        ...
        for (chars_typed = 0, errors = 0, error_sync = 0, chars_in_the_line_typed = 0; *widep != ASCII_NULL; widep++)
        {
            do
            {
                rc = getch_fl(chars_in_the_line_typed >= COLS ? *(widep + 1) : (*widep == ASCII_TAB ? ASCII_TAB : ASCII_SPACE));
            }
            ...
            if (rc == *widep || (cl_wp_emu && rc == ASCII_SPACE && *widep == ASCII_NL))
            {
            ...
            else
            {
                ...
                {
                    wideaddch_rev(*widep == ASCII_NL ? DRILL_NL_ERR : (*widep == ASCII_TAB ? ASCII_TAB : (cl_rev_video_errors ? rc : DRILL_CH_ERR)));
                    chars_in_the_line_typed++;
                }
            ...
            if (cl_wp_emu)
            {
                if (rc == ASCII_SPACE)
                {
                    while (*(widep + 1) == ASCII_SPACE && *(widep + 1) != ASCII_NULL)
                    {
                        widep++;
                        wideaddch(*widep);
                        chars_in_the_line_typed++;
                ...
                {
                    while ((*(widep + 1) == ASCII_SPACE || *(widep + 1) == ASCII_NL) && *(widep + 1) != ASCII_NULL)
                    {
                        widep++;
                        wideaddch(*widep);
                        chars_in_the_line_typed++;
                ...
                else if (isalpha(*widep) && *(widep + 1) == ASCII_DASH && *(widep + 2) == ASCII_NL)
                {
                    widep++;
                    wideaddch(*widep);
                    widep++;
        ...
        if (rc != ASCII_ESC)
        {
            ...
            if (drill_type != C_DRILL_PRACTICE_ONLY && is_error_too_high(chars_typed, errors))
            {
...
static void do_speedtest(FILE * script, char *line)
{
    ...
    while (1)
    {
        ...
        for (chars_typed = 0, errors = 0, error_sync = 0; *widep != ASCII_NULL; widep++)
        {
        ...
        if (rc != ASCII_ESC)
        {
            ...
            if (drill_type != C_SPEEDTEST_PRACTICE_ONLY && is_error_too_high(chars_typed, errors))
            {
                sprintf(message, ERROR_TOO_HIGH_MSG, global_error_max);
...
static char do_query_repeat(FILE * script, bool allow_next)
{
    ...
    while (TRUE)
    {
        ...
        if (towideupper(resp) == 'R' || towideupper(resp) == RNE[0])
        {
            resp = 'R';
            break;
        }
        if (allow_next && (towideupper(resp) == 'N' || towideupper(resp) == RNE[2]))
        {
            resp = 'N';
            break;
        }
...
static bool do_query(FILE * script, char *line)
{
    ...
    while (TRUE)
    {
        ...
        {
            if (resp == pfkeys[fkey - 1] || (fkey < 11 && resp == (fkey + '0')) || (fkey == 10 && (resp == '0')) || (fkey == 11 && (resp == 'a' || resp == 'A')) || (fkey == 12 && (resp == 's' || resp == 'S')))
            {
                resp = KEY_F(fkey);
                break;
            }
        ...
        for (fkey = 1; fkey <= NFKEYS; fkey++)
        {
            if (resp == KEY_F(fkey) && fkey_bindings[fkey - 1] != NULL)
            {
                seek_label(script, fkey_bindings[fkey - 1], NULL);
                break;
            }
        }
        if (fkey <= NFKEYS)
        ...
        if (towideupper(resp) == QUERY_Y || towideupper(resp) == YN[0])
        {
            ret_code = TRUE;
            global_resp_flag = TRUE;
            break;
        }
        if (towideupper(resp) == QUERY_N || towideupper(resp) == YN[2])
        {
            ret_code = TRUE;
...
static void do_on_failure_label_set(FILE * script, char *line)
{
    ...
    line_iterator = line + strlen(line) - 1;
    while (line_iterator != line && !star && (isspace(*line_iterator) || *line_iterator == '*'))
    {
        if (*line_iterator == '*')
            star = TRUE;
        *line_iterator = '\0';
    ...
    else
    {
        ...
        for (global_on_failure_label = global_label_list[i]; global_on_failure_label != NULL; global_on_failure_label = global_on_failure_label->next)
        {
...
static void parse_file(FILE * script, char *label)
{
    ...
    while (!feof(script))
    {
        ...
        {
            case C_TUTORIAL:
                do_tutorial(script, line);
                break;
            case C_INSTRUCTION:
                do_instruction(script, line);
                break;
            case C_CLEAR:
                do_clear(script, line);
                break;
            case C_GOTO:
                do_goto(script, line, TRUE);
                break;
            case C_EXIT:
                do_exit(script);
                break;
            case C_QUERY:
                do_query(script, line);
                break;
            case C_YGOTO:
                do_goto(script, line, global_resp_flag);
                break;
            case C_NGOTO:
                do_goto(script, line, !global_resp_flag);
                break;
            ...
            case C_DRILL_PRACTICE_ONLY:
                do_drill(script, line);
                break;
            case C_SPEEDTEST:
            case C_SPEEDTEST_PRACTICE_ONLY:
                do_speedtest(script, line);
                break;
            case C_KEYBIND:
                do_keybind(script, line);
                break;
            ...
            case C_LABEL:
                ...
                get_script_line(script, line);
                break;
            case C_ERROR_MAX_SET:
                do_error_max_set(script, line);
                break;
            case C_ON_FAILURE_SET:
                do_on_failure_label_set(script, line);
                break;
            case C_MENU:
                do_menu(script, line);
                break;
...
static void print_usage_item(char *op, char *lop, char *help, int col_op, int col_lop, int col_help, int last_col)
{
    ...
    assert(0 <= col_op && col_op < col_lop && col_lop < col_help && col_help < last_col);
...
static void print_help()
{
    ...
    char *help[] = {
        ...
        _("set top banner colours (background, foreground, package and version " "respectively)"),
...
static void parse_cmdline(int argc, char **argv)
{
    ...
    while ((c = getopt_long(argc, argv, "be:ntf:c:sql:wkihvS", long_options, &option_index)) != -1)
    {
        switch (c)
        {
            case 'b':
            ...
            case 'e':
                ...
                if (sscanf(optarg, "%f", &cl_default_error_max) != 1 || cl_default_error_max < 0.0 || cl_default_error_max > 100.0)
                {
            ...
            case 't':
                ...
                break;
            case 'f':
                if (sscanf(optarg, "%d", &cl_curs_flash) != 1 || cl_curs_flash < 0 || cl_curs_flash > 512)
                {
                ...
                break;
            case 'c':
                if (sscanf(optarg, "%d,%d", &cl_fgcolour, &cl_bgcolour) != 2 || cl_fgcolour < 0 || cl_fgcolour >= NUM_COLOURS || cl_bgcolour < 0 || cl_bgcolour >= NUM_COLOURS)
                {
                ...
                break;
            case '\x01':
                if (sscanf(optarg, "%d,%d,%d,%d", &cl_banner_bg_colour, &cl_banner_fg_colour, &cl_prog_name_colour, &cl_prog_version_colour) != 4)
                {
                ...
                if (cl_banner_bg_colour < 0 || cl_banner_bg_colour >= NUM_COLOURS ||
                    cl_banner_fg_colour < 0 || cl_banner_bg_colour >= NUM_COLOURS || cl_prog_version_colour < 0 || cl_prog_version_colour >= NUM_COLOURS || cl_prog_name_colour < 0 || cl_prog_name_colour >= NUM_COLOURS)
                {
...
int main(int argc, char **argv)
{
...
#endif
    isUTF8Locale = strcasecmp(locale_encoding, "UTF-8") == 0 || strcasecmp(locale_encoding, "UTF8") == 0;
    ...
    {
        fprintf(stderr, "%s: i18n problem: invalid value for msgid \"Y/N\" (3 uppercase UTF-8 chars?): %ls\n", argv0, YN);
        exit(1);
    }
    ...
    if (wcslen(RNE) != 5 || !iswideupper(RNE[0]) || RNE[1] != '/' || !iswideupper(RNE[2]) || RNE[3] != '/' || !iswideupper(RNE[4]))
    {
        fprintf(stderr, "%s: i18n problem: invalid value for msgid \"R/N/E\" (5 uppercase UTF-8 chars?): %ls\n", argv0, RNE);
        exit(1);
    }
    ...
    if (argc - optind == 1)
    {
        ...
        {
            for (p = strtok(getenv("GTYPIST_PATH"), ":"); p != NULL; p = strtok(NULL, ":"))
            {
                strcpy(filepath, p);
                strcat(filepath, "/");
...
#    endif
    ...
    if (has_colors())
    {
        ...
        init_pair(C_NORMAL, colour_array[cl_fgcolour], colour_array[cl_bgcolour]);
        wbkgdset(stdscr, COLOR_PAIR(C_NORMAL));

        init_pair(C_BANNER, colour_array[cl_banner_fg_colour], colour_array[cl_banner_bg_colour]);
        init_pair(C_PROG_NAME, colour_array[cl_banner_fg_colour], colour_array[cl_prog_name_colour]);
        init_pair(C_PROG_VERSION, colour_array[cl_banner_fg_colour], colour_array[cl_prog_version_colour]);
        init_pair(C_MENU_TITLE, colour_array[cl_menu_title_colour], colour_array[cl_bgcolour]);
    }
...
bool get_best_speed(const char *script_filename, const char *excersise_label, double *adjusted_cpm)
{
    ...
    /* calculate filename */
    filename = (char *) malloc(strlen(getenv(global_home_env)) + strlen(BESTLOG_FILENAME) + 2);
    if (filename == NULL)
    {
        perror("malloc");
    ...
    while (fgets(line, FILENAME_MAX, blfile))
    {
        ...
        if (!strncmp(search, line + 19, search_len) && sscanf(line + 19 + search_len, "%lg", adjusted_cpm) == 1)
        {
            found = TRUE;
        }
    }
...
void put_best_speed(const char *script_filename, const char *excersise_label, double adjusted_cpm)
{
    FILE *blfile;                /* bestlog file */
    char *filename;                /* bestlog filename */
    char *fixed_script_filename;    /* fixed-up script filename */
    ...
    /* append new score */
    fprintf(blfile, "%04d-%02d-%02d %02d:%02d:%02d %s:%s %g\n", now->tm_year + 1900, now->tm_mon + 1, now->tm_mday, now->tm_hour, now->tm_min, now->tm_sec, fixed_script_filename, excersise_label, adjusted_cpm);
