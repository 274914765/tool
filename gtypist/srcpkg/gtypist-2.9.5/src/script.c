...
void build_label_index(FILE * script)
{
    ...
    while (!feof(script))
    {
        ...
        if (SCR_COMMAND(line) == C_LABEL)
        {
            ...
            new_label->label = malloc(strlen(SCR_DATA(line)) + 1);
            ...
            for (check_label = global_label_list[hash]; check_label != NULL; check_label = check_label->next)
            {
...
void get_script_line(FILE * script, char *line)
{
    ...
    global_line_counter++;
    while (!feof(script) && (line_is_empty(line) || SCR_COMMAND(line) == C_COMMENT || SCR_COMMAND(line) == C_ALT_COMMENT))
    {
        fgets(line, MAX_SCR_LINE, script);
        global_line_counter++;
    }
    ...
    if (!feof(script))
    {
        ...
        if (SCR_COMMAND(line) != C_LABEL && SCR_COMMAND(line) != C_GOTO && SCR_COMMAND(line) != C_YGOTO && SCR_COMMAND(line) != C_NGOTO && utf8len(SCR_DATA(line)) > COLS)
...
char *buffer_command(FILE * script, char *line)
{
    ...
    do
    {
        data = (char *) realloc(data, (data ? strlen(data) : 0) + strlen(SCR_DATA(line)) + strlen(STRING_NL) + 1);
        if (data == NULL)
            fatal_error(_("internal error: malloc"), line);
