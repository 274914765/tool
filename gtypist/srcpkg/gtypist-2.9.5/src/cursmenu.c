...
//  num_items = j;
    ...
    while (isspace(data[i]))
        i++;
    if (strncmp(data + i, "up=", 3) == 0 || strncmp(data + i, "UP=", 3) == 0)
    {                            /* I expect to see <up=LABEL> */
        ...
        while (data[i] != ' ')
            i++;
        data[i] = 0;
        if (strcmp(up, "_exit") == 0 || strcmp(up, "_EXIT") == 0)
            up = NULL;
    ...
    items_per_page = min(num_items, columns * min(MENU_HEIGHT_MAX, items_first_column));
    ...
    mvwideaddstr(LINES - 1, 0, _("Use arrowed keys to move around, " "SPACE or RETURN to select and ESCAPE to go back"));
    ...
    do
    {
        ...
        {
            /* write 1 column */
            for (j = 0; j < real_items_per_column && (idx = i * real_items_per_column + j + start_idx) <= end_idx; j++)
            {
                if (idx == cur_choice)
                ...
                mvwideaddstr(start_y + j, (i + 1) * spacing + i * max_width, descriptions[idx]);
