...
static void telrcv(struct connectdata *, unsigned char *inbuf,    /* Data received from socket */
...
static void printoption(struct SessionHandle *data, const char *direction, int cmd, int option)
{
    ...
    if (data->set.verbose)
    {
        ...
        {
            fmt = (cmd == CURL_WILL) ? "WILL" : (cmd == CURL_WONT) ? "WONT" : (cmd == CURL_DO) ? "DO" : (cmd == CURL_DONT) ? "DONT" : 0;
            if (fmt)
            {
...
static void printsub(struct SessionHandle *data, int direction,    /* '<' or '>' */
...
static void telrcv(struct connectdata *conn, unsigned char *inbuf,    /* Data received from socket */
                   ssize_t count)    /* Number of bytes received */
{
    unsigned char c;
    int in = 0;
...
#    ifdef USE_WINSOCK
    ...
    {
        failf(data, "failed to find WSACreateEvent function (%d)", GetLastError());
        FreeLibrary(wsock2);
        return CURLE_FAILED_INIT;
    }
    ...
    {
        failf(data, "failed to find WSACloseEvent function (%d)", GetLastError());
        FreeLibrary(wsock2);
        return CURLE_FAILED_INIT;
    }
    ...
    {
        failf(data, "failed to find WSAEventSelect function (%d)", GetLastError());
        FreeLibrary(wsock2);
        return CURLE_FAILED_INIT;
    }
    ...
    {
        failf(data, "failed to find WSAEnumNetworkEvents function (%d)", GetLastError());
        FreeLibrary(wsock2);
        return CURLE_FAILED_INIT;
    }
    ...
    while (keepon)
    {
        ...
        switch (waitret)
        {
            case WAIT_TIMEOUT:
                {
                    ...
                    while (1)
                    {
                        ...
                        if (!ReadFile(stdin_handle, buf, sizeof(data->state.buffer), &readfile_read, NULL))
                        {
                            keepon = FALSE;
                            break;
                        }
                        ...
                        while (nread--)
                        {
                            ...
                            Curl_write(conn, conn->sock[FIRSTSOCKET], outbuf, out_count, &bytes_written);
                        }
                    }
                }
                break;
            ...
            case WAIT_OBJECT_0:
                if (enum_netevents_func(sockfd, event_handle, &events) != SOCKET_ERROR)
                {
