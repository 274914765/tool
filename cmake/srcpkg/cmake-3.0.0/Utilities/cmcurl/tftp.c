...
static CURLcode tftp_send_first(tftp_state_data_t * state, tftp_event_t event)
{
    ...
    switch (event)
    {
        ...
        case TFTP_EVENT_TIMEOUT:    /* Resend the first packet out */
            ...
            snprintf((char *) &state->spacket.data[2], TFTP_BLOCKSIZE, "%s%c%s%c", filename, '\0', mode, '\0');
            sbytes = 4 + (int) strlen(filename) + (int) strlen(mode);
            sbytes = sendto(state->sockfd, (void *) &state->spacket, sbytes, 0, state->conn->ip_addr->ai_addr, state->conn->ip_addr->ai_addrlen);
            if (sbytes < 0)
            {
...
static CURLcode tftp_rx(tftp_state_data_t * state, tftp_event_t event)
{
    ...
    switch (event)
    {
        ...
        case TFTP_EVENT_TIMEOUT:
            ...
            else
            {
                ...
                sbytes = sendto(state->sockfd, (void *) &state->spacket, 4, SEND_4TH_ARG, (struct sockaddr *) &state->remote_addr, state->remote_addrlen);
    ...
    Curl_pgrsSetDownloadCounter(data, (curl_off_t) state->block * TFTP_BLOCKSIZE);
    return CURLE_OK;
}
        ...
        case TFTP_EVENT_ACK:
            ...
            if (rblock != state->block)
            {
                ...
                else
                {
                    ...
                    sbytes = sendto(state->sockfd, (void *) &state->spacket, 4 + state->sbytes, SEND_4TH_ARG, (struct sockaddr *) &state->remote_addr, state->remote_addrlen);
                    /* Check all sbytes were sent */
                    if (sbytes < 0)
                    {
                        failf(data, "%s\n", Curl_strerror(state->conn, Curl_sockerrno()));
            ...
            if (res)
                return res;
            sbytes = sendto(state->sockfd, (void *) state->spacket.data, 4 + state->sbytes, SEND_4TH_ARG, (struct sockaddr *) &state->remote_addr, state->remote_addrlen);
            /* Check all sbytes were sent */
            if (sbytes < 0)
            {
                failf(data, "%s\n", Curl_strerror(state->conn, Curl_sockerrno()));
...
CURLcode Curl_tftp_connect(struct connectdata * conn, bool * done)
{
    ...
    ((struct sockaddr *) &state->local_addr)->sa_family = conn->ip_addr->ai_family;
...
CURLcode Curl_tftp(struct connectdata * conn, bool * done)
{
    ...
    *done = TRUE;
    ...
    for (tftp_state_machine(state, TFTP_EVENT_INIT); state->state != TFTP_STATE_FIN; tftp_state_machine(state, event))
    {
        ...
        else
        {
            ...
            state->rbytes = recvfrom(state->sockfd, (void *) &state->rpacket, sizeof(state->rpacket), 0, (struct sockaddr *) &fromaddr, &fromlen);
            ...
            else
            {
                ...
                switch (event)
                {
                    case TFTP_EVENT_DATA:
                        ...
                        if (state->rbytes > 4 && ((state->block + 1) == getrpacketblock(&state->rpacket)))
                        {
                            code = Curl_client_write(conn, CLIENTWRITE_BODY, (char *) &state->rpacket.data[4], state->rbytes - 4);
                            if (code)
                                return code;
                        }
                        break;
