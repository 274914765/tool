...
static CURLcode Curl_output_basic(struct connectdata *conn, bool proxy)
{
    ...
    if (Curl_base64_encode(data, data->state.buffer, strlen(data->state.buffer), &authorization) > 0)
    {
        if (*userp)
            free(*userp);
...
static CURLcode perhapsrewind(struct connectdata *conn)
{
    ...
    if ((expectsend == -1) || (expectsend > bytessent))
    {
        ...
        {
            if (((expectsend - bytessent) < 2000) || (conn->ntlm.state != NTLMSTATE_NONE))
            {
            ...
            infof(data, "NTLM send, close instead of sending %" FORMAT_OFF_T " bytes\n", (curl_off_t) (expectsend - bytessent));
        }
...
CURLcode Curl_http_auth_act(struct connectdata * conn)
{
    ...
    if (conn->bits.user_passwd && ((data->reqdata.keep.httpcode == 401) || (conn->bits.authneg && data->reqdata.keep.httpcode < 300)))
    {
        pickhost = pickoneauth(&data->state.authhost);
        if (!pickhost)
            data->state.authproblem = TRUE;
    }
    if (conn->bits.proxy_user_passwd && ((data->reqdata.keep.httpcode == 407) || (conn->bits.authneg && data->reqdata.keep.httpcode < 300)))
    {
    ...
    else if ((data->reqdata.keep.httpcode < 300) && (!data->state.authhost.done) && conn->bits.authneg)
    {
    ...
    {
        failf(data, "The requested URL returned error: %d", data->reqdata.keep.httpcode);
 ...
 * @returns CURLcode
 */
static CURLcode Curl_http_output_auth(struct connectdata *conn, char *request, char *path, bool proxytunnel)    /* TRUE if this is the request setting
                                                                                                                   up the proxy tunnel */
{
...
#    endif
        ...
        {
            /* Basic */
            if (conn->bits.proxy_user_passwd && !checkheaders(data, "Proxy-authorization:"))
            {
                auth = (char *) "Basic";
...
#    ifndef CURL_DISABLE_CRYPTO_AUTH
        ...
        {
            auth = (char *) "Digest";
            result = Curl_output_digest(conn, TRUE,    /* proxy */
                                        (unsigned char *) request, (unsigned char *) path);
            if (result)
                return result;
        }
#    endif
    ...
    if (!data->state.this_is_a_follow || conn->bits.netrc || !data->state.first_host || curl_strequal(data->state.first_host, conn->host.name) || data->set.http_disable_hostname_check_before_authentication)
    {
        ...
        /* Send web authentication header if needed */
        {
            auth = NULL;
#    ifdef HAVE_GSSAPI
            if ((authhost->picked == CURLAUTH_GSSNEGOTIATE) && data->state.negotiate.context && !GSS_ERROR(data->state.negotiate.status))
            {
...
#    ifndef CURL_DISABLE_CRYPTO_AUTH
                ...
                {
                    auth = (char *) "Digest";
                    result = Curl_output_digest(conn, FALSE,    /* not a proxy */
                                                (unsigned char *) request, (unsigned char *) path);
                    if (result)
                        return result;
                }
...
CURLcode Curl_http_input_auth(struct connectdata * conn, int httpcode, char *header)    /* the first non-space */
{
    /*
...
#    ifdef HAVE_GSSAPI
    if (checkprefix("GSS-Negotiate", start) || checkprefix("Negotiate", start))
    {
    ...
    if (checkprefix("NTLM", start))
    {
        *availp |= CURLAUTH_NTLM;
        ...
        if (authp->picked == CURLAUTH_NTLM)
        {
            ...
            CURLntlm ntlm = Curl_input_ntlm(conn, (bool) (httpcode == 407), start);
...
int Curl_http_should_fail(struct connectdata *conn)
{
    ...
    if (data->reqdata.resume_from && (data->set.httpreq == HTTPREQ_GET) && (k->httpcode == 416))
    {
     ...
     ** a terminal error
     */
    if ((k->httpcode != 401) && (k->httpcode != 407))
        return 1;
 ...
 * Returns CURLcode
 */
static CURLcode add_buffer_send(send_buffer * in, struct connectdata *conn, long *bytes_written,    /* add the number of sent
...
#    endif                        /* CURL_DOES_CONVERSIONS */
    ...
    if (CURLE_OK == res)
    {
        ...
        if (conn->data->set.verbose)
        {
            ...
            Curl_debug(conn->data, CURLINFO_HEADER_OUT, ptr, (size_t) (amount - included_body_bytes), conn);
            if (included_body_bytes)
                Curl_debug(conn->data, CURLINFO_DATA_OUT, ptr + amount - included_body_bytes, (size_t) included_body_bytes, conn);
        }
...
CURLcode Curl_proxyCONNECT(struct connectdata * conn, int sockindex, char *hostname, int remote_port)
{
    ...
    ssize_t gotbytes;
    char *ptr;
    long timeout = data->set.timeout ? data->set.timeout : 3600;    /* in seconds */
    char *line_start;
    char *host_port;
    ...
    do
    {
        ...
        if (CURLE_OK == result)
        {
            ...
            if (CURLE_OK == result)
            {
                ...
                /* BLOCKING */
                result = add_bufferf(req_buffer, "CONNECT %s:%d HTTP/1.0\r\n" "%s"    /* Host: */
                                     "%s"    /* Proxy-Authorization */
                                     "%s"    /* User-Agent */
                                     "%s",    /* Proxy-Connection */
                                     hostname, remote_port, host, conn->allocptr.proxyuserpwd ? conn->allocptr.proxyuserpwd : "", useragent, proxyconn);
                ...
                if (CURLE_OK == result)
                    ...
                    result = add_buffer_send(req_buffer, conn, &data->info.request_size, 0, sockindex);
            }
            if (result)
                                ...
                                if (*ptr == '\n')
                                {
                                    ...
                                    if (('\r' == line_start[0]) || ('\n' == line_start[0]))
                                    {
                                        ...
                                        if (cl && (407 == k->httpcode) && !data->state.authproblem)
                                        {
                                             ...
                                             * response-body */
                                            keepon = 2;
                                            infof(data, "Ignore %" FORMAT_OFF_T " bytes of response-body\n", cl);
                                    ...
                                    {
                                        cl = curlx_strtoofft(line_start + strlen("Content-Length:"), NULL, 10);
                                    }
                                    else if (Curl_compareheader(line_start, "Connection:", "close"))
                                        closeConnection = TRUE;
                                    else if (2 == sscanf(line_start, "HTTP/1.%d %d", &subversion, &k->httpcode))
                                    {
...
CURLcode Curl_http_done(struct connectdata * conn, CURLcode status, bool premature)
{
    ...
    if (!conn->bits.retry && ((http->readbytecount + conn->headerbytecount - conn->deductheadercount)) <= 0)
    {
...
static CURLcode expect100(struct SessionHandle *data, send_buffer * req_buffer)
{
    ...
    data->state.expect100header = FALSE;    /* default to false unless it is set
                                               to TRUE below */
    if ((data->set.httpversion != CURL_HTTP_VERSION_1_0) && !checkheaders(data, "Expect:"))
    {
...
CURLcode Curl_http(struct connectdata * conn, bool * done)
{
    ...
    *done = TRUE;
    ...
    if ((conn->protocol & (PROT_HTTP | PROT_FTP)) && data->set.upload)
    {
        httpreq = HTTPREQ_PUT;
    }
    ...
    if (!checkheaders(data, "Accept-Encoding:") && data->set.encoding)
    {
    ...
    if (ptr)
    {
        ...
        conn->bits.upload_chunky = Curl_compareheader(ptr, "Transfer-Encoding:", "chunked");
    }
    else
    {
...
#    endif
    ...
    else
    {
        ...
        if (((conn->protocol & PROT_HTTPS) && (conn->remote_port == PORT_HTTPS)) || (!(conn->protocol & PROT_HTTPS) && (conn->remote_port == PORT_HTTP)))
    ...
    if (HTTPREQ_POST_FORM == httpreq)
    {
        ...
        result = Curl_getFormData(&http->sendit, data->set.httppost, checkheaders(data, "Content-Type:"), &http->postsize);
        if (CURLE_OK != result)
        {
    ...
    if (((HTTPREQ_POST == httpreq) || (HTTPREQ_POST_FORM == httpreq) || (HTTPREQ_PUT == httpreq)) && data->reqdata.resume_from)
    {
    /**********************************************************************
     ...
     *********************************************************************/
        ...
        if (data->reqdata.resume_from)
        {
            ...
            do
            {
                ...
                actuallyread = data->set.fread(data->state.buffer, 1, (size_t) readthisamountnow, data->set.in);

                passed += actuallyread;
                if (actuallyread != readthisamountnow)
                {
                    failf(data, "Could only read %" FORMAT_OFF_T " bytes from the input", passed);
    ...
    if (data->reqdata.use_range)
    {
         ...
         */
        if ((httpreq == HTTPREQ_GET) && !checkheaders(data, "Range:"))
        {
        ...
        else if ((httpreq != HTTPREQ_GET) && !checkheaders(data, "Content-Range:"))
        {

            if (data->reqdata.resume_from)
            {
                /* This is because "resume" was selected */
                curl_off_t total_expected_size = data->reqdata.resume_from + data->set.infilesize;
                conn->allocptr.rangeline = aprintf("Content-Range: bytes %s%" FORMAT_OFF_T "/%" FORMAT_OFF_T "\r\n", data->reqdata.range, total_expected_size - 1, total_expected_size);
            }
            else
            {
                /* Range was selected and then we just pass the incoming range and
                   append total size */
                conn->allocptr.rangeline = aprintf("Content-Range: bytes %s/%" FORMAT_OFF_T "\r\n", data->reqdata.range, data->set.infilesize);
            }
        }
    }

    {
        ...
        const char *httpstring = data->set.httpversion == CURL_HTTP_VERSION_1_0 ? "1.0" : "1.1";
        ...
        result = add_bufferf(req_buffer, "%s "    /* GET/HEAD/POST/PUT */
                             "%s HTTP/%s\r\n"    /* path + HTTP version */
                             "%s"    /* proxyuserpwd */
                             "%s"    /* userpwd */
                             "%s"    /* range */
                             ...
                             "%s"    /* Proxy-Connection */
                             "%s",    /* transfer-encoding */
                             request, ppath, httpstring, conn->allocptr.proxyuserpwd ? conn->allocptr.proxyuserpwd : "", conn->allocptr.userpwd ? conn->allocptr.userpwd : "", (data->reqdata.use_range && conn->allocptr.rangeline) ? conn->allocptr.rangeline : "", (data->set.useragent && *data->set.useragent && conn->allocptr.uagent) ? conn->allocptr.uagent : "", (conn->allocptr.host ? conn->allocptr.host : ""),    /* Host: host */
                             http->p_pragma ? http->p_pragma : "",
                             http->p_accept ? http->p_accept : "",
                             (data->set.encoding && *data->set.encoding && conn->allocptr.accept_encoding) ? conn->allocptr.accept_encoding : "", (data->change.referer && conn->allocptr.ref) ? conn->allocptr.ref : "" /* Referer: <data> */ ,
                             (conn->bits.httpproxy && !conn->bits.tunnel_proxy && !checkheaders(data, "Proxy-Connection:")) ? "Proxy-Connection: Keep-Alive\r\n" : "", te);
...
#    if !defined(CURL_DISABLE_COOKIES)
        if (data->cookies || addcookies)
        {
            ...
            if (data->cookies)
            {
                ...
                co = Curl_cookie_getlist(data->cookies, conn->allocptr.cookiehost ? conn->allocptr.cookiehost : host, data->reqdata.path, (bool) (conn->protocol & PROT_HTTPS ? TRUE : FALSE));
            ...
            if (co)
            {
                ...
                while (co)
                {
                    if (co->value)
                    {
                        ...
                        result = add_bufferf(req_buffer, "%s%s=%s", count ? "; " : "", co->name, co->value);
                        if (result)
                            break;
                        count++;
                    }
            ...
            if (addcookies && (CURLE_OK == result))
            {
                ...
                if (CURLE_OK == result)
                {
                    result = add_bufferf(req_buffer, "%s%s", count ? "; " : "", addcookies);
                    count++;
                }
            }
...
#    endif
            ...
            switch (data->set.timecondition)
            {
                ...
                default:
                    result = add_bufferf(req_buffer, "If-Modified-Since: %s\r\n", buf);
                    break;
                case CURL_TIMECOND_IFUNMODSINCE:
                    result = add_bufferf(req_buffer, "If-Unmodified-Since: %s\r\n", buf);
                    break;
                case CURL_TIMECOND_LASTMOD:
                    result = add_bufferf(req_buffer, "Last-Modified: %s\r\n", buf);
                    break;
            }
            if (result)
                return result;
        ...
        switch (httpreq)
        {
            ...
            case HTTPREQ_POST_FORM:
                if (!http->sendit || conn->bits.authneg)
                {
                    ...
                    result = add_buffer_send(req_buffer, conn, &data->info.request_size, 0, FIRSTSOCKET);
                    if (result)
                        failf(data, "Failed sending POST request");
                    else
                        /* setup variables for the upcoming transfer */
                        result = Curl_setup_transfer(conn, FIRSTSOCKET, -1, TRUE, &http->readbytecount, -1, NULL);
                    break;
                }
                ...
                if (!conn->bits.upload_chunky)
                {
                    ...
                    result = add_bufferf(req_buffer, "Content-Length: %" FORMAT_OFF_T "\r\n", http->postsize);
                    if (result)
                        return result;
                }
                ...
                if (result)
                    ...
                    /* setup variables for the upcoming transfer */
                    result = Curl_setup_transfer(conn, FIRSTSOCKET, -1, TRUE, &http->readbytecount, FIRSTSOCKET, &http->writebytecount);
...
#    endif                        /* CURL_DOES_CONVERSIONS */
            ...
            case HTTPREQ_PUT:    /* Let's PUT the data to the server! */
                ...
                if (result)
                    ...
                    result = Curl_setup_transfer(conn, FIRSTSOCKET, -1, TRUE, &http->readbytecount, postsize ? FIRSTSOCKET : -1, postsize ? &http->writebytecount : NULL);
                if (result)
                    return result;
                break;
            ...
            case HTTPREQ_POST:
                ...
                if (conn->bits.authneg)
                    ...
                    postsize = (data->set.postfieldsize != -1) ? data->set.postfieldsize : (data->set.postfields ? (curl_off_t) strlen(data->set.postfields) : 0);
                ...
                {
                    result = add_bufferf(req_buffer, "Content-Type: application/x-www-form-urlencoded\r\n");
                    if (result)
                        return result;
                }
                ...
                if (data->set.postfields)
                {
                    ...
                    if (!data->state.expect100header && (postsize < MAX_INITIAL_POST_SIZE))
                    {
                        ...
                        if (!conn->bits.upload_chunky)
                        {
                            ...
                            result = add_buffer(req_buffer, data->set.postfields, (size_t) postsize);
                        ...
                        else
                        {
                            ...
                            if (CURLE_OK == result)
                                result = add_buffer(req_buffer, data->set.postfields, (size_t) postsize);
                            if (CURLE_OK == result)
                                result = add_buffer(req_buffer, "\x0d\x0a\x30\x0d\x0a\x0d\x0a", 7);
                            /* CR  LF   0  CR  LF  CR  LF */
                ...
                /* issue the request */
                result = add_buffer_send(req_buffer, conn, &data->info.request_size, (size_t) included_body, FIRSTSOCKET);

                if (result)
                    failf(data, "Failed sending HTTP POST request");
                else
                    result = Curl_setup_transfer(conn, FIRSTSOCKET, -1, TRUE, &http->readbytecount, http->postdata ? FIRSTSOCKET : -1, http->postdata ? &http->writebytecount : NULL);
                break;
