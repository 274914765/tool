...
static CURLcode AllowServerConnect(struct connectdata *conn)
{
    ...
    long timeout = data->set.connecttimeout ? data->set.connecttimeout : (data->set.timeout ? data->set.timeout : 0);
...
static CURLcode ftp_readresp(curl_socket_t sockfd, struct connectdata *conn, int *ftpcode,    /* return the ftp-code if done */
        ...
        else
        {
            ...
            for (i = 0; i < gotbytes; ptr++, i++)
            {
                ...
                if (*ptr == '\n')
                {
                    ...
                    if (data->set.verbose)
                        Curl_debug(data, CURLINFO_HEADER_IN, ftpc->linestart_resp, (size_t) perline, conn);
                     ...
                     * headers.
                     */
                    result = Curl_client_write(conn, CLIENTWRITE_HEADER, ftpc->linestart_resp, perline);
                    if (result)
                        return result;
    ...
    *nreadp = 0;
    ...
    while ((*nreadp < BUFSIZE) && (keepon && !result))
    {
        ...
        if (data->set.ftp_response_timeout)
            ...
            timeout = ftpc->response_time - Curl_tvdiff(Curl_tvnow(), now) / 1000;    /* spent time */
        ...
        if (!ftpc->cache)
        {
            ...
            switch (Curl_select(sockfd, CURL_SOCKET_BAD, interval_ms))
            {
                case -1:        /* select() error, stop reading */
                    result = CURLE_RECV_ERROR;
                    failf(data, "FTP response aborted due to select() error: %d", Curl_sockerrno());
                    break;
                case 0:        /* timeout */
                ...
                *nreadp += gotbytes;
                for (i = 0; i < gotbytes; ptr++, i++)
                {
                    ...
                    if (*ptr == '\n')
                    {
                         ...
                         * headers.
                         */
                        result = Curl_client_write(conn, CLIENTWRITE_HEADER, line_start, perline);
                        if (result)
                            return result;
...
#    ifdef ENABLE_IPV6
   ...
   */
    ...
    if (!host)
    {
        ...
        {
            failf(data, "getsockname() failed: %s", Curl_strerror(conn, Curl_sockerrno()));
            return CURLE_FTP_PORT_FAILED;
        }

        if (sslen > (socklen_t) sizeof(ss))
            sslen = sizeof(ss);
        rc = getnameinfo((struct sockaddr *) &ss, sslen, hbuf, sizeof(hbuf), NULL, 0, NIFLAGS);
        if (rc)
        {
    ...
    if (bind(portsock, ai->ai_addr, ai->ai_addrlen))
    {
        ...
        sslen = sizeof(ss);
        if (getsockname(conn->sock[FIRSTSOCKET], (struct sockaddr *) sa, &sslen))
        {
            failf(data, "getsockname() failed: %s", Curl_strerror(conn, Curl_sockerrno()));
            sclose(portsock);
            return CURLE_FTP_PORT_FAILED;
...
#        endif
    ...
    for (; fcmd != DONE; fcmd++)
    {
        ...
        if (EPRT == fcmd)
        {
            ...
            result = Curl_nbftpsendf(conn, "%s |%d|%s|%d|", mode[fcmd], ai->ai_family == AF_INET ? 1 : 2, myhost, port);
            if (result)
                return result;
            break;
        }
...
#    else
   ...
   */
    ...
    if (addr || sa_filled_in)
    {
        ...
        if (CURL_SOCKET_BAD != portsock)
        {
            ...
            if (bind(portsock, (struct sockaddr *) &sa, sslen) == 0)
            {
                ...
                if (getsockname(portsock, (struct sockaddr *) &add, &socksize))
                {
                    failf(data, "getsockname() failed: %s", Curl_strerror(conn, Curl_sockerrno()));
                    return CURLE_FTP_PORT_FAILED;
                }
    ...
    if (sa_filled_in)
        Curl_inet_ntop(AF_INET, &((struct sockaddr_in *) &sa)->sin_addr, myhost, sizeof(myhost));
    else
        Curl_printable_address(addr, myhost, sizeof(myhost));

    if (4 == sscanf(myhost, "%hu.%hu.%hu.%hu", &ip[0], &ip[1], &ip[2], &ip[3]))
    {

        infof(data, "Telling server to connect to %d.%d.%d.%d:%d\n", ip[0], ip[1], ip[2], ip[3], porttouse);

        result = Curl_nbftpsendf(conn, "PORT %d,%d,%d,%d,%d,%d", ip[0], ip[1], ip[2], ip[3], porttouse >> 8, porttouse & 255);
        if (result)
            return result;
    }
    else
...
static CURLcode ftp_state_post_listtype(struct connectdata *conn)
{
    ...
    NBFTPSENDF(conn, "%s", data->set.customrequest ? data->set.customrequest : (data->set.ftp_list_only ? "NLST" : "LIST"));
...
static CURLcode ftp_state_post_mdtm(struct connectdata *conn)
{
    ...
    /* If we have selected NOBODY and HEADER, it means that we only want file
       ...
       date. */
    if (conn->bits.no_body && data->set.include_header && ftp->file && ftp_need_type(conn, data->set.prefer_ascii))
    {
...
static CURLcode ftp_state_ul_setup(struct connectdata *conn, bool sizechecked)
{
    ...
    if ((data->reqdata.resume_from && !sizechecked) || ((data->reqdata.resume_from > 0) && sizechecked))
    {
        ...
        do
        {
            ...
            actuallyread = (curl_off_t) conn->fread(data->state.buffer, 1, (size_t) readthisamountnow, conn->fread_in);
    ...
    NBFTPSENDF(conn, data->set.ftp_append ? "APPE %s" : "STOR %s", ftp->file);
...
#    define NEWHOST_BUFSIZE 48
    ...
    else if ((ftpc->count1 == 1) && (ftpcode == 227))
    {
        ...
        while (*str)
        {
            if (6 == sscanf(str, "%d,%d,%d,%d,%d,%d", &ip[0], &ip[1], &ip[2], &ip[3], &port[0], &port[1]))
                break;
            str++;
        }
        ...
        if (data->set.ftp_skip_ip)
        {
            ...
            infof(data, "Skips %d.%d.%d.%d for data connection, uses %s instead\n", ip[0], ip[1], ip[2], ip[3], conn->ip_addr_str);
            if (conn->bits.tunnel_proxy)
                /* proxy tunnel -> use other host info because ip_addr_str is the
                   proxy address not the ftp host */
                snprintf(newhost, sizeof(newhost), "%s", conn->host.name);
            ...
            snprintf(newhost, sizeof(newhost), "%d.%d.%d.%d", ip[0], ip[1], ip[2], ip[3]);
    ...
    result = Curl_connecthost(conn, addr, &conn->sock[SECONDARYSOCKET], &conninfo, &connected);

    Curl_resolv_unlock(data, addr);    /* we're done using this address */

    if (result && ftpc->count1 == 0 && ftpcode == 229)
    {
        infof(data, "got positive EPSV response, but can't connect. " "Disabling EPSV\n");
        /* disable it for next transfer */
        conn->bits.ftp_use_epsv = FALSE;
        ...
        case 213:
            {
                ...
                char *buf = data->state.buffer;
                if (6 == sscanf(buf + 4, "%04d%02d%02d%02d%02d%02d", &year, &month, &day, &hour, &minute, &second))
                {
                    /* we have a time, reformat it */
                    time_t secs = time(NULL);
                    /* using the good old yacc/bison yuck */
                    snprintf(buf, sizeof(conn->data->state.buffer), "%04d%02d%02d %02d:%02d:%02d GMT", year, month, day, hour, minute, second);
                ...
                if (conn->bits.no_body && data->set.include_header && ftp->file && data->set.get_filetime && (data->info.filetime >= 0))
                {
                    struct tm *tm;
    ...
    if (data->reqdata.resume_from)
    {
        ...
        else
        {
            ...
            if (data->reqdata.resume_from < 0)
            {
                ...
                {
                    failf(data, "Offset (%" FORMAT_OFF_T ") was beyond file size (%" FORMAT_OFF_T ")", data->reqdata.resume_from, filesize);
...
static CURLcode ftp_state_size_resp(struct connectdata *conn, int ftpcode, ftpstate instate)
{
    ...
    if (instate == FTP_SIZE)
    {
        if (-1 != filesize)
        {
            snprintf(buf, sizeof(data->state.buffer), "Content-Length: %" FORMAT_OFF_T "\r\n", filesize);
            result = Curl_client_write(conn, CLIENTWRITE_BOTH, buf, 0);
            if (result)
                return result;
        }
...
static CURLcode ftp_state_get_resp(struct connectdata *conn, int ftpcode, ftpstate instate)
{
    ...
    if ((ftpcode == 150) || (ftpcode == 125))
    {
        ...
        /* FTP download: */
        result = Curl_setup_transfer(conn, SECONDARYSOCKET, size, FALSE, ftp->bytecountp, -1, NULL);    /* no upload here */
        if (result)
            return result;
...
static CURLcode ftp_state_user_resp(struct connectdata *conn, int ftpcode, ftpstate instate)
{
    ...
    else
    {
        ...
        if (conn->data->set.ftp_alternative_to_user && !conn->data->state.ftp_trying_alternative)
        {
...
static CURLcode ftp_statemach_act(struct connectdata *conn)
{
    ...
    if (ftpc->sendleft)
    {
        ...
        ssize_t written;
        result = Curl_write(conn, sock, ftpc->sendthis + ftpc->sendsize - ftpc->sendleft, ftpc->sendleft, &written);
        if (result)
            return result;
...
#    endif
            ...
            case FTP_PBSZ:
                ...
                if (!conn->ssl[SECONDARYSOCKET].use)
                {
                    NBFTPSENDF(conn, "PROT %c", data->set.ftp_ssl == CURLFTPSSL_CONTROL ? 'C' : 'P');
                    state(conn, FTP_PROT);
                }
                else
                {
            ...
            case FTP_PROT:
                if (ftpcode / 100 == 2)
                    ...
                    conn->ssl[SECONDARYSOCKET].use = (bool) (data->set.ftp_ssl != CURLFTPSSL_CONTROL);
...
static long ftp_state_timeout(struct connectdata *conn)
{
    ...
    if (data->set.ftp_response_timeout)
        ...
        timeout_ms = ftpc->response_time * 1000 - Curl_tvdiff(Curl_tvnow(), ftpc->response);    /* spent time */
 ...
 */
CURLcode Curl_ftp_connect(struct connectdata * conn, bool * done)    /* see description above */
{
    CURLcode result;
#    ifndef CURL_DISABLE_HTTP
...
#    endif
    ...
    {
        if ((-1 != data->set.infilesize) && (data->set.infilesize != *ftp->bytecountp) && !data->set.crlf && !ftp->no_transfer)
        {
            failf(data, "Uploaded unaligned file size (%" FORMAT_OFF_T " out of %" FORMAT_OFF_T " bytes)", *ftp->bytecountp, data->set.infilesize);
            result = CURLE_PARTIAL_FILE;
        }
    }
    else
...
#    endif                        /* CURL_DO_LINEEND_CONV */
        ...
        {
            failf(data, "Received only partial file: %" FORMAT_OFF_T " bytes", *ftp->bytecountp);
            result = CURLE_PARTIAL_FILE;
        }
        else if (!ftpc->dont_check && !*ftp->bytecountp && (k->size > 0))
        {
...
static CURLcode ftp_range(struct connectdata *conn)
{
    ...
    if (data->reqdata.use_range && data->reqdata.range)
    {
        ...
        else
        {
            ...
            data->reqdata.resume_from = from;
            DEBUGF(infof(conn->data, "FTP RANGE from %" FORMAT_OFF_T " getting %" FORMAT_OFF_T " bytes\n", from, data->reqdata.maxdownload));
        }
        DEBUGF(infof(conn->data, "range-download from %" FORMAT_OFF_T " to %" FORMAT_OFF_T ", totally %" FORMAT_OFF_T " bytes\n", from, to, data->reqdata.maxdownload));
...
static CURLcode ftp_parse_url_path(struct connectdata *conn)
{
    ...
    switch (data->set.ftp_filemethod)
    {
        ...
        case FTPFILE_SINGLECWD:
            ...
            if (slash_pos || !cur_pos || !*cur_pos)
            {
                ...
                ftpc->dirs[0] = curl_easy_unescape(conn->data, slash_pos ? cur_pos : "/", slash_pos ? (int) (slash_pos - cur_pos) : 1, NULL);
                if (!ftpc->dirs[0])
                {
                    free(ftpc->dirs);
                    return CURLE_OUT_OF_MEMORY;
        ...
        case FTPFILE_MULTICWD:
            ...
            while ((slash_pos = strchr(cur_pos, '/')) != NULL)
            {
                ...
                bool absolute_dir = (bool) ((cur_pos - data->reqdata.path > 0) && (ftpc->dirdepth == 0));
    ...
    if (data->set.upload && !ftp->file && (!ftp->no_transfer || conn->bits.no_body))
    {
    ...
    if (ftpc->prevpath)
    {
        ...
        if ((dlen == strlen(ftpc->prevpath)) && curl_strnequal(path, ftpc->prevpath, dlen))
        {
