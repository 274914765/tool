...
static bool init_resolve_thread(struct connectdata *conn, const char *hostname, int port, const Curl_addrinfo * hints);
...
static void dump_addrinfo(struct connectdata *conn, const struct addrinfo *ai)
{
    ...
    for (; ai; ai = ai->ai_next)
    {
        ...
        trace_it("    fam %2d, CNAME %s, ", ai->ai_family, ai->ai_canonname ? ai->ai_canonname : "<none>");
...
static BOOL init_thread_sync_data(struct thread_data *td, char *hostname, struct thread_sync_data *tsd)
{
    HANDLE curr_proc = GetCurrentProcess();

    memset(tsd, 0, sizeof(*tsd));
    if (!DuplicateHandle(curr_proc, td->mutex_waiting, curr_proc, &tsd->mutex_waiting, 0, FALSE, DUPLICATE_SAME_ACCESS))
    {
        /* failed to duplicate the mutex, no point in continuing */
        destroy_thread_sync_data(tsd);
        return FALSE;
    }
    if (!DuplicateHandle(curr_proc, td->mutex_terminate, curr_proc, &tsd->mutex_terminate, 0, FALSE, DUPLICATE_SAME_ACCESS))
    {
        /* failed to duplicate the mutex, no point in continuing */
        destroy_thread_sync_data(tsd);
        return FALSE;
    }
    if (!DuplicateHandle(curr_proc, td->event_terminate, curr_proc, &tsd->event_terminate, 0, FALSE, DUPLICATE_SAME_ACCESS))
    {
...
static BOOL acquire_thread_sync(struct thread_sync_data *tsd)
{
...
static void release_thread_sync(struct thread_sync_data *tsd)
{
...
static unsigned __stdcall gethostbyname_thread(void *arg)
{
    ...
    if (acquire_thread_sync(&tsd))
    {
        ...
        TRACE(("Winsock-error %d, addr %s\n", conn->async.status, he ? inet_ntoa(*(struct in_addr *) he->h_addr) : "unknown"));
...
#    else
    td->thread_hnd = (HANDLE) _beginthreadex(NULL, 0, THREAD_FUNC, conn, 0, &td->thread_id);
#    endif
...
#    endif
    ...
    if (WaitForMultipleObjects(sizeof(thread_and_event) / sizeof(thread_and_event[0]), (const HANDLE *) thread_and_event, FALSE, INFINITE) == WAIT_FAILED)
    {
...
CURLcode Curl_wait_for_resolv(struct connectdata * conn, struct Curl_dns_entry ** entry)
{
    ...
    timeout = conn->data->set.connecttimeout ? conn->data->set.connecttimeout : conn->data->set.timeout ? conn->data->set.timeout : CURL_TIMEOUT_RESOLVE;    /* default name resolve timeout */
    ticks = GetTickCount();
    ...
    if (status == WAIT_OBJECT_0)
    {
        ...
        else
        {
            ...
            TRACE(("%s() status %lu, thread retval %lu, ", THREAD_NAME, status, td->thread_status));
        }
    }
    else
    {
    ...
    if (!conn->async.dns)
    {
        ...
        else if (conn->async.done)
        {
            ...
            {
                failf(data, "Could not resolve proxy: %s; %s", conn->proxy.dispname, Curl_strerror(conn, conn->async.status));
                rc = CURLE_COULDNT_RESOLVE_PROXY;
            }
            else
            {
                failf(data, "Could not resolve host: %s; %s", conn->host.name, Curl_strerror(conn, conn->async.status));
 ...
 */
CURLcode Curl_is_resolved(struct connectdata * conn, struct Curl_dns_entry ** entry)
{
    *entry = NULL;
...
int Curl_resolv_getsock(struct connectdata *conn, curl_socket_t * socks, int numsocks)
{
    const struct thread_data *td = (const struct thread_data *) conn->async.os_specific;
