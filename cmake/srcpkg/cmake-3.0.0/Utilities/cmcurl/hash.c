...
static unsigned long hash_str(const char *key, size_t key_length)
{
...
int Curl_hash_init(struct curl_hash *h, int slots, curl_hash_dtor dtor)
{
    int i;
...
struct curl_hash *Curl_hash_alloc(int slots, curl_hash_dtor dtor)
{
    struct curl_hash *h;
...
static int hash_key_compare(char *key1, size_t key1_len, char *key2, size_t key2_len)
{
    if (key1_len == key2_len && *key1 == *key2 && memcmp(key1, key2, key1_len) == 0)
    {
        return 1;
    }
...
static struct curl_hash_element *mk_hash_element(char *key, size_t key_len, const void *p)
{
...
void *Curl_hash_add(struct curl_hash *h, char *key, size_t key_len, void *p)
{
...
void *Curl_hash_pick(struct curl_hash *h, char *key, size_t key_len)
{
    struct curl_llist_element *le;
    struct curl_hash_element *he;
    struct curl_llist *l = FETCH_LIST(h, key, key_len);
