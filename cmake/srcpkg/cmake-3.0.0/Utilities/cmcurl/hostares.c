...
int Curl_resolv_getsock(struct connectdata *conn, curl_socket_t * socks, int numsocks)
{
    ...
    Curl_expire(conn->data, (timeout.tv_sec * 1000) + (timeout.tv_usec / 1000));
...
CURLcode Curl_is_resolved(struct connectdata * conn, struct Curl_dns_entry ** dns)
{
    ...
    (void) select(nfds, &read_fds, &write_fds, NULL, (struct timeval *) &tv);
    ...
    *dns = NULL;
    ...
    if (conn->async.done)
    {
        ...
        if (!conn->async.dns)
        {
            failf(data, "Could not resolve host: %s (%s)", conn->host.dispname, ares_strerror(conn->async.status));
...
Curl_addrinfo *Curl_getaddrinfo(struct connectdata * conn, const char *hostname, int port, int *waitp)
{
    ...
    *waitp = FALSE;
    ...
    if (bufp)
    {
        ...
        ares_gethostbyname(data->state.areschannel, hostname, PF_INET, (ares_host_callback) Curl_addrinfo4_callback, conn);
