...
static curl_socket_t singleipconnect(struct connectdata *conn, const Curl_addrinfo * ai,    /* start connecting to this */
                                     long timeout_ms, bool * connected);
...
static int waitconnect(curl_socket_t sockfd,    /* socket */
                       long timeout_msec)
{
    int rc;
#ifdef mpeix
...
static CURLcode bindlocal(struct connectdata *conn, curl_socket_t sockfd)
{
   ...
   *************************************************************/
    if (data->set.device && (strlen(data->set.device) < 255))
    {
        ...
        if ((in == CURL_INADDR_NONE) && Curl_if2ip(data->set.device, myhost, sizeof(myhost)))
        {
            /*
        ...
        if (!was_iface)
        {
            ...
            if (h)
            {
                if (in == CURL_INADDR_NONE)
                    ...
                    Curl_inet_ntop(h->addr->ai_addr->sa_family, &((struct sockaddr_in *) h->addr->ai_addr)->sin_addr, myhost, sizeof myhost);
                else
        ...
        if (was_iface)
        {
             ...
             */
            if (setsockopt(sockfd, SOL_SOCKET, SO_BINDTODEVICE, data->set.device, strlen(data->set.device) + 1) != 0)
            {
...
#    endif
    ...
    if (-1 == getsockopt(sockfd, SOL_SOCKET, SO_ERROR, (void *) &err, &errSize))
        err = Curl_sockerrno();
    ...
    *connected = FALSE;            /* a very negative world view is best */
    ...
    else if (WAITCONN_TIMEOUT != rc)
    {
        ...
        if (trynextip(conn, sockindex, connected))
        {
            error = Curl_sockerrno();
            data->state.os_errno = error;
            failf(data, "Failed connect to %s:%d; %s", conn->host.name, conn->port, Curl_strerror(conn, error));
            code = CURLE_COULDNT_CONNECT;
        }
    }
    /*
...
#    endif
    ...
    if (setsockopt(sockfd, proto, TCP_NODELAY, (void *) &onoff, sizeof(onoff)) < 0)
...
static curl_socket_t singleipconnect(struct connectdata *conn, const Curl_addrinfo * ai, long timeout_ms, bool * connected)
{
    ...
    *connected = FALSE;            /* default is not connected */
    ...
    if (-1 == rc)
    {
        ...
        switch (error)
        {
            ...
            default:
                ...
                failf(data, "Failed to connect to %s: %s", addr_buf, Curl_strerror(conn, error));
                data->state.os_errno = error;
                break;
        }
    }
    ...
    if ((WAITCONN_TIMEOUT == rc) && (data->state.used_interface == Curl_if_multi))
    {
...
#    define min(a, b)   ((a) < (b) ? (a) : (b))
     ...
     */
    for (curr_addr = ai, aliasindex = 0; curr_addr; curr_addr = curr_addr->ai_next, aliasindex++)
    {
