...
#    define DLOPEN_MODE   RTLD_LAZY
                                /*! assume all dlopen() implementations have
                                   this */

#    if defined(RTLD_LAZY_GLOBAL)
                                /* It turns out some systems use this: */
#        undef  DLOPEN_MODE
...
#    endif
    ...
    int (__cdecl * ldap_search_s) (void *, char *, int, char *, char **, int, void **);
...
#    endif
    DYNA_GET_FUNCTION(int (__cdecl *) (void *, char *, int, char *, char **, int, void **), ldap_search_s);
    ...
    DYNA_GET_FUNCTION(void **(__cdecl *) (void *, void *, const char *), ldap_get_values_len);
    ...
    if (server == NULL)
    {
        failf(data, "LDAP local: Cannot connect to %s:%d", conn->host.name, conn->port);
    ...
    rc = (*ldap_simple_bind_s) (server, conn->bits.user_passwd ? conn->user : NULL, conn->bits.user_passwd ? conn->passwd : NULL);
    if (rc != 0)
    {
...
#    endif
    ...
    for (num = 0, entryIterator = (*ldap_first_entry) (server, result); entryIterator; entryIterator = (*ldap_next_entry) (server, entryIterator), num++)
    {
        ...
        {
            struct bv **vals = (struct bv **) (*ldap_get_values_len) (server, entryIterator, attribute);
            ...
            if (vals != NULL)
            {
                for (i = 0; (vals[i] != NULL); i++)
                {
                    ...
                    if ((strlen(attribute) > 7) && (strcmp(";binary", (char *) attribute + (strlen((char *) attribute) - 7)) == 0))
                    {
                        /* Binary attribute, encode to base64. */
                        val_b64_sz = Curl_base64_encode(conn->data, vals[i]->bv_val, vals[i]->bv_len, &val_b64);
                        if (val_b64_sz > 0)
                        {
                        ...
                        Curl_client_write(conn, CLIENTWRITE_BODY, vals[i]->bv_val, vals[i]->bv_len);
                    Curl_client_write(conn, CLIENTWRITE_BODY, (char *) "\n", 0);
                }
...
static char **split_str(char *str)
{
    ...
    for (i = 0, s = strtok_r(str, ",", &lasts); s; s = strtok_r(NULL, ",", &lasts), i++)
        res[i] = s;
    return res;
}
...
static int _ldap_url_parse2(const struct connectdata *conn, LDAPURLDesc * ludp)
{
    ...
    if (!conn->data || !conn->data->reqdata.path || conn->data->reqdata.path[0] != '/' || !checkprefix(conn->protostr, conn->data->change.url))
    ...
    LDAP_TRACE(("DN '%.*s'\n", p ? (size_t) (p - ludp->lud_dn) : strlen(ludp->lud_dn), ludp->lud_dn));
