...
#    endif
    {
        ...
        nread += RAND_load_file((data->set.ssl.random_file ? data->set.ssl.random_file : RANDOM_FILE), RAND_LOAD_LENGTH);
...
static int cert_stuff(struct connectdata *conn, SSL_CTX * ctx, char *cert_file, const char *cert_type, char *key_file, const char *key_type)
{
...
#    else
             ...
             */
            SSL_CTX_set_default_passwd_cb_userdata(ctx, data->set.key_passwd);
#    endif
            /* Set passwd callback: */
...
#    ifdef HAVE_PKCS12_SUPPORT
                    ...
                    {
                        failf(data, "could not parse PKCS12 file, check password, OpenSSL error %s", ERR_error_string(ERR_get_error(), NULL));
                        return 0;
                    }
...
#        endif
                        ...
                        priv_key = (EVP_PKEY *) ENGINE_load_private_key(conn->data->state.engine, key_file,
#        ifdef HAVE_ENGINE_LOAD_FOUR_ARGS
                                                                        ui_method,
#        endif
                                                                        data->set.key_passwd);
...
CURLcode Curl_ossl_set_engine(struct SessionHandle * data, const char *engine)
{
    ...
    if (!ENGINE_init(e))
    {
        ...
        ENGINE_free(e);
        failf(data, "Failed to initialise SSL Engine '%s':\n%s", engine, SSL_strerror(ERR_get_error(), buf, sizeof(buf)));
    ...
    if (connssl->handle)
    {
        while (!done)
        {
            ...
            if (what > 0)
            {
                ...
                nread = (ssize_t) SSL_read(conn->ssl[sockindex].handle, buf, sizeof(buf));
                ...
                switch (err)
                {
                    ...
                    default:
                        ...
                        failf(conn->data, "SSL read: %s, errno %d", ERR_error_string(sslerror, buf), Curl_sockerrno());
                        done = 1;
                        break;
                }
            }
...
static int Curl_ASN1_UTCTIME_output(struct connectdata *conn, const char *prefix, ASN1_UTCTIME * tm)
{
    char *asn1_string;
    int gmt = FALSE;
    int i;
    ...
    if ((asn1_string[10] >= '0') && (asn1_string[10] <= '9') && (asn1_string[11] >= '0') && (asn1_string[11] <= '9'))
...
{
    return (type == SSL3_RT_CHANGE_CIPHER_SPEC ? "TLS change cipher, " : type == SSL3_RT_ALERT ? "TLS alert, " : type == SSL3_RT_HANDSHAKE ? "TLS handshake, " : type == SSL3_RT_APPLICATION_DATA ? "TLS app data, " : "TLS Unknown, ");
}

/*
 * Our callback from the SSL/TLS layers.
 */
static void ssl_tls_trace(int direction, int ssl_ver, int content_type, const void *buf, size_t len, const SSL * ssl, struct connectdata *conn)
{
    ...
    data = conn->data;
    ssl_ver >>= 8;
    ver = (ssl_ver == SSL2_VERSION_MAJOR ? '2' : ssl_ver == SSL3_VERSION_MAJOR ? '3' : '?');
...
static CURLcode Curl_ossl_connect_step1(struct connectdata *conn, int sockindex)
{
...
#    ifdef SSL_CTRL_SET_MSG_CALLBACK
    if (data->set.fdebug && data->set.verbose)
    {
        ...
        if (!SSL_CTX_callback_ctrl(connssl->ctx, SSL_CTRL_SET_MSG_CALLBACK, (void (*)(void)) ssl_tls_trace))
        {
            infof(data, "SSL: couldn't set callback!\n");
        }
        else if (!SSL_CTX_ctrl(connssl->ctx, SSL_CTRL_SET_MSG_CALLBACK_ARG, 0, conn))
        {
...
#    endif
    ...
    if (data->set.cert)
    {
        if (!cert_stuff(conn, connssl->ctx, data->set.cert, data->set.cert_type, data->set.key, data->set.key_type))
        {
    ...
    {
        if (!SSL_CTX_set_cipher_list(connssl->ctx, data->set.ssl.cipher_list))
        {
    ...
    if (data->set.ssl.CAfile || data->set.ssl.CApath)
    {
        ...
        if (!SSL_CTX_load_verify_locations(connssl->ctx, data->set.ssl.CAfile, data->set.ssl.CApath))
        {
            if (data->set.ssl.verifypeer)
            {
                /* Fail if we insist on successfully verifying the server. */
                failf(data, "error setting certificate verify locations:\n" "  CAfile: %s\n  CApath: %s\n", data->set.ssl.CAfile ? data->set.ssl.CAfile : "none", data->set.ssl.CApath ? data->set.ssl.CApath : "none");
    ...
    SSL_CTX_set_verify(connssl->ctx, data->set.ssl.verifypeer ? SSL_VERIFY_PEER : SSL_VERIFY_NONE, cert_verify_callback);

    /* give application a chance to interfere with SSL set up. */
    if (data->set.ssl.fsslctx)
    {
        retcode = (*data->set.ssl.fsslctx) (data, connssl->ctx, data->set.ssl.fsslctxp);
        if (retcode)
        {
    ...
    if (!Curl_ssl_getsessionid(conn, &ssl_sessionid, NULL))
    {
        ...
        {
            failf(data, "SSL: SSL_set_session failed: %s", ERR_error_string(ERR_get_error(), NULL));
    ...
    {
        failf(data, "SSL: SSL_set_fd failed: %s", ERR_error_string(ERR_get_error(), NULL));
        return CURLE_SSL_CONNECT_ERROR;
    }
...
static CURLcode Curl_ossl_connect_step2(struct connectdata *conn, int sockindex, long *timeout_ms)
{
    struct SessionHandle *data = conn->data;
    int err;
    long has_passed;
    ...
    curlassert(ssl_connect_2 == connssl->connecting_state || ssl_connect_2_reading == connssl->connecting_state || ssl_connect_2_writing == connssl->connecting_state);
    ...
    *timeout_ms -= has_passed;
    ...
    if (1 != err)
    {
        ...
        else
        {
            ...
            switch (errdetail)
            {
                ...
                case 0x14090086:
                    /* 14090086:
                       ...
                       certificate verify failed */
                    cert_problem = "SSL certificate problem, verify that the CA cert is" " OK. Details:\n";
                    rc = CURLE_SSL_CACERT;
                    break;
                default:
            ...
            {
                failf(data, "Unknown SSL protocol error in connection to %s:%d ", conn->host.name, conn->port);
                return rc;
            }
...
static CURLcode Curl_ossl_connect_step3(struct connectdata *conn, int sockindex)
{
    CURLcode retcode = CURLE_OK;
    char *str;
    long lerr;
...
#    else
         ...
         */
#    endif
        retcode = Curl_ssl_addsessionid(conn, our_ssl_sessionid, 0 /* unknown size */ );
        if (retcode)
        {
    ...
    str = X509_NAME_oneline(X509_get_subject_name(connssl->server_cert), NULL, 0);
    if (!str)
    {
    ...
    str = X509_NAME_oneline(X509_get_issuer_name(connssl->server_cert), NULL, 0);
    if (!str)
    {
    ...
    else
    {
        ...
        lerr = data->set.ssl.certverifyresult = SSL_get_verify_result(connssl->handle);
...
static CURLcode Curl_ossl_connect_common(struct connectdata *conn, int sockindex, bool nonblocking, bool * done)
{
    CURLcode retcode;
    struct SessionHandle *data = conn->data;
    struct ssl_connect_data *connssl = &conn->ssl[sockindex];
    ...
    while (ssl_connect_2 == connssl->connecting_state || ssl_connect_2_reading == connssl->connecting_state || ssl_connect_2_writing == connssl->connecting_state)
    {
        ...
        if (connssl->connecting_state == ssl_connect_2_reading || connssl->connecting_state == ssl_connect_2_writing)
        {
            ...
            int writefd = ssl_connect_2_writing == connssl->connecting_state ? sockfd : CURL_SOCKET_BAD;
            int readfd = ssl_connect_2_reading == connssl->connecting_state ? sockfd : CURL_SOCKET_BAD;
    ...
    if (nread < 0)
    {
        ...
        switch (err)
        {
            ...
            default:
                /* openssl/ssl.h says "look at error stack/return value/errno" */
                sslerror = ERR_get_error();
                failf(conn->data, "SSL read: %s, errno %d", ERR_error_string(sslerror, error_buffer), Curl_sockerrno());
                return -1;
        }
    }
    return nread;
...
#        if (SSLEAY_VERSION_NUMBER >= 0x905000)
    {
        ...
        return snprintf(buffer, size, " OpenSSL/%lx.%lx.%lx%s", (ssleay_value >> 28) & 0xf, (ssleay_value >> 20) & 0xff, (ssleay_value >> 12) & 0xff, sub);
    }

#        else                    /* SSLEAY_VERSION_NUMBER is less than 0.9.5 */

#            if (SSLEAY_VERSION_NUMBER >= 0x900000)
    return snprintf(buffer, size, " OpenSSL/%lx.%lx.%lx", (SSLEAY_VERSION_NUMBER >> 28) & 0xff, (SSLEAY_VERSION_NUMBER >> 20) & 0xff, (SSLEAY_VERSION_NUMBER >> 12) & 0xf);
...
#            else                /* (SSLEAY_VERSION_NUMBER >= 0x900000) */
    {
        ...
        return snprintf(buffer, size, " SSL/%x.%x.%x%s", (SSLEAY_VERSION_NUMBER >> 12) & 0xff, (SSLEAY_VERSION_NUMBER >> 8) & 0xf, (SSLEAY_VERSION_NUMBER >> 4) & 0xf, sub);
    }
