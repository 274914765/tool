        ...
        for (i = 0; i < c->num; i++)
        {
            ...
            if (pipe)
            {
                for (curr = pipe->head; curr; curr = curr->next)
                {
                    ...
                    {
                        fprintf(stderr, "MAJOR problem we %p are still in send pipe for %p done %d\n", data, connptr, connptr->bits.done);
                    }
                }
            }
            ...
            if (pipe)
            {
                for (curr = pipe->head; curr; curr = curr->next)
                {
                    if (data == (struct SessionHandle *) curr->ptr)
                    {
                        fprintf(stderr, "MAJOR problem we %p are still in recv pipe for %p done %d\n", data, connptr, connptr->bits.done);
                    }
                }
            }
        }
...
struct conncache *Curl_mk_connc(int type, int amount)    /* set -1 to use default */
{
    /* It is subject for debate how many default connections to have for a multi
       connection cache... */
    int default_amount = amount == -1 ? ((type == CONNCACHE_PRIVATE) ? 5 : 10) : amount;
    struct conncache *c;
...
CURLcode Curl_setopt(struct SessionHandle * data, CURLoption option, va_list param)
{
    ...
    switch (option)
    {
        ...
        case CURLOPT_UNRESTRICTED_AUTH:
             ...
             * hostname changed.
             */
            data->set.http_disable_hostname_check_before_authentication = (bool) (0 != va_arg(param, long));
            break;
...
#    if !defined(CURL_DISABLE_COOKIES)
        ...
        case CURLOPT_COOKIEJAR:
             ...
             * have been made.
             */
            data->cookies = Curl_cookie_init(data, NULL, data->cookies, data->set.cookiesession);
            break;
...
static bool IsPipeliningPossible(struct SessionHandle *handle)
{
    if (handle->multi && Curl_multi_canPipeline(handle->multi) && (handle->set.httpreq == HTTPREQ_GET || handle->set.httpreq == HTTPREQ_HEAD) && handle->set.httpversion != CURL_HTTP_VERSION_1_0)
        return TRUE;
...
void Curl_addHandleToPipeline(struct SessionHandle *data, struct curl_llist *pipe)
{
#ifdef CURLDEBUG
...
static bool ConnectionExists(struct SessionHandle *data, struct connectdata *needle, struct connectdata **usethis)
{
    ...
    for (i = 0; i < data->state.connc->num; i++)
    {
        ...
        {
            infof(data, "Connection #%ld has not finished name resolve, can't reuse\n", check->connectindex);
            continue;
        }
#endif
        ...
        if (!needle->bits.httpproxy || needle->protocol & PROT_SSL)
        {
            ...
            if (strequal(needle->protostr, check->protostr) && strequal(needle->host.name, check->host.name) && (needle->remote_port == check->remote_port))
            {
                if (needle->protocol & PROT_SSL)
                {
                    ...
                    {
                        infof(data, "Connection #%ld has different SSL parameters, " "can't reuse\n", check->connectindex);
                        continue;
                    }
                }
                if ((needle->protocol & PROT_FTP) || ((needle->protocol & PROT_HTTP) && (data->state.authhost.want == CURLAUTH_NTLM)))
                {
        ...
        {                        /* The requested needle connection is using a proxy,
            ...
            if (check->bits.httpproxy && strequal(needle->proxy.name, check->proxy.name) && needle->port == check->port)
            {
...
static CURLcode ConnectPlease(struct SessionHandle *data, struct connectdata *conn, struct Curl_dns_entry *hostaddr, bool * connected)
{
    ...
    infof(data, "About to connect() to %s%s port %d (#%d)\n", conn->bits.httpproxy ? "proxy " : "", hostname, conn->port, conn->connectindex);

  /*************************************************************
   * Connect to server/proxy
   *************************************************************/
    result = Curl_connecthost(conn, hostaddr, &conn->sock[FIRSTSOCKET], &addr, connected);
    if (CURLE_OK == result)
    {
...
{
    infof(conn->data, "Connected to %s (%s) port %d (#%d)\n", conn->bits.httpproxy ? conn->proxy.dispname : conn->host.dispname, conn->ip_addr_str, conn->port, conn->connectindex);
}
...
#    else
              "<no msg>",
#    endif
              err_pos, uc_name[err_pos], uc_name[err_pos] & 255);
...
static void fix_hostname(struct SessionHandle *data, struct connectdata *conn, struct hostname *host)
{
   ...
   *************************************************************/
    if (!is_ASCII_name(host->name) && stringprep_check_version(LIBIDN_REQUIRED_VERSION))
    {
...
static CURLcode ParseURLAndFillConnection(struct SessionHandle *data, struct connectdata *conn)
{
   ...
   * url ...
   ************************************************************/
    if ((2 == sscanf(data->change.url, "%15[^:]:%[^\n]", conn->protostr, path)) && strequal(conn->protostr, "file"))
    {
        if (path[0] == '/' && path[1] == '/')
        {
    ...
    else
    {
        ...
        if (2 > sscanf(data->change.url, "%15[^\n:]://%[^\n/]%[^\n]", conn->protostr, conn->host.name, path))
        {
   ...
   *************************************************************/
    ...
    *in_connect = conn;
    ...
    conn->bits.httpproxy = (bool) (data->set.proxy    /* http proxy or not */
                                   && *data->set.proxy && (data->set.proxytype == CURLPROXY_HTTP));
    ...
    if (!conn->bits.httpproxy)
    {
        ...
        if (!no_proxy || !strequal("*", no_proxy))
        {
            ...
            while (nope)
            {
                ...
                {
                    char *checkn = conn->host.name + namelen - strlen(nope);
...
#else
        failf(data, LIBCURL_NAME " was built with HTTP disabled, http: not supported!");
...
#else                            /* USE_SSL */
        failf(data, LIBCURL_NAME " was built with SSL disabled, https: not supported!");
        return CURLE_UNSUPPORTED_PROTOCOL;
#endif                            /* !USE_SSL */
    }
    else if (strequal(conn->protostr, "FTP") || strequal(conn->protostr, "FTPS"))
    {
...
#    ifdef USE_SSL
            ...
            port = PORT_FTPS;
#    else
            failf(data, LIBCURL_NAME " was built with SSL disabled, ftps: not supported!");
            return CURLE_UNSUPPORTED_PROTOCOL;
#    endif                        /* !USE_SSL */
        }
...
#    endif
...
#else
        failf(data, LIBCURL_NAME " was built with TELNET disabled!");
#endif
    }
...
#else
        failf(data, LIBCURL_NAME " was built with DICT disabled!");
#endif
    }
    else if (strequal(conn->protostr, "LDAP"))
    {
        ...
        conn->curl_done = (Curl_done_func) ZERO_NULL;
#else
        failf(data, LIBCURL_NAME " was built with LDAP disabled!");
#endif
    }
    else if (strequal(conn->protostr, "FILE"))
    {
        ...
        return result;
#else
        failf(data, LIBCURL_NAME " was built with FILE disabled!");
#endif
    }
...
#else
        failf(data, LIBCURL_NAME " was built with TFTP disabled!");
#endif
    }
    else if (strequal(conn->protostr, "SCP"))
    {
        ...
        conn->curl_do_more = (Curl_do_more_func) ZERO_NULL;
#else
        failf(data, LIBCURL_NAME " was built without LIBSSH2, scp: not supported!");
        return CURLE_UNSUPPORTED_PROTOCOL;
#endif
    }
    ...
    if (proxy && *proxy)
    {
        ...
        if (atsign)
        {
            ...
            if (1 <= sscanf(proxyptr, "%" MAX_CURL_USER_LENGTH_TXT "[^:]:" "%" MAX_CURL_PASSWORD_LENGTH_TXT "[^@]", proxyuser, proxypasswd))
            {
                CURLcode res = CURLE_OK;
        ...
        if (ptr != NULL)
        {
            ...
            if (data->set.use_netrc != CURL_NETRC_REQUIRED)
            {
                ...
                if (*userpass != ':')
                {
                    ...
                    sscanf(userpass, "%" MAX_CURL_USER_LENGTH_TXT "[^:@]:" "%" MAX_CURL_PASSWORD_LENGTH_TXT "[^@]", user, passwd);
                }
                else
   ...
   *************************************************************/
    if ((1 == sscanf(conn->host.name, "[%*39[0-9a-fA-F:.]%c", &endbracket)) && (']' == endbracket))
    {
    ...
    if (data->set.use_port && data->state.allow_port)
    {
        ...
        if (conn->bits.httpproxy)
        {
            ...
            url = aprintf("http://%s:%d%s", conn->host.name, conn->remote_port, data->reqdata.path);
            if (!url)
                return CURLE_OUT_OF_MEMORY;
    ...
    {
        if (Curl_parsenetrc(conn->host.name, user, passwd, data->set.netrc_file))
        {
    ...
    if ((conn->protocol & PROT_FTP) && !conn->bits.user_passwd)
    {
   ...
   *************************************************************/
    ...
    if (reuse)
    {
        ...
        *in_connect = conn;        /* return this instead! */
        ...
        infof(data, "Re-using existing connection! (#%ld) with host %s\n", conn->connectindex, conn->bits.httpproxy ? conn->proxy.dispname : conn->host.dispname);
    }
    else
    {
        /*
...
#        ifdef HAVE_ALARM
        ...
        if (shortest && data->set.connecttimeout && (data->set.connecttimeout < shortest))
...
CURLcode Curl_done(struct connectdata ** connp, CURLcode status, bool premature)    /* an error if this is called after an
                                                                                       error was detected */
{
    CURLcode result;
