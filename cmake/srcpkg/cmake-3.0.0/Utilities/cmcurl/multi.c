...
static void multistate(struct Curl_one_easy *easy, CURLMstate state)
{
    ...
    infof(easy->easy_handle, "STATE: %s => %s handle %p; (connection #%ld) \n", statename[oldstate], statename[easy->state], (char *) easy, index);
#endif
...
CURLMcode curl_multi_add_handle(CURLM * multi_handle, CURL * easy_handle)
{
    ...
    if (easy->easy_handle->dns.hostcache && (easy->easy_handle->dns.hostcachetype == HCACHE_PRIVATE))
    {
    ...
    if (!easy->easy_handle->dns.hostcache || (easy->easy_handle->dns.hostcachetype == HCACHE_NONE))
    {
...
static void debug_print_sock_hash(void *p)
{
    ...
    fprintf(stderr, " [easy %p/magic %x/socket %d]", (void *) sh->easy, sh->easy->magic, sh->socket);
}
#endif
...
CURLMcode curl_multi_remove_handle(CURLM * multi_handle, CURL * curl_handle)
{
    ...
    if (easy)
    {
        ...
        if (easy->easy_handle->state.is_in_pipeline && easy->state > CURLM_STATE_DO)
        {
...
static int multi_getsock(struct Curl_one_easy *easy, curl_socket_t * socks,    /* points to numsocks number
                                                                               of sockets */
                         int numsocks)
{
...
static CURLMcode multi_runsingle(struct Curl_multi *multi, struct Curl_one_easy *easy)
{
    ...
    do
    {
        ...
        if (CURLM_STATE_WAITCONNECT <= easy->state && easy->state <= CURLM_STATE_DO && easy->easy_handle->change.url_changed)
        {
            char *gotourl;
        ...
        switch (easy->state)
        {
            ...
            case CURLM_STATE_CONNECT:
                ...
                easy->result = Curl_connect(easy->easy_handle, &easy->easy_conn, &async, &protocol_connect);

                if (CURLE_OK == easy->result)
                {
                    /* Add this handle to the send pipeline */
                    Curl_addHandleToPipeline(easy->easy_handle, easy->easy_conn->send_pipe);
            ...
            case CURLM_STATE_WAITRESOLVE:
                /* awaiting an asynch name resolve to complete */
                {
                    ...
                    if (dns)
                    {
                        ...
                        easy->result = Curl_async_resolved(easy->easy_conn, &protocol_connect);
            ...
            case CURLM_STATE_WAITCONNECT:
                ...
                if (connected)
                    easy->result = Curl_protocol_connect(easy->easy_conn, &protocol_connect);
            ...
            case CURLM_STATE_WAITDO:
                ...
                infof(easy->easy_handle, "Conn %d send pipe %d inuse %d athead %d\n",
                      easy->easy_conn->connectindex, easy->easy_conn->send_pipe->size, easy->easy_conn->writechannel_inuse, Curl_isHandleAtHead(easy->easy_handle, easy->easy_conn->send_pipe));
#endif
                if (!easy->easy_conn->writechannel_inuse && Curl_isHandleAtHead(easy->easy_handle, easy->easy_conn->send_pipe))
                {
                    /* Grab the channel */
            ...
            case CURLM_STATE_DO_MORE:
                /* Ready to do more? */
                easy->result = Curl_is_connected(easy->easy_conn, SECONDARYSOCKET, &connected);
                if (connected)
                {
                    /*
                    ...
                    if (CURLE_OK == easy->result)
                        ...
                        Curl_removeHandleFromPipeline(easy->easy_handle, easy->easy_conn->send_pipe);
            ...
            case CURLM_STATE_DO_DONE:
                ...
                Curl_addHandleToPipeline(easy->easy_handle, easy->easy_conn->recv_pipe);
            ...
            case CURLM_STATE_WAITPERFORM:
                ...
                infof(easy->easy_handle, "Conn %d recv pipe %d inuse %d athead %d\n",
                      easy->easy_conn->connectindex, easy->easy_conn->recv_pipe->size, easy->easy_conn->readchannel_inuse, Curl_isHandleAtHead(easy->easy_handle, easy->easy_conn->recv_pipe));
#endif
                /* Wait for our turn to PERFORM */
                if (!easy->easy_conn->readchannel_inuse && Curl_isHandleAtHead(easy->easy_handle, easy->easy_conn->recv_pipe))
                {
                    /* Grab the channel */
            ...
            case CURLM_STATE_TOOFAST:    /* limit-rate exceeded in either direction */
                ...
                if (((easy->easy_handle->set.max_send_speed == 0) ||
                     (easy->easy_handle->progress.ulspeed < easy->easy_handle->set.max_send_speed)) && ((easy->easy_handle->set.max_recv_speed == 0) || (easy->easy_handle->progress.dlspeed < easy->easy_handle->set.max_recv_speed)))
                    multistate(easy, CURLM_STATE_PERFORM);
                break;
            ...
            case CURLM_STATE_PERFORM:
                ...
                else if (TRUE == done)
                {
                    ...
                    {
                        Curl_removeHandleFromPipeline(easy->easy_handle, easy->easy_conn->recv_pipe);
                        if (!retry)
                        {
...
CURLMcode curl_multi_cleanup(CURLM * multi_handle)
{
    ...
    if (GOOD_MULTI_HANDLE(multi))
    {
        ...
        {
            if (multi->connc->connects[i] && multi->connc->connects[i]->protocol & PROT_CLOSEACTION)
            {
...
static void singlesocket(struct Curl_multi *multi, struct Curl_one_easy *easy)
{
    ...
    for (i = 0; (i < MAX_SOCKSPEREASYHANDLE) && (curraction & (GETSOCK_READSOCK(i) | GETSOCK_WRITESOCK(i))); i++)
    {
        ...
        multi->socket_cb(easy->easy_handle, s, action, multi->socket_userp, entry ? entry->socketp : NULL);
    ...
    for (i = 0; i < easy->numsocks; i++)
    {
        ...
        if (s != CURL_SOCKET_BAD)
        {
            ...
            if (entry)
            {
                ...
                multi->socket_cb(easy->easy_handle, s, CURL_POLL_REMOVE, multi->socket_userp, entry ? entry->socketp : NULL);
...
static CURLMcode multi_socket(struct Curl_multi *multi, bool checkall, curl_socket_t s, int *running_handles)
{
    CURLMcode result = CURLM_OK;
    ...
    else if (s != CURL_SOCKET_TIMEOUT)
    {
        ...
        struct Curl_sh_entry *entry = Curl_hash_pick(multi->sockhash, (char *) &s, sizeof(s));
...
void Curl_expire(struct SessionHandle *data, long milli)
{
    ...
    else
    {
        ...
        *nowp = set;
        ...
        multi->timetree = Curl_splayinsert((int) nowp->tv_sec, multi->timetree, &data->state.timenode);
    }
#if 0
...
static bool multi_conn_using(struct Curl_multi *multi, struct SessionHandle *data)
{
    ...
    for (i = 0; i < multi->connc->num; i++)
    {
        if (multi->connc->connects[i] && (multi->connc->connects[i]->data == data) && multi->connc->connects[i]->protocol & PROT_CLOSEACTION)
            return TRUE;
    }
...
static void add_closure(struct Curl_multi *multi, struct SessionHandle *data)
{
    ...
    while (cl)
    {
        ...
        for (i = 0; i < multi->connc->num; i++)
        {
            if (multi->connc->connects[i] && (multi->connc->connects[i]->data == cl->easy_handle))
            {
                inuse = TRUE;
                break;
            }
