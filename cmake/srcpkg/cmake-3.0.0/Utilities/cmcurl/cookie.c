            ...
            if (sep && (!semiptr || (semiptr > sep)))
            {
                ...
                if (1 <= sscanf(ptr, "%" MAX_NAME_TXT "[^;=]=%" MAX_COOKIE_LINE_TXT "[^;\r\n]", name, what))
                {
                    ...
                    else if (strequal("max-age", name))
                    {
                        ...
                        co->expires = atoi((*co->maxage == '\"') ? &co->maxage[1] : &co->maxage[0]) + (long) now;
                    }
...
struct Cookie *Curl_cookie_getlist(struct CookieInfo *c, char *host, char *path, bool secure)
{
    ...
    while (co)
    {
        /* only process this cookie if it is not expired or had no expire
           ...
           continue if we are! */
        if ((co->expires <= 0 || (co->expires > now)) && (co->secure ? secure : TRUE))
        {

            /* now check if the domain is correct */
            if (!co->domain || (co->tailmatch && tailmatch(co->domain, host)) || (!co->tailmatch && strequal(host, co->domain)))
            {
...
static char *get_netscape_format(const struct Cookie *co)
{
    return aprintf("%s%s\t"        /* domain */
                   ...
                   co->domain ? co->domain : "unknown", co->tailmatch ? "TRUE" : "FALSE", co->path ? co->path : "/", co->secure ? "TRUE" : "FALSE", co->expires, co->name, co->value ? co->value : "");
}
