...
CURL *curl_easy_duphandle(CURL * incurl)
{
    ...
    struct SessionHandle *outcurl = (struct SessionHandle *) calloc(sizeof(struct SessionHandle), 1);
    ...
    do
    {
        ...
        outcurl->outbound_cd = iconv_open(CURL_ICONV_CODESET_OF_NETWORK, CURL_ICONV_CODESET_OF_HOST);
        outcurl->utf8_cd = iconv_open(CURL_ICONV_CODESET_OF_HOST, CURL_ICONV_CODESET_FOR_UTF8);
#endif
...
CURLcode Curl_convert_to_network(struct SessionHandle *data, char *buffer, size_t length)
{
    ...
    if (data->set.convtonetwork)
    {
        ...
        if (rc != CURLE_OK)
        {
            failf(data, "CURLOPT_CONV_TO_NETWORK_FUNCTION callback returned %i: %s", rc, curl_easy_strerror(rc));
        }
        return (rc);
    }
    else
...
#    ifdef HAVE_ICONV
        ...
        if (data->outbound_cd == (iconv_t) - 1)
        {
            ...
            if (data->outbound_cd == (iconv_t) - 1)
            {
                failf(data, "The iconv_open(\"%s\", \"%s\") call failed with errno %i: %s", CURL_ICONV_CODESET_OF_NETWORK, CURL_ICONV_CODESET_OF_HOST, errno, strerror(errno));
...
CURLcode Curl_convert_from_network(struct SessionHandle * data, char *buffer, size_t length)
{
    ...
    if (data->set.convfromnetwork)
    {
        ...
        if (rc != CURLE_OK)
        {
            failf(data, "CURLOPT_CONV_FROM_NETWORK_FUNCTION callback returned %i: %s", rc, curl_easy_strerror(rc));
        }
        return (rc);
    }
    else
...
#    ifdef HAVE_ICONV
        ...
        if (data->inbound_cd == (iconv_t) - 1)
        {
            ...
            if (data->inbound_cd == (iconv_t) - 1)
            {
                failf(data, "The iconv_open(\"%s\", \"%s\") call failed with errno %i: %s", CURL_ICONV_CODESET_OF_HOST, CURL_ICONV_CODESET_OF_NETWORK, errno, strerror(errno));
                return CURLE_CONV_FAILED;
            }
        }
        /* call iconv */
        input_ptr = output_ptr = buffer;
        in_bytes = out_bytes = length;
        rc = iconv(data->inbound_cd, (const char **) &input_ptr, &in_bytes, &output_ptr, &out_bytes);
        if ((rc == ICONV_ERROR) || (in_bytes != 0))
        {
            failf(data, "The Curl_convert_from_network iconv call failed with errno %i: %s", errno, strerror(errno));
            return CURLE_CONV_FAILED;
        }
#    else
...
CURLcode Curl_convert_from_utf8(struct SessionHandle * data, char *buffer, size_t length)
{
    ...
    if (data->set.convfromutf8)
    {
        ...
        if (rc != CURLE_OK)
        {
            failf(data, "CURLOPT_CONV_FROM_UTF8_FUNCTION callback returned %i: %s", rc, curl_easy_strerror(rc));
        }
        return (rc);
    }
    else
...
#    ifdef HAVE_ICONV
        ...
        if (data->utf8_cd == (iconv_t) - 1)
        {
            ...
            if (data->utf8_cd == (iconv_t) - 1)
            {
                failf(data, "The iconv_open(\"%s\", \"%s\") call failed with errno %i: %s", CURL_ICONV_CODESET_OF_HOST, CURL_ICONV_CODESET_FOR_UTF8, errno, strerror(errno));
                return CURLE_CONV_FAILED;
            }
        }
        /* call iconv */
        input_ptr = output_ptr = buffer;
        in_bytes = out_bytes = length;
        rc = iconv(data->utf8_cd, (const char **) &input_ptr, &in_bytes, &output_ptr, &out_bytes);
        if ((rc == ICONV_ERROR) || (in_bytes != 0))
        {
