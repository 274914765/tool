...
CURLntlm Curl_input_ntlm(struct connectdata *conn, bool proxy,    /* if proxy or not */
...
#        else
            ...
            if ((size < 32) || (memcmp(buffer, NTLMSSP_SIGNATURE, 8) != 0) || (memcmp(buffer + 8, type2_marker, sizeof(type2_marker)) != 0))
            {
...
static void setup_des_key(unsigned char *key_56, DES_key_schedule DESKEYARG(ks))
{
  ...
  */
static void lm_resp(unsigned char *keys, unsigned char *plaintext, unsigned char *results)
{
    DES_key_schedule ks;
 ...
 */
static void mk_lm_hash(struct SessionHandle *data, char *password, unsigned char *lmbuffer /* 21 bytes */ )
{
    unsigned char pw[14];
...
#            if USE_NTRESPONSES
static void utf8_to_unicode_le(unsigned char *dest, const char *src, size_t srclen)
{
    size_t i;
    for (i = 0; i < srclen; i++)
    {
 ...
 */
static void mk_nt_hash(struct SessionHandle *data, char *password, unsigned char *ntbuffer /* 21 bytes */ )
{
    size_t len = strlen(password);
...
#        ifdef USE_WINDOWS_SSPI
    ...
    if (s_hSecDll == NULL)
    {
        ...
        if (s_hSecDll != NULL)
        {
            ...
            pInitSecurityInterface = (INIT_SECURITY_INTERFACE) GetProcAddress(s_hSecDll, "InitSecurityInterfaceA");
...
#        ifdef USE_WINDOWS_SSPI
            {
                ...
                if (s_pSecFn->AcquireCredentialsHandle(NULL, (char *) "NTLM", SECPKG_CRED_OUTBOUND, NULL, ntlm->p_identity, NULL, NULL, &ntlm->handle, &tsDummy) != SEC_E_OK)
                {
                    return CURLE_OUT_OF_MEMORY;
                }
                ...
                status = s_pSecFn->InitializeSecurityContext(&ntlm->handle, NULL, (char *) host, ISC_REQ_CONFIDENTIALITY | ISC_REQ_REPLAY_DETECT | ISC_REQ_CONNECTION, 0, SECURITY_NETWORK_DREP, NULL, 0, &ntlm->c_handle, &desc, &attrs, &tsDummy);
...
#                define NTLM2FLAG 0
#            endif
            snprintf((char *) ntlmbuf, sizeof(ntlmbuf), NTLMSSP_SIGNATURE "%c" "\x01%c%c%c"    /* 32-bit type = 1 */
                     ...
                     SHORTPAIR(domlen), SHORTPAIR(domlen), SHORTPAIR(domoff), 0, 0, SHORTPAIR(hostlen), SHORTPAIR(hostlen), SHORTPAIR(hostoff), 0, 0, host /* this is empty */ , domain /* this is empty */ );
...
#        endif
            ...
            DEBUG_OUT(
                         ...
                         fprintf(stderr, "**** TYPE1 header flags=0x%02.2x%02.2x%02.2x%02.2x 0x%08.8x ",
                                 ...
                                 NTLMFLAG_NEGOTIATE_NTLM_KEY |
                                 NTLM2FLAG |
                                 NTLMFLAG_NEGOTIATE_ALWAYS_SIGN); print_flags(stderr, NTLMFLAG_NEGOTIATE_OEM | NTLMFLAG_REQUEST_TARGET | NTLMFLAG_NEGOTIATE_NTLM_KEY | NTLM2FLAG | NTLMFLAG_NEGOTIATE_ALWAYS_SIGN); fprintf(stderr, "\n****\n");}
            );
...
#        ifdef USE_WINDOWS_SSPI
                ...
                status = s_pSecFn->InitializeSecurityContext(&ntlm->handle, &ntlm->c_handle,
                                                             (char *) host, ISC_REQ_CONFIDENTIALITY | ISC_REQ_REPLAY_DETECT | ISC_REQ_CONNECTION, 0, SECURITY_NETWORK_DREP, &type_2_desc, 0, &ntlm->c_handle, &type_3_desc, &attrs, &tsDummy);
...
#            endif
                ...
                size = snprintf((char *) ntlmbuf, sizeof(ntlmbuf), NTLMSSP_SIGNATURE "%c" "\x03%c%c%c"    /* type-3, 32 bits */
...
#            if USE_NTRESPONSES
                                ...
                                SHORTPAIR(0x18), SHORTPAIR(ntrespoff), 0x0, 0x0,
#            else
...
#            endif
                                ...
                                SHORTPAIR(domlen),
                                SHORTPAIR(domoff),
                                0x0, 0x0, SHORTPAIR(userlen), SHORTPAIR(userlen), SHORTPAIR(useroff), 0x0, 0x0, SHORTPAIR(hostlen), SHORTPAIR(hostlen), SHORTPAIR(hostoff), 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, LONGQUARTET(ntlm->flags));
                ...
                DEBUG_OUT(
                             {
                             fprintf(stderr, "**** TYPE3 header lmresp="); print_hex(stderr, (char *) &ntlmbuf[lmrespoff], 0x18);});
...
#            if USE_NTRESPONSES
                ...
                DEBUG_OUT(
                             {
                             fprintf(stderr, "\n                  ntresp="); print_hex(stderr, (char *) &ntlmbuf[ntrespoff], 0x18);});
...
#            ifdef CURL_DOES_CONVERSIONS
                ...
                if (CURLE_OK != Curl_convert_to_network(conn->data, (char *) &ntlmbuf[domoff], size - domoff))
                {
