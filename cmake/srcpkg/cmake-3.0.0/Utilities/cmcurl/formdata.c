...
static struct curl_httppost *AddHttpPost(char *name, size_t namelength,
                                         ...
                                         char *contenttype, long flags, struct curl_slist *contentHeader, char *showfilename, struct curl_httppost *parent_post, struct curl_httppost **httppost, struct curl_httppost **last_post)
{
...
#    endif
            ...
            case CURLFORM_NAMELENGTH:
                if (current_form->namelength)
                    ...
                    current_form->namelength = array_state ? (long) array_value : (long) va_arg(params, long);
                break;
            ...
            case CURLFORM_CONTENTSLENGTH:
                if (current_form->contentslength)
                    ...
                    current_form->contentslength = array_state ? (long) array_value : va_arg(params, long);
                break;
            ...
            case CURLFORM_FILECONTENT:
                ...
                {
                    char *filename = array_state ? array_value : va_arg(params, char *);
                    if (filename)
                    {
            ...
            case CURLFORM_FILE:
                {
                    ...
                    if (current_form->value)
                    {
                        if (current_form->flags & HTTPPOST_FILENAME)
                        {
                            if (filename)
                            {
                                if ((current_form = AddFormInfo(strdup(filename), NULL, current_form)) == NULL)
                                    return_value = CURL_FORMADD_MEMORY;
                            }
                            else
                                return_value = CURL_FORMADD_NULL;
            ...
            case CURLFORM_BUFFERLENGTH:
                if (current_form->bufferlength)
                    ...
                    current_form->bufferlength = array_state ? (long) array_value : va_arg(params, long);
                break;

            case CURLFORM_CONTENTTYPE:
                {
                    char *contenttype = array_state ? array_value : va_arg(params, char *);
                    if (current_form->contenttype)
                    {
                        if (current_form->flags & HTTPPOST_FILENAME)
                        {
                            if (contenttype)
                            {
                                if ((current_form = AddFormInfo(NULL, strdup(contenttype), current_form)) == NULL)
                                    return_value = CURL_FORMADD_MEMORY;
                            }
                            else
                                return_value = CURL_FORMADD_NULL;
    ...
    if (CURL_FORMADD_OK == return_value)
    {
        ...
        for (form = first_form; form != NULL; form = form->more)
        {
            if (((!form->name || !form->value) && !post) ||
                ...
                ((form->flags & HTTPPOST_FILENAME) &&
                 (form->flags & HTTPPOST_PTRCONTENTS)) || ((!form->buffer) && (form->flags & HTTPPOST_BUFFER) && (form->flags & HTTPPOST_PTRBUFFER)) || ((form->flags & HTTPPOST_READFILE) && (form->flags & HTTPPOST_PTRCONTENTS)))
            {
            ...
            else
            {
                if (((form->flags & HTTPPOST_FILENAME) || (form->flags & HTTPPOST_BUFFER)) && !form->contenttype)
                {
                    ...
                    form->contenttype = strdup(ContentTypeForFilename(form->value, prevtype));
                    if (!form->contenttype)
                    {
                        return_value = CURL_FORMADD_MEMORY;
                        break;
                ...
                if (!(form->flags & HTTPPOST_FILENAME) && !(form->flags & HTTPPOST_READFILE) && !(form->flags & HTTPPOST_PTRCONTENTS) && !(form->flags & HTTPPOST_PTRBUFFER))
                {
                ...
                post = AddHttpPost(form->name, form->namelength, form->value, form->contentslength, form->buffer, form->bufferlength, form->contenttype, form->flags, form->contentheader, form->showfilename, post, httppost, last_post);
    ...
    *finalform = NULL;            /* default form is empty */
    ...
    do
    {
        ...
        do
        {
            ...
            {
                /* if multiple-file */
                char *filebasename = (!file->showfilename) ? strippath(file->contents) : NULL;

                result = AddFormDataf(&form, &size, "\r\n--%s\r\nContent-Disposition: " "attachment; filename=\"%s\"", fileboundary, (file->showfilename ? file->showfilename : filebasename));
                if (filebasename)
                    free(filebasename);
                if (result)
                    break;
            }
            else if ((post->flags & HTTPPOST_FILENAME) || (post->flags & HTTPPOST_BUFFER))
            {

                char *filebasename = (!post->showfilename) ? strippath(post->contents) : NULL;
...
#    if 0
            ...
            if (file->contenttype && !checkprefix("text/", file->contenttype))
            {
...
#    endif
            ...
            if ((post->flags & HTTPPOST_FILENAME) || (post->flags & HTTPPOST_READFILE))
            {
                ...
                fileread = strequal("-", file->contents) ? stdin : fopen(file->contents, "rb");    /* binary read for win32  */
...
#    endif
            ...
            else if (post->flags & HTTPPOST_BUFFER)
            {
                ...
                result = AddFormData(&form, FORM_CONTENT, post->buffer, post->bufferlength, &size);
                if (result)
                    break;
            }
            ...
            else
            {
                ...
                result = AddFormData(&form, FORM_CONTENT, post->contents, post->contentslength, &size);
                if (result)
                    break;
            }
        }
...
#    ifdef _FORM_DEBUG
int FormAddTest(const char *errormsg, struct curl_httppost **httppost, struct curl_httppost **last_post, ...)
{
    int result;
    va_list arg;
    va_start(arg, last_post);
...
int main()
{
    ...
    if (FormAddTest("simple COPYCONTENTS test", &httppost, &last_post, CURLFORM_COPYNAME, name1, CURLFORM_COPYCONTENTS, value1, CURLFORM_END))
        ++errors;
    if (FormAddTest("COPYCONTENTS  + CONTENTTYPE test", &httppost, &last_post, CURLFORM_COPYNAME, name2, CURLFORM_COPYCONTENTS, value2, CURLFORM_CONTENTTYPE, type2, CURLFORM_END))
        ++errors;
    /* make null character at start to check that contentslength works
       correctly */
    name3[1] = '\0';
    value3[1] = '\0';
    if (FormAddTest("PTRNAME + NAMELENGTH + COPYNAME + CONTENTSLENGTH test", &httppost, &last_post, CURLFORM_PTRNAME, name3, CURLFORM_COPYCONTENTS, value3, CURLFORM_CONTENTSLENGTH, value3length, CURLFORM_NAMELENGTH, name3length, CURLFORM_END))
        ++errors;
    if (FormAddTest("simple PTRCONTENTS test", &httppost, &last_post, CURLFORM_COPYNAME, name4, CURLFORM_PTRCONTENTS, value4, CURLFORM_END))
        ++errors;
    /* make null character at start to check that contentslength works
       correctly */
    value5[1] = '\0';
    if (FormAddTest("PTRCONTENTS + CONTENTSLENGTH test", &httppost, &last_post, CURLFORM_COPYNAME, name5, CURLFORM_PTRCONTENTS, value5, CURLFORM_CONTENTSLENGTH, value5length, CURLFORM_END))
        ++errors;
    /* make null character at start to check that contentslength works
       correctly */
    value6[1] = '\0';
    if (FormAddTest("PTRCONTENTS + CONTENTSLENGTH + CONTENTTYPE test", &httppost, &last_post, CURLFORM_COPYNAME, name6, CURLFORM_PTRCONTENTS, value6, CURLFORM_CONTENTSLENGTH, value6length, CURLFORM_CONTENTTYPE, type6, CURLFORM_END))
        ++errors;
    if (FormAddTest("FILE + CONTENTTYPE test", &httppost, &last_post, CURLFORM_COPYNAME, name7, CURLFORM_FILE, value7, CURLFORM_CONTENTTYPE, type7, CURLFORM_END))
        ++errors;
    if (FormAddTest("FILE1 + FILE2 test", &httppost, &last_post, CURLFORM_COPYNAME, name8, CURLFORM_FILE, value7, CURLFORM_FILE, value8, CURLFORM_END))
        ++errors;
    if (FormAddTest("FILE1 + FILE2 + FILE3 test", &httppost, &last_post, CURLFORM_COPYNAME, name9, CURLFORM_FILE, value7, CURLFORM_FILE, value8, CURLFORM_FILE, value7, CURLFORM_END))
        ++errors;
    ...
    if (FormAddTest("FILE1 + FILE2 + FILE3 ARRAY test", &httppost, &last_post, CURLFORM_COPYNAME, name10, CURLFORM_ARRAY, forms, CURLFORM_END))
        ++errors;
    if (FormAddTest("FILECONTENT test", &httppost, &last_post, CURLFORM_COPYNAME, name11, CURLFORM_FILECONTENT, value7, CURLFORM_END))
        ++errors;
    ...
    do
    {
        nread = Curl_FormReader(buffer, 1, sizeof(buffer), (FILE *) & formread);
