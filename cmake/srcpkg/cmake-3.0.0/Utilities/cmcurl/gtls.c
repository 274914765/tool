...
static void showtime(struct SessionHandle *data, const char *text, time_t stamp)
{
    struct tm *tm;
#    ifdef HAVE_GMTIME_R
    struct tm buffer;
...
#    else
    tm = gmtime(&stamp);
#    endif
    snprintf(data->state.buffer, BUFSIZE, "\t %s: %s, %02d %s %4d %02d:%02d:%02d GMT\n", text, Curl_wkday[tm->tm_wday ? tm->tm_wday - 1 : 6], tm->tm_mday, Curl_month[tm->tm_mon], tm->tm_year + 1900, tm->tm_hour, tm->tm_min, tm->tm_sec);
    infof(data, "%s", data->state.buffer);
}

/* this function does a BLOCKING SSL/TLS (re-)handshake */
static CURLcode handshake(struct connectdata *conn, gnutls_session session, int sockindex, bool duringconnect)
{
    ...
    do
    {
        ...
        if ((rc == GNUTLS_E_AGAIN) || (rc == GNUTLS_E_INTERRUPTED))
        {
            ...
            rc = Curl_select(conn->sock[sockindex], conn->sock[sockindex], (int) timeout_ms);
            if (rc > 0)
...
CURLcode Curl_gtls_connect(struct connectdata * conn, int sockindex)
{
    ...
    if (data->set.ssl.CAfile)
    {
        ...
        rc = gnutls_certificate_set_x509_trust_file(conn->ssl[sockindex].cred, data->set.ssl.CAfile, GNUTLS_X509_FMT_PEM);
        if (rc < 0)
        {
    ...
    if (data->set.cert)
    {
        if (gnutls_certificate_set_x509_key_file(conn->ssl[sockindex].cred, data->set.cert, data->set.key != 0 ? data->set.key : data->set.cert, do_file_type(data->set.cert_type)))
        {
    ...
    gnutls_transport_set_ptr(session, (gnutls_transport_ptr) conn->sock[sockindex]);
    ...
    if (verify_status & GNUTLS_CERT_INVALID)
    {
        if (data->set.ssl.verifypeer)
        {
            failf(data, "server certificate verification failed. CAfile: %s", data->set.ssl.CAfile ? data->set.ssl.CAfile : "none");
    ...
    if (!rc)
    {
        ...
        {
            failf(data, "SSL: certificate subject name (%s) does not match " "target host name '%s'", certbuf, conn->host.dispname);
...
ssize_t Curl_gtls_send(struct connectdata * conn, int sockindex, void *mem, size_t len)
{
...
static void close_one(struct connectdata *conn, int index)
{
                       ...
                       bool * wouldblock)
{
    ...
    *wouldblock = FALSE;
    ...
    if (ret < 0)
    {
        failf(conn->data, "GnuTLS recv error (%d): %s", (int) ret, gnutls_strerror(ret));
        return -1;
    }
