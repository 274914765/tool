...
CURLcode Curl_fillreadbuffer(struct connectdata *conn, int bytes, int *nreadp)
{
    ...
    nread = (int) conn->fread(data->reqdata.upload_fromhere, 1, buffersize, conn->fread_in);
...
CURLcode Curl_readrewind(struct connectdata * conn)
{
     ...
     */
    if (data->set.postfields || (data->set.httpreq == HTTPREQ_POST_FORM))
        ;                        /* do nothing */
    else
    {
        if (data->set.ioctl)
        {
            ...
            err = (data->set.ioctl) (data, CURLIOCMD_RESTARTREAD, data->set.ioctl_client);
...
static void read_rewind(struct connectdata *conn, size_t thismuch)
{
...
#ifdef CURLDEBUG
    {
        ...
        DEBUGF(infof(conn->data, "Buffer after stream rewind (read_pos = %d): [%s]", conn->read_pos, buf));
    }
#endif
}
...
CURLcode Curl_readwrite(struct connectdata *conn, bool * done)
{
    ...
    do
    {
        /* We go ahead and do a read if we have a readable socket or if
           ...
           buffer) */
        if ((k->keepon & KEEP_READ) && ((select_res & CSELECT_IN) || conn->bits.stream_was_rewound))
        {
            /* read */
             ...
             *****/
                        ...
                        if (!k->headerline)
                        {
                            ...
                            if ((k->hbuflen > 5) && !checkhttpprefix(data, data->state.headerbuff))
                            {
                                /* this is not the beginning of a HTTP first header line */
                                k->header = FALSE;
                                if (nread)
                        ...
                        if ((0x0a == *k->p) || (0x0d == *k->p))
                        {
                            ...
                            conn->deductheadercount = (100 == k->httpcode) ? conn->headerbytecount : 0;
...
#    define HEADER1 k->p        /* no conversion needed, just use k->p */
                            ...
                            nc = sscanf(HEADER1, " HTTP/%d.%d %3d", &httpversion_major, &k->httpversion, &k->httpcode);
                            if (nc == 3)
                            {
                            ...
                            if (nc)
                            {
                                 ...
                                 */
                                if (data->set.http_fail_on_error && (k->httpcode >= 400) && ((k->httpcode != 401) || !data->set.userpwd) && ((k->httpcode != 407) || !data->set.proxyuserpwd))
                                {
                        ...
                        if (!k->ignorecl && !data->set.ignorecl && checkprefix("Content-Length:", k->p))
                        {
                            ...
                            else
                            {
                                /* Negative Content-Length is really odd, and we know it
                                   ...
                                   files */
                                conn->bits.close = TRUE;
                                infof(data, "Negative content-length: %" FORMAT_OFF_T ", closing after transfer\n", contentlength);
                            }
                        }
                        ...
                        else if (checkprefix("Content-Type:", k->p))
                        {
                            ...
                            for (start = k->p + 13; *start && ISSPACE(*start); start++)
                                ;    /* empty loop */
...
#ifndef CURL_DISABLE_HTTP
                        else if ((k->httpversion == 10) && conn->bits.httpproxy && Curl_compareheader(k->p, "Proxy-Connection:", "keep-alive"))
                        {
                            /*
                        ...
                        else if ((k->httpversion == 11) && conn->bits.httpproxy && Curl_compareheader(k->p, "Proxy-Connection:", "close"))
                        {
                            /*
                        ...
                        else if (checkprefix("Trailer:", k->p) || checkprefix("Trailers:", k->p))
                        {
                            /*
                        ...
                        else if (checkprefix("Content-Encoding:", k->p) && data->set.encoding)
                        {
                            /*
                            ...
                            else if (checkprefix("compress", start) || checkprefix("x-compress", start))
...
#    if !defined(CURL_DISABLE_COOKIES)
                        else if (data->cookies && checkprefix("Set-Cookie:", k->p))
                        {
                            ...
                            Curl_cookie_add(data, data->cookies, TRUE, k->p + 11,
                                            ...
                                            conn->allocptr.cookiehost ? conn->allocptr.cookiehost : conn->host.name, data->reqdata.path);
...
#    endif
                        ...
                        else if ((k->httpcode >= 300 && k->httpcode < 400) && checkprefix("Location:", k->p))
                        {
                ...
                if (k->str && !k->header && (nread > 0 || is_empty_data))
                {
                    ...
                    if (0 == k->bodywrites && !is_empty_data)
                    {
                        ...
                        if (conn->protocol & PROT_HTTP)
                        {
                            ...
                            if (data->reqdata.resume_from && !k->content_range && (data->set.httpreq == HTTPREQ_GET) && !k->ignorebody)
                            {
                    ...
                    if (data->set.verbose)
                    {
                        if (k->badheader)
                        {
                            Curl_debug(data, CURLINFO_DATA_IN, data->state.headerbuff, (size_t) k->hbuflen, conn);
                    ...
                    if (!conn->bits.chunk && (nread || k->badheader || is_empty_data))
                    {
                        ...
                        if (k->badheader < HEADER_ALLBAD)
                        {
                            ...
                            switch (k->content_encoding)
                            {
                                ...
                                case COMPRESS:
                                default:
                                    failf(data, "Unrecognized content encoding type. " "libcurl understands `identity', `deflate' and `gzip' " "content encodings.");
            ...
            do
            {
                ...
                if (0 == data->reqdata.upload_present)
                {
                    ...
                    if (!k->upload_done)
                    {
                        ...
                        if (k->wait100_after_headers && (data->reqdata.proto.http->sending == HTTPSEND_BODY))
                        {
                ...
                if (data->set.verbose)
                    ...
                    Curl_debug(data, CURLINFO_DATA_OUT, data->reqdata.upload_fromhere, (size_t) bytes_written, conn);
    ...
    if (data->set.timeout && ((Curl_tvdiff(k->now, k->start) / 1000) >= data->set.timeout))
    {
        if (k->size != -1)
        {
            failf(data, "Operation timed out after %d seconds with %" FORMAT_OFF_T " out of %" FORMAT_OFF_T " bytes received", data->set.timeout, k->bytecount, k->size);
        }
        else
        {
            failf(data, "Operation timed out after %d seconds with %" FORMAT_OFF_T " bytes received", data->set.timeout, k->bytecount);
        }
    ...
    if (!k->keepon)
    {
        ...
        if (!(conn->bits.no_body) && (k->size != -1) && (k->bytecount != k->size) &&
#ifdef CURL_DO_LINEEND_CONV
            ...
            !data->reqdata.newurl)
        {
            failf(data, "transfer closed with %" FORMAT_OFF_T " bytes remaining to read", k->size - k->bytecount);
            return CURLE_PARTIAL_FILE;
        }
        else if (!(conn->bits.no_body) && conn->bits.chunk && (data->reqdata.proto.http->chunk.state != CHUNK_STOP))
        {
            /*
...
CURLcode Curl_readwrite_init(struct connectdata * conn)
{
    ...
    k->maxfd = (conn->sockfd > conn->writesockfd ? conn->sockfd : conn->writesockfd) + 1;
    k->hbufp = data->state.headerbuff;
    k->ignorebody = FALSE;
 ...
 */
int Curl_single_getsock(struct connectdata *conn, curl_socket_t * sock,    /* points to numsocks number
                                                                           of sockets */
                        int numsocks)
{
    struct SessionHandle *data = conn->data;
    ...
    if (data->reqdata.keep.keepon & KEEP_WRITE)
    {
        ...
        if ((conn->sockfd != conn->writesockfd) || !(data->reqdata.keep.keepon & KEEP_READ))
        {
...
static CURLcode Transfer(struct connectdata *conn)
{
    ...
    while (!done)
    {
        ...
        /* limit-rate logic: if speed exceeds threshold, then do not include fd in
           ...
           call */
        if ((k->keepon & KEEP_WRITE) && (!data->set.max_send_speed || (data->progress.ulspeed < data->set.max_send_speed)))
        {
            fd_write = conn->writesockfd;
        ...
        if ((k->keepon & KEEP_READ) && (!data->set.max_recv_speed || (data->progress.dlspeed < data->set.max_recv_speed)))
        {
            fd_read = conn->sockfd;
 ...
 */
CURLcode Curl_follow(struct SessionHandle *data, char *newurl,    /* this 'newurl' is the Location: string,
...
bool Curl_retry_request(struct connectdata * conn, char **url)
{
    ...
    if ((data->reqdata.keep.bytecount + conn->headerbytecount == 0) && conn->bits.reuse && !conn->bits.no_body)
    {
 ...
 * upcoming transfer.
 */
CURLcode Curl_setup_transfer(struct connectdata * c_conn,    /* connection data */
