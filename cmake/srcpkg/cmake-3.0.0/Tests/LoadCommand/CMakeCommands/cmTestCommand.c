...
static int CCONV InitialPass(void *inf, void *mf, int argc, char *argv[])
{
    ...
    cmVTKWrapTclData *cdata = (cmVTKWrapTclData *) malloc(sizeof(cmVTKWrapTclData));
    ...
    info->CAPI->AddCacheDefinition(mf, "SOME_CACHE_VARIABLE", "ON", "Test cache variable", CM_CACHE_BOOL);
    info->CAPI->AddCacheDefinition(mf, "SOME_CACHE_VARIABLE1", "", "Test cache variable 1", CM_CACHE_PATH);
    info->CAPI->AddCacheDefinition(mf, "SOME_CACHE_VARIABLE2", "", "Test cache variable 2", CM_CACHE_FILEPATH);
    info->CAPI->AddCacheDefinition(mf, "SOME_CACHE_VARIABLE3", "", "Test cache variable 3", CM_CACHE_STRING);
    info->CAPI->AddCacheDefinition(mf, "SOME_CACHE_VARIABLE4", "", "Test cache variable 4", CM_CACHE_INTERNAL);
    info->CAPI->AddCacheDefinition(mf, "SOME_CACHE_VARIABLE5", "", "Test cache variable 5", CM_CACHE_STATIC);
    ...
    sprintf(buffer, "Cache version: %d.%d, CMake version: %d.%d", info->CAPI->GetCacheMajorVersion(mf), info->CAPI->GetCacheMinorVersion(mf), info->CAPI->GetMajorVersion(mf), info->CAPI->GetMinorVersion(mf));
    ...
    info->CAPI->DisplaySatus(mf, buffer);
    info->CAPI->DefineSourceFileProperty(mf, "SOME_PROPERTY", "unused old prop", "This property is no longer used", 0);
    ...
    info->CAPI->DefineSourceFileProperty(mf, "SOME_PROPERTY2", "nice prop", "This property is for testing.", 0);
    ...
    ccDep[0] = ccArgs[2];
    ccOut[0] = ccArgs[3];
    info->CAPI->AddCustomCommand(mf, "LoadedCommand.cxx.in", file, 4, ccArgs, 1, ccDep, 1, ccOut, "LoadedCommand");

    ccArgs[2] = argv[1];
    ccArgs[3] = argv[2];
    ccDep[0] = ccArgs[2];
    ccOut[0] = ccArgs[3];
    info->CAPI->AddCustomCommandToOutput(mf, ccOut[0], file, 4, ccArgs, ccDep[0], 0, 0);

    ccArgs[1] = "echo";
    ccArgs[2] = "Build has finished";
    info->CAPI->AddCustomCommandToTarget(mf, "LoadedCommand", file, 3, ccArgs, CM_POST_BUILD);
...
static void CCONV FinalPass(void *inf, void *mf)
{
    ...
    if (strcmp(info->CAPI->GetDefinition(mf, "TEST_DEF"), "HOO") || strcmp(cdata->LibraryName, "HOO"))
    {
