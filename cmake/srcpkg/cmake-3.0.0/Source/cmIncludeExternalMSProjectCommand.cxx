...
bool cmIncludeExternalMSProjectCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 2)
    {
...
// only compile this for win32 to avoid coverage errors
    ...
    if (this->Makefile->GetDefinition("WIN32"))
    {
        ...
        if (!customGuid.empty())
        {
            ...
            this->Makefile->GetCMakeInstance()->AddCacheEntry(guidVariable.c_str(), customGuid.c_str(), "Stored GUID", cmCacheManager::INTERNAL);
        }
        ...
        for (std::vector < std::string >::const_iterator it = depends.begin(); it != depends.end(); ++it)
        {
