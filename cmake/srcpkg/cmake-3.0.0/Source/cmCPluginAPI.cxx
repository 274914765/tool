...
/* Add a definition to this makefile and the global cmake cache. */
    void CCONV cmAddCacheDefinition(void *arg, const char *name, const char *value, const char *doc, int type)
    {
        cmMakefile *mf = static_cast < cmMakefile * >(arg);
        ...
        {
            case CM_CACHE_BOOL:
                mf->AddCacheDefinition(name, value, doc, cmCacheManager::BOOL);
                break;
            case CM_CACHE_PATH:
                mf->AddCacheDefinition(name, value, doc, cmCacheManager::PATH);
                break;
            case CM_CACHE_FILEPATH:
                mf->AddCacheDefinition(name, value, doc, cmCacheManager::FILEPATH);
                break;
            case CM_CACHE_STRING:
                mf->AddCacheDefinition(name, value, doc, cmCacheManager::STRING);
                break;
            case CM_CACHE_INTERNAL:
                mf->AddCacheDefinition(name, value, doc, cmCacheManager::INTERNAL);
                break;
            case CM_CACHE_STATIC:
                mf->AddCacheDefinition(name, value, doc, cmCacheManager::STATIC);
                break;
        }
    }
...
/** Check if a command exists. */
    ...
    void CCONV cmAddExecutable(void *arg, const char *exename, int numSrcs, const char **srcs, int win32)
    {
        cmMakefile *mf = static_cast < cmMakefile * >(arg);
    ...
    void CCONV cmAddUtilityCommand(void *arg, const char *utilityName, const char *command, const char *arguments, int all, int numDepends, const char **depends, int, const char **)
    {
        ...
        mf->AddUtilityCommand(utilityName, (all ? false : true), 0, depends2, commandLines);
    }
    void CCONV cmAddCustomCommand(void *arg, const char *source, const char *command, int numArgs, const char **args, int numDepends, const char **depends, int numOutputs, const char **outputs, const char *target)
    {
        // Get the makefile instance.  Perform an extra variable expansion
        // now because the API caller expects it.
        cmMakefile *mf = static_cast < cmMakefile * >(arg);
        ...
        mf->AddCustomCommandOldStyle(target, outputs2, depends2, source, commandLines, no_comment);
    }

    void CCONV cmAddCustomCommandToOutput(void *arg, const char *output, const char *command, int numArgs, const char **args, const char *main_dependency, int numDepends, const char **depends)
    {
        // Get the makefile instance.  Perform an extra variable expansion
        // now because the API caller expects it.
        cmMakefile *mf = static_cast < cmMakefile * >(arg);
        ...
        mf->AddCustomCommandToOutput(output, depends2, main_dependency, commandLines, no_comment, no_working_dir);
    }

    void CCONV cmAddCustomCommandToTarget(void *arg, const char *target, const char *command, int numArgs, const char **args, int commandType)
    {
        ...
        const char *no_comment = 0;
        const char *no_working_dir = 0;
        mf->AddCustomCommandToTarget(target, no_depends, commandLines, cctype, no_comment, no_working_dir);
    }

    void CCONV cmAddLinkLibraryForTarget(void *arg, const char *tgt, const char *value, int libtype)
    {
        cmMakefile *mf = static_cast < cmMakefile * >(arg);
    ...
    void CCONV cmAddLibrary(void *arg, const char *libname, int shared, int numSrcs, const char **srcs)
    {
        cmMakefile *mf = static_cast < cmMakefile * >(arg);
        std::vector < std::string > srcs2;
        int i;
        ...
        mf->AddLibrary(libname, (shared ? cmTarget::SHARED_LIBRARY : cmTarget::STATIC_LIBRARY), srcs2);
    }

    char CCONV *cmExpandVariablesInString(void *arg, const char *source, int escapeQuotes, int atOnly)
    {
        cmMakefile *mf = static_cast < cmMakefile * >(arg);
        std::string barf = source;
        std::string result = mf->ExpandVariablesInString(barf, (escapeQuotes ? true : false), (atOnly ? true : false));
    ...
    int CCONV cmExecuteCommand(void *arg, const char *name, int numArgs, const char **args)
    {
        cmMakefile *mf = static_cast < cmMakefile * >(arg);
        ...
        for (int i = 0; i < numArgs; ++i)
        {
            ...
            lff.Arguments.push_back(cmListFileArgument(args[i], cmListFileArgument::Quoted, "[CMake-Plugin]", 0));
        }
        cmExecutionStatus status;
        return mf->ExecuteCommand(lff, status);
    }

    void CCONV cmExpandSourceListArguments(void *arg, int numArgs, const char **args, int *resArgc, char ***resArgv, unsigned int startArgumentIndex)
    {
        cmMakefile *mf = static_cast < cmMakefile * >(arg);
...
// the CPluginAPI proxy source file.
    class cmCPluginAPISourceFileMap:public std::map < cmSourceFile *, cmCPluginAPISourceFile * >
    {
public:
    ...
    void CCONV *cmGetSource(void *arg, const char *name)
    {
        ...
        if (cmSourceFile * rsf = mf->GetSource(name))
        {
            ...
            if (i == cmCPluginAPISourceFiles.end())
            {
                ...
                sf->SourceName = cmSystemTools::GetFilenameWithoutLastExtension(sf->FullPath.c_str());
                sf->SourceExtension = cmSystemTools::GetFilenameLastExtension(sf->FullPath.c_str());
    ...
    void *CCONV cmAddSource(void *arg, void *arg2)
    {
        ...
        for (std::vector < std::string >::iterator i = osf->Depends.begin(); i != osf->Depends.end(); ++i)
        {
    ...
    const char *CCONV cmSourceFileGetProperty(void *arg, const char *prop)
    {
        ...
        else
        {
            ...
            return sf->Properties.GetPropertyValue(prop, cmProperty::SOURCE_FILE, chain);
        }
    }
    ...
    void CCONV cmSourceFileSetProperty(void *arg, const char *prop, const char *value)
    {
        cmCPluginAPISourceFile *sf = static_cast < cmCPluginAPISourceFile * >(arg);
        if (cmSourceFile * rsf = sf->RealSourceFile)
        {
    ...
    void CCONV cmSourceFileSetName(void *arg, const char *name, const char *dir, int numSourceExtensions, const char **sourceExtensions, int numHeaderExtensions, const char **headerExtensions)
    {
        cmCPluginAPISourceFile *sf = static_cast < cmCPluginAPISourceFile * >(arg);
        if (sf->RealSourceFile)
        {
        ...
        for (std::vector < std::string >::const_iterator ext = sourceExts.begin(); ext != sourceExts.end(); ++ext)
        {
            hname = pathname;
            hname += ".";
            hname += *ext;
    ...
    void CCONV cmSourceFileSetName2(void *arg, const char *name, const char *dir, const char *ext, int headerFileOnly)
    {
        cmCPluginAPISourceFile *sf = static_cast < cmCPluginAPISourceFile * >(arg);
        if (sf->RealSourceFile)
        {
        ...
        if (headerFileOnly)
        {
            sf->Properties.SetProperty("HEADER_FILE_ONLY", "1", cmProperty::SOURCE_FILE);
        }
        sf->SourceName = name;
    ...
    void CCONV DefineSourceFileProperty(void *arg, const char *name, const char *briefDocs, const char *longDocs, int chained)
    {
        cmMakefile *mf = static_cast < cmMakefile * >(arg);
        mf->GetCMakeInstance()->DefineProperty(name, cmProperty::SOURCE_FILE, briefDocs, longDocs, chained != 0);
    }
