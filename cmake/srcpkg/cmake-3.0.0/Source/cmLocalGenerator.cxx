...
void cmLocalGenerator::ComputeObjectMaxPath()
{
    ...
    if (plen && *plen)
    {
        ...
        if (sscanf(plen, "%u", &pmax) == 1)
        {
            ...
            {
                cmOStringStream w;
                w << "CMAKE_OBJECT_PATH_MAX is set to " << pmax << ", which is less than the minimum of 128.  " << "The value will be ignored.";
        ...
        {
            cmOStringStream w;
            w << "CMAKE_OBJECT_PATH_MAX is set to \"" << plen << "\", which fails to parse as a positive integer.  " << "The value will be ignored.";
            this->Makefile->IssueMessage(cmake::AUTHOR_WARNING, w.str());
        }
    }
...
void cmLocalGenerator::SetupPathConversions()
{
    ...
    // Convert
    std::string outdir;
    outdir = cmSystemTools::CollapseFullPath(this->Makefile->GetHomeDirectory());
    cmSystemTools::SplitPath(outdir.c_str(), this->HomeDirectoryComponents);
    outdir = cmSystemTools::CollapseFullPath(this->Makefile->GetStartDirectory());
    cmSystemTools::SplitPath(outdir.c_str(), this->StartDirectoryComponents);

    outdir = cmSystemTools::CollapseFullPath(this->Makefile->GetHomeOutputDirectory());
    cmSystemTools::SplitPath(outdir.c_str(), this->HomeOutputDirectoryComponents);

    outdir = cmSystemTools::CollapseFullPath(this->Makefile->GetStartOutputDirectory());
...
void cmLocalGenerator::SetGlobalGenerator(cmGlobalGenerator * gg)
{
    ...
    this->Makefile->SetHomeDirectory(gg->GetCMakeInstance()->GetHomeDirectory());
    this->Makefile->SetHomeOutputDirectory(gg->GetCMakeInstance()->GetHomeOutputDirectory());
}
...
void cmLocalGenerator::TraceDependencies()
{
    ...
    {
        if (t->second->Target->IsImported() || t->second->Target->GetType() == cmTarget::INTERFACE_LIBRARY)
        {
            continue;
        }
...
void cmLocalGenerator::GenerateTestFiles()
{
    ...
    const char *config = this->Makefile->GetConfigurations(configurationTypes, false);
    ...
    fout << "# CMake generated Testfile for " << std::endl
        ...
        << "# " << std::endl << "# This file includes the relevant testing commands " << "required for " << std::endl << "# testing this directory and lists subdirectories to " << "be tested as well." << std::endl;
    ...
    std::vector < cmTestGenerator * >const & testers = this->Makefile->GetTestGenerators();
    for (std::vector < cmTestGenerator * >::const_iterator gi = testers.begin(); gi != testers.end(); ++gi)
    {
    ...
    if (this->Children.size())
    {
        ...
        for (i = 0; i < this->Children.size(); ++i)
        {
            ...
            fout << "subdirs(";
            std::string outP = this->Children[i]->GetMakefile()->GetStartOutputDirectory();
...
void cmLocalGenerator::GenerateInstallRules()
{
...
#endif
    if (const char *stagingPrefix = this->Makefile->GetDefinition("CMAKE_STAGING_PREFIX"))
    {
    ...
    {
        for (std::vector < std::string >::iterator i = configurationTypes.begin(); i != configurationTypes.end(); ++i)
        {
    ...
    // Write the header.
    fout << "# Install script for directory: " << this->Makefile->GetCurrentDirectory() << std::endl << std::endl;
    fout << "# Set the install prefix" << std::endl
        << "if(NOT DEFINED CMAKE_INSTALL_PREFIX)" << std::endl
        << "  set(CMAKE_INSTALL_PREFIX \"" << prefix << "\")" << std::endl << "endif()" << std::endl << "string(REGEX REPLACE \"/$\" \"\" CMAKE_INSTALL_PREFIX " << "\"${CMAKE_INSTALL_PREFIX}\")" << std::endl << std::endl;
    ...
    if (const char *so_no_exe = this->Makefile->GetDefinition("CMAKE_INSTALL_SO_NO_EXE"))
    {
        fout << "# Install shared libraries without execute permission?\n" "if(NOT DEFINED CMAKE_INSTALL_SO_NO_EXE)\n" "  set(CMAKE_INSTALL_SO_NO_EXE \"" << so_no_exe << "\")\n" "endif()\n" "\n";
    }

    // Ask each install generator to write its code.
    std::vector < cmInstallGenerator * >const &installers = this->Makefile->GetInstallGenerators();
    for (std::vector < cmInstallGenerator * >::const_iterator gi = installers.begin(); gi != installers.end(); ++gi)
    {
        (*gi)->Generate(fout, config, configurationTypes);
    }
    ...
    if (!this->Children.empty())
    {
        ...
        for (std::vector < cmLocalGenerator * >::const_iterator ci = this->Children.begin(); ci != this->Children.end(); ++ci)
        {
            if (!(*ci)->GetMakefile()->GetPropertyAsBool("EXCLUDE_FROM_ALL"))
            {
                std::string odir = (*ci)->GetMakefile()->GetStartOutputDirectory();
                cmSystemTools::ConvertToUnixSlashes(odir);
                fout << "  include(\"" << odir.c_str() << "/cmake_install.cmake\")" << std::endl;
            }
        }
        fout << "\n";
        fout << "endif()\n\n";
    ...
    if (toplevel_install)
    {
        fout << "if(CMAKE_INSTALL_COMPONENT)\n" "  set(CMAKE_INSTALL_MANIFEST \"install_manifest_" "${CMAKE_INSTALL_COMPONENT}.txt\")\n" "else()\n" "  set(CMAKE_INSTALL_MANIFEST \"install_manifest.txt\")\n" "endif()\n\n";
        fout << "file(WRITE \"" << homedir.c_str() << "/${CMAKE_INSTALL_MANIFEST}\" " << "\"\")" << std::endl;
        fout << "foreach(file ${CMAKE_INSTALL_MANIFEST_FILES})" << std::endl << "  file(APPEND \"" << homedir.c_str() << "/${CMAKE_INSTALL_MANIFEST}\" " << "\"${file}\\n\")" << std::endl << "endforeach()" << std::endl;
    }
}
...
void cmLocalGenerator::AddCustomCommandToCreateObject(const char *ofname, const char *lang, cmSourceFile & source, cmGeneratorTarget & target)
{
    ...
    std::string sourceFile = this->Convert(source.GetFullPath().c_str(), START_OUTPUT, SHELL, true);
    ...
    for (std::vector < std::string >::iterator i = commands.begin(); i != commands.end(); ++i)
    {
        ...
        for (std::vector < cmStdString >::iterator a = cmd.begin(); a != cmd.end(); ++a)
        {
...
void cmLocalGenerator::AddBuildTargetRule(const char *llang, cmGeneratorTarget & target)
{
    cmStdString objs;
    ...
    {
        cmSourceFile *sf = *i;
        if (!sf->GetCustomCommand() && !sf->GetPropertyAsBool("HEADER_FILE_ONLY") && !sf->GetPropertyAsBool("EXTERNAL_OBJECT"))
        {
            std::string dir_max;
    ...
    this->GetTargetFlags(linkLibs, frameworkPath, linkPath, flags, linkFlags, &target);
    ...
    this->Makefile->AddCustomCommandToOutput(targetFullPath.c_str(), objVector, 0, commandLines, comment.c_str(), this->Makefile->GetStartOutputDirectory());
    target.Target->AddSourceFile(this->Makefile->GetSource(targetFullPath.c_str()));
}
            ...
            case cmTarget::EXECUTABLE:
                {
                    ...
                    if (!llang)
                    {
                        cmSystemTools::Error("CMake can not determine linker language for target: ", target.Target->GetName());
                        return;
                    }
...
std::string cmLocalGenerator::ExpandRuleVariable(std::string const &variable, const RuleVariables & replaceValues)
{
    ...
    {
        const char *cmcommand = this->GetMakefile()->GetDefinition("CMAKE_COMMAND");
    ...
    {
        for (std::vector < std::string >::iterator i = enabledLanguages.begin(); i != enabledLanguages.end(); ++i)
        {
            ...
            if (actualReplace == "CMAKE_${LANG}_COMPILER")
            {
                ...
                compilerTarget = this->Makefile->GetDefinition((std::string("CMAKE_") + lang + "_COMPILER_TARGET").c_str());
                compilerOptionTarget = this->Makefile->GetDefinition((std::string("CMAKE_") + lang + "_COMPILE_OPTIONS_TARGET").c_str());
                compilerExternalToolchain = this->Makefile->GetDefinition((std::string("CMAKE_") + lang + "_COMPILER_EXTERNAL_TOOLCHAIN").c_str());
                compilerOptionExternalToolchain = this->Makefile->GetDefinition((std::string("CMAKE_") + lang + "_COMPILE_OPTIONS_EXTERNAL_TOOLCHAIN").c_str());
                compilerSysroot = this->Makefile->GetDefinition("CMAKE_SYSROOT");
                compilerOptionSysroot = this->Makefile->GetDefinition((std::string("CMAKE_") + lang + "_COMPILE_OPTIONS_SYSROOT").c_str());
            }
...
void cmLocalGenerator::ExpandRuleVariables(std::string & s, const RuleVariables & replaceValues)
{
    this->InsertRuleLauncher(s, replaceValues.CMTarget, replaceValues.RuleLauncher);
...
//----------------------------------------------------------------------------
const char *cmLocalGenerator::GetRuleLauncher(cmTarget * target, const char *prop)
{
    if (target)
    {
...
//----------------------------------------------------------------------------
void cmLocalGenerator::InsertRuleLauncher(std::string & s, cmTarget * target, const char *prop)
{
...
//----------------------------------------------------------------------------
std::string cmLocalGenerator::ConvertToOutputForExistingCommon(const char *remote, std::string const &result)
{
    // If this is a windows shell, the result has a space, and the path
    // already exists, we can use a short-path to reference it without a
    // space.
    if (this->WindowsShell && result.find(' ') != result.npos && cmSystemTools::FileExists(remote))
    {
        std::string tmp;
...
//----------------------------------------------------------------------------
std::string cmLocalGenerator::ConvertToOutputForExisting(const char *remote, RelativeRoot local)
{
...
//----------------------------------------------------------------------------
std::string cmLocalGenerator::ConvertToOutputForExisting(RelativeRoot remote, const char *local)
{
    // Perform standard conversion.
    std::string result = this->Convert(remote, local, SHELL, true);
...
//----------------------------------------------------------------------------
std::string cmLocalGenerator::ConvertToIncludeReference(std::string const &path)
{
    return this->ConvertToOutputForExisting(path.c_str());
}

//----------------------------------------------------------------------------
std::string cmLocalGenerator::GetIncludeFlags(const std::vector < std::string > &includes, cmGeneratorTarget * target, const char *lang, bool forResponseFile, const char *config)
{
    if (!lang)
    {
        return "";
    ...
    flagVar += lang;
    const char *includeFlag = this->Makefile->GetSafeDefinition(flagVar.c_str());
    ...
    {
        if (fwSearchFlag && *fwSearchFlag && this->Makefile->IsOn("APPLE") && cmSystemTools::IsPathToFramework(i->c_str()))
        {
            ...
            if (emitted.insert(frameworkDir).second)
            {
                ...
                includeFlags << fwSearchFlag << this->Convert(frameworkDir.c_str(), START_OUTPUT, format, true) << " ";
            }
            continue;
        }
        ...
        if (forResponseFile)
        {
            includePath = this->Convert(i->c_str(), START_OUTPUT, RESPONSE, true);
        }
        else
        {
...
//----------------------------------------------------------------------------
void cmLocalGenerator::AddCompileDefinitions(std::set < std::string > &defines, cmTarget const *target, const char *config)
{
...
//----------------------------------------------------------------------------
void cmLocalGenerator::AddCompileOptions(std::string & flags, cmTarget * target, const char *lang, const char *config)
{
    std::string langFlagRegexVar = std::string("CMAKE_") + lang + "_FLAG_REGEX";
    if (const char *langFlagRegexStr = this->Makefile->GetDefinition(langFlagRegexVar.c_str()))
    {
...
//----------------------------------------------------------------------------
void cmLocalGenerator::GetIncludeDirectories(std::vector < std::string > &dirs, cmGeneratorTarget * target, const char *lang, const char *config, bool stripImplicitInclDirs)
{
    ...
    if (includeBinaryDir)
    {
        if (emitted.find(this->Makefile->GetStartOutputDirectory()) == emitted.end())
        {
    ...
    if (const char *value = this->Makefile->GetDefinition(impDirVar.c_str()))
    {
        ...
        for (std::vector < std::string >::const_iterator i = impDirVec.begin(); i != impDirVec.end(); ++i)
        {
    ...
    if (this->Makefile->IsOn("CMAKE_INCLUDE_DIRECTORIES_PROJECT_BEFORE"))
    {
        ...
        for (std::vector < std::string >::const_iterator i = includes.begin(); i != includes.end(); ++i)
        {
            // Emit this directory only if it is a subdirectory of the
            // top-level source or binary tree.
            if (cmSystemTools::ComparePath(i->c_str(), topSourceDir) || cmSystemTools::ComparePath(i->c_str(), topBinaryDir) || cmSystemTools::IsSubDirectory(i->c_str(), topSourceDir) || cmSystemTools::IsSubDirectory(i->c_str(), topBinaryDir))
            {
    ...
    for (std::vector < std::string >::const_iterator i = implicitDirs.begin(); i != implicitDirs.end(); ++i)
    {
...
void cmLocalGenerator::GetStaticLibraryFlags(std::string & flags, std::string const &config, cmTarget * target)
{
    this->AppendFlags(flags, this->Makefile->GetSafeDefinition("CMAKE_STATIC_LINKER_FLAGS"));
    if (!config.empty())
    {
...
void cmLocalGenerator::GetTargetFlags(std::string & linkLibs, std::string & flags, std::string & linkFlags, std::string & frameworkPath, std::string & linkPath, cmGeneratorTarget * target)
{
                ...
                if (this->Makefile->IsOn("WIN32") && !(this->Makefile->IsOn("CYGWIN") || this->Makefile->IsOn("MINGW")))
                {
                    ...
                    for (std::vector < cmSourceFile * >::const_iterator i = sources.begin(); i != sources.end(); ++i)
                    {
                        ...
                        if (sf->GetExtension() == "def")
                        {
                            ...
                            linkFlags += this->Convert(sf->GetFullPath().c_str(), FULL, SHELL);
                            linkFlags += " ";
                        }
                    }
                }
        ...
        case cmTarget::EXECUTABLE:
            {
                ...
                {
                    std::string sFlagVar = std::string("CMAKE_SHARED_BUILD_") + linkLanguage + std::string("_FLAGS");
                ...
                if (target->Target->IsExecutableWithExports())
                {
                    ...
                    linkFlags += this->Makefile->GetSafeDefinition(exportFlagVar.c_str());
                    linkFlags += " ";
                }
                const char *targetLinkFlags = target->GetProperty("LINK_FLAGS");
                if (targetLinkFlags)
 ...
 */
void cmLocalGenerator::OutputLinkLibraries(std::string & linkLibraries, std::string & frameworkPath, std::string & linkPath, cmGeneratorTarget & tgt, bool relink)
{
    ...
    std::string libPathTerminator = this->Makefile->GetSafeDefinition("CMAKE_LIBRARY_PATH_TERMINATOR");
    ...
    if (cli.GetRuntimeSep().empty())
    {
        ...
        std::string rpath;
        for (std::vector < std::string >::iterator ri = runtimeDirs.begin(); ri != runtimeDirs.end(); ++ri)
        {
...
//----------------------------------------------------------------------------
void cmLocalGenerator::AddArchitectureFlags(std::string & flags, cmGeneratorTarget * target, const char *lang, const char *config)
{
    ...
    if (this->EmitUniversalBinaryFlags)
    {
        ...
        std::string sysrootFlagVar = std::string("CMAKE_") + lang + "_SYSROOT_FLAG";
        const char *sysrootFlag = this->Makefile->GetDefinition(sysrootFlagVar.c_str());
        const char *deploymentTarget = this->Makefile->GetDefinition("CMAKE_OSX_DEPLOYMENT_TARGET");
        std::string deploymentTargetFlagVar = std::string("CMAKE_") + lang + "_OSX_DEPLOYMENT_TARGET_FLAG";
        const char *deploymentTargetFlag = this->Makefile->GetDefinition(deploymentTargetFlagVar.c_str());
        ...
        if (deploymentTargetFlag && *deploymentTargetFlag && deploymentTarget && *deploymentTarget)
        {
            flags += " ";
...
//----------------------------------------------------------------------------
void cmLocalGenerator::AddLanguageFlags(std::string & flags, const char *lang, const char *config)
{
...
//----------------------------------------------------------------------------
bool cmLocalGenerator::GetRealDependency(const char *inName, const char *config, std::string & dep)
{
    ...
    if (cmTarget * target = this->Makefile->FindTargetToUse(name))
    {
        ...
        if (cmSystemTools::FileIsFullPath(inName))
        {
            ...
            if (target->GetType() >= cmTarget::EXECUTABLE && target->GetType() <= cmTarget::MODULE_LIBRARY)
            {
                tLocation = target->GetLocation(config);
                tLocation = cmSystemTools::GetFilenamePath(tLocation);
                tLocation = cmSystemTools::CollapseFullPath(tLocation.c_str());
            }
            std::string depLocation = cmSystemTools::GetFilenamePath(std::string(inName));
...
//----------------------------------------------------------------------------
void cmLocalGenerator::AddSharedFlags(std::string & flags, const char *lang, bool shared)
{
    std::string flagsVar;
...
static void AddVisibilityCompileOption(std::string & flags, cmTarget * target, cmLocalGenerator * lg, const char *lang)
{
    std::string l(lang);
    ...
    if (strcmp(prop, "hidden") != 0 && strcmp(prop, "default") != 0 && strcmp(prop, "protected") != 0 && strcmp(prop, "internal") != 0)
    {
        cmOStringStream e;
        e << "Target " << target->GetName() << " uses unsupported value \"" << prop << "\" for " << flagDefine << ".";
...
static void AddInlineVisibilityCompileOption(std::string & flags, cmTarget * target, cmLocalGenerator * lg)
{
    std::string compileOption = "CMAKE_CXX_COMPILE_OPTIONS_VISIBILITY_INLINES_HIDDEN";
    const char *opt = lg->GetMakefile()->GetDefinition(compileOption.c_str());
    if (!opt)
    {
        return;
...
//----------------------------------------------------------------------------
void cmLocalGenerator::AddVisibilityPresetFlags(std::string & flags, cmTarget * target, const char *lang)
{
    int targetType = target->GetType();
    bool suitableTarget = ((targetType == cmTarget::SHARED_LIBRARY) || (targetType == cmTarget::MODULE_LIBRARY) || (target->IsExecutableWithExports()));
...
//----------------------------------------------------------------------------
void cmLocalGenerator::AddCMP0018Flags(std::string & flags, cmTarget * target, std::string const &lang, const char *config)
{
    int targetType = target->GetType();

    bool shared = ((targetType == cmTarget::SHARED_LIBRARY) || (targetType == cmTarget::MODULE_LIBRARY));
    ...
    else
    {
        ...
        if (target->GetLinkInterfaceDependentBoolProperty("POSITION_INDEPENDENT_CODE", config))
        {
            this->AddPositionIndependentFlags(flags, lang, targetType);
        }
        if (shared)
...
//----------------------------------------------------------------------------
bool cmLocalGenerator::GetShouldUseOldFlags(bool shared, const std::string & lang) const
{
    std::string originalFlags = this->GlobalGenerator->GetSharedLibFlagsForLanguage(lang);
    if (shared)
    {
        std::string flagsVar = "CMAKE_SHARED_LIBRARY_";
        flagsVar += lang;
        flagsVar += "_FLAGS";
        const char *flags = this->Makefile->GetSafeDefinition(flagsVar.c_str());
                ...
                case cmPolicies::WARN:
                    {
                        ...
                        e << "Variable " << flagsVar << " has been modified. CMake "
                            ...
                            "instead.  This may cause errors if the original content of " << flagsVar << " was removed.\n" << this->Makefile->GetPolicies()->GetPolicyWarning(cmPolicies::CMP0018);
...
//----------------------------------------------------------------------------
void cmLocalGenerator::AddPositionIndependentFlags(std::string & flags, std::string const &lang, int targetType)
{
    ...
    if (picFlags)
    {
        ...
        for (std::vector < std::string >::const_iterator oi = options.begin(); oi != options.end(); ++oi)
        {
...
//----------------------------------------------------------------------------
void cmLocalGenerator::AddConfigVariableFlags(std::string & flags, const char *var, const char *config)
{
...
//----------------------------------------------------------------------------
void cmLocalGenerator::AppendFlagEscape(std::string & flags, const char *rawFlag)
{
    this->AppendFlags(flags, this->EscapeForShell(rawFlag).c_str());
}

//----------------------------------------------------------------------------
void cmLocalGenerator::AppendDefines(std::set < std::string > &defines, const char *defines_list)
{
...
//----------------------------------------------------------------------------
void cmLocalGenerator::AppendDefines(std::set < std::string > &defines, const std::vector < std::string > &defines_vec)
{
    for (std::vector < std::string >::const_iterator di = defines_vec.begin(); di != defines_vec.end(); ++di)
    {
...
//----------------------------------------------------------------------------
void cmLocalGenerator::JoinDefines(const std::set < std::string > &defines, std::string & definesString, const char *lang)
{
...
//----------------------------------------------------------------------------
void cmLocalGenerator::AppendFeatureOptions(std::string & flags, const char *lang, const char *feature)
{
...
//----------------------------------------------------------------------------
std::string cmLocalGenerator::ConstructComment(const cmCustomCommand & cc, const char *default_comment)
{
    ...
    if (!cc.GetOutputs().empty())
    {
        ...
        comment = "Generating ";
        const char *sep = "";
        for (std::vector < std::string >::const_iterator o = cc.GetOutputs().begin(); o != cc.GetOutputs().end(); ++o)
        {
            comment += sep;
...
//----------------------------------------------------------------------------
std::string cmLocalGenerator::ConvertToOptionallyRelativeOutputPath(const char *remote)
{
...
const char *cmLocalGenerator::GetRelativeRootPath(RelativeRoot relroot)
{
    ...
    {
        case HOME:
            return this->Makefile->GetHomeDirectory();
        case START:
            return this->Makefile->GetStartDirectory();
        case HOME_OUTPUT:
            return this->Makefile->GetHomeOutputDirectory();
        case START_OUTPUT:
...
//----------------------------------------------------------------------------
std::string cmLocalGenerator::Convert(const char *source, RelativeRoot relative, OutputFormat output, bool optional)
{
    ...
    if (!optional || this->UseRelativePaths)
    {
        switch (relative)
        {
            case HOME:
                ...
                result = this->ConvertToRelativePath(this->HomeDirectoryComponents, result.c_str());
                break;
            case START:
                //result = cmSystemTools::CollapseFullPath(result.c_str());
                result = this->ConvertToRelativePath(this->StartDirectoryComponents, result.c_str());
                break;
            case HOME_OUTPUT:
                //result = cmSystemTools::CollapseFullPath(result.c_str());
                result = this->ConvertToRelativePath(this->HomeOutputDirectoryComponents, result.c_str());
                break;
            case START_OUTPUT:
                //result = cmSystemTools::CollapseFullPath(result.c_str());
                result = this->ConvertToRelativePath(this->StartOutputDirectoryComponents, result.c_str());
                break;
            case FULL:
                result = cmSystemTools::CollapseFullPath(result.c_str());
                break;
...
//----------------------------------------------------------------------------
std::string cmLocalGenerator::ConvertToOutputFormat(const char *source, OutputFormat output)
{
    std::string result = source;
...
//----------------------------------------------------------------------------
std::string cmLocalGenerator::Convert(RelativeRoot remote, const char *local, OutputFormat output, bool optional)
{
    const char *remotePath = this->GetRelativeRootPath(remote);
...
std::string cmLocalGenerator::FindRelativePathTopSource()
{
    ...
    if (cmLocalGenerator * parent = this->GetParent())
    {
        ...
        if (cmSystemTools::IsSubDirectory(this->Makefile->GetStartDirectory(), parentTop.c_str()))
        {
            return parentTop;
        }
    }
...
std::string cmLocalGenerator::FindRelativePathTopBinary()
{
    ...
    if (cmLocalGenerator * parent = this->GetParent())
    {
        ...
        if (cmSystemTools::IsSubDirectory(this->Makefile->GetStartOutputDirectory(), parentTop.c_str()))
        {
            return parentTop;
        }
    }
...
{
    return (cmSystemTools::ComparePath(a, b) || cmSystemTools::IsSubDirectory(a, b));
}

//----------------------------------------------------------------------------
std::string cmLocalGenerator::ConvertToRelativePath(const std::vector < std::string > &local, const char *in_remote, bool force)
{
    ...
    if (!force)
    {
        ...
        if (!((cmLocalGeneratorNotAbove(local_path.c_str(),
               ...
               cmLocalGeneratorNotAbove(in_remote,
                                        this->RelativePathTopBinary.c_str())) || (cmLocalGeneratorNotAbove(local_path.c_str(), this->RelativePathTopSource.c_str()) && cmLocalGeneratorNotAbove(in_remote, this->RelativePathTopSource.c_str()))))
        {
            return in_remote;
        }
    }
    ...
    while (common < remote.size() && common < local.size() && cmSystemTools::ComparePath(remote[common].c_str(), local[common].c_str()))
    {
        ++common;
    }
    ...
    // just return a "./".
    if (common + 1 == remote.size() && remote[common].size() == 0 && common == local.size())
    {
        return "./";
    }
...
//----------------------------------------------------------------------------
void cmLocalGenerator::GenerateTargetInstallRules(std::ostream & os, const char *config, std::vector < std::string > const &configurationTypes)
{
...
static bool cmLocalGeneratorShortenObjectName(std::string & objName, std::string::size_type max_len)
{
...
static bool cmLocalGeneratorCheckObjectName(std::string & objName, std::string::size_type dir_len, std::string::size_type max_total_len)
{
...
//----------------------------------------------------------------------------
std::string & cmLocalGenerator::CreateSafeUniqueObjectFileName(const char *sin, std::string const &dir_max)
{
    ...
    if (it == this->UniqueObjectNamesMap.end())
    {
        ...
        if (this->Makefile->IsOn("CMAKE_MANGLE_OBJECT_FILE_NAMES"))
        {
            ...
            do
            {
                ...
                for (it = this->UniqueObjectNamesMap.begin(); it != this->UniqueObjectNamesMap.end(); ++it)
                {
                    if (it->second == ssin)
                    {
                        done = false;
...
#if defined(CM_LG_ENCODE_OBJECT_NAMES)
        if (!cmLocalGeneratorCheckObjectName(ssin, dir_max.size(), this->ObjectPathMax))
        {
...
//----------------------------------------------------------------------------
std::string cmLocalGenerator::GetObjectFileNameWithoutTarget(const cmSourceFile & source, std::string const &dir_max, bool * hasSourceExtension)
{
...
//----------------------------------------------------------------------------
const char *cmLocalGenerator::GetSourceFileLanguage(const cmSourceFile & source)
{
...
static bool cmLocalGeneratorIsShellOperator(const char *str)
{
    if (strcmp(str, "<") == 0 ||
        strcmp(str, ">") == 0 ||
        strcmp(str, "<<") == 0 ||
        strcmp(str, "> >") == 0 || strcmp(str, "|") == 0 || strcmp(str, "||") == 0 || strcmp(str, "&&") == 0 || strcmp(str, "&>") == 0 || strcmp(str, "1>") == 0 || strcmp(str, "2>") == 0 || strcmp(str, "2>&1") == 0 || strcmp(str, "1>&2") == 0)
    {
        return true;
    }
    return false;
...
//----------------------------------------------------------------------------
std::string cmLocalGenerator::EscapeForShell(const char *str, bool makeVars, bool forEcho)
{
...
//----------------------------------------------------------------------------
cmLocalGenerator::FortranFormat cmLocalGenerator::GetFortranFormat(const char *value)
{
    ...
    if (value && *value)
    {
        ...
        for (std::vector < std::string >::iterator fi = fmt.begin(); fi != fmt.end(); ++fi)
        {
            if (*fi == "FIXED")
            {
...
//----------------------------------------------------------------------------
std::string cmLocalGenerator::GetTargetDirectory(cmTarget const &) const
{
    cmSystemTools::Error("GetTargetDirectory" " called on cmLocalGenerator");
    return "";
}
...
cmIML_INT_uint64_t cmLocalGenerator::GetBackwardsCompatibility()
{
    ...
    if (!this->BackwardsCompatibilityFinal)
    {
        ...
        if (const char *value = this->Makefile->GetDefinition("CMAKE_BACKWARDS_COMPATIBILITY"))
        {
...
//----------------------------------------------------------------------------
void cmLocalGenerator::GenerateAppleInfoPList(cmTarget * target, const char *targetName, const char *fname)
{
    ...
    {
        cmOStringStream e;
        e << "Target " << target->GetName() << " Info.plist template \"" << inFile << "\" could not be found.";
        cmSystemTools::Error(e.str().c_str());
        return;
    }
...
//----------------------------------------------------------------------------
void cmLocalGenerator::GenerateFrameworkInfoPList(cmTarget * target, const char *targetName, const char *fname)
{
    // Find the Info.plist template.
