...
bool cmCreateTestSourceList::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 3)
    {
    ...
    {
        this->SetError("You must specify a file extension for the test driver file.");
        return false;
    }
    ...
    std::string configFile = this->Makefile->GetRequiredDefinition("CMAKE_ROOT");
    ...
    for (i = testsBegin; i != tests.end(); ++i)
    {
        ...
        {
            func_name = cmSystemTools::GetFilenamePath(*i) + "/" + cmSystemTools::GetFilenameWithoutLastExtension(*i);
        }
        else
        {
    ...
    {
        this->Makefile->AddDefinition("CMAKE_TESTDRIVER_EXTRA_INCLUDES", extraInclude.c_str());
    }
    if (function.size())
    {
        this->Makefile->AddDefinition("CMAKE_TESTDRIVER_ARGVC_FUNCTION", function.c_str());
    }
    this->Makefile->AddDefinition("CMAKE_FORWARD_DECLARE_TESTS", forwardDeclareCode.c_str());
    this->Makefile->AddDefinition("CMAKE_FUNCTION_TABLE_ENTIRES", functionMapCode.c_str());
    bool res = true;
    if (!this->Makefile->ConfigureFile(configFile.c_str(), driver.c_str(), false, true, false))
    {
        res = false;
    }
