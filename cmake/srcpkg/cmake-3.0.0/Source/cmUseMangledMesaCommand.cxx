...
bool cmUseMangledMesaCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (this->Disallowed(cmPolicies::CMP0030, "The use_mangled_mesa command should not be called; see CMP0030."))
    {
        return true;
    }
...
void cmUseMangledMesaCommand:: CopyAndFullPathMesaHeader(const char *source, const char *outdir)
{
    std::string dir, file;
    ...
    if (!fout)
    {
        cmSystemTools::Error("Could not open file for write in copy operation: ", tempOutputFile.c_str(), outdir);
    ...
    if (!fin)
    {
        cmSystemTools::Error("Could not open file for read in copy operation", source);
        return;
    }
