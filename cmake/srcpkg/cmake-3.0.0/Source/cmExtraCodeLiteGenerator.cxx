...
//----------------------------------------------------------------------------
void cmExtraCodeLiteGenerator::GetDocumentation(cmDocumentationEntry & entry, const char *) const
{
    entry.Name = this->GetName();
    entry.Brief = "Generates CodeLite project files.";
}

cmExtraCodeLiteGenerator::cmExtraCodeLiteGenerator():cmExternalMakefileProjectGenerator(), ConfigName("NoConfig"), CpuCount(2)
{
#if defined(_WIN32)
...
void cmExtraCodeLiteGenerator::Generate()
{
    ...
    for (std::map < cmStdString, std::vector < cmLocalGenerator * > >::const_iterator it = this->GlobalGenerator->GetProjectMap().begin(); it != this->GlobalGenerator->GetProjectMap().end(); ++it)
    {
        ...
        filename = cmSystemTools::RelativePath(this->WorkspacePath.c_str(), filename.c_str());

        // create a project file
        this->CreateProjectFile(it->second);
        fout << "  <Project Name=\"" << projectName << "\" Path=\"" << filename << "\" Active=\"No\"/>\n";
...
void cmExtraCodeLiteGenerator::CreateProjectFile(const std::vector < cmLocalGenerator * >&lgs)
{
...
void cmExtraCodeLiteGenerator::CreateNewProjectFile(const std::vector < cmLocalGenerator * >&lgs, const std::string & filename)
{
    const cmMakefile *mf = lgs[0]->GetMakefile();
    ...
    std::string projectName = mf->GetProjectName();
    std::string incDirs;
    std::vector < cmValueWithOrigin > incDirsVec = mf->GetIncludeDirectoriesEntries();
    ...
    for (std::map < std::string, cmSourceFile * >::const_iterator sit = cFiles.begin(); sit != cFiles.end(); ++sit)
    {
        std::string relativePath = cmSystemTools::RelativePath(projectPath.c_str(), sit->first.c_str());
    ...
    {
        std::string relativePath = cmSystemTools::RelativePath(projectPath.c_str(), sit->c_str());
        fout << "    <File Name=\"" << relativePath.c_str() << "\"/>\n";
    }
    fout << "  </VirtualDirectory>\n";
    ...
    this->CpuCount = info.GetNumberOfLogicalCPU() * info.GetNumberOfPhysicalCPU();
...
std::string cmExtraCodeLiteGenerator::GetCodeLiteCompilerName(const cmMakefile * mf) const
{
...
std::string cmExtraCodeLiteGenerator::GetConfigurationName(const cmMakefile * mf) const
{
...
std::string cmExtraCodeLiteGenerator::GetBuildCommand(const cmMakefile * mf) const
{
    std::stringstream ss;
...
std::string cmExtraCodeLiteGenerator::GetCleanCommand(const cmMakefile * mf) const
{
    return GetBuildCommand(mf) + " clean";
}

std::string cmExtraCodeLiteGenerator::GetRebuildCommand(const cmMakefile * mf) const
{
    return GetCleanCommand(mf) + cmXMLSafe(" && ").str() + GetBuildCommand(mf);
}

std::string cmExtraCodeLiteGenerator::GetSingleFileBuildCommand(const cmMakefile * mf)
     const
     {
