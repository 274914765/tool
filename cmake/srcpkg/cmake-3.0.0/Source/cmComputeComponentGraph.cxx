...
//----------------------------------------------------------------------------
cmComputeComponentGraph::cmComputeComponentGraph(Graph const &input):InputGraph(input)
{
    // Identify components.
    this->Tarjan();
...
void cmComputeComponentGraph::TarjanVisit(int i)
{
    ...
    for (EdgeList::const_iterator ni = nl.begin(); ni != nl.end(); ++ni)
    {
        ...
        if (this->TarjanVisited[j] > 0 && this->TarjanVisited[j] < this->TarjanWalkId)
        {
            continue;
        }
        ...
        {
            if (this->TarjanEntries[this->TarjanEntries[j].Root].VisitIndex < this->TarjanEntries[this->TarjanEntries[i].Root].VisitIndex)
            {
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        for (EdgeList::const_iterator ni = nl.begin(); ni != nl.end(); ++ni)
        {
            ...
            if (i_component != j_component)
            {
                ...
                this->ComponentGraph[i_component].push_back(cmGraphEdge(j_component, ni->IsStrong()));
            }
        }
    }
}
