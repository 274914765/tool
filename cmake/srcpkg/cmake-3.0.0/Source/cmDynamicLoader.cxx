...
void cmDynamicLoaderCache::CacheFile(const char *path, const cmsys::DynamicLoader::LibraryHandle & p)
{
...
bool cmDynamicLoaderCache::GetCacheFile(const char *path, cmsys::DynamicLoader::LibraryHandle & p)
{
    std::map < cmStdString, cmsys::DynamicLoader::LibraryHandle >::iterator it = this->CacheMap.find(path);
...
{
    for (std::map < cmStdString, cmsys::DynamicLoader::LibraryHandle >::iterator it = this->CacheMap.begin(); it != this->CacheMap.end(); it++)
    {
        cmsys::DynamicLoader::CloseLibrary(it->second);
    }
...
cmsys::DynamicLoader::LibraryHandle cmDynamicLoader::OpenLibrary(const char *libname)
{
