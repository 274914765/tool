...
void cmSystemTools::Error(const char *m1, const char *m2, const char *m3, const char *m4)
{
...
void cmSystemTools::Stdout(const char *s)
{
    if (s_StdoutCallback)
    {
        (*s_StdoutCallback) (s, static_cast < int >(strlen(s)), s_StdoutCallbackClientData);
    }
    else
    {
        std::cout << s;
...
bool cmSystemTools::IsOff(const char *val)
{
    ...
    return (v == "OFF" || v == "0" || v == "NO" || v == "FALSE" || v == "N" || cmSystemTools::IsNOTFOUND(v.c_str()) || v == "IGNORE");
}

//----------------------------------------------------------------------------
void cmSystemTools::ParseWindowsCommandLine(const char *command, std::vector < std::string > &args)
{
...
//----------------------------------------------------------------------------
void cmSystemTools::ParseUnixCommandLine(const char *command, std::vector < std::string > &args)
{
...
//----------------------------------------------------------------------------
void cmSystemTools::ParseUnixCommandLine(const char *command, std::vector < cmStdString > &args)
{
    // Invoke the underlying parser.
    cmSystemToolsArgV argv = cmsysSystem_Parse_CommandForUnix(command, 0);
    argv.Store(args);
...
std::vector < cmStdString > cmSystemTools::ParseArguments(const char *command)
{
    ...
    if ((command[0] != '/' && command[1] == ':' && command[2] == '\\') ||
        (command[0] == '\"' && command[1] != '/' && command[2] == ':' && command[3] == '\\') || (command[0] == '\'' && command[1] != '/' && command[2] == ':' && command[3] == '\\') || (command[0] == '\\' && command[1] == '\\'))
    {
        win_path = true;
    }
...
bool cmSystemTools::RunSingleCommand(std::vector < cmStdString > const &command, std::string * output, int *retVal, const char *dir, OutputOption outputflag, double timeout)
{
    std::vector < std::string > cmd;
    for (std::vector < cmStdString >::const_iterator i = command.begin(); i != command.end(); ++i)
    {
        cmd.push_back(*i);
    }
    return cmSystemTools::RunSingleCommand(cmd, output, retVal, dir, outputflag, timeout);
}

bool cmSystemTools::RunSingleCommand(std::vector < std::string > const &command, std::string * output, int *retVal, const char *dir, OutputOption outputflag, double timeout)
{
    std::vector < const char *>argv;
    for (std::vector < std::string >::const_iterator a = command.begin(); a != command.end(); ++a)
    {
...
bool cmSystemTools::RunSingleCommand(const char *command, std::string * output, int *retVal, const char *dir, OutputOption outputflag, double timeout)
{
    ...
    return cmSystemTools::RunSingleCommand(args, output, retVal, dir, outputflag, timeout);
}

std::string cmSystemTools::PrintSingleCommand(std::vector < std::string > const &command)
{
    std::string commandStr;
    const char *sep = "";
    for (std::vector < std::string >::const_iterator i = command.begin(); i != command.end(); ++i)
    {
        commandStr += sep;
        commandStr += "\"";
        commandStr += *i;
...
bool cmSystemTools::DoesFileExistWithExtensions(const char *name, const std::vector < std::string > &headerExts)
{
    std::string hname;

    for (std::vector < std::string >::const_iterator ext = headerExts.begin(); ext != headerExts.end(); ++ext)
    {
        hname = name;
        hname += ".";
        hname += *ext;
...
std::string cmSystemTools::FileExistsInParentDirectories(const char *fname, const char *directory, const char *toplevel)
{
...
bool cmSystemTools::CopyFileIfDifferent(const char *source, const char *destination)
{
...
cmSystemTools::WindowsFileRetry cmSystemTools::GetWindowsFileRetry()
{
    ...
    if (!retry.Count)
    {
        ...
        {
            HKEY hKey;
            if (RegOpenKeyExW(keys[k], L"Software\\Kitware\\CMake\\Config", 0, KEY_QUERY_VALUE, &hKey) == ERROR_SUCCESS)
            {
                for (int v = 0; v < 2; ++v)
                {
                    DWORD dwData, dwType, dwSize = 4;
                    if (!data[v] && RegQueryValueExW(hKey, values[v], 0, &dwType, (BYTE *) & dwData, &dwSize) == ERROR_SUCCESS && dwType == REG_DWORD && dwSize == 4)
                    {
...
#    endif
    ...
    while (!MoveFileExW(cmsys::Encoding::ToWide(oldname).c_str(), cmsys::Encoding::ToWide(newname).c_str(), MOVEFILE_REPLACE_EXISTING) && --retry.Count)
    {
        ...
        DWORD attrs = GetFileAttributesW(cmsys::Encoding::ToWide(newname).c_str());
        if ((attrs != INVALID_FILE_ATTRIBUTES) && (attrs & FILE_ATTRIBUTE_READONLY))
        {
            // Remove the read-only attribute from the destination file.
            SetFileAttributesW(cmsys::Encoding::ToWide(newname).c_str(), attrs & ~FILE_ATTRIBUTE_READONLY);
        }
        else
        {
...
void cmSystemTools::Glob(const char *directory, const char *regexp, std::vector < std::string > &files)
{
    cmsys::Directory d;
...
void cmSystemTools::GlobDirs(const char *fullPath, std::vector < std::string > &files)
{
    ...
    if (d.Load(startPath.c_str()))
    {
        ...
        {
            if ((std::string(d.GetFile(i)) != ".") && (std::string(d.GetFile(i)) != ".."))
            {
...
void cmSystemTools::ExpandList(std::vector < std::string > const &arguments, std::vector < std::string > &newargs)
{
...
void cmSystemTools::ExpandListArgument(const std::string & arg, std::vector < std::string > &newargs, bool emptyArgs)
{
...
bool cmSystemTools::SimpleGlob(const cmStdString & glob, std::vector < cmStdString > &files, int type /* = 0 */ )
{
    files.clear();
    if (glob[glob.size() - 1] != '*')
    {
    ...
    if (d.Load(path.c_str()))
    {
        for (unsigned int i = 0; i < d.GetNumberOfFiles(); ++i)
        {
            if ((std::string(d.GetFile(i)) != ".") && (std::string(d.GetFile(i)) != ".."))
            {
                ...
                if (sfname.size() >= ppath.size() && sfname.substr(0, ppath.size()) == ppath)
                {
                    files.push_back(fname);
                    res = true;
                }
...
cmSystemTools::FileFormat cmSystemTools::GetFileFormat(const char *cext)
{
    ...
    if (ext == "C" || ext == ".C" || ext == "M" || ext == ".M" || ext == "c++" || ext == ".c++" || ext == "cc" || ext == ".cc" || ext == "cpp" || ext == ".cpp" || ext == "cxx" || ext == ".cxx" || ext == "mm" || ext == ".mm")
    {
        return cmSystemTools::CXX_FILE_FORMAT;
    }
    if (ext == "f" || ext == ".f" || ext == "F" || ext == ".F" || ext == "f77" || ext == ".f77" || ext == "f90" || ext == ".f90" || ext == "for" || ext == ".for" || ext == "f95" || ext == ".f95")
    {
    ...
    if (ext == "H" || ext == ".H" ||
        ext == "h" || ext == ".h" || ext == "h++" || ext == ".h++" || ext == "hm" || ext == ".hm" || ext == "hpp" || ext == ".hpp" || ext == "hxx" || ext == ".hxx" || ext == "in" || ext == ".in" || ext == "txx" || ext == ".txx")
    {
...
#else                            // __APPLE__
    if (ext == "so" || ext == ".so" || ext == "sl" || ext == ".sl" || ext == "dll" || ext == ".dll")
    {
        return cmSystemTools::SHARED_LIBRARY_FILE_FORMAT;
    }
#endif                            // __APPLE__
...
std::string cmSystemTools::CollapseCombinedPath(std::string const &dir, std::string const &file)
{
    if (dir.empty() || dir == ".")
    {
        return file;
...
{
    for (std::vector < std::string >::const_iterator eit = env.begin(); eit != env.end(); ++eit)
    {
...
cmSystemTools::SaveRestoreEnvironment::~SaveRestoreEnvironment()
{
    ...
    for (std::vector < std::string >::const_iterator eit = currentEnv.begin(); eit != currentEnv.end(); ++eit)
    {
        std::string var(*eit);
...
bool cmSystemTools::CreateTar(const char *outFileName, const std::vector < cmStdString > &files, bool gzip, bool bzip2, bool verbose)
{
#if defined(CMAKE_BUILD_WITH_CMAKE)
    ...
    cmArchiveWrite a(fout, (gzip ? cmArchiveWrite::CompressGZip : (bzip2 ? cmArchiveWrite::CompressBZip2 : cmArchiveWrite::CompressNone)), cmArchiveWrite::TypeTAR);
    a.SetVerbose(verbose);
    for (std::vector < cmStdString >::const_iterator i = files.begin(); i != files.end(); ++i)
    {
        std::string path = *i;
...
#    endif
        ...
        for (;;)
        {
            ...
            if (r != ARCHIVE_OK)
            {
                cmSystemTools::Message("archive_write_data_block()", archive_error_string(aw));
                return (r);
            }
        }
        return r;
    ...
    bool extract_tar(const char *outFileName, bool verbose, bool extract)
    {
        ...
        if (r)
        {
            cmSystemTools::Error("Problem with archive_read_open_file(): ", archive_error_string(a));
            return false;
        }
        for (;;)
        {
            ...
            if (r != ARCHIVE_OK)
            {
                cmSystemTools::Error("Problem with archive_read_next_header(): ", archive_error_string(a));
                break;
            }
            if (verbose)
            {
            ...
            if (extract)
            {
                ...
                if (r != ARCHIVE_OK)
                {
                    cmSystemTools::Error("Problem with archive_write_disk_set_options(): ", archive_error_string(ext));
                    break;
                }
                ...
                if (r == ARCHIVE_OK)
                {
                    ...
                    if (r != ARCHIVE_OK)
                    {
                        cmSystemTools::Error("Problem with archive_write_finish_entry(): ", archive_error_string(ext));
                        break;
                    }
                }
#    ifdef _WIN32
                ...
                {
                    std::cerr << "cmake -E tar: warning: skipping symbolic link \"" << archive_entry_pathname(entry) << "\" -> \"" << linktext << "\"." << std::endl;
                }
#    endif
                else
                {
                    cmSystemTools::Error("Problem with archive_write_header(): ", archive_error_string(ext));
                    cmSystemTools::Error("Current file: ", archive_entry_pathname(entry));
                    break;
                }
            }
        }
...
bool cmSystemTools::ExtractTar(const char *outFileName, bool, bool verbose)
{
#if defined(CMAKE_BUILD_WITH_CMAKE)
...
bool cmSystemTools::ListTar(const char *outFileName, bool, bool verbose)
{
#if defined(CMAKE_BUILD_WITH_CMAKE)
...
int cmSystemTools::WaitForLine(cmsysProcess * process, std::string & line, double timeout, std::vector < char >&out, std::vector < char >&err)
{
    line = "";
...
void cmSystemTools::DoNotInheritStdPipes()
{
    ...
    }
    {
        ...
        DuplicateHandle(GetCurrentProcess(), out, GetCurrentProcess(), &out, 0, FALSE, DUPLICATE_SAME_ACCESS | DUPLICATE_CLOSE_SOURCE);
...
{
#if defined(_WIN32) && !defined(__CYGWIN__)
    cmSystemToolsWindowsHandle hFrom = CreateFileW(cmsys::Encoding::ToWide(fromFile).c_str(),
                                                   GENERIC_READ, FILE_SHARE_READ, 0,
                                                   OPEN_EXISTING, 0, 0);
    cmSystemToolsWindowsHandle hTo = CreateFileW(cmsys::Encoding::ToWide(toFile).c_str(),
...
{
#if defined(_WIN32) && !defined(__CYGWIN__)
    cmSystemToolsWindowsHandle h = CreateFileW(cmsys::Encoding::ToWide(fname).c_str(),
...
#        define CRYPT_SILENT 0x40
                                /* Not defined by VS 6 version of header.  */
#    endif
...
unsigned int cmSystemTools::RandomSeed()
{
    ...
    if (fin && fin.read(seed.bytes, sizeof(seed)) && fin.gcount() == sizeof(seed))
    {
        return seed.integer;
    }
...
void cmSystemTools::FindCMakeResources(const char *argv0)
{
    ...
    exe_dir = cmSystemTools::GetFilenamePath(cmsys::Encoding::ToNarrow(modulepath));
#elif defined(__APPLE__)
...
#    undef CM_EXE_PATH_LOCAL_SIZE
    ...
    exe_dir = cmSystemTools::GetFilenamePath(cmSystemTools::GetRealPath(exe_path));
    ...
    if (cmSystemTools::FindProgramPath(argv0, exe, errorMsg))
    {
        ...
        exe_dir = cmSystemTools::GetFilenamePath(exe.c_str());
    }
    else
    {
        // ???
    ...
    if (!cmSystemTools::FileExists((cmSystemToolsCMakeRoot + "/Modules/CMake.cmake").c_str()))
    {
        ...
        std::string src_dir;
        if (fin && cmSystemTools::GetLineFromStream(fin, src_dir) && cmSystemTools::FileIsDirectory(src_dir.c_str()))
        {
        ...
        else
        {
            ...
            if (fin2 && cmSystemTools::GetLineFromStream(fin2, src_dir) && cmSystemTools::FileIsDirectory(src_dir.c_str()))
            {
                cmSystemToolsCMakeRoot = src_dir;
            }
        }
...
//----------------------------------------------------------------------------
#if defined(CMAKE_BUILD_WITH_CMAKE)
void cmSystemTools::MakefileColorEcho(int color, const char *message, bool newline, bool enabled)
{
    ...
    if (cmSystemTools::GetEnv("DART_TEST_FROM_DART") || cmSystemTools::GetEnv("DASHBOARD_TEST_FROM_CTEST") || cmSystemTools::GetEnv("CTEST_INTERACTIVE_DEBUG_MODE"))
    {
    ...
    if (enabled)
    {
        cmsysTerminal_cfprintf(color | assumeTTY, stdout, "%s%s", message, newline ? "\n" : "");
    }
    else
    {
...
//----------------------------------------------------------------------------
bool cmSystemTools::GuessLibrarySOName(std::string const &fullPath, std::string & soname)
{
    ...
    if (soname.length() > name.length() && soname.substr(0, name.length()) == name)
    {
        return true;
    }
    return false;
...
//----------------------------------------------------------------------------
bool cmSystemTools::GuessLibraryInstallName(std::string const &fullPath, std::string & soname)
{
...
//----------------------------------------------------------------------------
#if defined(CMAKE_USE_ELF_PARSER)
std::string::size_type cmSystemToolsFindRPath(std::string const &have, std::string const &want)
{
...
//----------------------------------------------------------------------------
bool cmSystemTools::ChangeRPath(std::string const &file, std::string const &oldRPath, std::string const &newRPath, std::string * emsg, bool * changed)
{
#if defined(CMAKE_USE_ELF_PARSER)
    if (changed)
    {
    ...
    cmSystemToolsRPathInfo rp[2];
    {
        ...
        for (int i = 0; i < se_count; ++i)
        {
            ...
            std::string::size_type pos = cmSystemToolsFindRPath(se[i]->Value, oldRPath);
            if (pos == std::string::npos)
            {
            ...
            rp[rp_count].Value += se[i]->Value.substr(pos + oldRPath.length(), oldRPath.npos);
...

    {
        ...
        cmsys::ofstream f(file.c_str(), std::ios::in | std::ios::out | std::ios::binary);
        if (!f)
        {
            if (emsg)
            {
...
//----------------------------------------------------------------------------
bool cmSystemTools::VersionCompare(cmSystemTools::CompareOp op, const char *lhss, const char *rhss)
{
    // Parse out up to 8 components.
    unsigned int lhs[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
    unsigned int rhs[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };
    sscanf(lhss, "%u.%u.%u.%u.%u.%u.%u.%u", &lhs[0], &lhs[1], &lhs[2], &lhs[3], &lhs[4], &lhs[5], &lhs[6], &lhs[7]);
    sscanf(rhss, "%u.%u.%u.%u.%u.%u.%u.%u", &rhs[0], &rhs[1], &rhs[2], &rhs[3], &rhs[4], &rhs[5], &rhs[6], &rhs[7]);
...
//----------------------------------------------------------------------------
bool cmSystemTools::RemoveRPath(std::string const &file, std::string * emsg, bool * removed)
{
#if defined(CMAKE_USE_ELF_PARSER)
    if (removed)
    {
...
//----------------------------------------------------------------------------
bool cmSystemTools::CheckRPath(std::string const &file, std::string const &newRPath)
{
#if defined(CMAKE_USE_ELF_PARSER)
    // Parse the ELF binary.
    cmELF elf(file.c_str());
    ...
    {
        if (se && cmSystemToolsFindRPath(se->Value, newRPath) != std::string::npos)
        {
            return true;
        }
    }
...
//----------------------------------------------------------------------------
std::vector < std::string > cmSystemTools::tokenize(const std::string & str, const std::string & sep)
{
