...
void cmWarnUnusedCliWarning(const std::string & variable, int, void *ctx, const char *, const cmMakefile *)
{
...
cmake::~cmake()
{
    ...
    for (RegisteredCommandsMap::iterator j = this->Commands.begin(); j != this->Commands.end(); ++j)
    {
        delete(*j).second;
    }
    for (RegisteredGeneratorsVector::iterator j = this->Generators.begin(); j != this->Generators.end(); ++j)
    {
        delete *j;
    }
#ifdef CMAKE_BUILD_WITH_CMAKE
...
void cmake::RemoveUnscriptableCommands()
{
    ...
    for (cmake::RegisteredCommandsMap::const_iterator pos = commands->begin(); pos != commands->end(); ++pos)
    {
    ...
    for (std::vector < std::string >::const_iterator it = unscriptableCommands.begin(); it != unscriptableCommands.end(); ++it)
    {
    ...
    for (unsigned int i = 1; i < args.size(); ++i)
    {
        ...
        if (arg.find("-D", 0) == 0)
        {
            ...
            if (cmCacheManager::ParseEntry(entry.c_str(), var, value, type))
            {
                ...
                const char *cachedValue = this->CacheManager->GetCacheValue(var.c_str());

                this->CacheManager->AddCacheEntry(var.c_str(), value.c_str(), "No help, variable specified on the command line.", type);
                if (this->WarnUnusedCli)
                {
                    if (!cachedValue || strcmp(this->CacheManager->GetCacheValue(var.c_str()), cachedValue) != 0)
                    {
        ...
        else if (arg.find("-U", 0) == 0)
        {
            ...
            cmsys::RegularExpression regex(cmsys::Glob::PatternToRegex(entryPattern.c_str(), true, true).c_str());
            //go through all cache entries and collect the vars which will be removed
            std::vector < std::string > entriesToDelete;
            cmCacheManager::CacheIterator it = this->CacheManager->GetCacheIterator();
            ...
            for (std::vector < std::string >::const_iterator currentEntry = entriesToDelete.begin(); currentEntry != entriesToDelete.end(); ++currentEntry)
            {
...
void cmake::ReadListFile(const std::vector < std::string > &args, const char *path)
{
    ...
    if (path)
    {
        ...
        lg->GetMakefile()->SetHomeOutputDirectory(cmSystemTools::GetCurrentWorkingDirectory().c_str());
        lg->GetMakefile()->SetStartOutputDirectory(cmSystemTools::GetCurrentWorkingDirectory().c_str());
        lg->GetMakefile()->SetHomeDirectory(cmSystemTools::GetCurrentWorkingDirectory().c_str());
        lg->GetMakefile()->SetStartDirectory(cmSystemTools::GetCurrentWorkingDirectory().c_str());
...
bool cmake::FindPackage(const std::vector < std::string > &args)
{
    ...
    mf->SetHomeOutputDirectory(cmSystemTools::GetCurrentWorkingDirectory().c_str());
    mf->SetStartOutputDirectory(cmSystemTools::GetCurrentWorkingDirectory().c_str());
    mf->SetHomeDirectory(cmSystemTools::GetCurrentWorkingDirectory().c_str());
    mf->SetStartDirectory(cmSystemTools::GetCurrentWorkingDirectory().c_str());
    ...
    else if (mode == "COMPILE")
    {
        ...
        std::string includeFlags = lg->GetIncludeFlags(includeDirs, 0, language.c_str(), false);
    ...
    else if (mode == "LINK")
    {
        ...
        for (std::vector < std::string >::const_iterator libIt = libList.begin(); libIt != libList.end(); ++libIt)
        {
        ...
        lg->GetTargetFlags(linkLibs, frameworkPath, linkPath, flags, linkFlags, gtgt);
...
void cmake::SetArgs(const std::vector < std::string > &args, bool directoriesSetBefore)
{
    ...
    for (unsigned int i = 1; i < args.size(); ++i)
    {
        ...
        else if (arg.find("-G", 0) == 0)
        {
            ...
            cmGlobalGenerator *gen = this->CreateGlobalGenerator(value.c_str());
            if (!gen)
            {
                cmSystemTools::Error("Could not create named generator ", value.c_str());
            }
            else
            {
    ...
    if (!directoriesSet)
    {
        this->SetHomeOutputDirectory(cmSystemTools::GetCurrentWorkingDirectory().c_str());
        this->SetStartOutputDirectory(cmSystemTools::GetCurrentWorkingDirectory().c_str());
        this->SetHomeDirectory(cmSystemTools::GetCurrentWorkingDirectory().c_str());
        this->SetStartDirectory(cmSystemTools::GetCurrentWorkingDirectory().c_str());
    }
...
int cmake::AddCMakePaths()
{
    ...
    this->CacheManager->AddCacheEntry("CMAKE_COMMAND", cmSystemTools::GetCMakeCommand().c_str(), "Path to CMake executable.", cmCacheManager::INTERNAL);
#ifdef CMAKE_BUILD_WITH_CMAKE
    this->CacheManager->AddCacheEntry("CMAKE_CTEST_COMMAND", cmSystemTools::GetCTestCommand().c_str(), "Path to ctest program executable.", cmCacheManager::INTERNAL);
    this->CacheManager->AddCacheEntry("CMAKE_CPACK_COMMAND", cmSystemTools::GetCPackCommand().c_str(), "Path to cpack program executable.", cmCacheManager::INTERNAL);
#endif
    if (!cmSystemTools::FileExists((cmSystemTools::GetCMakeRoot() + "/Modules/CMake.cmake").c_str()))
    {
        // couldn't find modules
        cmSystemTools::Error("Could not find CMAKE_ROOT !!!\n" "CMake has most likely not been installed correctly.\n" "Modules directory not found in\n", cmSystemTools::GetCMakeRoot().c_str());
        return 0;
    }
    this->CacheManager->AddCacheEntry("CMAKE_ROOT", cmSystemTools::GetCMakeRoot().c_str(), "Path to CMake installation.", cmCacheManager::INTERNAL);

    return 1;
}

void cmake::AddExtraGenerator(const char *name, CreateExtraGeneratorFunctionType newFunction)
{
    cmExternalMakefileProjectGenerator *extraGenerator = newFunction();
    const std::vector < std::string > &supportedGlobalGenerators = extraGenerator->GetSupportedGlobalGenerators();

    for (std::vector < std::string >::const_iterator it = supportedGlobalGenerators.begin(); it != supportedGlobalGenerators.end(); ++it)
    {
        std::string fullName = cmExternalMakefileProjectGenerator::CreateFullGeneratorName(it->c_str(), name);
...
#    endif
    ...
    this->AddExtraGenerator(cmExtraCodeBlocksGenerator::GetActualName(), &cmExtraCodeBlocksGenerator::New);
    this->AddExtraGenerator(cmExtraCodeLiteGenerator::GetActualName(), &cmExtraCodeLiteGenerator::New);
    this->AddExtraGenerator(cmExtraSublimeTextGenerator::GetActualName(), &cmExtraSublimeTextGenerator::New);
    this->AddExtraGenerator(cmExtraKateGenerator::GetActualName(), &cmExtraKateGenerator::New);

#    ifdef CMAKE_USE_ECLIPSE
    this->AddExtraGenerator(cmExtraEclipseCDT4Generator::GetActualName(), &cmExtraEclipseCDT4Generator::New);
#    endif
...
#    ifdef CMAKE_USE_KDEVELOP
    ...
    this->ExtraGenerators[cmGlobalKdevelopGenerator::GetActualName()] = &cmGlobalKdevelopGenerator::New;
#    endif
...
{
    for (RegisteredGeneratorsVector::const_iterator i = this->Generators.begin(); i != this->Generators.end(); ++i)
    {
        (*i)->GetGenerators(names);
    }
    for (RegisteredExtraGeneratorsMap::const_iterator i = this->ExtraGenerators.begin(); i != this->ExtraGenerators.end(); ++i)
    {
...
cmGlobalGenerator *cmake::CreateGlobalGenerator(const char *name)
{
    ...
    RegisteredExtraGeneratorsMap::const_iterator extraGenIt = this->ExtraGenerators.find(name);
...
int cmake::DoPreConfigureChecks()
{
    ...
    if (!cmSystemTools::FileExists(srcList.c_str()))
    {
        ...
        {
            err << "The source directory \"" << this->GetHomeDirectory() << "\" does not appear to contain CMakeLists.txt.\n";
        }
        else if (cmSystemTools::FileExists(this->GetHomeDirectory()))
        {
            err << "The source directory \"" << this->GetHomeDirectory() << "\" is a file, not a directory.\n";
        }
        else
        {
            err << "The source directory \"" << this->GetHomeDirectory() << "\" does not exist.\n";
        }
    ...
    {
        std::string cacheStart = this->CacheManager->GetCacheValue("CMAKE_HOME_DIRECTORY");
...
int cmake::HandleDeleteCacheVariables(const char *var)
{
    ...
    for (std::vector < std::string >::iterator i = argsSplit.begin(); i != argsSplit.end(); ++i)
    {
        SaveCacheEntry save;
        save.key = *i;
        warning << *i << "= ";
    ...
    for (std::vector < SaveCacheEntry >::iterator i = saved.begin(); i != saved.end(); ++i)
    {
        this->AddCacheEntry(i->key.c_str(), i->value.c_str(), i->help.c_str(), i->type);
    }
...
int cmake::Configure()
{
    if (this->DoSuppressDevWarnings)
    {
        ...
        {
            this->CacheManager->AddCacheEntry("CMAKE_SUPPRESS_DEVELOPER_WARNINGS", "TRUE", "Suppress Warnings that are meant for" " the author of the CMakeLists.txt files.", cmCacheManager::INTERNAL);
        }
        else
        {
            this->CacheManager->AddCacheEntry("CMAKE_SUPPRESS_DEVELOPER_WARNINGS", "FALSE", "Suppress Warnings that are meant for" " the author of the CMakeLists.txt files.", cmCacheManager::INTERNAL);
        }
    }
...
int cmake::ActualConfigure()
{
    ...
    if (!res)
    {
        this->CacheManager->AddCacheEntry("CMAKE_HOME_DIRECTORY", this->GetHomeDirectory(), "Start directory with the top level CMakeLists.txt file for this " "project", cmCacheManager::INTERNAL);
    }
    ...
    if (!this->GlobalGenerator)
    {
        ...
        const char *extraGenName = this->CacheManager->GetCacheValue("CMAKE_EXTRA_GENERATOR");
        if (genName)
        {
            std::string fullName = cmExternalMakefileProjectGenerator::CreateFullGeneratorName(genName, extraGenName);
    ...
    {
        this->CacheManager->AddCacheEntry("CMAKE_GENERATOR", this->GlobalGenerator->GetName(), "Name of generator.", cmCacheManager::INTERNAL);
        this->CacheManager->AddCacheEntry("CMAKE_EXTRA_GENERATOR", this->GlobalGenerator->GetExtraGeneratorName(), "Name of external makefile project generator.", cmCacheManager::INTERNAL);
    }

    if (const char *tsName = this->CacheManager->GetCacheValue("CMAKE_GENERATOR_TOOLSET"))
    {
    ...
    {
        this->CacheManager->AddCacheEntry("CMAKE_GENERATOR_TOOLSET", this->GeneratorToolset.c_str(), "Name of generator toolset.", cmCacheManager::INTERNAL);
    }
    if (!this->GeneratorToolset.empty() && !this->GlobalGenerator->SetGeneratorToolset(this->GeneratorToolset))
    {
        return -2;
    }
    ...
    if (this->GetCacheManager()->GetCacheValue("CMAKE_BACKWARDS_COMPATIBILITY"))
    {
        ...
        {
            this->CacheManager->AddCacheEntry("LIBRARY_OUTPUT_PATH", "", "Single output directory for building all libraries.", cmCacheManager::PATH);
        }
        if (!this->CacheManager->GetCacheValue("EXECUTABLE_OUTPUT_PATH"))
        {
            this->CacheManager->AddCacheEntry("EXECUTABLE_OUTPUT_PATH", "", "Single output directory for building all executables.", cmCacheManager::PATH);
        }
    }
    if (!this->CacheManager->GetCacheValue("CMAKE_USE_RELATIVE_PATHS"))
    {
        this->CacheManager->AddCacheEntry("CMAKE_USE_RELATIVE_PATHS", "OFF", "If true, cmake will use relative paths in makefiles and projects.", cmCacheManager::BOOL);
        cmCacheManager::CacheIterator it = this->CacheManager->GetCacheIterator("CMAKE_USE_RELATIVE_PATHS");
    ...
    if (cmSystemTools::GetFatalErrorOccured() && (!this->CacheManager->GetCacheValue("CMAKE_MAKE_PROGRAM") || cmSystemTools::IsOff(this->CacheManager->GetCacheValue("CMAKE_MAKE_PROGRAM"))))
    {
    ...
    if (mf->IsOn("CTEST_USE_LAUNCHERS") && !this->GetProperty("RULE_LAUNCH_COMPILE", cmProperty::GLOBAL))
    {
        cmSystemTools::Error("CTEST_USE_LAUNCHERS is enabled, but the " "RULE_LAUNCH_COMPILE global property is not defined.\n" "Did you forget to include(CTest) in the toplevel " "CMakeLists.txt ?");
    }
...
int cmake::Run(const std::vector < std::string > &args, bool noconfigure)
{
    ...
    if (!this->CheckStampList.empty() && cmakeCheckStampList(this->CheckStampList.c_str()))
    {
        return 0;
    }

    // If we are given a stamp file check if it is really out of date.
    if (!this->CheckStampFile.empty() && cmakeCheckStampFile(this->CheckStampFile.c_str()))
    {
        return 0;
    }
    ...
    if (ret || this->GetWorkingMode() != NORMAL_MODE)
    {
        ...
        if (!this->VSSolutionFile.empty() && this->GlobalGenerator)
        {
            ...
            cmSystemTools::Message("CMake Configure step failed.  " "Build files cannot be regenerated correctly.  " "Attempting to stop IDE build.");
            cmGlobalVisualStudioGenerator *gg = static_cast < cmGlobalVisualStudioGenerator * >(this->GlobalGenerator);
            gg->CallVisualStudioMacro(cmGlobalVisualStudioGenerator::MacroStop, this->VSSolutionFile.c_str());
        }
#endif
        return ret;
    }
...
void cmake::AddCacheEntry(const char *key, const char *value, const char *helpString, int type)
{
    this->CacheManager->AddCacheEntry(key, value, helpString, cmCacheManager::CacheEntryType(type));
}
...
void cmake::AddDefaultCommands()
{
    ...
    for (std::list < cmCommand * >::iterator i = commands.begin(); i != commands.end(); ++i)
    {
        this->AddCommand(*i);
    }
}
...
#if defined(_WIN32) && !defined(__CYGWIN__)
#    if !defined(CMAKE_BOOT_MINGW)
    this->Generators.push_back(cmGlobalVisualStudio6Generator::NewFactory());
    this->Generators.push_back(cmGlobalVisualStudio7Generator::NewFactory());
...
void cmake::UpdateConversionPathTable()
{
    ...
    const char *tablepath = this->CacheManager->GetCacheValue("CMAKE_PATH_TRANSLATION_FILE");
...
int cmake::CheckBuildSystem()
{
    ...
    bool verbose = ((cmSystemTools::GetEnv("VERBOSE") != 0) && (cmSystemTools::GetEnv("CMAKE_NO_VERBOSE") == 0));
    ...
    if (!cmSystemTools::FileExists(this->CheckBuildSystemArgument.c_str()))
    {
        ...
        {
            cmOStringStream msg;
            msg << "Re-run cmake missing file: " << this->CheckBuildSystemArgument.c_str() << "\n";
            cmSystemTools::Stdout(msg.str().c_str());
        }
        return 1;
    }
    ...
    cmsys::auto_ptr < cmLocalGenerator > lg(gg.CreateLocalGenerator());
    cmMakefile *mf = lg->GetMakefile();
    if (!mf->ReadListFile(0, this->CheckBuildSystemArgument.c_str()) || cmSystemTools::GetErrorOccuredFlag())
    {
        if (verbose)
        {
            cmOStringStream msg;
            msg << "Re-run cmake error reading : " << this->CheckBuildSystemArgument.c_str() << "\n";
            cmSystemTools::Stdout(msg.str().c_str());
        }
    ...
    if (this->ClearBuildSystem)
    {
        ...
        cmsys::auto_ptr < cmGlobalGenerator > ggd(this->CreateGlobalGenerator(genName));
        if (ggd.get())
        {
    ...
    for (std::vector < std::string >::const_iterator pi = products.begin(); pi != products.end(); ++pi)
    {
        if (!(cmSystemTools::FileExists(pi->c_str()) || cmSystemTools::FileIsSymlink(pi->c_str())))
        {
            if (verbose)
            {
                cmOStringStream msg;
    ...
    if (depends.empty() || outputs.empty())
    {
        ...
        {
            cmOStringStream msg;
            msg << "Re-run cmake no CMAKE_MAKEFILE_DEPENDS " "or CMAKE_MAKEFILE_OUTPUTS :\n";
            cmSystemTools::Stdout(msg.str().c_str());
        }
        return 1;
    }
    ...
    {
        int result = 0;
        if (this->FileComparison->FileTimeCompare(dep_newest.c_str(), dep->c_str(), &result))
        {
            if (result < 0)
            {
                dep_newest = *dep;
    ...
    {
        int result = 0;
        if (this->FileComparison->FileTimeCompare(out_oldest.c_str(), out->c_str(), &result))
        {
            if (result > 0)
            {
                out_oldest = *out;
    ...
    {
        int result = 0;
        if (!this->FileComparison->FileTimeCompare(out_oldest.c_str(), dep_newest.c_str(), &result) || result < 0)
        {
            if (verbose)
            {
                cmOStringStream msg;
                msg << "Re-run cmake file: " << out_oldest.c_str() << " older than: " << dep_newest.c_str() << "\n";
                cmSystemTools::Stdout(msg.str().c_str());
            }
            return 1;
        }
...
{
#ifdef CMAKE_BUILD_WITH_CMAKE
    cmsys::auto_ptr < cmGraphVizWriter > gvWriter(new cmGraphVizWriter(this->GetGlobalGenerator()->GetLocalGenerators()));
...
void cmake::DefineProperty(const char *name, cmProperty::ScopeType scope, const char *ShortDescription, const char *FullDescription, bool chained)
{
    this->PropertyDefinitions[scope].DefineProperty(name, scope, ShortDescription, FullDescription, chained);
}

cmPropertyDefinition *cmake::GetPropertyDefinition(const char *name, cmProperty::ScopeType scope)
{
...
const char *cmake::GetProperty(const char *prop, cmProperty::ScopeType scope)
{
    ...
    {
        cmake::RegisteredCommandsMap::iterator cmds = this->GetCommands()->begin();
    ...
    {
        this->SetProperty("IN_TRY_COMPILE", this->GetIsInTryCompile()? "1" : "0");
    }
    else if (propname == "ENABLED_LANGUAGES")
    {
        ...
        if (this->GlobalGenerator)
        {
            ...
            const char *sep = "";
            for (std::vector < std::string >::iterator i = enLangs.begin(); i != enLangs.end(); ++i)
            {
                lang += sep;
                sep = ";";
                lang += *i;
...
int cmake::GetSystemInformation(std::vector < std::string > &args)
{
    ...
    {
        std::cerr << "Error: --system-information must be run from a " "writable directory!\n";
        return 1;
    }
    ...
    this->AddCMakePaths();
    std::string modulesPath = this->CacheManager->GetCacheValue("CMAKE_ROOT");
    modulesPath += "/Modules";
    ...
    {
        std::cerr << "Error copying file \"" << inFile.c_str() << "\" to \"" << outFile.c_str() << "\".\n";
        return 1;
    }
...
static bool cmakeCheckStampFile(const char *stampName)
{
    ...
    {
        int result;
        if (dep.length() >= 1 && dep[0] != '#' && (!ftc.FileTimeCompare(stampDepends.c_str(), dep.c_str(), &result) || result < 0))
        {
            // The stamp depends file is older than this dependency.  The
            // build system is really out of date.
            std::cout << "CMake is re-running because " << stampName << " is out-of-date.\n";
...
static bool cmakeCheckStampList(const char *stampList)
{
    ...
    {
        std::cout << "CMake is re-running because generate.stamp.list " << "is missing.\n";
        return false;
    }
    cmsys::ifstream fin(stampList);
    if (!fin)
    {
        std::cout << "CMake is re-running because generate.stamp.list " << "could not be read.\n";
        return false;
    }
...
//----------------------------------------------------------------------------
void cmake::IssueMessage(cmake::MessageType t, std::string const &text, cmListFileBacktrace const &backtrace)
{
    cmOStringStream msg;
    bool isError = false;
    ...
    else
    {
        ...
        if (t == cmake::AUTHOR_WARNING)
        {
            ...
            cmCacheManager::CacheIterator it = this->CacheManager->GetCacheIterator("CMAKE_SUPPRESS_DEVELOPER_WARNINGS");
    ...
    if (t == cmake::AUTHOR_WARNING)
    {
        msg << "This warning is for project developers.  Use -Wno-dev to suppress it.";
    }
...
std::vector < std::string > const &cmake::GetDebugConfigs()
{
    ...
    if (this->DebugConfigs.empty())
    {
        if (const char *config_list = this->GetProperty("DEBUG_CONFIGURATIONS"))
        {
            ...
            for (std::vector < std::string >::iterator i = this->DebugConfigs.begin(); i != this->DebugConfigs.end(); ++i)
            {
...
int cmake::Build(const std::string & dir, const std::string & target, const std::string & config, const std::vector < std::string > &nativeOptions, bool clean)
{
    ...
    cmsys::auto_ptr < cmGlobalGenerator > gen(this->CreateGlobalGenerator(it.GetValue()));
    if (!gen.get())
    {
        std::cerr << "Error: could create CMAKE_GENERATOR \"" << it.GetValue() << "\"\n";
        return 1;
    }
    std::string output;
    std::string projName;
    ...
    return gen->Build(0, dir.c_str(), projName.c_str(), target.c_str(), &output, 0, config.c_str(), clean, false, 0, cmSystemTools::OUTPUT_PASSTHROUGH, nativeOptions);
}
...
void cmake::RunCheckForUnusedVariables()
{
    ...
    for (std::map < cmStdString, bool >::const_iterator it = this->UsedCliVariables.begin(); it != this->UsedCliVariables.end(); ++it)
    {
        if (!it->second)
        {
            haveUnused = true;
