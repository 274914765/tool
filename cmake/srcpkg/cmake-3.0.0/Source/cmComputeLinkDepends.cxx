...
//----------------------------------------------------------------------------
cmComputeLinkDepends::cmComputeLinkDepends(cmTarget const *target, const char *config, cmTarget const *head)
{
...
{
    for (std::vector < DependSetList * >::iterator i = this->InferredDependSets.begin(); i != this->InferredDependSets.end(); ++i)
    {
        delete *i;
    }
    delete this->CCG;
...
//----------------------------------------------------------------------------
std::vector < cmComputeLinkDepends::LinkEntry > const & cmComputeLinkDepends::Compute()
{
    ...
    for (std::vector < int >::const_iterator li = this->FinalLinkOrder.begin(); li != this->FinalLinkOrder.end(); ++li)
    {
...
//----------------------------------------------------------------------------
std::map < cmStdString, int >::iterator cmComputeLinkDepends::AllocateLinkEntry(std::string const &item)
{
    std::map < cmStdString, int >::value_type index_entry(item, static_cast < int >(this->EntryList.size()));
    std::map < cmStdString, int >::iterator lei = this->LinkEntryIndex.insert(index_entry).first;
...
//----------------------------------------------------------------------------
int cmComputeLinkDepends::AddLinkEntry(int depender_index, std::string const &item)
{
    ...
    entry.IsFlag = (!entry.Target && item[0] == '-' && item[1] != 'l' && item.substr(0, 10) != "-framework");
...
void cmComputeLinkDepends::FollowLinkEntry(BFSEntry const &qe)
{
    ...
    if (entry.Target)
    {
        ...
        if (cmTarget::LinkInterface const *iface = entry.Target->GetLinkInterface(this->Config, this->HeadTarget))
        {
            const bool isIface = entry.Target->GetType() == cmTarget::INTERFACE_LIBRARY;
            ...
            // Support for CMP0003.
            for (std::vector < std::string >::const_iterator oi = iface->WrongConfigLibraries.begin(); oi != iface->WrongConfigLibraries.end(); ++oi)
            {
...
//----------------------------------------------------------------------------
void cmComputeLinkDepends::FollowSharedDeps(int depender_index, cmTarget::LinkInterface const *iface, bool follow_interface)
{
...
//----------------------------------------------------------------------------
void cmComputeLinkDepends::QueueSharedDependencies(int depender_index, std::vector < std::string > const &deps)
{
    for (std::vector < std::string >::const_iterator li = deps.begin(); li != deps.end(); ++li)
    {
        SharedDepEntry qe;
        qe.Item = *li;
...
void cmComputeLinkDepends::HandleSharedDependency(SharedDepEntry const &dep)
{
    ...
    std::map < cmStdString, int >::iterator lei = this->LinkEntryIndex.find(dep.Item);
    if (lei == this->LinkEntryIndex.end())
    {
        // Allocate a spot for the item entry.
        ...
        entry.Item = dep.Item;
        entry.Target = this->FindTargetToLink(dep.DependerIndex, dep.Item.c_str());
...
//----------------------------------------------------------------------------
void cmComputeLinkDepends::AddVarLinkEntries(int depender_index, const char *value)
{
    ...
    bool haveLLT = false;
    for (std::vector < std::string >::const_iterator di = deplist.begin(); di != deplist.end(); ++di)
    {
        if (*di == "debug")
        {
            llt = cmTarget::DEBUG;
...
void cmComputeLinkDepends::AddDirectLinkEntries()
{
    ...
    cmTarget::LinkImplementation const *impl = this->Target->GetLinkImplementation(this->Config, this->HeadTarget);
    this->AddLinkEntries(-1, impl->Libraries);
    for (std::vector < std::string >::const_iterator wi = impl->WrongConfigLibraries.begin(); wi != impl->WrongConfigLibraries.end(); ++wi)
    {
...
//----------------------------------------------------------------------------
void cmComputeLinkDepends::AddLinkEntries(int depender_index, std::vector < std::string > const &libs)
{
    // Track inferred dependency sets implied by this list.
    std::map < int, DependSet > dependSets;

    // Loop over the libraries linked directly by the depender.
    for (std::vector < std::string >::const_iterator li = libs.begin(); li != libs.end(); ++li)
    {
        ...
        for (std::map < int, DependSet >::iterator dsi = dependSets.begin(); dsi != dependSets.end(); ++dsi)
        {
            // Add this item to the inferred dependencies of other items.
            // Target items are never inferred dependees because unknown
            // items are outside libraries that should not be depending on
            // targets.
            if (!this->EntryList[dependee_index].Target && !this->EntryList[dependee_index].IsFlag && dependee_index != dsi->first)
            {
...
//----------------------------------------------------------------------------
cmTarget const *cmComputeLinkDepends::FindTargetToLink(int depender_index, const char *name)
{
    ...
    // within the project.
    if (tgt && tgt->GetType() == cmTarget::EXECUTABLE && !tgt->IsExecutableWithExports())
    {
        tgt = 0;
    }
    ...
    if (tgt && tgt->GetType() == cmTarget::OBJECT_LIBRARY)
    {
        ...
        e << "Target \"" << this->Target->GetName() << "\" links to "
            "OBJECT library \"" << tgt->GetName() << "\" but this is not " "allowed.  " "One may link only to STATIC or SHARED libraries, or to executables " "with the ENABLE_EXPORTS property set.";
...
void cmComputeLinkDepends::InferDependencies()
{
    ...
    for (unsigned int depender_index = 0; depender_index < this->InferredDependSets.size(); ++depender_index)
    {
        ...
        for (++i; i != sets->end(); ++i)
        {
            ...
            cmsys_stl::set_intersection(common.begin(), common.end(), i->begin(), i->end(), std::inserter(intersection, intersection.begin()));
            common = intersection;
        }
...
{
    for (Graph::iterator i = this->EntryConstraintGraph.begin(); i != this->EntryConstraintGraph.end(); ++i)
    {
...
void cmComputeLinkDepends::OrderLinkEntires()
{
    ...
    for (std::vector < int >::const_iterator i = this->OriginalEntries.begin(); i != this->OriginalEntries.end(); ++i)
    {
        this->VisitEntry(*i);
    }
...
void cmComputeLinkDepends::VisitComponent(unsigned int c)
{
    ...
    for (EdgeList::const_reverse_iterator ni = nl.rbegin(); ni != nl.rend(); ++ni)
    {
...
void cmComputeLinkDepends::VisitEntry(int index)
{
    ...
    std::map < int, PendingComponent >::iterator mi = this->PendingComponents.find(this->ComponentOrder[component]);
...
//----------------------------------------------------------------------------
cmComputeLinkDepends::PendingComponent & cmComputeLinkDepends::MakePendingComponent(unsigned int component)
{
    // Create an entry (in topological order) for the component.
    PendingComponent & pc = this->PendingComponents[this->ComponentOrder[component]];
    pc.Id = component;
    NodeList const &nl = this->CCG->GetComponent(component);
...
int cmComputeLinkDepends::ComputeComponentCount(NodeList const &nl)
{
    ...
    for (NodeList::const_iterator ni = nl.begin(); ni != nl.end(); ++ni)
    {
        ...
        {
            if (cmTarget::LinkInterface const *iface = target->GetLinkInterface(this->Config, this->HeadTarget))
            {
...
void cmComputeLinkDepends::DisplayFinalEntries()
{
    ...
    for (std::vector < LinkEntry >::const_iterator lei = this->FinalLinkEntries.begin(); lei != this->FinalLinkEntries.end(); ++lei)
    {
        if (lei->Target)
        {
...
//----------------------------------------------------------------------------
void cmComputeLinkDepends::CheckWrongConfigItem(int depender_index, std::string const &item)
{
    ...
    // directories.
    if (cmTarget const *tgt = this->FindTargetToLink(depender_index, item.c_str()))
    {
        if (!tgt->IsImported())
        {
