...
{
    return (int) this->InitializeParser() && this->ParseChunk(string, strlen(string)) && this->CleanupParser();
}
...
int cmXMLParser::InitializeParser()
{
    ...
    XML_SetElementHandler(static_cast < XML_Parser > (this->Parser), &cmXMLParserStartElement, &cmXMLParserEndElement);
    XML_SetCharacterDataHandler(static_cast < XML_Parser > (this->Parser), &cmXMLParserCharacterDataHandler);
...
//----------------------------------------------------------------------------
int cmXMLParser::ParseChunk(const char *inputString, std::string::size_type length)
{
    if (!this->Parser)
    {
...
int cmXMLParser::ParseBuffer(const char *buffer, std::string::size_type count)
{
    ...
    if (!XML_Parse(static_cast < XML_Parser > (this->Parser), buffer, static_cast < int >(count), 0))
    {
        this->ReportXmlParseError();
        return 0;
    }
...
//----------------------------------------------------------------------------
void cmXMLParser::StartElement(const char *name, const char ** /*atts */ )
{
...
//----------------------------------------------------------------------------
const char *cmXMLParser::FindAttribute(const char **atts, const char *attribute)
{
    if (atts && attribute)
    {
...
//----------------------------------------------------------------------------
void cmXMLParserCharacterDataHandler(void *parser, const char *data, int length)
{
...
void cmXMLParser::ReportXmlParseError()
{
    ...
    this->ReportError(XML_GetCurrentLineNumber(parser), XML_GetCurrentColumnNumber(parser), XML_ErrorString(XML_GetErrorCode(parser)));
}

//----------------------------------------------------------------------------
void cmXMLParser::ReportError(int line, int, const char *msg)
{
    std::cerr << "Error parsing XML in stream at line " << line << ": " << msg << std::endl;
}
