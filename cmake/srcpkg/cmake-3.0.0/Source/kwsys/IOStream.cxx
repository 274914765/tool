...
// Scan an input stream for an integer value.
    static int IOStreamScanStream(kwsys_ios::istream & is, char *buffer)
    {
        ...
        if (flags == kwsys_ios::ios_base::oct)
        {
            base = 8;
        }
        else if (flags == kwsys_ios::ios_base::dec)
        {
            base = 10;
        }
        else if (flags == kwsys_ios::ios_base::hex)
        {
            base = 16;
        }
...
// Read an integer value from an input stream.
    template < class T > kwsys_ios::istream & IOStreamScanTemplate(kwsys_ios::istream & is, T & value, char type)
    {
...
#    endif
                ...
                if (is.peek() == EOF)
                {
                    state |= kwsys_ios::ios_base::eofbit;
                }
                if (!success)
                {
                    state |= kwsys_ios::ios_base::failbit;
                }
                else
                {
                    value = result;
...
#    if KWSYS_IOS_USE_ANSI
            ...
            {
                state |= kwsys_ios::ios_base::badbit;
            }
#    endif
        }
...
// Print an integer value to an output stream.
    template < class T > kwsys_ios::ostream & IOStreamPrintTemplate(kwsys_ios::ostream & os, T value, char type)
    {
#    if KWSYS_IOS_USE_ANSI
...
#    endif
                ...
                char *f = format;
                *f++ = '%';
                if (os.flags() & kwsys_ios::ios_base::showpos)
                {
                    *f++ = '+';
                }
                if (os.flags() & kwsys_ios::ios_base::showbase)
                {
                    *f++ = '#';
                }
#    if defined(_MSC_VER)
...
#    endif
                ...
                if (bflags == kwsys_ios::ios_base::oct)
                {
                    *f++ = 'o';
                }
                else if (bflags != kwsys_ios::ios_base::hex)
                {
                    *f++ = type;
                }
                else if (os.flags() & kwsys_ios::ios_base::uppercase)
                {
                    *f++ = 'X';
                }
                else
...
#    if KWSYS_IOS_USE_ANSI
            ...
            {
                os.clear(os.rdstate() | kwsys_ios::ios_base::badbit);
            }
#    endif
        }
        return os;
