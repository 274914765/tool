...
/*--------------------------------------------------------------------------*/
static int kwsysTerminalStreamIsVT100(FILE * stream, int default_vt100, int default_tty);
static void kwsysTerminalSetVT100Color(FILE * stream, int color);
#if defined(KWSYS_TERMINAL_SUPPORT_CONSOLE)
static HANDLE kwsysTerminalGetStreamHandle(FILE * stream);
static void kwsysTerminalSetConsoleColor(HANDLE hOut, CONSOLE_SCREEN_BUFFER_INFO * hOutInfo, FILE * stream, int color);
#endif
...
void kwsysTerminal_cfprintf(int color, FILE * stream, const char *format, ...)
{
...
#endif
    if (!pipeIsConsole && kwsysTerminalStreamIsVT100(stream, default_vt100, default_tty))
    {
        pipeIsVT100 = 1;
    ...
    if (pipeIsConsole)
    {
        kwsysTerminalSetConsoleColor(hOut, &hOutInfo, stream, kwsysTerminal_Color_Normal);
    }
#endif
    if (pipeIsVT100)
    {
