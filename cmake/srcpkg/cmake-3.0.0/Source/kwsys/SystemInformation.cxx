...
#            ifdef CPUID_AWARE_COMPILER
            ...
            these registers to change. push eax push ebx push ecx push edx
#            endif
             ;
            <<CPUID >> mov eax, select
#            ifdef CPUID_AWARE_COMPILER
                cpuid
#            else
                _asm _emit 0x0f _asm _emit 0xa2
#            endif
            mov tmp[0 * TYPE int], eax mov tmp[1 * TYPE int], ebx mov tmp[2 * TYPE int], ecx mov tmp[3 * TYPE int], edx
#            ifdef CPUID_AWARE_COMPILER
...
public:
        ...
        LongLong GetProcMemoryAvailable(const char *hostLimitEnvVarName, const char *procLimitEnvVarName);
        ...
        // get current stack
        static kwsys_stl::string GetProgramStack(int firstFrame, int wholePath);
...
protected:
        ...
        bool RetreiveInformationFromCpuInfoFile();
          kwsys_stl::string ExtractValueFromCpuInfoFile(kwsys_stl::string buffer, const char *word, size_t init = 0);
    ...
    kwsys_stl::string SystemInformation::GetCPUDescription()
    {
        ...
        {
            oss << this->GetProcessorClockFrequency() << " MHz " << this->GetVendorString() << " " << this->GetExtendedProcessorName();
        }
        else
        {
    ...
    {
        kwsys_ios::ostringstream oss;
        oss << this->GetOSName() << " " << this->GetOSRelease() << " " << this->GetOSVersion();
...
// Retrieve memory information in megabyte.
    ...
    kwsys_stl::string SystemInformation::GetMemoryDescription(const char *hostLimitEnvVarName, const char *procLimitEnvVarName)
    {
        kwsys_ios::ostringstream oss;
        oss << "Host Total: "
            << iostreamLongLong(this->GetHostMemoryTotal())
            << " KiB, Host Available: " << iostreamLongLong(this->GetHostMemoryAvailable(hostLimitEnvVarName)) << " KiB, Process Available: " << iostreamLongLong(this->GetProcMemoryAvailable(hostLimitEnvVarName, procLimitEnvVarName)) << " KiB";
        return oss.str();
    }
...
// host memory info in units of KiB.
    ...
    SystemInformation::LongLong SystemInformation::GetHostMemoryAvailable(const char *hostLimitEnvVarName)
    {
...
// process memory info in units of KiB.
    SystemInformation::LongLong SystemInformation::GetProcMemoryAvailable(const char *hostLimitEnvVarName, const char *procLimitEnvVarName)
    {
        return this->Implementation->GetProcMemoryAvailable(hostLimitEnvVarName, procLimitEnvVarName);
    }
...
// *****************************************************************************
        ...
        int LoadLines(FILE * file, kwsys_stl::vector < kwsys_stl::string > &lines)
        {
...
#    if defined(__linux)
// *****************************************************************************
        int LoadLines(const char *fileName, kwsys_stl::vector < kwsys_stl::string > &lines)
        {
...
// ****************************************************************************
        template < typename T > int NameValue(kwsys_stl::vector < kwsys_stl::string > &lines, kwsys_stl::string name, T & value)
        {
...
#if defined(__linux)
// ****************************************************************************
        template < typename T > int GetFieldsFromFile(const char *fileName, const char **fieldNames, T * values)
        {
...
// ****************************************************************************
        template < typename T > int GetFieldFromFile(const char *fileName, const char *fieldName, T & value)
        {
...
// ****************************************************************************
#if defined(__APPLE__)
        template < typename T > int GetFieldsFromCommand(const char *command, const char **fieldNames, T * values)
        {
...
// ****************************************************************************
        ...
        void StacktraceSignalHandler(int sigNo, siginfo_t * sigInfo, void * /*sigContext */ )
        {
#    if defined(__linux) || defined(__APPLE__)
            kwsys_ios::ostringstream oss;
            oss << kwsys_ios::endl << "=========================================================" << kwsys_ios::endl << "Process id " << getpid() << " ";
            switch (sigNo)
            {
                case SIGINT:
                    oss << "Caught SIGINT";
                ...
                case SIGFPE:
                    oss << "Caught SIGFPE at " << (sigInfo->si_addr == 0 ? "0x" : "") << sigInfo->si_addr << " ";
...
#        endif
                ...
                case SIGSEGV:
                    oss << "Caught SIGSEGV at " << (sigInfo->si_addr == 0 ? "0x" : "") << sigInfo->si_addr << " ";
                    switch (sigInfo->si_code)
                    {
                        case SEGV_MAPERR:
                ...
                case SIGBUS:
                    oss << "Caught SIGBUS at " << (sigInfo->si_addr == 0 ? "0x" : "") << sigInfo->si_addr << " ";
                    switch (sigInfo->si_code)
                    {
                        case BUS_ADRALN:
...
#        endif
                ...
                case SIGILL:
                    oss << "Caught SIGILL at " << (sigInfo->si_addr == 0 ? "0x" : "") << sigInfo->si_addr << " ";
                    switch (sigInfo->si_code)
                    {
                        case ILL_ILLOPC:
                            oss << "illegal opcode";
...
#        endif
            ...
            oss << kwsys_ios::endl << "Program Stack:" << kwsys_ios::endl << SystemInformationImplementation::GetProgramStack(2, 0) << "=========================================================" << kwsys_ios::endl;
...
// --------------------------------------------------------------------------
        kwsys_ios::ostream & operator<<(kwsys_ios::ostream & os, const SymbolProperties & sp)
        {
#    if defined(KWSYS_SYSTEMINFORMATION_HAS_SYMBOL_LOOKUP)
            os << kwsys_ios::hex << sp.GetAddress() << " : " << sp.GetFunction() << " [(" << sp.GetBinary() << ") " << sp.GetSourceFile() << ":" << kwsys_ios::dec << sp.GetLineNumber() << "]";
...
#    if defined(KWSYS_SYSTEMINFORMATION_HAS_CPP_DEMANGLE)
            ...
            char *demangledSymbol = abi::__cxa_demangle(symbol, buffer, &bufferLen, &status);
            if (!status)
            {
...
/** Get the FQDN */
    int SystemInformationImplementation::GetFullyQualifiedDomainName(kwsys_stl::string & fqdn)
    {
        ...
        for (ifa = ifas; ifa != NULL; ifa = ifa->ifa_next)
        {
            ...
            if ((fam == AF_INET) || (fam == AF_INET6))
            {
                ...
                const size_t addrlen = (fam == AF_INET ? sizeof(struct sockaddr_in) : sizeof(struct sockaddr_in6));

                ierr = getnameinfo(ifa->ifa_addr, static_cast < socklen_t > (addrlen), host, NI_MAXHOST, NULL, 0, NI_NAMEREQD);
                if (ierr)
                {
...
/** Return the chosen cache size. */
    ...
    bool SystemInformationImplementation::DoesCPUSupportFeature(long int dwFeature)
    {
        ...
        // Check for MMX+ instructions.
        if (((dwFeature & MMX_PLUS_FEATURE) != 0) && this->Features.ExtendedFeatures.HasMMXPlus)
            bHasFeature = true;
        ...
        if (((dwFeature & SSE_MMX_FEATURE) != 0) && this->Features.ExtendedFeatures.HasSSEMMX)
            bHasFeature = true;
        ...
        if (((dwFeature & AMD_3DNOW_FEATURE) != 0) && this->Features.ExtendedFeatures.Has3DNow)
            bHasFeature = true;

        // Check for 3DNow+ instructions.
        if (((dwFeature & AMD_3DNOW_PLUS_FEATURE) != 0) && this->Features.ExtendedFeatures.Has3DNowPlus)
            bHasFeature = true;
        ...
        if (((dwFeature & L1CACHE_FEATURE) != 0) && (this->Features.L1CacheSize != -1))
            bHasFeature = true;

        // Check for L2 cache size.
        if (((dwFeature & L2CACHE_FEATURE) != 0) && (this->Features.L2CacheSize != -1))
            bHasFeature = true;

        // Check for L3 cache size.
        if (((dwFeature & L3CACHE_FEATURE) != 0) && (this->Features.L3CacheSize != -1))
            bHasFeature = true;
        ...
        if (((dwFeature & TEMPSENSEDIODE_FEATURE) != 0) && this->Features.ExtendedFeatures.PowerManagement.HasTempSenseDiode)
            bHasFeature = true;

        // Check for frequency ID support.
        if (((dwFeature & FREQUENCYID_FEATURE) != 0) && this->Features.ExtendedFeatures.PowerManagement.HasFrequencyID)
            bHasFeature = true;

        // Check for voltage ID support.
        if (((dwFeature & VOLTAGEID_FREQUENCY) != 0) && this->Features.ExtendedFeatures.PowerManagement.HasVoltageID)
            bHasFeature = true;
...
#    if USE_ASM_INSTRUCTIONS
        ...
        if (this->Features.HasSSE)
        {
            ...
            __try
            {
                ...
                _asm
                {
                _emit 0x0f _emit 0x56 _emit 0xc0}
...
/** */
    bool SystemInformationImplementation::RetrieveCPUClockSpeed()
    {
        ...
        if (this->Features.HasTSC && GetCyclesDifference(SystemInformationImplementation::Delay, uiMSecPerRepetition) > 0)
        {
        ...
        if (!retrieved)
        {
            ...
            if (ERROR_SUCCESS == err)
            {
                ...
                err = RegQueryValueExW(hKey, L"~MHz", 0, &dwType, (LPBYTE) & data, &dwSize);
...
/** */
    bool SystemInformationImplementation::RetrieveClassicalCPUClockSpeed()
    {
        ...
        __try
        {
            _asm
            {
            mov eax, 0x80000000 mov ebx, CLASSICAL_CPU_FREQ_LOOP Timer_Loop: bsf ecx, eax dec ebx jnz Timer_Loop}
        }
        __except(1)
        {
            return false;
...
/** */
    bool SystemInformationImplementation::RetrieveProcessorSerialNumber()
    {
        ...
        sprintf(sn, "%.2x%.2x-%.2x%.2x-%.2x%.2x-%.2x%.2x-%.2x%.2x-%.2x%.2x",
                ...
                ((SerialNumber[2] & 0x000000ff) >> 0), ((SerialNumber[3] & 0xff000000) >> 24), ((SerialNumber[3] & 0x00ff0000) >> 16), ((SerialNumber[3] & 0x0000ff00) >> 8), ((SerialNumber[3] & 0x000000ff) >> 0));
                    ...
                    case 6:
                        switch (this->ChipID.Model)
                        {
                            ...
                            case 7:
                                ...
                                break;
                            case 8:
                                this->ChipID.ProcessorName = "Pentium III (0.18 micron) With 256 KB On-Die L2 Cache ";
                                break;
                            case 0xa:
                                this->ChipID.ProcessorName = "Pentium III (0.18 micron) With 1 Or 2 MB On-Die L2 Cache ";
                                break;
                            case 0xb:
                                this->ChipID.ProcessorName = "Pentium III (0.13 micron) With 256 Or 512 KB On-Die L2 Cache ";
                                break;
                            case 23:
                                this->ChipID.ProcessorName = "Intel(R) Core(TM)2 Duo CPU     T9500  @ 2.60GHz";
                                break;
                            default:
...
/** Query for the cpu status */
    bool SystemInformationImplementation::RetreiveInformationFromCpuInfoFile()
    {
        ...
        kwsys_stl::string cores = this->ExtractValueFromCpuInfoFile(buffer, "cpu cores");
        int numberOfCoresPerCPU = atoi(cores.c_str());
        if (maxId > 0)
        {
            this->NumberOfPhysicalCPU = static_cast < unsigned int >(numberOfCoresPerCPU * (maxId + 1));
        }
        else
        {
            // Linux Sparc: get cpu count
            this->NumberOfPhysicalCPU = atoi(this->ExtractValueFromCpuInfoFile(buffer, "ncpus active").c_str());
        }

#else                            // __CYGWIN__
        // does not have "physical id" entries, neither "cpu cores"
        // this has to be fixed for hyper-threading.
        kwsys_stl::string cpucount = this->ExtractValueFromCpuInfoFile(buffer, "cpu count");
        this->NumberOfPhysicalCPU = this->NumberOfLogicalCPU = atoi(cpucount.c_str());
#endif
        ...
        else
        {
            ...
            this->CPUSpeedInMHz = static_cast < float >(strtoull(CPUSpeed.c_str(), 0, 16)) / 1000000.0f;
        }
#endif

        // Chip family
        kwsys_stl::string familyStr = this->ExtractValueFromCpuInfoFile(buffer, "cpu family");
        if (familyStr.empty())
        {
...
*/
    SystemInformation::LongLong SystemInformationImplementation::GetHostMemoryTotal()
    {
#if defined(_WIN32)
...
*/
    SystemInformation::LongLong SystemInformationImplementation::GetHostMemoryAvailable(const char *hostLimitEnvVarName)
    {
...
*/
    SystemInformation::LongLong SystemInformationImplementation::GetProcMemoryAvailable(const char *hostLimitEnvVarName, const char *procLimitEnvVarName)
    {
        SystemInformation::LongLong memAvail = this->GetHostMemoryAvailable(hostLimitEnvVarName);
...
*/
    SystemInformation::LongLong SystemInformationImplementation::GetHostMemoryUsed()
    {
#if defined(_WIN32)
#    if defined(_MSC_VER) && _MSC_VER < 1300
        MEMORYSTATUS stat;
...
#    endif
        ...
        SystemInformation::LongLong values[2] =
        {
        SystemInformation::LongLong(0)};
        ...
        SystemInformation::LongLong values[3] =
        {
        SystemInformation::LongLong(0)};
...
*/
    SystemInformation::LongLong SystemInformationImplementation::GetProcMemoryUsed()
    {
...
*/
    kwsys_stl::string SystemInformationImplementation::GetProgramStack(int firstFrame, int wholePath)
    {
...
#else
#    if !defined(KWSYS_SYSTEMINFORMATION_HAS_SYMBOL_LOOKUP)
            "WARNING: Function names will not be demangled because " "dladdr is not available.\n"
#    endif
#    if !defined(KWSYS_SYSTEMINFORMATION_HAS_CPP_DEMANGLE)
            "WARNING: Function names will not be demangled " "because cxxabi is not available.\n"
#    endif
#endif
            ;
...
#    endif
    ...
    bool SystemInformationImplementation::QueryLinuxMemory()
    {
        ...
        if (linuxMajor >= 3 || ((linuxMajor >= 2) && (linuxMinor >= 6)))
        {
            ...
            enum
            { mMemTotal, mMemFree, mBuffers, mCached, mSwapTotal, mSwapFree };
            const char *format[6] = { "MemTotal:%lu kB", "MemFree:%lu kB", "Buffers:%lu kB",
            ...
            if (count == 6)
            {
                ...
                this->AvailablePhysicalMemory = (value[mMemFree] + value[mBuffers] + value[mCached]) / 1024;
...
/** Get Cycle differences */
    SystemInformation::LongLong SystemInformationImplementation::GetCyclesDifference(DELAY_FUNC DelayFunction, unsigned int uiParameter)
    {
#if defined(_MSC_VER) && (_MSC_VER >= 1400)
        ...
        __try
        {
            ...
            {
                push uiParameter;
                push parameter param mov ebx, DelayFunction;
                store func in ebx RDTSC_INSTRUCTION mov esi, eax;
                esi = eax mov edi, edx;
                edi = edx call ebx;
                call the delay functions RDTSC_INSTRUCTION pop ebx mov edx2, edx;
                edx2 = edx mov eax2, eax;
                eax2 = eax mov edx1, edi;
                edx2 = edi mov eax1, esi;
            eax2 = esi}
        }
        __except(1)
        {
...
// CPU Info
        ...
        this->Features.ExtendedFeatures.LogicalProcessorsPerPhysical = this->LogicalCPUPerPhysicalCPU();
        ...
        err = sysctlbyname("machdep.cpu.family", &this->ChipID.Family, &len, NULL, 0);
...
/** Run a given process */
    kwsys_stl::string SystemInformationImplementation::RunProcess(kwsys_stl::vector < const char *>args)
    {
        ...
        while ((pipe = kwsysProcess_WaitForData(gp, &data, &length, &timeout), (pipe == kwsysProcess_Pipe_STDOUT || pipe == kwsysProcess_Pipe_STDERR)))    // wait for 1s
        {
        ...
        switch (kwsysProcess_GetState(gp))
        {
            ...
            case kwsysProcess_State_Exception:
                {
                    kwsys_ios::cerr << "Error: " << args[0] << " terminated with an exception: " << kwsysProcess_GetExceptionString(gp) << "\n";
                }
                break;
            ...
            case kwsysProcess_State_Killed:
                {
                    // Should not get here.
                    kwsys_ios::cerr << "Unexpected ending state after running " << args[0] << kwsys_ios::endl;
                }
                break;
        }
...
#    endif
        ...
        char *tail;
        unsigned long totalMemory = strtoul(this->ParseValueFromKStat("-s physmem").c_str(), &tail, 0);
...
/** Querying for system information from Haiku OS */
    bool SystemInformationImplementation::QueryHaikuInfo()
    {
        ...
        this->AvailablePhysicalMemory = this->TotalPhysicalMemory - ((info.used_pages * B_PAGE_SIZE) / (1024 * 1024));
...
#    endif                        // VER_NT_WORKSTATION
                ...
                if (osvi.dwMajorVersion <= 4)
                {
                    ...
                    sprintf(operatingSystem, "version %ld.%ld %s (Build %ld)", osvi.dwMajorVersion, osvi.dwMinorVersion, osvi.szCSDVersion, osvi.dwBuildNumber & 0xFFFF);
                    this->OSVersion = operatingSystem;
                }
...
#    endif
    ...
    int SystemInformationImplementation::CallSwVers(const char *arg, kwsys_stl::string & ver)
    {
#ifdef __APPLE__
        kwsys_stl::vector < const char *>args;
