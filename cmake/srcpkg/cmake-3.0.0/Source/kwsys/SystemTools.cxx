...
inline void Realpath(const char *path, kwsys_stl::string & resolved_path)
{
    ...
    if (GetFullPathNameW(tmp.c_str(), sizeof(fullpath) / sizeof(fullpath[0]), fullpath, &ptemp))
    {
...
extern "C"
{
    ...
    extern char *realpath(__const char *__restrict __name, char *__restrict __resolved) __THROW;
...
namespace KWSYS_NAMESPACE
{
    ...
    double SystemTools::GetTime(void)
    {
        ...
        FILETIME ft;
          ...
          return (429.4967296 * ft.dwHighDateTime + 0.0000001 * ft.dwLowDateTime - 11644473600.0);
#else
        struct timeval t;
          gettimeofday(&t, 0);
    ...
    class SystemToolsTranslationMap: public kwsys_stl::map < kwsys_stl::string, kwsys_stl::string >
    {
    };
...
// adds the elements of the env variable path to the arg passed in
    void SystemTools::GetPath(kwsys_stl::vector < kwsys_stl::string > &path, const char *env)
    {
        ...
        for (kwsys_stl::vector < kwsys_stl::string >::iterator i = path.begin(); i != path.end(); ++i)
        {
...
/* Manipulate the "environ" global directly.  */
    static int kwsysUnPutEnv(const char *env)
    {
        ...
        while (environ[in])
        {
            if (strlen(environ[in]) > len && environ[in][len] == '=' && strncmp(env, environ[in], len) == 0)
            {
                ++in;
            }
            else
...
#        pragma warning disable 444
                                /* base has non-virtual destructor */
#    endif
...
//----------------------------------------------------------------------------
    ...
    {
#ifdef _WIN32
        return _wfopen(Encoding::ToWide(file).c_str(), Encoding::ToWide(mode).c_str());
#else
...
// write the result into source.
    void SystemTools::ReplaceString(kwsys_stl::string & source, const char *replace, const char *with)
    {
...
#    define KWSYS_ST_KEY_WOW64_64KEY 0x0100
...
#if defined(_WIN32) && !defined(__CYGWIN__)
    static bool SystemToolsParseRegistryKey(const char *key, HKEY & primaryKey, kwsys_stl::string & second, kwsys_stl::string & valuename)
    {
    ...
    static DWORD SystemToolsMakeRegistryMode(DWORD mode, SystemTools::KeyWOW64 view)
    {
...
#if defined(_WIN32) && !defined(__CYGWIN__)
    bool SystemTools::GetRegistrySubKeys(const char *key, kwsys_stl::vector < kwsys_stl::string > &subkeys, KeyWOW64 view)
    {
        ...
        HKEY hKey;
        if (RegOpenKeyExW(primaryKey, Encoding::ToWide(second).c_str(), 0, SystemToolsMakeRegistryMode(KEY_READ, view), &hKey) != ERROR_SUCCESS)
        {
            return false;
        }
        else
...
#else
    bool SystemTools::GetRegistrySubKeys(const char *, kwsys_stl::vector < kwsys_stl::string > &, KeyWOW64)
    {
        return false;
    }
#endif
...
#if defined(_WIN32) && !defined(__CYGWIN__)
    bool SystemTools::ReadRegistryValue(const char *key, kwsys_stl::string & value, KeyWOW64 view)
    {
        bool valueset = false;
        HKEY primaryKey = HKEY_CURRENT_USER;
        kwsys_stl::string second;
        ...
        else
        {
            ...
            dwSize = 1023;
            wchar_t data[1024];
            if (RegQueryValueExW(hKey, Encoding::ToWide(valuename).c_str(), NULL, &dwType, (BYTE *) data, &dwSize) == ERROR_SUCCESS)
            {
                if (dwType == REG_SZ)
                {
...
#else
    bool SystemTools::ReadRegistryValue(const char *, kwsys_stl::string &, KeyWOW64)
    {
        return false;
    }
#endif
...
#if defined(_WIN32) && !defined(__CYGWIN__)
    bool SystemTools::WriteRegistryValue(const char *key, const char *value, KeyWOW64 view)
    {
        HKEY primaryKey = HKEY_CURRENT_USER;
        kwsys_stl::string second;
        kwsys_stl::string valuename;
        ...
        DWORD dwDummy;
        wchar_t lpClass[] = L"";
        if (RegCreateKeyExW(primaryKey, Encoding::ToWide(second).c_str(), 0, lpClass, REG_OPTION_NON_VOLATILE, SystemToolsMakeRegistryMode(KEY_WRITE, view), NULL, &hKey, &dwDummy) != ERROR_SUCCESS)
        {
            return false;
        }

        std::wstring wvalue = Encoding::ToWide(value);
        if (RegSetValueExW(hKey, Encoding::ToWide(valuename).c_str(), 0, REG_SZ, (CONST BYTE *) wvalue.c_str(), (DWORD) (sizeof(wchar_t) * (wvalue.size() + 1))) == ERROR_SUCCESS)
        {
            return true;
        }
        return false;
...
//      =>  will delete  the data of the "Root" value of the key
    ...
    bool SystemTools::DeleteRegistryValue(const char *key, KeyWOW64 view)
    {
        ...
        HKEY hKey;
        if (RegOpenKeyExW(primaryKey, Encoding::ToWide(second).c_str(), 0, SystemToolsMakeRegistryMode(KEY_WRITE, view), &hKey) != ERROR_SUCCESS)
        {
            return false;
        }
        else
    ...
    bool SystemTools::SameFile(const char *file1, const char *file2)
    {
        ...
        hFile1 = CreateFileW(Encoding::ToWide(file1).c_str(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
        hFile2 = CreateFileW(Encoding::ToWide(file2).c_str(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, NULL);
        ...
        CloseHandle(hFile1);
        CloseHandle(hFile2);
        return (fiBuf1.dwVolumeSerialNumber == fiBuf2.dwVolumeSerialNumber && fiBuf1.nFileIndexHigh == fiBuf2.nFileIndexHigh && fiBuf1.nFileIndexLow == fiBuf2.nFileIndexLow);
#else
        ...
        if (stat(file1, &fileStat1) == 0 && stat(file2, &fileStat2) == 0)
        {
            ...
            if (memcmp(&fileStat2.st_dev, &fileStat1.st_dev, sizeof(fileStat1.st_dev)) == 0 && memcmp(&fileStat2.st_ino, &fileStat1.st_ino, sizeof(fileStat1.st_ino)) == 0 && fileStat2.st_size == fileStat1.st_size)
            {
                return true;
            }
        }
...
#elif defined(_WIN32)
        return (GetFileAttributesW(Encoding::ToWide(filename).c_str()) != INVALID_FILE_ATTRIBUTES);
#else
        return access(filename, R_OK) == 0;
#endif
    }
...
//----------------------------------------------------------------------------
    ...
    {
        SystemToolsTranslationMap::iterator i = SystemTools::Cyg2Win32Map->find(path);
...
#    endif
    ...
    bool SystemTools::FileTimeCompare(const char *f1, const char *f2, int *result)
    {
...
#    endif
        ...
        if (!GetFileAttributesExW(Encoding::ToWide(f1).c_str(), GetFileExInfoStandard, &f1d))
        {
            return false;
        }
        if (!GetFileAttributesExW(Encoding::ToWide(f2).c_str(), GetFileExInfoStandard, &f2d))
        {
            return false;
        }
...
// Return capitalized words
    kwsys_stl::string SystemTools::CapitalizedWords(const kwsys_stl::string & s)
    {
        ...
        for (size_t i = 0; i < s.size(); i++)
        {
            ...
            if ((int) s[i] >= 0 && isalpha(s[i]) && (i == 0 || ((int) s[i - 1] >= 0 && isspace(s[i - 1]))))
#else
...
// only works for words with at least two letters
    kwsys_stl::string SystemTools::AddSpaceBetweenCapitalizedWords(const kwsys_stl::string & s)
    {
        kwsys_stl::string n;
        if (s.size())
        {
    ...
    char *SystemTools::AppendStrings(const char *str1, const char *str2, const char *str3)
    {
        if (!str1)
        {
...
// Return a cropped string
    kwsys_stl::string SystemTools::CropString(const kwsys_stl::string & s, size_t max_len)
    {
...
//----------------------------------------------------------------------------
    ...
    kwsys_stl::string SystemTools::EscapeChars(const char *str, const char *chars_to_escape, char escape_char)
    {
        kwsys_stl::string n;
        if (str)
        {
...
// remove double slashes not at the start
    kwsys_stl::string SystemTools::ConvertToWindowsOutputPath(const char *path)
    {
        ...
        if (ret.find(' ') != kwsys_stl::string::npos && ret[0] != '\"')
        {
            ret.insert(static_cast < kwsys_stl::string::size_type > (0), static_cast < kwsys_stl::string::size_type > (1), '\"');
            ret.append(1, '\"');
        }
        return ret;
    }

    bool SystemTools::CopyFileIfDifferent(const char *source, const char *destination)
    {
...
#define KWSYS_ST_BUFFER 4096
    ...
    bool SystemTools::FilesDiffer(const char *source, const char *destination)
    {

#if defined(_WIN32)
        WIN32_FILE_ATTRIBUTE_DATA statSource;
        if (GetFileAttributesExW(Encoding::ToWide(source).c_str(), GetFileExInfoStandard, &statSource) == 0)
        {
            return true;
        }

        WIN32_FILE_ATTRIBUTE_DATA statDestination;
        if (GetFileAttributesExW(Encoding::ToWide(destination).c_str(), GetFileExInfoStandard, &statDestination) == 0)
        {
            return true;
        }

        if (statSource.nFileSizeHigh != statDestination.nFileSizeHigh || statSource.nFileSizeLow != statDestination.nFileSizeLow)
        {
            return true;
        }
        ...
        off_t nleft = ((__int64) statSource.nFileSizeHigh << 32) + statSource.nFileSizeLow;
...
#if defined(_WIN32)
        kwsys::ifstream finSource(source, (kwsys_ios::ios::binary | kwsys_ios::ios::in));
        kwsys::ifstream finDestination(destination, (kwsys_ios::ios::binary | kwsys_ios::ios::in));
#else
        ...
        while (nleft > 0)
        {
            ...
            if (static_cast < kwsys_ios::streamsize > (finSource.gcount()) != nnext || static_cast < kwsys_ios::streamsize > (finDestination.gcount()) != nnext)
            {
                return true;
            }

            // If this block differs the file differs.
            if (memcmp(static_cast < const void *>(source_buf), static_cast < const void *>(dest_buf), static_cast < size_t > (nnext)) != 0)
            {
                return true;
            }
 ...
 */
    bool SystemTools::CopyFileAlways(const char *source, const char *destination)
    {
        ...
        if (SystemTools::FileExists(destination) && SystemTools::FileIsDirectory(destination))
        {
...
#if defined(_WIN32) || defined(__CYGWIN__)
        kwsys::ifstream fin(source, kwsys_ios::ios::binary | kwsys_ios::ios::in);
#else
...
#if defined(_WIN32) || defined(__CYGWIN__)
        kwsys::ofstream fout(destination, kwsys_ios::ios::binary | kwsys_ios::ios::out | kwsys_ios::ios::trunc);
#else
        kwsys::ofstream fout(destination, kwsys_ios::ios::out | kwsys_ios::ios::trunc);
#endif
        if (!fout)
        {
            return false;
...
//----------------------------------------------------------------------------
    bool SystemTools::CopyAFile(const char *source, const char *destination, bool always)
    {
        if (always)
        {
            return SystemTools::CopyFileAlways(source, destination);
 ...
 * "destination".
 */
    bool SystemTools::CopyADirectory(const char *source, const char *destination, bool always)
    {
        Directory dir;
        dir.Load(source);
        size_t fileNum;
        ...
        {
            if (strcmp(dir.GetFile(static_cast < unsigned long >(fileNum)), ".") && strcmp(dir.GetFile(static_cast < unsigned long >(fileNum)), ".."))
            {
                ...
                if (SystemTools::FileIsDirectory(fullPath.c_str()))
                {
                    ...
                    if (!SystemTools::CopyADirectory(fullPath.c_str(), fullDestPath.c_str(), always))
                    {
                        return false;
                    }
                }
    ...
    bool SystemTools::RemoveADirectory(const char *source)
    {
        ...
        for (fileNum = 0; fileNum < dir.GetNumberOfFiles(); ++fileNum)
        {
            if (strcmp(dir.GetFile(static_cast < unsigned long >(fileNum)), ".") && strcmp(dir.GetFile(static_cast < unsigned long >(fileNum)), ".."))
            {
                ...
                fullPath += "/";
                fullPath += dir.GetFile(static_cast < unsigned long >(fileNum));
                if (SystemTools::FileIsDirectory(fullPath.c_str()) && !SystemTools::FileIsSymlink(fullPath.c_str()))
                {
 ...
 */
    kwsys_stl::string SystemTools::FindName(const char *name, const kwsys_stl::vector < kwsys_stl::string > &userPaths, bool no_system_path)
    {
        ...
        {
            for (kwsys_stl::vector < kwsys_stl::string >::const_iterator i = userPaths.begin(); i != userPaths.end(); ++i)
            {
                path.push_back(*i);
            }
        }
        ...
        for (kwsys_stl::vector < kwsys_stl::string >::const_iterator p = path.begin(); p != path.end(); ++p)
        {
            tryPath = *p;
            tryPath += name;
 ...
 * found.  Otherwise, the empty string is returned.
 */
    kwsys_stl::string SystemTools::FindFile(const char *name, const kwsys_stl::vector < kwsys_stl::string > &userPaths, bool no_system_path)
    {
 ...
 * found.  Otherwise, the empty string is returned.
 */
    kwsys_stl::string SystemTools::FindDirectory(const char *name, const kwsys_stl::vector < kwsys_stl::string > &userPaths, bool no_system_path)
    {
        kwsys_stl::string tryPath = SystemTools::FindName(name, userPaths, no_system_path);
        if (tryPath != "" && SystemTools::FileIsDirectory(tryPath.c_str()))
        {
 ...
 * found.  Otherwise, the empty string is returned.
 */
    kwsys_stl::string SystemTools::FindProgram(const char *nameIn, const kwsys_stl::vector < kwsys_stl::string > &userPaths, bool no_system_path)
    {
        if (!nameIn || !*nameIn)
        {
            return "";
        ...
        if (extensions.size())
        {
            for (kwsys_stl::vector < kwsys_stl::string >::iterator i = extensions.begin(); i != extensions.end(); ++i)
            {
                tryPath = name;
                tryPath += *i;
        ...
        // Try each path
        for (kwsys_stl::vector < kwsys_stl::string >::iterator p = path.begin(); p != path.end(); ++p)
        {
#ifdef _WIN32
            ...
            if (extensions.size())
            {
                for (kwsys_stl::vector < kwsys_stl::string >::iterator ext = extensions.begin(); ext != extensions.end(); ++ext)
                {
                    tryPath = *p;
                    tryPath += name;
                    tryPath += *ext;
    ...
    kwsys_stl::string SystemTools::FindProgram(const kwsys_stl::vector < kwsys_stl::string > &names, const kwsys_stl::vector < kwsys_stl::string > &path, bool noSystemPath)
    {
        for (kwsys_stl::vector < kwsys_stl::string >::const_iterator it = names.begin(); it != names.end(); ++it)
        {
            // Try to find the program.
            kwsys_stl::string result = SystemTools::FindProgram(it->c_str(), path, noSystemPath);
            if (!result.empty())
            {
                return result;
            }
 ...
 * found.  Otherwise, the empty string is returned.
 */
    kwsys_stl::string SystemTools::FindLibrary(const char *name, const kwsys_stl::vector < kwsys_stl::string > &userPaths)
    {
    ...
    bool SystemTools::FileIsDirectory(const char *name)
    {
        ...
        if (last > 0 && (name[last] == '/' || name[last] == '\\') && strcmp(name, "/") != 0 && name[last - 1] != ':')
        {
...
#else
    bool SystemTools::ReadSymlink(const char *newName, kwsys_stl::string & origName)
    {
        char buf[KWSYS_SYSTEMTOOLS_MAXPATH + 1];
        int count = static_cast < int >(readlink(newName, buf, KWSYS_SYSTEMTOOLS_MAXPATH));
        if (count >= 0)
        {
            // Add null-terminator.
            buf[count] = 0;
    ...
    bool SystemTools::SplitProgramPath(const char *in_name, kwsys_stl::string & dir, kwsys_stl::string & file, bool)
    {
        dir = in_name;
        file = "";
        SystemTools::ConvertToUnixSlashes(dir);
    ...
    bool SystemTools::FindProgramPath(const char *argv0, kwsys_stl::string & pathOut, kwsys_stl::string & errorMsg, const char *exeName, const char *buildDir, const char *installPrefix)
    {
    ...
    void SystemTools::AddTranslationPath(const char *a, const char *b)
    {
        ...
        if (SystemTools::FileIsDirectory(path_a.c_str()))
        {
            ...
            if (SystemTools::FileIsFullPath(path_b.c_str()) && path_b.find("..") == kwsys_stl::string::npos)
            {
                ...
                if (!(path_a == path_b))
                {
                    SystemTools::TranslationMap->insert(SystemToolsTranslationMap::value_type(path_a, path_b));
                }
            }
        }
    }
    ...
    void SystemTools::CheckTranslationPath(kwsys_stl::string & path)
    {
        ...
        for (it = SystemTools::TranslationMap->begin(); it != SystemTools::TranslationMap->end(); ++it)
        {
    ...
    static void SystemToolsAppendComponents(kwsys_stl::vector < kwsys_stl::string > &out_components, kwsys_stl::vector < kwsys_stl::string >::const_iterator first, kwsys_stl::vector < kwsys_stl::string >::const_iterator last)
    {
        for (kwsys_stl::vector < kwsys_stl::string >::const_iterator i = first; i != last; ++i)
        {
            if (*i == "..")
            {
    ...
    kwsys_stl::string SystemTools::CollapseFullPath(const char *in_path, const char *in_base)
    {
        ...
        if (path_components[0].length() == 0)
        {
            ...
            SystemToolsAppendComponents(out_components, base_components.begin() + 1, base_components.end());
        }

        // Append input path components to the output path.
        SystemToolsAppendComponents(out_components, path_components.begin(), path_components.end());
...
// compute the relative path from here to there
    kwsys_stl::string SystemTools::RelativePath(const char *local, const char *remote)
    {
        ...
        while (((sameCount <= (localSplit.size() - 1)) && (sameCount <= (remoteSplit.size() - 1))) &&
// for windows and apple do a case insensitive string compare
#if defined(_WIN32) || defined(__APPLE__)
               SystemTools::Strucmp(localSplit[sameCount].c_str(), remoteSplit[sameCount].c_str()) == 0
#else
        ...
        // to the final path.
        for (kwsys_stl::vector < String >::iterator vit = remoteSplit.begin(); vit != remoteSplit.end(); ++vit)
        {
            if (vit->size())
            {
        ...
        for (kwsys_stl::vector < String >::iterator vit1 = finalPath.begin(); vit1 != finalPath.end(); ++vit1)
        {
...
#ifdef _WIN32
    static int GetCasePathName(const kwsys_stl::string & pathIn, kwsys_stl::string & casePath)
    {
        kwsys_stl::vector < kwsys_stl::string > path_components;
...
//----------------------------------------------------------------------------
    kwsys_stl::string SystemTools::GetActualCaseForPath(const char *p)
    {
        ...
        SystemToolsTranslationMap::iterator i = SystemTools::LongPathMap->find(casePath);
...
//----------------------------------------------------------------------------
    const char *SystemTools::SplitPathRootComponent(const char *p, kwsys_stl::string * root)
    {
...
//----------------------------------------------------------------------------
    void SystemTools::SplitPath(const char *p, kwsys_stl::vector < kwsys_stl::string > &components, bool expand_home_dir)
    {
        const char *c = p;
        components.clear();
...
#endif
                if (!homedir.empty() && (homedir[homedir.size() - 1] == '/' || homedir[homedir.size() - 1] == '\\'))
                {
        ...
        for (; *last; ++last)
        {
            if (*last == '/' || *last == '\\')
            {
                ...
                components.push_back(kwsys_stl::string(first, static_cast < kwsys_stl::string::size_type > (last - first)));
                first = last + 1;
            }
        }
...
//----------------------------------------------------------------------------
    kwsys_stl::string SystemTools::JoinPath(const kwsys_stl::vector < kwsys_stl::string > &components)
    {
        return SystemTools::JoinPath(components.begin(), components.end());
    }

//----------------------------------------------------------------------------
    kwsys_stl::string SystemTools::JoinPath(kwsys_stl::vector < kwsys_stl::string >::const_iterator first, kwsys_stl::vector < kwsys_stl::string >::const_iterator last)
    {
 ...
 * from .tar.gz)
 */
    kwsys_stl::string SystemTools::GetFilenameWithoutLastExtension(const kwsys_stl::string & filename)
    {
        kwsys_stl::string name = SystemTools::GetFilenameName(filename);
        kwsys_stl::string::size_type dot_pos = name.rfind(".");
        if (dot_pos != kwsys_stl::string::npos)
    ...
    bool SystemTools::FileHasSignature(const char *filename, const char *signature, long offset)
    {
    ...
    SystemTools::FileTypeEnum SystemTools::DetectFileType(const char *filename, unsigned long length, double percent_bin)
    {
    ...
    bool SystemTools::LocateFileInDir(const char *filename, const char *dir, kwsys_stl::string & filename_found, int try_filename_dirs)
    {
        if (!filename || !dir)
        {
            return false;
        ...
        {
            size_t dir_len = strlen(dir);
            int need_slash = (dir_len && dir[dir_len - 1] != '/' && dir[dir_len - 1] != '\\');
            ...
            else if (try_filename_dirs)
            {
                ...
                do
                {
...
#if defined( _WIN32 )
                    if (!filename_dir_base.size() || filename_dir_base[filename_dir_base.size() - 1] == ':')
#else
                    ...
                    res = SystemTools::LocateFileInDir(filename_base.c_str(), temp.c_str(), filename_found, 0);
    ...
    bool SystemTools::GetShortPath(const char *path, kwsys_stl::string & shortPath)
    {
        ...
        buffer[0] = 0;
        ret = GetShortPathNameW(Encoding::ToWide(tempPath).c_str(), &buffer[0], static_cast < DWORD > (wtempPath.size()));
    ...
    void SystemTools::SplitProgramFromArgs(const char *path, kwsys_stl::string & program, kwsys_stl::string & args)
    {
    ...
    kwsys_stl::string SystemTools::MakeCidentifier(const char *s)
    {
        ...
        kwsys_stl::string permited_chars("_" "abcdefghijklmnopqrstuvwxyz" "ABCDEFGHIJKLMNOPQRSTUVWXYZ" "0123456789");
        kwsys_stl::string::size_type pos = 0;
...
// if any data were read before the end-of-file was reached.
    bool SystemTools::GetLineFromStream(kwsys_ios::istream & is, kwsys_stl::string & line, bool * has_newline /* = 0 */ ,
                                        long sizeLimit /* = -1 */ )
    {
        ...
        while (!haveNewline && leftToRead != 0 && (is.clear(is.rdstate() & ~kwsys_ios::ios::failbit), is.getline(buffer, bufferSize), is.gcount() > 0))
        {
    ...
    bool SystemTools::GetPermissions(const char *file, mode_t & mode)
    {
        ...
        {
            mode = (_S_IWRITE | (_S_IWRITE >> 3) | (_S_IWRITE >> 6)) | (_S_IREAD | (_S_IREAD >> 3) | (_S_IREAD >> 6));
        }
        ...
        if (ext && (Strucmp(ext, ".exe") == 0 || Strucmp(ext, ".com") == 0 || Strucmp(ext, ".cmd") == 0 || Strucmp(ext, ".bat") == 0))
        {
    ...
    void SystemTools::ConvertWindowsCommandLineToUnixArguments(const char *cmd_line, int *argc, char ***argv)
    {
...
#    define BUFSIZE 80
                    ...
                    lRet = RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\Control\\ProductOptions", 0, KEY_QUERY_VALUE, &hKey);
                    if (lRet != ERROR_SUCCESS)
                    {
                        return 0;
                    }

                    lRet = RegQueryValueExW(hKey, L"ProductType", NULL, NULL, (LPBYTE) szProductType, &dwBufLen);
                ...
                if (osvi.dwMajorVersion == 4 && lstrcmpiA(osvi.szCSDVersion, "Service Pack 6") == 0)
                {
                    ...
                    lRet = RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Hotfix\\Q246009", 0, KEY_QUERY_VALUE, &hKey);
...
// ----------------------------------------------------------------------
    bool SystemTools::ParseURLProtocol(const kwsys_stl::string & URL, kwsys_stl::string & protocol, kwsys_stl::string & dataglom)
    {
        // match 0 entire url
        // match 1 protocol
...
// ----------------------------------------------------------------------
    bool SystemTools::ParseURL(const kwsys_stl::string & URL, kwsys_stl::string & protocol, kwsys_stl::string & username, kwsys_stl::string & password, kwsys_stl::string & hostname, kwsys_stl::string & dataport, kwsys_stl::string & database)
    {
    ...
    void SystemTools::ClassInitialize()
    {
        ...
        if (const char *pwd = getenv("PWD"))
        {
            ...
            if (const char *cwd = Getcwd(buf, 2048))
            {
                ...
                {
                    SystemTools::AddTranslationPath(cwd_changed.c_str(), pwd_changed.c_str());
                }
            }
        }
#endif
