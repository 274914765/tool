...
static int kwsysProcessCreate(kwsysProcess * cp, int index, kwsysProcessCreateInformation * si, PHANDLE readEnd);
...
static int kwsysProcessSetupPipeNative(PHANDLE handle, HANDLE p[2], int isWrite);
static void kwsysProcessCleanupHandle(PHANDLE h);
static void kwsysProcessCleanupHandleSafe(PHANDLE h, DWORD nStdHandle);
static void kwsysProcessCleanup(kwsysProcess * cp, int error);
static void kwsysProcessCleanErrorMessage(kwsysProcess * cp);
static int kwsysProcessComputeCommandLength(kwsysProcess * cp, char const *const *command);
static void kwsysProcessComputeCommandLine(kwsysProcess * cp, char const *const *command, char *cmd);
static int kwsysProcessGetTimeoutTime(kwsysProcess * cp, double *userTimeout, kwsysProcessTime * timeoutTime);
static int kwsysProcessGetTimeoutLeft(kwsysProcessTime * timeoutTime, double *userTimeout, kwsysProcessTime * timeoutLength);
...
kwsysProcess *kwsysProcess_New(void)
{
    ...
    for (i = 0; i < KWSYSPE_PIPE_COUNT; ++i)
    {
        ...
        /* Create the reading thread.  It will block immediately.  The
           ...
           small stack.  */
        if (!(cp->Pipe[i].Reader.Thread = CreateThread(0, 1024, kwsysProcessPipeThreadRead, &cp->Pipe[i], 0, &dummy)))
        {
            kwsysProcess_Delete(cp);
            return 0;
        }
        ...
        /* Create the waking thread.  It will block immediately.  The
           thread will not make deeply nested calls, so we need only a
           small stack.  */
        if (!(cp->Pipe[i].Waker.Thread = CreateThread(0, 1024, kwsysProcessPipeThreadWake, &cp->Pipe[i], 0, &dummy)))
        {
            kwsysProcess_Delete(cp);
            return 0;
        }
...
int kwsysProcess_AddCommand(kwsysProcess * cp, char const *const *command)
{
       ...
       argument.  */
    {
        ...
        kwsysProcessComputeCommandLine(cp, command, new_cmd);
...
void kwsysProcess_Execute(kwsysProcess * cp)
{
    ...
    if (cp->WorkingDirectory)
    {
        if (!GetCurrentDirectoryW(cp->RealWorkingDirectoryLength, cp->RealWorkingDirectory))
        {
    ...
    si.StartupInfo.wShowWindow = (unsigned short) (cp->HideWindow ? SW_HIDE : SW_SHOWDEFAULT);
    ...
    if (!CreatePipe(&cp->Pipe[KWSYSPE_PIPE_STDERR].Read, &cp->Pipe[KWSYSPE_PIPE_STDERR].Write, 0, 0))
    {
        kwsysProcessCleanup(cp, 1);
        return;
    }
    ...
    if (!DuplicateHandle(GetCurrentProcess(), cp->Pipe[KWSYSPE_PIPE_STDERR].Write, GetCurrentProcess(), &si.StartupInfo.hStdError, 0, TRUE, DUPLICATE_SAME_ACCESS))
    {
        kwsysProcessCleanup(cp, 1);
    ...
    {
        if (!kwsysProcessSetupOutputPipeFile(&si.StartupInfo.hStdError, cp->PipeFileSTDERR))
        {
            kwsysProcessCleanup(cp, 1);
            kwsysProcessCleanupHandle(&si.StartupInfo.hStdError);
            return;
    ...
    {
        if (!kwsysProcessSetupSharedPipe(STD_ERROR_HANDLE, &si.StartupInfo.hStdError))
        {
            kwsysProcessCleanup(cp, 1);
    ...
    {
        if (!kwsysProcessSetupPipeNative(&si.StartupInfo.hStdError, cp->PipeNativeSTDERR, 1))
        {
            kwsysProcessCleanup(cp, 1);
            kwsysProcessCleanupHandleSafe(&si.StartupInfo.hStdError, STD_ERROR_HANDLE);
            return;
    ...
    /* Create the pipeline of processes.  */
    {
        ...
        for (i = 0; i < cp->NumberOfCommands; ++i)
        {
            ...
            else
            {
                ...
                kwsysProcessCleanupHandleSafe(&si.StartupInfo.hStdInput, STD_INPUT_HANDLE);
                kwsysProcessCleanupHandleSafe(&si.StartupInfo.hStdOutput, STD_OUTPUT_HANDLE);
                kwsysProcessCleanupHandleSafe(&si.StartupInfo.hStdError, STD_ERROR_HANDLE);
                return;
            }
        }
...
void kwsysProcess_Disown(kwsysProcess * cp)
{
    ...
    if (!cp || !cp->Detached || cp->State != kwsysProcess_State_Executing || cp->TimeoutExpired || cp->Killed || cp->Terminated)
    {
        return;
    }
...
int kwsysProcess_WaitForData(kwsysProcess * cp, char **data, int *length, double *userTimeout)
{
    ...
    if (!cp || cp->State != kwsysProcess_State_Executing || cp->Killed || cp->TimeoutExpired)
    {
    ...
    while (!done && cp->PipesLeft > 0)
    {
        ...
        w = WaitForMultipleObjects(cp->ProcessEventsLength, cp->ProcessEvents, 0, timeout);
        if (w == WAIT_TIMEOUT)
        {
            /* Timeout has expired.  */
            expired = 1;
...
void kwsysProcess_Kill(kwsysProcess * cp)
{
    int i;
    /* Make sure we are executing a process.  */
    if (!cp || cp->State != kwsysProcess_State_Executing || cp->TimeoutExpired || cp->Killed)
    {
...
int kwsysProcessInitialize(kwsysProcess * cp)
{
    ...
    cp->ProcessInformation = (PROCESS_INFORMATION *) malloc(sizeof(PROCESS_INFORMATION) * cp->NumberOfCommands);
    if (!cp->ProcessInformation)
    {
        return 0;
    }
    ZeroMemory(cp->ProcessInformation, sizeof(PROCESS_INFORMATION) * cp->NumberOfCommands);
    if (cp->CommandExitCodes)
    {
        free(cp->CommandExitCodes);
    }
...
/*--------------------------------------------------------------------------*/
int kwsysProcessCreate(kwsysProcess * cp, int index, kwsysProcessCreateInformation * si, PHANDLE readEnd)
{
    ...
    if (*readEnd)
    {
        ...
        if (!DuplicateHandle(GetCurrentProcess(), *readEnd, GetCurrentProcess(), readEnd, 0, TRUE, (DUPLICATE_CLOSE_SOURCE | DUPLICATE_SAME_ACCESS)))
        {
            return 0;
        }
    ...
    else if (cp->PipeFileSTDIN)
    {
        ...
        if (!DuplicateHandle(GetCurrentProcess(), fin, GetCurrentProcess(), &fin, 0, TRUE, (DUPLICATE_CLOSE_SOURCE | DUPLICATE_SAME_ACCESS)))
        {
            return 0;
        }
    ...
    else if (cp->PipeSharedSTDIN)
    {
        ...
        if (!kwsysProcessSetupSharedPipe(STD_INPUT_HANDLE, &si->StartupInfo.hStdInput))
        {
            return 0;
        }
    }
    else if (cp->PipeNativeSTDIN[0])
    {
        ...
        if (!kwsysProcessSetupPipeNative(&si->StartupInfo.hStdInput, cp->PipeNativeSTDIN, 0))
        {
            return 0;
        }
    }
    ...
    /* Setup the process's stdout.  */
    {
        ...
        if (!DuplicateHandle(GetCurrentProcess(), writeEnd, GetCurrentProcess(), &writeEnd, 0, TRUE, (maybeClose | DUPLICATE_SAME_ACCESS)))
        {
            return 0;
        }
    ...
    {
        if (!kwsysProcessSetupSharedPipe(STD_OUTPUT_HANDLE, &si->StartupInfo.hStdOutput))
        {
            return 0;
        }
    }
    ...
    {
        if (!kwsysProcessSetupPipeNative(&si->StartupInfo.hStdOutput, cp->PipeNativeSTDOUT, 1))
        {
            return 0;
        }
    }
    ...
    if (!CreateProcessW(0, cp->Commands[index], 0, 0, TRUE, CREATE_SUSPENDED, 0, 0, &si->StartupInfo, &cp->ProcessInformation[index]))
    {
        return 0;
    }
...
void kwsysProcessDestroy(kwsysProcess * cp, int event)
{
    ...
    GetExitCodeProcess(cp->ProcessInformation[index].hProcess, &cp->CommandExitCodes[index]);
...
int kwsysProcessSetupOutputPipeFile(PHANDLE phandle, const char *name)
{
    ...
    fout = CreateFileW(wname, GENERIC_WRITE, FILE_SHARE_READ, 0, CREATE_ALWAYS, 0, 0);
    free(wname);
    ...
    /* Create an inherited duplicate of the handle.  This also closes
       the non-inherited version.  */
    if (!DuplicateHandle(GetCurrentProcess(), fout, GetCurrentProcess(), &fout, 0, TRUE, (DUPLICATE_CLOSE_SOURCE | DUPLICATE_SAME_ACCESS)))
    {
        return 0;
    }
...
int kwsysProcessSetupSharedPipe(DWORD nStdHandle, PHANDLE handle)
{
    ...
    DWORD flags;
    int inherited = 0;
    if (GetHandleInformation(GetStdHandle(nStdHandle), &flags) && (flags & HANDLE_FLAG_INHERIT))
    {
        inherited = 1;
    }
    ...
    else if (DuplicateHandle(GetCurrentProcess(), GetStdHandle(nStdHandle), GetCurrentProcess(), handle, 0, TRUE, DUPLICATE_SAME_ACCESS))
    {
        return 1;
    }
    else
    {
        ...
        if (!DuplicateHandle(GetCurrentProcess(), emptyPipe[child_end], GetCurrentProcess(), &emptyPipe[child_end], 0, TRUE, (DUPLICATE_CLOSE_SOURCE | DUPLICATE_SAME_ACCESS)))
        {
            return 0;
        }
...
int kwsysProcessSetupPipeNative(PHANDLE handle, HANDLE p[2], int isWrite)
{
    ...
    /* Create an inherited duplicate of the handle.  This also closes
       the non-inherited version.  */
    if (!DuplicateHandle(GetCurrentProcess(), p[isWrite ? 1 : 0], GetCurrentProcess(), handle, 0, TRUE, (DUPLICATE_CLOSE_SOURCE | DUPLICATE_SAME_ACCESS)))
    {
        return 0;
    }
...
void kwsysProcessCleanup(kwsysProcess * cp, int error)
{
    ...
    if (error)
    {
        ...
        if (cp->ErrorMessage[0] == 0)
        {
            ...
            DWORD length = FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, 0, original,
                                          MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),
                                          err_msg, KWSYSPE_PIPE_BUFFER_SIZE, 0);
            WideCharToMultiByte(CP_UTF8, 0, err_msg, -1, cp->ErrorMessage, KWSYSPE_PIPE_BUFFER_SIZE, NULL, NULL);
            if (length < 1)
            {
                /* FormatMessage failed.  Use a default message.  */
                _snprintf(cp->ErrorMessage, KWSYSPE_PIPE_BUFFER_SIZE, "Process execution failed with error 0x%X.  " "FormatMessage failed with error 0x%X", original, GetLastError());
            }
        }
...
/*--------------------------------------------------------------------------*/
int kwsysProcessComputeCommandLength(kwsysProcess * cp, char const *const *command)
{
    int length = 0;
    if (cp->Verbatim)
    {
...
/*--------------------------------------------------------------------------*/
void kwsysProcessComputeCommandLine(kwsysProcess * cp, char const *const *command, char *cmd)
{
    if (cp->Verbatim)
    {
...
int kwsysProcessGetTimeoutTime(kwsysProcess * cp, double *userTimeout, kwsysProcessTime * timeoutTime)
{
    ...
    *timeoutTime = cp->TimeoutTime;
    ...
    if (userTimeout)
    {
        ...
        kwsysProcessTime userTimeoutTime = kwsysProcessTimeAdd(currentTime,
                                                               userTimeoutLength);
        if (timeoutTime->QuadPart < 0 || kwsysProcessTimeLess(userTimeoutTime, *timeoutTime))
        {
...
int kwsysProcessGetTimeoutLeft(kwsysProcessTime * timeoutTime, double *userTimeout, kwsysProcessTime * timeoutLength)
{
    if (timeoutTime->QuadPart < 0)
    {
...
typedef NTSTATUS(WINAPI * ZwQuerySystemInformationType) (ULONG, PVOID, ULONG, PULONG);
...
static kwsysProcess_List *kwsysProcess_List_New(void)
{
    ...
    osv.dwOSVersionInfoSize = sizeof(osv);
    GetVersionEx(&osv);
    self->NT4 = (osv.dwPlatformId == VER_PLATFORM_WIN32_NT && osv.dwMajorVersion < 5) ? 1 : 0;

    /* Initialize the selected implementation.  */
    if (!(self->NT4 ? kwsysProcess_List__New_NT4(self) : kwsysProcess_List__New_Snapshot(self)))
    {
...
static int kwsysProcess_List_Update(kwsysProcess_List * self)
{
    return self ? (self->NT4 ? kwsysProcess_List__Update_NT4(self) : kwsysProcess_List__Update_Snapshot(self)) : 0;
}

/*--------------------------------------------------------------------------*/
static int kwsysProcess_List_GetCurrentProcessId(kwsysProcess_List * self)
{
    return self ? (self->NT4 ? kwsysProcess_List__GetProcessId_NT4(self) : kwsysProcess_List__GetProcessId_Snapshot(self)) : -1;
...
static int kwsysProcess_List_GetCurrentParentId(kwsysProcess_List * self)
{
    return self ? (self->NT4 ? kwsysProcess_List__GetParentId_NT4(self) : kwsysProcess_List__GetParentId_Snapshot(self)) : -1;
...
static int kwsysProcess_List_NextProcess(kwsysProcess_List * self)
{
    return (self ? (self->NT4 ? kwsysProcess_List__Next_NT4(self) : kwsysProcess_List__Next_Snapshot(self)) : 0);
}
...
static int kwsysProcess_List__New_NT4(kwsysProcess_List * self)
{
    ...
    if (hNT)
    {
        ...
        self->P_ZwQuerySystemInformation = ((ZwQuerySystemInformationType) GetProcAddress(hNT, "ZwQuerySystemInformation"));
    }
...
static int kwsysProcess_List__Update_NT4(kwsysProcess_List * self)
{
    ...
    for (;;)
    {
        ...
        NTSTATUS status = self->P_ZwQuerySystemInformation(5, self->Buffer, self->BufferSize, 0);
...
static int kwsysProcess_List__Next_NT4(kwsysProcess_List * self)
{
    if (self->CurrentInfo)
    {
        ...
        {
            self->CurrentInfo = ((PSYSTEM_PROCESS_INFORMATION) ((char *) self->CurrentInfo + self->CurrentInfo->NextEntryDelta));
            return 1;
        }
        self->CurrentInfo = 0;
    }
...
static int kwsysProcess_List__New_Snapshot(kwsysProcess_List * self)
{
    ...
    if (hKernel)
    {
        self->P_CreateToolhelp32Snapshot = ((CreateToolhelp32SnapshotType) GetProcAddress(hKernel, "CreateToolhelp32Snapshot"));
        self->P_Process32First = ((Process32FirstType) GetProcAddress(hKernel, "Process32First"));
        self->P_Process32Next = ((Process32NextType) GetProcAddress(hKernel, "Process32Next"));
    }
    return (self->P_CreateToolhelp32Snapshot && self->P_Process32First && self->P_Process32Next) ? 1 : 0;
}
...
static int kwsysProcess_List__Update_Snapshot(kwsysProcess_List * self)
{
    ...
    if (!(self->Snapshot = self->P_CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0)))
    {
        return 0;
    }
