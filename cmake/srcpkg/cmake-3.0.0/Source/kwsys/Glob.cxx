...
//----------------------------------------------------------------------------
    kwsys_stl::string Glob::PatternToRegex(const kwsys_stl::string & pattern, bool require_whole_string, bool preserve_case)
    {
        ...
        for (kwsys_stl::string::const_iterator i = pattern_first; i != pattern_last; ++i)
        {
            int c = *i;
            if (c == '*')
            {
            ...
            else if (c == '[')
            {
                ...
                if (bracket_last != pattern_last && (*bracket_last == '!' || *bracket_last == '^'))
                {
                    ++bracket_last;
                }
...
//----------------------------------------------------------------------------
    void Glob::RecurseDirectory(kwsys_stl::string::size_type start, const kwsys_stl::string & dir)
    {
        kwsys::Directory d;
        if (!d.Load(dir.c_str()))
        {
        ...
        for (cc = 0; cc < d.GetNumberOfFiles(); cc++)
        {
            ...
            {
                if ((this->Internals->Expressions.size() > 0) && this->Internals->Expressions[this->Internals->Expressions.size() - 1].find(fname.c_str()))
                {
...
//----------------------------------------------------------------------------
    void Glob::ProcessDirectory(kwsys_stl::string::size_type start, const kwsys_stl::string & dir)
    {
        ...
        for (cc = 0; cc < d.GetNumberOfFiles(); cc++)
        {
            ...
            if (!last && !kwsys::SystemTools::FileIsDirectory(realname.c_str()))
            {
                continue;
            }
...
//----------------------------------------------------------------------------
    bool Glob::FindFiles(const kwsys_stl::string & inexpr)
    {
        ...
        for (cc = 0; cc < expr.size(); cc++)
        {
            ...
            if (cc > 0 && (expr[cc] == '[' || expr[cc] == '?' || expr[cc] == '*') && expr[cc - 1] != '\\')
            {
                break;
            }
        }
...
//----------------------------------------------------------------------------
    ...
    {
        this->Internals->Expressions.push_back(kwsys::RegularExpression(this->PatternToRegex(expr).c_str()));
    }
