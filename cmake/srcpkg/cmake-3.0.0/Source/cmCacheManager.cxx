...
const char *cmCacheManagerTypes[] = { "BOOL",
    "PATH",
    "FILEPATH",
    "STRING",
    "INTERNAL",
...
bool cmCacheManager::LoadCache(const char *path, bool internal)
{
    std::set < cmStdString > emptySet;
    return this->LoadCache(path, internal, emptySet, emptySet);
}

static bool ParseEntryWithoutType(const char *entry, std::string & var, std::string & value)
{
    ...
    if (flag && value.size() >= 2 && value[0] == '\'' && value[value.size() - 1] == '\'')
    {
...
bool cmCacheManager::ParseEntry(const char *entry, std::string & var, std::string & value, CacheEntryType & type)
{
...
bool cmCacheManager::LoadCache(const char *path, bool internal, std::set < cmStdString > &excludes, std::set < cmStdString > &includes)
{
    ...
    while (fin)
    {
        ...
        while (*realbuffer != '0' && (*realbuffer == ' ' || *realbuffer == '\t' || *realbuffer == '\r' || *realbuffer == '\n'))
        {
            realbuffer++;
        }
        ...
        if (cmCacheManager::ParseEntry(realbuffer, entryKey, e.Value, e.Type))
        {
            if (excludes.find(entryKey) == excludes.end())
            {
                ...
                // imported, load it.
                if (internal || (e.Type != INTERNAL) || (includes.find(entryKey) != includes.end()))
                {
                    ...
                    if (!internal)
                    {
                        ...
                        helpString += entryKey;
                        helpString += " loaded from external file.  " "To change this value edit this file: ";
                        helpString += path;
        ...
        {
            cmSystemTools::Error("Parse error in cache file ", cacheFile.c_str(), ". Offending entry: ", realbuffer);
        }
    }
    this->CacheMajorVersion = 0;
    this->CacheMinorVersion = 0;
    ...
    else
    {
        ...
        // Set as version 0.0
        this->AddCacheEntry("CMAKE_CACHE_MINOR_VERSION", "0", "Minor version of cmake used to create the " "current loaded cache", cmCacheManager::INTERNAL);
        this->AddCacheEntry("CMAKE_CACHE_MAJOR_VERSION", "0", "Major version of cmake used to create the " "current loaded cache", cmCacheManager::INTERNAL);
    ...
    if (internal && this->GetCacheValue("CMAKE_CACHEFILE_DIR"))
    {
        ...
        if (!cmSystemTools::SameFile(oldcwd.c_str(), currentcwd.c_str()))
        {
            std::string message =
                ...
                std::string(this->GetCacheValue("CMAKE_CACHEFILE_DIR")) + std::string(" where CMakeCache.txt was created. This may result " "in binaries being created in the wrong place. If you " "are not sure, reedit the CMakeCache.txt");
...
//----------------------------------------------------------------------------
bool cmCacheManager::ReadPropertyEntry(std::string const &entryKey, CacheEntry & e)
{
...
//----------------------------------------------------------------------------
void cmCacheManager::WritePropertyEntries(std::ostream & os, CacheIterator const &i)
{
    for (const char **p = this->PersistentProperties; *p; ++p)
    {
...
bool cmCacheManager::SaveCache(const char *path)
{
    ...
    if (!fout)
    {
        cmSystemTools::Error("Unable to open cache file for save. ", cacheFile.c_str());
    ...
    this->AddCacheEntry("CMAKE_CACHE_MINOR_VERSION", temp, "Minor version of cmake used to create the " "current loaded cache", cmCacheManager::INTERNAL);
    sprintf(temp, "%d", cmVersion::GetMajorVersion());
    this->AddCacheEntry("CMAKE_CACHE_MAJOR_VERSION", temp, "Major version of cmake used to create the " "current loaded cache", cmCacheManager::INTERNAL);
    sprintf(temp, "%d", cmVersion::GetPatchVersion());
    this->AddCacheEntry("CMAKE_CACHE_PATCH_VERSION", temp, "Patch version of cmake used to create the " "current loaded cache", cmCacheManager::INTERNAL);
    ...
    cmSystemTools::ConvertToUnixSlashes(currentcwd);
    this->AddCacheEntry("CMAKE_CACHEFILE_DIR", currentcwd.c_str(), "This is the directory where this CMakeCache.txt" " was created", cmCacheManager::INTERNAL);

    fout << "# This is the CMakeCache file.\n" << "# For build in directory: " << currentcwd << "\n";
    cmCacheManager::CacheEntry * cmakeCacheEntry = this->GetCacheEntry("CMAKE_COMMAND");
    if (cmakeCacheEntry)
    {
        fout << "# It was generated by CMake: " << cmakeCacheEntry->Value << std::endl;
    }
    ...
    for (std::map < cmStdString, CacheEntry >::const_iterator i = this->Cache.begin(); i != this->Cache.end(); ++i)
    {
    ...
    for (cmCacheManager::CacheIterator i = this->NewIterator(); !i.IsAtEnd(); i.Next())
    {
        if (!i.Initialized())
        {
            continue;
    ...
    if (!checkCache)
    {
        cmSystemTools::Error("Unable to open check cache file for write. ", checkCacheFile.c_str());
        return false;
    }
...
void cmCacheManager::OutputKey(std::ostream & fout, std::string const &key)
{
    ...
    const char *q = (key.find(':') != key.npos || key.find("//") == 0) ? "\"" : "";
    fout << q << key << q;
}
...
void cmCacheManager::OutputHelpString(std::ostream & fout, const std::string & helpString)
{
...
void cmCacheManager::PrintCache(std::ostream & out) const
{
    ...
    for (std::map < cmStdString, CacheEntry >::const_iterator i = this->Cache.begin(); i != this->Cache.end(); ++i)
    {
        ...
        {
            out << (*i).first.c_str() << " = " << (*i).second.Value.c_str() << std::endl;
        }
    }
    out << "\n\n";
...
void cmCacheManager::AddCacheEntry(const char *key, const char *value, const char *helpString, CacheEntryType type)
{
    ...
    e.SetProperty("HELPSTRING", helpString ? helpString : "(This variable does not exist and should not be used)");
...
//----------------------------------------------------------------------------
const char *cmCacheManager::CacheEntry::GetProperty(const char *prop) const
{
    ...
    bool c = false;
    return this->Properties.GetPropertyValue(prop, cmProperty::CACHE, c);
}

//----------------------------------------------------------------------------
void cmCacheManager::CacheEntry::SetProperty(const char *prop, const char *value)
{
    if (strcmp(prop, "TYPE") == 0)
    {
...
//----------------------------------------------------------------------------
void cmCacheManager::CacheEntry::AppendProperty(const char *prop, const char *value, bool asString)
{
    if (strcmp(prop, "TYPE") == 0)
    {
        this->Type = cmCacheManager::StringToType(value ? value : "STRING");
...
//----------------------------------------------------------------------------
void cmCacheManager::CacheIterator::AppendProperty(const char *p, const char *v, bool asString)
{
    if (!this->IsAtEnd())
    {
...
bool cmCacheManager::NeedCacheCompatibility(int major, int minor)
{
    ...
    cmIML_INT_uint64_t actual_compat = CMake_VERSION_ENCODE(this->CacheMajorVersion, this->CacheMinorVersion, 0);
    return (actual_compat && actual_compat <= CMake_VERSION_ENCODE(major, minor, 0));
}
