...
// cmLibraryCommand
bool cmFileCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 2)
    {
...
//----------------------------------------------------------------------------
bool cmFileCommand::HandleWriteCommand(std::vector < std::string > const &args, bool append)
{
    std::string message;
...
bool cmFileCommand::HandleReadCommand(std::vector < std::string > const &args)
{
    if (args.size() < 3)
    {
        this->SetError("READ must be called with at least two additional " "arguments");
        return false;
    }
...
#if defined(_WIN32) || defined(__CYGWIN__)
    cmsys::ifstream file(fileName.c_str(), std::ios::in | (hexOutputArg.IsEnabled()? std::ios::binary : std::ios::in));
#else
    ...
    else
    {
        ...
        while (sizeLimit != 0 && cmSystemTools::GetLineFromStream(file, line, &has_newline, sizeLimit))
        {
            if (sizeLimit > 0)
            {
...
bool cmFileCommand::HandleStringsCommand(std::vector < std::string > const &args)
{
    ...
    for (unsigned int i = 3; i < args.size(); ++i)
    {
        ...
        {
            if (sscanf(args[i].c_str(), "%d", &limit_input) != 1 || limit_input < 0)
            {
                cmOStringStream e;
                e << "STRINGS option LIMIT_INPUT value \"" << args[i] << "\" is not an unsigned integer.";
                this->SetError(e.str().c_str());
                return false;
            }
            arg_mode = arg_none;
        ...
        {
            if (sscanf(args[i].c_str(), "%d", &limit_output) != 1 || limit_output < 0)
            {
                cmOStringStream e;
                e << "STRINGS option LIMIT_OUTPUT value \"" << args[i] << "\" is not an unsigned integer.";
                this->SetError(e.str().c_str());
                return false;
            }
            arg_mode = arg_none;
        ...
        else if (arg_mode == arg_limit_count)
        {
            ...
            {
                cmOStringStream e;
                e << "STRINGS option LIMIT_COUNT value \"" << args[i] << "\" is not an unsigned integer.";
                this->SetError(e.str().c_str());
                return false;
            }
            limit_count = count;
        ...
        else if (arg_mode == arg_length_minimum)
        {
            ...
            {
                cmOStringStream e;
                e << "STRINGS option LENGTH_MINIMUM value \"" << args[i] << "\" is not an unsigned integer.";
                this->SetError(e.str().c_str());
                return false;
            }
            minlen = len;
        ...
        else if (arg_mode == arg_length_maximum)
        {
            ...
            {
                cmOStringStream e;
                e << "STRINGS option LENGTH_MAXIMUM value \"" << args[i] << "\" is not an unsigned integer.";
                this->SetError(e.str().c_str());
                return false;
            }
            maxlen = len;
    ...
    std::string s;
    int c;
    while ((!limit_count || strings.size() < limit_count) && (limit_input < 0 || static_cast < int >(fin.tellg()) < limit_input) &&(c = fin.get(), fin))
    {
        if (c == '\n' && !newline_consume)
        {
        ...
        else
        {
            ...
            if (s.length() >= minlen && s.length() >= 1 && (!have_regex || regex.find(s.c_str())))
            {
                output_size += static_cast < int >(s.size()) + 1;
                if (limit_output >= 0 && output_size >= limit_output)
                {
    ...
    if ((!limit_count || strings.size() < limit_count) && !s.empty() && s.length() >= minlen && (!have_regex || regex.find(s.c_str())))
    {
        output_size += static_cast < int >(s.size()) + 1;
        if (limit_output < 0 || output_size < limit_output)
        {
...
//----------------------------------------------------------------------------
bool cmFileCommand::HandleGlobCommand(std::vector < std::string > const &args, bool recurse)
{
    ...
    cmPolicies::PolicyStatus status = this->Makefile->GetPolicyStatus(cmPolicies::CMP0009);
    if (recurse)
    {
        switch (status)
        {
    ...
    for (; i != args.end(); ++i)
    {
        if (recurse && (*i == "FOLLOW_SYMLINKS"))
        {
            ...
            if (i == args.end())
            {
                this->SetError("GLOB_RECURSE requires a glob expression after FOLLOW_SYMLINKS");
                return false;
            }
        }
    ...
    if (recurse && !explicitFollowSymlinks)
    {
        switch (status)
        {
            ...
            case cmPolicies::WARN:
                ...
                {
                    this->Makefile->IssueMessage(cmake::AUTHOR_WARNING, this->Makefile->GetPolicies()->GetPolicyWarning(cmPolicies::CMP0009));
                }
                break;
            case cmPolicies::REQUIRED_IF_USED:
            case cmPolicies::REQUIRED_ALWAYS:
                this->SetError("policy CMP0009 error");
                this->Makefile->IssueMessage(cmake::FATAL_ERROR, this->Makefile->GetPolicies()->GetRequiredPolicyError(cmPolicies::CMP0009));
                return false;
        }
    }
...
bool cmFileCommand::HandleMakeDirectoryCommand(std::vector < std::string > const &args)
{
    ...
    for (; i != args.end(); ++i)
    {
        ...
        {
            std::string e = "attempted to create a directory: " + *cdir + " into a source directory.";
            this->SetError(e.c_str());
            cmSystemTools::SetFatalErrorOccured();
            return false;
        }
...
//----------------------------------------------------------------------------
bool cmFileCommand::HandleDifferentCommand(std::vector < std::string > const &args)
{
    /*
    ...
    const char *var = 0;
    enum Doing
    { DoingNone, DoingVar, DoingFileLHS, DoingFileRHS };
    ...
    // Compare the files.
    const char *result = cmSystemTools::FilesDiffer(file_lhs, file_rhs) ? "1" : "0";
...
cmFileCopier(cmFileCommand * command, const char *name = "COPY"):
    FileCommand(command),
        ...
        Name(name), Always(false), MatchlessFiles(true), FilePermissions(0), DirPermissions(0), CurrentMatchRule(0), UseGivenPermissionsFile(false), UseGivenPermissionsDir(false), UseSourcePermissions(true), Doing(DoingNone)
    {
    }
...
protected:
    ...
    struct MatchRule
    {
        ...
        MatchRule(std::string const &regex):Regex(regex.c_str()), RegexString(regex)
        {
        }
    };
    ...
    MatchProperties CollectMatchProperties(const char *file)
    {
        ...
        for (std::vector < MatchRule >::iterator mr = this->MatchRules.begin(); mr != this->MatchRules.end(); ++mr)
        {
    ...
    bool InstallFile(const char *fromFile, const char *toFile, MatchProperties const &match_properties);
    bool InstallDirectory(const char *source, const char *destination, MatchProperties const &match_properties);
...
bool cmFileCopier::CheckValue(std::string const &arg)
{
    switch (this->Doing)
    {
        ...
        case DoingPermissionsMatch:
            if (!this->CheckPermissions(arg, this->CurrentMatchRule->Properties.Permissions))
            {
                this->Doing = DoingError;
            }
            break;
...
bool cmFileCopier::Run(std::vector < std::string > const &args)
{
    ...
    for (std::vector < std::string >::size_type i = 0; i < files.size(); ++i)
    {
        ...
        std::string fromDir = cmSystemTools::JoinPath(fromPathComponents.begin(), fromPathComponents.end() - 1);
...
bool cmFileCopier::InstallSymlink(const char *fromFile, const char *toFile)
{
    ...
    {
        cmOStringStream e;
        e << this->Name << " cannot read symlink \"" << fromFile << "\" to duplicate at \"" << toFile << "\".";
        this->FileCommand->SetError(e.str().c_str());
        return false;
    }
    ...
    if (copy)
    {
        ...
        {
            cmOStringStream e;
            e << this->Name << " cannot duplicate symlink \"" << fromFile << "\" at \"" << toFile << "\".";
            this->FileCommand->SetError(e.str().c_str());
            return false;
        }
    }
...
//----------------------------------------------------------------------------
bool cmFileCopier::InstallFile(const char *fromFile, const char *toFile, MatchProperties const &match_properties)
{
    ...
    {
        cmOStringStream e;
        e << this->Name << " cannot copy file \"" << fromFile << "\" to \"" << toFile << "\".";
        this->FileCommand->SetError(e.str().c_str());
        return false;
    }
...
//----------------------------------------------------------------------------
bool cmFileCopier::InstallDirectory(const char *source, const char *destination, MatchProperties const &match_properties)
{
    ...
    {
        cmOStringStream e;
        e << this->Name << " cannot make directory \"" << destination << "\": " << cmSystemTools::GetLastSystemError();
        this->FileCommand->SetError(e.str().c_str());
        return false;
    }

    // Compute the requested permissions for the destination directory.
    mode_t permissions = (match_properties.Permissions ? match_properties.Permissions : this->DirPermissions);
    if (!permissions)
    {
        // No permissions were explicitly provided but the user requested
    ...
    mode_t required_permissions = mode_owner_read | mode_owner_write | mode_owner_execute;
    ...
    {
        if (!(strcmp(dir.GetFile(fileNum), ".") == 0 || strcmp(dir.GetFile(fileNum), "..") == 0))
        {
...
{
    cmFileInstaller(cmFileCommand * command):cmFileCopier(command, "INSTALL"), InstallType(cmInstallType_FILES), Optional(false), DestDirLength(0)
    {
        // Installation does not use source permissions by default.
        this->UseSourcePermissions = false;
        // Check whether to copy files always or only if they have changed.
        this->Always = cmSystemTools::IsOn(cmSystemTools::GetEnv("CMAKE_INSTALL_ALWAYS"));
        // Get the current manifest.
        this->Manifest = this->Makefile->GetSafeDefinition("CMAKE_INSTALL_MANIFEST_FILES");
    }
    ~cmFileInstaller()
    {
        // Save the updated install manifest.
        this->Makefile->AddDefinition("CMAKE_INSTALL_MANIFEST_FILES", this->Manifest.c_str());
    }
...
bool cmFileInstaller::Parse(std::vector < std::string > const &args)
{
    ...
    {
        if (this->InstallType != cmInstallType_FILES && this->InstallType != cmInstallType_PROGRAMS)
        {
            this->FileCommand->SetError("INSTALL option RENAME may be used " "only with FILES or PROGRAMS.");
            return false;
        }
        if (this->Files.size() > 1)
        {
            this->FileCommand->SetError("INSTALL option RENAME may be used " "only with one file.");
            return false;
        }
    }
...
bool cmFileInstaller::CheckKeyword(std::string const &arg)
{
    ...
    else if (arg == "COMPONENTS" || arg == "CONFIGURATIONS" || arg == "PROPERTIES")
    {
        cmOStringStream e;
        e << "INSTALL called with old-style " << arg << " argument.  " << "This script was generated with an older version of CMake.  " << "Re-run this cmake version on your build tree.";
        this->FileCommand->SetError(e.str().c_str());
        this->Doing = DoingError;
    }
    else
...
bool cmFileInstaller::HandleInstallDestination()
{
    ...
    {
        this->FileCommand->SetError("called with inappropriate arguments. " "No DESTINATION provided or .");
        return false;
    }
    ...
    if (destdir && *destdir)
    {
        ...
        {
            int relative = 0;
            if (((ch1 >= 'a' && ch1 <= 'z') || (ch1 >= 'A' && ch1 <= 'Z')) && ch2 == ':')
            {
                // Assume windows
            ...
            if (relative)
            {
                ...
                this->FileCommand->SetError("called with relative DESTINATION. This " "does not make sense when using DESTDIR. Specify " "absolute path or remove DESTDIR environment variable.");
                return false;
            }
        }
        else
        {
            if (ch2 == '/')
            {
                ...
                std::string message = "called with network path DESTINATION. This " "does not make sense when using DESTDIR. Specify local " "absolute path or remove DESTDIR environment variable." "\nDESTINATION=\n";
                message += destination;
    ...
    if (!cmSystemTools::FileExists(destination.c_str()))
    {
        ...
        {
            std::string errstring = "cannot create directory: " + destination + ". Maybe need administrative privileges.";
    ...
    {
        std::string errstring = "INSTALL destination: " + destination + " is not a directory.";
        this->FileCommand->SetError(errstring.c_str());
        return false;
    }
    return true;
...
//----------------------------------------------------------------------------
bool cmFileCommand::HandleRPathChangeCommand(std::vector < std::string > const &args)
{
    // Evaluate arguments.
    const char *file = 0;
    ...
    enum Doing
    { DoingNone, DoingFile, DoingOld, DoingNew };
...
//----------------------------------------------------------------------------
bool cmFileCommand::HandleRPathRemoveCommand(std::vector < std::string > const &args)
{
    // Evaluate arguments.
    const char *file = 0;
    enum Doing
...
//----------------------------------------------------------------------------
bool cmFileCommand::HandleRPathCheckCommand(std::vector < std::string > const &args)
{
    // Evaluate arguments.
    const char *file = 0;
    const char *rpath = 0;
    enum Doing
    { DoingNone, DoingFile, DoingRPath };
    Doing doing = DoingNone;
    for (unsigned int i = 1; i < args.size(); ++i)
    {
        if (args[i] == "RPATH")
    ...
    if (cmSystemTools::FileExists(file, true) && !cmSystemTools::CheckRPath(file, rpath))
    {
...
//----------------------------------------------------------------------------
bool cmFileCommand::HandleRelativePathCommand(std::vector < std::string > const &args)
{
    if (args.size() != 4)
    {
    ...
    {
        std::string errstring = "RELATIVE_PATH must be passed a full path to the directory: " + directoryName;
        this->SetError(errstring.c_str());
        return false;
    }
    if (!cmSystemTools::FileIsFullPath(fileName.c_str()))
    {
        std::string errstring = "RELATIVE_PATH must be passed a full path to the file: " + fileName;
        this->SetError(errstring.c_str());
        return false;
    }

    std::string res = cmSystemTools::RelativePath(directoryName.c_str(), fileName.c_str());
    this->Makefile->AddDefinition(outVar.c_str(), res.c_str());
    return true;
}
...
//----------------------------------------------------------------------------
bool cmFileCommand::HandleRemove(std::vector < std::string > const &args, bool recurse)
{
    ...
    for (; i != args.end(); ++i)
    {
        ...
        if (cmSystemTools::FileIsDirectory(fileName.c_str()) && !cmSystemTools::FileIsSymlink(fileName.c_str()) && recurse)
        {
...
//----------------------------------------------------------------------------
bool cmFileCommand::HandleCMakePathCommand(std::vector < std::string > const &args, bool nativePath)
{
    std::vector < std::string >::const_iterator i = args.begin();
    if (args.size() != 3)
    {
        this->SetError("FILE([TO_CMAKE_PATH|TO_NATIVE_PATH] path result) must be " "called with exactly three arguments.");
        return false;
    }
    i++;                        // Get rid of subcommand
#if defined(_WIN32) && !defined(__CYGWIN__)
    ...
    std::string value;
    for (std::vector < cmsys::String >::iterator j = path.begin(); j != path.end(); ++j)
    {
        if (j != path.begin())
        {
            value += ";";
...
namespace
{
    ...
    size_t cmWriteToMemoryCallback(void *ptr, size_t size, size_t nmemb, void *data)
    {
        int realsize = (int) (size * nmemb);
        cmFileCommandVectorOfChar *vec = static_cast < cmFileCommandVectorOfChar * >(data);
        const char *chPtr = static_cast < char *>(ptr);
        vec->insert(vec->end(), chPtr, chPtr + realsize);
        return realsize;
    }

    static size_t cmFileCommandCurlDebugCallback(CURL *, curl_infotype, char *chPtr, size_t size, void *data)
    {
        cmFileCommandVectorOfChar *vec = static_cast < cmFileCommandVectorOfChar * >(data);
...
public:
        ...
        bool UpdatePercentage(double value, double total, std::string & status)
        {
            ...
            {
                cmOStringStream oss;
                oss << "[" << this->Text << " " << this->CurrentPercentage << "% complete]";
                status = oss.str();
            }
...
private:
    ...
    static int cmFileDownloadProgressCallback(void *clientp, double dltotal, double dlnow, double ultotal, double ulnow)
    {
    ...
    static int cmFileUploadProgressCallback(void *clientp, double dltotal, double dlnow, double ultotal, double ulnow)
    {
        cURLProgressHelper *helper = reinterpret_cast < cURLProgressHelper * >(clientp);
...
namespace
{
    ...
    {
public:
        cURLEasyGuard(CURL * easy):Easy(easy)
        {
        }
...
bool cmFileCommand::HandleDownloadCommand(std::vector < std::string > const &args)
{
    ...
    while (i != args.end())
    {
        ...
        else if (*i == "EXPECTED_HASH")
        {
            ...
            {
                std::string err = "DOWNLOAD EXPECTED_HASH expects ALGO=value but got: ";
                err += *i;
    ...
    if (cmSystemTools::FileExists(file.c_str()) && hash.get())
    {
        ...
        if (actualHash == expectedHash)
        {
            ...
            if (statusVar.size())
            {
                ...
                result << (int) 0 << ";\"" << msg;
                this->Makefile->AddDefinition(statusVar.c_str(), result.str().c_str());
            }
            return true;
        }
    }
    ...
    if (!cmSystemTools::FileExists(dir.c_str()) && !cmSystemTools::MakeDirectory(dir.c_str()))
    {
        std::string errstring = "DOWNLOAD error: cannot create directory '" + dir + "' - Specify file by full path name and verify that you " "have directory creation and file write privileges.";
        this->SetError(errstring.c_str());
        return false;
    }
    ...
    res =::curl_easy_setopt(curl, CURLOPT_DEBUGFUNCTION, cmFileCommandCurlDebugCallback);
    ...
    if (showProgress)
    {
        ...
        res =::curl_easy_setopt(curl, CURLOPT_PROGRESSFUNCTION, cmFileDownloadProgressCallback);
        check_curl_result(res, "DOWNLOAD cannot set progress function: ");

        res =::curl_easy_setopt(curl, CURLOPT_PROGRESSDATA, reinterpret_cast < void *>(&helper));
    ...
    if (hash.get())
    {
        ...
        if (expectedHash != actualHash)
        {
            ...
            oss << "DOWNLOAD HASH mismatch" << std::endl
                ...
                << "    expected hash: [" << expectedHash << "]" << std::endl << "      actual hash: [" << actualHash << "]" << std::endl << "           status: [" << (int) res << ";\"" <<::curl_easy_strerror(res) << "\"]" << std::endl;
    ...
    if (chunkDebug.size())
    {
        ...
        if (CURLE_OPERATION_TIMEOUTED == res)
        {
            ...
            if (verboseLog.size())
            {
                this->Makefile->AddDefinition(verboseLog.c_str(), &*chunkDebug.begin());
            }
        }
...
bool cmFileCommand::HandleUploadCommand(std::vector < std::string > const &args)
{
    ...
    res =::curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, cmWriteToMemoryCallback);
    ...
    if (showProgress)
    {
        ...
        res =::curl_easy_setopt(curl, CURLOPT_PROGRESSFUNCTION, cmFileUploadProgressCallback);
...
//----------------------------------------------------------------------------
void cmFileCommand::AddEvaluationFile(const std::string & inputName, const std::string & outputExpr, const std::string & condition, bool inputIsContent)
{
    cmListFileBacktrace lfbt;
    this->Makefile->GetBacktrace(lfbt);

    cmGeneratorExpression outputGe(lfbt);
    cmsys::auto_ptr < cmCompiledGeneratorExpression > outputCge = outputGe.Parse(outputExpr);

    cmGeneratorExpression conditionGe(lfbt);
    cmsys::auto_ptr < cmCompiledGeneratorExpression > conditionCge = conditionGe.Parse(condition);

    this->Makefile->GetLocalGenerator()->GetGlobalGenerator()->AddEvaluationFile(inputName, outputCge, this->Makefile, conditionCge, inputIsContent);
}
...
bool cmFileCommand::HandleGenerateCommand(std::vector < std::string > const &args)
{
    ...
    if (args.size() > 5)
    {
        ...
        if (condition.empty())
        {
            this->SetError("CONDITION of sub-command GENERATE must not be empty if " "specified.");
            return false;
        }
    }
...
//----------------------------------------------------------------------------
bool cmFileCommand::HandleTimestampCommand(std::vector < std::string > const &args)
{
    if (args.size() < 3)
    {
    ...
    if (args.size() > argsIndex)
    {
        ...
        {
            std::string e = " TIMESTAMP sub-command does not recognize option " + args[argsIndex] + ".";
            this->SetError(e.c_str());
            return false;
        }
    }
    ...
    std::string result = timestamp.FileModificationTime(filename.c_str(), formatString, utcFlag);
