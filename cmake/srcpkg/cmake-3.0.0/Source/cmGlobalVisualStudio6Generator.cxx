...
void cmGlobalVisualStudio6Generator::EnableLanguage(std::vector < std::string > const &lang, cmMakefile * mf, bool optional)
{
...
void cmGlobalVisualStudio6Generator::GenerateConfigurations(cmMakefile * mf)
{
    ...
    {
        cmSystemTools::Error("Cannot open ", fname.c_str(), ".  Please copy this file from the main " "CMake/Templates directory and edit it for " "your build configurations.");
    }
    else if (!mf->GetDefinition("CMAKE_CONFIGURATION_TYPES"))
    {
        cmSystemTools::Error("CMAKE_CONFIGURATION_TYPES not set by ", fname.c_str(), ".  Please copy this file from the main " "CMake/Templates directory and edit it for " "your build configurations.");
    }
}
...
void cmGlobalVisualStudio6Generator::FindMakeProgram(cmMakefile * mf)
{
    ...
    mf->AddDefinition("CMAKE_VS_MSDEV_COMMAND", this->GetMSDevCommand().c_str());
}
...
//----------------------------------------------------------------------------
void cmGlobalVisualStudio6Generator::GenerateBuildCommand(std::vector < std::string > &makeCommand, const char *makeProgram, const char *projectName, const char * /*projectDir */ ,
                                                          const char *targetName, const char *config, bool /*fast */ ,
                                                          std::vector < std::string > const &makeOptions)
{
    // now build the test
    makeCommand.push_back(this->SelectMakeProgram(makeProgram, this->GetMSDevCommand()));
...
void cmGlobalVisualStudio6Generator::WriteDSWFile(std::ostream & fout, cmLocalGenerator * root, std::vector < cmLocalGenerator * >&generators)
{
    ...
    for (OrderedTargetDependSet::const_iterator tt = orderedProjectTargets.begin(); tt != orderedProjectTargets.end(); ++tt)
    {
        ...
        if (expath)
        {
            ...
            this->WriteExternalProject(fout, project.c_str(), location.c_str(), target->GetUtilities());
        }
        else
        {
...
void cmGlobalVisualStudio6Generator::OutputDSWFile(cmLocalGenerator * root, std::vector < cmLocalGenerator * >&generators)
{
    ...
    if (!fout)
    {
        cmSystemTools::Error("Error can not open DSW file for write: ", fname.c_str());
...
void cmGlobalVisualStudio6Generator::WriteProject(std::ostream & fout, const char *dspname, const char *dir, cmTarget const &target)
{
    fout << "#########################################################" "######################\n\n";
    ...
    if (ui != this->UtilityDepends.end())
    {
        ...
        fout << "Project: \"" << uname << "\"=" << dir << "\\" << uname << ".dsp - Package Owner=<4>\n\n";
...
// Note, that dependencies from executables to
// the libraries it uses are also done here
void cmGlobalVisualStudio6Generator::WriteExternalProject(std::ostream & fout, const char *name, const char *location, const std::set < cmStdString > &dependencies)
{
    fout << "#########################################################" "######################\n\n";
...
//----------------------------------------------------------------------------
std::string cmGlobalVisualStudio6Generator::WriteUtilityDepend(cmTarget const *target)
{
...
//----------------------------------------------------------------------------
void cmGlobalVisualStudio6Generator::GetDocumentation(cmDocumentationEntry & entry)
{
...
//----------------------------------------------------------------------------
void cmGlobalVisualStudio6Generator::AppendDirectoryForConfig(const char *prefix, const char *config, const char *suffix, std::string & dir)
{
    if (config)
    {
        dir += prefix;
