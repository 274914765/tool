...
//----------------------------------------------------------------------------
bool cmIDEOptions::CheckFlagTable(cmIDEFlagTable const *table, const char *flag, bool & flag_handled)
{
    ...
    for (cmIDEFlagTable const *entry = table; entry->IDEName; ++entry)
    {
        ...
        if (entry->special & cmIDEFlagTable::UserValue)
        {
            ...
            if (strncmp(flag + 1, entry->commandFlag, n) == 0 && (!(entry->special & cmIDEFlagTable::UserRequired) || static_cast < int >(strlen(flag + 1)) > n))
            {
