...
// cmExecutableCommand
bool cmInstallFilesCommand::InitialPass(std::vector < std::string > const &argsIn, cmExecutionStatus &)
{
    if (argsIn.size() < 2)
    {
    ...
    if ((args.size() > 1) && (args[1] == "FILES"))
    {
        ...
        for (std::vector < std::string >::const_iterator s = args.begin() + 2; s != args.end(); ++s)
        {
    ...
    this->Makefile->GetLocalGenerator()->GetGlobalGenerator()->AddInstallComponent(this->Makefile->GetSafeDefinition("CMAKE_INSTALL_DEFAULT_COMPONENT_NAME"));
...
void cmInstallFilesCommand::FinalPass()
{
    ...
    if (this->FinalArgs.size() > 1)
    {
        ...
        for (; s != this->FinalArgs.end(); ++s)
        {
            ...
            {
                testf = cmSystemTools::GetFilenamePath(temps) + "/" + cmSystemTools::GetFilenameWithoutLastExtension(temps) + ext;
            }
            else
            {
    ...
    else                        // reg exp list
    {
        ...
        cmSystemTools::Glob(this->Makefile->GetCurrentDirectory(), regex.c_str(), files);
...
void cmInstallFilesCommand::CreateInstallGenerator() const
{
    ...
    this->Makefile->AddInstallGenerator(new cmInstallFilesGenerator(this->Makefile, this->Files, destination.c_str(), false, no_permissions, no_configurations, no_component.c_str(), no_rename));
}
...
{
    if (cmSystemTools::FileIsFullPath(name) || cmGeneratorExpression::Find(name) == 0)
    {
        // This is a full path.
        return name;
    }
