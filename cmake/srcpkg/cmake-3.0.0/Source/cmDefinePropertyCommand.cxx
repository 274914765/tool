...
bool cmDefinePropertyCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 1)
    {
    ...
    {
        cmOStringStream e;
        e << "given invalid scope " << args[0] << ".  " << "Valid scopes are " << "GLOBAL, DIRECTORY, TARGET, SOURCE, " << "TEST, VARIABLE, CACHED_VARIABLE.";
    ...
    enum Doing
    { DoingNone, DoingProperty, DoingBrief, DoingFull };
    ...
    this->Makefile->GetCMakeInstance()->DefineProperty(this->PropertyName.c_str(), scope, this->BriefDocs.c_str(), this->FullDocs.c_str(), inherited);
