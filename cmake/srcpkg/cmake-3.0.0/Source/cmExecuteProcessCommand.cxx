...
void cmExecuteProcessCommandFixText(std::vector < char >&output, bool strip_trailing_whitespace);
void cmExecuteProcessCommandAppend(std::vector < char >&output, const char *data, int length);

// cmExecuteProcessCommand
bool cmExecuteProcessCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 1)
    {
    ...
    {
        std::string e = "attempted to output into a file: " + output_file + " into a source directory.";
    ...
    while ((p = cmsysProcess_WaitForData(cp, &data, &length, 0), p))
    {
        ...
        if ((p == cmsysProcess_Pipe_STDOUT && !output_quiet) || (p == cmsysProcess_Pipe_STDERR && !error_quiet && merge_output))
        {
    ...
    if (!result_variable.empty())
    {
        switch (cmsysProcess_GetState(cp))
        {
            ...
            case cmsysProcess_State_Exception:
                this->Makefile->AddDefinition(result_variable.c_str(), cmsysProcess_GetExceptionString(cp));
                break;
            case cmsysProcess_State_Error:
                this->Makefile->AddDefinition(result_variable.c_str(), cmsysProcess_GetErrorString(cp));
                break;
            case cmsysProcess_State_Expired:
                this->Makefile->AddDefinition(result_variable.c_str(), "Process terminated due to timeout");
                break;
        }
    }
...
void cmExecuteProcessCommandFixText(std::vector < char >&output, bool strip_trailing_whitespace)
{
    ...
    {
        while (out_index > 0 && cmExecuteProcessCommandIsWhitespace(output[out_index - 1]))
        {
            --out_index;
        }
    }
