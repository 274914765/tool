...
bool cmWhileFunctionBlocker::IsFunctionBlocked(const cmListFileFunction & lff, cmMakefile & mf, cmExecutionStatus & inStatus)
{
    ...
    else if (!cmSystemTools::Strucmp(lff.Name.c_str(), "endwhile"))
    {
        ...
        if (!this->Depth)
        {
            ...
            while (isTrue)
            {
                ...
                mf.ExpandArguments(this->Args, expandedArguments);
                isTrue = cmIfCommand::IsTrue(expandedArguments, errorString, &mf, messageType);
            }
            return true;
        }
        else
...
bool cmWhileFunctionBlocker::ShouldRemove(const cmListFileFunction & lff, cmMakefile &)
{
