...
public:
    ...
    void CopyVerbatim(const std::string & line)
    {
        this->Tag = line;
    }
...
//----------------------------------------------------------------------------
const std::string & cmVisualStudioSlnParser::ParsedLine::GetValue(size_t idxValue) const
{
...
//----------------------------------------------------------------------------
std::string cmVisualStudioSlnParser::ParsedLine::GetValueVerbatim(size_t idxValue) const
{
    if (idxValue < this->Values.size())
    {
...
public:
    ...
    bool Process(const cmVisualStudioSlnParser::ParsedLine & line, cmSlnData & output, cmVisualStudioSlnParser::ResultData & result);
...
RequestedData(requestedData), CurrentLine(0)
{
...
//----------------------------------------------------------------------------
bool cmVisualStudioSlnParser::State::ReadLine(std::istream & input, std::string & line)
{
    ++this->CurrentLine;
...
//----------------------------------------------------------------------------
bool cmVisualStudioSlnParser::State::Process(const cmVisualStudioSlnParser::ParsedLine & line, cmSlnData & output, cmVisualStudioSlnParser::ResultData & result)
{
    assert(!line.IsComment());
    switch (this->Stack.top())
    {
        case FileStateStart:
            if (!cmSystemTools::StringStartsWith(line.GetTag().c_str(), "Microsoft Visual Studio Solution File"))
            {
        ...
        case FileStateTopLevel:
            if (line.GetTag().compare("Project") == 0)
            {
                ...
                {
                    if (!output.AddProject(line.GetValue(2), line.GetValue(0), line.GetValue(1)))
                    {
        ...
        case FileStateProject:
            ...
            {
                if (line.GetArg().compare("ProjectDependencies") == 0 && line.GetValue(0).compare("postProject") == 0)
                {
                    if (this->RequestedData.test(DataGroupProjectDependenciesBit))
        ...
        case FileStateGlobal:
            ...
            {
                if (line.GetArg().compare("SolutionConfigurationPlatforms") == 0 && line.GetValue(0).compare("preSolution") == 0)
                {
                ...
                else if (line.GetArg().compare("ProjectConfigurationPlatforms") == 0 && line.GetValue(0).compare("postSolution") == 0)
                {
                ...
                else if (line.GetArg().compare("NestedProjects") == 0 && line.GetValue(0).compare("preSolution") == 0)
                {
...
//----------------------------------------------------------------------------
bool cmVisualStudioSlnParser::State::Finished(cmVisualStudioSlnParser::ResultData & result)
{
...
//----------------------------------------------------------------------------
cmVisualStudioSlnParser::ResultData::ResultData():Result(ResultOK), ResultLine(0)
{
}
...
//----------------------------------------------------------------------------
void cmVisualStudioSlnParser::ResultData::SetError(ParseResult error, size_t line)
{
    this->Result = error;
...
//----------------------------------------------------------------------------
const cmVisualStudioSlnParser::DataGroupSet cmVisualStudioSlnParser::DataGroupProjects(1 << cmVisualStudioSlnParser::DataGroupProjectsBit);

const cmVisualStudioSlnParser::DataGroupSet cmVisualStudioSlnParser::DataGroupProjectDependencies(1 << cmVisualStudioSlnParser::DataGroupProjectDependenciesBit);

const cmVisualStudioSlnParser::DataGroupSet cmVisualStudioSlnParser::DataGroupSolutionConfigurations(1 << cmVisualStudioSlnParser::DataGroupSolutionConfigurationsBit);

const cmVisualStudioSlnParser::DataGroupSet cmVisualStudioSlnParser::DataGroupProjectConfigurations(1 << cmVisualStudioSlnParser::DataGroupProjectConfigurationsBit);

const cmVisualStudioSlnParser::DataGroupSet cmVisualStudioSlnParser::DataGroupSolutionFilters(1 << cmVisualStudioSlnParser::DataGroupSolutionFiltersBit);

const cmVisualStudioSlnParser::DataGroupSet cmVisualStudioSlnParser::DataGroupGenericGlobalSections(1 << cmVisualStudioSlnParser::DataGroupGenericGlobalSectionsBit);

const cmVisualStudioSlnParser::DataGroupSet cmVisualStudioSlnParser::DataGroupAll(~0);

//----------------------------------------------------------------------------
bool cmVisualStudioSlnParser::Parse(std::istream & input, cmSlnData & output, DataGroupSet dataGroups)
{
...
//----------------------------------------------------------------------------
bool cmVisualStudioSlnParser::ParseFile(const std::string & file, cmSlnData & output, DataGroupSet dataGroups)
{
    this->LastResult.Clear();
    if (!this->IsDataGroupSetSupported(dataGroups))
    {
...
//----------------------------------------------------------------------------
cmVisualStudioSlnParser::ParseResult cmVisualStudioSlnParser::GetParseResult() const
{
...
//----------------------------------------------------------------------------
bool cmVisualStudioSlnParser::IsDataGroupSetSupported(DataGroupSet dataGroups) const
{
...
//----------------------------------------------------------------------------
bool cmVisualStudioSlnParser::ParseImpl(std::istream & input, cmSlnData & output, State & state)
{
    std::string line;
...
//----------------------------------------------------------------------------
bool cmVisualStudioSlnParser::ParseBOM(std::istream & input, std::string & line, State & state)
{
    char bom[4];
    if (!input.get(bom, 4))
    {
    ...
    this->LastResult.HadBOM = (bom[0] == char (0xEF) && bom[1] == char (0xBB) && bom[2] == char (0xBF));
...
//----------------------------------------------------------------------------
bool cmVisualStudioSlnParser::ParseMultiValueTag(const std::string & line, ParsedLine & parsedLine, State & state)
{
        ...
        if (idxFieldStart < line.size())
        {
            ...
            for (;;)
            {
                ...
                if (idxParsing == line.npos)
                {
                    ...
                    if (inQuotes)
                    {
                        this->LastResult.SetError(ResultErrorInputStructure, state.GetCurrentLine());
                        return false;
                    }
                }
                ...
                if (fieldOver)
                {
                    if (!this->ParseValue(line.substr(idxFieldStart, idxParsing - idxFieldStart), parsedLine))
                        return false;
                    if (idxParsing == line.npos)
...
//----------------------------------------------------------------------------
bool cmVisualStudioSlnParser::ParseSingleValueTag(const std::string & line, ParsedLine & parsedLine, State & state)
{
    size_t idxEqualSign = line.find('=');
    const std::string & fullTag = line.substr(0, idxEqualSign);
    if (!this->ParseTag(fullTag, parsedLine, state))
...
//----------------------------------------------------------------------------
bool cmVisualStudioSlnParser::ParseKeyValuePair(const std::string & line, ParsedLine & parsedLine, State & /*state */ )
{
    size_t idxEqualSign = line.find('=');
    if (idxEqualSign == line.npos)
    {
...
//----------------------------------------------------------------------------
bool cmVisualStudioSlnParser::ParseTag(const std::string & fullTag, ParsedLine & parsedLine, State & state)
{
    ...
    parsedLine.SetTag(cmSystemTools::TrimWhitespace(fullTag.substr(0, idxLeftParen)));
    ...
    const std::string & arg = cmSystemTools::TrimWhitespace(fullTag.substr(idxLeftParen + 1, idxRightParen - idxLeftParen - 1));
    if (arg[0] == '"')
    {
        if (arg[arg.size() - 1] != '"')
        {
...
//----------------------------------------------------------------------------
bool cmVisualStudioSlnParser::ParseValue(const std::string & value, ParsedLine & parsedLine)
{
