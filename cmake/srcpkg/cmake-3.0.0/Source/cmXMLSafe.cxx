...
//----------------------------------------------------------------------------
cmXMLSafe::cmXMLSafe(const char *s):Data(s), Size(static_cast < unsigned long >(strlen(s))), DoQuotes(true)
{
}

//----------------------------------------------------------------------------
cmXMLSafe::cmXMLSafe(cmsys_stl::string const &s):Data(s.c_str()), Size(static_cast < unsigned long >(s.length())), DoQuotes(true)
{
}
...
cmsys_ios::ostream & operator<<(cmsys_ios::ostream & os, cmXMLSafe const &self)
{
    ...
    while (first != last)
    {
        ...
        if (const char *next = cm_utf8_decode_character(first, last, &ch))
        {
            ...
            if ((ch >= 0x20 && ch <= 0xD7FF) || (ch >= 0xE000 && ch <= 0xFFFD) || (ch >= 0x10000 && ch <= 0x10FFFF) || ch == 0x9 || ch == 0xA || ch == 0xD)
            {
                switch (ch)
                {
