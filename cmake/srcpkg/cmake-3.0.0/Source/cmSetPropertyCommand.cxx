...
//----------------------------------------------------------------------------
bool cmSetPropertyCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 2)
    {
    ...
    {
        cmOStringStream e;
        e << "given invalid scope " << *arg << ".  " << "Valid scopes are GLOBAL, DIRECTORY, TARGET, SOURCE, TEST, CACHE.";
    ...
    enum Doing
    { DoingNone, DoingNames, DoingProperty, DoingValues };
...
bool cmSetPropertyCommand::HandleDirectoryMode()
{
    ...
    if (!this->Names.empty())
    {
        ...
        if (cmLocalGenerator * lg = (this->Makefile->GetLocalGenerator()->GetGlobalGenerator()->FindLocalGenerator(dir.c_str())))
        {
        ...
        else
        {
            ...
            this->SetError("DIRECTORY scope provided but requested directory was not found. " "This could be because the directory argument was invalid or, " "it is valid but has not been processed yet.");
            return false;
        }
    }
...
{
    for (std::set < cmStdString >::const_iterator ni = this->Names.begin(); ni != this->Names.end(); ++ni)
    {
        ...
        {
            cmOStringStream e;
            e << "could not find TARGET " << *ni << ".  Perhaps it has not yet been created.";
            this->SetError(e.str().c_str());
            return false;
        }
    }
...
bool cmSetPropertyCommand::HandleTestMode()
{
    ...
    for (std::set < cmStdString >::iterator ni = this->Names.begin(); ni != this->Names.end(); ni = next)
    {
        next = ni;
        ++next;
...
bool cmSetPropertyCommand::HandleCacheMode()
{
    ...
    {
        if (!this->Remove && !cmSystemTools::IsOn(this->PropertyValue.c_str()) && !cmSystemTools::IsOff(this->PropertyValue.c_str()))
        {
            cmOStringStream e;
            e << "given non-boolean value \"" << this->PropertyValue << "\" for CACHE property \"ADVANCED\".  ";
            this->SetError(e.str().c_str());
            return false;
        }
    }
    ...
    else if (this->PropertyName != "HELPSTRING" && this->PropertyName != "STRINGS" && this->PropertyName != "VALUE")
    {
        cmOStringStream e;
        e << "given invalid CACHE property " << this->PropertyName << ".  " << "Settable CACHE properties are: " << "ADVANCED, HELPSTRING, STRINGS, TYPE, and VALUE.";
        this->SetError(e.str().c_str());
        return false;
    }
    ...
    for (std::set < cmStdString >::const_iterator ni = this->Names.begin(); ni != this->Names.end(); ++ni)
    {
        ...
        cmCacheManager::CacheIterator it = cm->GetCacheManager()->GetCacheIterator(ni->c_str());
        if (!it.IsAtEnd())
        {
        ...
        {
            cmOStringStream e;
            e << "could not find CACHE variable " << *ni << ".  Perhaps it has not yet been created.";
            this->SetError(e.str().c_str());
            return false;
        }
    }
