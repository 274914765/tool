...
{
    const std::string registryBase = cmGlobalVisualStudioGenerator::GetRegistryBase(version);
    const std::string vckey = registryBase + "\\Setup\\VC;ProductDir";
    const std::string vskey = registryBase + "\\Setup\\VS;ProductDir";

    if (!cmSystemTools::ReadRegistryValue(vckey.c_str(), this->VcInstallDir, cmSystemTools::KeyWOW64_32) || !cmSystemTools::ReadRegistryValue(vskey.c_str(), this->VsInstallDir, cmSystemTools::KeyWOW64_32))
    {
        return 0;
    }
    ...
    const std::string configFilename = this->VcInstallDir + "vcpackages/WCE.VCPlatform.config";
...
{
    std::map < std::string, std::string >::const_iterator it = this->Macros.find("ARCHFAM");
...
void cmVisualStudioWCEPlatformParser::StartElement(const char *name, const char **attributes)
{
...
void cmVisualStudioWCEPlatformParser::CharacterDataHandler(const char *data, int length)
{
    this->CharacterData.append(data, length);
}

std::string cmVisualStudioWCEPlatformParser::FixPaths(const std::string & paths) const
{
