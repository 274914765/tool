...
#    include "cmDependsFortran.h"
                                // For -E cmake_copy_f90_mod callback.
...
int cmcmd::ExecuteCMakeCommand(std::vector < std::string > &args)
{
    ...
    if (args.size() > 1)
    {
        ...
        if (args[1] == "copy" && args.size() == 4)
        {
            ...
            {
                std::cerr << "Error copying file \"" << args[2].c_str() << "\" to \"" << args[3].c_str() << "\".\n";
                return 1;
            }
            return 0;
        }
        ...
        if (args[1] == "copy_if_different" && args.size() == 4)
        {
            ...
            {
                std::cerr << "Error copying file (if different) from \"" << args[2].c_str() << "\" to \"" << args[3].c_str() << "\".\n";
                return 1;
            }
            return 0;
        }
        ...
        if (args[1] == "copy_directory" && args.size() == 4)
        {
            ...
            {
                std::cerr << "Error copying directory from \"" << args[2].c_str() << "\" to \"" << args[3].c_str() << "\".\n";
                return 1;
            }
            return 0;
        }
        ...
        if (args[1] == "rename" && args.size() == 4)
        {
            if (!cmSystemTools::RenameFile(args[2].c_str(), args[3].c_str()))
            {
                ...
                std::cerr << "Error renaming from \"" << args[2].c_str() << "\" to \"" << args[3].c_str() << "\": " << e << "\n";
                return 1;
            }
            return 0;
        }
        ...
        if (args[1] == "compare_files" && args.size() == 4)
        {
            ...
            {
                std::cerr << "Files \"" << args[2].c_str() << "\" to \"" << args[3].c_str() << "\" are different.\n";
                return 1;
            }
            return 0;
        }
        ...
        {
            if (cmSystemTools::FileIsDirectory(args[2].c_str()) && !cmSystemTools::RemoveADirectory(args[2].c_str()))
            {
        ...
        else if (args[1] == "remove" && args.size() > 2)
        {
            ...
            for (std::string::size_type cc = 2; cc < args.size(); cc++)
            {
                ...
                else
                {
                    ...
                    if (!cmSystemTools::RemoveFile(args[cc].c_str()) && !force && cmSystemTools::FileExists(args[cc].c_str()))
                    {
                        return 1;
                    }
                }
        ...
        else if (args[1] == "time" && args.size() > 2)
        {
            ...
            std::cout << "Elapsed time: " << static_cast < long >(time_finish - time_start) << " s. (time)" << ", " << static_cast < double >(clock_finish - clock_start) / clocks_per_sec << " s. (clock)" << "\n";
            return ret;
        }
        ...
        else if (args[1] == "chdir" && args.size() >= 4)
        {
            ...
            {
                cmSystemTools::Error("Directory does not exist for chdir command: ", args[2].c_str());
                return 1;
            }
            ...
            int retval = 0;
            int timeout = 0;
            if (cmSystemTools::RunSingleCommand(command.c_str(), 0, &retval, directory.c_str(), cmSystemTools::OUTPUT_NORMAL, timeout))
            {
                return retval;
            }
        ...
        else if (args[1] == "cmake_progress_report" && args.size() >= 3)
        {
            ...
            int fileNum = static_cast < int >(cmsys::Directory::GetNumberOfFilesInDirectory(dirName.c_str()));
            if (count > 0)
            {
                // print the progress
        ...
        else if (args[1] == "create_symlink" && args.size() == 4)
        {
            ...
            if ((cmSystemTools::FileExists(destinationFileName) || cmSystemTools::FileIsSymlink(destinationFileName)) && !cmSystemTools::RemoveFile(destinationFileName))
            {
            ...
            {
                std::string emsg = cmSystemTools::GetLastSystemError();
                std::cerr << "failed to create symbolic link '" << destinationFileName << "': " << emsg << "\n";
                return 1;
            }
            return 0;
        }
        ...
        else if (args[1] == "cmake_call_visual_studio_macro" && args.size() >= 4)
        {
            ...
            return cmCallVisualStudioMacro::CallMacro(args[2], args[3], macroArgs, true);
        }
#endif
        ...
        else if (args[1] == "cmake_depends" && args.size() >= 6)
        {
            ...
            if (args.size() >= 8)
            {
                ...
                startOutDir = args[6];
                depInfo = args[7];
                if (args.size() >= 9 && args[8].length() >= 8 && args[8].substr(0, 8) == "--color=")
                {
                    // Enable or disable color based on the switch value.
                    color = (args[8].size() == 8 || cmSystemTools::IsOn(args[8].substr(8).c_str()));
                }
            }
            else
            {
        ...
        else if (args[1] == "tar" && args.size() > 3)
        {
            ...
            {
                if (!cmSystemTools::CreateTar(outFile.c_str(), files, gzip, bzip2, verbose))
                {
                    cmSystemTools::Error("Problem creating tar: ", outFile.c_str());
                    return 1;
                }
            ...
            {
                if (!cmSystemTools::ExtractTar(outFile.c_str(), gzip, verbose))
                {
        ...
        {
            return cmSystemTools::WriteRegistryValue(args[2].c_str(), args[3].c_str())? 0 : 1;
        }
...
{
    if (cmSystemTools::FileExists(link.c_str()) || cmSystemTools::FileIsSymlink(link.c_str()))
    {
    ...
    while (result == 0 && cmSystemTools::GetLineFromStream(fin, command))
    {
        ...
        switch (cmsysProcess_GetState(cp))
        {
            ...
            case cmsysProcess_State_Exception:
                std::cerr << "Error running link command: " << cmsysProcess_GetExceptionString(cp) << std::endl;
                result = 1;
                break;
            case cmsysProcess_State_Error:
                std::cerr << "Error running link command: " << cmsysProcess_GetErrorString(cp) << std::endl;
                result = 2;
                break;
            default:
                break;
...
int cmcmd::VisualStudioLink(std::vector < std::string > &args, int type)
{
    ...
    for (std::vector < std::string >::iterator i = expandedArgs.begin(); i != expandedArgs.end(); ++i)
    {
    ...
    return cmcmd::VisualStudioLinkNonIncremental(expandedArgs, type, hasManifest, verbose);
}

int cmcmd::ParseVisualStudioLinkCommand(std::vector < std::string > &args, std::vector < cmStdString > &command, std::string & targetName)
{
...
bool cmcmd::RunCommand(const char *comment, std::vector < cmStdString > &command, bool verbose, int *retCodeOut)
{
    if (verbose)
    {
        std::cout << comment << ":\n";
        for (std::vector < cmStdString >::iterator i = command.begin(); i != command.end(); ++i)
        {
            std::cout << i->c_str() << " ";
        }
        std::cout << "\n";
    ...
    cmSystemTools::RunSingleCommand(command, &output, &retCode, 0, cmSystemTools::OUTPUT_NONE);
    // always print the output of the command, unless
    // it is the dumb rc command banner, but if the command
    // returned an error code then print the output anyway as
    // the banner may be mixed with some other important information.
    if (output.find("Resource Compiler Version") == output.npos || retCode != 0)
    {
        std::cout << output;
    }
...
int cmcmd::VisualStudioLinkIncremental(std::vector < std::string > &args, int type, bool verbose)
{
    ...
    fout << type << " /* CREATEPROCESS_MANIFEST_RESOURCE_ID " "*/ 24 /* RT_MANIFEST */ " << "\"" << fullPath.c_str() << "\"";
    fout.close();
...
int cmcmd::VisualStudioLinkNonIncremental(std::vector < std::string > &args, int type, bool hasManifest, bool verbose)
{
    std::vector < cmStdString > linkCommand;
    std::string targetName;
    if (cmcmd::ParseVisualStudioLinkCommand(args, linkCommand, targetName) == -1)
