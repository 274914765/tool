...
public:
    ...
    void ParseError(const char **atts)
    {
        ...
        if (!cat)
        {
            ...
            cmCTestLog(this->CTest, ERROR_MESSAGE, "No Category found in Bounds checker XML\n");
            return;
        }
        ...
        {
            this->Errors.push_back(cmCTestMemCheckHandler::ABW);    // do not know
            cmCTestLog(this->CTest, ERROR_MESSAGE, "Found unknown Bounds Checker error " << ptr->ErrorCategory << std::endl);
        }
    }
    cmCTest *CTest;
...
int cmCTestMemCheckHandler::PreProcessHandler()
{
    ...
    {
        cmCTestLog(this->CTest, ERROR_MESSAGE, "Problem executing pre-memcheck command(s)." << std::endl);
        return 0;
    }
    return 1;
}
...
int cmCTestMemCheckHandler::PostProcessHandler()
{
    ...
    {
        cmCTestLog(this->CTest, ERROR_MESSAGE, "Problem executing post-memcheck command(s)." << std::endl);
        return 0;
    }
    return 1;
}
...
//----------------------------------------------------------------------
void cmCTestMemCheckHandler::GenerateTestCommand(std::vector < std::string > &args, int test)
{
    std::vector < cmStdString >::size_type pp;
    cmStdString index;
    cmOStringStream stream;
    std::string memcheckcommand = cmSystemTools::ConvertToOutputPath(this->MemoryTester.c_str());
    stream << test;
    index = stream.str();
    ...
    cmCTestLog(this->CTest, HANDLER_VERBOSE_OUTPUT, "Memory check command: " << memcheckcommand << std::endl);
}
...
void cmCTestMemCheckHandler::PopulateCustomVectors(cmMakefile * mf)
{
    ...
    this->CTest->PopulateCustomVector(mf, "CTEST_CUSTOM_PRE_MEMCHECK", this->CustomPreMemCheck);
    this->CTest->PopulateCustomVector(mf, "CTEST_CUSTOM_POST_MEMCHECK", this->CustomPostMemCheck);

    this->CTest->PopulateCustomVector(mf, "CTEST_CUSTOM_MEMCHECK_IGNORE", this->CustomTestsIgnore);
}
...
void cmCTestMemCheckHandler::GenerateDartOutput(std::ostream & os)
{
    ...
    os << "\t</TestList>\n";
    cmCTestLog(this->CTest, HANDLER_OUTPUT, "-- Processing memory checking output: ");
    ...
    for (cc = 0; cc < this->TestResults.size(); cc++)
    {
        ...
        this->CleanTestOutput(memcheckstr, static_cast < size_t > (this->CustomMaximumFailedTestOutputSize));
        ...
        for (kk = 0; cmCTestMemCheckResultLongStrings[kk]; kk++)
        {
            ...
            {
                os << "\t\t\t<Defect type=\"" << cmCTestMemCheckResultLongStrings[kk] << "\">" << memcheckresults[kk] << "</Defect>" << std::endl;
            }
    ...
    cmCTestLog(this->CTest, HANDLER_OUTPUT, "Memory checking results:" << std::endl);
...
#    undef cerr
            ...
            std::cerr.width(35);
#define cerr no_cerr
            cmCTestLog(this->CTest, HANDLER_OUTPUT, cmCTestMemCheckResultLongStrings[cc] << " - " << this->MemoryTesterGlobalResults[cc] << std::endl);
    ...
    os << "\t<EndTestTime>" << this->EndTestTime << "</EndTestTime>" << std::endl;
    os << "<ElapsedMinutes>" << static_cast < int >(this->ElapsedTestingTime / 6) / 10.0 << "</ElapsedMinutes>\n";
...
{
    // Setup the command
    if (cmSystemTools::FileExists(this->CTest->GetCTestConfiguration("MemoryCheckCommand").c_str()))
    {
        this->MemoryTester = this->CTest->GetCTestConfiguration("MemoryCheckCommand").c_str();
    ...
    else if (cmSystemTools::FileExists(this->CTest->GetCTestConfiguration("PurifyCommand").c_str()))
    {
        this->MemoryTester = this->CTest->GetCTestConfiguration("PurifyCommand").c_str();
        this->MemoryTesterStyle = cmCTestMemCheckHandler::PURIFY;
    }
    else if (cmSystemTools::FileExists(this->CTest->GetCTestConfiguration("ValgrindCommand").c_str()))
    {
        this->MemoryTester = this->CTest->GetCTestConfiguration("ValgrindCommand").c_str();
        this->MemoryTesterStyle = cmCTestMemCheckHandler::VALGRIND;
    }
    else if (cmSystemTools::FileExists(this->CTest->GetCTestConfiguration("BoundsCheckerCommand").c_str()))
    {
        this->MemoryTester = this->CTest->GetCTestConfiguration("BoundsCheckerCommand").c_str();
        this->MemoryTesterStyle = cmCTestMemCheckHandler::BOUNDS_CHECKER;
    }
    else
    {
        cmCTestLog(this->CTest, WARNING, "Memory checker (MemoryCheckCommand) " "not set, or cannot find the specified program." << std::endl);
        return false;
    }
    ...
    {
        memoryTesterOptions = this->CTest->GetCTestConfiguration("MemoryCheckCommandOptions");
    }
    else if (this->CTest->GetCTestConfiguration("ValgrindCommandOptions").size())
    {
        memoryTesterOptions = this->CTest->GetCTestConfiguration("ValgrindCommandOptions");
    }
    this->MemoryTesterOptions = cmSystemTools::ParseArguments(memoryTesterOptions.c_str());
    ...
    switch (this->MemoryTesterStyle)
    {
        case cmCTestMemCheckHandler::VALGRIND:
            {
                ...
                if (this->CTest->GetCTestConfiguration("MemoryCheckSuppressionFile").size())
                {
                    if (!cmSystemTools::FileExists(this->CTest->GetCTestConfiguration("MemoryCheckSuppressionFile").c_str()))
                    {
                        cmCTestLog(this->CTest, ERROR_MESSAGE, "Cannot find memory checker suppression file: " << this->CTest->GetCTestConfiguration("MemoryCheckSuppressionFile").c_str() << std::endl);
                        return false;
                    }
                    std::string suppressions = "--suppressions=" + this->CTest->GetCTestConfiguration("MemoryCheckSuppressionFile");
        ...
        case cmCTestMemCheckHandler::PURIFY:
            {
                ...
                if (this->CTest->GetCTestConfiguration("MemoryCheckSuppressionFile").size())
                {
                    ...
                    std::string filterFiles = "/FilterFiles=" + this->CTest->GetCTestConfiguration("MemoryCheckSuppressionFile");
        ...
        case cmCTestMemCheckHandler::BOUNDS_CHECKER:
            {
                ...
                std::string dpbdFile = this->CTest->GetBinaryDir() + "/Testing/Temporary/MemoryChecker.??.DPbd";
        ...
        default:
            cmCTestLog(this->CTest, ERROR_MESSAGE, "Do not understand memory checker: " << this->MemoryTester.c_str() << std::endl);
            return false;
    }
...
//----------------------------------------------------------------------
bool cmCTestMemCheckHandler::ProcessMemCheckOutput(const std::string & str, std::string & log, int *results)
{
    ...
    else if (this->MemoryTesterStyle == cmCTestMemCheckHandler::BOUNDS_CHECKER)
    {
...
//----------------------------------------------------------------------
bool cmCTestMemCheckHandler::ProcessMemCheckPurifyOutput(const std::string & str, std::string & log, int *results)
{
    ...
    for (std::vector < cmStdString >::iterator i = lines.begin(); i != lines.end(); ++i)
    {
        ...
        if (pfW.find(*i))
        {
            ...
            {
                cmCTestLog(this->CTest, ERROR_MESSAGE, "Unknown Purify memory fault: " << pfW.match(1) << std::endl);
...
//----------------------------------------------------------------------
bool cmCTestMemCheckHandler::ProcessMemCheckValgrindOutput(const std::string & str, std::string & log, int *results)
{
    std::vector < cmStdString > lines;
    cmSystemTools::Split(str.c_str(), lines);
    bool unlimitedOutput = false;
    if (str.find("CTEST_FULL_OUTPUT") != str.npos || this->CustomMaximumFailedTestOutputSize == 0)
    {
    ...
    cmsys::RegularExpression vgMLK1("== .*[0-9,]+ bytes in [0-9,]+ blocks are definitely lost" " in loss record [0-9,]+ of [0-9,]+");
    cmsys::RegularExpression vgMLK2("== .*[0-9,]+ \\([0-9,]+ direct, [0-9,]+ indirect\\)" " bytes in [0-9,]+ blocks are definitely lost" " in loss record [0-9,]+ of [0-9,]+");
    cmsys::RegularExpression vgPAR("== .*Syscall param .* (contains|points to) unaddressable byte\\(s\\)");
    cmsys::RegularExpression vgMPK1("== .*[0-9,]+ bytes in [0-9,]+ blocks are possibly lost in" " loss record [0-9,]+ of [0-9,]+");
    cmsys::RegularExpression vgMPK2("== .*[0-9,]+ bytes in [0-9,]+ blocks are still reachable" " in loss record [0-9,]+ of [0-9,]+");
    cmsys::RegularExpression vgUMC("== .*Conditional jump or move depends on uninitialised value\\(s\\)");
    cmsys::RegularExpression vgUMR1("== .*Use of uninitialised value of size [0-9,]+");
    ...
    {
        cmCTestLog(this->CTest, DEBUG, "test line " << lines[cc] << std::endl);

        if (valgrindLine.find(lines[cc]))
        {
            cmCTestLog(this->CTest, DEBUG, "valgrind  line " << lines[cc] << std::endl);
            int failure = cmCTestMemCheckHandler::NO_MEMORY_FAULT;
    ...
    if (!outputFull)
    {
        for (std::vector < std::string::size_type >::iterator i = nonValGrindOutput.begin(); i != nonValGrindOutput.end(); ++i)
        {
            totalOutputSize += lines[*i].size();
            cmCTestLog(this->CTest, DEBUG, "before xml safe " << lines[*i] << std::endl);
            cmCTestLog(this->CTest, DEBUG, "after  xml safe " << cmXMLSafe(lines[*i]) << std::endl);

            ostr << cmXMLSafe(lines[*i]) << std::endl;
            if (!unlimitedOutput && totalOutputSize > static_cast < size_t > (this->CustomMaximumFailedTestOutputSize))
            {
                outputFull = true;
                ostr << "....\n";
    ...
    cmCTestLog(this->CTest, DEBUG, "End test (elapsed: " << (cmSystemTools::GetTime() - sttime) << std::endl);
    log = ostr.str();
    if (defects)
    {
        return false;
...
//----------------------------------------------------------------------
bool cmCTestMemCheckHandler::ProcessMemCheckBoundsCheckerOutput(const std::string & str, std::string & log, int *results)
{
    log = "";
    double sttime = cmSystemTools::GetTime();
    std::vector < cmStdString > lines;
    ...
    if (cc < lines.size())
    {
        for (cc++; cc < lines.size(); ++cc)
        {
            ...
            {
                cmCTestLog(this->CTest, ERROR_MESSAGE, "Error in ParseChunk: " << theLine.c_str() << std::endl);
            }
        }
    }
    int defects = 0;
...
// for the test
void cmCTestMemCheckHandler::PostProcessBoundsCheckerTest(cmCTestTestResult & res, int test)
{
    cmCTestLog(this->CTest, HANDLER_VERBOSE_OUTPUT, "PostProcessBoundsCheckerTest for : " << res.Name.c_str() << std::endl);
    ...
    cmCTestLog(this->CTest, HANDLER_VERBOSE_OUTPUT, "Remove: " << this->BoundsCheckerDPBDFile.c_str() << std::endl);
    cmSystemTools::RemoveFile(this->BoundsCheckerXMLFile.c_str());
    cmCTestLog(this->CTest, HANDLER_VERBOSE_OUTPUT, "Remove: " << this->BoundsCheckerXMLFile.c_str() << std::endl);
}

void cmCTestMemCheckHandler::PostProcessPurifyTest(cmCTestTestResult & res, int test)
{
    cmCTestLog(this->CTest, HANDLER_VERBOSE_OUTPUT, "PostProcessPurifyTest for : " << res.Name.c_str() << std::endl);
    appendMemTesterOutput(res, test);
}

void cmCTestMemCheckHandler::PostProcessValgrindTest(cmCTestTestResult & res, int test)
{
    cmCTestLog(this->CTest, HANDLER_VERBOSE_OUTPUT, "PostProcessValgrindTest for : " << res.Name.c_str() << std::endl);
    appendMemTesterOutput(res, test);
}

void cmCTestMemCheckHandler::appendMemTesterOutput(cmCTestTestResult & res, int test)
{
    cmStdString ofile = testOutputFileName(test);
...
cmStdString cmCTestMemCheckHandler::testOutputFileName(int test)
{
    cmStdString index;
    cmOStringStream stream;
    stream << test;
