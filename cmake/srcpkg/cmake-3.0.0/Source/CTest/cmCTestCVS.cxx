...
public:
    UpdateParser(cmCTestCVS * cvs, const char *prefix):CVS(cvs)
    {
        ...
        this->RegexFileRemoved1.compile("cvs[^ ]* update: `?([^']*)'? is no longer in the repository");
        this->RegexFileRemoved2.compile("cvs[^ ]* update: " "warning: `?([^']*)'? is not \\(any longer\\) pertinent");
    }
private:
    cmCTestCVS * CVS;
...
public:
    ...
    LogParser(cmCTestCVS * cvs, const char *prefix, std::vector < Revision > &revs):CVS(cvs), Revisions(revs), Section(SectionHeader)
    {
        this->SetLog(&cvs->Log, prefix), this->RegexRevision.compile("^revision +([^ ]*) *$");
...
private:
    ...
    enum SectionType
    { SectionHeader, SectionRevisions, SectionEnd };
    SectionType Section;
    Revision Rev;
...
std::string cmCTestCVS::ComputeBranchFlag(std::string const &dir)
{
    ...
    if (tagStream && cmSystemTools::GetLineFromStream(tagStream, tagLine) && tagLine.size() > 1 && tagLine[0] == 'T')
    {
...
//----------------------------------------------------------------------------
void cmCTestCVS::LoadRevisions(std::string const &file, const char *branchFlag, std::vector < Revision > &revisions)
{
...
//----------------------------------------------------------------------------
void cmCTestCVS::WriteXMLDirectory(std::ostream & xml, std::string const &path, Directory const &dir)
{
...
{
    cmCTestLog(this->CTest, HANDLER_OUTPUT, "   Gathering version information (one . per updated file):\n" "    " << std::flush);

    for (std::map < cmStdString, Directory >::const_iterator di = this->Dirs.begin(); di != this->Dirs.end(); ++di)
    {
