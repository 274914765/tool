...
cmParseGTMCoverage::cmParseGTMCoverage(cmCTestCoverageHandlerContainer & cont, cmCTest * ctest):cmParseMumpsCoverage(cont, ctest)
{
}
...
bool cmParseGTMCoverage::ReadMCovFile(const char *file)
{
    ...
    while (cmSystemTools::GetLineFromStream(in, line))
    {
        ...
        if (function == lastfunction && lastroutine == routine)
        {
            ...
            {
                cmCTestLog(this->CTest, ERROR_MESSAGE, "Can not find mumps file : " << lastroutine << "  referenced in this line of mcov data:\n" "[" << line << "]\n");
            }
            continue;
        }
...
bool cmParseGTMCoverage::FindFunctionInMumpsFile(std::string const &filepath, std::string const &function, int &lineoffset)
{
...
bool cmParseGTMCoverage::ParseMCOVLine(std::string const &line, std::string & routine, std::string & function, int &linenumber, int &count)
{
