...
cmParsePHPCoverage::cmParsePHPCoverage(cmCTestCoverageHandlerContainer & cont, cmCTest * ctest):Coverage(cont), CTest(ctest)
{
}
...
bool cmParsePHPCoverage::ReadCoverageArray(std::istream & in, cmStdString const &fileName)
{
    cmCTestCoverageHandlerContainer::SingleFileCoverageVector & coverageVector = this->Coverage.TotalCoverage[fileName];
    ...
    if (strcmp(buf, ";a:") != 0)
    {
        cmCTestLog(this->CTest, ERROR_MESSAGE, "failed to read start of coverage array, found : " << buf << "\n");
        return false;
    }
    int size = 0;
...
bool cmParsePHPCoverage::ReadFileInformation(std::istream & in)
{
    ...
    if (strcmp(buf, "s:") != 0)
    {
        cmCTestLog(this->CTest, ERROR_MESSAGE, "failed to read start of file info found: [" << buf << "]\n");
        return false;
    }
    char c;
    int size = 0;
    if (this->ReadInt(in, size))
    {
        ...
        {
            cmCTestLog(this->CTest, ERROR_MESSAGE, "failed to read coverage array for file: " << fileName << "\n");
            return false;
        }
        return true;
    }
