...
cmParseMumpsCoverage::cmParseMumpsCoverage(cmCTestCoverageHandlerContainer & cont, cmCTest * ctest):Coverage(cont), CTest(ctest)
{
}
...
bool cmParseMumpsCoverage::ReadCoverageFile(const char *file)
{
    ...
    while (cmSystemTools::GetLineFromStream(in, line))
    {
        ...
        if (pos != std::string::npos)
        {
            ...
            {
                cmCTestLog(this->CTest, ERROR_MESSAGE, "Parse Error in Mumps coverage file :\n" << file << "\ntype: [" << type << "]\npath:[" << path << "]\n" "input line: [" << line << "]\n");
            }
        }
    }
    return true;
...
void cmParseMumpsCoverage::InitializeMumpsFile(std::string & file)
{
    ...
    std::string line;
    cmCTestCoverageHandlerContainer::SingleFileCoverageVector & coverageVector = this->Coverage.TotalCoverage[file];
...
bool cmParseMumpsCoverage::FindMumpsFile(std::string const &routine, std::string & filepath)
{
    std::map < cmStdString, cmStdString >::iterator i = this->RoutineToDirectory.find(routine);
