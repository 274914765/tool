...
cmParseCacheCoverage::cmParseCacheCoverage(cmCTestCoverageHandlerContainer & cont, cmCTest * ctest):cmParseMumpsCoverage(cont, ctest)
{
}
...
bool cmParseCacheCoverage::LoadCoverageData(const char *d)
{
    ...
    for (i = 0; i < numf; i++)
    {
        ...
        if (file != "." && file != ".." && !cmSystemTools::FileIsDirectory(file.c_str()))
        {
            std::string path = d;
            path += "/";
            path += file;
...
void cmParseCacheCoverage::RemoveUnCoveredFiles()
{
    ...
    cmCTestCoverageHandlerContainer::TotalCoverageMap::iterator ci = this->Coverage.TotalCoverage.begin();
    while (ci != this->Coverage.TotalCoverage.end())
    {
        cmCTestCoverageHandlerContainer::SingleFileCoverageVector & v = ci->second;
        bool nothing = true;
        for (cmCTestCoverageHandlerContainer::SingleFileCoverageVector::iterator i = v.begin(); i != v.end(); ++i)
        {
            if (*i > 0)
            {
                nothing = false;
        ...
        if (nothing)
        {
            cmCTestLog(this->CTest, HANDLER_VERBOSE_OUTPUT, "No coverage found in: " << ci->first << std::endl);
...
bool cmParseCacheCoverage::SplitString(std::vector < std::string > &args, std::string const &line)
{
...
bool cmParseCacheCoverage::ReadCMCovFile(const char *file)
{
    ...
    if (separateLine.size() != 4 || separateLine[0] != "Routine" || separateLine[1] != "Line" || separateLine[2] != "RtnLine" || separateLine[3] != "Code")
    {
        cmCTestLog(this->CTest, ERROR_MESSAGE, "Bad first line of cmcov file : " << file << "  line:\n" "[" << line << "]\n");
    }
    std::string routine;
    std::string filepath;
    while (cmSystemTools::GetLineFromStream(in, line))
    {
        ...
        {
            cmCTestLog(this->CTest, ERROR_MESSAGE, "Bad line of cmcov file expected at least 4 found: " << separateLine.size() << " " << file << "  line:\n" "[" << line << "]\n");
        ...
        if (routine.size() == 0)
        {
            ...
            {
                cmCTestLog(this->CTest, ERROR_MESSAGE, "Could not find mumps file for routine: " << routine << "\n");
                filepath = "";
        ...
        cmCTestCoverageHandlerContainer::SingleFileCoverageVector & coverageVector = this->Coverage.TotalCoverage[filepath];
        ...
        {
            cmCTestLog(this->CTest, ERROR_MESSAGE, "Parse error line is greater than number of lines in file: " << linenumber << " " << filepath << "\n");
