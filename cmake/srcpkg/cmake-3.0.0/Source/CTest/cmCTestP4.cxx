...
{
public:
    IdentifyParser(cmCTestP4 * p4, const char *prefix, std::string & rev):Rev(rev)
    {
...
{
public:
    DiffParser(cmCTestP4 * p4, const char *prefix):P4(p4), AlreadyNotified(false)
    {
        this->SetLog(&P4->Log, prefix);
...
private:
    ...
    bool ProcessLine()
    {
        if (!this->Line.empty() && this->Line[0] == '=' && this->RegexDiff.find(this->Line))
        {
...
{
    std::map < std::string, cmCTestP4::User >::const_iterator it = Users.find(username);
...
{
public:
    DescribeParser(cmCTestP4 * p4, const char *prefix):LineParser('\n', false), P4(p4), Section(SectionHeader)
    {
        this->SetLog(&P4->Log, prefix);
...
private:
    ...
    enum SectionType
    { SectionHeader, SectionBody, SectionDiffHeader,
...
void cmCTestP4::SetP4Options(std::vector < char const *>&CommandOptions)
{
    ...
    for (std::vector < std::string >::iterator i = P4Options.begin(); i != P4Options.end(); ++i)
    {
...
void cmCTestP4::LoadRevisions()
{
    ...
    {
        cmCTestLog(this->CTest, HANDLER_OUTPUT, "   At least one of the revisions " << "is unknown. No repository changes will be reported.\n");
        return;
    }

    range.append("@").append(this->OldRevision).append(",").append(this->NewRevision);
    ...
    for (std::vector < std::string >::reverse_iterator i = ChangeLists.rbegin(); i != ChangeLists.rend(); ++i)
    {
...
bool cmCTestP4::UpdateCustom(const std::string & custom)
{
    ...
    for (std::vector < std::string >::const_iterator i = p4_custom_command.begin(); i != p4_custom_command.end(); ++i)
    {
