...
{
public:
    InfoParser(cmCTestSVN * svn, const char *prefix, std::string & rev, SVNInfo & svninfo):Rev(rev), SVNRepo(svninfo)
    {
...
void cmCTestSVN::NoteOldRevision()
{
    ...
    for (; itbeg != itend; itbeg++)
    {
        ...
        this->Log << "Revision for repository '" << svninfo.LocalPath << "' before update: " << svninfo.OldRevision << "\n";
...
void cmCTestSVN::NoteNewRevision()
{
    ...
    for (; itbeg != itend; itbeg++)
    {
        ...
        this->Log << "Revision for repository '" << svninfo.LocalPath << "' after update: " << svninfo.NewRevision << "\n";
        ...
        this->Log << "Repository '" << svninfo.LocalPath << "' URL = " << svninfo.URL << "\n";
        this->Log << "Repository '" << svninfo.LocalPath << "' Root = " << svninfo.Root << "\n";

        // Compute the base path the working tree has checked out under
        // the repository root.
        if (!svninfo.Root.empty() && cmCTestSVNPathStarts(svninfo.URL, svninfo.Root))
        {
            svninfo.Base = cmCTest::DecodeURL(svninfo.URL.substr(svninfo.Root.size()));
            svninfo.Base += "/";
        }
        this->Log << "Repository '" << svninfo.LocalPath << "' Base = " << svninfo.Base << "\n";
...
//----------------------------------------------------------------------------
void cmCTestSVN::GuessBase(SVNInfo & svninfo, std::vector < Change > const &changes)
{
    ...
    for (std::string::size_type slash = svninfo.URL.find('/'); svninfo.Base.empty() && slash != std::string::npos; slash = svninfo.URL.find('/', slash + 1))
    {
        ...
        for (std::vector < Change >::const_iterator ci = changes.begin(); svninfo.Base.empty() && ci != changes.end(); ++ci)
        {
...
private:
    ...
    bool ProcessLine()
    {
        ...
        {
            this->DoPath(this->RegexUpdate.match(1)[0], this->RegexUpdate.match(2)[0], this->RegexUpdate.match(3));
        }
        return true;
    }
...
bool cmCTestSVN::UpdateImpl()
{
    ...
    for (std::vector < cmStdString >::const_iterator ai = args.begin(); ai != args.end(); ++ai)
    {
...
//----------------------------------------------------------------------------
bool cmCTestSVN::RunSVNCommand(std::vector < char const *>const &parameters, OutputParser * out, OutputParser * err)
{
    if (parameters.empty())
        return false;
    ...
    std::vector < cmStdString > parsedUserOptions = cmSystemTools::ParseArguments(userOptions.c_str());
    for (std::vector < cmStdString >::iterator i = parsedUserOptions.begin(); i != parsedUserOptions.end(); ++i)
    {
...
//----------------------------------------------------------------------------
class cmCTestSVN::LogParser:public cmCTestVC::OutputLogger, private cmXMLParser
{
public:
    LogParser(cmCTestSVN * svn, const char *prefix, SVNInfo & svninfo):OutputLogger(svn->Log, prefix), SVN(svn), SVNRepo(svninfo)
    {
...
//----------------------------------------------------------------------------
void cmCTestSVN::DoRevisionSVN(Revision const &revision, std::vector < Change > const &changes)
{
    ...
    if (revision.Rev == revision.SVNInfo->OldRevision && revision.SVNInfo->LocalPath != "")
    {
        return;
    }
...
private:
    ...
    bool ProcessLine()
    {
        ...
        {
            this->DoPath(this->RegexStatus.match(1)[0], this->RegexStatus.match(2)[0], this->RegexStatus.match(3));
        }
        return true;
    }
...
private:
    ...
    void DoPath(std::string const &path)
    {
        ...
        if (path.size() > this->SVN->SourceDirectory.size() && strncmp(path.c_str(), this->SVN->SourceDirectory.c_str(), this->SVN->SourceDirectory.size()) == 0)
        {
...
std::string cmCTestSVN::SVNInfo::BuildLocalPath(std::string const &path) const
{
    ...
    if (path.size() > this->Base.size() && strncmp(path.c_str(), this->Base.c_str(), this->Base.size()) == 0)
    {
