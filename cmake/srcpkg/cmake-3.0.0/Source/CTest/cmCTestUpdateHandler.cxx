...
{
    cmCTestLog(this->CTest, DEBUG, "Determine update type from command: " << cmd << " and type: " << type << std::endl);
    if (type && *type)
    {
    ...
    {
        cmCTestLog(this->CTest, DEBUG, "Type not specified, check command: " << cmd << std::endl);
...
int cmCTestUpdateHandler::ProcessHandler()
{
    ...
    if (!sourceDirectory)
    {
        cmCTestLog(this->CTest, ERROR_MESSAGE, "Cannot find SourceDirectory  key in the DartConfiguration.tcl" << std::endl);
        return -1;
    }
    ...
    cmCTestLog(this->CTest, HANDLER_OUTPUT, "   Updating the repository: " << sourceDirectory << std::endl);
    ...
    cmCTestLog(this->CTest, HANDLER_OUTPUT, "   Use " << cmCTestUpdateHandlerUpdateToString(this->UpdateType) << " repository type" << std::endl;
        );
    ...
    {
        cmCTestLog(this->CTest, ERROR_MESSAGE, "Cannot open log file" << std::endl);
        return -1;
    }
    ...
    if (numUpdated)
    {
        cmCTestLog(this->CTest, HANDLER_OUTPUT, "   Found " << numUpdated << " updated files\n");
    }
    if (int numModified = vc->GetPathCount(cmCTestVC::PathModified))
    {
        cmCTestLog(this->CTest, HANDLER_OUTPUT, "   Found " << numModified << " locally modified files\n");
    ...
    if (localModifications)
    {
        ...
        cmCTestLog(this->CTest, ERROR_MESSAGE, "   There are modified or conflicting files in the repository" << std::endl);
    }
    if (!updated)
    {
        os << "Update command failed:\n" << vc->GetUpdateCommandLine();
        cmCTestLog(this->CTest, ERROR_MESSAGE, "   Update command failed: " << vc->GetUpdateCommandLine() << "\n");
    }
...
int cmCTestUpdateHandler::DetectVCS(const char *dir)
{
    ...
    cmCTestLog(this->CTest, DEBUG, "Check directory: " << sourceDirectory.c_str() << std::endl);
    sourceDirectory += "/.svn";
...
bool cmCTestUpdateHandler::SelectVCS()
{
    ...
    if (this->UpdateType == e_UNKNOWN)
    {
        ...
        this->UpdateType = this->DetermineType(this->UpdateCommand.c_str(), this->CTest->GetCTestConfiguration("UpdateType").c_str());
    }
