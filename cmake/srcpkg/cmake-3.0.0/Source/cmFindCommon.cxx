...
void cmFindCommon::RerootPaths(std::vector < std::string > &paths)
{
    ...
    for (std::vector < std::string >::iterator ri = roots.begin(); ri != roots.end(); ++ri)
    {
        cmSystemTools::ConvertToUnixSlashes(*ri);
    }

    const char *stagePrefix = this->Makefile->GetDefinition("CMAKE_STAGING_PREFIX");

    // Copy the original set of unrooted paths.
    std::vector < std::string > unrootedPaths = paths;
    paths.clear();

    for (std::vector < std::string >::const_iterator ri = roots.begin(); ri != roots.end(); ++ri)
    {
        for (std::vector < std::string >::const_iterator ui = unrootedPaths.begin(); ui != unrootedPaths.end(); ++ui)
        {
            // Place the unrooted path under the current root if it is not
            // already inside.  Skip the unrooted path if it is relative to
            // a user home directory or is empty.
            std::string rootedDir;
            if (cmSystemTools::IsSubDirectory(ui->c_str(), ri->c_str()) || (stagePrefix && cmSystemTools::IsSubDirectory(ui->c_str(), stagePrefix)))
            {
                rootedDir = *ui;
            }
...
//----------------------------------------------------------------------------
void cmFindCommon::FilterPaths(std::vector < std::string > &paths, const std::set < std::string > &ignore)
{
    // Now filter out anything that's in the ignore set.
    std::vector < std::string > unfiltered;
    unfiltered.swap(paths);

    for (std::vector < std::string >::iterator pi = unfiltered.begin(); pi != unfiltered.end(); ++pi)
    {
...
void cmFindCommon::GetIgnoredPaths(std::vector < std::string > &ignore)
{
    ...
    static const char *paths[] = { "CMAKE_SYSTEM_IGNORE_PATH", "CMAKE_IGNORE_PATH", 0 };
    ...
    for (std::vector < std::string >::iterator i = ignore.begin(); i != ignore.end(); ++i)
    {
...
//----------------------------------------------------------------------------
void cmFindCommon::AddUserPath(std::string const &p, std::vector < std::string > &paths)
{
...
//----------------------------------------------------------------------------
void cmFindCommon::AddPathsInternal(std::vector < std::string > const &in_paths, PathType pathType)
{
    for (std::vector < std::string >::const_iterator i = in_paths.begin(); i != in_paths.end(); ++i)
    {
...
//----------------------------------------------------------------------------
void cmFindCommon::AddPathInternal(std::string const &in_path, PathType pathType)
{
    if (in_path.empty())
    {
        return;
    ...
    std::string fullPath = cmSystemTools::CollapseFullPath(in_path.c_str(), relbase);
