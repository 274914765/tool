...
cmCursesStringWidget::cmCursesStringWidget(int width, int height, int left, int top):
...
bool cmCursesStringWidget::HandleInput(int &key, cmCursesMainForm * fm, WINDOW * w)
{
    int x, y;
    ...
    while (!this->Done)
    {
        ...
        else if (key == KEY_DOWN || key == ctrl('n') || key == KEY_UP || key == ctrl('p') || key == KEY_NPAGE || key == ctrl('d') || key == KEY_PPAGE || key == ctrl('u'))
        {
            this->InEdit = false;
            delete[]this->OriginalString;
            // trick to force forms to update the field buffer
