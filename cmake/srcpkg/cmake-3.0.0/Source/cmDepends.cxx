...
//----------------------------------------------------------------------------
cmDepends::cmDepends(cmLocalGenerator * lg, const char *targetDir):CompileDirectory(), LocalGenerator(lg), Verbose(false), FileComparison(0), TargetDirectory(targetDir), MaxPath(16384), Dependee(new char[MaxPath]), Depender(new char[MaxPath])
{
}
...
bool cmDepends::Write(std::ostream & makeDepends, std::ostream & internalDepends)
{
    ...
    for (std::vector < std::string >::iterator si = pairs.begin(); si != pairs.end();)
    {
        ...
        obj = this->LocalGenerator->Convert(obj.c_str(), cmLocalGenerator::HOME_OUTPUT, cmLocalGenerator::MAKEFILE);
        dependencies[obj].insert(src);
    }
    for (std::map < std::string, std::set < std::string > >::const_iterator it = dependencies.begin(); it != dependencies.end(); ++it)
    {

        // Write the dependencies for this pair.
        if (!this->WriteDependencies(it->second, it->first, makeDepends, internalDepends))
        {
            return false;
        }
    }
...
//----------------------------------------------------------------------------
bool cmDepends::Check(const char *makeFile, const char *internalFile, std::map < std::string, DependencyVector > &validDeps)
{
...
//----------------------------------------------------------------------------
bool cmDepends::WriteDependencies(const std::set < std::string > &, const std::string &, std::ostream &, std::ostream &)
{
...
//----------------------------------------------------------------------------
bool cmDepends::CheckDependencies(std::istream & internalDepends, const char *internalDependsFileName, std::map < std::string, DependencyVector > &validDeps)
{
    ...
    while (internalDepends.getline(this->Dependee, this->MaxPath))
    {
        ...
        if (!cmSystemTools::FileExists(dependee))
        {
            ...
            {
                cmOStringStream msg;
                msg << "Dependee \"" << dependee << "\" does not exist for depender \"" << depender << "\"." << std::endl;
                cmSystemTools::Stdout(msg.str().c_str());
            }
        }
        else
        {
            if (dependerExists)
            {
                ...
                int result = 0;
                if ((!this->FileComparison->FileTimeCompare(depender, dependee, &result) || result < 0))
                {
                    ...
                    {
                        cmOStringStream msg;
                        msg << "Dependee \"" << dependee << "\" is newer than depender \"" << depender << "\"." << std::endl;
                        cmSystemTools::Stdout(msg.str().c_str());
                    }
                }
            }
            else
            {
                ...
                int result = 0;
                if ((!this->FileComparison->FileTimeCompare(internalDependsFileName, dependee, &result) || result < 0))
                {
                    ...
                    {
                        cmOStringStream msg;
                        msg << "Dependee \"" << dependee << "\" is newer than depends file \"" << internalDependsFileName << "\"." << std::endl;
                        cmSystemTools::Stdout(msg.str().c_str());
                    }
                }
            }
