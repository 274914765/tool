...
void cmDependsJavaParserHelper::CurrentClass::AddFileNamesForPrinting(std::vector < cmStdString > *files, const char *prefix, const char *sep)
{
    ...
    for (it = this->NestedClasses->begin(); it != this->NestedClasses->end(); ++it)
    {
...
{
    std::vector < cmStdString >::iterator it;
    for (it = this->PackagesImport.begin(); it != this->PackagesImport.end(); it++)
    {
        if (*it == sclass)
        {
            return;
...
void cmDependsJavaParserHelper::CombineUnions(char **out, const char *in1, char **in2, const char *sep)
{
    size_t len = 1;
    if (in1)
    {
...
void cmDependsJavaParserHelper::AllocateParserType(cmDependsJavaParserHelper::ParserType * pt, const char *str, int len)
{
    pt->str = 0;
    if (len == 0)
    {
...
std::vector < cmStdString > cmDependsJavaParserHelper::GetFilesProduced()
{
    ...
    std::vector < CurrentClass >::iterator it;
    for (it = toplevel->NestedClasses->begin(); it != toplevel->NestedClasses->end(); ++it)
    {
...
void cmDependsJavaParserHelper::Error(const char *str)
{
    ...
    int cc;
    std::cerr << "String: [";
    for (cc = 0; cc < 30 && *(this->InputBuffer.c_str() + this->InputBufferPos + cc); cc++)
    {
