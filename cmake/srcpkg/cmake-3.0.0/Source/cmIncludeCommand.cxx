...
// cmIncludeCommand
bool cmIncludeCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 1 || args.size() > 4)
    {
        this->SetError("called with wrong number of arguments.  " "include() only takes one file.");
        return false;
    }
    bool optional = false;
    ...
    for (unsigned int i = 1; i < args.size(); i++)
    {
        ...
        else if (args[i] == "RESULT_VARIABLE")
        {
            ...
            {
                this->SetError("called with invalid arguments: " "only one result variable allowed");
                return false;
            }
            if (++i < args.size())
            {
    ...
    if (fname.empty())
    {
        this->Makefile->IssueMessage(cmake::AUTHOR_WARNING, "include() given empty file name (ignored).");
        return true;
    }
    ...
    std::string fname_abs = cmSystemTools::CollapseFullPath(fname.c_str(), this->Makefile->GetStartDirectory());
    ...
    {
        this->Makefile->AddDefinition(resultVarName.c_str(), readit ? fullFilePath.c_str() : "NOTFOUND");
    }
