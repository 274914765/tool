...
{
public:
    cmOrderDirectoriesConstraint(cmOrderDirectories * od, std::string const &file):OD(od), GlobalGenerator(od->GlobalGenerator)
    {
        this->FullPath = file;
        ...
        if (file.rfind(".framework") != std::string::npos)
        {
            ...
            if (splitFramework.find(file) && (std::string::npos != splitFramework.match(3).find(splitFramework.match(2))))
            {
                this->Directory = splitFramework.match(1);
                this->FileName = std::string(file.begin() + this->Directory.size() + 1, file.end());
            }
        }
...
//----------------------------------------------------------------------------
bool cmOrderDirectoriesConstraint::FileMayConflict(std::string const &dir, std::string const &name)
{
    ...
    std::set < cmStdString > const &files = (this->GlobalGenerator->GetDirectoryContent(dir, false));
...
{
public:
    cmOrderDirectoriesConstraintSOName(cmOrderDirectories * od, std::string const &file, const char *soname):cmOrderDirectoriesConstraint(od, file), SOName(soname ? soname : "")
    {
...
bool cmOrderDirectoriesConstraintSOName::FindConflict(std::string const &dir)
{
    ...
    else
    {
        ...
        // that may conflict.
        std::set < cmStdString > const &files = (this->GlobalGenerator->GetDirectoryContent(dir, true));
...
{
public:
    cmOrderDirectoriesConstraintLibrary(cmOrderDirectories * od, std::string const &file):cmOrderDirectoriesConstraint(od, file)
    {
    }
...
bool cmOrderDirectoriesConstraintLibrary::FindConflict(std::string const &dir)
{
    ...
    // might consider.
    if (!this->OD->LinkExtensions.empty() && this->OD->RemoveLibraryExtension.find(this->FileName))
    {
        cmStdString lib = this->OD->RemoveLibraryExtension.match(1);
        cmStdString ext = this->OD->RemoveLibraryExtension.match(2);
        for (std::vector < std::string >::iterator i = this->OD->LinkExtensions.begin(); i != this->OD->LinkExtensions.end(); ++i)
        {
            if (*i != ext)
            {
...
//----------------------------------------------------------------------------
cmOrderDirectories::cmOrderDirectories(cmGlobalGenerator * gg, cmTarget const *target, const char *purpose)
{
...
{
    for (std::vector < cmOrderDirectoriesConstraint * >::iterator i = this->ConstraintEntries.begin(); i != this->ConstraintEntries.end(); ++i)
    {
        delete *i;
    }
    for (std::vector < cmOrderDirectoriesConstraint * >::iterator i = this->ImplicitDirEntries.begin(); i != this->ImplicitDirEntries.end(); ++i)
    {
        delete *i;
    }
}
...
//----------------------------------------------------------------------------
void cmOrderDirectories::AddRuntimeLibrary(std::string const &fullPath, const char *soname)
{
    ...
    if (this->EmmittedConstraintSOName.insert(fullPath).second)
    {
        ...
        if (!this->ImplicitDirectories.empty())
        {
            ...
            if (this->ImplicitDirectories.find(dir) != this->ImplicitDirectories.end())
            {
                this->ImplicitDirEntries.push_back(new cmOrderDirectoriesConstraintSOName(this, fullPath, soname));
                return;
            }
        }

        // Construct the runtime information entry for this library.
        this->ConstraintEntries.push_back(new cmOrderDirectoriesConstraintSOName(this, fullPath, soname));
    }
    else
    {
...
void cmOrderDirectories::AddLinkLibrary(std::string const &fullPath)
{
    ...
    if (this->EmmittedConstraintLibrary.insert(fullPath).second)
    {
        ...
        if (!this->ImplicitDirectories.empty())
        {
            ...
            if (this->ImplicitDirectories.find(dir) != this->ImplicitDirectories.end())
            {
                this->ImplicitDirEntries.push_back(new cmOrderDirectoriesConstraintLibrary(this, fullPath));
                return;
            }
        }

        // Construct the link library entry.
        this->ConstraintEntries.push_back(new cmOrderDirectoriesConstraintLibrary(this, fullPath));
    }
}

//----------------------------------------------------------------------------
void cmOrderDirectories::AddUserDirectories(std::vector < std::string > const &extra)
{
    this->UserDirectories.insert(this->UserDirectories.end(), extra.begin(), extra.end());
}

//----------------------------------------------------------------------------
void cmOrderDirectories::AddLanguageDirectories(std::vector < std::string > const &dirs)
{
    this->LanguageDirectories.insert(this->LanguageDirectories.end(), dirs.begin(), dirs.end());
}

//----------------------------------------------------------------------------
void cmOrderDirectories::SetImplicitDirectories(std::set < cmStdString > const &implicitDirs)
{
    this->ImplicitDirectories = implicitDirs;
}

//----------------------------------------------------------------------------
void cmOrderDirectories::SetLinkExtensionInfo(std::vector < std::string > const &linkExtensions, std::string const &removeExtRegex)
{
...
int cmOrderDirectories::AddOriginalDirectory(std::string const &dir)
{
    ...
    std::map < cmStdString, int >::iterator i = this->DirectoryIndex.find(dir);
    if (i == this->DirectoryIndex.end())
    {
        std::map < cmStdString, int >::value_type entry(dir, static_cast < int >(this->OriginalDirectories.size()));
...
//----------------------------------------------------------------------------
void cmOrderDirectories::AddOriginalDirectories(std::vector < std::string > const &dirs)
{
    for (std::vector < std::string >::const_iterator di = dirs.begin(); di != dirs.end(); ++di)
    {
        ...
        if (this->ImplicitDirectories.find(*di) != this->ImplicitDirectories.end())
        {
            continue;
        }
...
void cmOrderDirectories::FindConflicts()
{
    ...
    for (std::vector < ConflictList >::iterator i = this->ConflictGraph.begin(); i != this->ConflictGraph.end(); ++i)
    {
        ...
        ConflictList::iterator last = std::unique(i->begin(), i->end(), cmOrderDirectoriesCompare());
        i->erase(last, i->end());
    }
...
void cmOrderDirectories::FindImplicitConflicts()
{
    ...
    w << "Cannot generate a safe " << this->Purpose
        << " for target " << this->Target->GetName() << " because files in some directories may conflict with " << " libraries in implicit directories:\n" << text << "Some of these libraries may not be found correctly.";
    this->GlobalGenerator->GetCMakeInstance()->IssueMessage(cmake::WARNING, w.str(), this->Target->GetBacktrace());
}
...
void cmOrderDirectories::DiagnoseCycle()
{
    ...
    cmOStringStream e;
    e << "Cannot generate a safe " << this->Purpose << " for target " << this->Target->GetName() << " because there is a cycle in the constraint graph:\n";
    ...
    this->GlobalGenerator->GetCMakeInstance()->IssueMessage(cmake::WARNING, e.str(), this->Target->GetBacktrace());
}
