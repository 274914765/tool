...
bool cmAuxSourceDirectoryCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    ...
    if (dir.Load(tdir.c_str()))
    {
        ...
        for (size_t i = 0; i < numfiles; ++i)
        {
            ...
            if (dotpos != std::string::npos)
            {
                ...
                if (base.size() != 0 && std::find(this->Makefile->GetSourceExtensions().begin(), this->Makefile->GetSourceExtensions().end(), ext) != this->Makefile->GetSourceExtensions().end())
                {
                    ...
                    // depends can be done
                    cmSourceFile *sf = this->Makefile->GetOrCreateSource(fullname.c_str());
