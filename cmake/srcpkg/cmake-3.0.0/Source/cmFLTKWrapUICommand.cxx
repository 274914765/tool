...
// cmFLTKWrapUICommand
bool cmFLTKWrapUICommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 2)
    {
    ...
    const char *fluid_exe = this->Makefile->GetRequiredDefinition("FLTK_FLUID_EXECUTABLE");
    ...
    for (std::vector < std::string >::iterator i = (newArgs.begin() + 1); i != newArgs.end(); i++)
    {
        ...
        if (!curr || !curr->GetPropertyAsBool("WRAP_EXCLUDE"))
        {
            ...
            this->Makefile->AddCustomCommandToOutput(cxxres.c_str(), depends, no_main_dependency, commandLines, no_comment, no_working_dir);
            this->Makefile->AddCustomCommandToOutput(hname.c_str(), depends, no_main_dependency, commandLines, no_comment, no_working_dir);
...
void cmFLTKWrapUICommand::FinalPass()
{
    ...
    if (!target)
    {
        std::string msg = "FLTK_WRAP_UI was called with a target that was never created: ";
        msg += this->Target;
    ...
    {
        if (srcs[i]->GetFullPath() == this->GeneratedSourcesClasses[0]->GetFullPath())
        {
            found = true;
            break;
        }
    ...
    if (!found)
    {
        std::string msg =
            ...
            "of sources to add to your target when you call ADD_LIBRARY or " "ADD_EXECUTABLE. For now CMake will add the sources to your target " "for you as was done in CMake 2.0 and earlier. In the future this may " "become an error.";
        msg += "The problem was found while processing the source directory: ";
        msg += this->Makefile->GetStartDirectory();
        cmSystemTools::Message(msg.c_str(), "Warning");
        ...
        for (size_t classNum = 0; classNum < lastHeadersClass; classNum++)
        {
            this->Makefile->GetTargets()[this->Target].AddSourceFile(this->GeneratedSourcesClasses[classNum]);
        }
    }
}
