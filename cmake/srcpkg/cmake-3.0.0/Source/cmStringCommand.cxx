...
//----------------------------------------------------------------------------
bool cmStringCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 1)
    {
...
//----------------------------------------------------------------------------
bool cmStringCommand::HandleToUpperLowerCommand(std::vector < std::string > const &args, bool toUpper)
{
    if (args.size() < 3)
    {
...
//----------------------------------------------------------------------------
bool cmStringCommand::HandleConfigureCommand(std::vector < std::string > const &args)
{
    if (args.size() < 2)
    {
...
bool cmStringCommand::HandleRegexCommand(std::vector < std::string > const &args)
{
    ...
    if (mode == "MATCH")
    {
        if (args.size() < 5)
        {
            this->SetError("sub-command REGEX, mode MATCH needs " "at least 5 arguments total to command.");
            return false;
        }
    ...
    else if (mode == "MATCHALL")
    {
        if (args.size() < 5)
        {
            this->SetError("sub-command REGEX, mode MATCHALL needs " "at least 5 arguments total to command.");
            return false;
        }
    ...
    else if (mode == "REPLACE")
    {
        if (args.size() < 6)
        {
            this->SetError("sub-command REGEX, mode REPLACE needs " "at least 6 arguments total to command.");
            return false;
        }
...
bool cmStringCommand::RegexReplace(std::vector < std::string > const &args)
{
    ...
    while (l < replace.length())
    {
        ...
        else
        {
            ...
            {
                this->SetError("sub-command REGEX, mode REPLACE: " "replace-expression ends in a backslash.");
                return false;
            }
    ...
    while (re.find(input.c_str() + base))
    {
        ...
        for (unsigned int i = 0; i < replacement.size(); ++i)
        {
            ...
            else
            {
                ...
                if ((start != std::string::npos) && (end != std::string::npos) && (start <= len) && (end <= len))
                {
                    output += input.substr(base + start, end - start);
                }
                else
                {
                    std::string e = "sub-command REGEX, mode REPLACE: replace expression \"" + replace + "\" contains an out-of-range escape for regex \"" + regex + "\".";
                    this->SetError(e.c_str());
                    return false;
                }
            }
...
bool cmStringCommand::HandleFindCommand(std::vector < std::string > const &args)
{
    ...
    if (outvar == "REVERSE")
    {
        this->SetError("sub-command FIND does not allow to select REVERSE as " "the output variable.  " "Maybe you missed the actual output variable?");
        return false;
    }
...
bool cmStringCommand::HandleCompareCommand(std::vector < std::string > const &args)
{
    ...
    std::string mode = args[1];
    if ((mode == "EQUAL") || (mode == "NOTEQUAL") || (mode == "LESS") || (mode == "GREATER"))
    {
        if (args.size() < 5)
        {
...
bool cmStringCommand::HandleSubstringCommand(std::vector < std::string > const &args)
{
    ...
    {
        cmOStringStream ostr;
        ostr << "end index: " << end << " is out of range -1 - " << leftOverLength;
        this->SetError(ostr.str().c_str());
        return false;
    }

    this->Makefile->AddDefinition(variableName.c_str(), stringValue.substr(begin, end).c_str());
    return true;
}
...
//----------------------------------------------------------------------------
bool cmStringCommand::HandleMakeCIdentifierCommand(std::vector < std::string > const &args)
{
    if (args.size() != 3)
    {
    ...
    this->Makefile->AddDefinition(variableName.c_str(), cmSystemTools::MakeCidentifier(input.c_str()).c_str());
    return true;
}
...
bool cmStringCommand::HandleStripCommand(std::vector < std::string > const &args)
{
    ...
    this->Makefile->AddDefinition(variableName.c_str(), stringValue.substr(startPos, outLength).c_str());
    return true;
}
...
bool cmStringCommand::HandleRandomCommand(std::vector < std::string > const &args)
{
    ...
    unsigned int seed = 0;
    int length = 5;
    const char cmStringCommandDefaultAlphabet[] = "qwertyuiopasdfghjklzxcvbnm" "QWERTYUIOPASDFGHJKLZXCVBNM" "0123456789";
    std::string alphabet;
...
//----------------------------------------------------------------------------
bool cmStringCommand::HandleTimestampCommand(std::vector < std::string > const &args)
{
    if (args.size() < 2)
    {
