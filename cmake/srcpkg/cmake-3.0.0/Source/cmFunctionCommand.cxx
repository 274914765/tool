...
bool cmFunctionHelperCommand::InvokeInitialPass(const std::vector < cmListFileArgument > &args, cmExecutionStatus & inStatus)
{
    ...
    {
        std::string errorMsg = "Function invoked with incorrect arguments for function named: ";
    ...
    for (unsigned int t = 0; t < expandedArgs.size(); ++t)
    {
        ...
        tmpStream << "ARGV" << t;
        this->Makefile->AddDefinition(tmpStream.str().c_str(), expandedArgs[t].c_str());
    ...
    {
        this->Makefile->AddDefinition(this->Args[j].c_str(), expandedArgs[j - 1].c_str());
    }
    ...
    for (unsigned int c = 0; c < this->Functions.size(); ++c)
    {
        ...
        if (!this->Makefile->ExecuteCommand(this->Functions[c], status) || status.GetNestedError())
        {
...
bool cmFunctionFunctionBlocker::IsFunctionBlocked(const cmListFileFunction & lff, cmMakefile & mf, cmExecutionStatus &)
{
    ...
    else if (!cmSystemTools::Strucmp(lff.Name.c_str(), "endfunction"))
    {
        ...
        if (!this->Depth)
        {
            ...
            for (unsigned int i = 0; i < f->Functions.size(); ++i)
            {
                ...
                {
                    f->Functions[i].Arguments[j].FilePath = f->Functions[i].FilePath.c_str();
                }
            }
...
bool cmFunctionFunctionBlocker::ShouldRemove(const cmListFileFunction & lff, cmMakefile & mf)
{
...
bool cmFunctionCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 1)
    {
