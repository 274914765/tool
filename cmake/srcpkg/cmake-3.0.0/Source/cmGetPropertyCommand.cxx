...
//----------------------------------------------------------------------------
bool cmGetPropertyCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 3)
    {
    ...
    {
        cmOStringStream e;
        e << "given invalid scope " << args[1] << ".  " << "Valid scopes are " << "GLOBAL, DIRECTORY, TARGET, SOURCE, TEST, VARIABLE, CACHE.";
    ...
    enum Doing
    { DoingNone, DoingName, DoingProperty, DoingType };
    ...
    if (this->InfoType == OutBriefDoc)
    {
        ...
        std::string output;
        if (cmPropertyDefinition * def = this->Makefile->GetCMakeInstance()->GetPropertyDefinition(this->PropertyName.c_str(), scope))
        {
    ...
    else if (this->InfoType == OutDefined)
    {
        ...
        if (this->Makefile->GetCMakeInstance()->GetPropertyDefinition(this->PropertyName.c_str(), scope))
        {
...
bool cmGetPropertyCommand::HandleTargetMode()
{
    ...
    if (cmTarget * target = this->Makefile->FindTargetToUse(this->Name))
    {
        return this->StoreResult(target->GetProperty(this->PropertyName.c_str(), this->Makefile));
    }
    else
    {
        cmOStringStream e;
        e << "could not find TARGET " << this->Name << ".  Perhaps it has not yet been created.";
        this->SetError(e.str().c_str());
        return false;
    }
}
...
bool cmGetPropertyCommand::HandleSourceMode()
{
    ...
    // Get the source file.
    if (cmSourceFile * sf = this->Makefile->GetOrCreateSource(this->Name.c_str()))
    {
        return this->StoreResult(sf->GetPropertyForUser(this->PropertyName.c_str()));
    }
    else
    {
        cmOStringStream e;
...
bool cmGetPropertyCommand::HandleVariableMode()
{
    ...
    return this->StoreResult(this->Makefile->GetDefinition(this->PropertyName.c_str()));
}
...
bool cmGetPropertyCommand::HandleCacheMode()
{
    ...
    const char *value = 0;
    cmCacheManager::CacheIterator it = this->Makefile->GetCacheManager()->GetCacheIterator(this->Name.c_str());
    if (!it.IsAtEnd())
    {
