...
//----------------------------------------------------------------------------
void cmGlobalKdevelopGenerator::GetDocumentation(cmDocumentationEntry & entry, const char *) const
{
    entry.Name = this->GetName();
    entry.Brief = "Generates KDevelop 3 project files.";
}

cmGlobalKdevelopGenerator::cmGlobalKdevelopGenerator():cmExternalMakefileProjectGenerator()
{
...
void cmGlobalKdevelopGenerator::Generate()
{
    ...
    for (std::map < cmStdString, std::vector < cmLocalGenerator * > >::const_iterator it = this->GlobalGenerator->GetProjectMap().begin(); it != this->GlobalGenerator->GetProjectMap().end(); ++it)
    {
        ...
        if (!this->CreateFilelistFile(lgs, outputDir, projectDir, projectName, cmakeFilePattern, fileToOpen))
        {
        ...
        this->CreateProjectFile(outputDir, projectDir, projectName, executable, cmakeFilePattern, fileToOpen);
    }
}

bool cmGlobalKdevelopGenerator::CreateFilelistFile(const std::vector < cmLocalGenerator * >&lgs,
                                                   const std::string & outputDir, const std::string & projectDirIn, const std::string & projectname, std::string & cmakeFilePattern, std::string & fileToOpen)
{
    ...
    for (std::vector < cmLocalGenerator * >::const_iterator it = lgs.begin(); it != lgs.end(); it++)
    {
        ...
        for (cmTargets::iterator ti = targets.begin(); ti != targets.end(); ti++)
        {
            ...
            for (std::vector < cmSourceFile * >::const_iterator si = sources.begin(); si != sources.end(); si++)
            {
                ...
                if ((tmp[0] != '/') && (strstr(tmp.c_str(), cmake::GetCMakeFilesDirectoryPostSlash()) == 0) && (cmSystemTools::GetFilenameExtension(tmp) != ".moc"))
                {
                    files.insert(tmp);

                    // check if there's a matching header around
                    for (std::vector < std::string >::const_iterator ext = makefile->GetHeaderExtensions().begin(); ext != makefile->GetHeaderExtensions().end(); ++ext)
                    {
    ...
    for (std::set < cmStdString >::const_iterator it = files.begin(); it != files.end(); it++)
    {
        ...
        if (fileToOpen.empty())
        {
            ...
            if ((ext == ".c") || (ext == ".cc") || (ext == ".cpp") || (ext == ".cxx") || (ext == ".C") || (ext == ".h") || (ext == ".hpp"))
            {
                fileToOpen = tmp;
            }
        }
...
void cmGlobalKdevelopGenerator::CreateProjectFile(const std::string & outputDir, const std::string & projectDir, const std::string & projectname, const std::string & executable, const std::string & cmakeFilePattern, const std::string & fileToOpen)
{
    ...
    {
        this->MergeProjectFiles(outputDir, projectDir, filename, executable, cmakeFilePattern, fileToOpen, sessionFilename);
    }
    else
    {
        ...
        if (d.Load(projectDir.c_str()))
        {
            ...
            for (unsigned int i = 0; i < numf; i++)
            {
                ...
                if ((nextFile != ".") && (nextFile != ".."))
                {
                    ...
                    {
                        tmp += "/CMakeCache.txt";
                        if ((nextFile == "CMakeFiles") || (cmSystemTools::FileExists(tmp.c_str())))
                        {
...
void cmGlobalKdevelopGenerator::MergeProjectFiles(const std::string & outputDir,
                                                  const std::string & projectDir, const std::string & filename, const std::string & executable, const std::string & cmakeFilePattern, const std::string & fileToOpen, const std::string & sessionFilename)
{
    ...
    for (std::vector < std::string >::const_iterator it = lines.begin(); it != lines.end(); it++)
    {
        ...
        if ((strstr(line, "<projectdirectory>") != 0)
            || (strstr(line, "<projectmanagement>") != 0) || (strstr(line, "<absoluteprojectpath>") != 0) || (strstr(line, "<filelistdirectory>") != 0) || (strstr(line, "<buildtool>") != 0) || (strstr(line, "<builddir>") != 0))
        {
            continue;
        }
...
void cmGlobalKdevelopGenerator::CreateNewProjectFile(const std::string & outputDir,
                                                     const std::string & projectDir,
                                                     const std::string & filename, const std::string & executable, const std::string & cmakeFilePattern, const std::string & fileToOpen, const std::string & sessionFilename)
{
    cmGeneratedFileStream fout(filename.c_str());
    if (!fout)
    {
    ...
    bool enableCxx = (this->GlobalGenerator->GetLanguageEnabled("C") || this->GlobalGenerator->GetLanguageEnabled("CXX"));
    ...
    fout << "    </build>\n"
        ...
        "      <makebin>" << this->GlobalGenerator->GetLocalGenerators()[0]->GetMakefile()->GetRequiredDefinition("CMAKE_MAKE_PROGRAM")
        << " </makebin>\n"
        "      <selectedenvironment>default</selectedenvironment>\n"
        "      <environments>\n" "        <default>\n" "          <envvar value=\"1\" name=\"VERBOSE\" />\n" "          <envvar value=\"1\" name=\"CMAKE_NO_VERBOSE\" />\n" "        </default>\n" "      </environments>\n" "    </make>\n";

    fout << "    <blacklist>\n";
    for (std::vector < std::string >::const_iterator dirIt = this->Blacklist.begin(); dirIt != this->Blacklist.end(); ++dirIt)
    {
