...
{
public:
    cmLocalVisualStudio7GeneratorInternals(cmLocalVisualStudio7Generator * e):LocalGenerator(e)
    {
    }
...
void cmLocalVisualStudio7Generator::AddHelperCommands()
{
    ...
    cmGlobalVisualStudio7Generator *gg = static_cast < cmGlobalVisualStudio7Generator * >(this->GlobalGenerator);
    for (cmTargets::iterator l = tgts.begin(); l != tgts.end(); l++)
    {
        ...
        if (path)
        {
            this->ReadAndStoreExternalGUID(l->second.GetName(), path);
        }
        else
        {
...
void cmLocalVisualStudio7Generator::FixGlobalTargets()
{
    ...
    for (cmTargets::iterator l = tgts.begin(); l != tgts.end(); l++)
    {
        ...
        if (tgt.GetType() == cmTarget::GLOBAL_TARGET)
        {
            ...
            force += tgt.GetName();
            force += "_force";
            if (cmSourceFile * file = this->Makefile->AddCustomCommandToOutput(force.c_str(), no_depends, no_main_dependency, force_commands, " ", 0, true))
            {
...
void cmLocalVisualStudio7Generator::WriteStampFiles()
{
    ...
    for (std::vector < std::string >::const_iterator lf = listFiles.begin(); lf != listFiles.end(); ++lf)
    {
        depFile << *lf << std::endl;
    }
}
...
{
    this->FortranProject = static_cast < cmGlobalVisualStudioGenerator * >(this->GlobalGenerator)->TargetIsFortranOnly(target);
    this->WindowsCEProject = static_cast < cmGlobalVisualStudioGenerator * >(this->GlobalGenerator)->TargetsWindowsCE();
...
cmSourceFile *cmLocalVisualStudio7Generator::CreateVCProjBuildRule()
{
    ...
    std::string fullpathStampName = this->Convert(stampName.c_str(), FULL, UNCHANGED);
    this->Makefile->AddCustomCommandToOutput(fullpathStampName.c_str(), listFiles, makefileIn.c_str(), commandLines, comment.c_str(), no_working_directory, true);
...
void cmLocalVisualStudio7Generator::WriteConfigurations(std::ostream & fout, const char *libName, cmTarget & target)
{
...
{
public:
    EventWriter(cmLocalVisualStudio7Generator * lg, const char *config, std::ostream & os):LG(lg), Config(config), Stream(os), First(true)
    {
    }
...
//----------------------------------------------------------------------------
void cmLocalVisualStudio7Generator::WriteConfiguration(std::ostream & fout, const char *configName, const char *libName, cmTarget & target)
{
    ...
    {
        const char *linkLanguage = (this->FortranProject ? "Fortran" : target.GetLinkerLanguage(configName));
        if (!linkLanguage)
        {
    ...
    Options targetOptions(this, t, table, this->ExtraFlagTable);
    ...
    targetOptions.SetVerboseMakefile(this->Makefile->IsOn("CMAKE_VERBOSE_MAKEFILE"));
    ...
    if (this->FortranProject)
    {
        const char *target_mod_dir = target.GetProperty("Fortran_MODULE_DIRECTORY");
        std::string modDir;
        if (target_mod_dir)
        {
            modDir = this->Convert(target_mod_dir, cmLocalGenerator::START_OUTPUT, cmLocalGenerator::UNCHANGED);
        }
        else
        {
            modDir = ".";
        }
        fout << "\t\t\t\tModulePath=\"" << this->ConvertToXMLOutputPath(modDir.c_str()) << "\\$(ConfigurationName)\"\n";
    }
...
//----------------------------------------------------------------------------
std::string cmLocalVisualStudio7Generator::GetBuildTypeLinkerFlags(std::string rootLinkerFlags, const char *configName)
{
    std::string configTypeUpper = cmSystemTools::UpperCase(configName);
    std::string extraLinkOptionsBuildTypeDef = rootLinkerFlags + "_" + configTypeUpper;

    std::string extraLinkOptionsBuildType = this->Makefile->GetRequiredDefinition(extraLinkOptionsBuildTypeDef.c_str());

    return extraLinkOptionsBuildType;
}

void cmLocalVisualStudio7Generator::OutputBuildTool(std::ostream & fout, const char *configName, cmTarget & target, const Options & targetOptions)
{
    cmGlobalVisualStudio7Generator *gg = static_cast < cmGlobalVisualStudio7Generator * >(this->GlobalGenerator);
    std::string temp;
    ...
    {
        extraLinkOptions = this->Makefile->GetRequiredDefinition("CMAKE_EXE_LINKER_FLAGS") + std::string(" ") + GetBuildTypeLinkerFlags("CMAKE_EXE_LINKER_FLAGS", configName);
    }
    if (target.GetType() == cmTarget::SHARED_LIBRARY)
    {
        extraLinkOptions = this->Makefile->GetRequiredDefinition("CMAKE_SHARED_LINKER_FLAGS") + std::string(" ") + GetBuildTypeLinkerFlags("CMAKE_SHARED_LINKER_FLAGS", configName);
    }
    if (target.GetType() == cmTarget::MODULE_LIBRARY)
    {
        extraLinkOptions = this->Makefile->GetRequiredDefinition("CMAKE_MODULE_LINKER_FLAGS") + std::string(" ") + GetBuildTypeLinkerFlags("CMAKE_MODULE_LINKER_FLAGS", configName);
    }
    ...
    Options linkOptions(this, Options::Linker, cmLocalVisualStudio7GeneratorLinkFlagTable);
    linkOptions.Parse(extraLinkOptions.c_str());
    if (!this->ModuleDefinitionFile.empty())
    {
        std::string defFile = this->ConvertToXMLOutputPath(this->ModuleDefinitionFile.c_str());
        ...
        case cmTarget::OBJECT_LIBRARY:
            {
                ...
                libpath += ".lib";
                const char *tool = this->FortranProject ? "VFLibrarianTool" : "VCLibrarianTool";
                fout << "\t\t\t<Tool\n" << "\t\t\t\tName=\"" << tool << "\"\n";
        ...
        case cmTarget::MODULE_LIBRARY:
            {
                ...
                target.GetLibraryNames(targetName, targetNameSO, targetNameFull, targetNameImport, targetNamePDB, configName);
                ...
                fout << "\t\t\t\tAdditionalDependencies=\"$(NOINHERIT) " << this->Makefile->GetSafeDefinition(standardLibsVar.c_str());
                if (this->GetVersion() < VS8)
                {
                ...
                temp += "/";
                temp += targetNamePDB;
                fout << "\t\t\t\tProgramDatabaseFile=\"" << this->ConvertToXMLOutputPathSingle(temp.c_str()) << "\"\n";
        ...
        case cmTarget::EXECUTABLE:
            {
                ...
                std::string targetNameImport;
                std::string targetNamePDB;
                target.GetExecutableNames(targetName, targetNameFull, targetNameImport, targetNamePDB, configName);
                ...
                this->OutputLibraryDirectories(fout, cli.GetDirectories());
                fout << "\"\n";
                std::string path = this->ConvertToXMLOutputPathSingle(target.GetPDBDirectory(configName).c_str());
                ...
                if (this->WindowsCEProject)
                {
                    ...
                    {
                        const char *entryPointSymbol = targetOptions.UsingUnicode()? (isWin32Executable ? "wWinMainCRTStartup" : "mainWCRTStartup") : (isWin32Executable ? "WinMainCRTStartup" : "mainACRTStartup");
                        fout << "\t\t\t\tEntryPointSymbol=\"" << entryPointSymbol << "\"\n";
                    }
                }
                else if (this->FortranProject)
                {
                    fout << "\t\t\t\tSubSystem=\"" << (isWin32Executable ? "subSystemWindows" : "subSystemConsole") << "\"\n";
                }
                else
                {
...
//----------------------------------------------------------------------------
void cmLocalVisualStudio7Generator::WriteTargetVersionAttribute(std::ostream & fout, cmTarget & target)
{
    int major;
    int minor;
...
//----------------------------------------------------------------------------
void cmLocalVisualStudio7GeneratorInternals::OutputLibraries(std::ostream & fout, ItemVector const &libs)
{
    ...
    for (ItemVector::const_iterator l = libs.begin(); l != libs.end(); ++l)
    {
        if (l->IsPath)
        {
            std::string rel = lg->Convert(l->Value.c_str(), cmLocalGenerator::START_OUTPUT, cmLocalGenerator::UNCHANGED);
...
//----------------------------------------------------------------------------
void cmLocalVisualStudio7GeneratorInternals::OutputObjects(std::ostream & fout, cmTarget * t, const char *isep)
{
    // VS < 8 does not support per-config source locations so we
    // list object library content on the link line instead.
    cmLocalVisualStudio7Generator *lg = this->LocalGenerator;
    cmGeneratorTarget *gt = lg->GetGlobalGenerator()->GetGeneratorTarget(t);
    std::vector < std::string > objs;
    gt->UseObjectLibraries(objs);
    const char *sep = isep ? isep : "";
    for (std::vector < std::string >::const_iterator oi = objs.begin(); oi != objs.end(); ++oi)
    {
        std::string rel = lg->Convert(oi->c_str(), cmLocalGenerator::START_OUTPUT, cmLocalGenerator::UNCHANGED);
...
//----------------------------------------------------------------------------
void cmLocalVisualStudio7Generator::OutputLibraryDirectories(std::ostream & fout, std::vector < std::string > const &dirs)
{
    const char *comma = "";
    for (std::vector < std::string >::const_iterator d = dirs.begin(); d != dirs.end(); ++d)
    {
        ...
        // original directory.
        fout << comma << this->ConvertToXMLOutputPath((dir + "/$(OutDir)").c_str()) << "," << this->ConvertToXMLOutputPath(dir.c_str());
        comma = ",";
    }
}

void cmLocalVisualStudio7Generator::WriteVCProjFile(std::ostream & fout, const char *libName, cmTarget & target)
{
...
{
public:
    cmLocalVisualStudio7GeneratorFCInfo(cmLocalVisualStudio7Generator * lg, cmTarget & target, cmSourceFile const &sf, std::vector < std::string > *configs);
    std::map < cmStdString, cmLVS7GFileConfig > FileConfigMap;
};

cmLocalVisualStudio7GeneratorFCInfo::cmLocalVisualStudio7GeneratorFCInfo(cmLocalVisualStudio7Generator * lg, cmTarget & target, cmSourceFile const &sf, std::vector < std::string > *configs)
{
    cmGeneratorTarget *gt = lg->GetGlobalGenerator()->GetGeneratorTarget(&target);
    std::string objectName;
    ...
    for (std::vector < std::string >::iterator i = configs->begin(); i != configs->end(); ++i)
    {
        ...
        if (const char *deps = sf.GetProperty("OBJECT_DEPENDS"))
        {
            ...
            const char *sep = "";
            for (std::vector < std::string >::iterator j = depends.begin(); j != depends.end(); ++j)
            {
        ...
        const char *lang = lg->GlobalGenerator->GetLanguageFromExtension(sf.GetExtension().c_str());
...
//----------------------------------------------------------------------------
std::string cmLocalVisualStudio7Generator::ComputeLongestObjectDirectory(cmTarget & target) const
{
    std::vector < std::string > *configs = static_cast < cmGlobalVisualStudio7Generator * >(this->GlobalGenerator)->GetConfigurations();
...
bool cmLocalVisualStudio7Generator::WriteGroup(const cmSourceGroup * sg, cmTarget & target, std::ostream & fout, const char *libName, std::vector < std::string > *configs)
{
    ...
    for (std::vector < const cmSourceFile * >::const_iterator sf = sourceFiles.begin(); sf != sourceFiles.end(); ++sf)
    {
        ...
        if (source != libName || target.GetType() == cmTarget::UTILITY || target.GetType() == cmTarget::GLOBAL_TARGET)
        {
            fout << "\t\t\t<File\n";
            ...
            else if (!fcinfo.FileConfigMap.empty())
            {
                ...
                for (std::map < cmStdString, cmLVS7GFileConfig >::const_iterator fci = fcinfo.FileConfigMap.begin(); fci != fcinfo.FileConfigMap.end(); ++fci)
                {
                    ...
                    fout << ">\n";
                    fout << "\t\t\t\t\t<Tool\n" << "\t\t\t\t\tName=\"" << aCompilerTool << "\"\n";
                    if (!fc.CompileFlags.empty() || !fc.CompileDefs.empty() || !fc.CompileDefsConfig.empty())
                    {
                        ...
                        Options fileOptions(this, tool, table, this->ExtraFlagTable);
                    ...
                    {
                        fout << "\t\t\t\t\tAdditionalDependencies=\"" << fc.AdditionalDeps.c_str() << "\"\n";
                    }
                    if (!fc.ObjectName.empty())
                    {
                        fout << "\t\t\t\t\tObjectFile=\"$(IntDir)/" << fc.ObjectName.c_str() << "\"\n";
                    }
                    fout << "\t\t\t\t\t/>\n" << "\t\t\t\t</FileConfiguration>\n";
                }
            }
...
void cmLocalVisualStudio7Generator:: WriteCustomRule(std::ostream & fout, const char *source, const cmCustomCommand & command, FCInfo & fcinfo)
{
...
void cmLocalVisualStudio7Generator::WriteVCProjBeginGroup(std::ostream & fout, const char *group, const char *)
{
    fout << "\t\t<Filter\n" << "\t\t\tName=\"" << group << "\"\n" << "\t\t\tFilter=\"\">\n";
}
...
void cmLocalVisualStudio7Generator::OutputTargetRules(std::ostream & fout, const char *configName, cmTarget & target, const char * /*libName */ )
{
    ...
    // Add pre-build event.
    const char *tool = this->FortranProject ? "VFPreBuildEventTool" : "VCPreBuildEventTool";
    event.Start(tool);
    ...
    cmsys::auto_ptr < cmCustomCommand > pcc(this->MaybeCreateImplibDir(target, configName, this->FortranProject));
    if (pcc.get())
    {
        event.Write(*pcc);
    }
...
void cmLocalVisualStudio7Generator::WriteProjectSCC(std::ostream & fout, cmTarget & target)
{
...
void cmLocalVisualStudio7Generator::WriteProjectStartFortran(std::ostream & fout, const char *libName, cmTarget & target)
{
...
void cmLocalVisualStudio7Generator::WriteProjectStart(std::ostream & fout, const char *libName, cmTarget & target, std::vector < cmSourceGroup > &)
{
    if (this->FortranProject)
    {
    ...
    this->WriteProjectSCC(fout, target);
    if (const char *targetFrameworkVersion = target.GetProperty("VS_DOTNET_TARGET_FRAMEWORK_VERSION"))
    {
...
void cmLocalVisualStudio7Generator::ReadAndStoreExternalGUID(const char *name, const char *path)
{
    ...
    this->GlobalGenerator->GetCMakeInstance()->AddCacheEntry(guidStoreName.c_str(), parser.GUID.c_str(), "Stored GUID", cmCacheManager::INTERNAL);
}

//----------------------------------------------------------------------------
std::string cmLocalVisualStudio7Generator::GetTargetDirectory(cmTarget const &target) const
{
    std::string dir;
    dir += target.GetName();
    dir += ".dir";
...
static bool cmLVS6G_IsFAT(const char *dir)
{
    if (dir[0] && dir[1] == ':')
    {
        ...
        volRoot[0] = dir[0];
        char fsName[16];
        if (GetVolumeInformationA(volRoot, 0, 0, 0, 0, 0, fsName, 16) && strstr(fsName, "FAT") != 0)
        {
            return true;
        }
    }
