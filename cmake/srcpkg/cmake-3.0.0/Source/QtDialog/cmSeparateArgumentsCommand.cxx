...
bool cmSeparateArgumentsCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.empty())
    {
    ...
    std::string command;
    enum Mode
    { ModeOld, ModeUnix, ModeWindows };
    Mode mode = ModeOld;
    enum Doing
    { DoingNone, DoingVariable, DoingMode, DoingCommand };
    ...
    else
    {
        ...
        std::string value;
        const char *sep = "";
        for (std::vector < std::string >::const_iterator vi = vec.begin(); vi != vec.end(); ++vi)
        {
            ...
            // Preserve semicolons.
            for (std::string::const_iterator si = vi->begin(); si != vi->end(); ++si)
            {
                if (*si == ';')
                {
                    value += '\\';
