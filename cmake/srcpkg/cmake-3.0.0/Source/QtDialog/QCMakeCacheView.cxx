...
{
public:
    QCMakeAdvancedFilter(QObject * o):QSortFilterProxyModel(o), ShowAdvanced(false)
    {
    }
...
QCMakeCacheView::QCMakeCacheView(QWidget * p):QTreeView(p)
{
...
QModelIndex QCMakeCacheView::moveCursor(CursorAction act, Qt::KeyboardModifiers mod)
{
...
QCMakeCacheModel::QCMakeCacheModel(QObject * p):QStandardItemModel(p), EditEnabled(true), NewPropertyCount(0), View(FlatView)
{
...
void QCMakeCacheModel::setProperties(const QCMakePropertyList & props)
{
    ...
    else if (this->View == GroupView)
    {
        ...
        foreach(QString key, newPropsTree.keys())
        {
            ...
            parentItems.append(new QStandardItem(key.isEmpty()? tr("Ungrouped Entries") : key));
        ...
        foreach(QString key, newPropsTree2.keys())
        {
            ...
            QStandardItem *parentItem = new QStandardItem(key.isEmpty()? tr("Ungrouped Entries") : key);
...
void QCMakeCacheModel::setPropertyData(const QModelIndex & idx1, const QCMakeProperty & prop, bool isNew)
{
...
void QCMakeCacheModel::getPropertyData(const QModelIndex & idx1, QCMakeProperty & prop) const
{
    QModelIndex idx2 = idx1.sibling(idx1.row(), 1);
...
void QCMakeCacheModel::breakProperties(const QSet < QCMakeProperty > &props, QMap < QString, QCMakePropertyList > &result)
{
...
bool QCMakeCacheModel::insertProperty(QCMakeProperty::PropertyType t, const QString & name, const QString & description, const QVariant & value, bool advanced)
{
    QCMakeProperty prop;
    prop.Key = name;
    prop.Value = value;
...
{
    if (!this->hasChildren(idx) && this->data(idx, TypeRole).toInt() != QCMakeProperty::BOOL)
    {
...
QCMakeCacheModelDelegate::QCMakeCacheModelDelegate(QObject * p):QItemDelegate(p), FileDialogFlag(false)
{
}
...
QWidget *QCMakeCacheModelDelegate::createEditor(QWidget * p, const QStyleOptionViewItem &, const QModelIndex & idx) const
{
    ...
    {
        QCMakePathEditor *editor = new QCMakePathEditor(p,
                                                        var.data(Qt::DisplayRole).toString());
        QObject::connect(editor, SIGNAL(fileDialogExists(bool)), this, SLOT(setFileDialogFlag(bool)));
        return editor;
    }
    else if (type == QCMakeProperty::FILEPATH)
    {
        QCMakeFilePathEditor *editor = new QCMakeFilePathEditor(p,
                                                                var.data(Qt::DisplayRole).toString());
        QObject::connect(editor, SIGNAL(fileDialogExists(bool)), this, SLOT(setFileDialogFlag(bool)));
        return editor;
    }
    else if (type == QCMakeProperty::STRING && var.data(QCMakeCacheModel::StringsRole).isValid())
    {
        QCMakeComboBox *editor = new QCMakeComboBox(p, var.data(QCMakeCacheModel::StringsRole).toStringList());
...
bool QCMakeCacheModelDelegate::editorEvent(QEvent * e, QAbstractItemModel * model, const QStyleOptionViewItem & option, const QModelIndex & index)
{
    ...
    {
        if (static_cast < QKeyEvent * >(e)->key() != Qt::Key_Space && static_cast < QKeyEvent * >(e)->key() != Qt::Key_Select)
        {
            return false;
        }
    }
...
void QCMakeCacheModelDelegate::setModelData(QWidget * editor, QAbstractItemModel * model, const QModelIndex & index) const
{
