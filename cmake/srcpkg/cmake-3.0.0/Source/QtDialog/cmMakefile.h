...
public:
   ...
   */
    bool ReadListFile(const char *listfile, const char *external = 0, std::string * fullPath = 0, bool noPolicyScope = true);
   ...
   */
      cmsys::auto_ptr < cmFunctionBlocker > RemoveFunctionBlocker(cmFunctionBlocker * fb, const cmListFileFunction & lff);
...
private:
   ...
   */
    int TryCompile(const char *srcdir, const char *bindir, const char *projectName, const char *targetName, bool fast, const std::vector < std::string > *cmakeArgs, std::string * output);
   ...
   */
    bool EnforceUniqueName(std::string const &name, std::string & msg, bool isCustom = false) const;
  ...
  /** Add a custom command to the build.  */
    void AddCustomCommandToTarget(const char *target,
                                  const std::vector < std::string > &depends, const cmCustomCommandLines & commandLines, cmTarget::CustomCommandType type, const char *comment, const char *workingDir, bool escapeOldStyle = true) const;
    cmSourceFile *AddCustomCommandToOutput(const std::vector < std::string > &outputs,
                                           const std::vector < std::string > &depends,
                                           const char *main_dependency, const cmCustomCommandLines & commandLines, const char *comment, const char *workingDir, bool replace = false, bool escapeOldStyle = true);
    cmSourceFile *AddCustomCommandToOutput(const char *output,
                                           const std::vector < std::string > &depends,
                                           const char *main_dependency, const cmCustomCommandLines & commandLines, const char *comment, const char *workingDir, bool replace = false, bool escapeOldStyle = true);
    void AddCustomCommandOldStyle(const char *target, const std::vector < std::string > &outputs, const std::vector < std::string > &depends, const char *source, const cmCustomCommandLines & commandLines, const char *comment);
  ...
  /** Create a new imported target with the name and type given.  */
    cmTarget *AddImportedTarget(const char *name, cmTarget::TargetType type, bool global);
   ...
   */
    cmTarget *AddExecutable(const char *exename, const std::vector < std::string > &srcs, bool excludeFromAll = false);
   ...
   */
    void AddUtilityCommand(const char *utilityName, bool excludeFromAll,
                           const std::vector < std::string > &depends, const char *workingDirectory, const char *command, const char *arg1 = 0, const char *arg2 = 0, const char *arg3 = 0, const char *arg4 = 0);
    cmTarget *AddUtilityCommand(const char *utilityName, bool excludeFromAll, const char *workingDirectory, const std::vector < std::string > &depends, const cmCustomCommandLines & commandLines, bool escapeOldStyle = true, const char *comment = 0);
   ...
   */
    ...
    void AddLinkLibraryForTarget(const char *tgt, const char *, cmTarget::LinkLibraryType type);
   ...
   */
    void AddSubDirectory(const char *, bool excludeFromAll = false, bool preorder = false);
    void AddSubDirectory(const char *fullSrcDir, const char *fullBinDir, bool excludeFromAll, bool preorder, bool immediate);
   ...
   */
    void AddIncludeDirectories(const std::vector < std::string > &incs, bool before = false);
   ...
   */
    ...
    void AddCacheDefinition(const char *name, const char *value, const char *doc, cmCacheManager::CacheEntryType type, bool force = false);
  ...
  /** Get the configurations to be generated.  */
    const char *GetConfigurations(std::vector < std::string > &configs, bool single = true) const;

  /**
   * Set the name of the library.
   */
    cmTarget *AddLibrary(const char *libname, cmTarget::TargetType type, const std::vector < std::string > &srcs, bool excludeFromAll = false);
...
#    endif
  ...
  /** Helper class to push and pop policies automatically.  */
    ...
    {
public:
        PolicyPushPop(cmMakefile * m, bool weak = false, cmPolicies::PolicyMap const &pm = cmPolicies::PolicyMap());
         ~PolicyPushPop();
        void Quiet()
        {
            this->ReportError = false;
...
private:
   ...
   */
    void MakeStartDirectoriesCurrent()
    {
        ...
        this->AddDefinition("CMAKE_CURRENT_BINARY_DIR", this->StartOutputDirectory.c_str());
    }
   ...
   */
    void SetStartDirectory(const char *dir)
    {
        ...
        this->cmStartDirectory = cmSystemTools::CollapseFullPath(this->cmStartDirectory.c_str());
        this->AddDefinition("CMAKE_CURRENT_SOURCE_DIR", this->cmStartDirectory.c_str());
    }
    ...
    void SetStartOutputDirectory(const char *lib)
    {
        ...
        this->StartOutputDirectory = cmSystemTools::CollapseFullPath(this->StartOutputDirectory.c_str());
   ...
   */
    const cmTargets & GetTargets() const
    {
        return this->Targets;
    }
  ...
  /** Find a target to use in place of the given name.  The target
    ...
    cmTarget *FindTargetToUse(const std::string & name, bool excludeAliases = false) const;
   ...
   */
    void ExpandSourceListArguments(std::vector < std::string > const &argsIn, std::vector < std::string > &argsOut, unsigned int startArgumentIndex) const;
   ...
   */
    cmSourceFile *GetOrCreateSource(const char *sourceName, bool generated = false);
...
#    endif
   ...
   */
    ...
    const char *ExpandVariablesInString(std::string & source, bool escapeQuotes, bool noEscapes, bool atOnly = false, const char *filename = 0, long line = -1, bool removeEmpty = false, bool replaceAt = true) const;
   ...
   */
    void ConfigureString(const std::string & input, std::string & output, bool atOnly, bool escapeQuotes) const;

  /**
   * Copy file but change lines acording to ConfigureString
   */
    int ConfigureFile(const char *infile, const char *outfile, bool copyonly, bool atOnly, bool escapeQuotes, const cmNewLineStyle & = cmNewLineStyle());

#    if defined(CMAKE_BUILD_WITH_CMAKE)
  /**
   * find what source group this source is in
   */
    cmSourceGroup *FindSourceGroup(const char *source, std::vector < cmSourceGroup > &groups) const;
#    endif
...
#    endif
   ...
   */
    bool ExpandArguments(std::vector < cmListFileArgument > const &inArgs, std::vector < std::string > &outArgs) const;
  /**
   * Get the instance
   */
   ...
   */
    ...
    std::vector < cmSourceFile * >&GetSourceFiles()
    {
        return this->SourceFiles;
    }
  ...
  /** Helper class to push and pop scopes automatically.  */
    ...
    {
public:
        ScopePushPop(cmMakefile * m):Makefile(m)
        {
...
private:
  ...
  /** Set whether or not to report a CMP0000 violation.  */
    ...
    bool IsGeneratingBuildSystem() const
    {
        return this->GeneratingBuildSystem;
    }
    void SetGeneratingBuildSystem()
    {
        this->GeneratingBuildSystem = true;
    }
...
#    endif
    ...
    bool IsFunctionBlocked(const cmListFileFunction & lff, cmExecutionStatus & status);
...
private:
    ...
    void PushPolicy(bool weak = false, cmPolicies::PolicyMap const &pm = cmPolicies::PolicyMap());
    void PopPolicy();
...
PolicyStackEntry(derived const &d, bool w = false):derived(d), Weak(w)
    ...
    cmPolicies::PolicyStatus GetPolicyStatusInternal(cmPolicies::PolicyID id) const;
...
#    endif
    ...
    void UpdateOutputToSourceMap(std::vector < std::string > const &outputs, cmSourceFile * source);
...
{
public:
    cmMakefileCall(cmMakefile * mf, cmListFileContext const &lfc, cmExecutionStatus & status):Makefile(mf)
    {
        cmMakefile::CallStackEntry entry =
        {
        &lfc, &status};
