...
public:
   ...
   */
    static void ExpandList(std::vector < std::string > const &argsIn, std::vector < std::string > &argsOut);
    static void ExpandListArgument(const std::string & arg, std::vector < std::string > &argsOut, bool emptyArgs = false);

  /**
   * Look for and replace registry values in a string
   */
    static void ExpandRegistryValues(std::string & source, KeyWOW64 view = KeyWOW64_Default);
   ...
   */
    static void Error(const char *m, const char *m2 = 0, const char *m3 = 0, const char *m4 = 0);
   ...
   */
    ...
    {
        return cmSystemTools::s_ErrorOccured || cmSystemTools::s_FatalErrorOccured || GetInterruptFlag();
    }
   ...
   */
    ...
    static bool DoesFileExistWithExtensions(const char *name, const std::vector < std::string > &sourceExts);
   ...
   */
    static std::string FileExistsInParentDirectories(const char *fname, const char *directory, const char *toplevel);

    static void Glob(const char *directory, const char *regexp, std::vector < std::string > &files);
   ...
   */
    static bool SimpleGlob(const cmStdString & glob, std::vector < cmStdString > &files, int type = 0);
   ...
   */
    ...
    static bool RunSingleCommand(const char *command, std::string * output = 0, int *retVal = 0, const char *dir = 0, OutputOption outputflag = OUTPUT_MERGE, double timeout = 0.0);
  /**
   ...
   */
    static bool RunSingleCommand(std::vector < std::string > const &command, std::string * output = 0, int *retVal = 0, const char *dir = 0, OutputOption outputflag = OUTPUT_MERGE, double timeout = 0.0);
    static bool RunSingleCommand(std::vector < cmStdString > const &command, std::string * output = 0, int *retVal = 0, const char *dir = 0, OutputOption outputflag = OUTPUT_MERGE, double timeout = 0.0);
  ...
  /** Parse arguments out of a windows command line string.  */
    static void ParseWindowsCommandLine(const char *command, std::vector < std::string > &args);

  /** Parse arguments out of a unix command line string.  */
    static void ParseUnixCommandLine(const char *command, std::vector < std::string > &args);
    static void ParseUnixCommandLine(const char *command, std::vector < cmStdString > &args);

  /** Compute an escaped version of the given argument for use in a
      windows shell.  See kwsys/System.h.in for details.  */
    static std::string EscapeWindowsShellArgument(const char *arg, int shell_flags);

    static void EnableMessages()
    {
        s_DisableMessages = false;
    }
    static void DisableMessages()
    {
        s_DisableMessages = true;
    }
    static void DisableRunCommandOutput()
    {
        s_DisableRunCommandOutput = true;
    }
    static void EnableRunCommandOutput()
    {
        s_DisableRunCommandOutput = false;
    }
    static bool GetRunCommandOutput()
    {
        return s_DisableRunCommandOutput;
    }
   ...
   */
    ...
    static bool GetRunCommandHideConsole()
    {
        return s_RunCommandHideConsole;
    }
  ...
  /** a general output handler for cmsysProcess  */
    static int WaitForLine(cmsysProcess * process, std::string & line, double timeout, std::vector < char >&out, std::vector < char >&err);
  ...
  */
   ...
   */
    static std::string CollapseCombinedPath(std::string const &dir, std::string const &file);
...
#    endif
  ...
  /** Create tar */
    static bool ListTar(const char *outFileName, bool gzip, bool verbose);
    static bool CreateTar(const char *outFileName, const std::vector < cmStdString > &files, bool gzip, bool bzip2, bool verbose);
    static bool ExtractTar(const char *inFileName, bool gzip, bool verbose);
...
#    endif
  ...
  /** Try to guess the soname of a shared library.  */
    static bool GuessLibrarySOName(std::string const &fullPath, std::string & soname);

  /** Try to guess the install name of a shared library.  */
    static bool GuessLibraryInstallName(std::string const &fullPath, std::string & soname);

  /** Try to set the RPATH in an ELF binary.  */
    static bool ChangeRPath(std::string const &file, std::string const &oldRPath, std::string const &newRPath, std::string * emsg = 0, bool * changed = 0);

  /** Try to remove the RPATH from an ELF binary.  */
    static bool RemoveRPath(std::string const &file, std::string * emsg = 0, bool * removed = 0);
