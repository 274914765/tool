...
//----------------------------------------------------------------------------
cmComputeLinkInformation::cmComputeLinkInformation(cmTarget const *target, const char *config, cmTarget const *headTarget)
{
    ...
    this->OpenBSD = this->Makefile->GetCMakeInstance()->GetPropertyAsBool("FIND_LIBRARY_USE_OPENBSD_VERSIONING");

    // The configuration being linked.
    this->Config = config;

    // Allocate internals.
    this->OrderLinkerSearchPath = new cmOrderDirectories(this->GlobalGenerator, target, "linker search path");
    this->OrderRuntimeSearchPath = new cmOrderDirectories(this->GlobalGenerator, target, "runtime search path");
    ...
    this->UseImportLibrary = this->Makefile->GetDefinition("CMAKE_IMPORT_LIBRARY_SUFFIX") ? true : false;

    // Check whether we should skip dependencies on shared library files.
    this->LinkDependsNoShared = this->Target->GetPropertyAsBool("LINK_DEPENDS_NO_SHARED");

    // On platforms without import libraries there may be a special flag
    // to use when creating a plugin (module) that obtains symbols from
    // the program that will load it.
    this->LoaderFlag = 0;
    if (!this->UseImportLibrary && this->Target->GetType() == cmTarget::MODULE_LIBRARY)
    {
    ...
    this->LibLinkFlag = this->Makefile->GetSafeDefinition("CMAKE_LINK_LIBRARY_FLAG");
    this->LibLinkFileFlag = this->Makefile->GetSafeDefinition("CMAKE_LINK_LIBRARY_FILE_FLAG");
    this->LibLinkSuffix = this->Makefile->GetSafeDefinition("CMAKE_LINK_LIBRARY_SUFFIX");

    // Get options needed to specify RPATHs.
    this->RuntimeUseChrpath = false;
    if (this->Target->GetType() != cmTarget::STATIC_LIBRARY)
    {
        const char *tType = ((this->Target->GetType() == cmTarget::EXECUTABLE) ? "EXECUTABLE" : "SHARED_LIBRARY");
        ...
        this->RuntimeAlways = (this->Makefile->GetSafeDefinition("CMAKE_PLATFORM_REQUIRED_RUNTIME_PATH"));
    ...
    this->NoSONameUsesPath = this->Makefile->IsOn("CMAKE_PLATFORM_USES_PATH_WHEN_NO_SONAME");
    ...
    else if (!this->RPathLinkFlag.empty())
    {
        ...
        this->OrderDependentRPath = new cmOrderDirectories(this->GlobalGenerator, target, "dependent library path");
    }

    // Add the search path entries requested by the user to path ordering.
    this->OrderLinkerSearchPath->AddUserDirectories(this->Target->GetLinkDirectories());
    this->OrderRuntimeSearchPath->AddUserDirectories(this->Target->GetLinkDirectories());
    ...
    this->OldLinkDirMode = this->Target->GetPolicyStatusCMP0003() != cmPolicies::NEW;
    if (this->OldLinkDirMode)
    {
        // Construct a mask to not bother with this behavior for link
        // directories already specified by the user.
        std::vector < std::string > const &dirs = this->Target->GetLinkDirectories();
        for (std::vector < std::string >::const_iterator di = dirs.begin(); di != dirs.end(); ++di)
        {
...
//----------------------------------------------------------------------------
cmComputeLinkInformation::ItemVector const & cmComputeLinkInformation::GetItems()
{
    return this->Items;
}
...
std::string cmComputeLinkInformation::GetRPathLinkString()
{
    ...
    const char *sep = "";
    std::vector < std::string > const &dirs = this->OrderDependentRPath->GetOrderedDirectories();
    for (std::vector < std::string >::const_iterator di = dirs.begin(); di != dirs.end(); ++di)
    {
        rpath_link += sep;
        sep = ":";
...
//----------------------------------------------------------------------------
std::set < cmTarget const *>const & cmComputeLinkInformation::GetSharedLibrariesLinked()
{
...
bool cmComputeLinkInformation::Compute()
{
    ...
    if (!(this->Target->GetType() == cmTarget::EXECUTABLE || this->Target->GetType() == cmTarget::SHARED_LIBRARY || this->Target->GetType() == cmTarget::MODULE_LIBRARY || this->Target->GetType() == cmTarget::STATIC_LIBRARY))
    {
        return false;
    }
    ...
    if (!this->LinkLanguage)
    {
        cmSystemTools::Error("CMake can not determine linker language for target: ", this->Target->GetName());
        return false;
    }
    ...
    for (cmComputeLinkDepends::EntryVector::const_iterator lei = linkEntries.begin(); lei != linkEntries.end(); ++lei)
    {
        if (lei->IsSharedDep)
        {
    ...
    if (this->OldLinkDirMode)
    {
        ...
        for (std::set < cmTarget const *>::const_iterator i = wrongItems.begin(); i != wrongItems.end(); ++i)
        {
            cmTarget const *tgt = *i;
            bool implib = (this->UseImportLibrary && (tgt->GetType() == cmTarget::SHARED_LIBRARY));
...
void cmComputeLinkInformation::AddImplicitLinkInfo()
{
    ...
    cmTarget::LinkClosure const *lc = this->Target->GetLinkClosure(this->Config,
                                                                   this->HeadTarget);
    for (std::vector < std::string >::const_iterator li = lc->Languages.begin(); li != lc->Languages.end(); ++li)
    {
...
void cmComputeLinkInformation::AddImplicitLinkInfo(std::string const &lang)
{
    ...
    if (const char *libs = this->Makefile->GetDefinition(libVar.c_str()))
    {
        ...
        for (std::vector < std::string >::const_iterator i = libsVec.begin(); i != libsVec.end(); ++i)
        {
...
//----------------------------------------------------------------------------
void cmComputeLinkInformation::AddItem(std::string const &item, cmTarget const *tgt)
{
    ...
    if (tgt && tgt->IsLinkable())
    {
        ...
        else
        {
            ...
            bool implib = (this->UseImportLibrary && (impexe || tgt->GetType() == cmTarget::SHARED_LIBRARY));
            ...
            if (!this->LinkDependsNoShared || tgt->GetType() != cmTarget::SHARED_LIBRARY)
            {
...
//----------------------------------------------------------------------------
void cmComputeLinkInformation::AddSharedDepItem(std::string const &item, cmTarget const *tgt)
{
    ...
    if (this->SharedDependencyMode == SharedDepModeLibDir && !this->LinkWithRuntimePath /* AddLibraryRuntimeInfo adds it */ )
    {
...
void cmComputeLinkInformation::ComputeLinkTypeInfo()
{
    ...
    this->ArchivesMayBeShared = this->CMakeInstance->GetPropertyAsBool("TARGET_ARCHIVES_MAY_BE_SHARED_LIBS");
    ...
    if (target_type_str)
    {
        ...
        static_link_type_flag = this->Makefile->GetDefinition(static_link_type_flag_var.c_str());
        ...
        shared_link_type_flag = this->Makefile->GetDefinition(shared_link_type_flag_var.c_str());
    }

    // We can support link type switching only if all needed flags are
    // known.
    if (static_link_type_flag && *static_link_type_flag && shared_link_type_flag && *shared_link_type_flag)
    {
...
void cmComputeLinkInformation::ComputeItemParserInfo()
{
    ...
    this->AddLinkExtension(mf->GetDefinition("CMAKE_IMPORT_LIBRARY_SUFFIX"), LinkShared);
    this->AddLinkExtension(mf->GetDefinition("CMAKE_STATIC_LIBRARY_SUFFIX"), LinkStatic);
    this->AddLinkExtension(mf->GetDefinition("CMAKE_SHARED_LIBRARY_SUFFIX"), LinkShared);
    this->AddLinkExtension(mf->GetDefinition("CMAKE_LINK_LIBRARY_SUFFIX"), LinkUnknown);
    if (const char *linkSuffixes = mf->GetDefinition("CMAKE_EXTRA_LINK_EXTENSIONS"))
    {
        ...
        for (std::vector < std::string >::iterator i = linkSuffixVec.begin(); i != linkSuffixVec.end(); ++i)
        {
    ...
    if (const char *sharedSuffixes = mf->GetDefinition("CMAKE_EXTRA_SHARED_LIBRARY_SUFFIXES"))
    {
        ...
        for (std::vector < std::string >::iterator i = sharedSuffixVec.begin(); i != sharedSuffixVec.end(); ++i)
        {
    ...
    std::string libext = this->CreateExtensionRegex(this->LinkExtensions, LinkUnknown);
    ...
    // extension.
    reg = "^(";
    for (std::set < cmStdString >::iterator p = this->LinkPrefixes.begin(); p != this->LinkPrefixes.end(); ++p)
    {
        reg += *p;
        reg += "|";
    }
    ...
    if (!this->StaticLinkExtensions.empty())
    {
        ...
        reg_static += this->CreateExtensionRegex(this->StaticLinkExtensions, LinkStatic);
#ifdef CM_COMPUTE_LINK_INFO_DEBUG
    ...
    if (!this->SharedLinkExtensions.empty())
    {
        ...
        this->SharedRegexString = this->CreateExtensionRegex(this->SharedLinkExtensions, LinkShared);
...
//----------------------------------------------------------------------------
std::string cmComputeLinkInformation::CreateExtensionRegex(std::vector < std::string > const &exts, LinkType type)
{
...
//----------------------------------------------------------------------------
void cmComputeLinkInformation::AddTargetItem(std::string const &item, cmTarget const *target)
{
    ...
    if (this->NoSONameUsesPath && target->IsImportedSharedLibWithoutSOName(this->Config))
    {
    ...
    if (this->OldLinkDirMode && !target->IsFrameworkOnApple() && this->OldLinkDirMask.find(cmSystemTools::GetFilenamePath(item)) == this->OldLinkDirMask.end())
    {
...
void cmComputeLinkInformation::AddFullItem(std::string const &item)
{
    ...
    if (this->Target->GetPolicyStatusCMP0008() != cmPolicies::NEW && (strstr(this->GlobalGenerator->GetName(), "Visual Studio") || strstr(this->GlobalGenerator->GetName(), "Xcode")))
    {
        std::string file = cmSystemTools::GetFilenameName(item);
    ...
    // For compatibility with CMake 2.4 include the item's directory in
    // the linker search path.
    if (this->OldLinkDirMode && this->OldLinkDirMask.find(cmSystemTools::GetFilenamePath(item)) == this->OldLinkDirMask.end())
    {
        this->OldLinkDirItems.push_back(item);
    }
...
//----------------------------------------------------------------------------
void cmComputeLinkInformation::AddUserItem(std::string const &item, bool pathNotKnown)
{
...
#ifdef CM_COMPUTE_LINK_INFO_DEBUG
        fprintf(stderr, "shared regex matched [%s] [%s] [%s]\n", this->ExtractSharedLibraryName.match(1).c_str(), this->ExtractSharedLibraryName.match(2).c_str(), this->ExtractSharedLibraryName.match(3).c_str());
#endif
...
#ifdef CM_COMPUTE_LINK_INFO_DEBUG
        fprintf(stderr, "static regex matched [%s] [%s] [%s]\n", this->ExtractStaticLibraryName.match(1).c_str(), this->ExtractStaticLibraryName.match(2).c_str(), this->ExtractStaticLibraryName.match(3).c_str());
#endif
...
#ifdef CM_COMPUTE_LINK_INFO_DEBUG
        fprintf(stderr, "any regex matched [%s] [%s] [%s]\n", this->ExtractAnyLibraryName.match(1).c_str(), this->ExtractAnyLibraryName.match(2).c_str(), this->ExtractAnyLibraryName.match(3).c_str());
#endif
        // Restore the target link type since this item does not specify
        // one.
        this->SetCurrentLinkType(this->StartLinkType);
...
void cmComputeLinkInformation::AddFrameworkItem(std::string const &item)
{
    ...
    {
        cmOStringStream e;
        e << "Could not parse framework path \"" << item << "\" " << "linked by target " << this->Target->GetName() << ".";
...
{
    if (this->Makefile->IsOn("APPLE") && cmSystemTools::IsPathToFramework(item.c_str()))
    {
...
void cmComputeLinkInformation::DropDirectoryItem(std::string const &item)
{
    ...
    // user.
    cmOStringStream e;
    e << "WARNING: Target \"" << this->Target->GetName() << "\" requests linking to directory \"" << item << "\".  " << "Targets may link only to libraries.  " << "CMake is dropping the item.";
...
void cmComputeLinkInformation::ComputeFrameworkInfo()
{
    ...
    if (const char *implicitLinks = this->Makefile->GetDefinition("CMAKE_PLATFORM_IMPLICIT_LINK_FRAMEWORK_DIRECTORIES"))
    {
    ...
    if (const char *implicitDirs = this->Makefile->GetDefinition(implicitDirVar.c_str()))
    {
        cmSystemTools::ExpandListArgument(implicitDirs, implicitDirVec);
    }

    for (std::vector < std::string >::const_iterator i = implicitDirVec.begin(); i != implicitDirVec.end(); ++i)
    {
...
//----------------------------------------------------------------------------
void cmComputeLinkInformation::HandleBadFullItem(std::string const &item, std::string const &file)
{
    // Do not depend on things that do not exist.
    std::vector < std::string >::iterator i = std::find(this->Depends.begin(), this->Depends.end(), item);
    ...
    switch (this->Target->GetPolicyStatusCMP0008())
    {
        case cmPolicies::WARN:
            {
                ...
                if (!this->CMakeInstance->GetPropertyAsBool(wid.c_str()))
                {
                    ...
                    cmOStringStream w;
                    w << (this->Makefile->GetPolicies()->GetPolicyWarning(cmPolicies::CMP0008)) << "\n"
                        << "Target \"" << this->Target->GetName() << "\" links to item\n" << "  " << item << "\n" << "which is a full-path but not a valid library file name.";
                    this->CMakeInstance->IssueMessage(cmake::AUTHOR_WARNING, w.str(), this->Target->GetBacktrace());
                }
            }
        case cmPolicies::OLD:
        ...
        case cmPolicies::REQUIRED_ALWAYS:
            {
                cmOStringStream e;
                e << (this->Makefile->GetPolicies()->GetRequiredPolicyError(cmPolicies::CMP0008)) << "\n"
                    << "Target \"" << this->Target->GetName() << "\" links to item\n" << "  " << item << "\n" << "which is a full-path but not a valid library file name.";
                this->CMakeInstance->IssueMessage(cmake::FATAL_ERROR, e.str(), this->Target->GetBacktrace());
            }
            break;
    }
}
...
bool cmComputeLinkInformation::FinishLinkerSearchDirectories()
{
    ...
    if (this->OldLinkDirItems.empty() || this->OldUserFlagItems.empty() || !this->OldLinkDirMode)
    {
        return true;
    }
    ...
    switch (this->Target->GetPolicyStatusCMP0003())
    {
        ...
        case cmPolicies::REQUIRED_ALWAYS:
            {
                cmOStringStream e;
                e << (this->Makefile->GetPolicies()->GetRequiredPolicyError(cmPolicies::CMP0003)) << "\n";
    ...
    for (std::vector < std::string >::const_iterator i = this->OldLinkDirItems.begin(); i != this->OldLinkDirItems.end(); ++i)
    {
...
void cmComputeLinkInformation::PrintLinkPolicyDiagnosis(std::ostream & os)
{
    ...
    os << "This warning appears because target \"" << this->Target->GetName() << "\" " << "links to some libraries for which the linker must search:\n";
    {
        ...
        std::string line;
        const char *sep = "  ";
        for (std::vector < std::string >::const_iterator i = this->OldUserFlagItems.begin(); i != this->OldUserFlagItems.end(); ++i)
        {
    ...
    os << "CMake is adding directories in the second list to the linker "
        ...
        << "first list (for backwards compatibility with CMake 2.4).  " << "Set policy CMP0003 to OLD or NEW to enable or disable this " << "behavior explicitly.  " << "Run \"cmake --help-policy CMP0003\" for more information.";
}
...
void cmComputeLinkInformation::LoadImplicitLinkInfo()
{
    ...
    // Get platform-wide implicit directories.
    if (const char *implicitLinks = (this->Makefile->GetDefinition("CMAKE_PLATFORM_IMPLICIT_LINK_DIRECTORIES")))
    {
        cmSystemTools::ExpandListArgument(implicitLinks, implicitDirVec);
    }
    ...
    if (const char *libraryArch = this->Makefile->GetDefinition("CMAKE_LIBRARY_ARCHITECTURE"))
    {
        for (std::vector < std::string >::const_iterator i = implicitDirVec.begin(); i != implicitDirVec.end(); ++i)
        {
    ...
    if (const char *implicitLibs = this->Makefile->GetDefinition(implicitLibVar.c_str()))
    {
        cmSystemTools::ExpandListArgument(implicitLibs, implicitLibVec);
    }

    // Store implicit link libraries.
    for (std::vector < std::string >::const_iterator i = implicitLibVec.begin(); i != implicitLibVec.end(); ++i)
    {
    ...
    if (const char *rpathDirs = (this->Makefile->GetDefinition("CMAKE_PLATFORM_RUNTIME_PATH")))
    {
...
//----------------------------------------------------------------------------
std::vector < std::string > const & cmComputeLinkInformation::GetRuntimeSearchPath()
{
    return this->OrderRuntimeSearchPath->GetOrderedDirectories();
}

//----------------------------------------------------------------------------
void cmComputeLinkInformation::AddLibraryRuntimeInfo(std::string const &fullPath, cmTarget const *target)
{
...
//----------------------------------------------------------------------------
void cmComputeLinkInformation::AddLibraryRuntimeInfo(std::string const &fullPath)
{
    // Get the name of the library from the file name.
...
//----------------------------------------------------------------------------
static void cmCLI_ExpandListUnique(const char *str, std::vector < std::string > &out, std::set < cmStdString > &emitted)
{
...
//----------------------------------------------------------------------------
void cmComputeLinkInformation::GetRPath(std::vector < std::string > &runtimeDirs, bool for_install)
{
    // Select whether to generate runtime search directories.
    bool outputRuntime = !this->Makefile->IsOn("CMAKE_SKIP_RPATH") && !this->RuntimeFlag.empty();

    // Select whether to generate an rpath for the install tree or the
    // build tree.
    bool linking_for_install = (for_install || this->Target->GetPropertyAsBool("BUILD_WITH_INSTALL_RPATH"));
    bool use_install_rpath = (outputRuntime && this->Target->HaveInstallTreeRPATH() && linking_for_install);
    bool use_build_rpath = (outputRuntime && this->Target->HaveBuildTreeRPATH(this->Config) && !linking_for_install);
    bool use_link_rpath = outputRuntime && linking_for_install && !this->Makefile->IsOn("CMAKE_SKIP_INSTALL_RPATH") && this->Target->GetPropertyAsBool("INSTALL_RPATH_USE_LINK_PATH");
    ...
    if (use_build_rpath || use_link_rpath)
    {
        ...
        const char *stagePath = this->Makefile->GetDefinition("CMAKE_STAGING_PREFIX");
        const char *installPrefix = this->Makefile->GetSafeDefinition("CMAKE_INSTALL_PREFIX");
        cmSystemTools::ConvertToUnixSlashes(rootPath);
        std::vector < std::string > const &rdirs = this->GetRuntimeSearchPath();
        for (std::vector < std::string >::const_iterator ri = rdirs.begin(); ri != rdirs.end(); ++ri)
        {
            ...
            else if (use_link_rpath)
            {
                ...
                if (!cmSystemTools::ComparePath(ri->c_str(), topSourceDir) &&
                    !cmSystemTools::ComparePath(ri->c_str(), topBinaryDir) && !cmSystemTools::IsSubDirectory(ri->c_str(), topSourceDir) && !cmSystemTools::IsSubDirectory(ri->c_str(), topBinaryDir))
                {
                    std::string d = *ri;
                    if (!rootPath.empty() && d.find(rootPath) == 0)
                    {
    ...
    {
        cmTarget::LinkClosure const *lc = this->Target->GetLinkClosure(this->Config, this->HeadTarget);
        for (std::vector < std::string >::const_iterator li = lc->Languages.begin(); li != lc->Languages.end(); ++li)
        {
            std::string useVar = "CMAKE_" + *li + "_USE_IMPLICIT_LINK_DIRECTORIES_IN_RUNTIME_PATH";
...
std::string cmComputeLinkInformation::GetRPathString(bool for_install)
{
    ...
    std::string rpath;
    const char *sep = "";
    for (std::vector < std::string >::const_iterator ri = runtimeDirs.begin(); ri != runtimeDirs.end(); ++ri)
    {
