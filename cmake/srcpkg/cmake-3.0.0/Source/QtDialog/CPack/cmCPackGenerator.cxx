...
int cmCPackGenerator::PrepareNames()
{
    ...
    if (IsOn("CPACK_SET_DESTDIR"))
    {
        ...
        {
            cmCPackLogger(cmCPackLog::LOG_ERROR, "CPACK_SET_DESTDIR is set to ON but the '" << Name << "' generator does NOT support it." << std::endl);
            return 0;
        }
        else if (SETDESTDIR_SHOULD_NOT_BE_USED == SupportsSetDestdir())
        {
            cmCPackLogger(cmCPackLog::LOG_WARNING, "CPACK_SET_DESTDIR is set to ON but it is " << "usually a bad idea to do that with '" << Name << "' generator. Use at your own risk." << std::endl);
        }
    }
    ...
    this->SetOptionIfNotSet("CPACK_NATIVE_INSTALL_DIRECTORY", cmsys::SystemTools::ConvertToOutputPath(this->GetInstallPath()).c_str());
    this->SetOptionIfNotSet("CPACK_TEMPORARY_INSTALL_DIRECTORY", tempDirectory.c_str());
    ...
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "Project description not specified. Please specify " "CPACK_PACKAGE_DESCRIPTION or CPACK_PACKAGE_DESCRIPTION_FILE." << std::endl);
        return 0;
    }
...
int cmCPackGenerator::InstallProject()
{
    ...
    bool setDestDir = cmSystemTools::IsOn(this->GetOption("CPACK_SET_DESTDIR")) | cmSystemTools::IsInternallyOn(this->GetOption("CPACK_SET_DESTDIR"));
    if (!setDestDir)
    {
    ...
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "Problem creating temporary directory: " << (tempInstallDirectory ? tempInstallDirectory : "(NULL)") << std::endl);
        return 0;
    }
    ...
    if (!this->InstallProjectViaInstalledDirectories(setDestDir, tempInstallDirectory))
    {
        return 0;
    }
    ...
    if (!this->InstallProjectViaInstallCMakeProjects(setDestDir, bareTempInstallDirectory.c_str()))
    {
        return 0;
    }
...
//----------------------------------------------------------------------
int cmCPackGenerator::InstallProjectViaInstallCommands(bool setDestDir, const char *tempInstallDirectory)
{
    (void) setDestDir;
    ...
    if (installCommands && *installCommands)
    {
        ...
        for (it = installCommandsVector.begin(); it != installCommandsVector.end(); ++it)
        {
            ...
            if (!resB || retVal)
            {
                ...
                ofs << "# Run command: " << it->c_str() << std::endl << "# Output:" << std::endl << output.c_str() << std::endl;
                cmCPackLogger(cmCPackLog::LOG_ERROR, "Problem running install command: " << it->c_str() << std::endl << "Please check " << tmpFile.c_str() << " for errors" << std::endl);
                return 0;
            }
        }
    }
...
int cmCPackGenerator::InstallProjectViaInstalledDirectories(bool setDestDir, const char *tempInstallDirectory)
{
    ...
    if (cpackIgnoreFiles)
    {
        ...
        std::vector < std::string >::iterator it;
        for (it = ignoreFilesRegexString.begin(); it != ignoreFilesRegexString.end(); ++it)
        {
            cmCPackLogger(cmCPackLog::LOG_VERBOSE, "Create ignore files regex for: " << it->c_str() << std::endl);
    ...
    if (installDirectories && *installDirectories)
    {
        ...
        for (it = installDirectoriesVector.begin(); it != installDirectoriesVector.end(); ++it)
        {
            ...
            for (gfit = files.begin(); gfit != files.end(); ++gfit)
            {
                ...
                for (regIt = ignoreFilesRegex.begin(); regIt != ignoreFilesRegex.end(); ++regIt)
                {
                    if (regIt->find(inFile.c_str()))
                    {
                        cmCPackLogger(cmCPackLog::LOG_VERBOSE, "Ignore file: " << inFile.c_str() << std::endl);
                        skip = true;
                    }
                }
                if (skip)
                ...
                if (cmSystemTools::FileIsSymlink(inFile.c_str()))
                {
                    ...
                    std::string inFileRelative = cmSystemTools::RelativePath(top.c_str(), inFile.c_str());
                    cmSystemTools::ReadSymlink(inFile.c_str(), targetFile);
                    symlinkedFiles.push_back(std::pair < std::string, std::string > (targetFile, inFileRelative));
                }
                /* If it is not a symlink then do a plain copy */
                else if (!(cmSystemTools::CopyFileIfDifferent(inFile.c_str(), filePath.c_str()) && cmSystemTools::CopyFileTime(inFile.c_str(), filePath.c_str())))
                {
            ...
            if (symlinkedFiles.size() > 0)
            {
                ...
                for (symlinkedIt = symlinkedFiles.begin(); symlinkedIt != symlinkedFiles.end(); ++symlinkedIt)
                {
                    cmCPackLogger(cmCPackLog::LOG_DEBUG, "Will create a symlink: " << symlinkedIt->second << "--> " << symlinkedIt->first << std::endl);
                    if (!cmSystemTools::CreateSymlink((symlinkedIt->first).c_str(), (symlinkedIt->second).c_str()))
                    {
                        cmCPackLogger(cmCPackLog::LOG_ERROR, "Cannot create symlink: " << symlinkedIt->second << "--> " << symlinkedIt->first << std::endl);
                        return 0;
                    }
                }
                cmCPackLogger(cmCPackLog::LOG_DEBUG, "Going back to: " << curDir << std::endl);
...
//----------------------------------------------------------------------
int cmCPackGenerator::InstallProjectViaInstallScript(bool setDestDir, const char *tempInstallDirectory)
{
    ...
    if (cmakeScripts && *cmakeScripts)
    {
        ...
        std::vector < std::string >::iterator it;
        for (it = cmakeScriptsVector.begin(); it != cmakeScriptsVector.end(); ++it)
        {
            ...
            if (setDestDir)
            {
                ...
                cmCPackLogger(cmCPackLog::LOG_DEBUG, "- Using DESTDIR + CPACK_INSTALL_PREFIX... (this->SetOption)" << std::endl);
            ...
            else
            {
                ...
                cmCPackLogger(cmCPackLog::LOG_DEBUG, "- Setting CMAKE_INSTALL_PREFIX to '" << tempInstallDirectory << "'" << std::endl);
            }
...
//----------------------------------------------------------------------
int cmCPackGenerator::InstallProjectViaInstallCMakeProjects(bool setDestDir, const char *baseTempInstallDirectory)
{
    ...
    if (cmakeProjects && *cmakeProjects)
    {
        if (!cmakeGenerator)
        {
            cmCPackLogger(cmCPackLog::LOG_ERROR, "CPACK_INSTALL_CMAKE_PROJECTS is specified, but " "CPACK_CMAKE_GENERATOR is not. CPACK_CMAKE_GENERATOR " "is required to install the project." << std::endl);
            return 0;
        }
        std::vector < std::string > cmakeProjectsVector;
        cmSystemTools::ExpandListArgument(cmakeProjects, cmakeProjectsVector);
        std::vector < std::string >::iterator it;
        for (it = cmakeProjectsVector.begin(); it != cmakeProjectsVector.end(); ++it)
        {
            if (it + 1 == cmakeProjectsVector.end() || it + 2 == cmakeProjectsVector.end() || it + 3 == cmakeProjectsVector.end())
            {
                cmCPackLogger(cmCPackLog::LOG_ERROR,
                              "Not enough items on list: CPACK_INSTALL_CMAKE_PROJECTS. "
                              "CPACK_INSTALL_CMAKE_PROJECTS should hold quadruplet of install " "directory, install project name, install component, and install " "subdirectory." << std::endl);
                return 0;
            }
            ...
            if (this->SupportsComponentInstallation() & !(this->IsOn("CPACK_MONOLITHIC_INSTALL")))
            {
                ...
                std::string installTypesVar = "CPACK_" + cmSystemTools::UpperCase(installComponent) + "_INSTALL_TYPES";
                ...
                if (installTypes && *installTypes)
                {
                    ...
                    for (installTypeIt = installTypesVector.begin(); installTypeIt != installTypesVector.end(); ++installTypeIt)
                    {
                        this->GetInstallationType(installProjectName.c_str(), installTypeIt->c_str());
                    }
                }

                // Determine the set of components that will be used in this project
                std::string componentsVar = "CPACK_COMPONENTS_" + cmSystemTools::UpperCase(installComponent);
                ...
                if (components && *components)
                {
                    ...
                    for (compIt = componentsVector.begin(); compIt != componentsVector.end(); ++compIt)
                    {
            ...
            cmGlobalGenerator *globalGenerator = this->MakefileMap->GetCMakeInstance()->CreateGlobalGenerator(cmakeGenerator);
            ...
            if (globalGenerator->GetPreinstallTargetName())
            {
                ...
                const char *cmakeMakeProgram = this->MakefileMap->GetDefinition("CMAKE_MAKE_PROGRAM");
                std::vector < std::string > buildCommand;
                globalGenerator->GenerateBuildCommand(buildCommand, cmakeMakeProgram, installProjectName.c_str(), installDirectory.c_str(), globalGenerator->GetPreinstallTargetName(), buildConfig, false);
                std::string buildCommandStr = cmSystemTools::PrintSingleCommand(buildCommand);
                cmCPackLogger(cmCPackLog::LOG_DEBUG, "- Install command: " << buildCommandStr << std::endl);
                cmCPackLogger(cmCPackLog::LOG_OUTPUT, "- Run preinstall target for: " << installProjectName << std::endl);
                std::string output;
                int retVal = 1;
                bool resB = cmSystemTools::RunSingleCommand(buildCommand,
                                                            &output,
                                                            &retVal,
                ...
                if (!resB || retVal)
                {
                    ...
                    cmGeneratedFileStream ofs(tmpFile.c_str());
                    ofs << "# Run command: " << buildCommandStr.c_str() << std::endl << "# Directory: " << installDirectory.c_str() << std::endl << "# Output:" << std::endl << output.c_str() << std::endl;
                    cmCPackLogger(cmCPackLog::LOG_ERROR, "Problem running install command: " << buildCommandStr.c_str() << std::endl << "Please check " << tmpFile.c_str() << " for errors" << std::endl);
                    return 0;
                }
            }
            delete globalGenerator;

            cmCPackLogger(cmCPackLog::LOG_OUTPUT, "- Install project: " << installProjectName << std::endl);

            // Run the installation for each component
            std::vector < std::string >::iterator componentIt;
            for (componentIt = componentsVector.begin(); componentIt != componentsVector.end(); ++componentIt)
            {
                std::string tempInstallDirectory = baseTempInstallDirectory;
                installComponent = *componentIt;
                if (componentInstall)
                {
                    cmCPackLogger(cmCPackLog::LOG_OUTPUT, "-   Install component: " << installComponent << std::endl);
                }
                ...
                if (componentInstall)
                {
                    ...
                    tempInstallDirectory += GetComponentInstallDirNameSuffix(installComponent);
                ...
                if (setDestDir)
                {
                    ...
                    {
                        cmCPackLogger(cmCPackLog::LOG_ERROR, "Problem creating temporary directory: " << dir << std::endl);
                        return 0;
                    }
                }
                else
                ...
                std::string installComponentLowerCase = cmSystemTools::LowerCase(installComponent);
                ...
                if (!SupportsAbsoluteDestination() || this->IsOn("CPACK_ERROR_ON_ABSOLUTE_INSTALL_DESTINATION"))
                {
                ...
                {
                    mf->AddDefinition("CPACK_ABSOLUTE_DESTINATION_FILES", mf->GetDefinition("CMAKE_ABSOLUTE_DESTINATION_FILES"));
                }
                ...
                if (componentInstall)
                {
                    ...
                    diff = std::set_difference(filesAfter.begin(), filesAfter.end(), filesBefore.begin(), filesBefore.end(), result.begin());
                    ...
                    for (fit = result.begin(); fit != diff; ++fit)
                    {
                        ...
                        localFileName = localFileName.substr(localFileName.find_first_not_of('/'), std::string::npos);
                        Components[installComponent].Files.push_back(localFileName);
                        cmCPackLogger(cmCPackLog::LOG_DEBUG, "Adding file <" << localFileName << "> to component <" << installComponent << ">" << std::endl);
                    }
                }
                ...
                if (NULL != mf->GetDefinition("CPACK_ABSOLUTE_DESTINATION_FILES"))
                {
                    ...
                    if (componentInstall)
                    {
                        std::string absoluteDestFileComponent = std::string("CPACK_ABSOLUTE_DESTINATION_FILES") + "_" + GetComponentInstallDirNameSuffix(installComponent);
                        if (NULL != this->GetOption(absoluteDestFileComponent.c_str()))
                        {
                            std::string absoluteDestFilesListComponent = this->GetOption(absoluteDestFileComponent.c_str());
                            absoluteDestFilesListComponent += ";";
                            absoluteDestFilesListComponent += mf->GetDefinition("CPACK_ABSOLUTE_DESTINATION_FILES");
                            this->SetOption(absoluteDestFileComponent.c_str(), absoluteDestFilesListComponent.c_str());
                        }
                        else
                        {
                            this->SetOption(absoluteDestFileComponent.c_str(), mf->GetDefinition("CPACK_ABSOLUTE_DESTINATION_FILES"));
                        }
                    }
                }
                if (cmSystemTools::GetErrorOccuredFlag() || !res)
...
//----------------------------------------------------------------------
void cmCPackGenerator::SetOptionIfNotSet(const char *op, const char *value)
{
...
{
    cmCPackLogger(cmCPackLog::LOG_OUTPUT, "Create package using " << this->Name.c_str() << std::endl);
    ...
    if (cmSystemTools::IsOn(this->GetOption("CPACK_REMOVE_TOPLEVEL_DIRECTORY")))
    {
        ...
        if (cmSystemTools::FileExists(toplevelDirectory))
        {
            ...
            {
                cmCPackLogger(cmCPackLog::LOG_ERROR, "Problem removing toplevel directory: " << toplevelDirectory << std::endl);
                return 0;
            }
        }
    }
    ...
    cmCPackLogger(cmCPackLog::LOG_VERBOSE, "Package files to: " << (tempPackageFileName ? tempPackageFileName : "(NULL)") << std::endl);
     ...
     */
    cmCPackLogger(cmCPackLog::LOG_VERBOSE, "Copying final package(s) [" << packageFileNames.size() << "]:" << std::endl);
    std::vector < std::string >::iterator it;
    ...
    for (it = packageFileNames.begin(); it != packageFileNames.end(); ++it)
    {
        ...
        cmCPackLogger(cmCPackLog::LOG_DEBUG, "Copy final package(s): " << (tempPackageFileName ? tempPackageFileName : "(NULL)") << " to " << (packageFileName ? packageFileName : "(NULL)") << std::endl);
        if (!cmSystemTools::CopyFileIfDifferent(tempPackageFileName, packageFileName))
        {
            cmCPackLogger(cmCPackLog::LOG_ERROR, "Problem copying the package: " << (tempPackageFileName ? tempPackageFileName : "(NULL)") << " to " << (packageFileName ? packageFileName : "(NULL)") << std::endl);
            return 0;
        }
...
{
    cmCPackLogger(cmCPackLog::LOG_DEBUG, "GetPackagingInstallPrefix: '" << this->GetOption("CPACK_PACKAGING_INSTALL_PREFIX") << "'" << std::endl);
...
{
    cmCPackLogger(cmCPackLog::LOG_DEBUG, "Look for template: " << (name ? name : "(NULL)") << std::endl);
...
//----------------------------------------------------------------------
bool cmCPackGenerator::ConfigureString(const std::string & inString, std::string & outString)
{
...
//----------------------------------------------------------------------
bool cmCPackGenerator::ConfigureFile(const char *inName, const char *outName, bool copyOnly /* = false */ )
{
...
int cmCPackGenerator::CleanTemporaryDirectory()
{
    ...
    if (cmsys::SystemTools::FileExists(tempInstallDirectory))
    {
        ...
        {
            cmCPackLogger(cmCPackLog::LOG_ERROR, "Problem removing temporary directory: " << tempInstallDirectory << std::endl);
            return 0;
        }
    }
    return 1;
...
int cmCPackGenerator::PrepareGroupingKind()
{
    ...
    {
        cmCPackLogger(cmCPackLog::LOG_VERBOSE, "[" << this->Name << "]" << " requested component grouping = " << groupingType << std::endl);
        ...
        {
            cmCPackLogger(cmCPackLog::LOG_WARNING, "[" << this->Name << "]" << " requested component grouping type <" << groupingType << "> UNKNOWN not in (ALL_COMPONENTS_IN_ONE,IGNORE,ONE_PER_GROUP)" << std::endl);
        }
    }
    ...
    if (method == ONE_PACKAGE_PER_GROUP && this->ComponentGroups.empty() && !this->Components.empty())
    {
        ...
        cmCPackLogger(cmCPackLog::LOG_WARNING, "[" << this->Name << "]" << " One package per component group requested, " << "but NO component groups exist: Ignoring component group." << std::endl);
    }
    ...
    cmCPackLogger(cmCPackLog::LOG_VERBOSE, "[" << this->Name << "]" << " requested component grouping = " << method_names[componentPackageMethod] << std::endl);

    return 1;
}

//----------------------------------------------------------------------
std::string cmCPackGenerator::GetComponentInstallDirNameSuffix(const std::string & componentName)
{
    return componentName;
}

//----------------------------------------------------------------------
std::string cmCPackGenerator::GetComponentPackageFileName(const std::string & initialPackageFileName, const std::string & groupOrComponentName, bool isGroupName)
{
    ...
    if (IsOn(dispNameVar.c_str()))
    {
        ...
        if (isGroupName)
        {
            std::string groupDispVar = "CPACK_COMPONENT_GROUP_" + cmSystemTools::UpperCase(groupOrComponentName) + "_DISPLAY_NAME";
...
//----------------------------------------------------------------------
cmCPackInstallationType *cmCPackGenerator::GetInstallationType(const char *projectName, const char *name)
{
    (void) projectName;
    ...
    if (!hasInstallationType)
    {
        ...
        const char *displayName = this->GetOption((macroPrefix + "_DISPLAY_NAME").c_str());
...
//----------------------------------------------------------------------
cmCPackComponent *cmCPackGenerator::GetComponent(const char *projectName, const char *name)
{
    ...
    if (!hasComponent)
    {
        ...
        component->IsHidden = this->IsOn((macroPrefix + "_HIDDEN").c_str());
        component->IsRequired = this->IsOn((macroPrefix + "_REQUIRED").c_str());
        component->IsDisabledByDefault = this->IsOn((macroPrefix + "_DISABLED").c_str());
        component->IsDownloaded = this->IsOn((macroPrefix + "_DOWNLOADED").c_str()) || cmSystemTools::IsOn(this->GetOption("CPACK_DOWNLOAD_ALL"));
        ...
        const char *description = this->GetOption((macroPrefix + "_DESCRIPTION").c_str());
        ...
        const char *installTypes = this->GetOption((macroPrefix + "_INSTALL_TYPES").c_str());
        if (installTypes && *installTypes)
        {
            std::vector < std::string > installTypesVector;
            cmSystemTools::ExpandListArgument(installTypes, installTypesVector);
            std::vector < std::string >::iterator installTypesIt;
            for (installTypesIt = installTypesVector.begin(); installTypesIt != installTypesVector.end(); ++installTypesIt)
            {
                component->InstallationTypes.push_back(this->GetInstallationType(projectName, installTypesIt->c_str()));
            }
        }
        ...
        if (depends && *depends)
        {
            ...
            for (dependIt = dependsVector.begin(); dependIt != dependsVector.end(); ++dependIt)
            {
...
//----------------------------------------------------------------------
cmCPackComponentGroup *cmCPackGenerator::GetComponentGroup(const char *projectName, const char *name)
{
    (void) projectName;
    ...
    if (!hasGroup)
    {
        ...
        group->IsBold = this->IsOn((macroPrefix + "_BOLD_TITLE").c_str());
        group->IsExpandedByDefault = this->IsOn((macroPrefix + "_EXPANDED").c_str());
        const char *parentGroupName = this->GetOption((macroPrefix + "_PARENT_GROUP").c_str());
