...
int cmCPackSTGZGenerator::InitializeInternal()
{
    ...
    if (inFile.empty())
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "Cannot find template file: " << inFile.c_str() << std::endl);
        return 0;
    }
...
int cmCPackSTGZGenerator::PackageFiles()
{
     ...
     */
    for (std::vector < std::string >::iterator it = packageFileNames.begin(); it != packageFileNames.end(); ++it)
    {
