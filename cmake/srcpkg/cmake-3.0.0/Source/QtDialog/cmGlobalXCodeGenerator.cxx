...
public:
    BuildObjectListOrString(cmGlobalXCodeGenerator * gen, bool buildObjectList):Generator(gen), Group(0), Empty(true)
    {
        if (buildObjectList)
        {
...
//----------------------------------------------------------------------------
cmGlobalGenerator *cmGlobalXCodeGenerator::Factory::CreateGlobalGenerator(const char *name) const
{
    ...
    std::string versionFile;
    {
        ...
        if (cmSystemTools::RunSingleCommand("xcode-select --print-path", &out, 0, 0, cmSystemTools::OUTPUT_NONE) && (pos = out.find(".app/"), pos != out.npos))
        {
    ...
    else if (cmSystemTools::FileExists("/Applications/Xcode.app/Contents/version.plist"))
    {
    ...
    cmsys::auto_ptr < cmGlobalXCodeGenerator > gg(new cmGlobalXCodeGenerator(parser.Version));
    if (gg->XcodeVersion == 20)
    {
        cmSystemTools::Message("Xcode 2.0 not really supported by cmake, " "using Xcode 15 generator\n");
        gg->XcodeVersion = 15;
    }
    return gg.release();
#else
...
//----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::EnableLanguage(std::vector < std::string > const &lang, cmMakefile * mf, bool optional)
{
    ...
    else
    {
        ...
        {
            mf->AddCacheDefinition("CMAKE_CONFIGURATION_TYPES",
                                   "Debug;Release;MinSizeRel;RelWithDebInfo",
                                   "Semicolon separated list of supported configuration types, " "only supports Debug, Release, MinSizeRel, and RelWithDebInfo, " "anything else will be ignored.", cmCacheManager::STRING);
        }
    }
    mf->AddDefinition("CMAKE_GENERATOR_NO_COMPILER_ENV", "1");
    if (!this->PlatformToolset.empty())
    {
        mf->AddDefinition("CMAKE_XCODE_PLATFORM_TOOLSET", this->PlatformToolset.c_str());
    }
    ...
    if (osxArch && sysroot)
    {
        ...
        cmSystemTools::ExpandListArgument(std::string(osxArch), this->Architectures);
    }
}

//----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::GenerateBuildCommand(std::vector < std::string > &makeCommand, const char *makeProgram, const char *projectName, const char * /*projectDir */ ,
                                                  const char *targetName, const char *config, bool /*fast */ ,
                                                  std::vector < std::string > const &makeOptions)
{
    // now build the test
    makeCommand.push_back(this->SelectMakeProgram(makeProgram, "xcodebuild"));
...
void cmGlobalXCodeGenerator::SetGenerationRoot(cmLocalGenerator * root)
{
    ...
    cmSystemTools::SplitPath(this->CurrentMakefile->GetCurrentDirectory(), this->ProjectSourceDirectoryComponents);
    cmSystemTools::SplitPath(this->CurrentMakefile->GetCurrentOutputDirectory(), this->ProjectOutputDirectoryComponents);

    this->CurrentXCodeHackMakefile = root->GetMakefile()->GetCurrentOutputDirectory();
...
//----------------------------------------------------------------------------
std::string cmGlobalXCodeGenerator::PostBuildMakeTarget(std::string const &tName, std::string const &configName)
{
...
//----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::AddExtraTargets(cmLocalGenerator * root, std::vector < cmLocalGenerator * >&gens)
{
    ...
    mf->AddUtilityCommand("ALL_BUILD", true, no_depends, no_working_directory, "echo", "Build all projects");
    ...
    if (regenerate)
    {
        ...
        std::string file = this->ConvertToRelativeForMake(this->CurrentReRunCMakeMakefile.c_str());
        cmSystemTools::ReplaceString(file, "\\ ", " ");
        mf->AddUtilityCommand(CMAKE_CHECK_BUILD_SYSTEM_TARGET, true, no_depends, no_working_directory, "make", "-f", file.c_str());
    }

    // now make the allbuild depend on all the non-utility targets
    // in the project
    for (std::vector < cmLocalGenerator * >::iterator i = gens.begin(); i != gens.end(); ++i)
    {
        ...
        for (cmTargets::iterator l = tgts.begin(); l != tgts.end(); l++)
        {
            ...
            if (!makeHelper.empty() && (target.GetType() == cmTarget::EXECUTABLE ||
// Nope - no post-build for OBJECT_LIRBRARY
//          target.GetType() == cmTarget::OBJECT_LIBRARY ||
                                        target.GetType() == cmTarget::STATIC_LIBRARY || target.GetType() == cmTarget::SHARED_LIBRARY || target.GetType() == cmTarget::MODULE_LIBRARY))
            {
                makeHelper[makeHelper.size() - 1] =    // fill placeholder
                    this->PostBuildMakeTarget(target.GetName(), "$(CONFIGURATION)");
                cmCustomCommandLines commandLines;
                commandLines.push_back(makeHelper);
                lg->GetMakefile()->AddCustomCommandToTarget(target.GetName(), no_depends, commandLines, cmTarget::POST_BUILD, "Depend check for xcode", dir.c_str());
            }

            if (target.GetType() != cmTarget::INTERFACE_LIBRARY && !target.GetPropertyAsBool("EXCLUDE_FROM_ALL"))
            {
...
//----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::CreateReRunCMakeFile(cmLocalGenerator * root, std::vector < cmLocalGenerator * >const &gens)
{
    cmMakefile *mf = root->GetMakefile();
    std::vector < std::string > lfiles;
    for (std::vector < cmLocalGenerator * >::const_iterator gi = gens.begin(); gi != gens.end(); ++gi)
    {
    ...
    std::vector < std::string >::iterator new_end = std::unique(lfiles.begin(), lfiles.end());
    ...
    for (std::vector < std::string >::const_iterator i = lfiles.begin(); i != lfiles.end(); ++i)
    {
...
void cmGlobalXCodeGenerator::addObject(cmXCodeObject * obj)
{
    if (obj->GetType() == cmXCodeObject::OBJECT)
    {
        ...
        {
            cmSystemTools::Error("Xcode generator: duplicate object ids not allowed");
        }
...
//----------------------------------------------------------------------------
cmStdString GetGroupMapKeyFromPath(cmTarget & cmtarget, const std::string & fullpath)
{
...
//----------------------------------------------------------------------------
cmXCodeObject *cmGlobalXCodeGenerator::CreateXCodeSourceFileFromPath(const std::string & fullpath, cmTarget & cmtarget, const std::string & lang)
{
    // Using a map and the full path guarantees that we will always get the same
    // fileRef object for any given full path.
    //
    cmXCodeObject *fileRef = this->CreateXCodeFileReferenceFromPath(fullpath, cmtarget, lang);
...
//----------------------------------------------------------------------------
cmXCodeObject *cmGlobalXCodeGenerator::CreateXCodeSourceFile(cmLocalGenerator * lg, cmSourceFile * sf, cmTarget & cmtarget)
{
    ...
    const char *lang = this->CurrentLocalGenerator->GetSourceFileLanguage(*sf);
    if (!lang)
    {
        lang = "";
    }

    cmXCodeObject *buildFile = this->CreateXCodeSourceFileFromPath(sf->GetFullPath(), cmtarget, lang);
    ...
    // already there.
    //
    if (isResource && this->ResourcesGroupChildren && !this->ResourcesGroupChildren->HasObject(fileRef))
    {
...
//----------------------------------------------------------------------------
std::string GetSourcecodeValueFromFileExtension(const std::string & _ext, const std::string & lang, bool & keepLastKnownFileType)
{
    ...
    else if (ext == "hxx" || ext == "hpp" || ext == "txx" || ext == "pch" || ext == "hh")
    {
        sourcecode += ".cpp.h";
    }
...
//----------------------------------------------------------------------------
cmXCodeObject *cmGlobalXCodeGenerator::CreateXCodeFileReferenceFromPath(const std::string & fullpath, cmTarget & cmtarget, const std::string & lang)
{
    ...
    {
        fileRef->AddAttribute("lastKnownFileType", this->CreateString("folder"));
    }
    else
    {
        bool keepLastKnownFileType = false;
        std::string sourcecode = GetSourcecodeValueFromFileExtension(ext, lang, keepLastKnownFileType);
        const char *attribute = keepLastKnownFileType ? "lastKnownFileType" : "explicitFileType";
        fileRef->AddAttribute(attribute, this->CreateString(sourcecode.c_str()));
    }

    // Store the file path relative to the top of the source tree.
    std::string path = this->RelativeToSource(fullpath.c_str());
    std::string name = cmSystemTools::GetFilenameName(path.c_str());
    const char *sourceTree = (cmSystemTools::FileIsFullPath(path.c_str())? "<absolute>" : "SOURCE_ROOT");
...
//----------------------------------------------------------------------------
cmXCodeObject *cmGlobalXCodeGenerator::CreateXCodeFileReference(cmSourceFile * sf, cmTarget & cmtarget)
{
    const char *lang = this->CurrentLocalGenerator->GetSourceFileLanguage(*sf);
    if (!lang)
    {
    ...
    return this->CreateXCodeFileReferenceFromPath(sf->GetFullPath(), cmtarget, lang);
}

//----------------------------------------------------------------------------
bool cmGlobalXCodeGenerator::SpecialTargetEmitted(std::string const &tname)
{
    if (tname == "ALL_BUILD" || tname == "XCODE_DEPEND_HELPER" || tname == "install" || tname == "package" || tname == "RUN_TESTS" || tname == CMAKE_CHECK_BUILD_SYSTEM_TARGET)
    {
...
void cmGlobalXCodeGenerator::SetCurrentLocalGenerator(cmLocalGenerator * gen)
{
    ...
    std::string outdir = cmSystemTools::CollapseFullPath(this->CurrentMakefile->GetCurrentOutputDirectory());
...
//----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::CreateXCodeTargets(cmLocalGenerator * gen, std::vector < cmXCodeObject * >& targets)
{
    ...
    for (cmTargets::iterator l = tgts.begin(); l != tgts.end(); l++)
    {
        ...
        if (cmtarget.GetType() == cmTarget::UTILITY || cmtarget.GetType() == cmTarget::GLOBAL_TARGET)
        {
        ...
        for (std::vector < cmSourceFile * >::const_iterator i = classes.begin(); i != classes.end(); ++i)
        {
            cmXCodeObject *xsf = this->CreateXCodeSourceFile(this->CurrentLocalGenerator,
                                                             *i, cmtarget);
            cmXCodeObject *fr = xsf->GetObject("fileRef");
            cmXCodeObject *filetype = fr->GetObject()->GetObject("explicitFileType");

            cmTarget::SourceFileFlags tsFlags = cmtarget.GetTargetSourceFileFlags(*i);

            if (filetype && strcmp(filetype->GetString(), "compiled.mach-o.objfile") == 0)
            {
                externalObjFiles.push_back(xsf);
            }
            else if (this->IsHeaderFile(*i) || (tsFlags.Type == cmTarget::SourceFileTypePrivateHeader) || (tsFlags.Type == cmTarget::SourceFileTypePublicHeader))
            {
            ...
            else if (!(*i)->GetPropertyAsBool("HEADER_FILE_ONLY"))
            {
                ...
                // generator.
                if (this->CurrentLocalGenerator->GetSourceFileLanguage(**i) && !this->IgnoreFile((*i)->GetExtension().c_str()))
                {
        ...
        if (this->XcodeVersion < 50)
        {
            ...
            for (std::vector < std::string >::const_iterator oi = objs.begin(); oi != objs.end(); ++oi)
            {
                std::string obj = *oi;
                cmXCodeObject *xsf = this->CreateXCodeSourceFileFromPath(obj, cmtarget, "");
                externalObjFiles.push_back(xsf);
            }
        }
        ...
        if (!sourceFiles.empty())
        {
            ...
            for (std::vector < cmXCodeObject * >::iterator i = sourceFiles.begin(); i != sourceFiles.end(); ++i)
            {
        ...
        if (!headerFiles.empty() && isFrameworkTarget)
        {
            ...
            buildFiles = this->CreateObject(cmXCodeObject::OBJECT_LIST);
            for (std::vector < cmXCodeObject * >::iterator i = headerFiles.begin(); i != headerFiles.end(); ++i)
            {
                buildFiles->AddObject(*i);
            }
        ...
        if (!resourceFiles.empty() && (isFrameworkTarget || isBundleTarget || isCFBundleTarget))
        {
            resourceBuildPhase = this->CreateObject(cmXCodeObject::PBXResourcesBuildPhase);
            resourceBuildPhase->SetComment("Resources");
            resourceBuildPhase->AddAttribute("buildActionMask", this->CreateString("2147483647"));
            buildFiles = this->CreateObject(cmXCodeObject::OBJECT_LIST);
            for (std::vector < cmXCodeObject * >::iterator i = resourceFiles.begin(); i != resourceFiles.end(); ++i)
            {
                buildFiles->AddObject(*i);
            }
            resourceBuildPhase->AddAttribute("files", buildFiles);
            resourceBuildPhase->AddAttribute("runOnlyForDeploymentPostprocessing", this->CreateString("0"));
        }
        ...
        {
            typedef std::map < cmStdString, std::vector < cmSourceFile * > > mapOfVectorOfSourceFiles;
            ...
            for (mit = bundleFiles.begin(); mit != bundleFiles.end(); ++mit)
            {
                ...
                copyFilesBuildPhase->AddAttribute("buildActionMask", this->CreateString("2147483647"));
                copyFilesBuildPhase->AddAttribute("dstSubfolderSpec", this->CreateString("6"));
                ...
                copyFilesBuildPhase->AddAttribute("dstPath", this->CreateString(ostr.str().c_str()));
        ...
        {
            frameworkBuildPhase = this->CreateObject(cmXCodeObject::PBXFrameworksBuildPhase);
            ...
            for (std::vector < cmXCodeObject * >::iterator i = externalObjFiles.begin(); i != externalObjFiles.end(); ++i)
            {
                buildFiles->AddObject(*i);
            }
        ...
        this->CreateCustomCommands(buildPhases, sourceBuildPhase, headerBuildPhase, resourceBuildPhase, contentBuildPhases, frameworkBuildPhase, cmtarget);
...
void cmGlobalXCodeGenerator::ForceLinkerLanguages()
{
    ...
    for (std::map < cmStdString, cmTarget * >::const_iterator ti = this->TotalTargets.begin(); ti != this->TotalTargets.end(); ++ti)
    {
...
void cmGlobalXCodeGenerator::ForceLinkerLanguage(cmTarget & cmtarget)
{
    ...
    if (cmtarget.GetType() != cmTarget::EXECUTABLE && cmtarget.GetType() != cmTarget::SHARED_LIBRARY && cmtarget.GetType() != cmTarget::MODULE_LIBRARY)
    {
        return;
    }
    ...
    cmTarget::LinkImplementation const *impl = cmtarget.GetLinkImplementation("NOCONFIG", &cmtarget);
...
{
    const std::vector < std::string > &hdrExts = this->CurrentMakefile->GetHeaderExtensions();
    return (std::find(hdrExts.begin(), hdrExts.end(), sf->GetExtension()) != hdrExts.end());
}

//----------------------------------------------------------------------------
cmXCodeObject *cmGlobalXCodeGenerator::CreateBuildPhase(const char *name, const char *name2, cmTarget & cmtarget, const std::vector < cmCustomCommand > &commands)
{
    if (commands.size() == 0 && strcmp(name, "CMake ReRun") != 0)
    {
        return 0;
    }
    cmXCodeObject *buildPhase = this->CreateObject(cmXCodeObject::PBXShellScriptBuildPhase);
    buildPhase->AddAttribute("buildActionMask", this->CreateString("2147483647"));
    cmXCodeObject *buildFiles = this->CreateObject(cmXCodeObject::OBJECT_LIST);
    buildPhase->AddAttribute("files", buildFiles);
    buildPhase->AddAttribute("name", this->CreateString(name));
    buildPhase->AddAttribute("runOnlyForDeploymentPostprocessing", this->CreateString("0"));
    buildPhase->AddAttribute("shellPath", this->CreateString("/bin/sh"));
    this->AddCommandsToBuildPhase(buildPhase, cmtarget, commands, name2);
    return buildPhase;
}
...
void cmGlobalXCodeGenerator::CreateCustomCommands(cmXCodeObject * buildPhases,
                                                  cmXCodeObject *
                                                  sourceBuildPhase, cmXCodeObject * headerBuildPhase, cmXCodeObject * resourceBuildPhase, std::vector < cmXCodeObject * > contentBuildPhases, cmXCodeObject * frameworkBuildPhase, cmTarget & cmtarget)
{
    std::vector < cmCustomCommand > const &prebuild = cmtarget.GetPreBuildCommands();
    std::vector < cmCustomCommand > const &prelink = cmtarget.GetPreLinkCommands();
    std::vector < cmCustomCommand > postbuild = cmtarget.GetPostBuildCommands();

    if (cmtarget.GetType() == cmTarget::SHARED_LIBRARY && !cmtarget.IsFrameworkOnApple())
    {
        ...
        cmCustomCommand command(this->CurrentMakefile, std::vector < std::string > (), std::vector < std::string > (), cmd, "Creating symlinks", "");
    ...
    cmXCodeObject *cmakeRulesBuildPhase = this->CreateBuildPhase("CMake Rules",
    ...
    cmXCodeObject *postBuildPhase = this->CreateBuildPhase("CMake PostBuild Rules", "postBuildPhase",
                                                           cmtarget, postbuild);
...
std::string cmGlobalXCodeGenerator::ExtractFlag(const char *flag, std::string & flags)
{
    std::string retFlag;
...
//----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::AddCommandsToBuildPhase(cmXCodeObject * buildphase, cmTarget & target, std::vector < cmCustomCommand > const &commands, const char *name)
{
    ...
    for (std::vector < cmCustomCommand >::const_iterator i = commands.begin(); i != commands.end(); ++i)
    {
    ...
    for (std::vector < std::string >::const_iterator currentConfig = this->CurrentConfigurationTypes.begin(); currentConfig != this->CurrentConfigurationTypes.end(); currentConfig++)
    {
        this->CreateCustomRulesMakefile(makefile.c_str(), target, commands, currentConfig->c_str(), multipleOutputPairs);
    }
...
//----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::CreateCustomRulesMakefile(const char *makefileBasename, cmTarget & target, std::vector < cmCustomCommand > const &commands, const char *configName, const std::map < cmStdString, cmStdString > &multipleOutputPairs)
{
    ...
    for (std::vector < cmCustomCommand >::const_iterator i = commands.begin(); i != commands.end(); ++i)
    {
        ...
        if (!cc.GetCommandLines().empty())
        {
            ...
            if (!outputs.empty())
            {
                ...
                std::string primary_output = this->ConvertToRelativeForMake(outputs.begin()->c_str());
            ...
            for (std::vector < std::string >::const_iterator d = cc.GetDepends().begin(); d != cc.GetDepends().end(); ++d)
            {
                std::string dep;
                if (this->CurrentLocalGenerator->GetRealDependency(d->c_str(), configName, dep))
                {
            ...
            if (const char *comment = cc.GetComment())
            {
                ...
                echo_cmd += (this->CurrentLocalGenerator->EscapeForShell(comment, cc.GetEscapeAllowMakeVars()));
    ...
    if (!multipleOutputPairs.empty())
    {
        ...
        for (std::map < cmStdString, cmStdString >::const_iterator o = multipleOutputPairs.begin(); o != multipleOutputPairs.end(); ++o)
        {
        ...
        for (std::map < cmStdString, cmStdString >::const_iterator o = multipleOutputPairs.begin(); o != multipleOutputPairs.end(); ++o)
        {
            makefileStream << "\t@if [ ! -f " << o->first << " ]; then rm -f " << o->second << "; fi\n";
        }
    }
}

//----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::CreateBuildSettings(cmTarget & target, cmXCodeObject * buildSettings, const char *configName)
{
    ...
    std::string flags;
    std::string defFlags;
    bool shared = ((target.GetType() == cmTarget::SHARED_LIBRARY) || (target.GetType() == cmTarget::MODULE_LIBRARY));
    bool binary = ((target.GetType() == cmTarget::OBJECT_LIBRARY) || (target.GetType() == cmTarget::STATIC_LIBRARY) || (target.GetType() == cmTarget::EXECUTABLE) || shared);
    ...
    if (lang)
    {
        ...
        if (strcmp(lang, "CXX") == 0)
        {
            ...
            this->CurrentLocalGenerator->AddCMP0018Flags(cflags, &target, "C", configName);
            this->CurrentLocalGenerator->AddCompileOptions(cflags, &target, "C", configName);
        }
        ...
        this->CurrentLocalGenerator->AddCMP0018Flags(flags, &target, lang, configName);

        this->CurrentLocalGenerator->AddVisibilityPresetFlags(flags, &target, lang);

        this->CurrentLocalGenerator->AddCompileOptions(flags, &target, lang, configName);
    }
    else if (binary)
    {
        cmSystemTools::Error("CMake can not determine linker language for target: ", target.GetName());
        return;
    }

    // Add define flags
    this->CurrentLocalGenerator->AppendFlags(defFlags, this->CurrentMakefile->GetDefineFlags());
    ...
    buildSettings->AddAttribute("GCC_PREPROCESSOR_DEFINITIONS", ppDefs.CreateList());
    ...
    {
        this->CurrentLocalGenerator->AddConfigVariableFlags(extraLinkOptions, extraLinkOptionsVar.c_str(), configName);
    }

    if (target.GetType() == cmTarget::OBJECT_LIBRARY || target.GetType() == cmTarget::STATIC_LIBRARY)
    {
        this->CurrentLocalGenerator->GetStaticLibraryFlags(extraLinkOptions, cmSystemTools::UpperCase(configName), &target);
    }
    else
    {
        ...
        if (targetLinkFlags)
        {
            this->CurrentLocalGenerator->AppendFlags(extraLinkOptions, targetLinkFlags);
        }
    ...
    if (!archs.empty())
    {
        ...
        if (archs.size() == 1)
        {
            buildSettings->AddAttribute("ARCHS", this->CreateString(archs[0].c_str()));
        }
        else
        {
            cmXCodeObject *archObjects = this->CreateObject(cmXCodeObject::OBJECT_LIST);
            for (std::vector < std::string >::iterator i = archs.begin(); i != archs.end(); i++)
            {
    ...
    if (target.GetType() == cmTarget::STATIC_LIBRARY || target.GetType() == cmTarget::SHARED_LIBRARY || target.GetType() == cmTarget::MODULE_LIBRARY || target.GetType() == cmTarget::EXECUTABLE)
    {
        if (this->XcodeVersion >= 21)
        {
            if (!target.UsesDefaultOutputDir(configName, false))
            {
                ...
                buildSettings->AddAttribute("CONFIGURATION_BUILD_DIR", this->CreateString(pncdir.c_str()));
            }
        }
        else
        {
            buildSettings->AddAttribute("OBJROOT", this->CreateString(pndir.c_str()));
        ...
        buildSettings->AddAttribute("EXECUTABLE_PREFIX", this->CreateString(pnprefix.c_str()));
        buildSettings->AddAttribute("EXECUTABLE_SUFFIX", this->CreateString(pnsuffix.c_str()));
    }
    else if (target.GetType() == cmTarget::OBJECT_LIBRARY)
    {
        ...
        if (this->XcodeVersion >= 21)
        {
            std::string pncdir = this->GetObjectsNormalDirectory(this->CurrentProject, configName, &target);
            buildSettings->AddAttribute("CONFIGURATION_BUILD_DIR", this->CreateString(pncdir.c_str()));
        }
        else
        {
            buildSettings->AddAttribute("OBJROOT", this->CreateString(pndir.c_str()));
            pndir = this->GetObjectsNormalDirectory(this->CurrentProject, configName, &target);
        }
    }

    // Store the product name for all target types.
    buildSettings->AddAttribute("PRODUCT_NAME", this->CreateString(realName.c_str()));
    buildSettings->AddAttribute("SYMROOT", this->CreateString(pndir.c_str()));
    ...
    switch (target.GetType())
    {
        ...
        case cmTarget::STATIC_LIBRARY:
            {
                buildSettings->AddAttribute("LIBRARY_STYLE", this->CreateString("STATIC"));
                break;
            }

        case cmTarget::MODULE_LIBRARY:
            {
                buildSettings->AddAttribute("LIBRARY_STYLE", this->CreateString("BUNDLE"));
                if (target.GetPropertyAsBool("BUNDLE"))
                {
                    // It turns out that a BUNDLE is basically the same
                    // in many ways as an application bundle, as far as
                    // link flags go
                    std::string createFlags = this->LookupFlags("CMAKE_SHARED_MODULE_CREATE_", lang, "_FLAGS", "-bundle");
                    ...
                    this->CurrentLocalGenerator->GenerateAppleInfoPList(&target, "$(EXECUTABLE_NAME)", plist.c_str());
                    std::string path = this->ConvertToRelativeForXCode(plist.c_str());
                    buildSettings->AddAttribute("INFOPLIST_FILE", this->CreateString(path.c_str()));
                }
                else if (this->XcodeVersion >= 22)
                {
                    buildSettings->AddAttribute("MACH_O_TYPE", this->CreateString("mh_bundle"));
                    buildSettings->AddAttribute("GCC_DYNAMIC_NO_PIC", this->CreateString("NO"));
                    // Add the flags to create an executable.
                    std::string createFlags = this->LookupFlags("CMAKE_", lang, "_LINK_FLAGS", "");
                    if (!createFlags.empty())
                    {
                        extraLinkOptions += " ";
                        extraLinkOptions += createFlags;
        ...
        case cmTarget::SHARED_LIBRARY:
            {
                if (target.GetPropertyAsBool("FRAMEWORK"))
                {
                    ...
                    buildSettings->AddAttribute("FRAMEWORK_VERSION", this->CreateString(fw_version.c_str()));

                    std::string plist = this->ComputeInfoPListLocation(target);
                    // Xcode will create the final version of Info.plist at build time,
                    // so let it replace the framework name. This avoids creating
                    // a per-configuration Info.plist file.
                    this->CurrentLocalGenerator->GenerateFrameworkInfoPList(&target, "$(EXECUTABLE_NAME)", plist.c_str());
                    std::string path = this->ConvertToRelativeForXCode(plist.c_str());
                    buildSettings->AddAttribute("INFOPLIST_FILE", this->CreateString(path.c_str()));
                }
                else
                {
                    ...
                    std::string createFlags = this->LookupFlags("CMAKE_SHARED_LIBRARY_CREATE_", lang, "_FLAGS", "-dynamiclib");
                    if (!createFlags.empty())
                    {
                        extraLinkOptions += " ";
                        extraLinkOptions += createFlags;
                ...
                buildSettings->AddAttribute("LIBRARY_STYLE", this->CreateString("DYNAMIC"));
                break;
            }
    ...
    this->CurrentLocalGenerator->GetIncludeDirectories(includes, gtgt, "C", configName);
    ...
    for (std::vector < std::string >::iterator i = includes.begin(); i != includes.end(); ++i)
    {
        ...
        {
            std::string incpath = this->XCodeEscapePath(i->c_str());
    ...
    buildSettings->AddAttribute("COMBINE_HIDPI_IMAGES", this->CreateString("YES"));
    buildSettings->AddAttribute("GCC_GENERATE_DEBUGGING_SYMBOLS", this->CreateString(debugStr));
    buildSettings->AddAttribute("GCC_OPTIMIZATION_LEVEL", this->CreateString(optLevel));
    buildSettings->AddAttribute("GCC_SYMBOLS_PRIVATE_EXTERN", this->CreateString("NO"));
    buildSettings->AddAttribute("GCC_INLINES_ARE_PRIVATE_EXTERN", this->CreateString("NO"));
    if (lang && strcmp(lang, "CXX") == 0)
    {
        flags += " ";
        flags += defFlags;
        buildSettings->AddAttribute("OTHER_CPLUSPLUSFLAGS", this->CreateString(flags.c_str()));
        cflags += " ";
        cflags += defFlags;
        buildSettings->AddAttribute("OTHER_CFLAGS", this->CreateString(cflags.c_str()));
    ...
    if (format)
    {
        buildSettings->AddAttribute("IFORT_LANG_SRCFMT", this->CreateString(format));
    }
    ...
    buildSettings->AddAttribute("INSTALL_PATH", this->CreateString(install_name_dir.c_str()));
    ...
    if (pcli)
    {
        ...
        for (std::vector < std::string >::const_iterator i = runtimeDirs.begin(); i != runtimeDirs.end(); ++i)
        {
        ...
        if (!search_paths.empty())
        {
            buildSettings->AddAttribute("LD_RUNPATH_SEARCH_PATHS", this->CreateString(search_paths.c_str()));
        }
    }

    buildSettings->AddAttribute("OTHER_LDFLAGS", this->CreateString(extraLinkOptions.c_str()));
    ...
    {
        buildSettings->AddAttribute("WARNING_CFLAGS", this->CreateString("-Wmost -Wno-four-char-constants" " -Wno-unknown-pragmas"));
    }
    ...
    if (target.GetType() == cmTarget::SHARED_LIBRARY)
    {
        ...
        buildSettings->AddAttribute("DYLIB_CURRENT_VERSION", this->CreateString(v.str().c_str()));
        ...
        buildSettings->AddAttribute("DYLIB_COMPATIBILITY_VERSION", this->CreateString(vso.str().c_str()));
    }
    ...
    // Convert "XCODE_ATTRIBUTE_*" properties directly.
    {
        ...
        for (cmPropertyMap::const_iterator i = props.begin(); i != props.end(); ++i)
        {
            if (i->first.find("XCODE_ATTRIBUTE_") == 0)
            {
                ...
                cmStdString::size_type beginVariant = attribute.find("[variant=");
                if (beginVariant != cmStdString::npos)
                {
                    cmStdString::size_type endVariant = attribute.find("]", beginVariant + 9);
                    if (endVariant != cmStdString::npos)
                    {
                        // Compare the variant to the configuration.
                        cmStdString variant = attribute.substr(beginVariant + 9, endVariant - beginVariant - 9);
                ...
                if (!attribute.empty())
                {
                    buildSettings->AddAttribute(attribute.c_str(), this->CreateString(i->second.GetValue()));
                }
            }
        }
    }
...
//----------------------------------------------------------------------------
cmXCodeObject *cmGlobalXCodeGenerator::CreateUtilityTarget(cmTarget & cmtarget)
{
    cmXCodeObject *shellBuildPhase = this->CreateObject(cmXCodeObject::PBXShellScriptBuildPhase);
    shellBuildPhase->AddAttribute("buildActionMask", this->CreateString("2147483647"));
    cmXCodeObject *buildFiles = this->CreateObject(cmXCodeObject::OBJECT_LIST);
    ...
    shellBuildPhase->AddAttribute("runOnlyForDeploymentPostprocessing", this->CreateString("0"));
    shellBuildPhase->AddAttribute("shellPath", this->CreateString("/bin/sh"));
    shellBuildPhase->AddAttribute("shellScript", this->CreateString("# shell script goes here\nexit 0"));
    shellBuildPhase->AddAttribute("showEnvVarsInLog", this->CreateString("0"));

    cmXCodeObject *target = this->CreateObject(cmXCodeObject::PBXAggregateTarget);
    ...
    {
        const char *theConfig = this->CurrentMakefile->GetDefinition("CMAKE_BUILD_TYPE");
    ...
    if (cmtarget.GetType() == cmTarget::UTILITY)
    {
        ...
        for (std::vector < cmSourceFile * >::const_iterator i = sources.begin(); i != sources.end(); ++i)
        {
...
//----------------------------------------------------------------------------
std::string cmGlobalXCodeGenerator::AddConfigurations(cmXCodeObject * target, cmTarget & cmtarget)
{
    ...
    cmXCodeObject *buildConfigurations = this->CreateObject(cmXCodeObject::OBJECT_LIST);
    ...
    target->AddAttribute("buildConfigurationList", this->CreateObjectReference(configlist));
    for (unsigned int i = 0; i < configVector.size(); ++i)
    {
        ...
        cmXCodeObject *buildSettings = this->CreateObject(cmXCodeObject::ATTRIBUTE_GROUP);
        this->CreateBuildSettings(cmtarget, buildSettings, configVector[i].c_str());
    ...
    {
        configlist->AddAttribute("defaultConfigurationName", this->CreateString(configVector[0].c_str()));
    ...
    switch (cmtarget.GetType())
    {
        ...
        case cmTarget::MODULE_LIBRARY:
            if (cmtarget.IsCFBundleOnApple())
                ...
                return ((this->XcodeVersion >= 22) ? "compiled.mach-o.executable" : "compiled.mach-o.dylib");
        case cmTarget::SHARED_LIBRARY:
            return (cmtarget.GetPropertyAsBool("FRAMEWORK") ? "wrapper.framework" : "compiled.mach-o.dylib");
        case cmTarget::EXECUTABLE:
    ...
    switch (cmtarget.GetType())
    {
        ...
        case cmTarget::MODULE_LIBRARY:
            if (cmtarget.IsCFBundleOnApple())
                ...
                return ((this->XcodeVersion >= 22) ? "com.apple.product-type.tool" : "com.apple.product-type.library.dynamic");
        case cmTarget::SHARED_LIBRARY:
            return (cmtarget.GetPropertyAsBool("FRAMEWORK") ? "com.apple.product-type.framework" : "com.apple.product-type.library.dynamic");
        case cmTarget::EXECUTABLE:
            return (cmtarget.GetPropertyAsBool("MACOSX_BUNDLE") ? "com.apple.product-type.application" : "com.apple.product-type.tool");
        default:
            break;
    }
    return 0;
...
//----------------------------------------------------------------------------
cmXCodeObject *cmGlobalXCodeGenerator::CreateXCodeTarget(cmTarget & cmtarget, cmXCodeObject * buildPhases)
{
    if (cmtarget.GetType() == cmTarget::INTERFACE_LIBRARY)
    {
        return 0;
...
cmXCodeObject *cmGlobalXCodeGenerator::FindXCodeTarget(cmTarget const *t)
{
    ...
    for (std::vector < cmXCodeObject * >::iterator i = this->XCodeObjects.begin(); i != this->XCodeObjects.end(); ++i)
    {
        cmXCodeObject *o = *i;
        if (o->GetTarget() == t)
        {
...
std::string cmGlobalXCodeGenerator::GetOrCreateId(const char *name, const char *id)
{
    ...
    this->CMakeInstance->AddCacheEntry(guidStoreName.c_str(), id, "Stored Xcode object GUID", cmCacheManager::INTERNAL);

    return id;
}

//----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::AddDependTarget(cmXCodeObject * target, cmXCodeObject * dependTarget)
{
    // This is called once for every edge in the target dependency graph.
    cmXCodeObject *container = this->CreateObject(cmXCodeObject::PBXContainerItemProxy);
    container->SetComment("PBXContainerItemProxy");
    container->AddAttribute("containerPortal", this->CreateObjectReference(this->RootObject));
    container->AddAttribute("proxyType", this->CreateString("1"));
    container->AddAttribute("remoteGlobalIDString", this->CreateObjectReference(dependTarget));
    container->AddAttribute("remoteInfo", this->CreateString(dependTarget->GetTarget()->GetName()));
    cmXCodeObject *targetdep = this->CreateObject(cmXCodeObject::PBXTargetDependency);
    targetdep->SetComment("PBXTargetDependency");
    targetdep->AddAttribute("target", this->CreateObjectReference(dependTarget));
    targetdep->AddAttribute("targetProxy", this->CreateObjectReference(container));

    cmXCodeObject *depends = target->GetObject("dependencies");
    if (!depends)
    {
        cmSystemTools::Error("target does not have dependencies attribute error..");
...
//----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::AppendOrAddBuildSetting(cmXCodeObject * settings, const char *attribute, const char *value)
{
    if (settings)
    {
...
//----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::AppendBuildSettingAttribute(cmXCodeObject * target, const char *attribute, const char *value, const char *configName)
{
    if (this->XcodeVersion < 21)
    {
        // There is only one configuration.  Add the setting to the buildSettings
        // of the target.
        this->AppendOrAddBuildSetting(target->GetObject("buildSettings"), attribute, value);
    }
    else
    {
...
void cmGlobalXCodeGenerator::AddDependAndLinkInformation(cmXCodeObject * target)
{
    ...
    for (std::vector < std::string >::iterator i = this->CurrentConfigurationTypes.begin(); i != this->CurrentConfigurationTypes.end(); ++i)
    {
        ...
        if (this->XcodeVersion >= 50)
        {
            ...
            this->AppendBuildSettingAttribute(target, "OTHER_LDFLAGS", linkObjs.c_str(), configName);
        }

        // Skip link information for object libraries.
        if (cmtarget->GetType() == cmTarget::OBJECT_LIBRARY || cmtarget->GetType() == cmTarget::STATIC_LIBRARY)
        {
            continue;
        }
        ...
        // Add dependencies directly on library files.
        {
            ...
            for (std::vector < std::string >::const_iterator j = libDeps.begin(); j != libDeps.end(); ++j)
            {
        ...
        // add the library search paths
        {
            ...
            std::string linkDirs;
            for (std::vector < std::string >::const_iterator libDir = libDirs.begin(); libDir != libDirs.end(); ++libDir)
            {
            ...
            this->AppendBuildSettingAttribute(target, "LIBRARY_SEARCH_PATHS", linkDirs.c_str(), configName);
        }
        ...
        // now add the link libraries
        {
            ...
            for (ItemVector::const_iterator li = libNames.begin(); li != libNames.end(); ++li)
            {
                linkLibs += sep;
                sep = " ";
                if (li->IsPath)
                ...
                else if (!li->Target || li->Target->GetType() != cmTarget::INTERFACE_LIBRARY)
                {
                    linkLibs += li->Value;
                }
            ...
            this->AppendBuildSettingAttribute(target, "OTHER_LDFLAGS", linkLibs.c_str(), configName);
        }
    }
}

//----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::CreateGroups(cmLocalGenerator * root, std::vector < cmLocalGenerator * >& generators)
{
    for (std::vector < cmLocalGenerator * >::iterator i = generators.begin(); i != generators.end(); ++i)
    {
        if (this->IsExcluded(root, *i))
        {
            continue;
        ...
        for (cmTargets::iterator l = tgts.begin(); l != tgts.end(); l++)
        {
            ...
            for (std::vector < cmSourceFile * >::const_iterator s = classes.begin(); s != classes.end(); s++)
            {
                cmSourceFile *sf = *s;
                // Add the file to the list of sources.
                std::string const &source = sf->GetFullPath();
                cmSourceGroup *sourceGroup = mf->FindSourceGroup(source.c_str(), sourceGroups);
...
cmXCodeObject *cmGlobalXCodeGenerator::CreatePBXGroup(cmXCodeObject * parent, cmStdString name)
{
    cmXCodeObject *parentChildren = NULL;
    if (parent)
        parentChildren = parent->GetObject("children");
    cmXCodeObject *group = this->CreateObject(cmXCodeObject::PBXGroup);
    cmXCodeObject *groupChildren = this->CreateObject(cmXCodeObject::OBJECT_LIST);
...
//----------------------------------------------------------------------------
cmXCodeObject *cmGlobalXCodeGenerator::CreateOrGetPBXGroup(cmTarget & cmtarget, cmSourceGroup * sg)
{
    cmStdString s;
    cmStdString target;
    ...
    s = target + "/";
    s += sg->GetFullName();
    std::map < cmStdString, cmXCodeObject * >::iterator it = this->GroupNameMap.find(s);
...
void cmGlobalXCodeGenerator::CreateXCodeObjects(cmLocalGenerator * root, std::vector < cmLocalGenerator * >& generators)
{
    ...
    else
    {
        ...
        {
            cmXCodeObject *buildStyle = this->CreateObject(cmXCodeObject::PBXBuildStyle);
    ...
    this->ResourcesGroupChildren = this->CreateObject(cmXCodeObject::OBJECT_LIST);
    ...
    cmXCodeObject *productGroupChildren = this->CreateObject(cmXCodeObject::OBJECT_LIST);
    ...
    group = this->CreateObject(cmXCodeObject::ATTRIBUTE_GROUP);
    this->RootObject->AddAttribute("mainGroup", this->CreateObjectReference(mainGroup));
    this->RootObject->AddAttribute("buildSettings", group);
    this->RootObject->AddAttribute("buildStyles", listObjs);
    this->RootObject->AddAttribute("hasScannedForEncodings", this->CreateString("0"));
    if (this->XcodeVersion >= 30)
    {
        group = this->CreateObject(cmXCodeObject::ATTRIBUTE_GROUP);
    ...
    {
        std::string pdir = this->RelativeToBinary(root->GetMakefile()->GetCurrentDirectory());
    ...
    for (std::vector < cmXCodeObject * >::iterator c = configs.begin(); c != configs.end(); ++c)
    {
    ...
    configlist->SetComment(comment.c_str());
    configlist->AddAttribute("defaultConfigurationIsVisible", this->CreateString("0"));
    configlist->AddAttribute("defaultConfigurationName", this->CreateString(defaultConfigName));
    cmXCodeObject *buildSettings = this->CreateObject(cmXCodeObject::ATTRIBUTE_GROUP);
    const char *osxArch = this->CurrentMakefile->GetDefinition("CMAKE_OSX_ARCHITECTURES");
    ...
    const char *sysroot = this->CurrentMakefile->GetDefinition("CMAKE_OSX_SYSROOT");
    const char *deploymentTarget = this->CurrentMakefile->GetDefinition("CMAKE_OSX_DEPLOYMENT_TARGET");
    if (osxArch && sysroot)
    {
        ...
        const char *sep = "";
        for (std::vector < std::string >::iterator i = this->Architectures.begin(); i != this->Architectures.end(); ++i)
        {
            archString += sep;
            archString += *i;
            sep = " ";
        }
        buildSettings->AddAttribute("ARCHS", this->CreateString(archString.c_str()));
    }
    if (deploymentTarget && *deploymentTarget)
    {
        buildSettings->AddAttribute("MACOSX_DEPLOYMENT_TARGET", this->CreateString(deploymentTarget));
    }
    if (!this->PlatformToolset.empty())
    {
        buildSettings->AddAttribute("GCC_VERSION", this->CreateString(this->PlatformToolset.c_str()));
    }
    ...
    // Convert "CMAKE_XCODE_ATTRIBUTE_*" variables directly.
    {
        ...
        for (std::vector < std::string >::const_iterator i = vars.begin(); i != vars.end(); ++i)
        {
            ...
            {
                buildSettings->AddAttribute(i->substr(22).c_str(), this->CreateString(this->CurrentMakefile->GetDefinition(i->c_str())));
            }
        }
    }
    ...
    for (std::vector < cmXCodeObject * >::iterator i = configs.begin(); i != configs.end(); ++i)
    {
        (*i)->AddAttribute("buildSettings", buildSettings);
    }

    this->RootObject->AddAttribute("buildConfigurationList", this->CreateObjectReference(configlist));
    ...
    for (std::vector < cmXCodeObject * >::iterator i = targets.begin(); i != targets.end(); ++i)
    {
        cmXCodeObject *t = *i;
...
//----------------------------------------------------------------------------
std::string cmGlobalXCodeGenerator::GetObjectsNormalDirectory(const std::string & projName, const std::string & configName, const cmTarget * t) const
{
    std::string dir = t->GetMakefile()->GetCurrentOutputDirectory();
    dir += "/";
    dir += projName;
    dir += ".build/";
    dir += configName;
...
//----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::CreateXCodeDependHackTarget(std::vector < cmXCodeObject * >&targets)
{
    cmGeneratedFileStream makefileStream(this->CurrentXCodeHackMakefile.c_str());
    if (!makefileStream)
    {
        cmSystemTools::Error("Could not create", this->CurrentXCodeHackMakefile.c_str());
        return;
    }
    makefileStream.SetCopyIfDifferent(true);
    ...
    for (std::vector < cmXCodeObject * >::iterator i = targets.begin(); i != targets.end(); ++i)
    {
        ...
        std::map < cmStdString, cmXCodeObject::StringVec > const &deplibs = target->GetDependLibraries();
        for (std::map < cmStdString, cmXCodeObject::StringVec >::const_iterator ci = deplibs.begin(); ci != deplibs.end(); ++ci)
        {
            for (cmXCodeObject::StringVec::const_iterator d = ci->second.begin(); d != ci->second.end(); ++d)
            {
    ...
    makefileStream << "# Rules to remove targets that are older than anything to which they\n" "# link.  This forces Xcode to relink the targets from scratch.  It\n" "# does not seem to check these dependencies itself.\n";
    for (std::vector < std::string >::const_iterator ct = this->CurrentConfigurationTypes.begin(); ct != this->CurrentConfigurationTypes.end(); ++ct)
    {
...
//         t->GetType() == cmTarget::OBJECT_LIBRARY ||
                t->GetType() == cmTarget::STATIC_LIBRARY || t->GetType() == cmTarget::SHARED_LIBRARY || t->GetType() == cmTarget::MODULE_LIBRARY)
            {
                // Declare an entry point for the target post-build phase.
                makefileStream << this->PostBuildMakeTarget(t->GetName(), *ct) << ":\n";
            }

            if (t->GetType() == cmTarget::EXECUTABLE || t->GetType() == cmTarget::SHARED_LIBRARY || t->GetType() == cmTarget::MODULE_LIBRARY)
            {
                std::string tfull = t->GetFullPath(configName);
                std::string trel = this->ConvertToRelativeForMake(tfull.c_str());

                // Add this target to the post-build phases of its dependencies.
                std::map < cmStdString, cmXCodeObject::StringVec >::const_iterator y = target->GetDependTargets().find(*ct);
                if (y != target->GetDependTargets().end())
                {
                    std::vector < cmStdString > const &deptgts = y->second;
                    for (std::vector < cmStdString >::const_iterator d = deptgts.begin(); d != deptgts.end(); ++d)
                    {
                        makefileStream << this->PostBuildMakeTarget(*d, *ct) << ": " << trel << "\n";
                    }
                }
                ...
                std::map < cmStdString, cmXCodeObject::StringVec >::const_iterator x = target->GetDependLibraries().find(*ct);
                if (x != target->GetDependLibraries().end())
                {
                    std::vector < cmStdString > const &deplibs = x->second;
                    for (std::vector < cmStdString >::const_iterator d = deplibs.begin(); d != deplibs.end(); ++d)
                    {
                ...
                {
                    std::string universal = this->GetObjectsNormalDirectory(this->CurrentProject, configName, t);
                    for (std::vector < std::string >::iterator arch = this->Architectures.begin(); arch != this->Architectures.end(); ++arch)
                    {
                        std::string universalFile = universal;
                        universalFile += *arch;
                        universalFile += "/";
                        universalFile += t->GetFullName(configName);
                        makefileStream << "\t/bin/rm -f " << this->ConvertToRelativeForMake(universalFile.c_str()) << "\n";
                    }
                }
                makefileStream << "\n\n";
            }
...
//----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::OutputXCodeProject(cmLocalGenerator * root, std::vector < cmLocalGenerator * >& generators)
{
    ...
    for (std::vector < cmLocalGenerator * >::iterator g = generators.begin(); g != generators.end(); ++g)
    {
    ...
    this->CreateXCodeObjects(root, generators);
...
//----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::WriteXCodePBXProj(std::ostream & fout, cmLocalGenerator *, std::vector < cmLocalGenerator * >&)
{
    fout << "// !$*UTF8*$!\n";
    fout << "{\n";
...
{
    return this->XcodeVersion >= 21 ? "$(CONFIGURATION)$(EFFECTIVE_PLATFORM_NAME)" : ".";
}

std::string cmGlobalXCodeGenerator::ExpandCFGIntDir(const std::string & str, const std::string & config) const
{
    std::string replace1 = "$(CONFIGURATION)$(EFFECTIVE_PLATFORM_NAME)";
    std::string replace2 = "$(CONFIGURATION)";

    std::string tmp = str;
    for (std::string::size_type i = tmp.find(replace1); i != std::string::npos; i = tmp.find(replace1, i))
    {
        tmp.replace(i, replace1.size(), config);
        i += config.size();
    }
    for (std::string::size_type i = tmp.find(replace2); i != std::string::npos; i = tmp.find(replace2, i))
    {
...
std::string cmGlobalXCodeGenerator::ConvertToRelativeForMake(const char *p)
{
    ...
    {
        std::string ret = this->CurrentLocalGenerator->ConvertToRelativePath(this->CurrentOutputDirectoryComponents, p);
...
std::string cmGlobalXCodeGenerator::ConvertToRelativeForXCode(const char *p)
{
    ...
    {
        std::string ret = this->CurrentLocalGenerator->ConvertToRelativePath(this->ProjectOutputDirectoryComponents, p);
        return cmSystemTools::ConvertToOutputPath(ret.c_str());
    }
}
...
std::string cmGlobalXCodeGenerator::RelativeToSource(const char *p)
{
    ...
    return this->CurrentLocalGenerator->ConvertToRelativePath(this->ProjectSourceDirectoryComponents, p, true);
}

//----------------------------------------------------------------------------
std::string cmGlobalXCodeGenerator::RelativeToBinary(const char *p)
{
    return this->CurrentLocalGenerator->ConvertToRelativePath(this->ProjectOutputDirectoryComponents, p);
}
...
//----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::AppendDirectoryForConfig(const char *prefix, const char *config, const char *suffix, std::string & dir)
{
    if (this->XcodeVersion > 20)
    {
        if (config)
...
//----------------------------------------------------------------------------
std::string cmGlobalXCodeGenerator::LookupFlags(const char *varNamePrefix, const char *varNameLang, const char *varNameSuffix, const char *default_flags)
{
    if (varNameLang)
    {
        ...
        if (const char *varValue = this->CurrentMakefile->GetDefinition(varName.c_str()))
        {
            if (*varValue)
            {
                return varValue;
...
//----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::AppendDefines(BuildObjectListOrString & defs, const char *defines_list, bool dflag)
{
...
//----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::AppendDefines(BuildObjectListOrString & defs, std::vector < std::string > const &defines, bool dflag)
{
    // GCC_PREPROCESSOR_DEFINITIONS is a space-separated list of definitions.
    std::string def;
    for (std::vector < std::string >::const_iterator di = defines.begin(); di != defines.end(); ++di)
    {
...
//----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::AppendFlag(std::string & flags, std::string const &flag)
{
    ...
    bool quoteFlag = flag.find_first_of("`~!@#$%^&*()+={}[]|:;\"'<>,.? ") != flag.npos;
...
//----------------------------------------------------------------------------
std::string cmGlobalXCodeGenerator::ComputeInfoPListLocation(cmTarget & target)
{
...
bool cmGlobalXCodeGenerator::IsMultiConfig()
{
 ...
 //----------------------------------------------------------------------------
void cmGlobalXCodeGenerator::ComputeTargetObjects(cmGeneratorTarget * gt) const
{
    ...
    for (std::vector < cmSourceFile * >::const_iterator si = objectSources.begin(); si != objectSources.end(); ++si)
    {
        cmSourceFile *sf = *si;
        std::string objectName = cmSystemTools::GetFilenameWithoutLastExtension(sf->GetFullPath());
        objectName += ".o";
