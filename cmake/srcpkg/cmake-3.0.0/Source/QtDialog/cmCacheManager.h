...
public:
    ...
    enum CacheEntryType
    { BOOL = 0, PATH, FILEPATH, STRING, INTERNAL, STATIC,
        UNINITIALIZED
    };
...
private:
    struct CacheEntry
    {
        ...
        void AppendProperty(const char *property, const char *value, bool asString = false);
        bool Initialized;
...
public:
        ...
        {
            return this->GetEntry().Value.c_str();
        }
        bool GetValueAsBool() const;
        void SetValue(const char *);
        CacheEntryType GetType() const
        {
            return this->GetEntry().Type;
        }
        void SetType(CacheEntryType ty)
        {
            this->GetEntry().Type = ty;
        }
        bool Initialized()
        {
            return this->GetEntry().Initialized;
        }
...
private:
        CacheEntry const &GetEntry() const
        {
   ...
   */
    ...
    bool LoadCache(const char *path, bool internal, std::set < cmStdString > &excludes, std::set < cmStdString > &includes);
    ...
    static bool ParseEntry(const char *entry, std::string & var, std::string & value, CacheEntryType & type);
...
protected:
    ...
    void AddCacheEntry(const char *key, const char *value, const char *helpString, CacheEntryType type);
...
private:
    ...
    static void OutputHelpString(std::ostream & fout, const std::string & helpString);
