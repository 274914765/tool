...
#    ifdef CMAKE_BUILD_WITH_CMAKE
   ...
   */
    static int HTTPRequest(std::string url, HTTPMethod method, std::string & response, std::string fields = "", std::string putFile = "", int timeout = 0);
#    endif
   ...
   */
    ...
    void PopulateCustomVector(cmMakefile * mf, const char *definition, VectorOfStrings & vec);
    void PopulateCustomInteger(cmMakefile * mf, const char *def, int &val);
   ...
   */
    ...
    bool OpenOutputFile(const std::string & path, const std::string & name, cmGeneratedFileStream & stream, bool compress = false);

    ///! Should we only show what we would do?
    bool GetShowOnly();

    bool ShouldUseHTTP10()
    {
        return this->UseHTTP10;
    }

    bool ShouldPrintLabels()
    {
    ...
    void SetScheduleType(std::string type)
    {
   ...
   */
    bool RunCommand(const char *command, std::string * stdOut, std::string * stdErr, int *retVal = 0, const char *dir = 0, double timeout = 0.0);
    ...
    int RunMakeCommand(const char *command, std::string * output, int *retVal, const char *dir, int timeout, std::ostream & ofs);
    ...
    int RunTest(std::vector < const char *>args, std::string * output, int *retVal, std::ostream * logfile, double testTimeOut, std::vector < std::string > *environment);
  ...
  /** Decode a URL to the original string.  */
    ...
    int GetDropSiteCDash()
    {
        return this->DropSiteCDash;
    }
    ...
    void SetBatchJobs(bool batch = true)
    {
        this->BatchJobs = batch;
    }
    bool GetBatchJobs()
    {
...
private:
   ...
   */
    ...
    void HandleCommandLineArguments(size_t & i, std::vector < std::string > &args);
    ...
    int GenerateCTestNotesOutput(std::ostream & os, const VectorOfStrings & files);
