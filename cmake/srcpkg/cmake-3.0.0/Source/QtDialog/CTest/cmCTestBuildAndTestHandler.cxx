...
{
    this->BuildTargets.erase(this->BuildTargets.begin(), this->BuildTargets.end());
...
//----------------------------------------------------------------------
int cmCTestBuildAndTestHandler::RunCMake(std::string * outstring, cmOStringStream & out, std::string & cmakeOutString, std::string & cwd, cmake * cm)
{
    unsigned int k;
...
int cmCTestBuildAndTestHandler::RunCMakeAndTest(std::string * outstring)
{
    ...
    if (!this->BuildGenerator.size())
    {
        if (outstring)
        {
            *outstring = "--build-and-test requires that the generator " "be provided using the --build-generator " "command line option. ";
        }
        return 1;
    }

    if (this->CTest->GetConfigType().size() == 0 && this->ConfigSample.size())
    {
        ...
        fullPath = cmCTestTestHandler::FindExecutable(this->CTest, this->ConfigSample.c_str(), resultingConfig, extraPaths, failed);
        if (fullPath.size() && resultingConfig.size())
        {
            this->CTest->SetConfigType(resultingConfig.c_str());
        }
        out << "Using config sample with results: " << fullPath << " and " << resultingConfig << std::endl;
    }
    ...
    out << "Internal cmake changing into directory: " << this->BinaryDir << std::endl;
    ...
    if (this->BuildNoCMake)
    {
        ...
        cm.SetGlobalGenerator(cm.CreateGlobalGenerator(this->BuildGenerator.c_str()));
    ...
    for (tarIt = this->BuildTargets.begin(); tarIt != this->BuildTargets.end(); ++tarIt)
    {
        ...
        int retVal = cm.GetGlobalGenerator()->Build(this->SourceDir.c_str(), this->BinaryDir.c_str(),
    ...
    std::vector < std::string > failed;
    fullPath = cmCTestTestHandler::FindExecutable(this->CTest, this->TestCommand.c_str(), resultingConfig, extraPaths, failed);
...
//----------------------------------------------------------------------
int cmCTestBuildAndTestHandler::ProcessCommandLineArguments(const std::string & currentArg, size_t & idx, const std::vector < std::string > &allArgs)
{
    ...
    if (currentArg.find("--build-and-test", 0) == 0 && idx < allArgs.size() - 1)
    {
        if (idx + 2 < allArgs.size())
        {
            ...
            cmSystemTools::MakeDirectory(this->BinaryDir.c_str());
            this->BinaryDir = cmSystemTools::CollapseFullPath(this->BinaryDir.c_str());
            this->SourceDir = cmSystemTools::CollapseFullPath(this->SourceDir.c_str());
        }
        else
        {
            cmCTestLog(this->CTest, ERROR_MESSAGE, "--build-and-test must have source and binary dir" << std::endl);
            return 0;
        }
    }
    ...
    if (currentArg.find("--build-config-sample", 0) == 0 && idx < allArgs.size() - 1)
    {
        idx++;
    ...
    {
        while (idx + 1 < allArgs.size() && allArgs[idx + 1] != "--build-target" && allArgs[idx + 1] != "--test-command")
        {
            ++idx;
