...
void cmCTestLaunch::ComputeFileNames()
{
    ...
    for (std::vector < std::string >::const_iterator ai = this->RealArgs.begin(); ai != this->RealArgs.end(); ++ai)
    {
...
//----------------------------------------------------------------------------
bool cmCTestLaunch::SourceMatches(std::string const &lhs, std::string const &rhs)
{
...
void cmCTestLaunch::WriteXML()
{
    ...
    fxml << "\t<Failure type=\"" << (this->IsError()? "Error" : "Warning") << "\">\n";
...
void cmCTestLaunch::WriteXMLAction(std::ostream & fxml)
{
    ...
    {
        fxml << "\t\t\t<TargetName>" << cmXMLSafe(this->OptionTargetName) << "</TargetName>\n";
    }

    // Language
    if (!this->OptionLanguage.empty())
    {
        fxml << "\t\t\t<Language>" << cmXMLSafe(this->OptionLanguage) << "</Language>\n";
    }
    ...
    if (!this->OptionSource.empty())
    {
        ...
        if (cmSystemTools::FileIsFullPath(this->SourceDir.c_str()) && cmSystemTools::FileIsFullPath(source.c_str()) && cmSystemTools::IsSubDirectory(source.c_str(), this->SourceDir.c_str()))
        {
            source = cmSystemTools::RelativePath(this->SourceDir.c_str(), source.c_str());
        }

        fxml << "\t\t\t<SourceFile>" << cmXMLSafe(source) << "</SourceFile>\n";
    }

    // OutputFile
    if (!this->OptionOutput.empty())
    {
        fxml << "\t\t\t<OutputFile>" << cmXMLSafe(this->OptionOutput) << "</OutputFile>\n";
    }
    ...
    if (outputType)
    {
        fxml << "\t\t\t<OutputType>" << cmXMLSafe(outputType) << "</OutputType>\n";
    }
...
void cmCTestLaunch::WriteXMLCommand(std::ostream & fxml)
{
    ...
    if (!this->CWD.empty())
    {
        fxml << "\t\t\t<WorkingDirectory>" << cmXMLSafe(this->CWD) << "</WorkingDirectory>\n";
    }
    for (std::vector < std::string >::const_iterator ai = this->RealArgs.begin(); ai != this->RealArgs.end(); ++ai)
    {
        fxml << "\t\t\t<Argument>" << cmXMLSafe(ai->c_str()) << "</Argument>\n";
    }
    fxml << "\t\t</Command>\n";
}
...
void cmCTestLaunch::WriteXMLResult(std::ostream & fxml)
{
    ...
    switch (cmsysProcess_GetState(cp))
    {
        case cmsysProcess_State_Starting:
            fxml << "No process has been executed";
            break;
        case cmsysProcess_State_Executing:
            fxml << "The process is still executing";
            break;
        case cmsysProcess_State_Disowned:
            fxml << "Disowned";
            break;
        case cmsysProcess_State_Killed:
            fxml << "Killed by parent";
            break;

        case cmsysProcess_State_Expired:
            fxml << "Killed when timeout expired";
            break;
        case cmsysProcess_State_Exited:
            fxml << this->ExitCode;
            break;
        case cmsysProcess_State_Exception:
            fxml << "Terminated abnormally: " << cmXMLSafe(cmsysProcess_GetExceptionString(cp));
            break;
        case cmsysProcess_State_Error:
            fxml << "Error administrating child process: " << cmXMLSafe(cmsysProcess_GetErrorString(cp));
            break;
    };
    fxml << "</ExitCondition>\n";
...
void cmCTestLaunch::WriteXMLLabels(std::ostream & fxml)
{
    ...
    if (!this->Labels.empty())
    {
        ...
        fxml << "\t\t<Labels>\n";
        for (std::set < cmStdString >::const_iterator li = this->Labels.begin(); li != this->Labels.end(); ++li)
        {
...
//----------------------------------------------------------------------------
void cmCTestLaunch::DumpFileToXML(std::ostream & fxml, std::string const &fname)
{
    cmsys::ifstream fin(fname.c_str(), std::ios::in | std::ios::binary);
...
bool cmCTestLaunch::CheckResults()
{
    ...
    if ((this->HaveErr && this->ScrapeLog(this->LogErr)) || (this->HaveOut && this->ScrapeLog(this->LogOut)))
    {
        return false;
    }
    return true;
...
//----------------------------------------------------------------------------
void cmCTestLaunch::LoadScrapeRules(const char *purpose, std::vector < cmsys::RegularExpression > &regexps)
{
...
bool cmCTestLaunch::ScrapeLog(std::string const &fname)
{
    ...
    while (cmSystemTools::GetLineFromStream(fin, line))
    {
        ...
        if (this->Match(line.c_str(), this->RegexWarning) && !this->Match(line.c_str(), this->RegexWarningSuppress))
        {
            return true;
        }
    }
...
//----------------------------------------------------------------------------
bool cmCTestLaunch::Match(std::string const &line, std::vector < cmsys::RegularExpression > &regexps)
{
    for (std::vector < cmsys::RegularExpression >::iterator ri = regexps.begin(); ri != regexps.end(); ++ri)
    {
...
{
    if (this->OptionFilterPrefix.size() && cmSystemTools::StringStartsWith(line.c_str(), this->OptionFilterPrefix.c_str()))
    {
        return true;
    }
    return false;
...
int cmCTestLaunch::Main(int argc, const char *const argv[])
{
    if (argc == 2)
    {
        std::cerr << "ctest --launch: this mode is for internal CTest use only" << std::endl;
        return 1;
    }
...
void cmCTestLaunch::LoadConfig()
{
    ...
    if (cmSystemTools::FileExists(fname.c_str()) && mf->ReadListFile(0, fname.c_str()))
    {
