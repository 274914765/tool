...
//----------------------------------------------------------------------------
static unsigned int cmCTestGITVersion(unsigned int epic, unsigned int major, unsigned int minor, unsigned int fix)
{
...
{
public:
    OneLineParser(cmCTestGIT * git, const char *prefix, std::string & l):Line1(l)
    {
...
std::string cmCTestGIT::FindTopDir()
{
    ...
    OutputLogger rev_parse_err(this->Log, "rev-parse-err> ");
    if (this->RunChild(git_rev_parse, &rev_parse_out, &rev_parse_err) && !cdup.empty())
    {
        top_dir += "/";
        top_dir += cdup;
...
bool cmCTestGIT::UpdateByCustom(std::string const &custom)
{
    ...
    for (std::vector < std::string >::const_iterator i = git_custom_command.begin(); i != git_custom_command.end(); ++i)
    {
...
unsigned int cmCTestGIT::GetGitVersion()
{
    if (!this->CurrentGitVersion)
    {
        ...
        if (this->RunChild(git_version, &version_out, &version_err) && sscanf(version.c_str(), "git version %u.%u.%u.%u", &v[0], &v[1], &v[2], &v[3]) >= 3)
        {
...
{
public:
    DiffParser(cmCTestGIT * git, const char *prefix):LineParser('\0', false), GIT(git), DiffField(DiffFieldNone)
    {
        this->SetLog(&git->Log, prefix);
    }
...
{
public:
    CommitParser(cmCTestGIT * git, const char *prefix):DiffParser(git, prefix), Section(SectionHeader)
    {
...
private:
    ...
    enum SectionType
    { SectionHeader, SectionBody, SectionDiff, SectionCount };
    ...
    std::string FormatDateTime(Person const &person)
    {
        ...
        char dt[1024];
        sprintf(dt, "%04d-%02d-%02d %02d:%02d:%02d", t->tm_year + 1900, t->tm_mon + 1, t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec);
        std::string out = dt;
...
void cmCTestGIT::LoadRevisions()
{
    ...
    const char *git = this->CommandLineTool.c_str();
    const char *git_rev_list[] = { git, "rev-list", "--reverse", range.c_str(), "--", 0 };
    const char *git_diff_tree[] = { git, "diff-tree", "--stdin", "--always", "-z", "-r", "--pretty=raw",
        "--encoding=utf-8", 0
    };
...
void cmCTestGIT::LoadModifications()
{
    ...
    for (std::vector < Change >::const_iterator ci = out.Changes.begin(); ci != out.Changes.end(); ++ci)
    {
