...
{
    this->CTest->PopulateCustomVector(mf, "CTEST_CUSTOM_ERROR_MATCH", this->CustomErrorMatches);
    this->CTest->PopulateCustomVector(mf, "CTEST_CUSTOM_ERROR_EXCEPTION", this->CustomErrorExceptions);
    this->CTest->PopulateCustomVector(mf, "CTEST_CUSTOM_WARNING_MATCH", this->CustomWarningMatches);
    this->CTest->PopulateCustomVector(mf, "CTEST_CUSTOM_WARNING_EXCEPTION", this->CustomWarningExceptions);
    this->CTest->PopulateCustomInteger(mf, "CTEST_CUSTOM_MAXIMUM_NUMBER_OF_ERRORS", this->MaxErrors);
    this->CTest->PopulateCustomInteger(mf, "CTEST_CUSTOM_MAXIMUM_NUMBER_OF_WARNINGS", this->MaxWarnings);
    ...
    if (const char *customWarningMatchers = mf->GetDefinition("CTEST_CUSTOM_WARNING_MATCH"))
    {
        cmSystemTools::ExpandListArgument(customWarningMatchers, this->ReallyCustomWarningMatches);
    }
    if (const char *customWarningExceptions = mf->GetDefinition("CTEST_CUSTOM_WARNING_EXCEPTION"))
    {
        cmSystemTools::ExpandListArgument(customWarningExceptions, this->ReallyCustomWarningExceptions);
    }
}
...
std::string cmCTestBuildHandler::GetMakeCommand()
{
    ...
    cmCTestLog(this->CTest, HANDLER_VERBOSE_OUTPUT, "MakeCommand:" << makeCommand << "\n");

    std::string configType = this->CTest->GetConfigType();
    if (configType == "")
    {
        configType = this->CTest->GetCTestConfiguration("DefaultCTestConfigurationType");
    }
    if (configType == "")
    {
        configType = "Release";
    ...
    cmSystemTools::ReplaceString(makeCommand, "${CTEST_CONFIGURATION_TYPE}", configType.c_str());
...
int cmCTestBuildHandler::ProcessHandler()
{
    ...
    int entry;
    for (entry = 0; cmCTestWarningErrorFileLine[entry].RegularExpressionString; ++entry)
    {
        ...
        {
            cmCTestLog(this->CTest, ERROR_MESSAGE, "Problem Compiling regular expression: " << cmCTestWarningErrorFileLine[entry].RegularExpressionString << std::endl);
        }
    }
    ...
    {
        cmCTestLog(this->CTest, ERROR_MESSAGE, "Cannot find MakeCommand key in the DartConfiguration.tcl" << std::endl);
        return -1;
    }
    ...
    std::string const &useLaunchers = this->CTest->GetCTestConfiguration("UseLaunchers");
    ...
    {
        cmCTestLog(this->CTest, ERROR_MESSAGE, "Cannot create build log file" << std::endl);
    }
...
#define cmCTestBuildHandlerPopulateRegexVector(strings, regexes) \
  ...
  for ( it = strings.begin(); it != strings.end(); ++it ) \
    ...
    cmCTestBuildHandlerPopulateRegexVector(this->CustomErrorMatches, this->ErrorMatchRegex);
    cmCTestBuildHandlerPopulateRegexVector(this->CustomErrorExceptions, this->ErrorExceptionRegex);
    cmCTestBuildHandlerPopulateRegexVector(this->CustomWarningMatches, this->WarningMatchRegex);
    cmCTestBuildHandlerPopulateRegexVector(this->CustomWarningExceptions, this->WarningExceptionRegex);
    ...
    {
        cmCTestLog(this->CTest, DEBUG, "Build with command: " << makeCommand << std::endl);
    }
    ...
    {
        for (evit = this->ErrorsAndWarnings.begin(); evit != this->ErrorsAndWarnings.end(); ++evit)
        {
            cmSystemTools::ReplaceString(evit->Text, this->SimplifySourceDir.c_str(), "/.../");
            cmSystemTools::ReplaceString(evit->PreContext, this->SimplifySourceDir.c_str(), "/.../");
            cmSystemTools::ReplaceString(evit->PostContext, this->SimplifySourceDir.c_str(), "/.../");
        }
    }
    ...
    if (!this->SimplifyBuildDir.empty())
    {
        for (evit = this->ErrorsAndWarnings.begin(); evit != this->ErrorsAndWarnings.end(); ++evit)
        {
            ...
            cmSystemTools::ReplaceString(evit->PreContext, this->SimplifyBuildDir.c_str(), "/.../");
            cmSystemTools::ReplaceString(evit->PostContext, this->SimplifyBuildDir.c_str(), "/.../");
        }
    }
    ...
    {
        cmCTestLog(this->CTest, ERROR_MESSAGE, "Cannot create build XML file" << std::endl);
        return -1;
    }
    ...
    {
        cmCTestLog(this->CTest, ERROR_MESSAGE, "Error(s) when building project" << std::endl);
    }

    // Display message about number of errors and warnings
    cmCTestLog(this->CTest, HANDLER_OUTPUT, "   " << this->TotalErrors << (this->TotalErrors >= this->MaxErrors ? " or more" : "") << " Compiler errors" << std::endl);
    cmCTestLog(this->CTest, HANDLER_OUTPUT, "   " << this->TotalWarnings << (this->TotalWarnings >= this->MaxWarnings ? " or more" : "") << " Compiler warnings" << std::endl);
...
public:
    ...
    bool operator() (std::string const &l, std::string const &r)
    {
        ...
        int result;
        if (this->FTC->FileTimeCompare(l.c_str(), r.c_str(), &result) && result != 0)
        {
            return result < 0;
        }
        else
...
void cmCTestBuildHandler::GenerateXMLLaunched(std::ostream & os)
{
    ...
    for (Fragments::const_iterator fi = fragments.begin(); fi != fragments.end(); ++fi)
    {
...
void cmCTestBuildHandler::GenerateXMLLogScraped(std::ostream & os)
{
    ...
    srcdir += "/";
    for (it = ew.begin(); it != ew.end() && (numErrorsAllowed || numWarningsAllowed); it++)
    {
        cmCTestBuildErrorWarning *cm = &(*it);
        if ((cm->Error && numErrorsAllowed) || (!cm->Error && numWarningsAllowed))
        {
            if (cm->Error)
            {
                numErrorsAllowed--;
            ...
            for (rit = this->ErrorWarningFileLineRegex.begin(); rit != this->ErrorWarningFileLineRegex.end(); ++rit)
            {
                ...
                if (re->find(cm->Text.c_str()))
                {
                    ...
                    if (cm->SourceFile.find("/.../") != cm->SourceFile.npos)
                    {
                        ...
                        {
                            cm->SourceFile = cm->SourceFile.substr(p + 1, cm->SourceFile.size() - p);
                        }
                    }
                    else
                    {
                        // make sure it is a full path with the correct case
                        cm->SourceFile = cmSystemTools::CollapseFullPath(cm->SourceFile.c_str());
            ...
            if (!cm->SourceFile.empty() && cm->LineNumber >= 0)
            {
                ...
                {
                    os << "\t\t<SourceFile>" << cm->SourceFile << "</SourceFile>" << std::endl;
                }
                if (cm->SourceFileTail.size() > 0)
                {
                    os << "\t\t<SourceFileTail>" << cm->SourceFileTail << "</SourceFileTail>" << std::endl;
                }
                if (cm->LineNumber >= 0)
                {
                    os << "\t\t<SourceLineNumber>" << cm->LineNumber << "</SourceLineNumber>" << std::endl;
                }
            }
            os << "\t\t<PreContext>" << cmXMLSafe(cm->PreContext).Quotes(false) << "</PreContext>\n" << "\t\t<PostContext>" << cmXMLSafe(cm->PostContext).Quotes(false);
...
//----------------------------------------------------------------------------
void cmCTestBuildHandler::GenerateXMLFooter(std::ostream & os, double elapsed_build_time)
{
    os << "\t<Log Encoding=\"base64\" Compression=\"/bin/gzip\">\n\t</Log>\n"
        << "\t<EndDateTime>" << this->EndBuild << "</EndDateTime>\n"
        << "\t<EndBuildTime>" << static_cast < unsigned int >(this->EndBuildTime) << "</EndBuildTime>\n" << "<ElapsedMinutes>" << static_cast < int >(elapsed_build_time / 6) / 10.0 << "</ElapsedMinutes>" << "</Build>" << std::endl;
    this->CTest->EndXML(os);
}

//----------------------------------------------------------------------------
void cmCTestBuildHandler::GenerateXMLLaunchedFragment(std::ostream & os, const char *fname)
{
...
{
    // error-{hash}.xml
    return (cmHasLiteralPrefix(fname, "error-") && strcmp(fname + strlen(fname) - 4, ".xml") == 0);
}
...
{
    // warning-{hash}.xml
    return (cmHasLiteralPrefix(fname, "warning-") && strcmp(fname + strlen(fname) - 4, ".xml") == 0);
}
...
private:
    ...
    void WriteScrapeMatchers(const char *purpose, std::vector < std::string > const &matchers);
};
...
{
    this->WriteScrapeMatchers("Warning", this->Handler->ReallyCustomWarningMatches);
    this->WriteScrapeMatchers("WarningSuppress", this->Handler->ReallyCustomWarningExceptions);
...
//----------------------------------------------------------------------------
void cmCTestBuildHandler::LaunchHelper::WriteScrapeMatchers(const char *purpose, std::vector < std::string > const &matchers)
{
    if (matchers.empty())
    {
        return;
    ...
    fname += ".txt";
    cmGeneratedFileStream fout(fname.c_str());
    for (std::vector < std::string >::const_iterator mi = matchers.begin(); mi != matchers.end(); ++mi)
    {
        fout << *mi << "\n";
    }
}
...
//----------------------------------------------------------------------
int cmCTestBuildHandler::RunMakeCommand(const char *command, int *retVal, const char *dir, int timeout, std::ostream & ofs)
{
    ...
    char *data;
    int length;
    cmCTestLog(this->CTest, HANDLER_OUTPUT, "   Each symbol represents " << tick_len << " bytes of output." << std::endl << (this->UseCTestLaunch ? "" : "   '!' represents an error and '*' a warning.\n") << "    " << std::flush);
    ...
    while ((res = cmsysProcess_WaitForData(cp, &data, &length, 0)))
    {
        ...
        {
            this->ProcessBuffer(data, length, tick, tick_len, ofs, &this->BuildProcessingErrorQueue);
        }
        else
        {
            this->ProcessBuffer(data, length, tick, tick_len, ofs, &this->BuildProcessingQueue);
        }
    }

    this->ProcessBuffer(0, 0, tick, tick_len, ofs, &this->BuildProcessingQueue);
    this->ProcessBuffer(0, 0, tick, tick_len, ofs, &this->BuildProcessingErrorQueue);
    cmCTestLog(this->CTest, OUTPUT, " Size of output: " << ((this->BuildOutputLogSize + 512) / 1024) << "K" << std::endl);
    ...
    if (result == cmsysProcess_State_Exited)
    {
        if (retVal)
        {
            *retVal = cmsysProcess_GetExitValue(cp);
            cmCTestLog(this->CTest, HANDLER_VERBOSE_OUTPUT, "Command exited with the value: " << *retVal << std::endl);
            ...
            if (*retVal)
            {
                ...
                errorwarning.Text = "*** WARNING non-zero return value in ctest from: ";
    ...
    else if (result == cmsysProcess_State_Exception)
    {
        if (retVal)
        {
            *retVal = cmsysProcess_GetExitException(cp);
            cmCTestLog(this->CTest, WARNING, "There was an exception: " << *retVal << std::endl);
        }
    }
    ...
    else if (result == cmsysProcess_State_Error)
    {
        ...
        this->ErrorsAndWarnings.push_back(errorwarning);
        this->TotalErrors++;
        cmCTestLog(this->CTest, ERROR_MESSAGE, "There was an error: " << cmsysProcess_GetErrorString(cp) << std::endl);
    }
...
//----------------------------------------------------------------------
void cmCTestBuildHandler::ProcessBuffer(const char *data, int length, size_t & tick, size_t tick_len, std::ostream & ofs, t_BuildProcessingQueueType * queue)
{
    ...
    while (1)
    {
        ...
        if (it != queue->end())
        {
            ...
            if (found)
            {
                ...
                for (pcit = this->PreContext.begin(); pcit != this->PreContext.end(); ++pcit)
                {
            ...
            else
            {
                ...
                if (this->ErrorsAndWarnings.size() && this->LastErrorOrWarning != this->ErrorsAndWarnings.end() && this->PostContextCount < this->MaxPostContext)
                {
                ...
                else
                {
                    ...
                    {
                        this->PreContext.erase(this->PreContext.begin(), this->PreContext.end() - this->MaxPreContext);
                    }
                }
            }
    ...
    while (this->BuildOutputLogSize > (tick * tick_len))
    {
        ...
        {
            cmCTestLog(this->CTest, HANDLER_OUTPUT, "  Size: " << ((this->BuildOutputLogSize + 512) / 1024) << "K" << std::endl << "    ");
        }
    }
    if (tickDisplayed)
    {
...
int cmCTestBuildHandler::ProcessSingleLine(const char *data)
{
    ...
    if (!this->ErrorQuotaReached)
    {
        // Errors
        int wrxCnt = 0;
        for (it = this->ErrorMatchRegex.begin(); it != this->ErrorMatchRegex.end(); ++it)
        {
            if (it->find(data))
            {
                errorLine = 1;
                cmCTestLog(this->CTest, DEBUG, "  Error Line: " << data << " (matches: " << this->CustomErrorMatches[wrxCnt] << ")" << std::endl);
                break;
            }
            wrxCnt++;
        }
        // Error exceptions
        wrxCnt = 0;
        for (it = this->ErrorExceptionRegex.begin(); it != this->ErrorExceptionRegex.end(); ++it)
        {
            if (it->find(data))
            {
                errorLine = 0;
                cmCTestLog(this->CTest, DEBUG, "  Not an error Line: " << data << " (matches: " << this->CustomErrorExceptions[wrxCnt] << ")" << std::endl);
                break;
            }
            wrxCnt++;
        }
    ...
    if (!this->WarningQuotaReached)
    {
        // Warnings
        int wrxCnt = 0;
        for (it = this->WarningMatchRegex.begin(); it != this->WarningMatchRegex.end(); ++it)
        {
            if (it->find(data))
            {
                warningLine = 1;
                cmCTestLog(this->CTest, DEBUG, "  Warning Line: " << data << " (matches: " << this->CustomWarningMatches[wrxCnt] << ")" << std::endl);
                break;
            }
            wrxCnt++;
        }
        ...
        wrxCnt = 0;
        // Warning exceptions
        for (it = this->WarningExceptionRegex.begin(); it != this->WarningExceptionRegex.end(); ++it)
        {
            if (it->find(data))
            {
                warningLine = 0;
                cmCTestLog(this->CTest, DEBUG, "  Not a warning Line: " << data << " (matches: " << this->CustomWarningExceptions[wrxCnt] << ")" << std::endl);
                break;
            }
            wrxCnt++;
        }
