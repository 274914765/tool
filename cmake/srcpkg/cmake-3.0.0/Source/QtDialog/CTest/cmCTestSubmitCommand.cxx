...
{
    const char *ctestDropMethod = this->Makefile->GetDefinition("CTEST_DROP_METHOD");
    const char *ctestDropSite = this->Makefile->GetDefinition("CTEST_DROP_SITE");
    const char *ctestDropLocation = this->Makefile->GetDefinition("CTEST_DROP_LOCATION");
    const char *ctestTriggerSite = this->Makefile->GetDefinition("CTEST_TRIGGER_SITE");
    ...
    this->CTest->SetCTestConfiguration("IsCDash", ctestDropSiteCDash ? "TRUE" : "FALSE");
    ...
    this->CTest->SetCTestConfigurationFromCMakeVariable(this->Makefile, "CurlOptions", "CTEST_CURL_OPTIONS");
    this->CTest->SetCTestConfigurationFromCMakeVariable(this->Makefile, "DropSiteUser", "CTEST_DROP_SITE_USER");
    this->CTest->SetCTestConfigurationFromCMakeVariable(this->Makefile, "DropSitePassword", "CTEST_DROP_SITE_PASSWORD");
    this->CTest->SetCTestConfigurationFromCMakeVariable(this->Makefile, "ScpCommand", "CTEST_SCP_COMMAND");

    const char *notesFilesVariable = this->Makefile->GetDefinition("CTEST_NOTES_FILES");
    if (notesFilesVariable)
    {
        ...
        for (it = notesFiles.begin(); it != notesFiles.end(); ++it)
        {
    ...
    const char *extraFilesVariable = this->Makefile->GetDefinition("CTEST_EXTRA_SUBMIT_FILES");
    if (extraFilesVariable)
    {
        ...
        std::vector < std::string >::iterator it;
        for (it = extraFiles.begin(); it != extraFiles.end(); ++it)
        {
    ...
    cmCTestGenericHandler *handler = this->CTest->GetInitializedHandler("submit");
    if (!handler)
    {
    ...
    static_cast < cmCTestSubmitHandler * >(handler)->SetOption("RetryDelay", this->RetryDelay.c_str());
    static_cast < cmCTestSubmitHandler * >(handler)->SetOption("RetryCount", this->RetryCount.c_str());
    static_cast < cmCTestSubmitHandler * >(handler)->SetOption("InternalTest", this->InternalTest ? "ON" : "OFF");
...
bool cmCTestSubmitCommand::CheckArgumentValue(std::string const &arg)
{
    ...
    if (this->ArgumentDoing == ArgumentDoingFiles)
    {
        ...
        {
            cmOStringStream e;
            e << "File \"" << filename << "\" does not exist. Cannot submit " << "a non-existent file.";
            this->Makefile->IssueMessage(cmake::FATAL_ERROR, e.str());
            this->ArgumentDoing = ArgumentDoingError;
        }
        return true;
