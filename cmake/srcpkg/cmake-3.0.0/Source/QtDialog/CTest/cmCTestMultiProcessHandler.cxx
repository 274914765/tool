...
public:
    ...
    {
        return Handler->Properties[index1]->Cost > Handler->Properties[index2]->Cost;
    }
...
cmCTestMultiProcessHandler::~cmCTestMultiProcessHandler()
{
  ...
  // Set the tests
void cmCTestMultiProcessHandler::SetTests(TestMap & tests, PropertiesMap & properties)
{
    this->Tests = tests;
...
void cmCTestMultiProcessHandler::StartTestProcess(int test)
{
    ...
    else
    {
        ...
        for (TestMap::iterator j = this->Tests.begin(); j != this->Tests.end(); ++j)
        {
            j->second.erase(test);
        }
...
{
    for (std::set < std::string >::iterator i = this->Properties[index]->LockedResources.begin(); i != this->Properties[index]->LockedResources.end(); ++i)
    {
...
void cmCTestMultiProcessHandler::EraseTest(int test)
{
    ...
    this->SortedTests.erase(std::find(this->SortedTests.begin(), this->SortedTests.end(), test));
}
...
inline size_t cmCTestMultiProcessHandler::GetProcessorsUsed(int test)
{
    ...
    if (this->Properties[test]->RunSerial || processors > this->ParallelLevel)
    {
...
bool cmCTestMultiProcessHandler::StartTest(int test)
{
    ...
    for (std::set < std::string >::iterator i = this->Properties[test]->LockedResources.begin(); i != this->Properties[test]->LockedResources.end(); ++i)
    {
...
bool cmCTestMultiProcessHandler::CheckOutput()
{
    ...
    std::string out, err;
    for (std::set < cmCTestRunTest * >::const_iterator i = this->RunningTests.begin(); i != this->RunningTests.end(); ++i)
    {
        cmCTestRunTest *p = *i;
        if (!p->CheckOutput())
        {
    ...
    for (std::vector < cmCTestRunTest * >::iterator i = finished.begin(); i != finished.end(); ++i)
    {
        this->Completed++;
        cmCTestRunTest *p = *i;
        int test = p->GetIndex();
...
void cmCTestMultiProcessHandler::UpdateCostData()
{
    ...
    if (cmSystemTools::FileExists(fname.c_str()))
    {
        ...
        while (std::getline(fin, line))
        {
            if (line == "---")
                break;
            std::vector < cmsys::String > parts = cmSystemTools::SplitString(line.c_str(), ' ');
            ...
            else
            {
                ...
                fout << name << " " << this->Properties[index]->PreviousRuns << " " << this->Properties[index]->Cost << "\n";
                temp.erase(index);
            }
        }
        fin.close();
    ...
    {
        fout << i->second->Name << " " << i->second->PreviousRuns << " " << i->second->Cost << "\n";
    }

    // Write list of failed tests
    fout << "---\n";
    for (std::vector < cmStdString >::iterator i = this->Failed->begin(); i != this->Failed->end(); ++i)
    {
        fout << i->c_str() << "\n";
    }
    fout.close();
...
void cmCTestMultiProcessHandler::ReadCostData()
{
    ...
    if (cmSystemTools::FileExists(fname.c_str(), true))
    {
        ...
        while (std::getline(fin, line))
        {
            ...
            if (this->ParallelLevel > 1 && this->Properties[index] && this->Properties[index]->Cost == 0)
            {
...
int cmCTestMultiProcessHandler::SearchByName(std::string name)
{
    ...
    for (PropertiesMap::iterator i = this->Properties.begin(); i != this->Properties.end(); ++i)
    {
...
void cmCTestMultiProcessHandler::CreateParallelTestCostList()
{
    ...
    for (TestMap::const_iterator i = this->Tests.begin(); i != this->Tests.end(); ++i)
    {
        if (std::find(this->LastTestsFailed.begin(), this->LastTestsFailed.end(), this->Properties[i->first]->Name) != this->LastTestsFailed.end())
        {
    ...
    while (priorityStack.back().size())
    {
        ...
        for (TestSet::const_iterator i = previousSet.begin(); i != previousSet.end(); ++i)
        {
            TestSet const &dependencies = this->Tests[*i];
            for (TestSet::const_iterator j = dependencies.begin(); j != dependencies.end(); ++j)
            {
                currentSet.insert(*j);
            }
        }

        for (TestSet::const_iterator i = currentSet.begin(); i != currentSet.end(); ++i)
        {
            previousSet.erase(*i);
        }
    }
    ...
    for (std::list < TestSet >::reverse_iterator i = priorityStack.rbegin(); i != priorityStack.rend(); ++i)
    {
        ...
        for (TestSet::const_iterator j = currentSet.begin(); j != currentSet.end(); ++j)
        {
        ...
        for (TestList::const_iterator j = sortedCopy.begin(); j != sortedCopy.end(); ++j)
        {
...
//---------------------------------------------------------
void cmCTestMultiProcessHandler::GetAllTestDependencies(int test, TestList & dependencies)
{
    TestSet const &dependencySet = this->Tests[test];
    for (TestSet::const_iterator i = dependencySet.begin(); i != dependencySet.end(); ++i)
    {
...
void cmCTestMultiProcessHandler::CreateSerialTestCostList()
{
    ...
    for (TestList::const_iterator i = presortedList.begin(); i != presortedList.end(); ++i)
    {
        int test = *i;
        ...
        for (TestList::const_iterator j = dependencies.begin(); j != dependencies.end(); ++j)
        {
...
{
    std::string fname = this->CTest->GetBinaryDir() + "/Testing/Temporary/CTestCheckpoint.txt";
    std::fstream fout;
...
void cmCTestMultiProcessHandler::PrintTestList()
{
    ...
    for (PropertiesMap::iterator it = this->Properties.begin(); it != this->Properties.end(); ++it)
    {
        count++;
        ...
        for (std::vector < std::string >::iterator label = p.Labels.begin(); label != p.Labels.end(); ++label)
        {
    ...
    cmCTestLog(this->CTest, HANDLER_OUTPUT, std::endl << "Total Tests: " << this->Total << std::endl);
}
...
void cmCTestMultiProcessHandler::PrintLabels()
{
    ...
    for (std::set < std::string >::iterator label = allLabels.begin(); label != allLabels.end(); ++label)
    {
...
void cmCTestMultiProcessHandler::CheckResume()
{
    ...
    if (this->CTest->GetFailover())
    {
        if (cmSystemTools::FileExists(fname.c_str(), true))
        {
            *this->TestHandler->LogFile << "Resuming previously interrupted test set" << std::endl << "----------------------------------------------------------" << std::endl;
...
{
    cmCTestLog(this->CTest, HANDLER_VERBOSE_OUTPUT, "Checking test dependency graph..." << std::endl);
    for (TestMap::iterator it = this->Tests.begin(); it != this->Tests.end(); ++it)
    {
        ...
        while (!s.empty())
        {
            ...
            {
                for (TestSet::iterator d = this->Tests[test].begin(); d != this->Tests[test].end(); ++d)
                {
                    if (*d == root)
                    {
                        //cycle exists
                        cmCTestLog(this->CTest, ERROR_MESSAGE, "Error: a cycle exists in the test dependency graph " "for the test \"" << this->Properties[root]->Name << "\".\nPlease fix the cycle and run ctest again.\n");
                        return false;
                    }
                    else
                    {
    ...
    cmCTestLog(this->CTest, HANDLER_VERBOSE_OUTPUT, "Checking test dependency graph end" << std::endl);
    return true;
}
