...
public:
    ...
    virtual bool IsFunctionBlocked(const cmListFileFunction & lff, cmMakefile & mf, cmExecutionStatus &);
...
bool cmCTestScriptFunctionBlocker::IsFunctionBlocked(const cmListFileFunction &, cmMakefile &, cmExecutionStatus &)
{
...
int cmCTestScriptHandler::ProcessHandler()
{
    ...
    for (size_t i = 0; i < this->ConfigurationScripts.size(); ++i)
    {
        ...
        res += this->RunConfigurationScript(cmSystemTools::CollapseFullPath(this->ConfigurationScripts[i].c_str()), this->ScriptProcessScope[i]);
    }
    if (res)
    {
        return -1;
...
void cmCTestScriptHandler::UpdateElapsedTime()
{
    if (this->LocalGenerator)
    {
        ...
        char timeString[20];
        int itime = static_cast < unsigned int >(cmSystemTools::GetTime() - this->ScriptStartTime);
        sprintf(timeString, "%i", itime);
        this->LocalGenerator->GetMakefile()->AddDefinition("CTEST_ELAPSED_TIME", timeString);
    }
}
...
int cmCTestScriptHandler::ExecuteScript(const std::string & total_script_arg)
{
    ...
    std::vector < cmStdString > &initArgs = this->CTest->GetInitialCommandLineArguments();
    ...
    else if (result == cmsysProcess_State_Exception)
    {
        ...
        cmCTestLog(this->CTest, ERROR_MESSAGE, "\tThere was an exception: " << cmsysProcess_GetExceptionString(cp) << " " << retVal << std::endl);
        failed = true;
    }
    ...
    {
        cmCTestLog(this->CTest, ERROR_MESSAGE, "\tError executing ctest: " << cmsysProcess_GetErrorString(cp) << std::endl);
        failed = true;
    }
    ...
    if (failed)
    {
        ...
        message << result << "] ";
        for (std::vector < const char *>::iterator i = argv.begin(); i != argv.end(); ++i)
        {
            if (*i)
            {
                message << *i << " ";
        ...
        cmCTestLog(this->CTest, ERROR_MESSAGE, message.str() << argv[0] << std::endl);
        return -1;
    }
    return retVal;
}
...
int cmCTestScriptHandler::ReadInScript(const std::string & total_script_arg)
{
    ...
    this->Makefile->AddDefinition("CTEST_SCRIPT_DIRECTORY", cmSystemTools::GetFilenamePath(script).c_str());
    this->Makefile->AddDefinition("CTEST_SCRIPT_NAME", cmSystemTools::GetFilenameName(script).c_str());
    this->Makefile->AddDefinition("CTEST_EXECUTABLE_NAME", cmSystemTools::GetCTestCommand().c_str());
    this->Makefile->AddDefinition("CMAKE_EXECUTABLE_NAME", cmSystemTools::GetCMakeCommand().c_str());
    ...
    std::string systemFile = this->Makefile->GetModulesFile("CTestScriptMode.cmake");
    if (!this->Makefile->ReadListFile(0, systemFile.c_str()) || cmSystemTools::GetErrorOccuredFlag())
    {
        cmCTestLog(this->CTest, ERROR_MESSAGE, "Error in read:" << systemFile.c_str() << "\n");
        return 2;
    }

    // Add definitions of variables passed in on the command line:
    const std::map < std::string, std::string > &defs = this->CTest->GetDefinitions();
    for (std::map < std::string, std::string >::const_iterator it = defs.begin(); it != defs.end(); ++it)
    {
    ...
    {
        cmCTestLog(this->CTest, ERROR_MESSAGE, "Error in read script: " << script.c_str() << std::endl);
...
int cmCTestScriptHandler::ExtractVariables()
{
    ...
    this->SourceDir = this->Makefile->GetSafeDefinition("CTEST_SOURCE_DIRECTORY");
    this->BinaryDir = this->Makefile->GetSafeDefinition("CTEST_BINARY_DIRECTORY");
    ...
    this->InitialCache = this->Makefile->GetSafeDefinition("CTEST_INITIAL_CACHE");
    this->CMakeCmd = this->Makefile->GetSafeDefinition("CTEST_CMAKE_COMMAND");
    this->CMOutFile = this->Makefile->GetSafeDefinition("CTEST_CMAKE_OUTPUT_FILE_NAME");

    this->Backup = this->Makefile->IsOn("CTEST_BACKUP_AND_RESTORE");
    this->EmptyBinDir = this->Makefile->IsOn("CTEST_START_WITH_EMPTY_BINARY_DIRECTORY");
    this->EmptyBinDirOnce = this->Makefile->IsOn("CTEST_START_WITH_EMPTY_BINARY_DIRECTORY_ONCE");
    ...
    if (this->SourceDir.empty() || this->BinaryDir.empty() || this->CTestCmd.empty())
    {
...
int cmCTestScriptHandler::RunConfigurationScript(const std::string & total_script_arg, bool pscope)
{
    ...
    if (pscope)
    {
        cmCTestLog(this->CTest, HANDLER_VERBOSE_OUTPUT, "Reading Script: " << total_script_arg << std::endl);
        result = this->ReadInScript(total_script_arg);
    }
    else
    {
        cmCTestLog(this->CTest, HANDLER_VERBOSE_OUTPUT, "Executing Script: " << total_script_arg << std::endl);
...
int cmCTestScriptHandler::RunCurrentScript()
{
    ...
    if (this->ContinuousDuration >= 0)
    {
        ...
        do
        {
            ...
            {
                this->SleepInSeconds(static_cast < unsigned int >(this->MinimumInterval - interval));
            }
            if (this->EmptyBinDirOnce)
            {
                this->EmptyBinDir = false;
...
int cmCTestScriptHandler::CheckOutSourceDir()
{
    ...
    if (!cmSystemTools::FileExists(this->SourceDir.c_str()) && !this->CVSCheckOut.empty())
    {
        // we must now checkout the src dir
        output = "";
        cmCTestLog(this->CTest, HANDLER_VERBOSE_OUTPUT, "Run cvs: " << this->CVSCheckOut << std::endl);
        res = cmSystemTools::RunSingleCommand(this->CVSCheckOut.c_str(), &output, &retVal, this->CTestRoot.c_str(), this->HandlerVerbose, 0 /*this->TimeOut */ );
        if (!res || retVal != 0)
        {
...
int cmCTestScriptHandler::PerformExtraUpdates()
{
    ...
    for (it = this->ExtraUpdates.begin(); it != this->ExtraUpdates.end(); ++it)
    {
        ...
        if (cvsArgs.size() == 2)
        {
            ...
            output = "";
            retVal = 0;
            cmCTestLog(this->CTest, HANDLER_VERBOSE_OUTPUT, "Run Update: " << fullCommand.c_str() << std::endl);
            res = cmSystemTools::RunSingleCommand(fullCommand.c_str(), &output, &retVal, cvsArgs[0].c_str(), this->HandlerVerbose, 0 /*this->TimeOut */ );
            if (!res || retVal != 0)
            {
...
int cmCTestScriptHandler::RunConfigurationDashboard()
{
    ...
    if (this->EmptyBinDir)
    {
        ...
        {
            cmCTestLog(this->CTest, ERROR_MESSAGE, "Problem removing the binary directory" << std::endl);
        }
    }

    // make sure the binary directory exists if it isn't the srcdir
    if (!cmSystemTools::FileExists(this->BinaryDir.c_str()) && this->SourceDir != this->BinaryDir)
    {
    ...
    if (this->EmptyBinDir && this->SourceDir == this->BinaryDir)
    {
        ...
        if (this->CVSCheckOut.empty())
        {
            cmSystemTools::Error("You have specified the source and binary "
                                 ...
                                 "specified that the binary directory is to be erased. This means " "that the source will have to be checked out from CVS. But you have " "not specified CTEST_CVS_CHECKOUT");
            return 8;
        }
    ...
    {
        if (!this->WriteInitialCache(this->BinaryDir.c_str(), this->InitialCache.c_str()))
        {
            this->RestoreBackupDirectories();
            return 9;
        }
    ...
    if (!this->CMakeCmd.empty())
    {
        ...
        command += "\"";
        retVal = 0;
        cmCTestLog(this->CTest, HANDLER_VERBOSE_OUTPUT, "Run cmake command: " << command.c_str() << std::endl);
        res = cmSystemTools::RunSingleCommand(command.c_str(), &output, &retVal, this->BinaryDir.c_str(), this->HandlerVerbose, 0 /*this->TimeOut */ );
        ...
        if (!this->CMOutFile.empty())
        {
            ...
            cmCTestLog(this->CTest, HANDLER_VERBOSE_OUTPUT, "Write CMake output to file: " << cmakeOutputFile.c_str() << std::endl);
            ...
            {
                cmCTestLog(this->CTest, ERROR_MESSAGE, "Cannot open CMake output file: " << cmakeOutputFile.c_str() << " for writing" << std::endl);
            }
        }
        if (!res || retVal != 0)
        {
    ...
    for (unsigned i = 0; i < ctestCommands.size(); ++i)
    {
        ...
        output = "";
        retVal = 0;
        cmCTestLog(this->CTest, HANDLER_VERBOSE_OUTPUT, "Run ctest command: " << command.c_str() << std::endl);
        res = cmSystemTools::RunSingleCommand(command.c_str(), &output, &retVal, this->BinaryDir.c_str(), this->HandlerVerbose, 0 /*this->TimeOut */ );
        ...
        if (!res || cmakeFailed || retVal & cmCTest::BUILD_ERRORS)
        {
            ...
            if (cmakeFailed)
            {
                cmCTestLog(this->CTest, ERROR_MESSAGE, "Unable to run cmake:" << std::endl << cmakeFailedOuput.c_str() << std::endl);
                return 10;
            }
            cmCTestLog(this->CTest, ERROR_MESSAGE, "Unable to run ctest:" << std::endl << "command: " << command.c_str() << std::endl << "output: " << output.c_str() << std::endl);
            if (!res)
            {
                return 11;
            }
...
//-------------------------------------------------------------------------
bool cmCTestScriptHandler::WriteInitialCache(const char *directory, const char *text)
{
...
bool cmCTestScriptHandler::RunScript(cmCTest * ctest, const char *sname, bool InProcess, int *returnValue)
{
...
//-------------------------------------------------------------------------
bool cmCTestScriptHandler::TryToRemoveBinaryDirectoryOnce(const std::string & directoryPath)
{
    ...
    for (unsigned long i = 0; i < directory.GetNumberOfFiles(); ++i)
    {
        ...
        bool isDirectory = cmSystemTools::FileIsDirectory(fullPath.c_str()) && !cmSystemTools::FileIsSymlink(fullPath.c_str());
