...
{
    const char *ctestTimeout = this->Makefile->GetDefinition("CTEST_TEST_TIMEOUT");
    ...
    if (this->Values[ctt_START] || this->Values[ctt_END] || this->Values[ctt_STRIDE])
    {
    ...
    {
        handler->SetOption("ExcludeLabelRegularExpression", this->Values[ctt_EXCLUDE_LABEL]);
    }
    if (this->Values[ctt_INCLUDE_LABEL])
    {
        handler->SetOption("LabelRegularExpression", this->Values[ctt_INCLUDE_LABEL]);
    }
