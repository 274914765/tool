...
// cmQTWrapCPPCommand
bool cmQTWrapCPPCommand::InitialPass(std::vector < std::string > const &argsIn, cmExecutionStatus &)
{
    if (argsIn.size() < 3)
    {
    ...
    for (std::vector < std::string >::iterator j = (args.begin() + 2); j != args.end(); ++j)
    {
        ...
        if (!(curr && curr->GetPropertyAsBool("WRAP_EXCLUDE")))
        {
            ...
            newName += srcName;
            newName += ".cxx";
            cmSourceFile *sf = this->Makefile->GetOrCreateSource(newName.c_str(), true);
            if (curr)
            {
            ...
            this->Makefile->AddCustomCommandToOutput(newName.c_str(), depends, no_main_dependency, commandLines, "Qt Wrapped File", no_working_dir);
        }
    }
