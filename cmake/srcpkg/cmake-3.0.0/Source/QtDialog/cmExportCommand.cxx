...
cmExportCommand::cmExportCommand():cmCommand(), ArgumentGroup(), Targets(&Helper, "TARGETS"), Append(&Helper, "APPEND", &ArgumentGroup), ExportSetName(&Helper, "EXPORT", &ArgumentGroup), Namespace(&Helper, "NAMESPACE", &ArgumentGroup), Filename(&Helper, "FILE", &ArgumentGroup),
...
// cmExportCommand
bool cmExportCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 2)
    {
    ...
    else
    {
        ...
        if (cmSystemTools::GetFilenameLastExtension(this->Filename.GetCString()) != ".cmake")
        {
            cmOStringStream e;
            e << "FILE option given filename \"" << this->Filename.GetString() << "\" which does not have an extension of \".cmake\".\n";
    ...
    if (cmSystemTools::FileIsFullPath(fname.c_str()))
    {
        ...
        {
            cmOStringStream e;
            e << "FILE option given filename \"" << fname << "\" which is in the source tree.\n";
            this->SetError(e.str().c_str());
            return false;
        }
    }
    ...
    cmGlobalGenerator *gg = this->Makefile->GetLocalGenerator()->GetGlobalGenerator();
    ...
    if (args[0] == "EXPORT")
    {
        ...
        {
            cmOStringStream e;
            e << "EXPORT signature does not recognise the " "EXPORT_LINK_INTERFACE_LIBRARIES option.";
            this->SetError(e.str().c_str());
            return false;
        }
    ...
    {
        for (std::vector < std::string >::const_iterator currentTarget = this->Targets.GetVector().begin(); currentTarget != this->Targets.GetVector().end(); ++currentTarget)
        {
            if (this->Makefile->IsAlias(*currentTarget))
            {
                cmOStringStream e;
                e << "given ALIAS target \"" << *currentTarget << "\" which may not be exported.";
                this->SetError(e.str().c_str());
                return false;
            }
            ...
            if (cmTarget * target = gg->FindTarget(0, currentTarget->c_str()))
            {
                ...
                {
                    cmOStringStream e;
                    e << "given OBJECT library \"" << *currentTarget << "\" which may not be exported.";
                    this->SetError(e.str().c_str());
                    return false;
                }
            }
            ...
            {
                cmOStringStream e;
                e << "given target \"" << *currentTarget << "\" which is not built by this project.";
                this->SetError(e.str().c_str());
                return false;
            }
    ...
    {
        for (std::vector < std::string >::const_iterator ci = configurationTypes.begin(); ci != configurationTypes.end(); ++ci)
        {
...
bool cmExportCommand::HandlePackage(std::vector < std::string > const &args)
{
    ...
    {
        cmOStringStream e;
        e << "PACKAGE given invalid package name \"" << package << "\".  " << "Package names must match \"" << packageExpr << "\".";
        this->SetError(e.str().c_str());
        return false;
    }
...
//----------------------------------------------------------------------------
void cmExportCommand::ReportRegistryError(std::string const &msg, std::string const &key, long err)
{
    cmOStringStream e;
    e << msg << "\n" << "  HKEY_CURRENT_USER\\" << key << "\n";
    wchar_t winmsg[1024];
    if (FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, 0, err, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), winmsg, 1024, 0) > 0)
    {
...
//----------------------------------------------------------------------------
void cmExportCommand::StorePackageRegistryWin(std::string const &package, const char *content, const char *hash)
{
    ...
    err = RegSetValueExW(hKey, cmsys::Encoding::ToWide(hash).c_str(), 0, REG_SZ, (BYTE const *) wcontent.c_str(), static_cast < DWORD > (wcontent.size() + 1) * sizeof(wchar_t));
    RegCloseKey(hKey);
    if (err != ERROR_SUCCESS)
    {
        cmOStringStream msg;
...
#else
//----------------------------------------------------------------------------
void cmExportCommand::StorePackageRegistryDir(std::string const &package, const char *content, const char *hash)
{
#    if defined(__HAIKU__)
