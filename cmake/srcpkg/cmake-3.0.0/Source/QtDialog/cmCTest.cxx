...
struct tm *cmCTest::GetNightlyTime(std::string str, bool tomorrowtag)
{
    ...
    sprintf(buf, "%d%02d%02d %s", lctime->tm_year + 1900, lctime->tm_mon + 1, lctime->tm_mday, str.c_str());
    cmCTestLog(this, OUTPUT, "Determine Nightly Start Time" << std::endl << "   Specified time: " << str.c_str() << std::endl);
    ...
    cmCTestLog(this, DEBUG, "   Current time: " << tctime << " Nightly time: " << ntime << std::endl);
    if (tomorrowtag)
    {
...
//----------------------------------------------------------------------------
static size_t HTTPResponseCallback(void *ptr, size_t size, size_t nmemb, void *data)
{
...
//----------------------------------------------------------------------------
int cmCTest::HTTPRequest(std::string url, HTTPMethod method, std::string & response, std::string fields, std::string putFile, int timeout)
{
    CURL *curl;
    FILE *file;
...
std::string cmCTest::MakeURLSafe(const std::string & str)
{
    ...
    for (std::string::size_type pos = 0; pos < str.size(); pos++)
    {
        ...
        if ((ch > 126 || ch < 32 || ch == '&' || ch == '%' || ch == '+' || ch == '=' || ch == '@') && ch != 9)
        {
...
bool cmCTest::ShouldCompressTestOutput()
{
    if (!this->ComputedCompressTestOutput)
    {
        ...
        //version >= 1.6?
        bool cdashSupportsGzip = cmSystemTools::VersionCompare(cmSystemTools::OP_GREATER, cdashVersion.c_str(), "1.6") || cmSystemTools::VersionCompare(cmSystemTools::OP_EQUAL,
...
bool cmCTest::ShouldCompressMemCheckOutput()
{
    if (!this->ComputedCompressMemCheckOutput)
    {
        ...
        bool compressionSupported = cmSystemTools::VersionCompare(cmSystemTools::OP_GREATER, cdashVersion.c_str(), "1.9.0");
...
int cmCTest::Initialize(const char *binary_dir, cmCTestStartCommand * command)
{
    ...
    {
        cmCTestLog(this, DEBUG, "Here: " << __LINE__ << std::endl);
        cmCTestLog(this, OUTPUT, "   Site: " << this->GetCTestConfiguration("Site") << std::endl << "   Build name: " << this->GetCTestConfiguration("BuildName") << std::endl);
        cmCTestLog(this, DEBUG, "Produce XML is on" << std::endl);
        if (this->TestModel == cmCTest::NIGHTLY && this->GetCTestConfiguration("NightlyStartTime").empty())
        {
            cmCTestLog(this, WARNING, "WARNING: No nightly start time found please set in" " CTestConfig.cmake or DartConfig.cmake" << std::endl);
            cmCTestLog(this, DEBUG, "Here: " << __LINE__ << std::endl);
            return 0;
        }
    }
    ...
    if (this->ProduceXML)
    {
        ...
        if (cmSystemTools::FileExists(testingDir.c_str()))
        {
            ...
            {
                cmCTestLog(this, ERROR_MESSAGE, "File " << testingDir << " is in the place of the testing directory" << std::endl);
                return 0;
            }
        }
        else
        {
            ...
            {
                cmCTestLog(this, ERROR_MESSAGE, "Cannot create directory " << testingDir << std::endl);
                return 0;
            }
        }
        ...
        if (createNewTag)
        {
            ...
            if (tfin && cmSystemTools::GetLineFromStream(tfin, tag))
            {
                ...
                int hour = 0;
                int min = 0;
                sscanf(tag.c_str(), "%04d%02d%02d-%02d%02d", &year, &mon, &day, &hour, &min);
                if (year != lctime->tm_year + 1900 || mon != lctime->tm_mon + 1 || day != lctime->tm_mday)
                {
                    tag = "";
                }
                std::string tagmode;
            ...
            if (tag.size() == 0 || (0 != command) || this->Parts[PartStart])
            {
                ...
                {
                    lctime = this->GetNightlyTime(this->GetCTestConfiguration("NightlyStartTime"), this->TomorrowTag);
                }
                char datestring[100];
                sprintf(datestring, "%04d%02d%02d-%02d%02d", lctime->tm_year + 1900, lctime->tm_mon + 1, lctime->tm_mday, lctime->tm_hour, lctime->tm_min);
                tag = datestring;
                ...
                if (0 == command)
                {
                    cmCTestLog(this, OUTPUT, "Create new tag: " << tag << " - " << this->GetTestModelString() << std::endl);
                }
            }
        }
        else
        {
            ...
            if (tag.empty())
            {
                cmCTestLog(this, ERROR_MESSAGE, "Cannot read existing TAG file in " << testingDir << std::endl);
                return 0;
            }

            cmCTestLog(this, OUTPUT, "  Use existing tag: " << tag << " - " << this->GetTestModelString() << std::endl);
        }
...
bool cmCTest::InitializeFromCommand(cmCTestStartCommand * command)
{
    ...
    {
        cmCTestLog(this, WARNING, "Cannot locate CTest configuration: in BuildDirectory: " << bld_dir_fname.c_str() << std::endl);
        cmCTestLog(this, WARNING, "Cannot locate CTest configuration: in SourceDirectory: " << src_dir_fname.c_str() << std::endl);
    }
...
bool cmCTest::UpdateCTestConfiguration()
{
    ...
    cmCTestLog(this, HANDLER_VERBOSE_OUTPUT, "UpdateCTestConfiguration  from :" << fileName.c_str() << "\n");
    if (!cmSystemTools::FileExists(fileName.c_str()))
    {
    ...
    if (this->ProduceXML)
    {
        this->CompressXMLFiles = cmSystemTools::IsOn(this->GetCTestConfiguration("CompressSubmission").c_str());
    }
    return true;
}
...
//----------------------------------------------------------------------
bool cmCTest::OpenOutputFile(const std::string & path, const std::string & name, cmGeneratedFileStream & stream, bool compress)
{
    std::string testingDir = this->BinaryDir + "/Testing";
    if (path.size() > 0)
    {
...
{
    std::string testingDir = this->BinaryDir + "/Testing/" + this->CurrentTag + "/" + filename;
...
int cmCTest::ProcessTests()
{
    ...
    if (this->Parts[PartUpdate] && (this->GetRemainingTimeAllowed() - 120 > 0))
    {
        ...
        uphandler->SetPersistentOption("SourceDirectory", this->GetCTestConfiguration("SourceDirectory").c_str());
    ...
    if (this->Parts[PartConfigure] && (this->GetRemainingTimeAllowed() - 120 > 0))
    {
    ...
    if ((this->Parts[PartTest] || notest) && (this->GetRemainingTimeAllowed() - 120 > 0))
    {
        this->UpdateCTestConfiguration();
    ...
    if (this->Parts[PartCoverage] && (this->GetRemainingTimeAllowed() - 120 > 0))
    {
        this->UpdateCTestConfiguration();
    ...
    if (this->Parts[PartMemCheck] && (this->GetRemainingTimeAllowed() - 120 > 0))
    {
        this->UpdateCTestConfiguration();
    ...
    if (!notest)
    {
        ...
        if (cmSystemTools::FileIsDirectory(notes_dir.c_str()))
        {
            ...
            for (kk = 0; kk < d.GetNumberOfFiles(); kk++)
            {
                ...
                if (cmSystemTools::FileExists(fullname.c_str()) && !cmSystemTools::FileIsDirectory(fullname.c_str()))
                {
...
//----------------------------------------------------------------------
int cmCTest::RunMakeCommand(const char *command, std::string * output, int *retVal, const char *dir, int timeout, std::ostream & ofs)
{
    ...
    for (std::vector < cmStdString >::const_iterator a = args.begin(); a != args.end(); ++a)
    {
    ...
    cmCTestLog(this, OUTPUT, " Size of output: " << int (double (output->size()) / 1024.0) << "K" << std::endl);
...
//----------------------------------------------------------------------
int cmCTest::RunTest(std::vector < const char *>argv, std::string * output, int *retVal, std::ostream * log, double testTimeOut, std::vector < std::string > *environment)
{
    ...
    cmCTestLog(this, HANDLER_VERBOSE_OUTPUT, "Test timeout computed to be: " << timeout << "\n");
    if (cmSystemTools::SameFile(argv[0], cmSystemTools::GetCTestCommand().c_str()) && !this->ForceNewCTestProcess)
    {
        cmCTest inst;
        ...
        *output += oss.str();
        ...
        cmCTestLog(this, HANDLER_VERBOSE_OUTPUT, "Internal cmCTest object used to run test." << std::endl << *output << std::endl);
...
void cmCTest::StartXML(std::ostream & ostr, bool append)
{
    ...
    {
        cmCTestLog(this, ERROR_MESSAGE, "Current Tag empty, this may mean" " NightlStartTime was not set correctly." << std::endl);
    ...
    std::string buildname = cmCTest::SafeBuildIdField(this->GetCTestConfiguration("BuildName"));
    std::string stamp = cmCTest::SafeBuildIdField(this->CurrentTag + "-" + this->GetTestModelString());
    std::string site = cmCTest::SafeBuildIdField(this->GetCTestConfiguration("Site"));
...
void cmCTest::AddSiteProperties(std::ostream & ostr)
{
    ...
    if (subproject)
    {
        ...
        const char *labels = ch->GetCMake()->GetProperty("SubProjectLabels", cmProperty::GLOBAL);
        if (labels)
        {
            ostr << "  <Labels>\n";
            std::string l = labels;
...
//----------------------------------------------------------------------
int cmCTest::GenerateCTestNotesOutput(std::ostream & os, const cmCTest::VectorOfStrings & files)
{
    ...
    os << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
        ...
        << "\" BuildStamp=\"" << this->CurrentTag << "-" << this->GetTestModelString() << "\" Name=\"" << this->GetCTestConfiguration("Site") << "\" Generator=\"ctest" << cmVersion::GetCMakeVersion() << "\">\n";
...
std::string cmCTest::Base64GzipEncodeFile(std::string file)
{
    ...
    {
        cmCTestLog(this, ERROR_MESSAGE, "Error creating tar while " "encoding file: " << file << std::endl);
        return "";
    }
...
bool cmCTest::SubmitExtraFiles(const std::vector < cmStdString > &files)
{
    ...
    for (it = files.begin(); it != files.end(); ++it)
    {
        ...
        {
            cmCTestLog(this, ERROR_MESSAGE, "Cannot find extra file: " << it->c_str() << " to submit." << std::endl;
                );
            return false;
        }
...
{
    cmCTestLog(this, ERROR_MESSAGE, "CTest -D called with incorrect option: " << val << std::endl);
    ...
    cmCTestLog(this, ERROR_MESSAGE,
               ...
               << "  ctest -D Nightly" << std::endl << "  ctest -D Nightly(Start|Update|Configure|Build)" << std::endl << "  ctest -D Nightly(Test|Coverage|MemCheck|Submit)" << std::endl << "  ctest -D NightlyMemoryCheck" << std::endl);
}
...
void cmCTest::HandleCommandLineArguments(size_t & i, std::vector < std::string > &args)
{
    ...
    {
        i++;
        this->GetHandler("test")->SetPersistentOption("TestsToRunInformation", args[i].c_str());
        this->GetHandler("memcheck")->SetPersistentOption("TestsToRunInformation", args[i].c_str());
    }
    ...
    {
        i++;
        this->GetHandler("test")->SetPersistentOption("IncludeRegularExpression", args[i].c_str());
        this->GetHandler("memcheck")->SetPersistentOption("IncludeRegularExpression", args[i].c_str());
    }
    if (this->CheckArgument(arg, "-L", "--label-regex") && i < args.size() - 1)
    {
        i++;
        this->GetHandler("test")->SetPersistentOption("LabelRegularExpression", args[i].c_str());
        this->GetHandler("memcheck")->SetPersistentOption("LabelRegularExpression", args[i].c_str());
    }
    if (this->CheckArgument(arg, "-LE", "--label-exclude") && i < args.size() - 1)
    {
        i++;
        this->GetHandler("test")->SetPersistentOption("ExcludeLabelRegularExpression", args[i].c_str());
        this->GetHandler("memcheck")->SetPersistentOption("ExcludeLabelRegularExpression", args[i].c_str());
    }

    if (this->CheckArgument(arg, "-E", "--exclude-regex") && i < args.size() - 1)
    {
        i++;
        this->GetHandler("test")->SetPersistentOption("ExcludeRegularExpression", args[i].c_str());
        this->GetHandler("memcheck")->SetPersistentOption("ExcludeRegularExpression", args[i].c_str());
    }
...
void cmCTest::HandleScriptArguments(size_t & i, std::vector < std::string > &args, bool & SRArgumentSpecified)
{
    std::string arg = args[i];
...
int cmCTest::Run(std::vector < std::string > &args, std::string * output)
{
    ...
    for (size_t i = 1; i < args.size(); ++i)
    {
        ...
        if (this->CheckArgument(arg, "-T", "--test-action") && (i < args.size() - 1))
        {
            ...
            if (!this->SetTest(args[i].c_str(), false))
            {
                ...
                cmCTestLog(this, ERROR_MESSAGE, "Available options are:" << std::endl
                           ...
                           << "  " << ctestExec << " -T coverage" << std::endl << "  " << ctestExec << " -T memcheck" << std::endl << "  " << ctestExec << " -T notes" << std::endl << "  " << ctestExec << " -T submit" << std::endl);
            }
        }
        ...
        if (this->CheckArgument(arg, "-M", "--test-model") && (i < args.size() - 1))
        {
            ...
            else
            {
                ...
                cmCTestLog(this, ERROR_MESSAGE, "Available options are:" << std::endl << "  " << ctestExec << " -M Continuous" << std::endl << "  " << ctestExec << " -M Experimental" << std::endl << "  " << ctestExec << " -M Nightly" << std::endl);
            }
        }
    ...
    {
        this->Verbose = true;
        cmCTestBuildAndTestHandler *handler = static_cast < cmCTestBuildAndTestHandler * >(this->GetHandler("buildtest"));
        ...
        *output = handler->GetOutput();
        ...
        if (retv != 0)
        {
            cmCTestLog(this, DEBUG, "build and test failing returning: " << retv << std::endl);
        }
        return retv;
    }
    ...
    if (executeTests)
    {
        ...
        if (this->RunConfigurationScript)
        {
            ...
            if (res != 0)
            {
                cmCTestLog(this, DEBUG, "running script failing returning: " << res << std::endl);
            }
        ...
        if (res != 0)
        {
            cmCTestLog(this, DEBUG, "Running a test(s) failed returning : " << res << std::endl);
        }
        return res;
    }
...
int cmCTest::ReadCustomConfigurationFileTree(const char *dir, cmMakefile * mf)
{
    ...
    cmCTestLog(this, DEBUG, "* Read custom CTest configuration directory: " << dir << std::endl);

    std::string fname = dir;
    fname += "/CTestCustom.cmake";
    cmCTestLog(this, DEBUG, "* Check for file: " << fname.c_str() << std::endl);
    if (cmSystemTools::FileExists(fname.c_str()))
    {
        cmCTestLog(this, DEBUG, "* Read custom CTest configuration file: " << fname.c_str() << std::endl);
        bool erroroc = cmSystemTools::GetErrorOccuredFlag();
        cmSystemTools::ResetErrorOccuredFlag();

        if (!mf->ReadListFile(0, fname.c_str()) || cmSystemTools::GetErrorOccuredFlag())
        {
            cmCTestLog(this, ERROR_MESSAGE, "Problem reading custom configuration: " << fname.c_str() << std::endl);
        }
        found = true;
        if (erroroc)
        {
    ...
    cmCTestLog(this, DEBUG, "* Check for file: " << rexpr.c_str() << std::endl);
    if (!found && cmSystemTools::FileExists(rexpr.c_str()))
    {
        ...
        for (fileIt = files.begin(); fileIt != files.end(); ++fileIt)
        {
            ...
            if (!mf->ReadListFile(0, fileIt->c_str()) || cmSystemTools::GetErrorOccuredFlag())
            {
    ...
    if (found)
    {
        ...
        for (it = this->TestingHandlers.begin(); it != this->TestingHandlers.end(); ++it)
        {
            cmCTestLog(this, DEBUG, "* Read custom CTest configuration vectors for handler: " << it->first.c_str() << " (" << it->second << ")" << std::endl);
...
//----------------------------------------------------------------------
void cmCTest::PopulateCustomVector(cmMakefile * mf, const char *def, VectorOfStrings & vec)
{
    if (!def)
    {
        return;
...
{
    const std::string & sourceDir = cmSystemTools::CollapseFullPath(this->GetCTestConfiguration("SourceDirectory").c_str());
    const std::string & buildDir = cmSystemTools::CollapseFullPath(this->GetCTestConfiguration("BuildDirectory").c_str());
    std::string fname = cmSystemTools::CollapseFullPath(cfname);

    // Find relative paths to both directories
    std::string srcRelpath = cmSystemTools::RelativePath(sourceDir.c_str(), fname.c_str());
    std::string bldRelpath = cmSystemTools::RelativePath(buildDir.c_str(), fname.c_str());
...
{
    if (this->CTestConfigurationOverwrites.find(name) != this->CTestConfigurationOverwrites.end())
    {
...
void cmCTest::DetermineNextDayStop()
{
    ...
    tzone_offset *= 100;
    char buf[1024];
    sprintf(buf, "%d%02d%02d %s %+05i", lctime->tm_year + 1900, lctime->tm_mon + 1, lctime->tm_mday, this->StopTime.c_str(), tzone_offset);
...
{
    cmCTestLog(this, HANDLER_VERBOSE_OUTPUT, "SetCTestConfiguration:" << name << ":" << (value ? value : "(null)") << "\n");
...
void cmCTest::AddCTestConfigurationOverwrite(const char *encstr)
{
    ...
    {
        cmCTestLog(this, ERROR_MESSAGE, "CTest configuration overwrite specified in the wrong format." << std::endl << "Valid format is: --overwrite key=value" << std::endl << "The specified was: --overwrite " << overStr.c_str() << std::endl);
        return;
    }
...
bool cmCTest::SetCTestConfigurationFromCMakeVariable(cmMakefile * mf, const char *dconfig, const char *cmake_var)
{
    ...
    cmCTestLog(this, HANDLER_VERBOSE_OUTPUT, "SetCTestConfigurationFromCMakeVariable:" << dconfig << ":" << cmake_var << std::endl);
    this->SetCTestConfiguration(dconfig, ctvar);
    return true;
}

bool cmCTest::RunCommand(const char *command, std::string * stdOut, std::string * stdErr, int *retVal, const char *dir, double timeout)
{
    std::vector < cmStdString > args = cmSystemTools::ParseArguments(command);
...
void cmCTest::Log(int logType, const char *file, int line, const char *msg)
{
    ...
    if (this->OutputLogFile)
    {
        ...
        if (logType == cmCTest::HANDLER_VERBOSE_OUTPUT && !this->Debug && !this->ExtraVerbose)
        {
            display = false;
        }
        if (display)
...
bool cmCTest::CompressString(std::string & str)
{
    ...
    unsigned char *in = reinterpret_cast < unsigned char *>(const_cast < char *>(str.c_str()));
    //zlib makes the guarantee that this is the maximum output size
    int outSize = static_cast < int >(static_cast < double >(str.size()) * 1.001 + 13.0);
    ...
    {
        cmCTestLog(this, ERROR_MESSAGE, "Error during gzip compression." << std::endl);
        delete[]out;
        return false;
    }
    ...
    unsigned char *base64EncodedBuffer = new unsigned char[static_cast < int >(outSize * 1.5)];

    unsigned long rlen = cmsysBase64_Encode(out, strm.total_out, base64EncodedBuffer, 1);
