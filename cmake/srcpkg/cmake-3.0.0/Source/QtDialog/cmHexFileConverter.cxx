...
static bool OutputBin(FILE * file, const char *buf, unsigned int startIndex, unsigned int stopIndex)
{
    bool success = true;
    char hexNumber[3];
    hexNumber[2] = '\0';
...
static bool ConvertMotorolaSrecLine(const char *buf, FILE * outFile)
{
    ...
    if ((slen < MOTOROLA_SREC_MIN_LINE_LENGTH) || (slen > MOTOROLA_SREC_MAX_LINE_LENGTH))
    {
        return false;
    }
...
cmHexFileConverter::FileType cmHexFileConverter::DetermineFileType(const char *inFileName)
{
    char buf[1024];
...
bool cmHexFileConverter::TryConvert(const char *inFileName, const char *outFileName)
{
