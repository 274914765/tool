...
cmMakeDepend::~cmMakeDepend()
{
    for (DependInformationMapType::iterator i = this->DependInformationMap.begin(); i != this->DependInformationMap.end(); ++i)
    {
        delete i->second;
    }
}
...
void cmMakeDepend::SetMakefile(cmMakefile * makefile)
{
    ...
    this->IncludeFileRegularExpression.compile(this->Makefile->IncludeFileRegularExpression.c_str());
    this->ComplainFileRegularExpression.compile(this->Makefile->ComplainFileRegularExpression.c_str());
    ...
    for (cmTargets::iterator l = targets.begin(); l != targets.end(); ++l)
    {
        ...
        for (std::vector < std::string >::const_iterator j = includes.begin(); j != includes.end(); ++j)
        {
            std::string path = *j;
    ...
    for (std::vector < std::string >::const_iterator it = orderedAndUniqueIncludes.begin(); it != orderedAndUniqueIncludes.end(); ++it)
    {
...
void cmMakeDepend::GenerateDependInformation(cmDependInformation * info)
{
    ...
    if (info->SourceFile != 0)
    {
        ...
        if (!cFile.GetDepends().empty())
        {
            ...
            // recursion.
            for (std::vector < std::string >::const_iterator file = cFile.GetDepends().begin(); file != cFile.GetDepends().end(); ++file)
            {
    ...
    if (!found)
    {
        ...
        cmSourceFile *srcFile = this->Makefile->GetSource(cmSystemTools::GetFilenameWithoutExtension(path).c_str());
        if (srcFile)
        {
            ...
            else
            {
                ...
                for (std::vector < std::string >::iterator t = this->IncludeDirectories.begin(); t != this->IncludeDirectories.end(); ++t)
                {
...
{
    cmDependInformation *dependInfo = this->GetDependInformation(file, info->PathOnly.c_str());
    this->GenerateDependInformation(dependInfo);
    info->AddDependencies(dependInfo);
}

cmDependInformation *cmMakeDepend::GetDependInformation(const char *file, const char *extraPath)
{
    ...
    DependInformationMapType::const_iterator result = this->DependInformationMap.find(fullPath);
...
std::string cmMakeDepend::FullPath(const char *fname, const char *extraPath)
{
    ...
    for (std::vector < std::string >::iterator i = this->IncludeDirectories.begin(); i != this->IncludeDirectories.end(); ++i)
    {
        std::string path = *i;
        ...
        path = path + fname;
        if (cmSystemTools::FileExists(path.c_str(), true) && !cmSystemTools::FileIsDirectory(path.c_str()))
        {
