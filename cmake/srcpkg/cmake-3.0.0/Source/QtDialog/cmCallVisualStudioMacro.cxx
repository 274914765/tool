...
HRESULT InstanceCallMacro(IDispatch * vsIDE, const std::string & macro, const std::string & args)
{
    ...
    if (0 != vsIDE)
    {
        ...
        hr = vsIDE->GetIDsOfNames(IID_NULL, &name, 1, LOCALE_USER_DEFAULT, &dispid);
        ...
        if (SUCCEEDED(hr))
        {
            ...
            hr = vsIDE->Invoke(dispid, IID_NULL, LOCALE_USER_DEFAULT, DISPATCH_METHOD, &params, &result, &excep, &arg);
            ...
            if (DISP_E_EXCEPTION == hr)
            {
                ...
                if (excep.bstrSource)
                {
                    oss << "  bstrSource: " << (const char *) (_bstr_t) excep.bstrSource << std::endl;
                }
                if (excep.bstrDescription)
                {
                    oss << "  bstrDescription: " << (const char *) (_bstr_t) excep.bstrDescription << std::endl;
                }
                if (excep.bstrHelpFile)
                {
                    oss << "  bstrHelpFile: " << (const char *) (_bstr_t) excep.bstrHelpFile << std::endl;
                }
...
HRESULT GetSolutionObject(IDispatch * vsIDE, IDispatchPtr & vsSolution)
{
    ...
    if (0 != vsIDE)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            hr = vsIDE->Invoke(dispid, IID_NULL, LOCALE_USER_DEFAULT, DISPATCH_PROPERTYGET, &params, &result, &excep, &arg);
...
HRESULT GetSolutionFullName(IDispatch * vsSolution, std::string & fullName)
{
    ...
    if (0 != vsSolution)
    {
        ...
        hr = vsSolution->GetIDsOfNames(IID_NULL, &name, 1, LOCALE_USER_DEFAULT, &dispid);
        ...
        if (SUCCEEDED(hr))
        {
            ...
            hr = vsSolution->Invoke(dispid, IID_NULL, LOCALE_USER_DEFAULT, DISPATCH_PROPERTYGET, &params, &result, &excep, &arg);
...
///! of Visual Studio.
HRESULT FindVisualStudioInstances(const std::string & slnFile, std::vector < IDispatchPtr > &instances)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        {
            if (cmSystemTools::StringStartsWith(it->first.c_str(), "!VisualStudio.DTE."))
            {
...
//----------------------------------------------------------------------------
int cmCallVisualStudioMacro::GetNumberOfRunningVisualStudioInstances(const std::string & slnFile)
{
    int count = 0;
...
///! Get all running objects from the Windows running object table.
///! Save them in a map by their display names.
int cmCallVisualStudioMacro::CallMacro(const std::string & slnFile, const std::string & macro, const std::string & args, const bool logErrorsAsMessages)
{
    ...
    if (LogErrorsAsMessages)
    {
        cmSystemTools::Message("cmCallVisualStudioMacro::CallMacro is not " "supported on this platform");
    }
#endif
