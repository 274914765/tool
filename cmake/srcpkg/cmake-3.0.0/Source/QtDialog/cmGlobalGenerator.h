...
public:
   ...
   */
    virtual void EnableLanguage(std::vector < std::string > const &languages, cmMakefile *, bool optional);

  /**
   * Resolve the CMAKE_<lang>_COMPILER setting for the given language.
   * Intended to be called from EnableLanguage.
   */
    void ResolveLanguageCompiler(const std::string & lang, cmMakefile * mf, bool optional) const;

  /**
   * Try to determine system infomation, get it from another generator
   */
    virtual void EnableLanguagesFromGenerator(cmGlobalGenerator * gen, cmMakefile * mf);

  /**
   * Try running cmake and building a file. This is used for dynamically
   * loaded commands, not as part of the usual build process.
   */
    virtual int TryCompile(const char *srcdir, const char *bindir, const char *projectName, const char *targetName, bool fast, std::string * output, cmMakefile * mf);
   ...
   */
    int Build(const char *srcdir, const char *bindir,
              ...
              std::string * output,
              const char *makeProgram, const char *config, bool clean, bool fast, double timeout, cmSystemTools::OutputOption outputflag = cmSystemTools::OUTPUT_NONE, std::vector < std::string > const &nativeOptions = std::vector < std::string > ());

    virtual void GenerateBuildCommand(std::vector < std::string > &makeCommand,
                                      const char *makeProgram, const char *projectName, const char *projectDir, const char *targetName, const char *config, bool fast, std::vector < std::string > const &makeOptions = std::vector < std::string > ());

  /** Generate a "cmake --build" call for a given target and config.  */
      std::string GenerateCMakeBuildCommand(const char *target, const char *config, const char *native, bool ignoreErrors);
    ...
    void SetExternalMakefileProjectGenerator(cmExternalMakefileProjectGenerator * extraGenerator);
  ...
  /** Add a file to the manifest of generated targets for a configuration.  */
    ...
    bool GetForceUnixPaths() const
    {
        return this->ForceUnixPaths;
    }
    bool GetToolSupportsColor() const
    {
        return this->ToolSupportsColor;
    }
    ...
    virtual std::string ExpandCFGIntDir(const std::string & str, const std::string & config) const;

  /** Get whether the generator should use a script for link commands.  */
    bool GetUseLinkScript() const
    {
        return this->UseLinkScript;
    }

  /** Get whether the generator should produce special marks on rules
      producing symbolic (non-file) outputs.  */
    bool GetNeedSymbolicMark() const
    {
        return this->NeedSymbolicMark;
    }
    ...
    cmTarget *FindTarget(const char *project, const char *name, bool excludeAliases = false) const;
  ...
  /** Append the subdirectory for the given configuration.  If anything is
    ...
    virtual void AppendDirectoryForConfig(const char *prefix, const char *config, const char *suffix, std::string & dir);
  ...
  /** Get the content of a directory.  Directory listings are loaded
    ...
    std::set < cmStdString > const &GetDirectoryContent(std::string const &dir, bool needDisk = true);

    void AddTarget(cmTarget * t);

    static bool IsReservedTarget(std::string const &name);

    virtual const char *GetAllTargetName() const
    {
        return "ALL_BUILD";
    }
    virtual const char *GetInstallTargetName() const
    {
        return "INSTALL";
    }
    ...
    virtual const char *GetPreinstallTargetName() const
    {
        return 0;
    }
    virtual const char *GetTestTargetName() const
    {
        return "RUN_TESTS";
    }
    virtual const char *GetPackageTargetName() const
    {
        return "PACKAGE";
    }
    ...
    virtual const char *GetCleanTargetName() const
    {
        return 0;
    }
  ...
  /** Get per-target generator information.  */
    ...
    const std::map < cmStdString, std::vector < cmLocalGenerator * >>&GetProjectMap() const
    {
    ...
    void AddRuleHash(const std::vector < std::string > &outputs, std::string const &content);
  ...
  /** Generate an <output>.rule file path for a given command output.  */
    ...
    void AddEvaluationFile(const std::string & inputFile, cmsys::auto_ptr < cmCompiledGeneratorExpression > outputName, cmMakefile * makefile, cmsys::auto_ptr < cmCompiledGeneratorExpression > condition, bool inputIsContent);
    ...
    cmExportBuildFileGenerator *GetExportedTargetsFile(const std::string & filename) const;
...
protected:
    ...
    virtual void GetTargetSets(TargetDependSet & projectTargets, TargetDependSet & originalTargets, cmLocalGenerator * root, GeneratorVector const &);
    bool IsRootOnlyTarget(cmTarget * target) const;
    void AddTargetDepends(cmTarget const *target, TargetDependSet & projectTargets);
    ...
    bool CheckTargets();
    typedef std::vector < std::pair < cmQtAutoGenerators, cmTarget const *>>AutogensType;
    void CreateQtAutoGeneratorsTargets(AutogensType & autogens);

    std::string SelectMakeProgram(const char *makeProgram, std::string makeDefault = "") const;
    ...
    cmTarget CreateGlobalTarget(const char *name, const char *message, const cmCustomCommandLines * commandLines, std::vector < std::string > depends, const char *workingDir);
    ...
    std::map < cmLocalGenerator *, std::set < cmTarget const *>> LocalGeneratorToTargetMap;
...
private:
    ...
    virtual void PrintCompilerAdvice(std::ostream & os, std::string lang, const char *envVar) const;
    ...
    virtual const char *GetBuildIgnoreErrorsFlag() const
    {
        return 0;
    }
    ...
    struct DirectoryContent:public std::set < cmStdString >
    {
        ...
        DirectoryContent(DirectoryContent const &dc):derived(dc), LoadedFromDisk(dc.LoadedFromDisk)
        {
        }
    };
