...
static int kwsysProcessCreate(kwsysProcess * cp, int prIndex, kwsysProcessCreateInformation * si, int *readEnd);
...
static int kwsysProcessGetTimeoutLeft(kwsysProcessTime * timeoutTime, double *userTimeout, kwsysProcessTimeNative * timeoutLength, int zeroIsExpired);
...
static pid_t kwsysProcessFork(kwsysProcess * cp, kwsysProcessCreateInformation * si);
...
static void kwsysProcessesSignalHandler(int signum, siginfo_t * info, void *ucontext);
#else
...
int kwsysProcess_AddCommand(kwsysProcess * cp, char const *const *command)
{
    ...
    if (cp->Verbatim)
    {
        /* In order to run the given command line verbatim we need to
           parse it.  */
        newCommands[cp->NumberOfCommands] = kwsysSystem_Parse_CommandForUnix(*command, 0);
        if (!newCommands[cp->NumberOfCommands] || !newCommands[cp->NumberOfCommands][0])
        {
    ...
    else
    {
        ...
        while (*c++) ;
        n = c - command - 1;
        newCommands[cp->NumberOfCommands] = (char **) malloc((size_t) (n + 1) * sizeof(char *));
...
void kwsysProcess_Execute(kwsysProcess * cp)
{
    ...
    {
        int r;
        if (!getcwd(cp->RealWorkingDirectory, (size_t) (cp->RealWorkingDirectoryLength)))
        {
            kwsysProcessCleanup(cp, 1);
            return;
        }
...
kwsysEXPORT void kwsysProcess_Disown(kwsysProcess * cp)
{
    ...
    if (!cp || !cp->Detached || cp->State != kwsysProcess_State_Executing || cp->TimeoutExpired || cp->Killed)
    {
        return;
    }
...
static int kwsysProcessWaitForPipe(kwsysProcess * cp, char **data, int *length, kwsysProcessWaitData * wd);
...
int kwsysProcess_WaitForData(kwsysProcess * cp, char **data, int *length, double *userTimeout)
{
    ...
    while (cp->PipesLeft > 0 && !kwsysProcessWaitForPipe(cp, data, length, &wd))
    {
    }
...
static int kwsysProcessWaitForPipe(kwsysProcess * cp, char **data, int *length, kwsysProcessWaitData * wd)
{
    ...
    {
        if (cp->PipeReadEnds[i] >= 0 && FD_ISSET(cp->PipeReadEnds[i], &cp->PipeSet))
        {
            ...
            while (((n = read(cp->PipeReadEnds[i], cp->PipeBuffer, KWSYSPE_PIPE_BUFFER_SIZE)) < 0) && (errno == EINTR)) ;
            if (n > 0)
            {
    ...
    if (kwsysProcessGetTimeoutLeft(&wd->TimeoutTime, wd->User ? wd->UserTimeout : 0, &timeoutLength, 0))
    {
    ...
    while (((numReady = select(max + 1, &cp->PipeSet, 0, 0, timeout)) < 0) && (errno == EINTR)) ;
...
#    endif
    ...
    if (kwsysProcessGetTimeoutLeft(&wd->TimeoutTime, wd->User ? wd->UserTimeout : 0, &timeoutLength, 1))
    {
        /* Timeout has already expired.  */
        wd->Expired = 1;
        return 1;
    ...
    {
        unsigned int msec = ((timeoutLength.tv_sec * 1000) + (timeoutLength.tv_usec / 1000));
        if (msec > 100000)
        {
...
static int kwsysProcessInitialize(kwsysProcess * cp)
{
    ...
    cp->CommandExitCodes = (int *) malloc(sizeof(int) * (size_t) (cp->NumberOfCommands));
    if (!cp->CommandExitCodes)
    {
        return 0;
    }
...
#endif
        cp->RealWorkingDirectory = malloc((size_t) (cp->RealWorkingDirectoryLength));
        if (!cp->RealWorkingDirectory)
        {
            return 0;
        }
    ...
    for (i = 0; i < KWSYSPE_PIPE_COUNT; ++i)
    {
        if (cp->PipeReadEnds[i] >= 0)
        {
            ...
            if (FD_ISSET(cp->PipeReadEnds[i], &cp->PipeSet))
            {
                ...
                /* The pipe is ready to read without blocking.  Keep trying to
                   read until the operation is not interrupted.  */
                while ((read(cp->PipeReadEnds[i], cp->PipeBuffer, KWSYSPE_PIPE_BUFFER_SIZE) < 0) && (errno == EINTR)) ;
            }
#endif
...
static int kwsysProcessCreate(kwsysProcess * cp, int prIndex, kwsysProcessCreateInformation * si, int *readEnd)
{
    ...
    else if (cp->PipeNativeSTDIN[0] >= 0)
    {
        ...
        /* Set close-on-exec flag on the pipe's ends.  The read end will
           ...
           the exec.  */
        if ((fcntl(cp->PipeNativeSTDIN[0], F_SETFD, FD_CLOEXEC) < 0) || (fcntl(cp->PipeNativeSTDIN[1], F_SETFD, FD_CLOEXEC) < 0))
        {
            return 0;
        }
    }
       ...
       pipe or writes data to the pipe to report an error.  */
    {
        ...
        while (total < KWSYSPE_PIPE_BUFFER_SIZE && n > 0)
        {
            ...
            while (((n = read(si->ErrorPipe[0], cp->ErrorMessage + total, (size_t) (KWSYSPE_PIPE_BUFFER_SIZE - total))) < 0) && (errno == EINTR)) ;
            if (n > 0)
            {
                total += n;
            }
...
static void kwsysProcessDestroy(kwsysProcess * cp)
{
    ...
    for (i = 0; i < cp->NumberOfCommands; ++i)
    {
        ...
        {
            int result;
            while (((result = waitpid(cp->ForkPIDs[i], &cp->CommandExitCodes[i], WNOHANG)) < 0) && (errno == EINTR)) ;
            if (result > 0)
            {
...
static int kwsysProcessSetupOutputPipeNative(int *p, int des[2])
{
    ...
    /* Set close-on-exec flag on the pipe's ends.  The proper end will
       ...
       before exec.  */
    if ((fcntl(des[0], F_SETFD, FD_CLOEXEC) < 0) || (fcntl(des[1], F_SETFD, FD_CLOEXEC) < 0))
    {
        return 0;
    }
    ...
    *timeoutTime = cp->TimeoutTime;
    ...
    if (userTimeout)
    {
        ...
        kwsysProcessTime userTimeoutTime = kwsysProcessTimeAdd(currentTime,
                                                               userTimeoutLength);
        if (timeoutTime->tv_sec < 0 || kwsysProcessTimeLess(userTimeoutTime, *timeoutTime))
        {
...
static int kwsysProcessGetTimeoutLeft(kwsysProcessTime * timeoutTime, double *userTimeout, kwsysProcessTimeNative * timeoutLength, int zeroIsExpired)
{
    ...
    else
    {
        ...
        if (timeLeft.tv_sec < 0 || (timeLeft.tv_sec == 0 && timeLeft.tv_usec == 0 && zeroIsExpired))
        {
            /* Timeout has already expired.  */
            return 1;
        }
...
static int kwsysProcessTimeLess(kwsysProcessTime in1, kwsysProcessTime in2)
{
    return ((in1.tv_sec < in2.tv_sec) || ((in1.tv_sec == in2.tv_sec) && (in1.tv_usec < in2.tv_usec)));
}
    ...
    if (cp->OptionDetach)
    {
        ...
        else if (middle_pid == 0)
        {
            ...
            else
            {
                ...
                while ((write(si->ErrorPipe[1], &child_pid, sizeof(child_pid)) < 0) && (errno == EINTR)) ;
        ...
        else
        {
            ...
            pid_t child_pid;
            int status;
            while ((read(si->ErrorPipe[0], &child_pid, sizeof(child_pid)) < 0) && (errno == EINTR)) ;
...
static int kwsysProcessesAdd(kwsysProcess * cp)
{
       ...
       given process object that a child has exited.  */
    {
        ...
        if (!kwsysProcessSetNonBlocking(p[0]) || !kwsysProcessSetNonBlocking(p[1]))
        {
            return 0;
        }
    ...
    /* Attempt to add the given signal pipe to the signal handler set.  */
    {
        ...
        if (oldProcesses.Count == oldProcesses.Size)
        {
            ...
            if ((newProcesses.Processes = ((kwsysProcess **) malloc((size_t) (newProcesses.Size) * sizeof(kwsysProcess *)))))
            {
                /* Copy the old pipe set to the new memory.  */
                if (oldProcesses.Count > 0)
                {
                    memcpy(newProcesses.Processes, oldProcesses.Processes, ((size_t) (oldProcesses.Count) * sizeof(kwsysProcess *)));
                }
            }
            else
            {
...
#endif
            while ((sigaction(SIGCHLD, &newSigChldAction, &kwsysProcessesOldSigChldAction) < 0) && (errno == EINTR)) ;
        }
    }
        ...
        if (i < newProcesses.Count)
        {
            ...
            if (newProcesses.Count == 0)
            {
                /* Restore the SIGCHLD handler.  Repeat call until it is not
                   interrupted.  */
                while ((sigaction(SIGCHLD, &kwsysProcessesOldSigChldAction, 0) < 0) && (errno == EINTR)) ;
    ...
    )
{
    ...
    /* Signal all process objects that a child has terminated.  */
    {
        ...
        for (i = 0; i < kwsysProcesses.Count; ++i)
        {
            ...
            kwsysProcess_ssize_t status = read(cp->PipeReadEnds[KWSYSPE_PIPE_SIGNAL], &buf, 1);
            (void) status;
