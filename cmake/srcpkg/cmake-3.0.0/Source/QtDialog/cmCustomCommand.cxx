...
//----------------------------------------------------------------------------
cmCustomCommand::cmCustomCommand(const cmCustomCommand & r):Outputs(r.Outputs),
Depends(r.Depends),
CommandLines(r.CommandLines), HaveComment(r.HaveComment), Comment(r.Comment), WorkingDirectory(r.WorkingDirectory), EscapeAllowMakeVars(r.EscapeAllowMakeVars), EscapeOldStyle(r.EscapeOldStyle), Backtrace(new cmListFileBacktrace(*r.Backtrace))
{
}
...
cmCustomCommand & cmCustomCommand::operator=(cmCustomCommand const &r)
{
    ...
    cmsys::auto_ptr < cmListFileBacktrace > newBacktrace(new cmListFileBacktrace(*r.Backtrace));
...
//----------------------------------------------------------------------------
cmCustomCommand::cmCustomCommand(cmMakefile const *mf, const std::vector < std::string > &outputs, const std::vector < std::string > &depends, const cmCustomCommandLines & commandLines, const char *comment, const char *workingDirectory):
Outputs(outputs),
Depends(depends),
CommandLines(commandLines), HaveComment(comment ? true : false), Comment(comment ? comment : ""), WorkingDirectory(workingDirectory ? workingDirectory : ""), EscapeAllowMakeVars(false), EscapeOldStyle(true), Backtrace(new cmListFileBacktrace)
{
    this->EscapeOldStyle = true;
    this->EscapeAllowMakeVars = false;
    if (mf)
...
{
    for (std::vector < std::string >::const_iterator i = depends.begin(); i != depends.end(); ++i)
    {
...
//----------------------------------------------------------------------------
cmCustomCommand::ImplicitDependsList const & cmCustomCommand::GetImplicitDepends() const
{
...
{
    this->ImplicitDepends.insert(this->ImplicitDepends.end(), l.begin(), l.end());
}
