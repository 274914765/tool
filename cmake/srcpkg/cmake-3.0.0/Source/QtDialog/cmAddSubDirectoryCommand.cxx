...
bool cmAddSubDirectoryCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 1)
    {
    ...
    if (binArg.empty())
    {
        ...
        // error.
        if (!cmSystemTools::IsSubDirectory(srcPath.c_str(), this->Makefile->GetCurrentDirectory()))
        {
            cmOStringStream e;
            e << "not given a binary directory but the given source directory "
                << "\"" << srcPath << "\" is not a subdirectory of \"" << this->Makefile->GetCurrentDirectory() << "\".  " << "When specifying an out-of-tree source a binary directory " << "must be explicitly specified.";
    ...
    this->Makefile->AddSubDirectory(srcPath.c_str(), binPath.c_str(), excludeFromAll, false, true);
