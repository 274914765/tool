...
// cmExecutableCommand
bool cmAddExecutableCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 2)
    {
    ...
    if (!nameOk)
    {
        ...
        if (issueMessage)
        {
            e << "The target name \"" << exename << "\" is reserved or not valid for certain " "CMake features, such as generator expressions, and may result " "in undefined behavior.";
    ...
    if (importTarget && (use_win32 || use_macbundle || excludeFromAll))
    {
        ...
        {
            this->SetError("may not be given EXCLUDE_FROM_ALL for an IMPORTED target.");
        }
        return false;
    }
    if (isAlias)
    {
        ...
        {
            cmOStringStream e;
            e << "cannot create ALIAS target \"" << exename << "\" because target \"" << aliasedName << "\" is itself an ALIAS.";
            this->SetError(e.str().c_str());
            return false;
        }
        ...
        {
            cmOStringStream e;
            e << "cannot create ALIAS target \"" << exename << "\" because target \"" << aliasedName << "\" does not already " "exist.";
            this->SetError(e.str().c_str());
            return false;
        }
        ...
        {
            cmOStringStream e;
            e << "cannot create ALIAS target \"" << exename << "\" because target \"" << aliasedName << "\" is not an " "executable.";
            this->SetError(e.str().c_str());
            return false;
        }
        ...
        {
            cmOStringStream e;
            e << "cannot create ALIAS target \"" << exename << "\" because target \"" << aliasedName << "\" is IMPORTED.";
            this->SetError(e.str().c_str());
            return false;
        }
    ...
    if (importTarget)
    {
        ...
        {
            cmOStringStream e;
            e << "cannot create imported target \"" << exename << "\" because another target with the same name already exists.";
            this->SetError(e.str().c_str());
            return false;
        }

        // Create the imported target.
        this->Makefile->AddImportedTarget(exename.c_str(), cmTarget::EXECUTABLE, importGlobal);
        return true;
    }
