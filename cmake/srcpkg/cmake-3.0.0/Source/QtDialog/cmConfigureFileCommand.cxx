...
bool cmConfigureFileCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 2)
    {
    ...
    {
        std::string e = "attempted to configure a file: " + this->OutputFile + " into a source directory.";
    ...
    for (unsigned int i = 2; i < args.size(); ++i)
    {
        if (args[i] == "COPYONLY")
        {
            ...
            {
                this->SetError("COPYONLY could not be used in combination " "with NEWLINE_STYLE");
                return false;
            }
        }
...
{
    return this->Makefile->ConfigureFile(this->InputFile.c_str(), this->OutputFile.c_str(), this->CopyOnly, this->AtOnly, this->EscapeQuotes, this->NewLineStyle);
}
