  ...
  /** Construct. */
    cmGeneratorExpression(cmListFileBacktrace const &backtrace);
     ~cmGeneratorExpression();
      ...
      cmsys::auto_ptr < cmCompiledGeneratorExpression > Parse(std::string const &input);
    ...
    static std::string Preprocess(const std::string & input, PreprocessContext context, bool resolveRelative = false);
...
{
public:
    const char *Evaluate(cmMakefile * mf, const char *config, bool quiet = false, cmTarget const *headTarget = 0, cmTarget const *currentTarget = 0, cmGeneratorExpressionDAGChecker * dagChecker = 0) const;
    const char *Evaluate(cmMakefile * mf, const char *config, bool quiet, cmTarget const *headTarget, cmGeneratorExpressionDAGChecker * dagChecker) const;
...
private:
      cmCompiledGeneratorExpression(cmListFileBacktrace const &backtrace, const char *input);
