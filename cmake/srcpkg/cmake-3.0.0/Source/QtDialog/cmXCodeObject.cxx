...
cmXCodeObject::cmXCodeObject(PBXType ptype, Type type)
{
    ...
    {
        this->Id = "Temporary cmake object, should not be referred to in Xcode file";
    }
...
void cmXCodeObject::Print(std::ostream & out)
{
    ...
    for (i = this->ObjectAttributes.begin(); i != this->ObjectAttributes.end(); ++i)
    {
        ...
        else if (object->TypeValue == ATTRIBUTE_GROUP)
        {
            ...
            out << i->first << " = {" << separator;
            for (j = object->ObjectAttributes.begin(); j != object->ObjectAttributes.end(); ++j)
            {
                cmXCodeObject::Indent(4 * indentFactor, out);
                ...
                {
                    cmXCodeObject::PrintString(out, j->first);
                    out << " = error_unexpected_TypeValue_" << j->second->TypeValue << ";";
                }
...
//----------------------------------------------------------------------------
void cmXCodeObject::PrintList(std::vector < cmXCodeObject * >const &objs, std::ostream & out)
{
...
void cmXCodeObject::PrintString(std::ostream & os, cmStdString String)
{
    ...
    bool needQuote = (String.empty() || String.find_first_of(" <>.+-=@$[],") != String.npos);
