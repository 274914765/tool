...
cmCommandArgument::cmCommandArgument(cmCommandArgumentsHelper * args, const char *key, cmCommandArgumentGroup * group):Key(key), Group(group), WasActive(false), ArgumentsBeforeEmpty(true), CurrentIndex(0)
{
    if (args != 0)
    {
...
void cmCommandArgument::FollowsGroup(const cmCommandArgumentGroup * group)
{
    ...
    {
        this->ArgumentsBeforeEmpty = false;
        for (std::vector < cmCommandArgument * >::const_iterator argIt = group->ContainedArguments.begin(); argIt != group->ContainedArguments.end(); ++argIt)
        {
...
bool cmCommandArgument::MayFollow(const cmCommandArgument * current) const
{
    ...
    std::set < const cmCommandArgument *>::const_iterator argIt = this->ArgumentsBefore.find(current);
...
void cmCommandArgument::ApplyOwnGroup()
{
    if (this->Group != 0)
    {
        for (std::vector < cmCommandArgument * >::const_iterator it = this->Group->ContainedArguments.begin(); it != this->Group->ContainedArguments.end(); ++it)
        {
            if (*it != this)
            {
...
cmCAStringVector::cmCAStringVector(cmCommandArgumentsHelper * args, const char *key, cmCommandArgumentGroup * group):cmCommandArgument(args, key, group), Ignore(0)
{
    if ((key == 0) || (*key == 0))
    {
        this->DataStart = 0;
...
cmCAString::cmCAString(cmCommandArgumentsHelper * args, const char *key, cmCommandArgumentGroup * group):cmCommandArgument(args, key, group)
{
    if ((key == 0) || (*key == 0))
    {
        this->DataStart = 0;
...
cmCAEnabler::cmCAEnabler(cmCommandArgumentsHelper * args, const char *key, cmCommandArgumentGroup * group):cmCommandArgument(args, key, group), Enabled(false)
{
}
...
cmCADisabler::cmCADisabler(cmCommandArgumentsHelper * args, const char *key, cmCommandArgumentGroup * group):cmCommandArgument(args, key, group), Enabled(true)
{
}
...
{
    for (std::vector < cmCommandArgument * >::iterator it = this->ContainedArguments.begin(); it != this->ContainedArguments.end(); ++it)
    {
        (*it)->Follows(arg);
    }
}
...
void cmCommandArgumentsHelper::Parse(const std::vector < std::string > *args, std::vector < std::string > *unconsumedArgs)
{
    if (args == 0)
    {
        return;
    ...
    for (std::vector < cmCommandArgument * >::iterator argIt = this->Arguments.begin(); argIt != this->Arguments.end(); ++argIt)
    {
        (*argIt)->ApplyOwnGroup();
        (*argIt)->Reset();
    }
