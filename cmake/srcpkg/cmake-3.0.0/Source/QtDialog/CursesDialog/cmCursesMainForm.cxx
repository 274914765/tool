...
cmCursesMainForm::cmCursesMainForm(std::vector < std::string > const &args, int initWidth):
Args(args), InitialWidth(initWidth)
{
    ...
    this->CMakeInstance->SetCMakeEditCommand(cmSystemTools::GetCMakeCursesCommand());
...
void cmCursesMainForm::InitializeUI()
{
    ...
    std::vector < cmCursesCacheEntryComposite * >*newEntries = new std::vector < cmCursesCacheEntryComposite * >;
    newEntries->reserve(this->CMakeInstance->GetCacheManager()->GetSize());

    // Count non-internal and non-static entries
    int count = 0;
    for (cmCacheManager::CacheIterator i = this->CMakeInstance->GetCacheManager()->NewIterator(); !i.IsAtEnd(); i.Next())
    {
        if (i.GetType() != cmCacheManager::INTERNAL && i.GetType() != cmCacheManager::STATIC && i.GetType() != cmCacheManager::UNINITIALIZED)
        {
            ++count;
        }
    }
    ...
    else
    {
        ...
        for (cmCacheManager::CacheIterator i = this->CMakeInstance->GetCacheManager()->NewIterator(); !i.IsAtEnd(); i.Next())
        {
            ...
            {
                newEntries->push_back(new cmCursesCacheEntryComposite(key, i, true, 30, entrywidth));
                this->OkToGenerate = false;
            }
        }
        ...
        for (cmCacheManager::CacheIterator i = this->CMakeInstance->GetCacheManager()->NewIterator(); !i.IsAtEnd(); i.Next())
        {
            ...
            if (this->LookForCacheEntry(key))
            {
                newEntries->push_back(new cmCursesCacheEntryComposite(key, i, false, 30, entrywidth));
            }
        }
    }
...
void cmCursesMainForm::RePost()
{
    ...
    else
    {
        ...
        for (it = this->Entries->begin(); it != this->Entries->end(); ++it)
        {
            cmCacheManager::CacheIterator mit = this->CMakeInstance->GetCacheManager()->GetCacheIterator((*it)->GetValue());
...
void cmCursesMainForm::Render(int left, int top, int width, int height)
{
    ...
    if (this->Form)
    {
        ...
        cmCursesWidget *cw = reinterpret_cast < cmCursesWidget * >(field_userptr(currentField));
        // If in edit mode, get out of it
        if (cw->GetType() == cmCacheManager::STRING || cw->GetType() == cmCacheManager::PATH || cw->GetType() == cmCacheManager::FILEPATH)
        {
...
void cmCursesMainForm::PrintKeys(int process /* = 0 */ )
{
    int x, y;
    getmaxyx(stdscr, y, x);
    if (x < cmCursesMainForm::MIN_WIDTH || x < this->InitialWidth || y < cmCursesMainForm::MIN_HEIGHT)
    {
        return;
    }
...
//    {
    ...
    else
    {
        ...
        {
            const char *toggleKeyInstruction = "Press [t] to toggle advanced mode (Currently %s)";
            sprintf(thirdLine, toggleKeyInstruction, this->AdvancedMode ? "On" : "Off");
        }
...
void cmCursesMainForm::UpdateStatusBar(const char *message)
{
    ...
    if (x < cmCursesMainForm::MIN_WIDTH || x < this->InitialWidth || y < cmCursesMainForm::MIN_HEIGHT)
    {
        ...
        printw(fmt, (cmCursesMainForm::MIN_WIDTH < this->InitialWidth ? this->InitialWidth : cmCursesMainForm::MIN_WIDTH), cmCursesMainForm::MIN_HEIGHT);
        touchwin(stdscr);
        wrefresh(stdscr);
        return;
    }
    ...
    if (lbl)
    {
        ...
        cmCacheManager::CacheIterator it = this->CMakeInstance->GetCacheManager()->GetCacheIterator(curField);
        if (!it.IsAtEnd())
        {
...
int cmCursesMainForm::Configure(int noconfigure)
{
    ...
    this->CMakeInstance->GetCacheManager()->SaveCache(this->CMakeInstance->GetHomeOutputDirectory());
    this->LoadCache(0);
    ...
    if (retVal != 0 || !this->Errors.empty())
    {
        ...
        cmCursesLongMessageForm *msgs = new cmCursesLongMessageForm(this->Errors,
                                                                    cmSystemTools::GetErrorOccuredFlag()? "Errors occurred during the last pass." : "CMake produced the following output.");
...
void cmCursesMainForm::FillCacheManagerFromUI()
{
    ...
    {
        cmCacheManager::CacheIterator it = this->CMakeInstance->GetCacheManager()->GetCacheIterator((*this->Entries)[i]->Key.c_str());
        if (!it.IsAtEnd())
        {
...
void cmCursesMainForm::FixValue(cmCacheManager::CacheEntryType type, const std::string & in, std::string & out) const
{
...
void cmCursesMainForm::HandleInput()
{
    ...
    for (;;)
    {
        ...
        currentWidget = reinterpret_cast < cmCursesWidget * >(field_userptr(currentField));
        ...
        if ((!currentWidget || !widgetHandled) && !this->SearchMode)
        {
            ...
            else if (key == 'h')
            {
                ...
                FIELD *cur = current_field(this->Form);
                int findex = field_index(cur);
                cmCursesWidget *lbl = reinterpret_cast < cmCursesWidget * >(field_userptr(this->Fields[findex - 2]));
                ...
                if (helpString)
                {
                    char *message = new char[strlen(curField) + strlen(helpString) + strlen("Current option is: \n Help string for this option is: \n") + 10];
