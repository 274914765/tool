...
cmCursesOptionsWidget::cmCursesOptionsWidget(int width, int height, int left, int top):
...
void cmCursesOptionsWidget::SetOption(const char *value)
{
    ...
    this->SetValue(value);
    int index = 0;
    for (std::vector < std::string >::iterator i = this->Options.begin(); i != this->Options.end(); ++i)
    {
        if (*i == value)
        {
