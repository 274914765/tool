...
{
public:
    cmComputeLinkInformation(cmTarget const *target, const char *config, cmTarget const *headTarget);
    ...
    struct Item
    {
        ...
        Item(Item const &item):Value(item.Value), IsPath(item.IsPath), Target(item.Target)
        {
        }
Item(std::string const &v, bool p, cmTarget const *target = 0):
    ...
    const char *GetLinkLanguage() const
    {
        return this->LinkLanguage;
    }
    std::vector < std::string > const &GetRuntimeSearchPath();
    std::string const &GetRuntimeFlag() const
    {
        return this->RuntimeFlag;
    }
    std::string const &GetRuntimeSep() const
    {
        return this->RuntimeSep;
    }
    ...
    std::string const &GetRPathLinkFlag() const
    {
        return this->RPathLinkFlag;
    }
...
private:
    ...
    enum LinkType
    { LinkUnknown, LinkStatic, LinkShared };
    ...
    std::string CreateExtensionRegex(std::vector < std::string > const &exts, LinkType type);
