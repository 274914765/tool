...
public:
   ...
   */
    virtual bool InvokeInitialPass(const std::vector < cmListFileArgument > &args, cmExecutionStatus &);
...
bool cmMacroHelperCommand::InvokeInitialPass(const std::vector < cmListFileArgument > &args, cmExecutionStatus & inStatus)
{
    ...
    {
        std::string errorMsg = "Macro invoked with incorrect arguments for macro named: ";
    ...
    for (unsigned int c = 0; c < this->Functions.size(); ++c)
    {
        ...
        for (std::vector < cmListFileArgument >::iterator k = this->Functions[c].Arguments.begin(); k != this->Functions[c].Arguments.end(); ++k)
        {
            ...
            else
            {
                ...
                for (unsigned int j = 1; j < this->Args.size(); ++j)
                {
                    ...
                    variable += "}";
                    cmSystemTools::ReplaceString(tmps, variable.c_str(), expandedArgs[j - 1].c_str());
                }
                // replace argc
                ...
                if (tmps.find("${ARGV") != std::string::npos)
                {
                    ...
                    for (unsigned int t = 0; t < expandedArgs.size(); ++t)
                    {
                        ...
                        cmSystemTools::ReplaceString(tmps, argvName, expandedArgs[t].c_str());
                    }
                }
        ...
        if (!this->Makefile->ExecuteCommand(newLFF, status) || status.GetNestedError())
        {
...
bool cmMacroFunctionBlocker::IsFunctionBlocked(const cmListFileFunction & lff, cmMakefile & mf, cmExecutionStatus &)
{
    ...
    else if (!cmSystemTools::Strucmp(lff.Name.c_str(), "endmacro"))
    {
        ...
        if (!this->Depth)
        {
            ...
            mf.GetCMakeInstance()->RenameCommand(this->Args[0].c_str(), newName.c_str());
            mf.AddCommand(f);
...
bool cmMacroFunctionBlocker::ShouldRemove(const cmListFileFunction & lff, cmMakefile & mf)
{
...
bool cmMacroCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 1)
    {
