...
bool cmAddCustomCommandCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
                ...
                case doing_implicit_depends_file:
                    {
                        ...
                        cmCustomCommand::ImplicitDependsPair entry(implicit_depends_lang, dep);
    ...
    if (append)
    {
        ...
        if (cmSourceFile * sf = this->Makefile->GetSourceFileWithOutput(output[0].c_str()))
        {
        ...
        cmOStringStream e;
        e << "given APPEND option with output \"" << output[0].c_str() << "\" which is not already a custom command output.";
    ...
    if (source.empty() && output.empty())
    {
        ...
        this->Makefile->AddCustomCommandToTarget(target.c_str(), no_depends, commandLines, cctype, comment, working.c_str(), escapeOldStyle);
    }
    else if (target.empty())
    {
        // Target is empty, use the output.
        this->Makefile->AddCustomCommandToOutput(output, depends, main_dependency.c_str(), commandLines, comment, working.c_str(), false, escapeOldStyle);
        ...
        if (!implicit_depends.empty())
        {
            ...
            {
                cmOStringStream e;
                e << "could not locate source file with a custom command producing \"" << output[0] << "\" even though this command tried to create it!";
                this->SetError(e.str().c_str());
                return false;
            }
        }
    ...
    else
    {
        ...
        if (issueMessage)
        {
            e << "The SOURCE signatures of add_custom_command are no longer " "supported.";
        ...
        this->Makefile->AddCustomCommandOldStyle(target.c_str(), outputs, depends, source.c_str(), commandLines, comment);
    }
...
//----------------------------------------------------------------------------
bool cmAddCustomCommandCommand::CheckOutputs(const std::vector < std::string > &outputs)
{
    for (std::vector < std::string >::const_iterator o = outputs.begin(); o != outputs.end(); ++o)
    {
        ...
        {
            std::string e = "attempted to have a file \"" + *o + "\" in a source directory as an output of custom command.";
