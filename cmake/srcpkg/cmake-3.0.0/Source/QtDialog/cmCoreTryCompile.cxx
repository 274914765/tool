...
int cmCoreTryCompile::TryCompileCode(std::vector < std::string > const &argv)
{
    ...
    enum Doing
    { DoingNone, DoingCMakeFlags, DoingCompileDefinitions,
        ...
        else if (doing == DoingLinkLibraries)
        {
            ...
            if (cmTarget * tgt = this->Makefile->FindTargetToUse(argv[i]))
            {
                switch (tgt->GetType())
                {
                    ...
                    default:
                        this->Makefile->IssueMessage(cmake::FATAL_ERROR, "Only libraries may be used as try_compile IMPORTED " "LINK_LIBRARIES.  Got " + std::string(tgt->GetName()) + " of " "type " + tgt->GetTargetTypeName(tgt->GetType()) + ".");
                        return -1;
                }
                if (tgt->IsImported())
                {
    ...
    {
        this->Makefile->IssueMessage(cmake::FATAL_ERROR, "COPY_FILE must be followed by a file path");
        return -1;
    }

    if (didCopyFileError && copyFileError.empty())
    {
        this->Makefile->IssueMessage(cmake::FATAL_ERROR, "COPY_FILE_ERROR must be followed by a variable name");
        return -1;
    }

    if (didCopyFileError && !didCopyFile)
    {
        this->Makefile->IssueMessage(cmake::FATAL_ERROR, "COPY_FILE_ERROR may be used only with COPY_FILE");
        return -1;
    }

    if (didOutputVariable && outputVariable.empty())
    {
        this->Makefile->IssueMessage(cmake::FATAL_ERROR, "OUTPUT_VARIABLE must be followed by a variable name");
        return -1;
    }

    if (useSources && sources.empty())
    {
        this->Makefile->IssueMessage(cmake::FATAL_ERROR, "SOURCES must be followed by at least one source file");
        return -1;
    }
    ...
    else
    {
        ...
        if (compileDefs.size())
        {
            this->Makefile->IssueMessage(cmake::FATAL_ERROR, "COMPILE_DEFINITIONS specified on a srcdir type TRY_COMPILE");
            return -1;
        }
        if (copyFile.size())
        {
            this->Makefile->IssueMessage(cmake::FATAL_ERROR, "COPY_FILE specified on a srcdir type TRY_COMPILE");
            return -1;
        }
    }
    ...
    if (this->SrcFileSignature)
    {
        ...
        for (std::vector < std::string >::iterator si = sources.begin(); si != sources.end(); ++si)
        {
            ...
            else
            {
                ...
                for (std::vector < std::string >::iterator l = langs.begin(); l != langs.end(); ++l)
                {
                    err << " " << *l;
                }
        ...
        fprintf(fout, "cmake_minimum_required(VERSION %u.%u.%u.%u)\n", cmVersion::GetMajorVersion(), cmVersion::GetMinorVersion(), cmVersion::GetPatchVersion(), cmVersion::GetTweakVersion());
        if (def)
        {
        ...
        for (std::set < std::string >::iterator li = testLangs.begin(); li != testLangs.end(); ++li)
        {
            projectLangs += " " + *li;
            std::string rulesOverrideBase = "CMAKE_USER_MAKE_RULES_OVERRIDE";
            std::string rulesOverrideLang = rulesOverrideBase + "_" + *li;
            if (const char *rulesOverridePath = this->Makefile->GetDefinition(rulesOverrideLang.c_str()))
            {
                fprintf(fout, "set(%s \"%s\")\n", rulesOverrideLang.c_str(), rulesOverridePath);
            }
            else if (const char *rulesOverridePath2 = this->Makefile->GetDefinition(rulesOverrideBase.c_str()))
            {
        ...
        for (std::set < std::string >::iterator li = testLangs.begin(); li != testLangs.end(); ++li)
        {
            ...
            fprintf(fout, "set(CMAKE_%s_FLAGS %s)\n", li->c_str(), lg->EscapeForCMake(flags ? flags : "").c_str());
            fprintf(fout, "set(CMAKE_%s_FLAGS \"${CMAKE_%s_FLAGS}" " ${COMPILE_DEFINITIONS}\")\n", li->c_str(), li->c_str());
        }
        ...
        sprintf(targetNameBuf, "cmTryCompileExec%u", cmSystemTools::RandomSeed());
        ...
        if (!targets.empty())
        {
            ...
            tcfg.SetConfig(this->Makefile->GetDefinition("CMAKE_TRY_COMPILE_CONFIGURATION"));

            if (!tcfg.GenerateImportFile())
            {
                this->Makefile->IssueMessage(cmake::FATAL_ERROR, "could not write export file.");
                fclose(fout);
                return -1;
            }
        ...
        if (this->Makefile->GetDefinition("CMAKE_TRY_COMPILE_OSX_ARCHITECTURES") != 0)
        {
            ...
            flag += this->Makefile->GetSafeDefinition("CMAKE_TRY_COMPILE_OSX_ARCHITECTURES");
        ...
        if (const char *cxxDef = this->Makefile->GetDefinition("CMAKE_CXX_COMPILER_TARGET"))
        {
            std::string flag = "-DCMAKE_CXX_COMPILER_TARGET=";
            flag += cxxDef;
            cmakeFlags.push_back(flag);
        }
        if (const char *cDef = this->Makefile->GetDefinition("CMAKE_C_COMPILER_TARGET"))
        {
        ...
        if (const char *tcDef = this->Makefile->GetDefinition("CMAKE_C_COMPILER_EXTERNAL_TOOLCHAIN"))
        {
    ...
    this->Makefile->AddCacheDefinition(argv[0].c_str(), (res == 0 ? "TRUE" : "FALSE"), "Result of TRY_COMPILE", cmCacheManager::INTERNAL);
    ...
    if (this->SrcFileSignature)
    {
        ...
        {
            if (this->OutputFile.empty() || !cmSystemTools::CopyFileAlways(this->OutputFile.c_str(), copyFile.c_str()))
            {
        ...
        if (!copyFileError.empty())
        {
            this->Makefile->AddDefinition(copyFileError.c_str(), copyFileErrorMessage.c_str());
        }
    }
    return res;
}
...
void cmCoreTryCompile::CleanupFiles(const char *binDir)
{
    ...
    for (fileNum = 0; fileNum < dir.GetNumberOfFiles(); ++fileNum)
    {
        if (strcmp(dir.GetFile(static_cast < unsigned long >(fileNum)), ".") && strcmp(dir.GetFile(static_cast < unsigned long >(fileNum)), ".."))
        {
            ...
            if (deletedFiles.find(dir.GetFile(static_cast < unsigned long >(fileNum))) == deletedFiles.end())
            {
                ...
                else
                {
                    ...
                    while (!cmSystemTools::RemoveFile(fullPath.c_str()) && --retry.Count && cmSystemTools::FileExists(fullPath.c_str()))
                    {
...
void cmCoreTryCompile::FindOutputFile(const char *targetName)
{
    ...
    for (std::vector < std::string >::const_iterator it = searchDirs.begin(); it != searchDirs.end(); ++it)
    {
