...
{
    cmDependsFortranFile(FILE * file, YY_BUFFER_STATE buffer, const std::string & dir):File(file), Buffer(buffer), Directory(dir)
    {
    }
    FILE *File;
...
{
    cmDependsFortranParser_s(cmDependsFortran * self, std::set < std::string > &ppDefines, cmDependsFortranSourceInfo & info);
...
public:
    ...
    ObjectInfoMap ObjectInfo;
      ...
      cmDependsFortranSourceInfo & CreateObjectInfo(const char *obj, const char *src)
    {
        std::map < cmStdString, cmDependsFortranSourceInfo >::iterator i = this->ObjectInfo.find(obj);
        if (i == this->ObjectInfo.end())
        {
            std::map < cmStdString, cmDependsFortranSourceInfo >::value_type entry(obj, cmDependsFortranSourceInfo());
...
cmDepends(lg), Internal(new cmDependsFortranInternals)
{
    ...
    if (const char *c_defines = mf->GetDefinition("CMAKE_TARGET_DEFINITIONS"))
    {
...
//----------------------------------------------------------------------------
bool cmDependsFortran::WriteDependencies(const std::set < std::string > &sources, const std::string & obj, std::ostream &, std::ostream &)
{
    ...
    bool okay = true;
    for (std::set < std::string >::const_iterator it = sources.begin(); it != sources.end(); ++it)
    {
        const std::string & src = *it;
        // Get the information object for this source.
        cmDependsFortranSourceInfo & info = this->Internal->CreateObjectInfo(obj.c_str(), src.c_str());

        // Make a copy of the macros defined via ADD_DEFINITIONS
        std::set < std::string > ppDefines(this->PPDefinitions.begin(), this->PPDefinitions.end());
...
//----------------------------------------------------------------------------
bool cmDependsFortran::Finalize(std::ostream & makeDepends, std::ostream & internalDepends)
{
    ...
    const char *mod_dir;
    cmMakefile *mf = this->LocalGenerator->GetMakefile();
    if (const char *target_mod_dir = mf->GetDefinition("CMAKE_Fortran_TARGET_MODULE_DIR"))
    {
        mod_dir = target_mod_dir;
    }
    else
    {
        mod_dir = this->LocalGenerator->GetMakefile()->GetCurrentOutputDirectory();
    }

    // Actually write dependencies to the streams.
    typedef cmDependsFortranInternals::ObjectInfoMap ObjectInfoMap;
    ObjectInfoMap const &objInfo = this->Internal->ObjectInfo;
    for (ObjectInfoMap::const_iterator i = objInfo.begin(); i != objInfo.end(); ++i)
    {
        if (!this->WriteDependenciesReal(i->first.c_str(), i->second, mod_dir, stamp_dir, makeDepends, internalDepends))
        {
            return false;
        }
    }
    ...
    for (std::set < cmStdString >::const_iterator i = provides.begin(); i != provides.end(); ++i)
    {
        fiStream << " " << *i << "\n";
    }
...
void cmDependsFortran::LocateModules()
{
    ...
    typedef cmDependsFortranInternals::ObjectInfoMap ObjectInfoMap;
    ObjectInfoMap const &objInfo = this->Internal->ObjectInfo;
    for (ObjectInfoMap::const_iterator infoI = objInfo.begin(); infoI != objInfo.end(); ++infoI)
    {
        cmDependsFortranSourceInfo const &info = infoI->second;
        for (std::set < cmStdString >::const_iterator i = info.Provides.begin(); i != info.Provides.end(); ++i)
        {
            // Include this module in the set provided by this target.
            this->Internal->TargetProvides.insert(*i);
        }

        for (std::set < cmStdString >::const_iterator i = info.Requires.begin(); i != info.Requires.end(); ++i)
        {
    ...
    for (std::vector < std::string >::const_iterator i = infoFiles.begin(); i != infoFiles.end(); ++i)
    {
...
//----------------------------------------------------------------------------
void cmDependsFortran::MatchRemoteModules(std::istream & fin, const char *stampDir)
{
    std::string line;
...
void cmDependsFortran::ConsiderModule(const char *name, const char *stampDir)
{
    ...
    TargetRequiresMap::iterator required = this->Internal->TargetRequires.find(name);
    if (required != this->Internal->TargetRequires.end() && required->second.empty())
    {
...
//----------------------------------------------------------------------------
bool cmDependsFortran::WriteDependenciesReal(const char *obj, cmDependsFortranSourceInfo const &info, const char *mod_dir, const char *stamp_dir, std::ostream & makeDepends, std::ostream & internalDepends)
{
    typedef cmDependsFortranInternals::TargetRequiresMap TargetRequiresMap;
    ...
    for (std::set < cmStdString >::const_iterator i = info.Includes.begin(); i != info.Includes.end(); ++i)
    {
        makeDepends << obj << ": " << this->LocalGenerator->Convert(i->c_str(), cmLocalGenerator::HOME_OUTPUT, cmLocalGenerator::MAKEFILE) << std::endl;
    ...
    for (std::set < cmStdString >::const_iterator i = info.Requires.begin(); i != info.Requires.end(); ++i)
    {
        ...
        if (std::set < cmStdString >::const_iterator(info.Provides.find(*i)) != info.Provides.end())
        {
            continue;
        }
        ...
        // needed.
        if (this->Internal->TargetProvides.find(*i) != this->Internal->TargetProvides.end())
        {
            ...
            proxy += *i;
            proxy += ".mod.proxy";
            proxy = this->LocalGenerator->Convert(proxy.c_str(), cmLocalGenerator::HOME_OUTPUT, cmLocalGenerator::MAKEFILE);
        ...
        // modules it uses.
        TargetRequiresMap::const_iterator required = this->Internal->TargetRequires.find(*i);
        ...
        if (!required->second.empty())
        {
            ...
            std::string stampFile = this->LocalGenerator->Convert(required->second.c_str(), cmLocalGenerator::HOME_OUTPUT, cmLocalGenerator::MAKEFILE);
        ...
        else
        {
            ...
            {
                module = this->LocalGenerator->Convert(module.c_str(), cmLocalGenerator::HOME_OUTPUT, cmLocalGenerator::MAKEFILE);
    ...
    if (!info.Provides.empty())
    {
        ...
        for (std::set < cmStdString >::const_iterator i = info.Provides.begin(); i != info.Provides.end(); ++i)
        {
            ...
            modFile += "/";
            modFile += *i;
            modFile = this->LocalGenerator->Convert(modFile.c_str(), cmLocalGenerator::HOME_OUTPUT, cmLocalGenerator::SHELL);
            std::string stampFile = stamp_dir;
            stampFile += "/";
            stampFile += m;
            stampFile += ".mod.stamp";
            stampFile = this->LocalGenerator->Convert(stampFile.c_str(), cmLocalGenerator::HOME_OUTPUT, cmLocalGenerator::SHELL);
            makeDepends << "\t$(CMAKE_COMMAND) -E cmake_copy_f90_mod " << modFile << " " << stampFile;
            cmMakefile *mf = this->LocalGenerator->GetMakefile();
        ...
        driver += "/build";
        driver = this->LocalGenerator->Convert(driver.c_str(), cmLocalGenerator::HOME_OUTPUT, cmLocalGenerator::MAKEFILE);
...
bool cmDependsFortran::FindModule(std::string const &name, std::string & module)
{
    ...
    std::string fullName;
    for (std::vector < std::string >::const_iterator i = this->IncludePath.begin(); i != this->IncludePath.end(); ++i)
    {
...
bool cmDependsFortran::CopyModule(const std::vector < std::string > &args)
{
    ...
    {
        if (cmDependsFortran::ModulesDiffer(mod_upper.c_str(), stamp.c_str(), compilerId.c_str()))
        {
            if (!cmSystemTools::CopyFileAlways(mod_upper.c_str(), stamp.c_str()))
            {
                std::cerr << "Error copying Fortran module from \"" << mod_upper.c_str() << "\" to \"" << stamp.c_str() << "\".\n";
                return false;
            }
        }
        return true;
    ...
    {
        if (cmDependsFortran::ModulesDiffer(mod_lower.c_str(), stamp.c_str(), compilerId.c_str()))
        {
            if (!cmSystemTools::CopyFileAlways(mod_lower.c_str(), stamp.c_str()))
            {
                std::cerr << "Error copying Fortran module from \"" << mod_lower.c_str() << "\" to \"" << stamp.c_str() << "\".\n";
                return false;
            }
        }
        return true;
...
static bool cmDependsFortranStreamContainsSequence(std::istream & ifs, const char *seq, int len)
{
    assert(len > 0);
...
static bool cmDependsFortranStreamsDiffer(std::istream & ifs1, std::istream & ifs2)
{
...
//----------------------------------------------------------------------------
bool cmDependsFortran::ModulesDiffer(const char *modFile, const char *stampFile, const char *compilerId)
{
    /*
       gnu:
    ...
    if (strcmp(compilerId, "GNU") == 0)
    {
        ...
        if (!cmDependsFortranStreamContainsSequence(finModFile, seq, seqlen))
        {
            ...
            std::cerr << compilerId << " fortran module " << modFile << " has unexpected format." << std::endl;
            return true;
        }
...
//----------------------------------------------------------------------------
bool cmDependsFortran::FindIncludeFile(const char *dir, const char *includeName, std::string & fileName)
{
...
//----------------------------------------------------------------------------
cmDependsFortranParser_s::cmDependsFortranParser_s(cmDependsFortran * self, std::set < std::string > &ppDefines, cmDependsFortranSourceInfo & info):
Self(self), PPDefinitions(ppDefines), Info(info)
{
    ...
    YY_BUFFER_STATE buffer = cmDependsFortran_yy_create_buffer(0, 4, this->Scanner);
...
//----------------------------------------------------------------------------
bool cmDependsFortranParser_FilePush(cmDependsFortranParser * parser, const char *fname)
{
    // Open the new file and push it onto the stack.  Save the old
    // buffer with it on the stack.
    if (FILE * file = cmsys::SystemTools::Fopen(fname, "rb"))
    {
        YY_BUFFER_STATE current = cmDependsFortranLexer_GetCurrentBuffer(parser->Scanner);
        std::string dir = cmSystemTools::GetParentDirectory(fname);
        cmDependsFortranFile f(file, current, dir);
        YY_BUFFER_STATE buffer = cmDependsFortran_yy_create_buffer(0, 16384, parser->Scanner);
...
bool cmDependsFortranParser_FilePop(cmDependsFortranParser * parser)
{
    ...
    {
        cmDependsFortranFile f = parser->FileStack.top();
...
//----------------------------------------------------------------------------
int cmDependsFortranParser_Input(cmDependsFortranParser * parser, char *buffer, size_t bufferSize)
{
...
//----------------------------------------------------------------------------
void cmDependsFortranParser_StringAppend(cmDependsFortranParser * parser, char c)
{
    parser->TokenString += c;
}

//----------------------------------------------------------------------------
void cmDependsFortranParser_SetInInterface(cmDependsFortranParser * parser, bool in)
{
...
//----------------------------------------------------------------------------
void cmDependsFortranParser_SetOldStartcond(cmDependsFortranParser * parser, int arg)
{
...
//----------------------------------------------------------------------------
void cmDependsFortranParser_RuleUse(cmDependsFortranParser * parser, const char *name)
{
    if (!parser->InPPFalseBranch)
    {
...
//----------------------------------------------------------------------------
void cmDependsFortranParser_RuleInclude(cmDependsFortranParser * parser, const char *name)
{
    if (parser->InPPFalseBranch)
    {
        return;
...
//----------------------------------------------------------------------------
void cmDependsFortranParser_RuleModule(cmDependsFortranParser * parser, const char *name)
{
...
//----------------------------------------------------------------------------
void cmDependsFortranParser_RuleDefine(cmDependsFortranParser * parser, const char *macro)
{
    if (!parser->InPPFalseBranch)
    {
...
//----------------------------------------------------------------------------
void cmDependsFortranParser_RuleUndef(cmDependsFortranParser * parser, const char *macro)
{
    if (!parser->InPPFalseBranch)
    {
...
//----------------------------------------------------------------------------
void cmDependsFortranParser_RuleIfdef(cmDependsFortranParser * parser, const char *macro)
{
...
//----------------------------------------------------------------------------
void cmDependsFortranParser_RuleIfndef(cmDependsFortranParser * parser, const char *macro)
{
    // A new PP branch has been opened
    parser->SkipToEnd.push(false);
...
void cmDependsFortranParser_RuleElif(cmDependsFortranParser * parser)
{
    ...
    if (!parser->SkipToEnd.empty() && parser->SkipToEnd.top() && !parser->InPPFalseBranch)
    {
        parser->InPPFalseBranch = 1;
    }
}
