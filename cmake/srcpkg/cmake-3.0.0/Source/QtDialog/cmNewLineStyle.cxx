...
bool cmNewLineStyle::ReadFromArguments(const std::vector < std::string > &args, std::string & errorString)
{
    ...
    for (size_t i = 0; i < args.size(); i++)
    {
        if (args[i] == "NEWLINE_STYLE")
        {
            ...
            if (args.size() > styleIndex)
            {
                ...
                {
                    errorString = "NEWLINE_STYLE sets an unknown style, only LF, " "CRLF, UNIX, DOS, and WIN32 are supported";
                    return false;
                }
            }
            else
            {
                errorString = "NEWLINE_STYLE must set a style: " "LF, CRLF, UNIX, DOS, or WIN32";
                return false;
            }
        }
    }
