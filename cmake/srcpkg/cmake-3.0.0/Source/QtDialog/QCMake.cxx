...
QCMake::QCMake(QObject * p):QObject(p)
{
    ...
    this->CMakeInstance->SetCMakeEditCommand(cmSystemTools::GetCMakeGUICommand());
...
{
    QString dir = QString::fromLocal8Bit(cmSystemTools::GetActualCaseForPath(_dir.toLocal8Bit().data()).c_str());
...
void QCMake::setBinaryDirectory(const QString & _dir)
{
    ...
    if (this->BinaryDirectory != dir)
    {
        ...
        if (!this->CMakeInstance->GetCacheManager()->LoadCache(this->BinaryDirectory.toLocal8Bit().data()))
        {
        ...
        if (itm.Find("CMAKE_GENERATOR"))
        {
            ...
            std::string curGen = cmExternalMakefileProjectGenerator::CreateFullGeneratorName(itm.GetValue(), extraGen);
...
void QCMake::configure()
{
    ...
    this->CMakeInstance->SetGlobalGenerator(this->CMakeInstance->CreateGlobalGenerator(this->Generator.toLocal8Bit().data()));
...
void QCMake::setProperties(const QCMakePropertyList & newProps)
{
    ...
    for (cmCacheManager::CacheIterator i = cachem->NewIterator(); !i.IsAtEnd(); i.Next())
    {
        ...
        if (i.GetType() == cmCacheManager::INTERNAL || i.GetType() == cmCacheManager::STATIC)
        {
            continue;
        }
    ...
    foreach(QCMakeProperty s, props)
    {
        ...
        {
            this->CMakeInstance->AddCacheEntry(s.Key.toLocal8Bit().data(), s.Value.toBool()? "ON" : "OFF", s.Help.toLocal8Bit().data(), cmCacheManager::BOOL);
        }
        else if (s.Type == QCMakeProperty::STRING)
        {
            this->CMakeInstance->AddCacheEntry(s.Key.toLocal8Bit().data(), s.Value.toString().toLocal8Bit().data(), s.Help.toLocal8Bit().data(), cmCacheManager::STRING);
        }
        else if (s.Type == QCMakeProperty::PATH)
        {
            this->CMakeInstance->AddCacheEntry(s.Key.toLocal8Bit().data(), s.Value.toString().toLocal8Bit().data(), s.Help.toLocal8Bit().data(), cmCacheManager::PATH);
        }
        else if (s.Type == QCMakeProperty::FILEPATH)
        {
            this->CMakeInstance->AddCacheEntry(s.Key.toLocal8Bit().data(), s.Value.toString().toLocal8Bit().data(), s.Help.toLocal8Bit().data(), cmCacheManager::FILEPATH);
        }
    }
