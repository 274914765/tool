...
public:
    enum Type
    { OBJECT_LIST, STRING, ATTRIBUTE_GROUP, OBJECT_REF, OBJECT };
    enum PBXType
    ...
    {
        for (std::vector < cmXCodeObject * >::iterator i = this->List.begin(); i != this->List.end(); ++i)
        {
            cmXCodeObject *o = *i;
            if (o->IsA == t)
            {
    ...
    void AddDependLibrary(const char *configName, const char *l)
    {
        if (!configName)
        {
            configName = "";
    ...
    void AddDependTarget(const char *configName, const char *tName)
    {
        if (!configName)
        {
            configName = "";
    ...
    std::vector < cmXCodeObject * >const &GetObjectList()
    {
        return this->List;
    }
