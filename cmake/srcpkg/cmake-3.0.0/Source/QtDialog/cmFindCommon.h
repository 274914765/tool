...
protected:
  ...
  /** Remove paths in the ignore set from the supplied vector.  */
    void FilterPaths(std::vector < std::string > &paths, const std::set < std::string > &ignore);
  ...
  /** Compute the current default bundle/framework search policy.  */
    ...
    void AddUserPath(std::string const &p, std::vector < std::string > &paths);
    void AddCMakePath(const char *variable);
    void AddEnvPath(const char *variable);
    void AddPathsInternal(std::vector < std::string > const &in_paths, PathType pathType);
