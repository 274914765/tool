...
// cmLibraryCommand
bool cmVariableRequiresCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (this->Disallowed(cmPolicies::CMP0035, "The variable_requires command should not be called; see CMP0035."))
    {
        return true;
    }
    if (args.size() < 3)
    ...
    for (unsigned int i = 2; i < args.size(); ++i)
    {
        if (!this->Makefile->IsOn(args[i].c_str()))
        {
            ...
            notSet += args[i];
            notSet += "\n";
            cmCacheManager::CacheIterator it = this->Makefile->GetCacheManager()->GetCacheIterator(args[i].c_str());
