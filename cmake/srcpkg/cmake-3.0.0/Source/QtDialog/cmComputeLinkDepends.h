...
{
public:
    cmComputeLinkDepends(cmTarget const *target, const char *config, cmTarget const *head);
    ...
    struct LinkEntry
    {
        ...
        LinkEntry(LinkEntry const &r):Item(r.Item), Target(r.Target), IsSharedDep(r.IsSharedDep), IsFlag(r.IsFlag)
        {
        }
    };
...
private:
    ...
    std::map < cmStdString, int >::iterator AllocateLinkEntry(std::string const &item);
    int AddLinkEntry(int depender_index, std::string const &item);
    void AddVarLinkEntries(int depender_index, const char *value);
    void AddDirectLinkEntries();
    void AddLinkEntries(int depender_index, std::vector < std::string > const &libs);
    ...
    void FollowSharedDeps(int depender_index, cmTarget::LinkInterface const *iface, bool follow_interface = false);
    void QueueSharedDependencies(int depender_index, std::vector < std::string > const &deps);
