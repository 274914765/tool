...
static cmVS7FlagTable const *cmVSGetCLFlagTable(cmLocalVisualStudioGenerator * lg)
{
...
static cmVS7FlagTable const *cmVSGetLibFlagTable(cmLocalVisualStudioGenerator * lg)
{
    if (lg->GetVersion() >= cmLocalVisualStudioGenerator::VS12)
    {
...
static cmVS7FlagTable const *cmVSGetLinkFlagTable(cmLocalVisualStudioGenerator * lg)
{
    if (lg->GetVersion() >= cmLocalVisualStudioGenerator::VS12)
    {
...
cmVisualStudio10TargetGenerator::cmVisualStudio10TargetGenerator(cmTarget * target, cmGlobalVisualStudio10Generator * gg)
{
    this->GlobalGenerator = gg;
    this->Target = target;
    this->GeneratorTarget = gg->GetGeneratorTarget(target);
    this->Makefile = target->GetMakefile();
    this->LocalGenerator = (cmLocalVisualStudio7Generator *) this->Makefile->GetLocalGenerator();
...
{
    for (OptionsMap::iterator i = this->ClOptions.begin(); i != this->ClOptions.end(); ++i)
    {
        delete i->second;
    }
    for (OptionsMap::iterator i = this->LinkOptions.begin(); i != this->LinkOptions.end(); ++i)
    {
        delete i->second;
    }
...
void cmVisualStudio10TargetGenerator::WritePlatformConfigTag(const char *tag, const char *config, int indentLevel, const char *attribute, const char *end, std::ostream * stream)
{
    if (!stream)
    {
...
void cmVisualStudio10TargetGenerator::WriteString(const char *line, int indentLevel)
{
...
void cmVisualStudio10TargetGenerator::Generate()
{
    ...
    if (this->Target->GetType() == cmTarget::INTERFACE_LIBRARY || this->Target->GetProperty("EXTERNAL_MSPROJECT"))
    {
        return;
    }
    // Tell the global generator the name of the project file
    this->Target->SetProperty("GENERATOR_FILE_NAME", this->Name.c_str());
    this->Target->SetProperty("GENERATOR_FILE_NAME_EXT", ".vcxproj");
    ...
    const char *vsProjectTypes = this->Target->GetProperty("VS_GLOBAL_PROJECT_TYPES");
    if (vsProjectTypes)
    {
        this->WriteString("<ProjectTypes>", 2);
        (*this->BuildFileStream) << cmVS10EscapeXML(vsProjectTypes) << "</ProjectTypes>\n";
    }
    ...
    if (vsProjectName && vsLocalPath && vsProvider)
    {
        ...
        (*this->BuildFileStream) << cmVS10EscapeXML(vsProjectName) << "</SccProjectName>\n";
        this->WriteString("<SccLocalPath>", 2);
        (*this->BuildFileStream) << cmVS10EscapeXML(vsLocalPath) << "</SccLocalPath>\n";
        this->WriteString("<SccProvider>", 2);
        (*this->BuildFileStream) << cmVS10EscapeXML(vsProvider) << "</SccProvider>\n";

        const char *vsAuxPath = this->Target->GetProperty("VS_SCC_AUXPATH");
        if (vsAuxPath)
        {
            this->WriteString("<SccAuxPath>", 2);
            (*this->BuildFileStream) << cmVS10EscapeXML(vsAuxPath) << "</SccAuxPath>\n";
        }
    }

    const char *vsGlobalKeyword = this->Target->GetProperty("VS_GLOBAL_KEYWORD");
    if (!vsGlobalKeyword)
    {
    ...
    else
    {
        ...
        (*this->BuildFileStream) << cmVS10EscapeXML(vsGlobalKeyword) << "</Keyword>\n";
    }

    const char *vsGlobalRootNamespace = this->Target->GetProperty("VS_GLOBAL_ROOTNAMESPACE");
    if (vsGlobalRootNamespace)
    {
        this->WriteString("<RootNamespace>", 2);
        (*this->BuildFileStream) << cmVS10EscapeXML(vsGlobalRootNamespace) << "</RootNamespace>\n";
    }
    ...
    if (const char *targetFrameworkVersion = this->Target->GetProperty("VS_DOTNET_TARGET_FRAMEWORK_VERSION"))
    {
        this->WriteString("<TargetFrameworkVersion>", 2);
        (*this->BuildFileStream) << targetFrameworkVersion << "</TargetFrameworkVersion>\n";
    }
    this->WriteString("</PropertyGroup>\n", 1);
    this->WriteString("<Import Project=" "\"$(VCTargetsPath)\\Microsoft.Cpp.Default.props\" />\n", 1);
    this->WriteProjectConfigurationValues();
    this->WriteString("<Import Project=\"$(VCTargetsPath)\\Microsoft.Cpp.props\" />\n", 1);
    ...
    this->WriteString("<Import Project=\"" VS10_USER_PROPS "\"" " Condition=\"exists('" VS10_USER_PROPS "')\"" " Label=\"LocalAppDataPlatform\" />", 2);
    this->WriteString("</ImportGroup>\n", 1);
    ...
    this->WriteString("<Import Project=\"$(VCTargetsPath)\\Microsoft.Cpp.targets\"" " />\n", 1);
...
void cmVisualStudio10TargetGenerator::WriteDotNetReferences()
{
    ...
    if (const char *vsDotNetReferences = this->Target->GetProperty("VS_DOTNET_REFERENCES"))
    {
    ...
    if (!references.empty())
    {
        ...
        for (std::vector < std::string >::iterator ri = references.begin(); ri != references.end(); ++ri)
        {
            this->WriteString("<Reference Include=\"", 2);
            (*this->BuildFileStream) << cmVS10EscapeXML(*ri) << "\">\n";
            this->WriteString("<CopyLocalSatelliteAssemblies>true" "</CopyLocalSatelliteAssemblies>\n", 3);
            this->WriteString("<ReferenceOutputAssembly>true" "</ReferenceOutputAssembly>\n", 3);
...
void cmVisualStudio10TargetGenerator::WriteEmbeddedResourceGroup()
{
    ...
    {
        this->WriteString("<ItemGroup>\n", 1);
        for (std::vector < cmSourceFile * >::const_iterator oi = resxObjs.begin(); oi != resxObjs.end(); ++oi)
        {
            ...
            std::vector < std::string > const *configs = this->GlobalGenerator->GetConfigurations();
            for (std::vector < std::string >::const_iterator i = configs->begin(); i != configs->end(); ++i)
            {
...
{
    std::vector < std::string > references;
    if (const char *vsWinRTReferences = this->Target->GetProperty("VS_WINRT_REFERENCES"))
    {
...
{
    cmGlobalVisualStudio10Generator *gg = static_cast < cmGlobalVisualStudio10Generator * >(this->GlobalGenerator);
    std::vector < std::string > *configs = static_cast < cmGlobalVisualStudio7Generator * >(this->GlobalGenerator)->GetConfigurations();
    for (std::vector < std::string >::iterator i = configs->begin(); i != configs->end(); ++i)
    {
        this->WritePlatformConfigTag("PropertyGroup", i->c_str(), 1, " Label=\"Configuration\"", "\n");
        ...
        const char *mfcFlag = this->Target->GetMakefile()->GetDefinition("CMAKE_MFC_FLAG");
        ...
        if ((this->Target->GetType() <= cmTarget::OBJECT_LIBRARY && this->ClOptions[*i]->UsingUnicode()) || this->Target->GetPropertyAsBool("VS_WINRT_EXTENSIONS"))
        {
            this->WriteString("<CharacterSet>Unicode</CharacterSet>\n", 2);
        }
        else if (this->Target->GetType() <= cmTarget::MODULE_LIBRARY && this->ClOptions[*i]->UsingSBCS())
        {
...
void cmVisualStudio10TargetGenerator::WriteCustomCommand(cmSourceFile * sf)
{
    ...
    {
        if (std::vector < cmSourceFile * >const *depends = this->GeneratorTarget->GetSourceDepends(sf))
        {
            for (std::vector < cmSourceFile * >::const_iterator di = depends->begin(); di != depends->end(); ++di)
            {
...
void cmVisualStudio10TargetGenerator::WriteCustomRule(cmSourceFile * source, cmCustomCommand const &command)
{
    ...
    if (source->GetPropertyAsBool("__CMAKE_RULE"))
    {
        if (!cmSystemTools::FileExists(sourcePath.c_str()))
        {
            ...
            else
            {
                ...
                error += sourcePath;
                error += "]  ";
                cmSystemTools::Error(error.c_str(), cmSystemTools::GetLastSystemError().c_str());
            }
        }
    }
    ...
    for (std::vector < std::string >::iterator i = configs->begin(); i != configs->end(); ++i)
    {
        std::string script = cmVS10EscapeXML(lg->ConstructScript(command, i->c_str()));
...
std::string cmVisualStudio10TargetGenerator::ConvertPath(std::string const &path, bool forceRelative)
{
    return forceRelative ? cmSystemTools::RelativePath(this->Makefile->GetCurrentOutputDirectory(), path.c_str()) : this->LocalGenerator->Convert(path.c_str(), cmLocalGenerator::START_OUTPUT, cmLocalGenerator::UNCHANGED);
}
...
void cmVisualStudio10TargetGenerator::WriteGroups()
{
    ...
    this->WriteString("<ItemGroup>\n", 1);
    for (std::set < cmSourceGroup * >::iterator g = groupsUsed.begin(); g != groupsUsed.end(); ++g)
    {
        cmSourceGroup *sg = *g;
        ...
        if (strlen(name) != 0)
        {
            ...
            (*this->BuildFileStream) << "{" << guid << "}" << "</UniqueIdentifier>\n";
...
void cmVisualStudio10TargetGenerator::AddMissingSourceGroups(std::set < cmSourceGroup * >&groupsUsed, const std::vector < cmSourceGroup > &allGroups)
{
    for (std::vector < cmSourceGroup >::const_iterator current = allGroups.begin(); current != allGroups.end(); ++current)
    {
...
void cmVisualStudio10TargetGenerator:: WriteGroupSources(const char *name, ToolSources const &sources, std::vector < cmSourceGroup > &sourceGroups)
{
    this->WriteString("<ItemGroup>\n", 1);
...
void cmVisualStudio10TargetGenerator::WriteSource(const char *tool, cmSourceFile * sf, const char *end)
{
    ...
    if (this->LocalGenerator->GetVersion() == cmLocalVisualStudioGenerator::VS10 && cmSystemTools::FileIsFullPath(sourceFile.c_str()))
    {
        ...
        if (sf->GetCustomCommand() || ((strlen(this->Makefile->GetCurrentOutputDirectory()) + 1 + sourceRel.length()) <= maxLen))
        {
            forceRelative = true;
    ...
    if (sf->GetExtension() == "h" && this->IsResxHeader(sf->GetFullPath()))
    {
        (*this->BuildFileStream) << ">\n";
...
void cmVisualStudio10TargetGenerator::WriteSources(const char *tool, std::vector < cmSourceFile * >const &sources)
{
...
void cmVisualStudio10TargetGenerator::WriteAllSources()
{
    ...
    for (std::vector < cmSourceFile * >::const_iterator si = objectSources.begin(); si != objectSources.end(); ++si)
    {
        ...
        else if (strcmp(lang, "ASM_MASM") == 0 && this->GlobalGenerator->IsMasmEnabled())
        {
            tool = "MASM";
        }
    ...
    else
    {
        ...
        {
            std::vector < cmSourceFile * >const *d = this->GeneratorTarget->GetSourceDepends(*si);
...
bool cmVisualStudio10TargetGenerator::OutputSourceSpecificFlags(cmSourceFile * source)
{
    ...
    const char *lang = this->GlobalGenerator->GetLanguageFromExtension(sf.GetExtension().c_str());
    ...
    for (std::vector < std::string >::iterator config = configs->begin(); config != configs->end(); ++config)
    {
        ...
        if (flags.size() || configDefines.size())
        {
            ...
            hasFlags = true;
            cmVisualStudioGeneratorOptions clOptions(this->LocalGenerator, cmVisualStudioGeneratorOptions::Compiler, cmVSGetCLFlagTable(this->LocalGenerator), 0, this);
...
void cmVisualStudio10TargetGenerator::WritePathAndIncrementalLinkOptions()
{
    ...
    this->WriteString("<PropertyGroup>\n", 2);
    this->WriteString("<_ProjectFileVersion>10.0.20506.1" "</_ProjectFileVersion>\n", 3);
    std::vector < std::string > *configs = static_cast < cmGlobalVisualStudio7Generator * >(this->GlobalGenerator)->GetConfigurations();
    for (std::vector < std::string >::iterator config = configs->begin(); config != configs->end(); ++config)
    {
        if (ttype >= cmTarget::UTILITY)
        {
            ...
            *this->BuildFileStream << "$(Platform)\\$(Configuration)\\$(ProjectName)\\" << "</IntDir>\n";
        }
        else
        {
            std::string intermediateDir = this->LocalGenerator->GetTargetDirectory(*this->Target);
            intermediateDir += "/";
            ...
            *this->BuildFileStream << outDir << "</OutDir>\n";

            this->WritePlatformConfigTag("IntDir", config->c_str(), 3);
            *this->BuildFileStream << intermediateDir << "</IntDir>\n";

            this->WritePlatformConfigTag("TargetName", config->c_str(), 3);
            *this->BuildFileStream << cmSystemTools::GetFilenameWithoutLastExtension(targetNameFull.c_str()) << "</TargetName>\n";

            this->WritePlatformConfigTag("TargetExt", config->c_str(), 3);
            *this->BuildFileStream << cmSystemTools::GetFilenameLastExtension(targetNameFull.c_str()) << "</TargetExt>\n";
...
void cmVisualStudio10TargetGenerator:: OutputLinkIncremental(std::string const &configName)
{
    ...
    *this->BuildFileStream << (manifest ? manifest : "true") << "</GenerateManifest>\n";
...
//----------------------------------------------------------------------------
bool cmVisualStudio10TargetGenerator::ComputeClOptions(std::string const &configName)
{
    // much of this was copied from here:
    // copied from cmLocalVisualStudio7Generator.cxx 805
    // TODO: Integrate code below with cmLocalVisualStudio7Generator.

    cmsys::auto_ptr < Options > pOptions(new Options(this->LocalGenerator, Options::Compiler, cmVSGetCLFlagTable(this->LocalGenerator)));
    ...
    if (this->Target->GetType() < cmTarget::UTILITY)
    {
        const char *linkLanguage = this->Target->GetLinkerLanguage(configName.c_str());
        if (!linkLanguage)
        {
            cmSystemTools::Error("CMake can not determine linker language for target: ", this->Name.c_str());
            return false;
        }
        if (strcmp(linkLanguage, "C") == 0 || strcmp(linkLanguage, "CXX") == 0 || strcmp(linkLanguage, "Fortran") == 0)
        {
            std::string baseFlagVar = "CMAKE_";
            baseFlagVar += linkLanguage;
            baseFlagVar += "_FLAGS";
            flags = this->Target->GetMakefile()->GetRequiredDefinition(baseFlagVar.c_str());
            std::string flagVar = baseFlagVar + std::string("_") + cmSystemTools::UpperCase(configName);
            flags += " ";
            flags += this->Target->GetMakefile()->GetRequiredDefinition(flagVar.c_str());
        }
        ...
        this->LocalGenerator->AddCompileOptions(flags, this->Target, linkLanguage, configName.c_str());
    }
    ...
    clOptions.SetVerboseMakefile(this->Makefile->IsOn("CMAKE_VERBOSE_MAKEFILE"));
...
//----------------------------------------------------------------------------
void cmVisualStudio10TargetGenerator::WriteClOptions(std::string const &configName, std::vector < std::string > const &includes)
{
...
void cmVisualStudio10TargetGenerator:: OutputIncludes(std::vector < std::string > const &includes)
{
...
void cmVisualStudio10TargetGenerator:: WriteRCOptions(std::string const &configName, std::vector < std::string > const &includes)
{
...
void cmVisualStudio10TargetGenerator::WriteLibOptions(std::string const &config)
{
    ...
    std::string libflags;
    this->LocalGenerator->GetStaticLibraryFlags(libflags, cmSystemTools::UpperCase(config), this->Target);
    if (!libflags.empty())
    {
        this->WriteString("<Lib>\n", 2);
        cmVisualStudioGeneratorOptions libOptions(this->LocalGenerator, cmVisualStudioGeneratorOptions::Linker, cmVSGetLibFlagTable(this->LocalGenerator), 0, this);
...
{
    if (this->Target->GetType() == cmTarget::EXECUTABLE || this->Target->GetType() == cmTarget::SHARED_LIBRARY || this->Target->GetType() == cmTarget::MODULE_LIBRARY)
    {
        std::vector < std::string > const *configs = this->GlobalGenerator->GetConfigurations();
        for (std::vector < std::string >::const_iterator i = configs->begin(); i != configs->end(); ++i)
        {
...
//----------------------------------------------------------------------------
bool cmVisualStudio10TargetGenerator::ComputeLinkOptions(std::string const &config)
{
    cmsys::auto_ptr < Options > pOptions(new Options(this->LocalGenerator, Options::Linker, cmVSGetLinkFlagTable(this->LocalGenerator), 0, this));
    Options & linkOptions = *pOptions;

    const char *linkLanguage = this->Target->GetLinkerLanguage(config.c_str());
    if (!linkLanguage)
    {
        cmSystemTools::Error("CMake can not determine linker language for target: ", this->Name.c_str());
        return false;
    ...
    flags += " ";
    flags += this->Target->GetMakefile()->GetRequiredDefinition(linkFlagVarBase.c_str());
    std::string linkFlagVar = linkFlagVarBase + "_" + CONFIG;
    flags += " ";
    flags += this->Target->GetMakefile()->GetRequiredDefinition(linkFlagVar.c_str());
    ...
    std::string libs = this->Makefile->GetSafeDefinition(standardLibsVar.c_str());
    ...
    cmComputeLinkInformation *pcli = this->Target->GetLinkInformation(config.c_str());
    if (!pcli)
    {
        cmSystemTools::Error("CMake can not compute cmComputeLinkInformation for target: ", this->Name.c_str());
        return false;
    }
    ...
    const char *sep = "";
    std::string linkDirs;
    for (std::vector < std::string >::const_iterator d = ldirs.begin(); d != ldirs.end(); ++d)
    {
    ...
    if (this->Target->GetType() == cmTarget::EXECUTABLE)
    {
        this->Target->GetExecutableNames(targetName, targetNameFull, targetNameImport, targetNamePDB, config.c_str());
    }
    else
    {
        this->Target->GetLibraryNames(targetName, targetNameSO, targetNameFull, targetNameImport, targetNamePDB, config.c_str());
    }
    ...
    {
        linkOptions.AddFlag("ModuleDefinitionFile", this->GeneratorTarget->ModuleDefinitionFile.c_str());
    }
...
//----------------------------------------------------------------------------
void cmVisualStudio10TargetGenerator::WriteLinkOptions(std::string const &config)
{
    if (this->Target->GetType() == cmTarget::STATIC_LIBRARY || this->Target->GetType() > cmTarget::MODULE_LIBRARY)
    {
        return;
    ...
    if (!this->GlobalGenerator->NeedLinkLibraryDependencies(*this->Target))
    {
        ...
        this->WriteString("  <LinkLibraryDependencies>false</LinkLibraryDependencies>\n", 2);
        this->WriteString("</ProjectReference>\n", 2);
    }
}

void cmVisualStudio10TargetGenerator::AddLibraries(cmComputeLinkInformation & cli, std::string & libstring)
{
    ...
    for (ItemVector::const_iterator l = libs.begin(); l != libs.end(); ++l)
    {
        if (l->IsPath)
        {
            std::string path = this->LocalGenerator->Convert(l->Value.c_str(), cmLocalGenerator::START_OUTPUT, cmLocalGenerator::UNCHANGED);
            this->ConvertToWindowsSlash(path);
            libstring += sep;
            libstring += path;
        }
...
void cmVisualStudio10TargetGenerator:: WriteMidlOptions(std::string const & /*config */ ,
                                                        std::vector < std::string > const &includes)
{
    ...
    this->WriteString("<InterfaceIdentifierFileName>" "%(Filename)_i.c</InterfaceIdentifierFileName>\n", 3);
...
void cmVisualStudio10TargetGenerator::WriteItemDefinitionGroups()
{
    ...
    for (std::vector < std::string >::iterator i = configs->begin(); i != configs->end(); ++i)
    {
        ...
        this->LocalGenerator->GetIncludeDirectories(includes, this->GeneratorTarget, "C", i->c_str());
...
void cmVisualStudio10TargetGenerator::WriteEvents(std::string const &configName)
{
    this->WriteEvent("PreLinkEvent", this->Target->GetPreLinkCommands(), configName);
    this->WriteEvent("PreBuildEvent", this->Target->GetPreBuildCommands(), configName);
    this->WriteEvent("PostBuildEvent", this->Target->GetPostBuildCommands(), configName);
}

void cmVisualStudio10TargetGenerator::WriteEvent(const char *name, std::vector < cmCustomCommand > const &commands, std::string const &configName)
{
    if (commands.size() == 0)
    {
        return;
    ...
    for (std::vector < cmCustomCommand >::const_iterator i = commands.begin(); i != commands.end(); ++i)
    {
        ...
        script += pre;
        pre = "\n";
        script += cmVS10EscapeXML(lg->ConstructScript(command, configName.c_str()));
    }
    comment = cmVS10EscapeComment(comment);
...
{
    cmGlobalGenerator::TargetDependSet const &unordered = this->GlobalGenerator->GetTargetDirectDepends(*this->Target);
    typedef cmGlobalVisualStudioGenerator::OrderedTargetDependSet OrderedTargetDependSet;
    OrderedTargetDependSet depends(unordered);
    this->WriteString("<ItemGroup>\n", 1);
    for (OrderedTargetDependSet::const_iterator i = depends.begin(); i != depends.end(); ++i)
    {
        cmTarget const *dt = *i;
        ...
        if (static_cast < cmGlobalVisualStudioGenerator * >(this->GlobalGenerator)->TargetIsFortranOnly(*dt))
        {
            continue;
        }
        ...
        (*this->BuildFileStream) << this->GlobalGenerator->GetGUID(name.c_str()) << "</Project>\n";
        this->WriteString("</ProjectReference>\n", 2);
    }
    this->WriteString("</ItemGroup>\n", 1);
}

bool cmVisualStudio10TargetGenerator::IsResxHeader(const std::string & headerFile)
{
    std::set < std::string > expectedResxHeaders;
    this->GeneratorTarget->GetExpectedResxHeaders(expectedResxHeaders);

    std::set < std::string >::const_iterator it = expectedResxHeaders.find(headerFile);
