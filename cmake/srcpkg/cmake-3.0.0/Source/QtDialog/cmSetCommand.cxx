...
// cmSetCommand
bool cmSetCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 1)
    {
    ...
    if (static_cast < unsigned short >(args.size()) > static_cast < unsigned short >(1 + ignoreLastArgs))
    {
        value = args[1];
    ...
    if ((args[args.size() - 1] == "CACHE") || (args.size() > 1 && args[args.size() - 2] == "CACHE") || (force && !cache))
    {
    ...
    cmCacheManager::CacheIterator it = this->Makefile->GetCacheManager()->GetCacheIterator(variable);
    ...
    if (cache)
    {
        this->Makefile->AddCacheDefinition(variable, value.c_str(), docstring, type, force);
    }
    else
    {
        // add the definition
