...
//----------------------------------------------------------------------------
cmSourceFileLocation::cmSourceFileLocation(cmMakefile const *mf, const char *name):Makefile(mf)
{
...
void cmSourceFileLocation::DirectoryUseSource()
{
    if (this->AmbiguousDirectory)
    {
        this->Directory = cmSystemTools::CollapseFullPath(this->Directory.c_str(), this->Makefile->GetCurrentDirectory());
        this->AmbiguousDirectory = false;
    }
}
...
void cmSourceFileLocation::DirectoryUseBinary()
{
    if (this->AmbiguousDirectory)
    {
        this->Directory = cmSystemTools::CollapseFullPath(this->Directory.c_str(), this->Makefile->GetCurrentOutputDirectory());
        this->AmbiguousDirectory = false;
    }
}
...
void cmSourceFileLocation::UpdateExtension(const char *name)
{
    ...
    if (gg->GetLanguageFromExtension(ext.c_str()) || std::find(srcExts.begin(), srcExts.end(), ext) != srcExts.end() || std::find(hdrExts.begin(), hdrExts.end(), ext) != hdrExts.end())
    {
...
//----------------------------------------------------------------------------
bool cmSourceFileLocation::MatchesAmbiguousExtension(cmSourceFileLocation const &loc) const
{
    ...
    // adding an extension.
    if (!(this->Name.size() > loc.Name.size() && this->Name.substr(0, loc.Name.size()) == loc.Name && this->Name[loc.Name.size()] == '.'))
    {
        return false;
    }
...
bool cmSourceFileLocation::Matches(cmSourceFileLocation const &loc)
{
    ...
    else if (this->AmbiguousDirectory && loc.AmbiguousDirectory && this->Makefile == loc.Makefile)
    {
    ...
    else if (this->AmbiguousDirectory && loc.AmbiguousDirectory)
    {
        ...
        this->Makefile->IssueMessage(cmake::INTERNAL_ERROR, "Matches error: Each side has a directory relative to a different " "location. This can occur when referencing a source file from a " "different directory.  This is not yet allowed.");
        return false;
    }
    else if (this->AmbiguousDirectory)
    {
        // Compare possible directory combinations.
        std::string srcDir = cmSystemTools::CollapseFullPath(this->Directory.c_str(), this->Makefile->GetCurrentDirectory());
        std::string binDir = cmSystemTools::CollapseFullPath(this->Directory.c_str(), this->Makefile->GetCurrentOutputDirectory());
    ...
    {
        // Compare possible directory combinations.
        std::string srcDir = cmSystemTools::CollapseFullPath(loc.Directory.c_str(), loc.Makefile->GetCurrentDirectory());
        std::string binDir = cmSystemTools::CollapseFullPath(loc.Directory.c_str(), loc.Makefile->GetCurrentOutputDirectory());
