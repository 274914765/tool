...
//----------------------------------------------------------------------------
cmSourceFile::cmSourceFile(cmMakefile * mf, const char *name):Location(mf, name)
{
...
const char *cmSourceFile::GetLanguage()
{
    ...
    if (this->FullPath.empty() && this->Language.empty())
    {
        ...
        if (this->Location.ExtensionIsAmbiguous() && this->Location.DirectoryIsAmbiguous())
        {
        ...
        else
        {
            ...
            std::string ext = cmSystemTools::GetFilenameLastExtension(this->Location.GetName());
...
bool cmSourceFile::FindFullPath(std::string * error)
{
    ...
    for (const char *const *di = tryDirs; *di; ++di)
    {
        ...
        for (std::vector < std::string >::const_iterator ei = srcExts.begin(); ei != srcExts.end(); ++ei)
        {
        ...
        for (std::vector < std::string >::const_iterator ei = hdrExts.begin(); ei != hdrExts.end(); ++ei)
        {
            if (this->TryFullPath(tryPath.c_str(), ei->c_str()))
            {
                return true;
    ...
    for (std::vector < std::string >::const_iterator ext = srcExts.begin(); ext != srcExts.end(); ++ext)
    {
        e << " ." << *ext;
    }
    for (std::vector < std::string >::const_iterator ext = hdrExts.begin(); ext != hdrExts.end(); ++ext)
    {
        e << " ." << *ext;
    }
    if (error)
...
void cmSourceFile::CheckExtension()
{
    ...
    std::string realExt = cmSystemTools::GetFilenameLastExtension(this->FullPath);
    if (!realExt.empty())
    {
    ...
    if (this->Extension == "obj" || this->Extension == "o" || this->Extension == "lo")
    {
...
//----------------------------------------------------------------------------
void cmSourceFile::AppendProperty(const char *prop, const char *value, bool asString)
{
    if (!prop)
    {
        return;
    }
    this->Properties.AppendProperty(prop, value, cmProperty::SOURCE_FILE, asString);
}
...
const char *cmSourceFile::GetProperty(const char *prop) const
{
    ...
    bool chain = false;
    const char *retVal = this->Properties.GetPropertyValue(prop, cmProperty::SOURCE_FILE, chain);
    if (chain)
    {
        cmMakefile const *mf = this->Location.GetMakefile();
