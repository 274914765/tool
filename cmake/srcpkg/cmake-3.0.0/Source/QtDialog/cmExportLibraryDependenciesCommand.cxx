...
bool cmExportLibraryDependenciesCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (this->Disallowed(cmPolicies::CMP0033, "The export_library_dependencies command should not be called; " "see CMP0033."))
    {
        return true;
    }
    if (args.size() < 1)
...
void cmExportLibraryDependenciesCommand::ConstFinalPass() const
{
    ...
    if (this->Append)
    {
        cmsys::auto_ptr < cmsys::ofstream > ap(new cmsys::ofstream(this->Filename.c_str(), std::ios::app));
        foutPtr = ap;
    }
    else
    {
        cmsys::auto_ptr < cmGeneratedFileStream > ap(new cmGeneratedFileStream(this->Filename.c_str(), true));
    ...
    for (std::vector < cmLocalGenerator * >::const_iterator i = locals.begin(); i != locals.end(); ++i)
    {
        ...
        for (cmTargets::const_iterator l = tgts.begin(); l != tgts.end(); ++l)
        {
            ...
            if (target.GetType() < cmTarget::STATIC_LIBRARY || target.GetType() > cmTarget::MODULE_LIBRARY)
            {
                continue;
            }
    ...
    // versions.
    const char *vertest = "\"${CMAKE_MAJOR_VERSION}.${CMAKE_MINOR_VERSION}\" GREATER 2.4";
    fout << "# Generated by CMake " << cmVersion::GetCMakeVersion() << "\n\n";
    fout << "if(" << vertest << ")\n";
    fout << "  # Information for CMake 2.6 and above.\n";
    for (std::map < cmStdString, cmStdString >::const_iterator i = libDepsNew.begin(); i != libDepsNew.end(); ++i)
    {
        if (!i->second.empty())
        {
    ...
    for (std::map < cmStdString, cmStdString >::const_iterator i = libDepsOld.begin(); i != libDepsOld.end(); ++i)
    {
        if (!i->second.empty())
        {
            fout << "  set(\"" << i->first << "\" \"" << i->second << "\")\n";
    ...
    for (std::map < cmStdString, cmStdString >::const_iterator i = libTypes.begin(); i != libTypes.end(); ++i)
    {
        if (i->second != "general")
        {
            fout << "  set(\"" << i->first << "\" \"" << i->second << "\")\n";
