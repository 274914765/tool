...
//----------------------------------------------------------------------------
cmDependsC::cmDependsC():ValidDeps(0)
{
}

//----------------------------------------------------------------------------
cmDependsC::cmDependsC(cmLocalGenerator * lg, const char *targetDir, const char *lang, const std::map < std::string, DependencyVector > *validDeps):cmDepends(lg, targetDir), ValidDeps(validDeps)
{
...
cmDependsC::~cmDependsC()
{
    ...
    for (std::map < cmStdString, cmIncludeLines * >::iterator it = this->FileCache.begin(); it != this->FileCache.end(); ++it)
    {
        delete it->second;
    }
}
...
//----------------------------------------------------------------------------
bool cmDependsC::WriteDependencies(const std::set < std::string > &sources, const std::string & obj, std::ostream & makeDepends, std::ostream & internalDepends)
{
    ...
    if (this->ValidDeps != 0)
    {
        std::map < std::string, DependencyVector >::const_iterator tmpIt = this->ValidDeps->find(obj);
        if (tmpIt != this->ValidDeps->end())
        {
            for (DependencyVector::const_iterator i = tmpIt->second.begin(); i != tmpIt->second.end(); ++i)
            {
    ...
    if (!haveDeps)
    {
        ...
        for (std::set < std::string >::const_iterator srcIt = sources.begin(); srcIt != sources.end(); ++srcIt)
        {
            UnscannedEntry root;
            root.FileName = *srcIt;
        ...
        while (!this->Unscanned.empty())
        {
            ...
            std::string fullName;
            if ((srcFiles > 0) || cmSystemTools::FileIsFullPath(current.FileName.c_str()))
            {
            ...
            else if (!current.QuotedLocation.empty() && cmSystemTools::FileExists(current.QuotedLocation.c_str(), true))
            {
            ...
            {
                std::map < cmStdString, cmStdString >::iterator headerLocationIt = this->HeaderLocationCache.find(current.FileName);
                if (headerLocationIt != this->HeaderLocationCache.end())
                {
                    ...
                    for (std::vector < std::string >::const_iterator i = this->IncludePath.begin(); i != this->IncludePath.end(); ++i)
                    {
                        ...
                        tempPathStr = cmSystemTools::CollapseCombinedPath(*i, current.FileName);
            ...
            // regex.
            if (fullName.empty() && this->IncludeRegexComplain.find(current.FileName.c_str()))
            {
            ...
            if (!fullName.empty() && (scanned.find(fullName) == scanned.end()))
            {
                ...
                std::map < cmStdString, cmIncludeLines * >::iterator fileIt = this->FileCache.find(fullName);
                if (fileIt != this->FileCache.end())
                {
                    fileIt->second->Used = true;
                    dependencies.insert(fullName);
                    for (std::vector < UnscannedEntry >::const_iterator incIt = fileIt->second->UnscannedEntries.begin(); incIt != fileIt->second->UnscannedEntries.end(); ++incIt)
                    {
                        if (this->Encountered.find(incIt->FileName) == this->Encountered.end())
                        {
    ...
    for (std::set < cmStdString >::const_iterator i = dependencies.begin(); i != dependencies.end(); ++i)
    {
...
void cmDependsC::WriteCacheFile() const
{
    ...
    for (std::map < cmStdString, cmIncludeLines * >::const_iterator fileIt = this->FileCache.begin(); fileIt != this->FileCache.end(); ++fileIt)
    {
...
void cmDependsC::Scan(std::istream & is, const char *directory, const cmStdString & fullName)
{
    ...
    while (cmSystemTools::GetLineFromStream(is, line))
    {
        ...
        if (this->IncludeRegexLine.find(line.c_str()))
        {
            ...
            if (this->IncludeRegexLine.match(3) == "\"" && !cmSystemTools::FileIsFullPath(entry.FileName.c_str()))
            {
                ...
                entry.QuotedLocation = cmSystemTools::CollapseCombinedPath(directory, entry.FileName);
            }
...
void cmDependsC::SetupTransforms()
{
    ...
    if (!this->TransformRules.empty())
    {
        ...
        const char *sep = "";
        for (TransformRulesType::const_iterator tri = this->TransformRules.begin(); tri != this->TransformRules.end(); ++tri)
        {
            xform += sep;
            xform += tri->first;
            sep = "|";
...
void cmDependsC::TransformLine(std::string & line)
{
    ...
    TransformRulesType::const_iterator tri = this->TransformRules.find(this->IncludeRegexTransform.match(3));
