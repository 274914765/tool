...
bool cmGlobalGenerator::SetGeneratorToolset(std::string const &ts)
{
    ...
    this->CMakeInstance->IssueMessage(cmake::FATAL_ERROR, e.str(), cmListFileBacktrace());
    return false;
}

std::string cmGlobalGenerator::SelectMakeProgram(const char *makeProgram, std::string makeDefault) const
{
    if (cmSystemTools::IsOff(makeProgram))
    {
        makeProgram = this->CMakeInstance->GetCacheDefinition("CMAKE_MAKE_PROGRAM");
        if (cmSystemTools::IsOff(makeProgram))
        {
            makeProgram = makeDefault.c_str();
        }
        if (cmSystemTools::IsOff(makeProgram) && !(makeProgram && *makeProgram))
        {
...
void cmGlobalGenerator::ResolveLanguageCompiler(const std::string & lang, cmMakefile * mf, bool optional) const
{
    ...
    if ((path.size() == 0 || !cmSystemTools::FileExists(path.c_str())) && (optional == false))
    {
        return;
    }
    std::string doc = lang;
    doc += " compiler.";
    const char *cname = this->GetCMakeInstance()->GetCacheManager()->GetCacheValue(langComp.c_str());
    ...
    if (cname && !optional)
    {
        ...
        {
            const char *cvars = this->GetCMakeInstance()->GetProperty("__CMAKE_DELETE_CACHE_CHANGE_VARS_");
            if (cvars)
            {
                changeVars += cvars;
                changeVars += ";";
            ...
            changeVars += ";";
            changeVars += cname;
            this->GetCMakeInstance()->SetProperty("__CMAKE_DELETE_CACHE_CHANGE_VARS_", changeVars.c_str());
        }
    }
    mf->AddCacheDefinition(langComp.c_str(), path.c_str(), doc.c_str(), cmCacheManager::FILEPATH);
}
...
void cmGlobalGenerator::AddBuildExportExportSet(cmExportBuildFileGenerator * gen)
{
    this->BuildExportSets[gen->GetMainExportFileName()] = gen;
...
{
    std::map < std::string, cmExportBuildFileGenerator * >::iterator it = this->BuildExportSets.find(file);
...
{
    const std::map < std::string, cmExportBuildFileGenerator * >::const_iterator it = this->BuildExportSets.find(filename);
    if (it == this->BuildExportSets.end())
    {
        return false;
    }
    return this->BuildExportExportSets.find(filename) == this->BuildExportExportSets.end();
}
...
void cmGlobalGenerator::FindMakeProgram(cmMakefile * mf)
{
    ...
    {
        cmSystemTools::Error("Generator implementation error, " "all generators must specify this->FindMakeProgramFile");
    }
    if (!mf->GetDefinition("CMAKE_MAKE_PROGRAM") || cmSystemTools::IsOff(mf->GetDefinition("CMAKE_MAKE_PROGRAM")))
    {
    ...
    {
        cmOStringStream err;
        err << "CMake was unable to find a build program corresponding to \"" << this->GetName() << "\".  CMAKE_MAKE_PROGRAM is not set.  You " << "probably need to select a different build tool.";
    ...
    if (makeProgram.find(' ') != makeProgram.npos)
    {
        std::string dir;
        std::string file;
        cmSystemTools::SplitProgramPath(makeProgram.c_str(), dir, file);
        ...
        mf->AddCacheDefinition("CMAKE_MAKE_PROGRAM", makeProgram.c_str(), "make program", cmCacheManager::FILEPATH);
    }
    ...
    if (makeProgram.find("xcodebuild") != makeProgram.npos)
    {
        ...
        std::string cmakexbuild = this->CMakeInstance->GetCacheManager()->GetCacheValue("CMAKE_COMMAND");
        cmakexbuild = cmakexbuild.substr(0, cmakexbuild.length() - 5);
        cmakexbuild += "cmakexbuild";

        mf->AddCacheDefinition("CMAKE_MAKE_PROGRAM", cmakexbuild.c_str(), "make program", cmCacheManager::FILEPATH);
    }
}
...
void cmGlobalGenerator::EnableLanguage(std::vector < std::string > const &languages, cmMakefile * mf, bool optional)
{
    if (languages.size() == 0)
    {
    ...
    if (this->TryCompileOuterMakefile)
    {
        ...
        for (std::vector < std::string >::const_iterator li = languages.begin(); li != languages.end(); ++li)
        {
            if (*li == "NONE")
            {
            ...
            else
            {
                ...
                {
                    cmOStringStream e;
                    e << "The test project needs language " << lang << " which is not enabled.";
                    this->TryCompileOuterMakefile->IssueMessage(cmake::FATAL_ERROR, e.str());
                    cmSystemTools::SetFatalErrorOccured();
                    return;
                }
            }
    ...
    if (!mf->GetDefinition("CMAKE_SYSTEM"))
    {
        ...
        mf->AddDefinition("CMAKE_HOST_SYSTEM_VERSION", windowsVersionString.str().c_str());
#endif
    ...
    for (std::vector < std::string >::const_iterator l = languages.begin(); l != languages.end(); ++l)
    {
        ...
        if (!this->GetLanguageEnabled(lang))
        {
            ...
            {
                cmSystemTools::Error("This should not have happened. " "If you see this message, you are probably " "using a broken CMakeLists.txt file or a " "problematic release of CMake");
            }
    ...
    for (std::vector < std::string >::const_iterator l = languages.begin(); l != languages.end(); ++l)
    {
        ...
        if (!compilerFile || !*compilerFile || cmSystemTools::IsNOTFOUND(compilerFile))
        {
        ...
        if (!noCompiler.str().empty())
        {
            ...
            if (!optional)
            {
                ...
                if (!this->CMakeInstance->GetIsInTryCompile())
                {
                    this->PrintCompilerAdvice(noCompiler, lang, mf->GetDefinition(compilerEnv.c_str()));
        ...
        if (!mf->GetDefinition(langLoadedVar.c_str()))
        {
            ...
            {
                cmSystemTools::Error("Could not process cmake module file: ", informationFile.c_str());
            }
        }
        ...
        const char *sharedLibFlags = mf->GetSafeDefinition(sharedLibFlagsVar.c_str());
        if (sharedLibFlags)
        {
...
//----------------------------------------------------------------------------
void cmGlobalGenerator::PrintCompilerAdvice(std::ostream & os, std::string lang, const char *envVar) const
{
    ...
    os << "the CMake cache entry CMAKE_" << lang << "_COMPILER " "to the full path to the compiler, or to the compiler name " "if it is in the PATH.";
}

//----------------------------------------------------------------------------
void cmGlobalGenerator::CheckCompilerIdCompatibility(cmMakefile * mf, std::string lang) const
{
    ...
    if (strcmp(compilerId, "AppleClang") == 0)
    {
        ...
        switch (mf->GetPolicyStatus(cmPolicies::CMP0025))
        {
            case cmPolicies::WARN:
                if (!this->CMakeInstance->GetIsInTryCompile() && mf->PolicyOptionalWarningEnabled("CMAKE_POLICY_WARNING_CMP0025"))
                {
                    cmOStringStream w;
                    w << policies->GetPolicyWarning(cmPolicies::CMP0025) << "\n" "Converting " << lang << " compiler id \"AppleClang\" to \"Clang\" for compatibility.";
            ...
            case cmPolicies::REQUIRED_ALWAYS:
                mf->IssueMessage(cmake::FATAL_ERROR, policies->GetRequiredPolicyError(cmPolicies::CMP0025));
            case cmPolicies::NEW:
    ...
    if (strcmp(compilerId, "QCC") == 0)
    {
        ...
        {
            case cmPolicies::WARN:
                if (!this->CMakeInstance->GetIsInTryCompile() && mf->PolicyOptionalWarningEnabled("CMAKE_POLICY_WARNING_CMP0047"))
                {
                    cmOStringStream w;
                    w << policies->GetPolicyWarning(cmPolicies::CMP0047) << "\n" "Converting " << lang << " compiler id \"QCC\" to \"GNU\" for compatibility.";
                    mf->IssueMessage(cmake::AUTHOR_WARNING, w.str());
                }
            case cmPolicies::OLD:
            ...
            case cmPolicies::REQUIRED_IF_USED:
            case cmPolicies::REQUIRED_ALWAYS:
                mf->IssueMessage(cmake::FATAL_ERROR, policies->GetRequiredPolicyError(cmPolicies::CMP0047));
            case cmPolicies::NEW:
...
//----------------------------------------------------------------------------
const char *cmGlobalGenerator::GetLanguageOutputExtension(cmSourceFile const &source) const
{
    if (const char *lang = source.GetLanguage())
    {
        std::map < cmStdString, cmStdString >::const_iterator it = this->LanguageToOutputExtension.find(lang);
...
const char *cmGlobalGenerator::GetLanguageFromExtension(const char *ext) const
{
    ...
    std::map < cmStdString, cmStdString >::const_iterator it = this->ExtensionToLanguage.find(ext);
...
void cmGlobalGenerator::SetLanguageEnabledMaps(const char *l, cmMakefile * mf)
{
    ...
    // run before
    if (this->LanguageToLinkerPreference.find(l) != this->LanguageToLinkerPreference.end())
    {
        return;
    }

    std::string linkerPrefVar = std::string("CMAKE_") + std::string(l) + std::string("_LINKER_PREFERENCE");
    ...
    std::string ignoreExtensionsVar = std::string("CMAKE_") + std::string(l) + std::string("_IGNORE_EXTENSIONS");
    std::string ignoreExts = mf->GetSafeDefinition(ignoreExtensionsVar.c_str());
    std::vector < std::string > extensionList;
    cmSystemTools::ExpandListArgument(ignoreExts, extensionList);
    for (std::vector < std::string >::iterator i = extensionList.begin(); i != extensionList.end(); ++i)
    {
...
void cmGlobalGenerator::FillExtensionToLanguageMap(const char *l, cmMakefile * mf)
{
...
bool cmGlobalGenerator::IsDependedOn(const char *project, cmTarget const *targetIn)
{
    // Get all local gens for this project
    std::map < cmStdString, std::vector < cmLocalGenerator * > >::const_iterator it = this->ProjectMap.find(project);
    ...
    for (std::vector < cmLocalGenerator * >::const_iterator geIt = it->second.begin(); geIt != it->second.end(); ++geIt)
    {
        cmTargets const &targets = (*geIt)->GetMakefile()->GetTargets();
        for (cmTargets::const_iterator l = targets.begin(); l != targets.end(); l++)
        {
...
void cmGlobalGenerator::Configure()
{
    ...
    lg->GetMakefile()->SetStartDirectory(this->CMakeInstance->GetStartDirectory());
    lg->GetMakefile()->SetStartOutputDirectory(this->CMakeInstance->GetStartOutputDirectory());
    ...
    this->GetCMakeInstance()->AddCacheEntry("CMAKE_NUMBER_OF_LOCAL_GENERATORS", num, "number of local generators", cmCacheManager::INTERNAL);
...
cmExportBuildFileGenerator *cmGlobalGenerator::GetExportedTargetsFile(const std::string & filename) const
{
    std::map < std::string, cmExportBuildFileGenerator * >::const_iterator it = this->BuildExportSets.find(filename);
...
bool cmGlobalGenerator::CheckALLOW_DUPLICATE_CUSTOM_TARGETS() const
{
    ...
    if (!this->CMakeInstance->GetPropertyAsBool("ALLOW_DUPLICATE_CUSTOM_TARGETS"))
    {
        return true;
    }
    ...
    e << "This project has enabled the ALLOW_DUPLICATE_CUSTOM_TARGETS "
        << "global property.  " << "The \"" << this->GetName() << "\" generator does not support " << "duplicate custom targets.  " << "Consider using a Makefiles generator or fix the project to not " << "use duplicate target names.";
...
void cmGlobalGenerator::Generate()
{
...
#ifdef CMAKE_BUILD_WITH_CMAKE
    for (AutogensType::iterator it = autogens.begin(); it != autogens.end(); ++it)
    {
    ...
    for (i = 0; i < this->LocalGenerators.size(); ++i)
    {
        ...
        if (!this->LocalGenerators[i]->GetMakefile()->IsOn("CMAKE_SKIP_INSTALL_RULES"))
        {
            this->LocalGenerators[i]->GenerateInstallRules();
        }
        this->LocalGenerators[i]->GenerateTestFiles();
        this->CMakeInstance->UpdateProgress("Generating", (static_cast < float >(i) + 1.0f) /static_cast < float >(this->LocalGenerators.size()));
    }
    this->SetCurrentLocalGenerator(0);

    for (std::map < std::string, cmExportBuildFileGenerator * >::iterator it = this->BuildExportSets.begin(); it != this->BuildExportSets.end(); ++it)
    {
        if (!it->second->GenerateImportFile() && !cmSystemTools::GetErrorOccuredFlag())
        {
            this->GetCMakeInstance()->IssueMessage(cmake::FATAL_ERROR, "Could not write export file.", cmListFileBacktrace());
            return;
        }
    }
    // Update rule hashes.
    ...
    {
        cmOStringStream w;
        w << (this->GetCMakeInstance()->GetPolicies()->GetPolicyWarning(cmPolicies::CMP0042)) << "\n";
        w << "MACOSX_RPATH is not specified for" " the following targets:\n";
        for (std::set < std::string >::iterator iter = this->CMP0042WarnTargets.begin(); iter != this->CMP0042WarnTargets.end(); ++iter)
        {
            w << " " << *iter << "\n";
        }
        this->GetCMakeInstance()->IssueMessage(cmake::AUTHOR_WARNING, w.str(), cmListFileBacktrace());
    }
...
bool cmGlobalGenerator::ComputeTargetDepends()
{
    ...
    for (std::vector < cmTarget const *>::const_iterator ti = targets.begin(); ti != targets.end(); ++ti)
    {
    ...
    for (unsigned int i = 0; i < this->LocalGenerators.size(); ++i)
    {
        ...
        for (cmTargets::iterator ti = targets.begin(); ti != targets.end(); ++ti)
        {
            ...
            if (target.GetType() == cmTarget::EXECUTABLE ||
                target.GetType() == cmTarget::STATIC_LIBRARY || target.GetType() == cmTarget::SHARED_LIBRARY || target.GetType() == cmTarget::MODULE_LIBRARY || target.GetType() == cmTarget::OBJECT_LIBRARY || target.GetType() == cmTarget::UTILITY)
            {
...
void cmGlobalGenerator::CreateQtAutoGeneratorsTargets(AutogensType & autogens)
{
    ...
    for (unsigned int i = 0; i < this->LocalGenerators.size(); ++i)
    {
        ...
        {
            cmTarget & target = ti->second;
            if (target.GetType() == cmTarget::EXECUTABLE || target.GetType() == cmTarget::STATIC_LIBRARY || target.GetType() == cmTarget::SHARED_LIBRARY || target.GetType() == cmTarget::MODULE_LIBRARY || target.GetType() == cmTarget::OBJECT_LIBRARY)
            {
                if ((target.GetPropertyAsBool("AUTOMOC") || target.GetPropertyAsBool("AUTOUIC") || target.GetPropertyAsBool("AUTORCC")) && !target.IsImported())
                {
...
void cmGlobalGenerator::FinalizeTargetCompileInfo()
{
    ...
    for (unsigned int i = 0; i < this->LocalGenerators.size(); ++i)
    {
        ...
        const std::vector < cmValueWithOrigin > noconfig_compile_definitions = mf->GetCompileDefinitionsEntries();
        ...
        for (cmTargets::iterator ti = targets.begin(); ti != targets.end(); ++ti)
        {
            ...
            for (std::vector < cmValueWithOrigin >::const_iterator it = noconfig_compile_definitions.begin(); it != noconfig_compile_definitions.end(); ++it)
            {
            ...
            if (polSt == cmPolicies::WARN || polSt == cmPolicies::OLD)
            {
                ...
                for (std::vector < std::string >::const_iterator ci = configs.begin(); ci != configs.end(); ++ci)
                {
                    ...
                    t->AppendProperty(defPropName.c_str(), mf->GetProperty(defPropName.c_str()));
                }
            }
        }
    }
...
void cmGlobalGenerator::CreateGeneratorTargets(cmMakefile * mf)
{
    ...
    for (std::vector < cmTarget * >::const_iterator j = mf->GetOwnedImportedTargets().begin(); j != mf->GetOwnedImportedTargets().end(); ++j)
    {
...
void cmGlobalGenerator::ComputeGeneratorTargetObjects()
{
    ...
    for (unsigned int i = 0; i < this->LocalGenerators.size(); ++i)
    {
        ...
        for (cmGeneratorTargetsType::iterator ti = targets.begin(); ti != targets.end(); ++ti)
        {
            if (ti->second->Target->IsImported() || ti->second->Target->GetType() == cmTarget::INTERFACE_LIBRARY)
            {
                continue;
            }
...
{
    for (cmGeneratorTargetsType::iterator i = this->GeneratorTargets.begin(); i != this->GeneratorTargets.end(); ++i)
    {
        delete i->second;
    }
    this->GeneratorTargets.clear();

    for (std::vector < cmGeneratorExpressionEvaluationFile * >::const_iterator li = this->EvaluationFiles.begin(); li != this->EvaluationFiles.end(); ++li)
    {
        delete *li;
    }
    this->EvaluationFiles.clear();

    for (std::map < std::string, cmExportBuildFileGenerator * >::iterator i = this->BuildExportSets.begin(); i != this->BuildExportSets.end(); ++i)
    {
        delete i->second;
    }
...
//----------------------------------------------------------------------------
cmGeneratorTarget *cmGlobalGenerator::GetGeneratorTarget(cmTarget const *t) const
{
    cmGeneratorTargetsType::const_iterator ti = this->GeneratorTargets.find(t);
    if (ti == this->GeneratorTargets.end())
    {
        this->CMakeInstance->IssueMessage(cmake::INTERNAL_ERROR, "Missing cmGeneratorTarget instance!", cmListFileBacktrace());
        return 0;
    }
    return ti->second;
}
...
//  std::set<cmStdString> notFoundMap;
    ...
    for (unsigned int i = 0; i < this->LocalGenerators.size(); ++i)
    {
        ...
        for (cmTargets::iterator l = targets.begin(); l != targets.end(); l++)
        {
            ...
            const cmTarget::LinkLibraryVectorType & libs = l->second.GetOriginalLinkLibraries();
            for (cmTarget::LinkLibraryVectorType::const_iterator lib = libs.begin(); lib != libs.end(); ++lib)
            {
                if (lib->first.size() > 9 && cmSystemTools::IsNOTFOUND(lib->first.c_str()))
                {
                    std::string varName = lib->first.substr(0, lib->first.size() - 9);
                    cmCacheManager::CacheIterator it = manager->GetCacheIterator(varName.c_str());
            ...
            std::string incDirs = cmGeneratorExpression::Preprocess(incDirProp, cmGeneratorExpression::StripAllGeneratorExpressions);

            cmSystemTools::ExpandListArgument(incDirs.c_str(), incs);

            for (std::vector < std::string >::const_iterator incDir = incs.begin(); incDir != incs.end(); ++incDir)
            {
        ...
        this->CMakeInstance->UpdateProgress("Configuring", 0.9f + 0.1f * (static_cast < float >(i) + 1.0f) /static_cast < float >(this->LocalGenerators.size()));
    }

    if (notFoundMap.size())
    {
        std::string notFoundVars;
        for (std::map < cmStdString, cmStdString >::const_iterator ii = notFoundMap.begin(); ii != notFoundMap.end(); ++ii)
        {
...
int cmGlobalGenerator::TryCompile(const char *srcdir, const char *bindir, const char *projectName, const char *target, bool fast, std::string * output, cmMakefile * mf)
{
    // if this is not set, then this is a first time configure
    // and there is a good chance that the try compile stuff will
    // take the bulk of the time, so try and guess some progress
    // by getting closer and closer to 100 without actually getting there.
    if (!this->CMakeInstance->GetCacheManager()->GetCacheValue("CMAKE_NUMBER_OF_LOCAL_GENERATORS"))
    {
        ...
        this->CMakeInstance->UpdateProgress("Configuring", this->FirstTimeProgress);
    }
...
#    endif                        // WIN32
    ...
    return this->Build(srcdir, bindir, projectName, newTarget.c_str(), output, 0, config, false, fast, this->TryCompileTimeout);
}

void cmGlobalGenerator::GenerateBuildCommand(std::vector < std::string > &makeCommand, const char *, const char *, const char *, const char *, const char *, bool, std::vector < std::string > const &)
{
    makeCommand.push_back("cmGlobalGenerator::GenerateBuildCommand not implemented");
}

int cmGlobalGenerator::Build(const char *, const char *bindir,
                             const char *projectName, const char *target,
                             std::string * output, const char *makeCommandCSTR, const char *config, bool clean, bool fast, double timeout, cmSystemTools::OutputOption outputflag, std::vector < std::string > const &nativeOptions)
{
  /**
   ...
   */
    ...
    if (clean)
    {
        ...
        this->GenerateBuildCommand(cleanCommand, makeCommandCSTR, projectName, bindir, "clean", config, fast);
        if (output)
        {
            *output += "\nRun Clean Command:";
        ...
        if (!cmSystemTools::RunSingleCommand(cleanCommand, outputPtr, &retVal, 0, outputflag, timeout))
        {
    ...
    this->GenerateBuildCommand(makeCommand, makeCommandCSTR, projectName, bindir, target, config, fast, nativeOptions);
    ...
    if (!cmSystemTools::RunSingleCommand(makeCommand, outputPtr, &retVal, 0, outputflag, timeout))
    {
        cmSystemTools::SetRunCommandHideConsole(hideconsole);
        cmSystemTools::Error("Generator: execution of make failed. Make command was: ", makeCommandStr.c_str());
        if (output)
        {
            *output += *outputPtr;
...
//----------------------------------------------------------------------------
std::string cmGlobalGenerator::GenerateCMakeBuildCommand(const char *target, const char *config, const char *native, bool ignoreErrors)
{
...
void cmGlobalGenerator::AddLocalGenerator(cmLocalGenerator * lg)
{
    ...
    const char *numGenC = this->CMakeInstance->GetCacheManager()->GetCacheValue("CMAKE_NUMBER_OF_LOCAL_GENERATORS");
    ...
    float prog = 0.9f * static_cast < float >(this->LocalGenerators.size()) / static_cast < float >(numGen);
    if (prog > 0.9f)
    {
        prog = 0.9f;
    }
...
void cmGlobalGenerator::EnableLanguagesFromGenerator(cmGlobalGenerator * gen, cmMakefile * mf)
{
    this->SetConfiguredFilesPath(gen);
    this->TryCompileOuterMakefile = mf;
    const char *make = gen->GetCMakeInstance()->GetCacheDefinition("CMAKE_MAKE_PROGRAM");
    this->GetCMakeInstance()->AddCacheEntry("CMAKE_MAKE_PROGRAM", make, "make program", cmCacheManager::FILEPATH);
...
bool cmGlobalGenerator::IsExcluded(cmLocalGenerator * root, cmLocalGenerator * gen) const
{
    if (!gen || gen == root)
    {
...
bool cmGlobalGenerator::IsExcluded(cmLocalGenerator * root, cmTarget const &target) const
{
...
void cmGlobalGenerator::GetEnabledLanguages(std::vector < std::string > &lang) const
{
    for (std::map < cmStdString, bool >::const_iterator i = this->LanguageEnabled.begin(); i != this->LanguageEnabled.end(); ++i)
    {
...
void cmGlobalGenerator::FillLocalGeneratorToTargetMap()
{
    ...
    for (std::vector < cmLocalGenerator * >::const_iterator lgi = this->LocalGenerators.begin(); lgi != this->LocalGenerators.end(); ++lgi)
    {
        ...
        for (cmTargets::const_iterator t = targets.begin(); t != targets.end(); ++t)
        {
            ...
            for (cmLocalGenerator * clg = lg; clg && !this->IsExcluded(clg, target); clg = clg->GetParent())
            {
                // This local generator includes the target.
                std::set < cmTarget const *>&targetSet = this->LocalGeneratorToTargetMap[clg];
                ...
                // non-excluded target.
                TargetDependSet const &tgtdeps = this->GetTargetDirectDepends(target);
                for (TargetDependSet::const_iterator ti = tgtdeps.begin(); ti != tgtdeps.end(); ++ti)
                {
...
cmLocalGenerator *cmGlobalGenerator::FindLocalGenerator(const char *start_dir) const
{
    for (std::vector < cmLocalGenerator * >::const_iterator it = this->LocalGenerators.begin(); it != this->LocalGenerators.end(); ++it)
    {
...
//----------------------------------------------------------------------------
cmTarget *cmGlobalGenerator::FindTarget(const char *project, const char *name, bool excludeAliases) const
{
    // if project specific
    if (project)
    {
        std::map < cmStdString, std::vector < cmLocalGenerator * > >::const_iterator gens = this->ProjectMap.find(project);
    ...
    else
    {
        if (!excludeAliases)
        {
            std::map < cmStdString, cmTarget * >::const_iterator ai = this->AliasTargets.find(name);
            if (ai != this->AliasTargets.end())
            {
                return ai->second;
            }
        }
        std::map < cmStdString, cmTarget * >::const_iterator i = this->TotalTargets.find(name);
...
//----------------------------------------------------------------------------
bool cmGlobalGenerator::NameResolvesToFramework(const std::string & libname) const
{
...
void cmGlobalGenerator::CreateDefaultGlobalTargets(cmTargets * targets)
{
    ...
    {
        const char *noPackageAll = mf->GetDefinition("CMAKE_SKIP_PACKAGE_ALL_DEPENDENCY");
    ...
    {
        (*targets)[this->GetPackageTargetName()] = this->CreateGlobalTarget(this->GetPackageTargetName(), "Run CPack packaging tool...", &cpackCommandLines, depends, workingDir.c_str());
    }
    // CPack source
    ...
    if (packageSourceTargetName)
    {
        ...
        if (cmSystemTools::FileExists(configFile.c_str()))
        {
            ...
            cpackCommandLines.push_back(singleLine);
            (*targets)[packageSourceTargetName] = this->CreateGlobalTarget(packageSourceTargetName, "Run CPack packaging tool for source...", &cpackCommandLines, depends, workingDir.c_str());
        }
    }
    ...
    if (mf->IsOn("CMAKE_TESTING_ENABLED"))
    {
        ...
        cpackCommandLines.push_back(singleLine);
        (*targets)[this->GetTestTargetName()] = this->CreateGlobalTarget(this->GetTestTargetName(), "Running tests...", &cpackCommandLines, depends, 0);
    }
    ...
    if (editCacheTargetName)
    {
        ...
        if (!edit_cmd.empty())
        {
            ...
            cpackCommandLines.push_back(singleLine);
            (*targets)[editCacheTargetName] = this->CreateGlobalTarget(editCacheTargetName, "Running CMake cache editor...", &cpackCommandLines, depends, 0);
        }
        else
        {
            ...
            cpackCommandLines.push_back(singleLine);
            (*targets)[editCacheTargetName] = this->CreateGlobalTarget(editCacheTargetName, "No interactive CMake dialog available...", &cpackCommandLines, depends, 0);
        }
    }
    ...
    if (rebuildCacheTargetName)
    {
        ...
        singleLine.push_back("-B$(CMAKE_BINARY_DIR)");
        cpackCommandLines.push_back(singleLine);
        (*targets)[rebuildCacheTargetName] = this->CreateGlobalTarget(rebuildCacheTargetName, "Running CMake to regenerate build system...", &cpackCommandLines, depends, 0);
    }
    ...
    {
        mf->IssueMessage(cmake::WARNING, "CMAKE_SKIP_INSTALL_RULES was enabled even though " "installation rules have been specified");
    }
    else if (this->InstallTargetEnabled && !skipInstallRules)
    {
        if (!cmakeCfgIntDir || !*cmakeCfgIntDir || cmakeCfgIntDir[0] == '.')
        {
            ...
            if (componentsSet->size() > 0)
            {
                ...
                for (it = componentsSet->begin(); it != componentsSet->end(); ++it)
                {
                    ostr << " \"" << it->c_str() << "\"";
                }
            }
            ...
            (*targets)["list_install_components"] = this->CreateGlobalTarget("list_install_components", ostr.str().c_str(), &cpackCommandLines, depends, 0);
        }
        ...
        {
            const char *noall = mf->GetDefinition("CMAKE_SKIP_INSTALL_ALL_DEPENDENCY");
        ...
        cpackCommandLines.push_back(singleLine);
        (*targets)[this->GetInstallTargetName()] = this->CreateGlobalTarget(this->GetInstallTargetName(), "Install the project...", &cpackCommandLines, depends, 0);
        ...
        if (const char *install_local = this->GetInstallLocalTargetName())
        {
            ...
            (*targets)[install_local] = this->CreateGlobalTarget(install_local, "Installing only the local directory...", &cpackCommandLines, depends, 0);
        }
        ...
        if ((install_strip != 0) && (mf->IsSet("CMAKE_STRIP")))
        {
            ...
            (*targets)[install_strip] = this->CreateGlobalTarget(install_strip, "Installing the project stripped...", &cpackCommandLines, depends, 0);
        }
    }
}
...
{
    const char *prop = this->GetCMakeInstance()->GetProperty("PREDEFINED_TARGETS_FOLDER");
...
//----------------------------------------------------------------------------
cmTarget cmGlobalGenerator::CreateGlobalTarget(const char *name, const char *message, const cmCustomCommandLines * commandLines, std::vector < std::string > depends, const char *workingDirectory)
{
    // Package
    cmTarget target;
    ...
    cmCustomCommand cc(0, no_outputs, no_depends, *commandLines, 0, workingDirectory);
...
//----------------------------------------------------------------------------
std::string cmGlobalGenerator::GenerateRuleFile(std::string const &output) const
{
    ...
    if (dir && dir[0] == '$')
    {
        cmSystemTools::ReplaceString(ruleFile, dir, cmake::GetCMakeFilesDirectory());
    }
    return ruleFile;
}

//----------------------------------------------------------------------------
std::string cmGlobalGenerator::GetSharedLibFlagsForLanguage(std::string const &l) const
{
    std::map < cmStdString, cmStdString >::const_iterator it = this->LanguageToOriginalSharedLibFlags.find(l);
...
//----------------------------------------------------------------------------
void cmGlobalGenerator::AppendDirectoryForConfig(const char *, const char *, const char *, std::string &)
{
...
//----------------------------------------------------------------------------
cmGlobalGenerator::TargetDependSet const & cmGlobalGenerator::GetTargetDirectDepends(cmTarget const &target)
{
...
bool cmGlobalGenerator::IsReservedTarget(std::string const &name)
{
    ...
    for (const char **reservedTarget = reservedTargets; *reservedTarget; ++reservedTarget)
    {
        if (name == *reservedTarget)
            return true;
    }
...
void cmGlobalGenerator::SetExternalMakefileProjectGenerator(cmExternalMakefileProjectGenerator * extraGenerator)
{
...
{
    filenames.clear();
    std::copy(this->FilesReplacedDuringGenerate.begin(), this->FilesReplacedDuringGenerate.end(), std::back_inserter(filenames));
}

//----------------------------------------------------------------------------
void cmGlobalGenerator::GetTargetSets(TargetDependSet & projectTargets, TargetDependSet & originalTargets, cmLocalGenerator * root, GeneratorVector const &generators)
{
    // loop over all local generators
    for (std::vector < cmLocalGenerator * >::const_iterator i = generators.begin(); i != generators.end(); ++i)
    {
        ...
        for (cmTargets::iterator l = tgts.begin(); l != tgts.end(); ++l)
        {
            ...
            if (this->IsRootOnlyTarget(target) && target->GetMakefile() != root->GetMakefile())
            {
                continue;
            }
...
{
    return (target->GetType() == cmTarget::GLOBAL_TARGET || strcmp(target->GetName(), this->GetAllTargetName()) == 0);
}

//----------------------------------------------------------------------------
void cmGlobalGenerator::AddTargetDepends(cmTarget const *target, TargetDependSet & projectTargets)
{
...
//----------------------------------------------------------------------------
void cmGlobalGenerator::AddToManifest(const char *config, std::string const &f)
{
...
//----------------------------------------------------------------------------
std::set < cmStdString > const & cmGlobalGenerator::GetDirectoryContent(std::string const &dir, bool needDisk)
{
...
//----------------------------------------------------------------------------
void cmGlobalGenerator::AddRuleHash(const std::vector < std::string > &outputs, std::string const &content)
{
#if defined(CMAKE_BUILD_WITH_CMAKE)
    ...
    RuleHash hash;
    {
        unsigned char const *data = reinterpret_cast < unsigned char const *>(content.c_str());
    ...
    std::string fname = lg->Convert(outputs[0].c_str(), cmLocalGenerator::HOME_OUTPUT);
...
//----------------------------------------------------------------------------
void cmGlobalGenerator::CheckRuleHashes(std::string const &pfile, std::string const &home)
{
#if defined(_WIN32) || defined(__CYGWIN__)
    ...
    while (cmSystemTools::GetLineFromStream(fin, line))
    {
        ...
        std::map < cmStdString, RuleHash >::const_iterator rhi = this->RuleHashes.find(fname);
        ...
        else
        {
            ...
            std::string fpath = cmSystemTools::CollapseFullPath(fname.c_str(), home.c_str());
            if (cmSystemTools::FileExists(fpath.c_str()))
            {
                RuleHash hash;
...
void cmGlobalGenerator::WriteRuleHashes(std::string const &pfile)
{
    ...
    else
    {
        ...
        for (std::map < cmStdString, RuleHash >::const_iterator rhi = this->RuleHashes.begin(); rhi != this->RuleHashes.end(); ++rhi)
        {
...
void cmGlobalGenerator::WriteSummary(cmTarget * target)
{
    ...
    if (const char *value = target->GetProperty("LABELS"))
    {
        ...
        {
            fout << "# Target labels\n";
            for (std::vector < std::string >::const_iterator li = labels.begin(); li != labels.end(); ++li)
            {
                fout << " " << *li << "\n";
            }
        }
...
//----------------------------------------------------------------------------
void cmGlobalGenerator::AddEvaluationFile(const std::string & inputFile, cmsys::auto_ptr < cmCompiledGeneratorExpression > outputExpr, cmMakefile * makefile, cmsys::auto_ptr < cmCompiledGeneratorExpression > condition, bool inputIsContent)
{
    this->EvaluationFiles.push_back(new cmGeneratorExpressionEvaluationFile(inputFile, outputExpr, makefile, condition, inputIsContent));
}
...
void cmGlobalGenerator::ProcessEvaluationFiles()
{
    ...
    for (std::vector < cmGeneratorExpressionEvaluationFile * >::const_iterator li = this->EvaluationFiles.begin(); li != this->EvaluationFiles.end(); ++li)
    {
        ...
        for (std::vector < std::string >::const_iterator fi = files.begin(); fi != files.end(); ++fi)
        {
            ...
            {
                cmSystemTools::Error("File to be generated by multiple different " "commands: ", fi->c_str());
                return;
            }
        }
    }
...
//----------------------------------------------------------------------------
std::string cmGlobalGenerator::ExpandCFGIntDir(const std::string & str, const std::string & /*config */ ) const
{
    return str;
}
