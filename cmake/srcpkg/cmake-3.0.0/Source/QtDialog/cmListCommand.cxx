...
#include <assert.h>
//----------------------------------------------------------------------------
bool cmListCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 2)
    {
...
bool cmListCommand::GetList(std::vector < std::string > &list, const char *var)
{
    ...
    switch (this->Makefile->GetPolicyStatus(cmPolicies::CMP0007))
    {
        case cmPolicies::WARN:
            {
                ...
                std::string warn = this->Makefile->GetPolicies()->GetPolicyWarning(cmPolicies::CMP0007);
                warn += " List has value = [";
                warn += listString;
                warn += "].";
        ...
        case cmPolicies::REQUIRED_ALWAYS:
            this->Makefile->IssueMessage(cmake::FATAL_ERROR, this->Makefile->GetPolicies()->GetRequiredPolicyError(cmPolicies::CMP0007));
            return false;
    }
    return true;
}
...
bool cmListCommand::HandleInsertCommand(std::vector < std::string > const &args)
{
    ...
    std::vector < std::string > varArgsExpanded;
    if ((!this->GetList(varArgsExpanded, listName.c_str()) || varArgsExpanded.empty()) && item != 0)
    {
        cmOStringStream str;
    ...
    if (varArgsExpanded.size() != 0)
    {
        ...
        {
            cmOStringStream str;
            str << "index: " << item << " out of range (-" << varArgsExpanded.size() << ", " << (varArgsExpanded.size() == 0 ? 0 : (varArgsExpanded.size() - 1)) << ")";
            this->SetError(str.str().c_str());
            return false;
        }
    }
...
//----------------------------------------------------------------------------
bool cmListCommand::HandleRemoveDuplicatesCommand(std::vector < std::string > const &args)
{
    assert(args.size() >= 2);
    if (args.size() > 2)
    {
...
bool cmListCommand::HandleRemoveAtCommand(std::vector < std::string > const &args)
{
    if (args.size() < 3)
    {
        this->SetError("sub-command REMOVE_AT requires at least " "two arguments.");
        return false;
    }
