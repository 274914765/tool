...
public:
    virtual cmGlobalGenerator * CreateGlobalGenerator(const char *name) const
    {
        ...
        if (p[0] == '\0')
        {
            return new cmGlobalVisualStudio8Generator(name, NULL, NULL);
        }
        ...
        if (!strcmp(p, "Win64"))
        {
            return new cmGlobalVisualStudio8Generator(name, "x64", "CMAKE_FORCE_WIN64");
        }
    ...
    virtual void GetGenerators(std::vector < std::string > &names) const
    {
        ...
        parser.ParseVersion("8.0");
        const std::vector < std::string > &availablePlatforms = parser.GetAvailablePlatforms();
        for (std::vector < std::string >::const_iterator i = availablePlatforms.begin(); i != availablePlatforms.end(); ++i)
        {
...
//----------------------------------------------------------------------------
cmGlobalVisualStudio8Generator::cmGlobalVisualStudio8Generator(const char *name, const char *platformName, const char *additionalPlatformDefinition):cmGlobalVisualStudio71Generator(platformName)
{
...
std::string cmGlobalVisualStudio8Generator::FindDevEnvCommand()
{
    ...
    vsxkey += ";InstallDir";
    if (cmSystemTools::ReadRegistryValue(vsxkey.c_str(), vsxcmd, cmSystemTools::KeyWOW64_32))
    {
...
{
    cmLocalVisualStudio7Generator *lg = new cmLocalVisualStudio7Generator(cmLocalVisualStudioGenerator::VS8);
...
//----------------------------------------------------------------------------
void cmGlobalVisualStudio8Generator::GetDocumentation(cmDocumentationEntry & entry)
{
...
std::string cmGlobalVisualStudio8Generator::GetUserMacrosDirectory()
{
    ...
    const char *vc8sp1Registry = "HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\VisualStudio\\8.0\\" "InstalledProducts\\KB926601;";
    const char *vc8exSP1Registry = "HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\VisualStudio\\8.0\\" "InstalledProducts\\KB926748;";
    std::string vc8sp1;
    if (!cmSystemTools::ReadRegistryValue(vc8sp1Registry, vc8sp1) && !cmSystemTools::ReadRegistryValue(vc8exSP1Registry, vc8sp1))
    {
        return "";
    }
    ...
    if (cmSystemTools::ReadRegistryValue("HKEY_CURRENT_USER\\Software\\Microsoft\\VisualStudio\\8.0;" "VisualStudioProjectsLocation", base))
    {
...
bool cmGlobalVisualStudio8Generator::AddCheckTarget()
{
    ...
    cmLocalVisualStudio7Generator *lg = static_cast < cmLocalVisualStudio7Generator * >(generators[0]);
    ...
    cmTarget *tgt = mf->AddUtilityCommand(CMAKE_CHECK_BUILD_SYSTEM_TARGET, false,
    ...
    {
        std::string stampListFile = generators[0]->GetMakefile()->GetCurrentOutputDirectory();
        stampListFile += "/";
        stampListFile += stampList;
        std::string stampFile;
        cmGeneratedFileStream fout(stampListFile.c_str());
        for (std::vector < cmLocalGenerator * >::const_iterator gi = generators.begin(); gi != generators.end(); ++gi)
        {
    ...
    // Add a custom rule to re-run CMake if any input files changed.
    {
        ...
        for (unsigned int j = 0; j < generators.size(); ++j)
        {
            ...
            listFiles.insert(listFiles.end(), lmf->GetListFiles().begin(), lmf->GetListFiles().end());
        }
        // Sort the list of input files and remove duplicates.
        std::sort(listFiles.begin(), listFiles.end(), std::less < std::string > ());
        std::vector < std::string >::iterator new_end = std::unique(listFiles.begin(), listFiles.end());
        ...
        std::string argH = "-H";
        argH += lg->Convert(mf->GetHomeDirectory(), cmLocalGenerator::START_OUTPUT, cmLocalGenerator::UNCHANGED, true);
        commandLine.push_back(argH);
        std::string argB = "-B";
        argB += lg->Convert(mf->GetHomeOutputDirectory(), cmLocalGenerator::START_OUTPUT, cmLocalGenerator::UNCHANGED, true);
        ...
        if (cmSourceFile * file = mf->AddCustomCommandToOutput(stamps, listFiles, no_main_dependency, commandLines, "Checking Build System", no_working_directory, true))
        {
...
//----------------------------------------------------------------------------
void cmGlobalVisualStudio8Generator::WriteSolutionConfigurations(std::ostream & fout)
{
...
//----------------------------------------------------------------------------
void cmGlobalVisualStudio8Generator::WriteProjectConfigurations(std::ostream & fout, const char *name, cmTarget::TargetType type, const std::set < std::string > &configsPartOfDefaultBuild, const char *platformMapping)
{
    std::string guid = this->GetGUID(name);
    for (std::vector < std::string >::iterator i = this->Configurations.begin(); i != this->Configurations.end(); ++i)
    {
        fout << "\t\t{" << guid << "}." << *i << "|" << this->GetPlatformName() << ".ActiveCfg = " << *i << "|" << (platformMapping ? platformMapping : this->GetPlatformName()) << "\n";
        std::set < std::string >::const_iterator ci = configsPartOfDefaultBuild.find(*i);
        if (!(ci == configsPartOfDefaultBuild.end()))
        {
            fout << "\t\t{" << guid << "}." << *i << "|" << this->GetPlatformName() << ".Build.0 = " << *i << "|" << (platformMapping ? platformMapping : this->GetPlatformName()) << "\n";
        }
        bool needsDeploy = (type == cmTarget::EXECUTABLE || type == cmTarget::SHARED_LIBRARY);
        if (this->TargetsWindowsCE() && needsDeploy)
        {
            fout << "\t\t{" << guid << "}." << *i << "|" << this->GetPlatformName() << ".Deploy.0 = " << *i << "|" << (platformMapping ? platformMapping : this->GetPlatformName()) << "\n";
        }
    }
}
...
//----------------------------------------------------------------------------
void cmGlobalVisualStudio8Generator::WriteProjectDepends(std::ostream & fout, const char *, const char *, cmTarget const &t)
{
...
bool cmGlobalVisualStudio8Generator::NeedLinkLibraryDependencies(cmTarget & target)
{
    ...
    for (std::set < cmStdString >::const_iterator ui = target.GetUtilities().begin(); ui != target.GetUtilities().end(); ++ui)
    {
        if (cmTarget * depTarget = this->FindTarget(0, ui->c_str()))
        {
            if (depTarget->GetType() != cmTarget::INTERFACE_LIBRARY && depTarget->GetProperty("EXTERNAL_MSPROJECT"))
            {
