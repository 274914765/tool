...
//----------------------------------------------------------------------------
bool cmFindPackageCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 1)
    {
    ...
    if (const char *rv = this->Makefile->GetDefinition("CMAKE_MINIMUM_REQUIRED_VERSION"))
    {
    ...
    if (const char *arch = this->Makefile->GetDefinition("CMAKE_LIBRARY_ARCHITECTURE"))
    {
        this->LibraryArchitecture = arch;
    }

    // Lookup whether lib64 paths should be used.
    if (this->Makefile->PlatformIs64Bit() && this->Makefile->GetCMakeInstance()->GetPropertyAsBool("FIND_LIBRARY_USE_LIB64_PATHS"))
    {
    ...
    // Parse the arguments.
    enum Doing
    { DoingNone, DoingComponents, DoingOptionalComponents, DoingNames,
    ...
    for (unsigned int i = 1; i < args.size(); ++i)
    {
        ...
        {
            if (args[i].find_first_of(":/\\") != args[i].npos || cmSystemTools::GetFilenameLastExtension(args[i]) != ".cmake")
            {
                cmOStringStream e;
                e << "given CONFIGS option followed by invalid file name \"" << args[i] << "\".  The names given must be file names without " << "a path and with a \".cmake\" extension.";
    ...
    std::set_intersection(requiredComponents.begin(), requiredComponents.end(), optionalComponents.begin(), optionalComponents.end(), std::back_inserter(doubledComponents));
    ...
    if (!this->UseFindModules && !this->UseConfigFiles)
    {
        ...
        for (std::set < unsigned int >::const_iterator si = moduleArgs.begin(); si != moduleArgs.end(); ++si)
        {
            e << "  " << args[*si] << "\n";
        }
        e << "and options exclusive to Config mode:\n";
        for (std::set < unsigned int >::const_iterator si = configArgs.begin(); si != configArgs.end(); ++si)
        {
            e << "  " << args[*si] << "\n";
        }
    ...
    {
        this->VersionExact = false;
        this->Makefile->IssueMessage(cmake::AUTHOR_WARNING, "Ignoring EXACT since no version is requested.");
    }
    ...
    if (!this->Version.empty())
    {
        ...
        this->VersionCount = sscanf(this->Version.c_str(), "%u.%u.%u.%u", &parsed_major, &parsed_minor, &parsed_patch, &parsed_tweak);
    ...
    if (this->Makefile->IsOn(disableFindPackageVar.c_str()))
    {
        ...
        {
            cmOStringStream e;
            e << "for module " << this->Name << " called with REQUIRED, but " << disableFindPackageVar << " is enabled. A REQUIRED package cannot be disabled.";
            this->SetError(e.str().c_str());
            return false;
        }
    ...
    if (this->UseFindModules && this->UseConfigFiles && this->Makefile->IsOn("CMAKE_FIND_PACKAGE_WARN_NO_MODULE"))
    {
        cmOStringStream aw;
        if (this->RequiredCMakeVersion >= CMake_VERSION_ENCODE(2, 8, 8))
        {
            aw << "find_package called without either MODULE or CONFIG option and "
                ...
                "Add CONFIG to exclusively request Config mode and search for a " "package configuration file provided by " << this->Name << " (" << this->Name << "Config.cmake or " << cmSystemTools::LowerCase(this->Name) << "-config.cmake).  ";
        }
        else
        {
            aw << "find_package called without NO_MODULE option and no "
                ...
                " (" << this->Name << "Config.cmake or " << cmSystemTools::LowerCase(this->Name) << "-config.cmake).  " "Otherwise make Find" << this->Name << ".cmake available in " "CMAKE_MODULE_PATH.";
        }
    ...
    {
        for (std::vector < std::string >::const_iterator ni = this->Names.begin(); ni != this->Names.end(); ++ni)
        {
...
{
    for (std::map < cmStdString, OriginalDef >::iterator i = this->OriginalDefs.begin(); i != this->OriginalDefs.end(); ++i)
    {
...
bool cmFindPackageCommand::HandlePackageMode()
{
    ...
    if (this->UseConfigFiles)
    {
        ...
        {
            this->Makefile->IssueMessage(cmake::INTERNAL_ERROR, "fileFound is true but FileFound is empty!");
            fileFound = false;
        }
    }
    ...
    if (fileFound)
    {
        if ((this->Makefile->IsDefinitionSet(foundVar.c_str())) && (this->Makefile->IsOn(foundVar.c_str()) == false))
        {
        ...
        if (this->ReadListFile(this->FileFound.c_str(), DoPolicyScope))
        {
            ...
            if ((this->Makefile->IsDefinitionSet(foundVar.c_str())) && (this->Makefile->IsOn(foundVar.c_str()) == false))
            {
                ...
                notFoundMessage = this->Makefile->GetSafeDefinition(notFoundMessageVar.c_str());
            }
        }
        else
        {
    ...
    if (result && !found && (!this->Quiet || this->Required))
    {
        ...
        {
            e << "Found package configuration file:\n" "  " << this->FileFound << "\n" "but it set " << foundVar << " to FALSE so package \"" << this->Name << "\" is considered to be NOT FOUND.";
        ...
        else if (this->ConsideredConfigs.size() > 0)
        {
            e << "Could not find a configuration file for package \""
                << this->Name << "\" that " << (this->VersionExact ? "exactly matches" : "is compatible with") << " requested version \"" << this->Version << "\".\n" << "The following configuration files were considered but not " "accepted:\n";
            for (std::vector < ConfigFileInfo >::size_type i = 0; i < this->ConsideredConfigs.size(); i++)
            {
        ...
        else
        {
            ...
            if (this->UseConfigFiles)
            {
                ...
                {
                    e << "Could not find a package configuration file named \"" << this->Configs[0] << "\" provided by package \"" << this->Name << "\"" << requestedVersionString << ".\n";
                }
                else
                {
                    e << "Could not find a package configuration file provided by \"" << this->Name << "\"" << requestedVersionString << " with any of the following names:\n";
                ...
                e << "Add the installation prefix of \"" << this->Name << "\" to "
                    "CMAKE_PREFIX_PATH or set \"" << this->Variable << "\" to a " "directory containing one of the above files. " "If \"" << this->Name << "\" provides a separate development " "package or SDK, be sure it has been installed.";
            }
            else                // if(!this->UseFindModules && !this->UseConfigFiles)
            {
                e << "No \"Find" << this->Name << ".cmake\" found in " << "CMAKE_MODULE_PATH.";
                ...
                aw << "Find" << this->Name << ".cmake must either be part of this "
                    ...
                    "provided by that package.  This error in general means that " "the buildsystem of this project is relying on a Find-module " "without ensuring that it is actually available.\n";
            }
        }

        this->Makefile->IssueMessage(this->Required ? cmake::FATAL_ERROR : cmake::WARNING, e.str());
        if (this->Required)
        {
    ...
    this->Makefile->AddDefinition(consideredConfigsVar.c_str(), consideredConfigFiles.c_str());

    this->Makefile->AddDefinition(consideredVersionsVar.c_str(), consideredVersions.c_str());
...
bool cmFindPackageCommand::FindConfig()
{
    ...
    std::string help = "The directory containing a CMake configuration file for ";
    help += this->Name;
    help += ".";
    // We force the value since we do not get here if it was already set.
    this->Makefile->AddCacheDefinition(this->Variable.c_str(), init.c_str(), help.c_str(), cmCacheManager::PATH, true);
    return found;
}
...
bool cmFindPackageCommand::FindPrefixedConfig()
{
    ...
    for (std::vector < std::string >::const_iterator pi = prefixes.begin(); pi != prefixes.end(); ++pi)
    {
...
{
    if (this->Makefile->ReadListFile(this->Makefile->GetCurrentListFile(), f, 0, !this->PolicyScope || psr == NoPolicyScope))
    {
        return true;
    }
...
void cmFindPackageCommand::AppendToFoundProperty(bool found)
{
    ...
    const char *foundProp = this->Makefile->GetCMakeInstance()->GetProperty("PACKAGES_FOUND");
    if (foundProp && *foundProp)
    {
        std::string tmp = foundProp;

        cmSystemTools::ExpandListArgument(tmp, foundContents, false);
        std::vector < std::string >::iterator nameIt = std::find(foundContents.begin(), foundContents.end(), this->Name);
    ...
    const char *notFoundProp = this->Makefile->GetCMakeInstance()->GetProperty("PACKAGES_NOT_FOUND");
    if (notFoundProp && *notFoundProp)
    {
        std::string tmp = notFoundProp;

        cmSystemTools::ExpandListArgument(tmp, notFoundContents, false);
        std::vector < std::string >::iterator nameIt = std::find(notFoundContents.begin(), notFoundContents.end(), this->Name);
    ...
    this->Makefile->GetCMakeInstance()->SetProperty("PACKAGES_FOUND", tmp.c_str());
    ...
    this->Makefile->GetCMakeInstance()->SetProperty("PACKAGES_NOT_FOUND", tmp.c_str());
}
...
{
    {
        ...
        this->Makefile->GetCMakeInstance()->SetProperty(transitivePropName.c_str(), "False");
    }
    ...
    bool packageFound = ((cmSystemTools::IsOn(result)) || (cmSystemTools::IsOn(upperResult)));
    ...
    this->Makefile->GetCMakeInstance()->SetProperty(quietInfoPropName.c_str(), this->Quiet ? "TRUE" : "FALSE");
    ...
    this->Makefile->GetCMakeInstance()->SetProperty(versionInfoPropName.c_str(), versionInfo.c_str());
    if (this->Required)
    {
        std::string requiredInfoPropName = "_CMAKE_";
        requiredInfoPropName += this->Name;
        requiredInfoPropName += "_TYPE";
        this->Makefile->GetCMakeInstance()->SetProperty(requiredInfoPropName.c_str(), "REQUIRED");
    }
...
void cmFindPackageCommand::AddPrefixesSystemEnvironment()
{
    if (!this->NoSystemEnvironmentPath && !this->NoDefaultPath)
    {
        ...
        for (std::vector < std::string >::iterator i = tmp.begin(); i != tmp.end(); ++i)
        {
            ...
            if ((cmHasLiteralSuffix(d, "/bin")) || (cmHasLiteralSuffix(d, "/sbin")))
            {
...
void cmFindPackageCommand::LoadPackageRegistryWin(bool user, unsigned int view)
{
    ...
    HKEY hKey;
    if (RegOpenKeyExW(user ? HKEY_CURRENT_USER : HKEY_LOCAL_MACHINE, key.c_str(), 0, KEY_QUERY_VALUE | view, &hKey) == ERROR_SUCCESS)
    {
        ...
        while (!done)
        {
            ...
            switch (RegEnumValueW(hKey, index, name, &nameSize, 0, &valueType, (BYTE *) & data[0], &dataSize))
            {
                case ERROR_SUCCESS:
                    ++index;
                    if (valueType == REG_SZ)
    ...
    if (user && !bad.empty() && RegOpenKeyExW(HKEY_CURRENT_USER, key.c_str(), 0, KEY_SET_VALUE | view, &hKey) == ERROR_SUCCESS)
    {
        for (std::set < std::wstring >::const_iterator vi = bad.begin(); vi != bad.end(); ++vi)
        {
...
bool cmFindPackageCommand::CheckPackageRegistryEntry(std::istream & is)
{
    ...
    std::string fname;
    if (cmSystemTools::GetLineFromStream(is, fname) && cmSystemTools::FileIsFullPath(fname.c_str()))
    {
...
void cmFindPackageCommand::AddPrefixesBuilds()
{
    if (!this->NoBuilds && !this->NoDefaultPath)
    {
        ...
        {
            cmOStringStream r;
            r << "[HKEY_CURRENT_USER\\Software\\Kitware\\CMakeSetup\\" "Settings\\StartPath;WhereBuild" << i << "]";
            std::string f = r.str();
            cmSystemTools::ExpandRegistryValues(f);
            cmSystemTools::ConvertToUnixSlashes(f);
            if (cmSystemTools::FileIsFullPath(f.c_str()) && cmSystemTools::FileIsDirectory(f.c_str()))
            {
...
bool cmFindPackageCommand::SearchDirectory(std::string const &dir)
{
    ...
    for (std::vector < std::string >::const_iterator si = this->SearchPathSuffixes.begin(); si != this->SearchPathSuffixes.end(); ++si)
    {
        std::string d = dir;
        if (!si->empty())
        {
...
//----------------------------------------------------------------------------
bool cmFindPackageCommand::FindConfigFile(std::string const &dir, std::string & file)
{
    ...
    for (std::vector < std::string >::const_iterator ci = this->Configs.begin(); ci != this->Configs.end(); ++ci)
    {
        ...
        if (cmSystemTools::FileExists(file.c_str(), true) && this->CheckVersion(file))
        {
            return true;
        }
    }
...
bool cmFindPackageCommand::CheckVersion(std::string const &config_file)
{
    ...
    if ((haveResult == false) && (cmSystemTools::FileExists(version_file.c_str(), true)))
    {
...
//----------------------------------------------------------------------------
bool cmFindPackageCommand::CheckVersionFile(std::string const &version_file, std::string & result_version)
{
    ...
    if (this->ReadListFile(version_file.c_str(), NoPolicyScope))
    {
        ...
        if (suitable)
        {
            ...
            this->VersionFound = this->Makefile->GetSafeDefinition("PACKAGE_VERSION");
            ...
            unsigned int parsed_patch;
            unsigned int parsed_tweak;
            this->VersionFoundCount = sscanf(this->VersionFound.c_str(), "%u.%u.%u.%u", &parsed_major, &parsed_minor, &parsed_patch, &parsed_tweak);
...
cmFileListGeneratorBase *cmFileListGeneratorBase::SetNext(cmFileListGeneratorBase const &next)
{
    this->Next = next.Clone();
    return this->Next.get();
}

bool cmFileListGeneratorBase::Consider(std::string const &fullPath, cmFileList & listing)
{
    if (this->Next.get())
    {
...
{
public:
    cmFileListGeneratorFixed(std::string const &str):cmFileListGeneratorBase(), String(str)
    {
    }
    cmFileListGeneratorFixed(cmFileListGeneratorFixed const &r):cmFileListGeneratorBase(), String(r.String)
    {
    }
private:
    std::string String;
    ...
    {
        cmsys::auto_ptr < cmFileListGeneratorBase > g(new cmFileListGeneratorFixed(*this));
        return g;
    }
};
...
{
public:
    cmFileListGeneratorEnumerate(std::vector < std::string > const &v):cmFileListGeneratorBase(), Vector(v)
    {
    }
    cmFileListGeneratorEnumerate(cmFileListGeneratorEnumerate const &r):cmFileListGeneratorBase(), Vector(r.Vector)
    {
    }
private:
    ...
    virtual bool Search(std::string const &parent, cmFileList & lister)
    {
        for (std::vector < std::string >::const_iterator i = this->Vector.begin(); i != this->Vector.end(); ++i)
        {
    ...
    virtual cmsys::auto_ptr < cmFileListGeneratorBase > Clone() const
    {
        cmsys::auto_ptr < cmFileListGeneratorBase > g(new cmFileListGeneratorEnumerate(*this));
        return g;
    }
};
...
{
public:
    cmFileListGeneratorProject(std::vector < std::string > const &names):cmFileListGeneratorBase(), Names(names)
    {
    }
    cmFileListGeneratorProject(cmFileListGeneratorProject const &r):cmFileListGeneratorBase(), Names(r.Names)
    {
    }
private:
    ...
    virtual cmsys::auto_ptr < cmFileListGeneratorBase > Clone() const
    {
        cmsys::auto_ptr < cmFileListGeneratorBase > g(new cmFileListGeneratorProject(*this));
        return g;
    }
};
...
{
public:
    cmFileListGeneratorMacProject(std::vector < std::string > const &names, const char *ext):cmFileListGeneratorBase(), Names(names), Extension(ext)
    {
    }
    cmFileListGeneratorMacProject(cmFileListGeneratorMacProject const &r):cmFileListGeneratorBase(), Names(r.Names), Extension(r.Extension)
    {
    }
private:
    std::vector < std::string > const &Names;
    ...
    virtual cmsys::auto_ptr < cmFileListGeneratorBase > Clone() const
    {
        cmsys::auto_ptr < cmFileListGeneratorBase > g(new cmFileListGeneratorMacProject(*this));
        return g;
    }
};
...
{
public:
    cmFileListGeneratorCaseInsensitive(std::string const &str):cmFileListGeneratorBase(), String(str)
    {
    }
    cmFileListGeneratorCaseInsensitive(cmFileListGeneratorCaseInsensitive const &r):cmFileListGeneratorBase(), String(r.String)
    {
    }
private:
    std::string String;
    ...
    virtual cmsys::auto_ptr < cmFileListGeneratorBase > Clone() const
    {
        cmsys::auto_ptr < cmFileListGeneratorBase > g(new cmFileListGeneratorCaseInsensitive(*this));
        return g;
    }
};
...
{
public:
    cmFileListGeneratorGlob(std::string const &str):cmFileListGeneratorBase(), Pattern(str)
    {
    }
    cmFileListGeneratorGlob(cmFileListGeneratorGlob const &r):cmFileListGeneratorBase(), Pattern(r.Pattern)
    {
    }
private:
    std::string Pattern;
...
bool cmFindPackageCommand::SearchPrefix(std::string const &prefix_in)
{
    ...
    //  PREFIX/(cmake|CMake)/ (useful on windows or in build trees)
    {
        ...
        lister / cmFileListGeneratorFixed(prefix) / cmFileListGeneratorCaseInsensitive("cmake");
        if (lister.Search())
        {
            return true;
        }
    ...
    {
        cmFindPackageFileList lister(this);
        lister / cmFileListGeneratorFixed(prefix) / cmFileListGeneratorProject(this->Names);
        if (lister.Search())
        {
            return true;
        }
    ...
    {
        cmFindPackageFileList lister(this);
        lister / cmFileListGeneratorFixed(prefix) / cmFileListGeneratorProject(this->Names) / cmFileListGeneratorCaseInsensitive("cmake");
        if (lister.Search())
        {
            return true;
        }
    ...
    {
        cmFindPackageFileList lister(this);
        lister / cmFileListGeneratorFixed(prefix) / cmFileListGeneratorEnumerate(common) / cmFileListGeneratorFixed("cmake") / cmFileListGeneratorProject(this->Names);
        if (lister.Search())
        {
            return true;
        }
    ...
    {
        cmFindPackageFileList lister(this);
        lister / cmFileListGeneratorFixed(prefix) / cmFileListGeneratorEnumerate(common) / cmFileListGeneratorProject(this->Names);
        if (lister.Search())
        {
            return true;
        }
    ...
    {
        cmFindPackageFileList lister(this);
        lister / cmFileListGeneratorFixed(prefix) / cmFileListGeneratorEnumerate(common) / cmFileListGeneratorProject(this->Names) / cmFileListGeneratorCaseInsensitive("cmake");
        if (lister.Search())
        {
            return true;
        }
...
bool cmFindPackageCommand::SearchFrameworkPrefix(std::string const &prefix_in)
{
    ...
    {
        cmFindPackageFileList lister(this);
        lister / cmFileListGeneratorFixed(prefix) / cmFileListGeneratorMacProject(this->Names, ".framework") / cmFileListGeneratorFixed("Resources");
        if (lister.Search())
        {
            return true;
        }
    ...
    {
        cmFindPackageFileList lister(this);
        lister / cmFileListGeneratorFixed(prefix) / cmFileListGeneratorMacProject(this->Names, ".framework") / cmFileListGeneratorFixed("Resources") / cmFileListGeneratorCaseInsensitive("cmake");
        if (lister.Search())
        {
            return true;
        }
    ...
    {
        cmFindPackageFileList lister(this);
        lister / cmFileListGeneratorFixed(prefix) / cmFileListGeneratorMacProject(this->Names, ".framework") / cmFileListGeneratorFixed("Versions") / cmFileListGeneratorGlob("*/Resources");
        if (lister.Search())
        {
            return true;
        }
    ...
    {
        cmFindPackageFileList lister(this);
        lister / cmFileListGeneratorFixed(prefix) / cmFileListGeneratorMacProject(this->Names, ".framework") / cmFileListGeneratorFixed("Versions") / cmFileListGeneratorGlob("*/Resources") / cmFileListGeneratorCaseInsensitive("cmake");
        if (lister.Search())
        {
            return true;
        }
...
bool cmFindPackageCommand::SearchAppBundlePrefix(std::string const &prefix_in)
{
    ...
    {
        cmFindPackageFileList lister(this);
        lister / cmFileListGeneratorFixed(prefix) / cmFileListGeneratorMacProject(this->Names, ".app") / cmFileListGeneratorFixed("Contents/Resources");
        if (lister.Search())
        {
            return true;
        }
    ...
    {
        cmFindPackageFileList lister(this);
        lister / cmFileListGeneratorFixed(prefix) / cmFileListGeneratorMacProject(this->Names, ".app") / cmFileListGeneratorFixed("Contents/Resources") / cmFileListGeneratorCaseInsensitive("cmake");
        if (lister.Search())
        {
            return true;
        }
