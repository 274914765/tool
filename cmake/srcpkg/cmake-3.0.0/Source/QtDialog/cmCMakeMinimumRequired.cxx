...
bool cmCMakeMinimumRequired::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    // Process arguments.
    ...
    this->Makefile->AddDefinition("CMAKE_MINIMUM_REQUIRED_VERSION", version_string.c_str());
    ...
    if (sscanf(version_string.c_str(), "%u.%u.%u.%u", &required_major, &required_minor, &required_patch, &required_tweak) < 2)
    {
        cmOStringStream e;
    ...
    if ((current_major < required_major) ||
        ...
        (current_major == required_major &&
         current_minor == required_minor && current_patch < required_patch) || (current_major == required_major && current_minor == required_minor && current_patch == required_patch && current_tweak < required_tweak))
    {
        // The current version is too low.
        cmOStringStream e;
        e << "CMake " << version_string.c_str() << " or higher is required.  You are running version " << cmVersion::GetCMakeVersion();
    ...
    {
        this->Makefile->IssueMessage(cmake::AUTHOR_WARNING, "Compatibility with CMake < 2.4 is not supported by CMake >= 3.0.");
