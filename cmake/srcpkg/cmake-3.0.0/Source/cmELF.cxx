...
public:
    ...
    // Construct and take ownership of the file stream object.
      cmELFInternal(cmELF * external, cmsys::auto_ptr < cmsys::ifstream > &fin, ByteOrderType order):External(external), Stream(*fin.release()), ByteOrder(order), ELFType(cmELF::FileTypeInvalid)
    {
    ...
    cmELF::FileType GetFileType()const
    {
        return this->ELFType;
    }
protected:
...
public:
    ...
    cmELFInternalImpl(cmELF * external, cmsys::auto_ptr < cmsys::ifstream > &fin, ByteOrderType order);
...
private:
    ...
    bool FileTypeValid(ELF_Half et)
    {
        ...
        if (eti == ET_NONE || eti == ET_REL || eti == ET_EXEC || eti == ET_DYN || eti == ET_CORE)
        {
            return true;
        }
    ...
    {
        if (this->Stream.read(reinterpret_cast < char *>(&x), sizeof(x)) && this->NeedSwap)
        {
            ByteSwap(x);
        }
        return this->Stream ? true : false;
    ...
    bool LoadSectionHeader(ELF_Half i)
    {
        ...
        this->Stream.seekg(this->ELFHeader.e_shoff + this->ELFHeader.e_shentsize * i);
...
//----------------------------------------------------------------------------
template < class Types > cmELFInternalImpl < Types >::cmELFInternalImpl(cmELF * external, cmsys::auto_ptr < cmsys::ifstream > &fin, ByteOrderType order):
...
//----------------------------------------------------------------------------
template < class Types > unsigned int cmELFInternalImpl < Types >::GetDynamicEntryCount()
{
...
//----------------------------------------------------------------------------
template < class Types > unsigned long cmELFInternalImpl < Types >::GetDynamicEntryPosition(int j)
{
    if (!this->LoadDynamicSection())
    {
        return 0;
...
//----------------------------------------------------------------------------
template < class Types > cmELF::StringEntry const *cmELFInternalImpl < Types >::GetDynamicSectionString(int tag)
{
    // Short-circuit if already checked.
    std::map < int, StringEntry >::iterator dssi = this->DynamicSectionStrings.find(tag);
    ...
    for (typename std::vector < ELF_Dyn >::iterator di = this->DynamicSectionEntries.begin(); di != this->DynamicSectionEntries.end(); ++di)
    {
        ELF_Dyn & dyn = *di;
        if (dyn.d_tag == tag)
        {
            ...
            {
                this->SetErrorMessage("Section DYNAMIC references string beyond " "the end of its string section.");
                return 0;
            }
...
{
    if (this->Valid() && this->Internal->GetFileType() == cmELF::FileTypeSharedLibrary)
    {
...
{
    if (this->Valid() && (this->Internal->GetFileType() == cmELF::FileTypeExecutable || this->Internal->GetFileType() == cmELF::FileTypeSharedLibrary))
    {
