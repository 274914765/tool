...
cmVariableWatch::~cmVariableWatch()
{
    ...
    for (svp_it = this->WatchMap.begin(); svp_it != this->WatchMap.end(); ++svp_it)
    {
        cmVariableWatch::VectorOfPairs::iterator p_it;

        for (p_it = svp_it->second.begin(); p_it != svp_it->second.end(); ++p_it)
        {
            delete *p_it;
        }
    }
...
bool cmVariableWatch::AddWatch(const std::string & variable, WatchMethod method, void *client_data /*=0*/ ,
                               DeleteData delete_data /*=0*/ )
{
    ...
    for (cc = 0; cc < vp->size(); cc++)
    {
        ...
        if (pair->Method == method && client_data && client_data == pair->ClientData)
        {
...
void cmVariableWatch::RemoveWatch(const std::string & variable, WatchMethod method, void *client_data /*=0*/ )
{
...
void cmVariableWatch::VariableAccessed(const std::string & variable, int access_type, const char *newValue, const cmMakefile * mf) const
{
    cmVariableWatch::StringToVectorOfPairs::const_iterator mit = this->WatchMap.find(variable);
