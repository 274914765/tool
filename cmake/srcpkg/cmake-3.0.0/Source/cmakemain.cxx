...
static void cmakemainErrorCallback(const char *m, const char *, bool &, void *clientdata)
{
...
int do_cmake(int ac, char const *const *av)
{
    ...
    if (list_cached || list_all_cached)
    {
        ...
        for (it.Begin(); !it.IsAtEnd(); it.Next())
        {
            ...
            if (t != cmCacheManager::INTERNAL && t != cmCacheManager::STATIC && t != cmCacheManager::UNINITIALIZED)
            {
                ...
                if (list_all_cached || !advanced)
                {
                    ...
                    std::cout << it.GetName() << ":" << cmCacheManager::TypeToString(it.GetType()) << "=" << it.GetValue() << std::endl;
                    if (list_help)
                    {
                        std::cout << std::endl;
                    }
...
static int do_build(int ac, char const *const *av)
{
    ...
    enum Doing
    { DoingNone, DoingDir, DoingTarget, DoingConfig, DoingNative };
    ...
    if (dir.empty())
    {
        std::cerr << "Usage: cmake --build <dir> [options] [-- [native-options]]\n" "Options:\n" CMAKE_BUILD_OPTIONS;
        return 1;
    }
