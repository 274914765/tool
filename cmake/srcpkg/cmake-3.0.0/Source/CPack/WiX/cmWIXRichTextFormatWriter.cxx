...
cmWIXRichTextFormatWriter::cmWIXRichTextFormatWriter(const std::string & filename):File(filename.c_str(), std::ios::binary)
{
    StartGroup();
    WriteHeader();
    WriteDocumentPrefix();
            ...
            default:
                {
                    ...
                    else
                    {
                        ...
                        {
                            EmitUnicodeCodepoint((text[i + 2] & 0x3F) | ((text[i + 1] & 0x3F) << 6) | ((c & 0xF) << 12));
                            i += 2;
                        }
                        else if (c < 0xF8 && i + 3 < text.size())
                        {
                            EmitUnicodeCodepoint((text[i + 3] & 0x3F) | ((text[i + 2] & 0x3F) << 6) | ((text[i + 1] & 0x3F) << 12) | ((c & 0x7) << 18));
                            i += 3;
                        }
                        else
                        {
