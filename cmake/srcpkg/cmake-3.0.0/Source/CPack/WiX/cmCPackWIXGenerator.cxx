...
bool cmCPackWIXGenerator::RunWiXCommand(const std::string & command)
{
    ...
    cmCPackLogger(cmCPackLog::LOG_DEBUG, "Running WiX command: " << command << std::endl);
    ...
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "Problem running WiX candle. " "Please check '" << logFileName << "' for errors." << std::endl);
...
bool cmCPackWIXGenerator::RunCandleCommand(const std::string & sourceFile, const std::string & objectFile)
{
    ...
    for (extension_set_t::const_iterator i = CandleExtensions.begin(); i != CandleExtensions.end(); ++i)
    {
...
bool cmCPackWIXGenerator::RunLightCommand(const std::string & objectFiles)
{
    ...
    for (extension_set_t::const_iterator i = LightExtensions.begin(); i != LightExtensions.end(); ++i)
    {
        command << " -ext " << QuotePath(*i);
    }
...
int cmCPackWIXGenerator::PackageFiles()
{
    ...
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "Fatal WiX Generator Error" << std::endl);
        return false;
    }
...
bool cmCPackWIXGenerator::InitializeWiXConfiguration()
{
    ...
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "Error while executing CPackWIX.cmake" << std::endl);
        return false;
    }
    ...
    if (GetOption("CPACK_WIX_PRODUCT_GUID") == 0)
    {
        ...
        cmCPackLogger(cmCPackLog::LOG_VERBOSE, "CPACK_WIX_PRODUCT_GUID implicitly set to " << guid << " . " << std::endl);
    }
    ...
    if (GetOption("CPACK_WIX_UPGRADE_GUID") == 0)
    {
        ...
        cmCPackLogger(cmCPackLog::LOG_WARNING, "CPACK_WIX_UPGRADE_GUID implicitly set to " << guid << " . " "Please refer to the documentation on how and why " "you might want to set this explicitly." << std::endl);
    }
    ...
    if (GetOption("CPACK_PACKAGE_VENDOR") == 0)
    {
        ...
        cmCPackLogger(cmCPackLog::LOG_VERBOSE, "CPACK_PACKAGE_VENDOR implicitly set to " << defaultVendor << " . " << std::endl);
    }
...
bool cmCPackWIXGenerator::PackageFilesImpl()
{
    ...
    for (size_t i = 0; i < WixSources.size(); ++i)
    {
        ...
        std::string objectFilename = cmSystemTools::GetFilenameWithoutExtension(sourceFilename) + ".wixobj";
...
void cmCPackWIXGenerator::AppendUserSuppliedExtraObjects(std::ostream & stream)
{
    ...
    cmSystemTools::ExpandListArgument(cpackWixExtraObjects, expandedExtraObjects);
...
bool cmCPackWIXGenerator::CreateWiXVariablesIncludeFile()
{
    ...
    std::string includeFilename = cpackTopLevel + "/cpack_variables.wxi";
    ...
    SetOptionIfNotSet("CPACK_WIX_PROGRAM_MENU_FOLDER", GetOption("CPACK_PACKAGE_NAME"));
...
void cmCPackWIXGenerator::CopyDefinition(cmWIXSourceWriter & source, const std::string & name)
{
...
void cmCPackWIXGenerator::AddDefinition(cmWIXSourceWriter & source, const std::string & name, const std::string & value)
{
    std::stringstream tmp;
    tmp << name << "=\"" << value << '"';

    source.AddProcessingInstruction("define", cmWIXSourceWriter::WindowsCodepageToUtf8(tmp.str()));
}
...
bool cmCPackWIXGenerator::CreateWiXSourceFiles()
{
    ...
    std::string directoryDefinitionsFilename = cpackTopLevel + "/directories.wxs";
    ...
    std::string fileDefinitionsFilename = cpackTopLevel + "/files.wxs";
    ...
    std::string featureDefinitionsFilename = cpackTopLevel + "/features.wxs";
    ...
    if (Components.empty())
    {
        AddComponentsToFeature(toplevel, "ProductFeature", directoryDefinitions, fileDefinitions, featureDefinitions, globalShortcuts);
    ...
    {
        for (std::map < std::string, cmCPackComponent >::const_iterator i = Components.begin(); i != Components.end(); ++i)
        {
            ...
            AddComponentsToFeature(componentPath, componentFeatureId, directoryDefinitions, fileDefinitions, featureDefinitions, featureShortcuts);
            ...
            if (featureShortcuts.size())
            {
                if (!CreateStartMenuShortcuts(component.Name, componentFeatureId, featureShortcuts, fileDefinitions, featureDefinitions))
                {
                    return false;
                }
            }
    ...
    if (hasShortcuts)
    {
        if (!CreateStartMenuShortcuts(std::string(), "ProductFeature", globalShortcuts, fileDefinitions, featureDefinitions))
        {
            return false;
        }
    }
    ...
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "Could not find CPack WiX template file WIX.template.in" << std::endl);
        return false;
    }
    ...
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "Failed creating '" << mainSourceFilePath << "'' from template." << std::endl);
    ...
    for (cmWIXPatchParser::fragment_map_t::const_iterator i = Fragments.begin(); i != Fragments.end(); ++i)
    {
    ...
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "Some XML patch fragments did not have matching IDs: " << fragmentList << std::endl);
        return false;
    }
...
bool cmCPackWIXGenerator::CreateCMakePackageRegistryEntry(cmWIXSourceWriter & featureDefinitions)
{
    ...
    std::string registryKey = std::string("Software\\Kitware\\CMake\\Packages\\") + package;
...
bool cmCPackWIXGenerator::CreateFeatureHierarchy(cmWIXSourceWriter & featureDefinitions)
{
    for (std::map < std::string, cmCPackComponentGroup >::const_iterator i = ComponentGroups.begin(); i != ComponentGroups.end(); ++i)
    {
...
bool cmCPackWIXGenerator::EmitFeatureForComponentGroup(cmWIXSourceWriter & featureDefinitions, cmCPackComponentGroup const &group)
{
    featureDefinitions.BeginElement("Feature");
    ...
    featureDefinitions.AddAttributeUnlessEmpty("Title", group.DisplayName);

    featureDefinitions.AddAttributeUnlessEmpty("Description", group.Description);

    for (std::vector < cmCPackComponentGroup * >::const_iterator i = group.Subgroups.begin(); i != group.Subgroups.end(); ++i)
    {
    ...
    for (std::vector < cmCPackComponent * >::const_iterator i = group.Components.begin(); i != group.Components.end(); ++i)
    {
...
bool cmCPackWIXGenerator::EmitFeatureForComponent(cmWIXSourceWriter & featureDefinitions, cmCPackComponent const &component)
{
    featureDefinitions.BeginElement("Feature");
    featureDefinitions.AddAttribute("Id", "CM_C_" + component.Name);

    featureDefinitions.AddAttributeUnlessEmpty("Title", component.DisplayName);

    featureDefinitions.AddAttributeUnlessEmpty("Description", component.Description);
...
bool cmCPackWIXGenerator::AddComponentsToFeature(std::string const &rootPath,
                                                 std::string const &featureId, cmWIXSourceWriter & directoryDefinitions, cmWIXSourceWriter & fileDefinitions, cmWIXSourceWriter & featureDefinitions, shortcut_map_t & shortcutMap)
{
    ...
    {
        cmSystemTools::ExpandListArgument(cpackPackageExecutables, cpackPackageExecutablesList);
        if (cpackPackageExecutablesList.size() % 2 != 0)
        {
            cmCPackLogger(cmCPackLog::LOG_ERROR, "CPACK_PACKAGE_EXECUTABLES should contain pairs of <executable> and " "<text label>." << std::endl);
            return false;
        }
    }
    ...
    {
        cmSystemTools::ExpandListArgument(cpackPackageDesktopLinks, cpackPackageDesktopLinksList);
    }

    AddDirectoryAndFileDefinitons(rootPath, "INSTALL_ROOT", directoryDefinitions, fileDefinitions, featureDefinitions, cpackPackageExecutablesList, cpackPackageDesktopLinksList, shortcutMap);
...
bool cmCPackWIXGenerator::CreateStartMenuShortcuts(std::string const &cpackComponentName, std::string const &featureId, shortcut_map_t & shortcutMap, cmWIXSourceWriter & fileDefinitions, cmWIXSourceWriter & featureDefinitions)
{
    ...
    for (shortcut_map_t::const_iterator i = shortcutMap.begin(); i != shortcutMap.end(); ++i)
    {
        ...
        fileDefinitions.AddAttribute("WorkingDirectory", shortcut.workingDirectoryId);
    ...
    fileDefinitions.AddAttribute("Id", "CM_REMOVE_PROGRAM_MENU_FOLDER" + idSuffix);
    fileDefinitions.AddAttribute("On", "uninstall");
    fileDefinitions.EndElement("RemoveFolder");

    std::string registryKey = std::string("Software\\") + cpackVendor + "\\" + cpackPackageName;
...
void cmCPackWIXGenerator::CreateUninstallShortcut(std::string const &packageName, cmWIXSourceWriter & fileDefinitions)
{
    fileDefinitions.BeginElement("Shortcut");
...
bool cmCPackWIXGenerator::CreateLicenseFile()
{
    ...
    if (extension == ".rtf")
    {
        cmSystemTools::CopyAFile(licenseSourceFilename.c_str(), licenseDestinationFilename.c_str());
    }
    ...
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "unsupported WiX License file extension '" << extension << "'" << std::endl);
...
void cmCPackWIXGenerator::AddDirectoryAndFileDefinitons(const std::string & topdir,
                                                        const std::string & directoryId,
                                                        cmWIXSourceWriter & directoryDefinitions,
                                                        cmWIXSourceWriter & fileDefinitions,
                                                        cmWIXSourceWriter & featureDefinitions, const std::vector < std::string > &packageExecutables, const std::vector < std::string > &desktopExecutables, shortcut_map_t & shortcutMap)
{
    cmsys::Directory dir;
    ...
    for (size_t i = 0; i < dir.GetNumberOfFiles(); ++i)
    {
        ...
        std::string relativePath = cmSystemTools::RelativePath(toplevel.c_str(), fullPath.c_str());
        ...
        if (cmSystemTools::FileIsDirectory(fullPath.c_str()))
        {
            ...
            AddDirectoryAndFileDefinitons(fullPath, subDirectoryId, directoryDefinitions, fileDefinitions, featureDefinitions, packageExecutables, desktopExecutables, shortcutMap);
        ...
        else
        {
            ...
            for (size_t j = 0; j < packageExecutables.size(); ++j)
            {
                ...
                if (cmSystemTools::LowerCase(fileName) == cmSystemTools::LowerCase(executableName) + ".exe")
                {
                    cmWIXShortcut & shortcut = shortcutMap[id];
                    shortcut.textLabel = textLabel;
                    shortcut.workingDirectoryId = directoryId;

                    if (desktopExecutables.size() && std::find(desktopExecutables.begin(), desktopExecutables.end(), executableName) != desktopExecutables.end())
                    {
...
bool cmCPackWIXGenerator::RequireOption(const std::string & name, std::string & value) const
{
    ...
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "Required variable " << name << " not set" << std::endl);
...
std::string cmCPackWIXGenerator::GenerateGUID()
{
    ...
    std::string result = cmsys::Encoding::ToNarrow(reinterpret_cast < wchar_t * >(tmp));
    RpcStringFreeW(&tmp);
...
std::string cmCPackWIXGenerator::GetRightmostExtension(const std::string & filename)
{
...
std::string cmCPackWIXGenerator::CreateNewIdForPath(const std::string & path)
{
    ...
    for (size_t i = 1; i < components.size(); ++i)
    {
        ...
        currentComponent = NormalizeComponentForId(components[i], replacementCount);
    ...
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "Error while trying to generate a unique Id for '" << path << "'" << std::endl);
...
std::string cmCPackWIXGenerator::CreateHashedId(const std::string & path, const std::string & normalizedFilename)
{
...
std::string cmCPackWIXGenerator::NormalizeComponentForId(const std::string & component, size_t & replacementCount)
{
    std::string result;
...
void cmCPackWIXGenerator::CollectExtensions(const std::string & variableName, extension_set_t & extensions)
{
...
void cmCPackWIXGenerator::AddCustomFlags(const std::string & variableName, std::ostream & stream)
{
    const char *variableContent = GetOption(variableName.c_str());
    if (!variableContent)
        return;
...
void cmCPackWIXGenerator::CreateStartMenuFolder(cmWIXSourceWriter & directoryDefinitions)
{
    directoryDefinitions.BeginElement("Directory");
...
void cmCPackWIXGenerator::CreateDesktopFolder(cmWIXSourceWriter & directoryDefinitions)
{
    directoryDefinitions.BeginElement("Directory");
...
void cmCPackWIXGenerator::ApplyPatchFragment(const std::string & id, cmWIXSourceWriter & writer)
{
    ...
    for (cmWIXPatchElement::child_list_t::const_iterator j = fragment.children.begin(); j != fragment.children.end(); ++j)
    {
...
void cmCPackWIXGenerator::ApplyPatchElement(const cmWIXPatchElement & element, cmWIXSourceWriter & writer)
{
    writer.BeginElement(element.name);

    for (cmWIXPatchElement::attributes_t::const_iterator i = element.attributes.begin(); i != element.attributes.end(); ++i)
    {
        writer.AddAttribute(i->first, i->second);
    }

    for (cmWIXPatchElement::child_list_t::const_iterator i = element.children.begin(); i != element.children.end(); ++i)
    {
