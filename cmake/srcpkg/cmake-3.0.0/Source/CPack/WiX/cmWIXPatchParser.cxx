...
cmWIXPatchParser::cmWIXPatchParser(fragment_map_t & fragments, cmCPackLog * logger):
Logger(logger), State(BEGIN_DOCUMENT), Valid(true), Fragments(fragments)
{
...
void cmWIXPatchParser::StartFragment(const char **attributes)
{
    for (size_t i = 0; attributes[i]; i += 2)
    {
        ...
        {
            ReportValidationError("The only allowed 'CPackWixFragment' attribute is 'Id'");
        }
    }
}
...
{
    cmCPackLogger(cmCPackLog::LOG_ERROR, "Error while processing XML patch file at " << line << ":" << column << ":  " << msg << std::endl);
    Valid = false;
}

void cmWIXPatchParser::ReportValidationError(const std::string & message)
{
    ReportError(XML_GetCurrentLineNumber(Parser), XML_GetCurrentColumnNumber(Parser), message.c_str());
}
