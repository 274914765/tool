...
int cmCPackCygwinSourceGenerator::PackageFiles()
{
    ...
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "No patch file specified for cygwin sources.");
        return 0;
    }
    if (!cmSystemTools::CopyFileAlways(this->GetOption("CPACK_CYGWIN_PATCH_FILE"), this->GetOption("CPACK_TOPLEVEL_DIRECTORY")))
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "problem copying: [" << this->GetOption("CPACK_CYGWIN_PATCH_FILE") << "]\nto\n[" << this->GetOption("CPACK_TOPLEVEL_DIRECTORY") << "]\n");
        return 0;
    }
    if (!this->GetOption("CPACK_CYGWIN_BUILD_SCRIPT"))
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "No build script specified for cygwin sources.");
        return 0;
    }
    ...
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "problem copying: " << this->GetOption("CPACK_CYGWIN_BUILD_SCRIPT") << "\nto\n" << this->GetOption("CPACK_TOPLEVEL_DIRECTORY") << "]\n");
        return 0;
    }
    ...
    if (!patch)
    {
        cmCPackLogger(cmCPackLog::LOG_WARNING, "CPACK_CYGWIN_PATCH_NUMBER" << " not specified, defaulting to 1\n");
        patch = "1";
    }
    outerTarFile += patch;
    ...
    buildScript += "/";
    buildScript += cmSystemTools::GetFilenameName(this->GetOption("CPACK_CYGWIN_BUILD_SCRIPT"));
