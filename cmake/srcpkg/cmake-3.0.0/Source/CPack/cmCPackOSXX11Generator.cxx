...
int cmCPackOSXX11Generator::PackageFiles()
{
    ...
    if (cpackPackageExecutables)
    {
        ...
        for (it = cpackPackageExecutablesVector.begin(); it != cpackPackageExecutablesVector.end(); ++it)
        {
            ...
            ++it;
            this->SetOptionIfNotSet("CPACK_EXECUTABLE_NAME", cpackExecutableName.c_str());
        }
    }
    ...
    if (!this->CopyResourcePlistFile("VolumeIcon.icns",
        ...
        !this->CopyResourcePlistFile("OSXX11.main.scpt", scrDir,
                                     "main.scpt", true) || !this->CopyResourcePlistFile("OSXScriptLauncher.rsrc", dir, rsrcFile, true) || !this->CopyResourcePlistFile("OSXScriptLauncher", appdir, this->GetOption("CPACK_PACKAGE_FILE_NAME"), true))
    {
    ...
    if (cmsys::SystemTools::GetPermissions(runTimeScript.c_str(), mode))
    {
        ...
        cmCPackLogger(cmCPackLog::LOG_OUTPUT, "Setting: " << runTimeScript << " to permission: " << mode << std::endl);
    }
    ...
    if (cmsys::SystemTools::GetPermissions(appScriptName.c_str(), mode))
    {
        ...
        cmCPackLogger(cmCPackLog::LOG_OUTPUT, "Setting: " << appScriptName << " to permission: " << mode << std::endl);
    }
    ...
    dmgCmd << "\"" << this->GetOption("CPACK_INSTALLER_PROGRAM_DISK_IMAGE") << "\" create -ov -format UDZO -srcfolder \"" << diskImageDirectory.c_str() << "\" \"" << packageFileNames[0] << "\"";
    cmCPackLogger(cmCPackLog::LOG_VERBOSE, "Compress disk image using command: " << dmgCmd.str().c_str() << std::endl);
...
{
    cmCPackLogger(cmCPackLog::LOG_DEBUG, "cmCPackOSXX11Generator::Initialize()" << std::endl);
...
//----------------------------------------------------------------------
bool cmCPackOSXX11Generator::CopyResourcePlistFile(const char *name, const char *dir, const char *outputFileName /* = 0 */ ,
                                                   bool copyOnly /* = false */ )
{
