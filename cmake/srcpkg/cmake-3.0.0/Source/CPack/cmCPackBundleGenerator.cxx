...
int cmCPackBundleGenerator::InitializeInternal()
{
    ...
    if (0 == name)
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "CPACK_BUNDLE_NAME must be set to use the Bundle generator." << std::endl);
...
int cmCPackBundleGenerator::PackageFiles()
{
    ...
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "CPACK_BUNDLE_NAME must be set." << std::endl);
    ...
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "CPACK_BUNDLE_PLIST must be set." << std::endl);
    ...
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "CPACK_BUNDLE_ICON must be set." << std::endl);
    ...
    contents << staging.str() << "/" << cpack_bundle_name << ".app/" << "Contents";
    ...
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "Error copying plist.  Check the value of CPACK_BUNDLE_PLIST." << std::endl);
    ...
    {
        cmCPackLogger(cmCPackLog::LOG_ERROR, "Error copying bundle icon.  Check the value of CPACK_BUNDLE_ICON." << std::endl);
    ...
    if (!cpack_bundle_startup_command.empty())
    {
        ...
        {
            cmCPackLogger(cmCPackLog::LOG_ERROR, "Error copying startup command. " " Check the value of CPACK_BUNDLE_STARTUP_COMMAND." << std::endl);
