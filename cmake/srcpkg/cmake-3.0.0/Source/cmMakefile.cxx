...
{
    for (std::vector < cmInstallGenerator * >::iterator i = this->InstallGenerators.begin(); i != this->InstallGenerators.end(); ++i)
    {
        delete *i;
    }
    for (std::vector < cmTestGenerator * >::iterator i = this->TestGenerators.begin(); i != this->TestGenerators.end(); ++i)
    {
        delete *i;
    }
    for (std::vector < cmSourceFile * >::iterator i = this->SourceFiles.begin(); i != this->SourceFiles.end(); ++i)
    {
        delete *i;
    }
    for (std::map < cmStdString, cmTest * >::iterator i = this->Tests.begin(); i != this->Tests.end(); ++i)
    {
        delete i->second;
    }
    for (std::vector < cmTarget * >::iterator i = this->ImportedTargetsOwned.begin(); i != this->ImportedTargetsOwned.end(); ++i)
    {
        delete *i;
    }
    ...
    for (pos = this->FunctionBlockers.begin(); pos != this->FunctionBlockers.end(); ++pos)
    {
    ...
    {
        cmSystemTools::Error("Internal CMake Error, Policy Stack has not been" " popped properly");
    }
}
...
{
    std::cout << s << ": ( \n";
    for (std::vector < std::pair < cmStdString, bool > >::const_iterator i = v.begin(); i != v.end(); ++i)
    {
...
void cmMakefile::Print() const
{
    ...
    for (cmTargets::iterator l = this->Targets.begin(); l != this->Targets.end(); l++)
    {
        std::cout << l->first << std::endl;
    }

    std::cout << " this->StartOutputDirectory; " << this->StartOutputDirectory.c_str() << std::endl;
    std::cout << " this->HomeOutputDirectory; " << this->HomeOutputDirectory.c_str() << std::endl;
    std::cout << " this->cmStartDirectory; " << this->cmStartDirectory.c_str() << std::endl;
    std::cout << " this->cmHomeDirectory; " << this->cmHomeDirectory.c_str() << std::endl;
...
#if defined(CMAKE_BUILD_WITH_CMAKE)
    for (std::vector < cmSourceGroup >::const_iterator i = this->SourceGroups.begin(); i != this->SourceGroups.end(); ++i)
    {
...
void cmMakefile::IssueMessage(cmake::MessageType t, std::string const &text) const
{
    ...
    else
    {
        ...
        {
            lfc.FilePath = this->LocalGenerator->Convert(lfc.FilePath.c_str(), cmLocalGenerator::HOME);
        }
...
bool cmMakefile::GetBacktrace(cmListFileBacktrace & backtrace) const
{
    ...
    for (CallStackType::const_reverse_iterator i = this->CallStack.rbegin(); i != this->CallStack.rend(); ++i)
    {
...
void cmMakefile::PrintCommandTrace(const cmListFileFunction & lff) const
{
    ...
    msg << lff.Name << "(";
    for (std::vector < cmListFileArgument >::const_iterator i = lff.Arguments.begin(); i != lff.Arguments.end(); ++i)
    {
        msg << i->Value;
        msg << " ";
    }
...
//----------------------------------------------------------------------------
bool cmMakefile::ExecuteCommand(const cmListFileFunction & lff, cmExecutionStatus & status)
{
    bool result = true;
    ...
    if (cmCommand * proto = this->GetCMakeInstance()->GetCommand(name.c_str()))
    {
        ...
        if (pcmd->GetEnabled() && !cmSystemTools::GetFatalErrorOccured() && (this->GetCMakeInstance()->GetWorkingMode() != cmake::SCRIPT_MODE || pcmd->IsScriptable()))

        {
            ...
            if (!pcmd->InvokeInitialPass(lff.Arguments, status) || status.GetNestedError())
            {
        ...
        else if (this->GetCMakeInstance()->GetWorkingMode() == cmake::SCRIPT_MODE && !pcmd->IsScriptable())
        {
...
//----------------------------------------------------------------------------
cmMakefile::IncludeScope::IncludeScope(cmMakefile * mf, const char *fname, bool noPolicyScope):Makefile(mf), File(fname), NoPolicyScope(noPolicyScope), CheckCMP0011(false), ReportError(true)
{
...
//
bool cmMakefile::ReadListFile(const char *filename_in, const char *external_in, std::string * fullPath, bool noPolicyScope)
{
    std::string currentParentFile = this->GetSafeDefinition("CMAKE_PARENT_LIST_FILE");
    ...
    if (external_in)
    {
        external_abs = cmSystemTools::CollapseFullPath(external_in, this->cmStartDirectory.c_str());
        external = external_abs.c_str();
        if (filename_in)
        {
            filename_abs = cmSystemTools::CollapseFullPath(filename_in, this->cmStartDirectory.c_str());
    ...
    this->AddDefinition("CMAKE_CURRENT_LIST_DIR", cmSystemTools::GetFilenamePath(filenametoread).c_str());
    ...
    {
        if (cmSystemTools::LowerCase(cmSystemTools::GetFilenameName(filename)) == "cmakelists.txt")
        {
    ...
    if (!cacheFile.ParseFile(filenametoread, requireProjectCommand, this))
    {
        ...
        this->MarkVariableAsUsed("CMAKE_CURRENT_LIST_FILE");
        this->AddDefinition("CMAKE_CURRENT_LIST_DIR", cmSystemTools::GetFilenamePath(currentFile).c_str());
        this->MarkVariableAsUsed("CMAKE_CURRENT_LIST_DIR");
        return false;
    }
...
#if defined(CMAKE_BUILD_WITH_CMAKE)
    this->AddSourceGroup("", "^.*$");
    this->AddSourceGroup("Source Files", "\\.(C|M|c|c\\+\\+|cc|cpp|cxx|f|f90|for|fpp" "|ftn|m|mm|rc|def|r|odl|idl|hpj|bat)$");
...
namespace
{
    struct file_not_persistent
    {
        ...
        {
            return !(path.find("CMakeTmp") == path.npos && cmSystemTools::FileExists(path.c_str()));
        }
    };
}
...
void cmMakefile::FinalPass()
{
    ...
    for (std::vector < cmCommand * >::iterator i = this->FinalPassCommands.begin(); i != this->FinalPassCommands.end(); ++i)
    {
        (*i)->FinalPass();
    }
    ...
    //back of the vector
    std::vector < std::string >::iterator new_output_files_end = std::remove_if(this->OutputFiles.begin(), this->OutputFiles.end(), file_not_persistent());
    ...
    std::vector < std::string >::iterator new_list_files_end = std::remove_if(this->ListFiles.begin(), this->ListFiles.end(), file_not_persistent());
...
void cmMakefile::ConfigureFinalPass()
{
    ...
    if (oldValue && cmSystemTools::VersionCompare(cmSystemTools::OP_LESS, oldValue, "2.4"))
    {
        this->IssueMessage(cmake::FATAL_ERROR,
                           ...
                           "than 2.4. This version of CMake only supports backwards compatibility " "with CMake 2.4 or later. For compatibility with older versions please " "use any CMake 2.8.x release or lower.");
    }
    for (cmTargets::iterator l = this->Targets.begin(); l != this->Targets.end(); l++)
    {
...
//----------------------------------------------------------------------------
void cmMakefile::AddCustomCommandToTarget(const char *target, const std::vector < std::string > &depends, const cmCustomCommandLines & commandLines, cmTarget::CustomCommandType type, const char *comment, const char *workingDir, bool escapeOldStyle) const
{
    ...
    if (ti == this->Targets.end())
    {
        ...
        {
            case cmPolicies::WARN:
                e << (this->GetPolicies()->GetPolicyWarning(cmPolicies::CMP0040)) << "\n";
                issueMessage = true;
            case cmPolicies::OLD:
                break;
            case cmPolicies::NEW:
    ...
    {
        cmOStringStream e;
        e << "Target \"" << target << "\" is an OBJECT library " "that may not have PRE_BUILD, PRE_LINK, or POST_BUILD commands.";
    ...
    {
        cmOStringStream e;
        e << "Target \"" << target << "\" is an INTERFACE library " "that may not have PRE_BUILD, PRE_LINK, or POST_BUILD commands.";
        this->IssueMessage(cmake::FATAL_ERROR, e.str());
        return;
    }
    // Add the command to the appropriate build step for the target.
    std::vector < std::string > no_output;
    cmCustomCommand cc(this, no_output, depends, commandLines, comment, workingDir);
...
//----------------------------------------------------------------------------
cmSourceFile *cmMakefile::AddCustomCommandToOutput(const std::vector < std::string > &outputs,
                                                   const std::vector < std::string > &depends, const char *main_dependency, const cmCustomCommandLines & commandLines, const char *comment, const char *workingDir, bool replace, bool escapeOldStyle)
{
    ...
    for (cmCustomCommandLines::const_iterator i = commandLines.begin(); i != commandLines.end(); ++i)
    {
    ...
    if (!file)
    {
        ...
        if (file && file->GetCustomCommand() && !replace)
        {
            ...
            if (commandLines != file->GetCustomCommand()->GetCommandLines())
            {
                cmSystemTools::Error("Attempt to add a custom rule to output \"", outName.c_str(), "\" which already has a custom rule.");
            }
            return file;
        }
    ...
    if (file)
    {
        cmCustomCommand *cc = new cmCustomCommand(this, outputs, depends2, commandLines,
                                                  comment, workingDir);
        cc->SetEscapeOldStyle(escapeOldStyle);
        cc->SetEscapeAllowMakeVars(true);
...
//----------------------------------------------------------------------------
void cmMakefile::UpdateOutputToSourceMap(std::vector < std::string > const &outputs, cmSourceFile * source)
{
    for (std::vector < std::string >::const_iterator o = outputs.begin(); o != outputs.end(); ++o)
    {
...
//----------------------------------------------------------------------------
void cmMakefile::UpdateOutputToSourceMap(std::string const &output, cmSourceFile * source)
{
...
//----------------------------------------------------------------------------
cmSourceFile *cmMakefile::AddCustomCommandToOutput(const char *output,
                                                   const std::vector < std::string > &depends, const char *main_dependency, const cmCustomCommandLines & commandLines, const char *comment, const char *workingDir, bool replace, bool escapeOldStyle)
{
    std::vector < std::string > outputs;
    outputs.push_back(output);
    return this->AddCustomCommandToOutput(outputs, depends, main_dependency, commandLines, comment, workingDir, replace, escapeOldStyle);
}

//----------------------------------------------------------------------------
void cmMakefile::AddCustomCommandOldStyle(const char *target, const std::vector < std::string > &outputs, const std::vector < std::string > &depends, const char *source, const cmCustomCommandLines & commandLines, const char *comment)
{
    ...
    if (strcmp(source, target) == 0)
    {
        ...
        // this behavior.
        this->AddCustomCommandToTarget(target, depends, commandLines, cmTarget::POST_BUILD, comment, 0);
        return;
    }

    // Each output must get its own copy of this rule.
    cmsys::RegularExpression sourceFiles("\\.(C|M|c|c\\+\\+|cc|cpp|cxx|m|mm|" "rc|def|r|odl|idl|hpj|bat|h|h\\+\\+|" "hm|hpp|hxx|in|txx|inl)$");
    for (std::vector < std::string >::const_iterator oi = outputs.begin(); oi != outputs.end(); ++oi)
    {
        ...
        if (sourceFiles.find(source))
        {
            ...
            sf = this->AddCustomCommandToOutput(output, depends, source, commandLines, comment, 0);
        }
        else
        {
            ...
            sf = this->AddCustomCommandToOutput(output, depends2, no_main_dependency, commandLines, comment, 0);
        }
...
//----------------------------------------------------------------------------
void cmMakefile::AddUtilityCommand(const char *utilityName, bool excludeFromAll, const std::vector < std::string > &depends, const char *workingDirectory, const char *command, const char *arg1, const char *arg2, const char *arg3, const char *arg4)
{
    ...
    this->AddUtilityCommand(utilityName, excludeFromAll, workingDirectory, depends, commandLines);
}

//----------------------------------------------------------------------------
cmTarget *cmMakefile::AddUtilityCommand(const char *utilityName, bool excludeFromAll, const char *workingDirectory, const std::vector < std::string > &depends, const cmCustomCommandLines & commandLines, bool escapeOldStyle, const char *comment)
{
    ...
    this->AddCustomCommandToOutput(force.c_str(), depends, no_main_dependency, commandLines, comment, workingDirectory, no_replace, escapeOldStyle);
    ...
    {
        cmSystemTools::Error("Could not get source file entry for ", force.c_str());
    }
    return target;
}
...
void cmMakefile::RemoveDefineFlag(const char *flag, std::string::size_type len, std::string & dflags)
{
    ...
    for (std::string::size_type lpos = dflags.find(flag, 0); lpos != std::string::npos; lpos = dflags.find(flag, lpos))
    {
        ...
        if ((lpos <= 0 || isspace(dflags[lpos - 1])) && (rpos >= dflags.size() || isspace(dflags[rpos])))
        {
...
bool cmMakefile::ParseDefineFlag(std::string const &def, bool remove)
{
    ...
    // combination with '"', '$', or ';'.
    if ((strcmp(this->LocalGenerator->GetGlobalGenerator()->GetName(), "Visual Studio 6") == 0) && (def.find(" ") != def.npos && def.find_first_of("\"$;") != def.npos))
    {
        return false;
    }

    // Definitions with non-trivial values require a policy check.
    static cmsys::RegularExpression trivial("^[-/]D[A-Za-z_][A-Za-z0-9_]*(=[A-Za-z0-9_.]+)?$");
    if (!trivial.find(def.c_str()))
    {
        ...
        {
            case cmPolicies::WARN:
                this->IssueMessage(cmake::AUTHOR_WARNING, this->GetPolicies()->GetPolicyWarning(cmPolicies::CMP0005));
            case cmPolicies::OLD:
            ...
            case cmPolicies::REQUIRED_IF_USED:
            case cmPolicies::REQUIRED_ALWAYS:
                this->IssueMessage(cmake::FATAL_ERROR, this->GetPolicies()->GetRequiredPolicyError(cmPolicies::CMP0005));
                return false;
            case cmPolicies::NEW:
    ...
    if (remove)
    {
        if (const char *cdefs = this->GetProperty("COMPILE_DEFINITIONS"))
        {
            ...
            std::string ndefs;
            const char *sep = "";
            for (std::vector < std::string >::const_iterator di = defs.begin(); di != defs.end(); ++di)
            {
                if (*di != define)
                {
                    ndefs += sep;
...
void cmMakefile::AddLinkLibraryForTarget(const char *target, const char *lib, cmTarget::LinkLibraryType llt)
{
    cmTargets::iterator i = this->Targets.find(target);
    if (i != this->Targets.end())
    {
        cmTarget *tgt = this->GetCMakeInstance()->GetGlobalGenerator()->FindTarget(0, lib);
        if (tgt)
        {
            // if it is not a static or shared library then you can not link to it
            if (!((tgt->GetType() == cmTarget::STATIC_LIBRARY) || (tgt->GetType() == cmTarget::SHARED_LIBRARY) || (tgt->GetType() == cmTarget::INTERFACE_LIBRARY) || tgt->IsExecutableWithExports()))
            {
                cmOStringStream e;
                e << "Target \"" << lib << "\" of type "
                    << cmTarget::GetTargetTypeName(tgt->GetType()) << " may not be linked into another target.  " << "One may link only to STATIC or SHARED libraries, or " << "to executables with the ENABLE_EXPORTS property set.";
    ...
    {
        cmOStringStream e;
        e << "Attempt to add link library \"" << lib << "\" to target \"" << target << "\" which is not built in this directory.";
        this->IssueMessage(cmake::FATAL_ERROR, e.str().c_str());
    }
}

void cmMakefile::AddLinkDirectoryForTarget(const char *target, const char *d)
{
    cmTargets::iterator i = this->Targets.find(target);
    if (i != this->Targets.end())
    {
        ...
        {
            cmOStringStream e;
            e << "ALIAS target \"" << target << "\" " << "may not be linked into another target.";
            this->IssueMessage(cmake::FATAL_ERROR, e.str().c_str());
            return;
        }
    ...
    {
        cmSystemTools::Error("Attempt to add link directories to non-existent target: ", target, " for directory ", d);
    }
}
...
void cmMakefile::AddLinkDirectory(const char *dir)
{
    ...
    if (dir[strlen(dir) - 1] == '/')
    {
        ...
        if (std::find(this->LinkDirectories.begin(), this->LinkDirectories.end(), newdir.c_str()) == this->LinkDirectories.end())
        {
    ...
    {
        if (std::find(this->LinkDirectories.begin(), this->LinkDirectories.end(), dir) == this->LinkDirectories.end())
        {
...
void cmMakefile::InitializeFromParent()
{
    ...
    const std::vector < cmValueWithOrigin > &parentIncludes = parent->GetIncludeDirectoriesEntries();
    this->IncludeDirectoriesEntries.insert(this->IncludeDirectoriesEntries.end(), parentIncludes.begin(), parentIncludes.end());

    const std::vector < cmValueWithOrigin > &parentOptions = parent->GetCompileOptionsEntries();
    this->CompileOptionsEntries.insert(this->CompileOptionsEntries.end(), parentOptions.begin(), parentOptions.end());

    const std::vector < cmValueWithOrigin > &parentDefines = parent->GetCompileDefinitionsEntries();
    this->CompileDefinitionsEntries.insert(this->CompileDefinitionsEntries.end(), parentDefines.begin(), parentDefines.end());
    ...
    {
        this->SetProperty("COMPILE_DEFINITIONS", parent->GetProperty("COMPILE_DEFINITIONS"));
        std::vector < std::string > configs;
        this->GetConfigurations(configs);
        for (std::vector < std::string >::const_iterator ci = configs.begin(); ci != configs.end(); ++ci)
        {
...
void cmMakefile::AddSubDirectory(const char *sub, bool excludeFromAll, bool preorder)
{
    ...
    this->AddSubDirectory(srcPath.c_str(), binPath.c_str(), excludeFromAll, preorder, false);
}

void cmMakefile::AddSubDirectory(const char *srcPath, const char *binPath, bool excludeFromAll, bool preorder, bool immediate)
{
    ...
    cmLocalGenerator *lg2 = this->LocalGenerator->GetGlobalGenerator()->CreateLocalGenerator();
...
//----------------------------------------------------------------------------
void cmMakefile::AddIncludeDirectories(const std::vector < std::string > &incs, bool before)
{
    if (incs.empty())
    {
        return;
    ...
    std::vector < cmValueWithOrigin >::iterator position = before ? this->IncludeDirectoriesEntries.begin() : this->IncludeDirectoriesEntries.end();
...
//----------------------------------------------------------------------------
void cmMakefile::AddSystemIncludeDirectories(const std::set < cmStdString > &incs)
{
...
void cmMakefile::AddDefinition(const char *name, const char *value)
{
    ...
    if (this->Internal->VarUsageStack.size() && this->VariableInitialized(name))
    {
    ...
    if (vv)
    {
        vv->VariableAccessed(name, cmVariableWatch::VARIABLE_MODIFIED_ACCESS, value, this);
    }
#endif
}

void cmMakefile::AddCacheDefinition(const char *name, const char *value, const char *doc, cmCacheManager::CacheEntryType type, bool force)
{
    const char *val = value;
    cmCacheManager::CacheIterator it = this->GetCacheManager()->GetCacheIterator(name);
    if (!it.IsAtEnd() && (it.GetType() == cmCacheManager::UNINITIALIZED) && it.Initialized())
    {
...
void cmMakefile::AddDefinition(const char *name, bool value)
{
    ...
    if (vv)
    {
        vv->VariableAccessed(name, cmVariableWatch::VARIABLE_MODIFIED_ACCESS, value ? "ON" : "OFF", this);
    }
#endif
}
...
void cmMakefile::CheckForUnused(const char *reason, const char *name) const
{
    if (this->WarnUnused && !this->VariableUsed(name))
    {
        ...
        if (this->CheckSystemVars ||
            cmSystemTools::IsSubDirectory(path.c_str(), this->GetHomeDirectory()) || (cmSystemTools::IsSubDirectory(path.c_str(), this->GetHomeOutputDirectory()) && !cmSystemTools::IsSubDirectory(path.c_str(), cmake::GetCMakeFilesDirectory())))
        {
            cmOStringStream msg;
            msg << "unused variable (" << reason << ") \'" << name << "\'";
            this->GetCMakeInstance()->IssueMessage(cmake::AUTHOR_WARNING, msg.str().c_str(), bt);
        }
    }
}
...
void cmMakefile::RemoveDefinition(const char *name)
{
    ...
    if (vv)
    {
        vv->VariableAccessed(name, cmVariableWatch::VARIABLE_REMOVED_ACCESS, 0, this);
    }
#endif
}
...
void cmMakefile::AddGlobalLinkInformation(const char *name, cmTarget & target)
{
    ...
    for (j = this->LinkDirectories.begin(); j != this->LinkDirectories.end(); ++j)
    {
...
cmTarget *cmMakefile::AddLibrary(const char *lname, cmTarget::TargetType type, const std::vector < std::string > &srcs, bool excludeFromAll)
{
    // wrong type ? default to STATIC
    if ((type != cmTarget::STATIC_LIBRARY) && (type != cmTarget::SHARED_LIBRARY) && (type != cmTarget::MODULE_LIBRARY) && (type != cmTarget::OBJECT_LIBRARY) && (type != cmTarget::INTERFACE_LIBRARY))
    {
        this->IssueMessage(cmake::INTERNAL_ERROR, "cmMakefile::AddLibrary given invalid target type.");
...
cmTarget *cmMakefile::AddExecutable(const char *exeName, const std::vector < std::string > &srcs, bool excludeFromAll)
{
...
{
    cmTargets::iterator it = this->Targets.insert(cmTargets::value_type(name, cmTarget())).first;
...
cmSourceFile *cmMakefile::LinearGetSourceFileWithOutput(const char *cname) const
{
    ...
    for (std::vector < cmSourceFile * >::const_iterator i = this->SourceFiles.begin(); i != this->SourceFiles.end(); ++i)
    {
        ...
        if ((*i)->GetCustomCommand())
        {
            ...
            const std::vector < std::string > &outputs = (*i)->GetCustomCommand()->GetOutputs();
            for (std::vector < std::string >::const_iterator o = outputs.begin(); o != outputs.end(); ++o)
            {
                out = *o;
                std::string::size_type pos = out.rfind(name);
                // If the output matches exactly
                if (pos != out.npos && pos == out.size() - name.size() && (pos == 0 || out[pos - 1] == '/'))
                {
                    return *i;
                }
            }
...
#if defined(CMAKE_BUILD_WITH_CMAKE)
cmSourceGroup *cmMakefile::GetSourceGroup(const std::vector < std::string > &name) const
{
    cmSourceGroup *sg = 0;

    // first look for source group starting with the same as the one we wants
    for (std::vector < cmSourceGroup >::const_iterator sgIt = this->SourceGroups.begin(); sgIt != this->SourceGroups.end(); ++sgIt)
    {
...
void cmMakefile::AddSourceGroup(const std::vector < std::string > &name, const char *regex)
{
    cmSourceGroup *sg = 0;
...
void cmMakefile::ExpandVariablesCMP0019()
{
    ...
    for (std::vector < std::string >::iterator d = this->LinkDirectories.begin(); d != this->LinkDirectories.end(); ++d)
    {
    ...
    for (cmTarget::LinkLibraryVectorType::iterator l = this->LinkLibraries.begin(); l != this->LinkLibraries.end(); ++l)
    {
...
bool cmMakefile::CanIWriteThisFile(const char *fileName) const
{
    ...
    if (cmSystemTools::IsSubDirectory(fileName, this->GetHomeDirectory()) && !cmSystemTools::IsSubDirectory(fileName, this->GetHomeOutputDirectory()))
    {
        return false;
    }
    return true;
...
bool cmMakefile::IsDefinitionSet(const char *name) const
{
    ...
    if (cmVariableWatch * vv = this->GetVariableWatch())
    {
        if (!def)
        {
            vv->VariableAccessed(name, cmVariableWatch::UNKNOWN_VARIABLE_DEFINED_ACCESS, def, this);
        }
    }
#endif
...
const char *cmMakefile::GetDefinition(const char *name) const
{
    ...
    if (vv)
    {
        if (def)
        {
            vv->VariableAccessed(name, cmVariableWatch::VARIABLE_READ_ACCESS, def, this);
        }
        else
        {
            // are unknown access allowed
            const char *allow = this->Internal->VarStack.top().Get("CMAKE_ALLOW_UNKNOWN_VARIABLE_READ_ACCESS");
            if (cmSystemTools::IsOn(allow))
            {
                vv->VariableAccessed(name, cmVariableWatch::ALLOWED_UNKNOWN_VARIABLE_READ_ACCESS, def, this);
            }
            else
            {
                vv->VariableAccessed(name, cmVariableWatch::UNKNOWN_VARIABLE_READ_ACCESS, def, this);
            }
        }
    }
#endif
...
const char *cmMakefile::ExpandVariablesInString(std::string & source, bool escapeQuotes, bool noEscapes, bool atOnly, const char *filename, long line, bool removeEmpty, bool replaceAt) const
{
    ...
    if (atOnly)
    {
        if (!noEscapes || !removeEmpty || !replaceAt)
        {
            ...
            this->IssueMessage(cmake::INTERNAL_ERROR, "ExpandVariablesInString @ONLY called " "on something with escapes.");
        }
    ...
    else
    {
        ...
        if (!res)
        {
            ...
            {
                case cmPolicies::WARN:
                    error << "\n" << (this->GetPolicies()->GetPolicyWarning(cmPolicies::CMP0010));
                case cmPolicies::OLD:
                ...
                case cmPolicies::REQUIRED_IF_USED:
                case cmPolicies::REQUIRED_ALWAYS:
                    error << "\n" << (this->GetPolicies()->GetRequiredPolicyError(cmPolicies::CMP0010));
                case cmPolicies::NEW:
...
void cmMakefile::RemoveVariablesInString(std::string & source, bool atOnly) const
{
    if (!atOnly)
    {
...
/* Up to CMake 2.4 here only WIN32, UNIX and APPLE were set.
  ...
  files are executed. */
    ...
    this->AddDefinition("CMAKE_FILES_DIRECTORY", cmake::GetCMakeFilesDirectory());
}

//----------------------------------------------------------------------------
const char *cmMakefile::GetConfigurations(std::vector < std::string > &configs, bool single) const
{
    if (this->LocalGenerator->GetGlobalGenerator()->IsMultiConfig())
    {
        if (const char *configTypes = this->GetDefinition("CMAKE_CONFIGURATION_TYPES"))
        {
 ...
 * inherited ones.
 */
cmSourceGroup *cmMakefile::FindSourceGroup(const char *source, std::vector < cmSourceGroup > &groups) const
{
    // First search for a group that lists the file explicitly.
    for (std::vector < cmSourceGroup >::reverse_iterator sg = groups.rbegin(); sg != groups.rend(); ++sg)
    {
...
bool cmMakefile::IsFunctionBlocked(const cmListFileFunction & lff, cmExecutionStatus & status)
{
    ...
    for (pos = this->FunctionBlockers.rbegin(); pos != this->FunctionBlockers.rend(); ++pos)
    {
...
void cmMakefile::PopFunctionBlockerBarrier(bool reportError)
{
    ...
    FunctionBlockersType::size_type barrier = this->FunctionBlockerBarriers.back();
...
bool cmMakefile::ExpandArguments(std::vector < cmListFileArgument > const &inArgs, std::vector < std::string > &outArgs) const
{
    ...
    for (i = inArgs.begin(); i != inArgs.end(); ++i)
    {
        ...
        value = i->Value;
        this->ExpandVariablesInString(value, false, false, false, i->FilePath, i->Line, false, true);
...
cmsys::auto_ptr < cmFunctionBlocker > cmMakefile::RemoveFunctionBlocker(cmFunctionBlocker * fb, const cmListFileFunction & lff)
{
    ...
    for (FunctionBlockersType::size_type i = this->FunctionBlockers.size(); i > barrier; --i)
    {
        std::vector < cmFunctionBlocker * >::iterator pos = this->FunctionBlockers.begin() + (i - 1);
        if (*pos == fb)
        {
...
cmSourceFile *cmMakefile::GetSource(const char *sourceName) const
{
    ...
    for (std::vector < cmSourceFile * >::const_iterator sfi = this->SourceFiles.begin(); sfi != this->SourceFiles.end(); ++sfi)
    {
        cmSourceFile *sf = *sfi;
        if (sf->Matches(sfl))
        {
...
//----------------------------------------------------------------------------
cmSourceFile *cmMakefile::GetOrCreateSource(const char *sourceName, bool generated)
{
...
void cmMakefile::EnableLanguage(std::vector < std::string > const &lang, bool optional)
{
    this->AddDefinition("CMAKE_CFG_INTDIR", this->LocalGenerator->GetGlobalGenerator()->GetCMakeCFGIntDir());
    this->LocalGenerator->GetGlobalGenerator()->EnableLanguage(lang, this, optional);
}

void cmMakefile::ExpandSourceListArguments(std::vector < std::string > const &arguments, std::vector < std::string > &newargs, unsigned int /* start */ ) const
{
    // now expand the args
    unsigned int i;
...
int cmMakefile::TryCompile(const char *srcdir, const char *bindir, const char *projectName, const char *targetName, bool fast, const std::vector < std::string > *cmakeArgs, std::string * output)
{
    ...
    cmGlobalGenerator *gg = cm.CreateGlobalGenerator(this->LocalGenerator->GetGlobalGenerator()->GetName());
    if (!gg)
    {
        cmSystemTools::Error("Internal CMake error, TryCompile bad GlobalGenerator");
    ...
    if (!gg->IsMultiConfig())
    {
        if (const char *config = this->GetDefinition("CMAKE_TRY_COMPILE_CONFIGURATION"))
        {
            // Tell the single-configuration generator which one to use.
            // Add this before the user-provided CMake arguments in case
            // one of the arguments is -DCMAKE_BUILD_TYPE=...
            cm.AddCacheEntry("CMAKE_BUILD_TYPE", config, "Build configuration", cmCacheManager::STRING);
        }
    }
    ...
    gg->EnableLanguagesFromGenerator(this->LocalGenerator->GetGlobalGenerator(), this);
    if (this->IsOn("CMAKE_SUPPRESS_DEVELOPER_WARNINGS"))
    {
        cm.AddCacheEntry("CMAKE_SUPPRESS_DEVELOPER_WARNINGS", "TRUE", "", cmCacheManager::INTERNAL);
    }
    else
    {
        cm.AddCacheEntry("CMAKE_SUPPRESS_DEVELOPER_WARNINGS", "FALSE", "", cmCacheManager::INTERNAL);
    }
    if (cm.Configure() != 0)
    {
        cmSystemTools::Error("Internal CMake error, TryCompile configure of cmake failed");
        // return to the original directory
        cmSystemTools::ChangeDirectory(cwd.c_str());
        this->Internal->IsSourceFileTryCompile = false;
        return 1;
    ...
    if (cm.Generate() != 0)
    {
        cmSystemTools::Error("Internal CMake error, TryCompile generation of cmake failed");
        // return to the original directory
        cmSystemTools::ChangeDirectory(cwd.c_str());
        this->Internal->IsSourceFileTryCompile = false;
        return 1;
    ...
    int ret = this->LocalGenerator->GetGlobalGenerator()->TryCompile(srcdir, bindir,
                                                                     projectName,
                                                                     targetName,
                                                                     fast,
                                                                     output,
...
{
    cmake *cm = this->GetLocalGenerator()->GetGlobalGenerator()->GetCMakeInstance();
...
std::string cmMakefile::GetModulesFile(const char *filename) const
{
    ...
    if (cmakeModulePath)
    {
        ...
        for (std::vector < std::string >::iterator i = modulePath.begin(); i != modulePath.end(); ++i)
        {
            std::string itempl = *i;
                ...
                case cmPolicies::WARN:
                    {
                        ...
                        e << "File " << currentFile << " includes "
                            << moduleInCMakeModulePath << " (found via CMAKE_MODULE_PATH) which shadows " << moduleInCMakeRoot << ". This may cause errors later on .\n" << this->GetPolicies()->GetPolicyWarning(cmPolicies::CMP0017);
...
void cmMakefile::ConfigureString(const std::string & input, std::string & output, bool atOnly, bool escapeQuotes) const
{
...
int cmMakefile::ConfigureFile(const char *infile, const char *outfile, bool copyonly, bool atOnly, bool escapeQuotes, const cmNewLineStyle & newLine)
{
    int res = 1;
    ...
    else
    {
        ...
        if (!fout)
        {
            cmSystemTools::Error("Could not open file for write in copy operation ", tempOutputFile.c_str());
        ...
        if (!fin)
        {
            cmSystemTools::Error("Could not open file for read in copy operation ", sinfile.c_str());
            return 0;
        }
...
void cmMakefile::SetProperty(const char *prop, const char *value)
{
    ...
    if (propname == "INCLUDE_DIRECTORIES")
    {
        ...
        cmListFileBacktrace lfbt;
        this->GetBacktrace(lfbt);
        this->IncludeDirectoriesEntries.push_back(cmValueWithOrigin(value, lfbt));
        return;
    }
    ...
    if (propname == "ADDITIONAL_MAKE_CLEAN_FILES")
    {
        ...
        if (strcmp(this->GetCurrentDirectory(), this->GetStartDirectory()) != 0)
        {
            return;
        }
    }
...
void cmMakefile::AppendProperty(const char *prop, const char *value, bool asString)
{
    if (!prop)
    {
        return;
    ...
    if (propname == "COMPILE_DEFINITIONS")
    {
        cmListFileBacktrace lfbt;
        this->GetBacktrace(lfbt);
        this->CompileDefinitionsEntries.push_back(cmValueWithOrigin(value, lfbt));
        return;
    }
    if (propname == "LINK_DIRECTORIES")
    {
        std::vector < std::string > varArgsExpanded;
        cmSystemTools::ExpandListArgument(value, varArgsExpanded);
        for (std::vector < std::string >::const_iterator vi = varArgsExpanded.begin(); vi != varArgsExpanded.end(); ++vi)
        {
...
const char *cmMakefile::GetProperty(const char *prop, cmProperty::ScopeType scope) const
{
    if (!prop)
    {
        return 0;
    ...
    {
        for (std::deque < cmStdString >::const_iterator i = this->ListFileStack.begin(); i != this->ListFileStack.end(); ++i)
        {
    ...
    {
        cmOStringStream str;
        for (std::vector < std::string >::const_iterator it = this->GetLinkDirectories().begin(); it != this->GetLinkDirectories().end(); ++it)
        {
    ...
    {
        std::string sep;
        for (std::vector < cmValueWithOrigin >::const_iterator it = this->IncludeDirectoriesEntries.begin(), end = this->IncludeDirectoriesEntries.end(); it != end; ++it)
        {
            output += sep;
            output += it->Value;
            sep = ";";
    ...
    {
        std::string sep;
        for (std::vector < cmValueWithOrigin >::const_iterator it = this->CompileOptionsEntries.begin(), end = this->CompileOptionsEntries.end(); it != end; ++it)
        {
            output += sep;
            output += it->Value;
            sep = ";";
    ...
    {
        std::string sep;
        for (std::vector < cmValueWithOrigin >::const_iterator it = this->CompileDefinitionsEntries.begin(), end = this->CompileDefinitionsEntries.end(); it != end; ++it)
        {
            output += sep;
            output += it->Value;
            sep = ";";
    ...
    bool chain = false;
    const char *retVal = this->Properties.GetPropertyValue(prop, scope, chain);
    if (chain)
    {
        if (this->LocalGenerator->GetParent())
        {
            return this->LocalGenerator->GetParent()->GetMakefile()->GetProperty(prop, scope);
        }
...
cmTarget *cmMakefile::FindTarget(const std::string & name, bool excludeAliases) const
{
    if (!excludeAliases)
    {
        std::map < std::string, cmTarget * >::const_iterator i = this->AliasTargets.find(name);
...
cmTest *cmMakefile::GetTest(const char *testName) const
{
    if (testName)
    {
        std::map < cmStdString, cmTest * >::const_iterator mi = this->Tests.find(testName);
...
void cmMakefile::AddCMakeDependFilesFromUser()
{
    ...
    for (std::vector < std::string >::iterator i = deps.begin(); i != deps.end(); ++i)
    {
...
{
    cm->DefineProperty("RULE_LAUNCH_COMPILE", cmProperty::DIRECTORY, "", "", true);
    cm->DefineProperty("RULE_LAUNCH_LINK", cmProperty::DIRECTORY, "", "", true);
    cm->DefineProperty("RULE_LAUNCH_CUSTOM", cmProperty::DIRECTORY, "", "", true);
}

//----------------------------------------------------------------------------
cmTarget *cmMakefile::AddImportedTarget(const char *name, cmTarget::TargetType type, bool global)
{
    // Create the target.
...
//----------------------------------------------------------------------------
cmTarget *cmMakefile::FindTargetToUse(const std::string & name, bool excludeAliases) const
{
    // Look for an imported target.  These take priority because they
    // are more local in scope and do not have to be globally unique.
    std::map < cmStdString, cmTarget * >::const_iterator imported = this->ImportedTargets.find(name);
    ...
    return this->LocalGenerator->GetGlobalGenerator()->FindTarget(0, name.c_str(), excludeAliases);
}
...
bool cmMakefile::IsAlias(const std::string & name) const
{
    if (this->AliasTargets.find(name) != this->AliasTargets.end())
        return true;
    return this->GetLocalGenerator()->GetGlobalGenerator()->IsAlias(name.c_str());
}

//----------------------------------------------------------------------------
cmGeneratorTarget *cmMakefile::FindGeneratorTargetToUse(const char *name) const
{
...
//----------------------------------------------------------------------------
bool cmMakefile::EnforceUniqueName(std::string const &name, std::string & msg, bool isCustom) const
{
    if (this->IsAlias(name))
    {
        cmOStringStream e;
        e << "cannot create target \"" << name << "\" because an alias with the same name already exists.";
        msg = e.str();
        return false;
    }
    if (cmTarget * existing = this->FindTargetToUse(name))
    {
        ...
        if (existing->IsImported())
        {
            ...
            cmOStringStream e;
            e << "cannot create target \"" << name << "\" because an imported target with the same name already exists.";
            msg = e.str();
            return false;
        }
        else
        {
            ...
            {
                case cmPolicies::WARN:
                    this->IssueMessage(cmake::AUTHOR_WARNING, this->GetPolicies()->GetPolicyWarning(cmPolicies::CMP0002));
                case cmPolicies::OLD:
                    return true;
                case cmPolicies::REQUIRED_IF_USED:
                case cmPolicies::REQUIRED_ALWAYS:
                    this->IssueMessage(cmake::FATAL_ERROR, this->GetPolicies()->GetRequiredPolicyError(cmPolicies::CMP0002));
                    return true;
                case cmPolicies::NEW:
                    break;
            }
            ...
            if (isCustom && existing->GetType() == cmTarget::UTILITY && this != existing->GetMakefile() && cm->GetPropertyAsBool("ALLOW_DUPLICATE_CUSTOM_TARGETS"))
            {
                return true;
            }
            ...
            // problem.
            cmOStringStream e;
            e << "cannot create target \"" << name << "\" because another target with the same name already exists.  " << "The existing target is ";
            ...
            e << "created in source directory \"" << existing->GetMakefile()->GetCurrentDirectory() << "\".  " << "See documentation for policy CMP0002 for more details.";
            msg = e.str();
            return false;
        }
    }
...
//----------------------------------------------------------------------------
bool cmMakefile::EnforceUniqueDir(const char *srcPath, const char *binPath) const
{
    // Make sure the binary directory is unique.
    cmGlobalGenerator *gg = this->LocalGenerator->GetGlobalGenerator();
    ...
    switch (this->GetPolicyStatus(cmPolicies::CMP0013))
    {
        case cmPolicies::WARN:
            ...
            e << this->GetPolicies()->GetPolicyWarning(cmPolicies::CMP0013)
                ...
                << "  " << srcPath << "\n" << "which can generate conflicting build files.  " << "CMake does not support this use case but it used " << "to work accidentally and is being allowed for " << "compatibility.";
            this->IssueMessage(cmake::AUTHOR_WARNING, e.str());
        case cmPolicies::OLD:
...
//----------------------------------------------------------------------------
cmPolicies::PolicyStatus cmMakefile::GetPolicyStatus(cmPolicies::PolicyID id) const
{
    ...
    {
        if (cur == cmPolicies::REQUIRED_ALWAYS || cur == cmPolicies::REQUIRED_IF_USED)
        {
            return cur;
        }
        cmPolicies::PolicyStatus def = this->GetPolicies()->GetPolicyStatus(id);
        if (def == cmPolicies::REQUIRED_ALWAYS || def == cmPolicies::REQUIRED_IF_USED)
        {
            return def;
        }
    }
...
//----------------------------------------------------------------------------
cmPolicies::PolicyStatus cmMakefile::GetPolicyStatusInternal(cmPolicies::PolicyID id) const
{
    // Is the policy set in our stack?
    for (PolicyStackType::const_reverse_iterator psi = this->PolicyStack.rbegin(); psi != this->PolicyStack.rend(); ++psi)
    {
...
//----------------------------------------------------------------------------
bool cmMakefile::SetPolicy(cmPolicies::PolicyID id, cmPolicies::PolicyStatus status)
{
    // A REQUIRED_ALWAYS policy may be set only to NEW.
    if (status != cmPolicies::NEW && this->GetPolicies()->GetPolicyStatus(id) == cmPolicies::REQUIRED_ALWAYS)
    {
        std::string msg = this->GetPolicies()->GetRequiredAlwaysPolicyError(id);
    ...
    for (PolicyStackType::reverse_iterator psi = this->PolicyStack.rbegin(); previous_was_weak && psi != this->PolicyStack.rend(); ++psi)
    {
        (*psi)[id] = status;
    ...
    if (id == cmPolicies::CMP0001 && (status == cmPolicies::WARN || status == cmPolicies::OLD))
    {
        if (!(this->GetCacheManager()->GetCacheValue("CMAKE_BACKWARDS_COMPATIBILITY")))
        {
            // Set it to 2.4 because that is the last version where the
            // variable had meaning.
            this->AddCacheDefinition("CMAKE_BACKWARDS_COMPATIBILITY", "2.4", "For backwards compatibility, what version of CMake " "commands and " "syntax should this version of CMake try to support.", cmCacheManager::STRING);
        }
    }
...
//----------------------------------------------------------------------------
cmMakefile::PolicyPushPop::PolicyPushPop(cmMakefile * m, bool weak, cmPolicies::PolicyMap const &pm):
...
void cmMakefile::PopPolicy()
{
    ...
    {
        this->IssueMessage(cmake::FATAL_ERROR, "cmake_policy POP without matching PUSH");
    }
}
...
{
    return this->GetCMakeInstance()->GetPolicies()->ApplyPolicyVersion(this, version);
}
...
void cmMakefile::RecordPolicies(cmPolicies::PolicyMap & pm)
{
    ...
    for (PolicyID pid = cmPolicies::CMP0000; pid != cmPolicies::CMPCOUNT; pid = PolicyID(pid + 1))
    {
