...
//----------------------------------------------------------------------------
cmRST::cmRST(std::ostream & os, std::string const &docroot):OS(os),
DocRoot(docroot),
IncludeDepth(0),
...
CMakeRole("(:cmake)?:("
          ...
          "prop_cache|prop_dir|prop_gbl|prop_sf|prop_test|prop_tgt|" "manual" "):`(<*([^`<]|[^` \t]<)*)([ \t]+<[^`]*>)?`"), Substitution("(^|[^A-Za-z0-9_])" "((\\|[^| \t\r\n]([^|\r\n]*[^| \t\r\n])?\\|)(__|_|))" "([^A-Za-z0-9_]|$)")
{
...
void cmRST::ProcessLine(std::string const &line)
{
    ...
    if (line == ".." || (line.size() >= 3 && line[0] == '.' && line[1] == '.' && isspace(line[2])))
    {
        this->Reset();
        this->Markup = (line.find_first_not_of(" \t", 2) == line.npos ? MarkupEmpty : MarkupNormal);
    ...
    {
        this->NormalLine(line);
        this->LastLineEndedInColonColon = (line.size() >= 2 && line[line.size() - 2] == ':' && line[line.size() - 1] == ':');
    }
}
...
void cmRST::OutputLine(std::string const &line_in, bool inlineMarkup)
{
    ...
    if (inlineMarkup)
    {
        ...
        while (this->CMakeRole.find(line.c_str() + pos))
        {
            ...
            if (this->CMakeRole.match(2) == "command" && this->CMakeRole.match(5).empty() && text.find_first_of("()") == text.npos)
            {
                text += "()";
            }
            this->OS << "``" << text << "``";
...
std::string cmRST::ReplaceSubstitutions(std::string const &line)
{
    ...
    while (this->Substitution.find(line.c_str() + pos))
    {
        ...
        std::map < cmStdString, cmStdString >::iterator replace = this->Replace.find(substitute);
        if (replace != this->Replace.end())
        {
            std::pair < std::set < cmStdString >::iterator, bool > replaced = this->Replaced.insert(substitute);
            if (replaced.second)
            {
...
{
    for (std::vector < std::string >::iterator i = this->MarkupLines.begin(); i != this->MarkupLines.end(); ++i)
    {
        std::string line = *i;
        if (!line.empty())
        {
