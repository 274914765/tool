...
public:
    virtual cmGlobalGenerator * CreateGlobalGenerator(const char *name) const
    {
        ...
        if (strcmp(p, "") == 0)
        {
            return new cmGlobalVisualStudio10Generator(name, NULL, NULL);
        }
        if (strcmp(p, " Win64") == 0)
        {
            return new cmGlobalVisualStudio10Generator(name, "x64", "CMAKE_FORCE_WIN64");
        }
        if (strcmp(p, " IA64") == 0)
        {
            return new cmGlobalVisualStudio10Generator(name, "Itanium", "CMAKE_FORCE_IA64");
        }
        return 0;
    }
...
//----------------------------------------------------------------------------
cmGlobalVisualStudio10Generator::cmGlobalVisualStudio10Generator(const char *name, const char *platformName, const char *additionalPlatformDefinition):cmGlobalVisualStudio8Generator(name, platformName, additionalPlatformDefinition)
{
    std::string vc10Express;
    this->ExpressEdition = cmSystemTools::ReadRegistryValue("HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\VCExpress\\10.0\\Setup\\VC;" "ProductDir", vc10Express, cmSystemTools::KeyWOW64_32);
    this->MasmEnabled = false;
    this->MSBuildCommandInitialized = false;
}

//----------------------------------------------------------------------------
bool cmGlobalVisualStudio10Generator::MatchesGeneratorName(const char *name) const
{
    std::string genName;
...
//----------------------------------------------------------------------------
bool cmGlobalVisualStudio10Generator::SetGeneratorToolset(std::string const &ts)
{
...
void cmGlobalVisualStudio10Generator::AddPlatformDefinitions(cmMakefile * mf)
{
    ...
    {
        mf->AddDefinition("CMAKE_VS_PLATFORM_TOOLSET", this->PlatformToolset.c_str());
    }
}
...
{
    cmLocalVisualStudio10Generator *lg = new cmLocalVisualStudio10Generator(cmLocalVisualStudioGenerator::VS10);
...
void cmGlobalVisualStudio10Generator::Generate()
{
    ...
    if (this->LongestSource.Length > 0)
    {
        ...
        cmOStringStream e;
        e << "The binary and/or source directory paths may be too long to generate "
            ...
            "the path length is too long for some internal buffer or API.  " "To avoid this problem CMake must use a full path for this file " "which then triggers the VS 10 property dialog bug.";
        mf->IssueMessage(cmake::WARNING, e.str().c_str());
    }
}

//----------------------------------------------------------------------------
void cmGlobalVisualStudio10Generator::EnableLanguage(std::vector < std::string > const &lang, cmMakefile * mf, bool optional)
{
    ...
    for (std::vector < std::string >::const_iterator it = lang.begin(); it != lang.end(); ++it)
    {
        if (*it == "ASM_MASM")
        {
...
std::string cmGlobalVisualStudio10Generator::GetUserMacrosDirectory()
{
    ...
    if (cmSystemTools::ReadRegistryValue("HKEY_CURRENT_USER\\Software\\Microsoft\\VisualStudio\\10.0;" "VisualStudioProjectsLocation", base))
    {
...
void cmGlobalVisualStudio10Generator::FindMakeProgram(cmMakefile * mf)
{
    ...
    mf->AddDefinition("CMAKE_VS_MSBUILD_COMMAND", this->GetMSBuildCommand().c_str());
}
...
{
    std::string msbuild;
    std::string mskey = "HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\MSBuild\\ToolsVersions\\";
    mskey += this->GetToolsVersion();
    mskey += ";MSBuildToolsPath";
    if (cmSystemTools::ReadRegistryValue(mskey.c_str(), msbuild, cmSystemTools::KeyWOW64_32))
    {
...
//----------------------------------------------------------------------------
void cmGlobalVisualStudio10Generator::GenerateBuildCommand(std::vector < std::string > &makeCommand,
                                                           const char *makeProgram, const char *projectName, const char *projectDir, const char *targetName, const char *config, bool fast, std::vector < std::string > const &makeOptions)
{
    // Select the caller- or user-preferred make program, else MSBuild.
    std::string makeProgramSelected = this->SelectMakeProgram(makeProgram, this->GetMSBuildCommand());
    ...
    bool useDevEnv = (makeProgramLower.find("devenv") != std::string::npos || makeProgramLower.find("vcexpress") != std::string::npos);
    ...
    cmSlnData slnData;
    {
        ...
        if (parser.ParseFile(slnFile, slnData, cmVisualStudioSlnParser::DataGroupProjects))
        {
            std::vector < cmSlnProjectEntry > slnProjects = slnData.GetProjects();
            for (std::vector < cmSlnProjectEntry >::iterator i = slnProjects.begin(); !useDevEnv && i != slnProjects.end(); ++i)
            {
    ...
    if (useDevEnv)
    {
        ...
        cmGlobalVisualStudio7Generator::GenerateBuildCommand(makeCommand, makeProgram, projectName, projectDir, targetName, config, fast, makeOptions);
        return;
    }
    ...
    else
    {
        ...
        if (targetProject.find('/') == std::string::npos)
        {
            ...
            if (cmSlnProjectEntry const *proj = slnData.GetProjectByName(targetName))
            {
    ...
    makeCommand.push_back(std::string("/p:VisualStudioVersion=") + this->GetIDEVersion());
...
bool cmGlobalVisualStudio10Generator::Find64BitTools(cmMakefile * mf)
{
    ...
    if (cmSystemTools::ReadRegistryValue("HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SDKs\\" "Windows\\v7.1;InstallationFolder", winSDK_7_1))
    {
        cmOStringStream m;
...
//----------------------------------------------------------------------------
std::string cmGlobalVisualStudio10Generator::GenerateRuleFile(std::string const &output) const
{
    ...
    ruleDir += "/";
    ruleDir += cmSystemTools::ComputeStringMD5(cmSystemTools::GetFilenamePath(output).c_str());
...
//----------------------------------------------------------------------------
void cmGlobalVisualStudio10Generator::PathTooLong(cmTarget * target, cmSourceFile * sf, std::string const &sfRel)
{
    size_t len = (strlen(target->GetMakefile()->GetCurrentOutputDirectory()) + 1 + sfRel.length());
