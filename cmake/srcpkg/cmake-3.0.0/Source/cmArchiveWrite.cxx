...
public:
    ...
    ~Entry()
    {
        archive_entry_free(this->Object);
    }
    operator  struct archive_entry *()
    {
        return this->Object;
    }
};
...
struct cmArchiveWrite::Callback
{
    ...
    static __LA_SSIZE_T Write(struct archive *, void *cd, const void *b, size_t n)
    {
        ...
        if (self->Stream.write(static_cast < const char *>(b), static_cast < cmsys_ios::streamsize > (n)))
        {
...
Stream(os), Archive(archive_write_new()), Disk(archive_read_disk_new()), Verbose(false)
{
    switch (c)
    {
        case CompressNone:
    ...
    if (archive_write_open(this->Archive, this, 0, reinterpret_cast < archive_write_callback * >(&Callback::Write), 0) != ARCHIVE_OK)
    {
...
bool cmArchiveWrite::AddPath(const char *path, size_t skip, const char *prefix)
{
    ...
    if (!cmSystemTools::FileIsDirectory(path) || cmSystemTools::FileIsSymlink(path))
    {
        return true;
    }
    cmsys::Directory d;
