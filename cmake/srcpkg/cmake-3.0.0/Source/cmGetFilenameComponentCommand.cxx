...
bool cmGetFilenameComponentCommand::InitialPass(std::vector < std::string > const &args, cmExecutionStatus &)
{
    if (args.size() < 3)
    {
    ...
    else if (args[2] == "ABSOLUTE" || args[2] == "REALPATH")
    {
        ...
        result = cmSystemTools::CollapseFullPath(filename.c_str(), this->Makefile->GetCurrentDirectory());
        if (args[2] == "REALPATH")
        {
    ...
    if (args.size() == 4 && args[3] == "CACHE")
    {
        ...
        {
            this->Makefile->AddCacheDefinition(storeArgs.c_str(), programArgs.c_str(), "", args[2] == "PATH" ? cmCacheManager::FILEPATH : cmCacheManager::STRING);
        }
        this->Makefile->AddCacheDefinition(args[0].c_str(), result.c_str(), "", args[2] == "PATH" ? cmCacheManager::FILEPATH : cmCacheManager::STRING);
    }
    else
    {
        if (programArgs.size() && storeArgs.size())
