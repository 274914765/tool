...
//----------------------------------------------------------------------------
void cmExtraKateGenerator::GetDocumentation(cmDocumentationEntry & entry, const char *) const
{
    entry.Name = this->GetName();
    entry.Brief = "Generates Kate project files.";
}

cmExtraKateGenerator::cmExtraKateGenerator():cmExternalMakefileProjectGenerator()
{
#if defined(_WIN32)
...
void cmExtraKateGenerator::Generate()
{
    ...
    this->ProjectName = this->GenerateProjectName(mf->GetProjectName(), mf->GetSafeDefinition("CMAKE_BUILD_TYPE"), this->GetPathBasename(mf->GetHomeOutputDirectory()));
...
void cmExtraKateGenerator::WriteTargets(const cmMakefile * mf, cmGeneratedFileStream & fout) const
{
    const std::string make = mf->GetRequiredDefinition("CMAKE_MAKE_PROGRAM");
    const std::string makeArgs = mf->GetSafeDefinition("CMAKE_KATE_MAKE_ARGUMENTS");
    ...
    this->AppendTarget(fout, "clean", make, makeArgs, homeOutputDir, homeOutputDir);
                ...
                case cmTarget::GLOBAL_TARGET:
                    {
                        ...
                        if (topLevel)
                        {
                            ...
                            {
                                const char *editCommand = makefile->GetDefinition("CMAKE_EDIT_COMMAND");
                                if (editCommand == 0)
                                {
                                    insertTarget = false;
                                }
                        ...
                        if (insertTarget)
                        {
                            this->AppendTarget(fout, ti->first, make, makeArgs, currentDir, homeOutputDir);
                        }
                    }
                    break;
                ...
                case cmTarget::OBJECT_LIBRARY:
                    {
                        ...
                        fastTarget += "/fast";
                        this->AppendTarget(fout, fastTarget, make, makeArgs, currentDir, homeOutputDir);
...
void cmExtraKateGenerator::AppendTarget(cmGeneratedFileStream & fout, const std::string & target, const std::string & make, const std::string & makeArgs, const std::string & path, const char *homeOutputDir) const
{
    static char JsonSep = ' ';
...
void cmExtraKateGenerator::CreateDummyKateProjectFile(const cmMakefile * mf) const
{
    std::string filename = mf->GetHomeOutputDirectory();
    filename += "/";
...
std::string cmExtraKateGenerator::GenerateFilesString(const cmMakefile * mf) const
{
    ...
    std::string tmp;
    const std::vector < cmLocalGenerator * >&lgs = this->GlobalGenerator->GetLocalGenerators();
...
std::string cmExtraKateGenerator::GenerateProjectName(const std::string & name, const std::string & type, const std::string & path) const
{
