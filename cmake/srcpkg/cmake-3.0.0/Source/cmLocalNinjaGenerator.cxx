...
cmLocalNinjaGenerator::cmLocalNinjaGenerator():cmLocalGenerator(), ConfigName(""), HomeRelativeOutputPath("")
{
#ifdef _WIN32
...
void cmLocalNinjaGenerator::Generate()
{
    ...
    if (this->isRootMakefile())
    {
        ...
        const std::string showIncludesPrefix = this->GetMakefile()->GetSafeDefinition("CMAKE_CL_SHOWINCLUDES_PREFIX");
        if (!showIncludesPrefix.empty())
        {
            cmGlobalNinjaGenerator::WriteComment(this->GetRulesFileStream(), "localized /showIncludes string");
    ...
    for (cmGeneratorTargetsType::iterator t = targets.begin(); t != targets.end(); ++t)
    {
        ...
        if (tg)
        {
            ...
            if (!this->GetGlobalNinjaGenerator()->IsExcluded(this->GetGlobalNinjaGenerator()->GetLocalGenerators()[0], *t->second->Target))
...
const cmGlobalNinjaGenerator *cmLocalNinjaGenerator::GetGlobalNinjaGenerator() const
{
...
std::string cmLocalNinjaGenerator::ConvertToIncludeReference(std::string const &path)
{
...
{
    return (strcmp(this->Makefile->GetCurrentDirectory(), this->GetCMakeInstance()->GetHomeDirectory()) == 0);
}
...
void cmLocalNinjaGenerator::WriteProjectHeader(std::ostream & os)
{
    ...
    os << "# Project: " << this->GetMakefile()->GetProjectName() << std::endl << "# Configuration: " << this->ConfigName << std::endl;
    cmGlobalNinjaGenerator::WriteDivider(os);
}
...
void cmLocalNinjaGenerator::WritePools(std::ostream & os)
{
    ...
    const char *jobpools = this->GetCMakeInstance()->GetProperty("JOB_POOLS", cmProperty::GLOBAL);
    if (jobpools)
    {
        cmGlobalNinjaGenerator::WriteComment(os, "Pools defined by global property JOB_POOLS");
        ...
        for (size_t i = 0; i < pools.size(); ++i)
        {
            ...
            unsigned int jobs;
            if (eq != std::string::npos && sscanf(pool.c_str() + eq, "=%u", &jobs) == 1)
            {
            ...
            {
                cmSystemTools::Error("Invalid pool defined by property 'JOB_POOLS': ", pool.c_str());
            }
        }
    }
}
...
void cmLocalNinjaGenerator::WriteNinjaFilesInclusion(std::ostream & os)
{
    ...
    cmGlobalNinjaGenerator::WriteInclude(os, cmGlobalNinjaGenerator::NINJA_RULES_FILE, "Include rules file.");
    os << "\n";
}
...
{
    cmGlobalNinjaGenerator::WriteDivider(os);
    os << "# Write statements declared in CMakeLists.txt:" << std::endl << "# " << this->Makefile->GetCurrentListFile() << std::endl;
    if (this->isRootMakefile())
...
void cmLocalNinjaGenerator::AppendTargetOutputs(cmTarget * target, cmNinjaDeps & outputs)
{
    this->GetGlobalNinjaGenerator()->AppendTargetOutputs(target, outputs);
}

void cmLocalNinjaGenerator::AppendTargetDepends(cmTarget * target, cmNinjaDeps & outputs)
{
...
std::string cmLocalNinjaGenerator::BuildCommandLine(const std::vector < std::string > &cmdLines)
{
    ...
    cmOStringStream cmd;
    for (std::vector < std::string >::const_iterator li = cmdLines.begin(); li != cmdLines.end(); ++li)
#ifdef _WIN32
    {
...
void cmLocalNinjaGenerator::AppendCustomCommandLines(const cmCustomCommand * cc, std::vector < std::string > &cmdLines)
{
    ...
    {
        cmdLines.push_back(launcher + this->ConvertToOutputFormat(ccg.GetCommand(i).c_str(), SHELL));
...
void cmLocalNinjaGenerator::WriteCustomCommandBuildStatement(cmCustomCommand const *cc, const cmNinjaDeps & orderOnlyDeps)
{
    ...
    std::transform(outputs.begin(), outputs.end(), ninjaOutputs.begin(), MapToNinjaPath());
    this->AppendCustomCommandDeps(cc, ninjaDeps);

    for (cmNinjaDeps::iterator i = ninjaOutputs.begin(); i != ninjaOutputs.end(); ++i)
    ...
    if (cmdLines.empty())
    {
        this->GetGlobalNinjaGenerator()->WritePhonyBuild(this->GetBuildFileStream(), "Phony custom command for " + ninjaOutputs[0], ninjaOutputs, ninjaDeps, cmNinjaDeps(), orderOnlyDeps, cmNinjaVars());
    }
    else
    {
        this->GetGlobalNinjaGenerator()->WriteCustomCommandBuild(this->BuildCommandLine(cmdLines), this->ConstructComment(*cc), "Custom command for " + ninjaOutputs[0], ninjaOutputs, ninjaDeps, orderOnlyDeps);
    }
}

void cmLocalNinjaGenerator::AddCustomCommandTarget(cmCustomCommand const *cc, cmTarget * target)
{
...
{
    for (CustomCommandTargetMap::iterator i = this->CustomCommandTargets.begin(); i != this->CustomCommandTargets.end(); ++i)
    {
        ...
        for (; j != i->second.end(); ++j)
        {
            ...
            std::set_intersection(ccTargetDeps.begin(), ccTargetDeps.end(), jDeps.begin(), jDeps.end(), std::back_inserter(depsIntersection));
...
std::string cmLocalNinjaGenerator::MakeCustomLauncher(const cmCustomCommand & cc)
{
    ...
    if (!outputs.empty())
    {
        RelativeRoot relative_root = cc.GetWorkingDirectory()? NONE : START_OUTPUT;
